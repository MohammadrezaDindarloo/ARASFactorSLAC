// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl9
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl9(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 503

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (155)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -_DeltaRot[0] * _Rot_init[0] - _DeltaRot[1] * _Rot_init[1] -
                        _DeltaRot[2] * _Rot_init[2] + _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = 2 * _tmp1;
  const Scalar _tmp12 = _tmp10 * _tmp11;
  const Scalar _tmp13 = -_tmp12 + _tmp9;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp1 * _tmp8;
  const Scalar _tmp16 = 2 * _tmp10 * _tmp3;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = _tmp11 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp8;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp29 = _tmp26 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp35 = -Scalar(0.010999999999999999) * _tmp2 -
                        Scalar(0.010999999999999999) * _tmp27 + Scalar(-0.010999999999999999);
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp37 = _tmp35 - _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = _tmp21 + _tmp29;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp18 + _tmp5;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(-110.0);
  const Scalar _tmp44 = _tmp40 + Scalar(-125.0);
  const Scalar _tmp45 =
      std::sqrt(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp44, Scalar(2))));
  const Scalar _tmp46 = Scalar(1.0) / (_tmp45);
  const Scalar _tmp47 = Scalar(1.0) / (_tmp44);
  const Scalar _tmp48 = _tmp45 * _tmp47;
  const Scalar _tmp49 = _tmp48 * (_tmp39 * _tmp43 * _tmp46 - _tmp41 * _tmp44 * _tmp46);
  const Scalar _tmp50 = _tmp43 * _tmp47;
  const Scalar _tmp51 = _tmp14 - _tmp17;
  const Scalar _tmp52 = _tmp5 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(-110.0);
  const Scalar _tmp55 = _tmp26 - _tmp28;
  const Scalar _tmp56 = _tmp21 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(0, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(125.0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp58 * _tmp59;
  const Scalar _tmp62 = Scalar(1.0) / (_tmp50 * _tmp61 - _tmp60);
  const Scalar _tmp63 = -_tmp34;
  const Scalar _tmp64 = _tmp35 + _tmp36;
  const Scalar _tmp65 = _tmp63 + _tmp64;
  const Scalar _tmp66 = _tmp34 + _tmp64;
  const Scalar _tmp67 = _tmp61 * _tmp66;
  const Scalar _tmp68 = _tmp62 * (-_tmp61 * _tmp65 + _tmp67);
  const Scalar _tmp69 = _tmp62 * (-_tmp50 * _tmp67 + _tmp60 * _tmp65);
  const Scalar _tmp70 = _tmp50 * _tmp66 + _tmp50 * _tmp69;
  const Scalar _tmp71 = Scalar(1.0) * _tmp41;
  const Scalar _tmp72 = -_tmp71;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp52 + _tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp39;
  const Scalar _tmp75 = -_tmp56 + _tmp74;
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp50 * _tmp68 - _tmp66 - _tmp70 * _tmp76;
  const Scalar _tmp78 = _tmp37 + _tmp63;
  const Scalar _tmp79 = _tmp51 + _tmp6;
  const Scalar _tmp80 = _tmp79 + position_vector(1, 0);
  const Scalar _tmp81 = _tmp80 + Scalar(110.0);
  const Scalar _tmp82 = _tmp22 + _tmp55;
  const Scalar _tmp83 = _tmp82 + position_vector(0, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(125.0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp81 * _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp85;
  const Scalar _tmp88 = _tmp50 * _tmp87 - _tmp86;
  const Scalar _tmp89 = _tmp66 * _tmp87;
  const Scalar _tmp90 = -_tmp50 * _tmp89 - _tmp69 * _tmp88 + _tmp78 * _tmp86;
  const Scalar _tmp91 = -_tmp68 * _tmp88 - _tmp76 * _tmp90 - _tmp78 * _tmp87 + _tmp89;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp62 * (_tmp49 * _tmp61 + _tmp52 * _tmp61 - _tmp56 * _tmp60);
  const Scalar _tmp94 = _tmp49 * _tmp87 + _tmp79 * _tmp87 - _tmp82 * _tmp86 - _tmp88 * _tmp93;
  const Scalar _tmp95 = _tmp92 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp97 = _tmp91 * _tmp96;
  const Scalar _tmp98 = _tmp97 * (-_tmp49 + _tmp50 * _tmp93 - _tmp77 * _tmp95);
  const Scalar _tmp99 = _tmp72 + _tmp79;
  const Scalar _tmp100 = _tmp76 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (-_tmp100 + _tmp74 - _tmp82);
  const Scalar _tmp102 = _tmp101 * _tmp99;
  const Scalar _tmp103 = _tmp77 + _tmp98;
  const Scalar _tmp104 = _tmp90 * _tmp92;
  const Scalar _tmp105 = _tmp102 * _tmp98 - _tmp103 * _tmp104 + _tmp70;
  const Scalar _tmp106 = Scalar(1.0) * _tmp73;
  const Scalar _tmp107 = Scalar(1.0) * _tmp101;
  const Scalar _tmp108 = _tmp31 + Scalar(-125.0);
  const Scalar _tmp109 = _tmp20 + Scalar(110.0);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp108 * _tmp110;
  const Scalar _tmp112 = _tmp111 * fh1;
  const Scalar _tmp113 = _tmp106 * _tmp69 * _tmp75 - Scalar(1.0) * _tmp68;
  const Scalar _tmp114 = _tmp97 * (-_tmp113 * _tmp95 - Scalar(1.0) * _tmp93);
  const Scalar _tmp115 = _tmp113 + _tmp114;
  const Scalar _tmp116 = _tmp102 * _tmp114 - _tmp104 * _tmp115 - Scalar(1.0) * _tmp69;
  const Scalar _tmp117 = _tmp109 * _tmp110;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = _tmp73 * _tmp99;
  const Scalar _tmp120 = _tmp38 * fh1;
  const Scalar _tmp121 = _tmp111 * _tmp120 + Scalar(2.3544) * _tmp25 + _tmp30 * fv1;
  const Scalar _tmp122 = -_tmp117 * _tmp120 - Scalar(2.3544) * _tmp13 - _tmp19 * fv1;
  const Scalar _tmp123 = _tmp100 * _tmp107 + Scalar(1.0);
  const Scalar _tmp124 = _tmp107 * _tmp76;
  const Scalar _tmp125 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp126 = _tmp71 * _tmp76 + _tmp74;
  const Scalar _tmp127 = _tmp101 * _tmp126;
  const Scalar _tmp128 = 0;
  const Scalar _tmp129 = _tmp73 * (-_tmp104 * _tmp128 - _tmp127 * _tmp99 + _tmp72);
  const Scalar _tmp130 = _tmp107 * _tmp97;
  const Scalar _tmp131 = Scalar(1.0) * _tmp96;
  const Scalar _tmp132 = _tmp130 * _tmp99 - _tmp131 * _tmp90;
  const Scalar _tmp133 = fh1 * (-_tmp111 * _tmp19 + _tmp117 * _tmp30);
  const Scalar _tmp134 = _tmp128 * _tmp92;
  const Scalar _tmp135 = _tmp61 * _tmp62;
  const Scalar _tmp136 = _tmp88 * _tmp92;
  const Scalar _tmp137 = _tmp128 * _tmp136;
  const Scalar _tmp138 = _tmp87 * _tmp92;
  const Scalar _tmp139 = -_tmp115 * _tmp136 + Scalar(1.0);
  const Scalar _tmp140 = -_tmp103 * _tmp136 - _tmp50;
  const Scalar _tmp141 = -_tmp112 * _tmp48 * (_tmp103 * _tmp138 + _tmp135 * _tmp140 + Scalar(1.0)) -
                         _tmp118 * _tmp48 * (_tmp115 * _tmp138 + _tmp135 * _tmp139) -
                         _tmp125 * _tmp48 * (_tmp134 * _tmp87 - _tmp135 * _tmp137) -
                         _tmp133 * _tmp48 * (-_tmp131 * _tmp135 * _tmp88 + _tmp131 * _tmp87);
  const Scalar _tmp142 = Scalar(1.0) / (_tmp141);
  const Scalar _tmp143 =
      std::asinh(_tmp142 * (Scalar(1.0) * _tmp112 * (-_tmp105 * _tmp106 + _tmp107 * _tmp98) +
                            Scalar(1.0) * _tmp118 * (-_tmp106 * _tmp116 + _tmp107 * _tmp114) +
                            Scalar(1.0) * _tmp121 * (_tmp107 * _tmp119 - _tmp107) +
                            Scalar(1.0) * _tmp122 * (-_tmp106 * _tmp123 + _tmp124) +
                            Scalar(1.0) * _tmp125 *
                                (-_tmp107 * _tmp126 - Scalar(1.0) * _tmp129 + Scalar(1.0)) +
                            Scalar(1.0) * _tmp133 * (-_tmp106 * _tmp132 + _tmp130)));
  const Scalar _tmp144 = Scalar(9.6622558468725703) * _tmp141;
  const Scalar _tmp145 = _tmp131 * _tmp133;
  const Scalar _tmp146 = _tmp112 * _tmp140 * _tmp62 + _tmp118 * _tmp139 * _tmp62 -
                         _tmp125 * _tmp137 * _tmp62 - _tmp145 * _tmp62 * _tmp88;
  const Scalar _tmp147 = Scalar(1.0) / (_tmp146);
  const Scalar _tmp148 = _tmp107 * _tmp121;
  const Scalar _tmp149 = std::asinh(
      _tmp147 * (_tmp105 * _tmp112 * _tmp73 + _tmp116 * _tmp118 * _tmp73 - _tmp119 * _tmp148 +
                 _tmp122 * _tmp123 * _tmp73 + _tmp125 * _tmp129 + _tmp132 * _tmp133 * _tmp73));
  const Scalar _tmp150 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp151 =
      _tmp103 * _tmp112 * _tmp92 + _tmp115 * _tmp118 * _tmp92 + _tmp125 * _tmp134 + _tmp145;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 =
      std::asinh(_tmp152 * (-_tmp101 * _tmp112 * _tmp98 - _tmp101 * _tmp114 * _tmp118 -
                            _tmp122 * _tmp124 + _tmp125 * _tmp127 - _tmp130 * _tmp133 + _tmp148));
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp151;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
          (Scalar(4.967784) * _tmp0 + std::cosh(Scalar(1.0) * _tmp32) -
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp32 * _tmp33 -
                Scalar(125.0) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp31), Scalar(2)) +
                        Scalar(0.77439999999999998) *
                            std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp20 - 1),
                                     Scalar(2))))))) +
      _tmp38 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp144 *
          (Scalar(4.967784) * _tmp142 + std::cosh(Scalar(1.0) * _tmp143) -
           std::cosh(
               Scalar(0.1034955) * _tmp142 *
               (-_tmp143 * _tmp144 -
                Scalar(125.0) *
                    std::sqrt(Scalar(
                        std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp40), Scalar(2)) +
                        Scalar(0.77439999999999998) *
                            std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp42),
                                     Scalar(2))))))) +
      _tmp66 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp150 *
          (Scalar(4.967784) * _tmp147 + std::cosh(Scalar(1.0) * _tmp149) -
           std::cosh(
               Scalar(0.1034955) * _tmp147 *
               (-_tmp149 * _tmp150 -
                Scalar(125.0) * std::sqrt(Scalar(
                                    Scalar(0.77439999999999998) *
                                        std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp53),
                                                 Scalar(2)) +
                                    std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp57 - 1),
                                             Scalar(2))))))) +
      _tmp65 + position_vector(2, 0);
  _res(3, 0) =
      -_tmp154 *
          (Scalar(4.967784) * _tmp152 + std::cosh(Scalar(1.0) * _tmp153) -
           std::cosh(Scalar(0.1034955) * _tmp152 *
                     (-_tmp153 * _tmp154 -
                      Scalar(125.0) *
                          std::sqrt(Scalar(
                              Scalar(0.77439999999999998) *
                                  std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp80 - 1),
                                           Scalar(2)) +
                              std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp83 - 1),
                                       Scalar(2))))))) +
      _tmp78 + position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
