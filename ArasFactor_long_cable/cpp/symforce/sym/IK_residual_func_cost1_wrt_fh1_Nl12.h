// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl12
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl12(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 660

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (213)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp3;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp1 * _tmp6;
  const Scalar _tmp14 = _tmp3 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp21 = _tmp1 * _tmp7;
  const Scalar _tmp22 = _tmp6 * _tmp9;
  const Scalar _tmp23 = _tmp21 - _tmp22;
  const Scalar _tmp24 = -Scalar(0.010999999999999999) * _tmp23;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(1, 0);
  const Scalar _tmp29 = _tmp0 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -Scalar(0.1034955) * _tmp31 * fh1 -
      Scalar(12.936937500000001) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp18), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp28), Scalar(2))));
  const Scalar _tmp33 = _tmp0 * _tmp32;
  const Scalar _tmp34 = Scalar(1.0) * _tmp30;
  const Scalar _tmp35 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp36 =
      std::pow(Scalar(_tmp35 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp37 = _tmp28 + Scalar(-110.0);
  const Scalar _tmp38 = _tmp18 + Scalar(-125.0);
  const Scalar _tmp39 = std::pow(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp38, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = _tmp38 * _tmp39;
  const Scalar _tmp41 = _tmp37 * _tmp39;
  const Scalar _tmp42 = _tmp17 * _tmp41 - _tmp27 * _tmp40;
  const Scalar _tmp43 = _tmp24 - _tmp25;
  const Scalar _tmp44 = _tmp20 + _tmp43;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-110.0);
  const Scalar _tmp47 = -_tmp5;
  const Scalar _tmp48 = _tmp16 + _tmp47;
  const Scalar _tmp49 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp50 = _tmp49 + Scalar(125.0);
  const Scalar _tmp51 = std::pow(Scalar(std::pow(_tmp46, Scalar(2)) + std::pow(_tmp50, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp52 = _tmp46 * _tmp51;
  const Scalar _tmp53 = -_tmp20;
  const Scalar _tmp54 = _tmp43 + _tmp53;
  const Scalar _tmp55 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(110.0);
  const Scalar _tmp57 = _tmp12 - _tmp15;
  const Scalar _tmp58 = _tmp47 + _tmp57;
  const Scalar _tmp59 = _tmp58 + position_vector(0, 0);
  const Scalar _tmp60 = _tmp59 + Scalar(125.0);
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = _tmp56 * _tmp61;
  const Scalar _tmp63 = _tmp50 * _tmp51;
  const Scalar _tmp64 = -_tmp52 + _tmp62 * _tmp63;
  const Scalar _tmp65 = _tmp26 + _tmp53;
  const Scalar _tmp66 = _tmp65 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 + Scalar(110.0);
  const Scalar _tmp68 = _tmp5 + _tmp57;
  const Scalar _tmp69 = _tmp68 + position_vector(0, 0);
  const Scalar _tmp70 = _tmp69 + Scalar(-125.0);
  const Scalar _tmp71 = std::pow(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp70, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp72 = _tmp70 * _tmp71;
  const Scalar _tmp73 =
      std::sqrt(Scalar(std::pow(_tmp56, Scalar(2)) + std::pow(_tmp60, Scalar(2))));
  const Scalar _tmp74 = Scalar(1.0) / (_tmp73);
  const Scalar _tmp75 = _tmp61 * _tmp73;
  const Scalar _tmp76 = _tmp75 * (-_tmp54 * _tmp60 * _tmp74 + _tmp56 * _tmp58 * _tmp74);
  const Scalar _tmp77 = _tmp67 * _tmp71;
  const Scalar _tmp78 = Scalar(1.0) / (_tmp62 * _tmp72 - _tmp77);
  const Scalar _tmp79 = _tmp78 * (_tmp65 * _tmp72 - _tmp68 * _tmp77 + _tmp72 * _tmp76);
  const Scalar _tmp80 = _tmp44 * _tmp63 - _tmp48 * _tmp52 + _tmp63 * _tmp76 - _tmp64 * _tmp79;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp80);
  const Scalar _tmp82 = Scalar(1.0) * _tmp78;
  const Scalar _tmp83 = Scalar(1.0) * _tmp81;
  const Scalar _tmp84 = _tmp42 * _tmp75 * (_tmp63 * _tmp83 - _tmp64 * _tmp72 * _tmp81 * _tmp82);
  const Scalar _tmp85 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp86 = -_tmp85;
  const Scalar _tmp87 = -Scalar(0.010999999999999999) * _tmp19 -
                        Scalar(0.010999999999999999) * _tmp2 + Scalar(-0.010999999999999999);
  const Scalar _tmp88 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp89 = _tmp87 + _tmp88;
  const Scalar _tmp90 = _tmp86 + _tmp89;
  const Scalar _tmp91 = _tmp87 - _tmp88;
  const Scalar _tmp92 = _tmp86 + _tmp91;
  const Scalar _tmp93 = _tmp63 * _tmp92;
  const Scalar _tmp94 = _tmp85 + _tmp91;
  const Scalar _tmp95 = _tmp72 * _tmp92;
  const Scalar _tmp96 = -_tmp62 * _tmp95 + _tmp77 * _tmp94;
  const Scalar _tmp97 = _tmp64 * _tmp78;
  const Scalar _tmp98 = _tmp52 * _tmp90 - _tmp62 * _tmp93 - _tmp96 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) * _tmp54;
  const Scalar _tmp100 = -_tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp100 + _tmp65);
  const Scalar _tmp102 = Scalar(1.0) * _tmp58;
  const Scalar _tmp103 = _tmp102 - _tmp68;
  const Scalar _tmp104 = _tmp101 * _tmp103;
  const Scalar _tmp105 = -_tmp72 * _tmp94 + _tmp95;
  const Scalar _tmp106 = -_tmp104 * _tmp98 - _tmp105 * _tmp97 - _tmp63 * _tmp90 + _tmp93;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = Scalar(1.0) * _tmp101;
  const Scalar _tmp109 = _tmp103 * _tmp108 * _tmp78 * _tmp96 - _tmp105 * _tmp82;
  const Scalar _tmp110 = _tmp107 * _tmp80;
  const Scalar _tmp111 = _tmp106 * _tmp81;
  const Scalar _tmp112 = _tmp111 * (-_tmp109 * _tmp110 - Scalar(1.0) * _tmp79);
  const Scalar _tmp113 = _tmp109 + _tmp112;
  const Scalar _tmp114 = _tmp107 * _tmp113;
  const Scalar _tmp115 = _tmp78 * (-_tmp114 * _tmp64 + Scalar(1.0));
  const Scalar _tmp116 = _tmp41 * _tmp75 * (_tmp114 * _tmp63 + _tmp115 * _tmp72);
  const Scalar _tmp117 = _tmp62 * _tmp78;
  const Scalar _tmp118 = _tmp117 * _tmp96 + _tmp62 * _tmp92;
  const Scalar _tmp119 = -_tmp104 * _tmp118 + _tmp105 * _tmp117 - _tmp92;
  const Scalar _tmp120 = _tmp111 * (-_tmp110 * _tmp119 + _tmp62 * _tmp79 - _tmp76);
  const Scalar _tmp121 = _tmp107 * (_tmp119 + _tmp120);
  const Scalar _tmp122 = _tmp78 * (-_tmp121 * _tmp64 - _tmp62);
  const Scalar _tmp123 = _tmp40 * _tmp75 * (_tmp121 * _tmp63 + _tmp122 * _tmp72 + Scalar(1.0));
  const Scalar _tmp124 = -_tmp116 - _tmp123 - _tmp84;
  const Scalar _tmp125 = _tmp85 + _tmp89;
  const Scalar _tmp126 = _tmp125 * fh1;
  const Scalar _tmp127 = Scalar(2.3544) * _tmp11 + _tmp126 * _tmp40 + _tmp17 * fv1;
  const Scalar _tmp128 = _tmp100 + _tmp44;
  const Scalar _tmp129 = _tmp104 * _tmp128;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp102 - _tmp129 - _tmp48);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp128 * _tmp131;
  const Scalar _tmp133 = Scalar(1.0) * _tmp101 * _tmp132 - Scalar(1.0) * _tmp131;
  const Scalar _tmp134 = _tmp128 * _tmp130;
  const Scalar _tmp135 = _tmp107 * _tmp98;
  const Scalar _tmp136 = _tmp112 * _tmp134 - _tmp113 * _tmp135 - _tmp82 * _tmp96;
  const Scalar _tmp137 = Scalar(1.0) * _tmp41 * (-_tmp108 * _tmp136 + _tmp112 * _tmp131);
  const Scalar _tmp138 = _tmp111 * _tmp131;
  const Scalar _tmp139 = _tmp111 * _tmp132 - _tmp83 * _tmp98;
  const Scalar _tmp140 = Scalar(1.0) * _tmp42;
  const Scalar _tmp141 = _tmp140 * (-_tmp108 * _tmp139 + _tmp138);
  const Scalar _tmp142 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp143 = _tmp102 + _tmp104 * _tmp99;
  const Scalar _tmp144 = 0;
  const Scalar _tmp145 = _tmp130 * _tmp143;
  const Scalar _tmp146 = _tmp100 - _tmp128 * _tmp145 - _tmp135 * _tmp144;
  const Scalar _tmp147 = -_tmp126 * _tmp41 - Scalar(2.3544) * _tmp23 - _tmp27 * fv1;
  const Scalar _tmp148 = _tmp104 * _tmp131;
  const Scalar _tmp149 = _tmp129 * _tmp131 + Scalar(1.0);
  const Scalar _tmp150 = -Scalar(1.0) * _tmp108 * _tmp149 + Scalar(1.0) * _tmp148;
  const Scalar _tmp151 = _tmp118 + _tmp120 * _tmp134 - _tmp121 * _tmp98;
  const Scalar _tmp152 = Scalar(1.0) * _tmp40 * (-_tmp108 * _tmp151 + _tmp120 * _tmp131);
  const Scalar _tmp153 =
      _tmp127 * _tmp133 + _tmp137 * fh1 + _tmp141 * fh1 +
      Scalar(1.0) * _tmp142 * (-_tmp108 * _tmp146 - _tmp131 * _tmp143 + Scalar(1.0)) +
      _tmp147 * _tmp150 + _tmp152 * fh1;
  const Scalar _tmp154 = _tmp107 * _tmp144;
  const Scalar _tmp155 = -_tmp116 * fh1 - _tmp123 * fh1 -
                         _tmp142 * _tmp75 * (_tmp154 * _tmp63 - _tmp154 * _tmp72 * _tmp97) -
                         _tmp84 * fh1;
  const Scalar _tmp156 = Scalar(1.0) / (_tmp155);
  const Scalar _tmp157 = std::asinh(_tmp153 * _tmp156);
  const Scalar _tmp158 = Scalar(1.0) * _tmp157;
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp157;
  const Scalar _tmp160 =
      -_tmp155 * _tmp159 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp55 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp59 - 1), Scalar(2))));
  const Scalar _tmp161 = Scalar(0.1034955) * _tmp156;
  const Scalar _tmp162 = _tmp160 * _tmp161;
  const Scalar _tmp163 = std::pow(_tmp155, Scalar(-2));
  const Scalar _tmp164 = _tmp125 * _tmp41;
  const Scalar _tmp165 = _tmp125 * _tmp40;
  const Scalar _tmp166 = _tmp124 * _tmp163;
  const Scalar _tmp167 = (-_tmp153 * _tmp166 + _tmp156 * (_tmp133 * _tmp165 + _tmp137 + _tmp141 -
                                                          _tmp150 * _tmp164 + _tmp152)) /
                         std::sqrt(Scalar(std::pow(_tmp153, Scalar(2)) * _tmp163 + 1));
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp155;
  const Scalar _tmp169 = _tmp142 * _tmp154;
  const Scalar _tmp170 = _tmp122 * _tmp40;
  const Scalar _tmp171 = _tmp140 * _tmp81;
  const Scalar _tmp172 = _tmp171 * fh1;
  const Scalar _tmp173 = _tmp115 * _tmp41;
  const Scalar _tmp174 = -_tmp169 * _tmp97 + _tmp170 * fh1 - _tmp172 * _tmp97 + _tmp173 * fh1;
  const Scalar _tmp175 = Scalar(1.0) / (_tmp174);
  const Scalar _tmp176 = _tmp101 * _tmp136 * _tmp41;
  const Scalar _tmp177 = _tmp101 * _tmp40;
  const Scalar _tmp178 = _tmp151 * _tmp177;
  const Scalar _tmp179 = _tmp101 * _tmp149;
  const Scalar _tmp180 = _tmp101 * _tmp139 * _tmp42;
  const Scalar _tmp181 = _tmp127 * _tmp131;
  const Scalar _tmp182 = -_tmp101 * _tmp128 * _tmp181 + _tmp101 * _tmp142 * _tmp146 +
                         _tmp147 * _tmp179 + _tmp176 * fh1 + _tmp178 * fh1 + _tmp180 * fh1;
  const Scalar _tmp183 = std::asinh(_tmp175 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp174;
  const Scalar _tmp185 =
      -_tmp183 * _tmp184 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp69), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp66 - 1), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp175;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp183;
  const Scalar _tmp189 = _tmp170 - _tmp171 * _tmp97 + _tmp173;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp189;
  const Scalar _tmp191 = std::pow(_tmp174, Scalar(-2));
  const Scalar _tmp192 = _tmp189 * _tmp191;
  const Scalar _tmp193 =
      (_tmp175 * (-_tmp125 * _tmp132 * _tmp177 - _tmp164 * _tmp179 + _tmp176 + _tmp178 + _tmp180) -
       _tmp182 * _tmp192) /
      std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp191 + 1));
  const Scalar _tmp194 = _tmp138 * _tmp42;
  const Scalar _tmp195 = _tmp112 * _tmp130 * _tmp41;
  const Scalar _tmp196 = _tmp120 * _tmp130 * _tmp40;
  const Scalar _tmp197 = _tmp142 * _tmp145 - _tmp147 * _tmp148 + _tmp181 - _tmp194 * fh1 -
                         _tmp195 * fh1 - _tmp196 * fh1;
  const Scalar _tmp198 = _tmp121 * _tmp40;
  const Scalar _tmp199 = _tmp114 * _tmp41;
  const Scalar _tmp200 = _tmp169 + _tmp172 + _tmp198 * fh1 + _tmp199 * fh1;
  const Scalar _tmp201 = Scalar(1.0) / (_tmp200);
  const Scalar _tmp202 = std::asinh(_tmp197 * _tmp201);
  const Scalar _tmp203 = Scalar(1.0) * _tmp202;
  const Scalar _tmp204 = std::pow(_tmp200, Scalar(-2));
  const Scalar _tmp205 = _tmp171 + _tmp198 + _tmp199;
  const Scalar _tmp206 = _tmp204 * _tmp205;
  const Scalar _tmp207 = (-_tmp197 * _tmp206 + _tmp201 * (_tmp131 * _tmp165 + _tmp148 * _tmp164 -
                                                          _tmp194 - _tmp195 - _tmp196)) /
                         std::sqrt(Scalar(std::pow(_tmp197, Scalar(2)) * _tmp204 + 1));
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp200;
  const Scalar _tmp209 =
      -_tmp202 * _tmp208 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp45), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp49 - 1), Scalar(2))));
  const Scalar _tmp210 = Scalar(0.1034955) * _tmp201;
  const Scalar _tmp211 = _tmp209 * _tmp210;
  const Scalar _tmp212 = Scalar(9.6622558468725703) * _tmp205;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(48.000000000000007) * _tmp0 -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp35 * _tmp36 * fv1 * std::sinh(_tmp34) - Scalar(4.967784) * _tmp35 -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp29 * _tmp36 - _tmp31) -
            _tmp32 * _tmp35) *
               std::sinh(_tmp33)) +
      Scalar(9.6622558468725703) * std::cosh(_tmp33) -
      Scalar(9.6622558468725703) * std::cosh(_tmp34);
  _res(1, 0) = -Scalar(9.6622558468725703) * _tmp124 *
                   (Scalar(4.967784) * _tmp156 + std::cosh(_tmp158) - std::cosh(_tmp162)) -
               _tmp168 * (-Scalar(4.967784) * _tmp166 + Scalar(1.0) * _tmp167 * std::sinh(_tmp158) -
                          (-Scalar(0.1034955) * _tmp160 * _tmp166 +
                           _tmp161 * (-_tmp124 * _tmp159 - _tmp167 * _tmp168)) *
                              std::sinh(_tmp162));
  _res(2, 0) =
      -_tmp184 * (-Scalar(4.967784) * _tmp192 + Scalar(1.0) * _tmp193 * std::sinh(_tmp188) -
                  (-Scalar(0.1034955) * _tmp185 * _tmp192 +
                   _tmp186 * (-_tmp183 * _tmp190 - _tmp184 * _tmp193)) *
                      std::sinh(_tmp187)) -
      _tmp190 * (Scalar(4.967784) * _tmp175 - std::cosh(_tmp187) + std::cosh(_tmp188));
  _res(3, 0) =
      -_tmp208 * (-Scalar(4.967784) * _tmp206 + Scalar(1.0) * _tmp207 * std::sinh(_tmp203) -
                  (-Scalar(0.1034955) * _tmp206 * _tmp209 +
                   _tmp210 * (-_tmp202 * _tmp212 - _tmp207 * _tmp208)) *
                      std::sinh(_tmp211)) -
      _tmp212 * (Scalar(4.967784) * _tmp201 + std::cosh(_tmp203) - std::cosh(_tmp211));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
