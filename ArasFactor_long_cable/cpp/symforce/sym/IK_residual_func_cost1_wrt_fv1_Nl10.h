// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 604

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp10 = 2 * _tmp9;
  const Scalar _tmp11 = _tmp10 * _tmp5;
  const Scalar _tmp12 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp13 = _tmp12 * _tmp3;
  const Scalar _tmp14 = _tmp11 - _tmp13;
  const Scalar _tmp15 = Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp15;
  const Scalar _tmp17 = _tmp10 * _tmp3;
  const Scalar _tmp18 = _tmp12 * _tmp5;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp16 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp25 = 2 * _tmp3 * _tmp5;
  const Scalar _tmp26 = _tmp12 * _tmp9;
  const Scalar _tmp27 = _tmp25 + _tmp26;
  const Scalar _tmp28 = -Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp30 = _tmp28 - _tmp29;
  const Scalar _tmp31 = _tmp24 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(0, 0);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp35 = _tmp20 + _tmp7;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp36 + Scalar(-110.0);
  const Scalar _tmp38 = _tmp28 + _tmp29;
  const Scalar _tmp39 = _tmp24 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(-125.0);
  const Scalar _tmp42 = std::pow(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp41, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp43 = _tmp41 * _tmp42;
  const Scalar _tmp44 = Scalar(0.20999999999999999) * _tmp25 - Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp45 = -Scalar(0.010999999999999999) * _tmp23 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp46 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp47 = _tmp45 + _tmp46;
  const Scalar _tmp48 = _tmp44 + _tmp47;
  const Scalar _tmp49 = -_tmp44;
  const Scalar _tmp50 = _tmp47 + _tmp49;
  const Scalar _tmp51 = -_tmp19;
  const Scalar _tmp52 = _tmp16 + _tmp51;
  const Scalar _tmp53 = _tmp52 + _tmp8;
  const Scalar _tmp54 = _tmp53 + position_vector(1, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(110.0);
  const Scalar _tmp56 = -_tmp24;
  const Scalar _tmp57 = _tmp30 + _tmp56;
  const Scalar _tmp58 = _tmp57 + position_vector(0, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(125.0);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp55, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp59 * _tmp60;
  const Scalar _tmp62 = _tmp50 * _tmp61;
  const Scalar _tmp63 = _tmp45 - _tmp46;
  const Scalar _tmp64 = _tmp49 + _tmp63;
  const Scalar _tmp65 = -_tmp61 * _tmp64 + _tmp62;
  const Scalar _tmp66 = _tmp52 + _tmp7;
  const Scalar _tmp67 = _tmp66 + position_vector(1, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(-110.0);
  const Scalar _tmp69 = _tmp38 + _tmp56;
  const Scalar _tmp70 = _tmp69 + position_vector(0, 0);
  const Scalar _tmp71 = _tmp70 + Scalar(125.0);
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = _tmp68 * _tmp72;
  const Scalar _tmp74 = _tmp55 * _tmp60;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp61 * _tmp73 - _tmp74);
  const Scalar _tmp76 = _tmp37 * _tmp42;
  const Scalar _tmp77 = _tmp43 * _tmp73 - _tmp76;
  const Scalar _tmp78 = _tmp75 * _tmp77;
  const Scalar _tmp79 = _tmp43 * _tmp50;
  const Scalar _tmp80 = -_tmp62 * _tmp73 + _tmp64 * _tmp74;
  const Scalar _tmp81 = _tmp48 * _tmp76 - _tmp73 * _tmp79 - _tmp78 * _tmp80;
  const Scalar _tmp82 = Scalar(1.0) * _tmp66;
  const Scalar _tmp83 = -_tmp82;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp53 + _tmp83);
  const Scalar _tmp85 = Scalar(1.0) * _tmp69;
  const Scalar _tmp86 = _tmp84 * (-_tmp57 + _tmp85);
  const Scalar _tmp87 = -_tmp43 * _tmp48 - _tmp65 * _tmp78 + _tmp79 - _tmp81 * _tmp86;
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = _tmp82 * _tmp86 + _tmp85;
  const Scalar _tmp90 = 0;
  const Scalar _tmp91 = _tmp61 * _tmp78;
  const Scalar _tmp92 =
      std::sqrt(Scalar(std::pow(_tmp68, Scalar(2)) + std::pow(_tmp71, Scalar(2))));
  const Scalar _tmp93 = _tmp72 * _tmp92;
  const Scalar _tmp94 = _tmp93 * (_tmp43 * _tmp90 - _tmp90 * _tmp91);
  const Scalar _tmp95 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp96 = _tmp93 * (-_tmp66 * _tmp71 * _tmp95 + _tmp68 * _tmp69 * _tmp95);
  const Scalar _tmp97 = _tmp53 * _tmp61 - _tmp57 * _tmp74 + _tmp61 * _tmp96;
  const Scalar _tmp98 = _tmp35 * _tmp43 - _tmp39 * _tmp76 + _tmp43 * _tmp96 - _tmp78 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp100 = Scalar(1.0) * _tmp99;
  const Scalar _tmp101 = _tmp22 + Scalar(110.0);
  const Scalar _tmp102 = _tmp32 + Scalar(-125.0);
  const Scalar _tmp103 =
      std::pow(Scalar(std::pow(_tmp101, Scalar(2)) + std::pow(_tmp102, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp104 = _tmp101 * _tmp103;
  const Scalar _tmp105 = _tmp102 * _tmp103;
  const Scalar _tmp106 = fh1 * (_tmp104 * _tmp31 - _tmp105 * _tmp21);
  const Scalar _tmp107 = Scalar(1.0) * _tmp75;
  const Scalar _tmp108 = _tmp107 * _tmp80;
  const Scalar _tmp109 = -_tmp107 * _tmp65 + _tmp108 * _tmp86;
  const Scalar _tmp110 = _tmp88 * _tmp98;
  const Scalar _tmp111 = _tmp87 * _tmp99;
  const Scalar _tmp112 = _tmp111 * (-_tmp107 * _tmp97 - _tmp109 * _tmp110);
  const Scalar _tmp113 = _tmp88 * (_tmp109 + _tmp112);
  const Scalar _tmp114 = -_tmp113 * _tmp77 + Scalar(1.0);
  const Scalar _tmp115 = _tmp61 * _tmp75;
  const Scalar _tmp116 = _tmp104 * fh1;
  const Scalar _tmp117 = _tmp73 * _tmp75;
  const Scalar _tmp118 = _tmp117 * _tmp80 + _tmp50 * _tmp73;
  const Scalar _tmp119 = _tmp117 * _tmp65 - _tmp118 * _tmp86 - _tmp50;
  const Scalar _tmp120 = _tmp111 * (-_tmp110 * _tmp119 + _tmp117 * _tmp97 - _tmp96);
  const Scalar _tmp121 = _tmp88 * (_tmp119 + _tmp120);
  const Scalar _tmp122 = -_tmp121 * _tmp77 - _tmp73;
  const Scalar _tmp123 = _tmp105 * fh1;
  const Scalar _tmp124 = -_tmp106 * _tmp93 * (_tmp100 * _tmp43 - _tmp100 * _tmp91) -
                         _tmp116 * _tmp93 * (_tmp113 * _tmp43 + _tmp114 * _tmp115) -
                         _tmp123 * _tmp93 * (_tmp115 * _tmp122 + _tmp121 * _tmp43 + Scalar(1.0)) -
                         _tmp34 * _tmp94;
  const Scalar _tmp125 = Scalar(1.0) / (_tmp124);
  const Scalar _tmp126 = fh1 * (_tmp44 + _tmp63);
  const Scalar _tmp127 = -_tmp104 * _tmp126 - Scalar(2.3544) * _tmp14 - _tmp21 * fv1;
  const Scalar _tmp128 = _tmp35 + _tmp83;
  const Scalar _tmp129 = _tmp128 * _tmp86;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp39 + _tmp85);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp131 * _tmp86;
  const Scalar _tmp133 = _tmp84 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp134 = Scalar(1.0) * _tmp132 - Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp111 * _tmp131;
  const Scalar _tmp136 = _tmp128 * _tmp131;
  const Scalar _tmp137 = -_tmp100 * _tmp81 + _tmp111 * _tmp136;
  const Scalar _tmp138 = Scalar(1.0) * _tmp84;
  const Scalar _tmp139 = _tmp128 * _tmp130;
  const Scalar _tmp140 = _tmp118 + _tmp120 * _tmp139 - _tmp121 * _tmp81;
  const Scalar _tmp141 = _tmp105 * _tmp126 + Scalar(2.3544) * _tmp27 + _tmp31 * fv1;
  const Scalar _tmp142 = -Scalar(1.0) * _tmp131 + Scalar(1.0) * _tmp136 * _tmp84;
  const Scalar _tmp143 = _tmp130 * _tmp89;
  const Scalar _tmp144 = _tmp84 * (-_tmp128 * _tmp143 - _tmp81 * _tmp90 + _tmp83);
  const Scalar _tmp145 = -Scalar(1.0) * _tmp131 * _tmp89 - Scalar(1.0) * _tmp144 + Scalar(1.0);
  const Scalar _tmp146 = -_tmp108 + _tmp112 * _tmp139 - _tmp113 * _tmp81;
  const Scalar _tmp147 = Scalar(1.0) * _tmp106 * (_tmp135 - _tmp137 * _tmp138) +
                         Scalar(1.0) * _tmp116 * (_tmp112 * _tmp131 - _tmp138 * _tmp146) +
                         Scalar(1.0) * _tmp123 * (_tmp120 * _tmp131 - _tmp138 * _tmp140) +
                         _tmp127 * _tmp134 + _tmp141 * _tmp142 + _tmp145 * _tmp34;
  const Scalar _tmp148 = std::asinh(_tmp125 * _tmp147);
  const Scalar _tmp149 = Scalar(1.0) * _tmp148;
  const Scalar _tmp150 = std::pow(_tmp124, Scalar(-2));
  const Scalar _tmp151 = _tmp15 + _tmp51 + _tmp7;
  const Scalar _tmp152 = _tmp150 * _tmp94;
  const Scalar _tmp153 =
      (_tmp125 * (_tmp134 * _tmp151 + _tmp142 * _tmp31 - _tmp145) - _tmp147 * _tmp152) /
      std::sqrt(Scalar(std::pow(_tmp147, Scalar(2)) * _tmp150 + 1));
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp94;
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp124;
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp125;
  const Scalar _tmp157 =
      -_tmp148 * _tmp155 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp67), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp70 - 1), Scalar(2))));
  const Scalar _tmp158 = _tmp156 * _tmp157;
  const Scalar _tmp159 = _tmp100 * _tmp106;
  const Scalar _tmp160 = _tmp34 * _tmp90;
  const Scalar _tmp161 =
      _tmp114 * _tmp116 * _tmp75 + _tmp122 * _tmp123 * _tmp75 - _tmp159 * _tmp78 - _tmp160 * _tmp78;
  const Scalar _tmp162 = std::pow(_tmp161, Scalar(-2));
  const Scalar _tmp163 = _tmp162 * _tmp78 * _tmp90;
  const Scalar _tmp164 = Scalar(1.0) / (_tmp161);
  const Scalar _tmp165 = _tmp131 * _tmp141;
  const Scalar _tmp166 = _tmp128 * _tmp84;
  const Scalar _tmp167 = _tmp106 * _tmp137 * _tmp84 + _tmp116 * _tmp146 * _tmp84 +
                         _tmp123 * _tmp140 * _tmp84 + _tmp127 * _tmp133 + _tmp144 * _tmp34 -
                         _tmp165 * _tmp166;
  const Scalar _tmp168 = std::asinh(_tmp164 * _tmp167);
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp161;
  const Scalar _tmp170 =
      -_tmp168 * _tmp169 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp54 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp58 - 1), Scalar(2))));
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp164;
  const Scalar _tmp172 = _tmp170 * _tmp171;
  const Scalar _tmp173 = _tmp131 * _tmp31;
  const Scalar _tmp174 =
      (-_tmp163 * _tmp167 + _tmp164 * (_tmp133 * _tmp151 - _tmp144 - _tmp166 * _tmp173)) /
      std::sqrt(Scalar(_tmp162 * std::pow(_tmp167, Scalar(2)) + 1));
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp90;
  const Scalar _tmp176 = _tmp175 * _tmp78;
  const Scalar _tmp177 = Scalar(1.0) * _tmp168;
  const Scalar _tmp178 = -_tmp106 * _tmp135 - _tmp112 * _tmp116 * _tmp130 -
                         _tmp120 * _tmp123 * _tmp130 - _tmp127 * _tmp132 + _tmp143 * _tmp34 +
                         _tmp165;
  const Scalar _tmp179 = _tmp113 * _tmp116 + _tmp121 * _tmp123 + _tmp159 + _tmp160;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = std::asinh(_tmp178 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp184 =
      -_tmp181 * _tmp183 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp36), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp40), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp188 = _tmp187 * _tmp90;
  const Scalar _tmp189 = (_tmp178 * _tmp188 + _tmp180 * (-_tmp132 * _tmp151 - _tmp143 + _tmp173)) /
                         std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp187 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp32),
                                           Scalar(2)) +
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp22 - 1),
                                               Scalar(2)))))));
  _res(1, 0) = -_tmp154 * (Scalar(4.967784) * _tmp125 + std::cosh(_tmp149) - std::cosh(_tmp158)) -
               _tmp155 * (-Scalar(4.967784) * _tmp152 + Scalar(1.0) * _tmp153 * std::sinh(_tmp149) -
                          (-Scalar(0.1034955) * _tmp152 * _tmp157 +
                           _tmp156 * (-_tmp148 * _tmp154 - _tmp153 * _tmp155)) *
                              std::sinh(_tmp158));
  _res(2, 0) =
      -_tmp169 * (-Scalar(4.967784) * _tmp163 + Scalar(1.0) * _tmp174 * std::sinh(_tmp177) -
                  (-Scalar(0.1034955) * _tmp163 * _tmp170 +
                   _tmp171 * (-_tmp168 * _tmp176 - _tmp169 * _tmp174)) *
                      std::sinh(_tmp172)) -
      _tmp176 * (Scalar(4.967784) * _tmp164 - std::cosh(_tmp172) + std::cosh(_tmp177));
  _res(3, 0) = _tmp175 * (Scalar(4.967784) * _tmp180 + std::cosh(_tmp182) - std::cosh(_tmp186)) -
               _tmp183 * (Scalar(4.967784) * _tmp188 + Scalar(1.0) * _tmp189 * std::sinh(_tmp182) -
                          (Scalar(0.1034955) * _tmp184 * _tmp188 +
                           _tmp185 * (_tmp175 * _tmp181 - _tmp183 * _tmp189)) *
                              std::sinh(_tmp186));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
