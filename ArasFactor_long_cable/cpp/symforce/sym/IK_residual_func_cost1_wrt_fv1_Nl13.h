// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl13
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl13(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 606

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (192)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp6;
  const Scalar _tmp11 = _tmp10 * _tmp4;
  const Scalar _tmp12 = _tmp3 * _tmp7;
  const Scalar _tmp13 = _tmp11 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp22 = _tmp10 * _tmp3;
  const Scalar _tmp23 = _tmp4 * _tmp7;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = -_tmp25;
  const Scalar _tmp27 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp15 +
                        Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999);
  const Scalar _tmp29 = _tmp26 + _tmp28;
  const Scalar _tmp30 = _tmp21 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = -_tmp28;
  const Scalar _tmp34 = _tmp26 + _tmp33;
  const Scalar _tmp35 = _tmp21 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp36 + Scalar(110.0);
  const Scalar _tmp38 = -_tmp9;
  const Scalar _tmp39 = _tmp18 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(-125.0);
  const Scalar _tmp42 = std::pow(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp41, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp43 = _tmp41 * _tmp42;
  const Scalar _tmp44 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp45 = -_tmp44;
  const Scalar _tmp46 =
      -Scalar(0.010999999999999999) * _tmp16 - Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp48 = _tmp46 + _tmp47;
  const Scalar _tmp49 = _tmp45 + _tmp48;
  const Scalar _tmp50 = _tmp37 * _tmp42;
  const Scalar _tmp51 = _tmp14 - _tmp17;
  const Scalar _tmp52 = _tmp51 + _tmp9;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(125.0);
  const Scalar _tmp55 = -_tmp21;
  const Scalar _tmp56 = _tmp29 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(-110.0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp46 - _tmp47;
  const Scalar _tmp62 = _tmp45 + _tmp61;
  const Scalar _tmp63 = _tmp34 + _tmp55;
  const Scalar _tmp64 = _tmp63 + position_vector(1, 0);
  const Scalar _tmp65 = _tmp64 + Scalar(110.0);
  const Scalar _tmp66 = _tmp38 + _tmp51;
  const Scalar _tmp67 = _tmp66 + position_vector(0, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(125.0);
  const Scalar _tmp69 = Scalar(1.0) / (_tmp68);
  const Scalar _tmp70 = _tmp65 * _tmp69;
  const Scalar _tmp71 = _tmp62 * _tmp70;
  const Scalar _tmp72 = _tmp44 + _tmp61;
  const Scalar _tmp73 = _tmp58 * _tmp59;
  const Scalar _tmp74 = -_tmp60 * _tmp71 + _tmp72 * _tmp73;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp60 * _tmp70 - _tmp73);
  const Scalar _tmp76 = _tmp43 * _tmp70 - _tmp50;
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = _tmp43 * _tmp62;
  const Scalar _tmp79 = _tmp49 * _tmp50 - _tmp70 * _tmp78 - _tmp74 * _tmp77;
  const Scalar _tmp80 = Scalar(1.0) * _tmp63;
  const Scalar _tmp81 = -_tmp80;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp56 + _tmp81);
  const Scalar _tmp83 = Scalar(1.0) * _tmp66;
  const Scalar _tmp84 = _tmp82 * (-_tmp52 + _tmp83);
  const Scalar _tmp85 = _tmp60 * _tmp62 - _tmp60 * _tmp72;
  const Scalar _tmp86 = -_tmp43 * _tmp49 - _tmp77 * _tmp85 + _tmp78 - _tmp79 * _tmp84;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp86);
  const Scalar _tmp88 = Scalar(1.0) * _tmp75;
  const Scalar _tmp89 = _tmp74 * _tmp88;
  const Scalar _tmp90 = _tmp84 * _tmp89 - _tmp85 * _tmp88;
  const Scalar _tmp91 =
      std::sqrt(Scalar(std::pow(_tmp65, Scalar(2)) + std::pow(_tmp68, Scalar(2))));
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp69 * _tmp91;
  const Scalar _tmp94 = _tmp93 * (-_tmp63 * _tmp68 * _tmp92 + _tmp65 * _tmp66 * _tmp92);
  const Scalar _tmp95 = -_tmp52 * _tmp73 + _tmp56 * _tmp60 + _tmp60 * _tmp94;
  const Scalar _tmp96 = _tmp35 * _tmp43 - _tmp39 * _tmp50 + _tmp43 * _tmp94 - _tmp77 * _tmp95;
  const Scalar _tmp97 = _tmp87 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp86 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp88 * _tmp95 - _tmp90 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp90;
  const Scalar _tmp102 = _tmp101 * _tmp87;
  const Scalar _tmp103 = -_tmp102 * _tmp76 + Scalar(1.0);
  const Scalar _tmp104 = _tmp60 * _tmp75;
  const Scalar _tmp105 = _tmp31 + Scalar(-110.0);
  const Scalar _tmp106 = _tmp20 + Scalar(-125.0);
  const Scalar _tmp107 =
      std::pow(Scalar(std::pow(_tmp105, Scalar(2)) + std::pow(_tmp106, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp108 * fh1;
  const Scalar _tmp110 = _tmp70 * _tmp75;
  const Scalar _tmp111 = _tmp110 * _tmp74 + _tmp71;
  const Scalar _tmp112 = _tmp110 * _tmp85 - _tmp111 * _tmp84 - _tmp62;
  const Scalar _tmp113 = _tmp99 * (_tmp110 * _tmp95 - _tmp112 * _tmp97 - _tmp94);
  const Scalar _tmp114 = _tmp87 * (_tmp112 + _tmp113);
  const Scalar _tmp115 = -_tmp114 * _tmp76 - _tmp70;
  const Scalar _tmp116 = _tmp106 * _tmp107;
  const Scalar _tmp117 = _tmp116 * fh1;
  const Scalar _tmp118 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp119 = _tmp80 * _tmp84 + _tmp83;
  const Scalar _tmp120 = 0;
  const Scalar _tmp121 = _tmp120 * _tmp87;
  const Scalar _tmp122 = _tmp121 * _tmp77;
  const Scalar _tmp123 = _tmp93 * (_tmp121 * _tmp43 - _tmp122 * _tmp60);
  const Scalar _tmp124 = Scalar(1.0) * _tmp98;
  const Scalar _tmp125 = _tmp76 * _tmp88 * _tmp98;
  const Scalar _tmp126 = fh1 * (_tmp108 * _tmp19 - _tmp116 * _tmp30);
  const Scalar _tmp127 = -_tmp109 * _tmp93 * (_tmp102 * _tmp43 + _tmp103 * _tmp104) -
                         _tmp117 * _tmp93 * (_tmp104 * _tmp115 + _tmp114 * _tmp43 + Scalar(1.0)) -
                         _tmp118 * _tmp123 -
                         _tmp126 * _tmp93 * (_tmp124 * _tmp43 - _tmp125 * _tmp60);
  const Scalar _tmp128 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp129 = _tmp123 * _tmp128;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp131 = fh1 * (_tmp44 + _tmp48);
  const Scalar _tmp132 = _tmp116 * _tmp131 + Scalar(2.3544) * _tmp13 + _tmp19 * fv1;
  const Scalar _tmp133 = _tmp35 + _tmp81;
  const Scalar _tmp134 = _tmp133 * _tmp84;
  const Scalar _tmp135 = Scalar(1.0) / (-_tmp134 - _tmp39 + _tmp83);
  const Scalar _tmp136 = Scalar(1.0) * _tmp135;
  const Scalar _tmp137 = _tmp133 * _tmp82;
  const Scalar _tmp138 = Scalar(1.0) * _tmp136 * _tmp137 - Scalar(1.0) * _tmp136;
  const Scalar _tmp139 = _tmp119 * _tmp135;
  const Scalar _tmp140 = _tmp79 * _tmp87;
  const Scalar _tmp141 = _tmp82 * (-_tmp120 * _tmp140 - _tmp133 * _tmp139 + _tmp81);
  const Scalar _tmp142 = -Scalar(1.0) * _tmp119 * _tmp136 - Scalar(1.0) * _tmp141 + Scalar(1.0);
  const Scalar _tmp143 = _tmp133 * _tmp135;
  const Scalar _tmp144 = _tmp100 * _tmp143 - _tmp101 * _tmp140 - _tmp89;
  const Scalar _tmp145 = Scalar(1.0) * _tmp82;
  const Scalar _tmp146 = -_tmp108 * _tmp131 - Scalar(2.3544) * _tmp24 - _tmp30 * fv1;
  const Scalar _tmp147 = _tmp136 * _tmp84;
  const Scalar _tmp148 = _tmp82 * (_tmp134 * _tmp136 + Scalar(1.0));
  const Scalar _tmp149 = Scalar(1.0) * _tmp147 - Scalar(1.0) * _tmp148;
  const Scalar _tmp150 = _tmp111 + _tmp113 * _tmp143 - _tmp114 * _tmp79;
  const Scalar _tmp151 = _tmp136 * _tmp99;
  const Scalar _tmp152 = -_tmp124 * _tmp79 + _tmp133 * _tmp151;
  const Scalar _tmp153 = Scalar(1.0) * _tmp109 * (_tmp100 * _tmp136 - _tmp144 * _tmp145) +
                         Scalar(1.0) * _tmp117 * (_tmp113 * _tmp136 - _tmp145 * _tmp150) +
                         _tmp118 * _tmp142 +
                         Scalar(1.0) * _tmp126 * (-_tmp145 * _tmp152 + _tmp151) +
                         _tmp132 * _tmp138 + _tmp146 * _tmp149;
  const Scalar _tmp154 = std::asinh(_tmp130 * _tmp153);
  const Scalar _tmp155 = Scalar(1.0) * _tmp154;
  const Scalar _tmp156 = _tmp25 + _tmp33 + _tmp55;
  const Scalar _tmp157 =
      (-_tmp129 * _tmp153 + _tmp130 * (_tmp138 * _tmp19 - _tmp142 + _tmp149 * _tmp156)) /
      std::sqrt(Scalar(_tmp128 * std::pow(_tmp153, Scalar(2)) + 1));
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp159 =
      -_tmp154 * _tmp158 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp64 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp67 - 1), Scalar(2))));
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp123;
  const Scalar _tmp161 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp162 = _tmp159 * _tmp161;
  const Scalar _tmp163 = _tmp118 * _tmp121;
  const Scalar _tmp164 = _tmp103 * _tmp109 * _tmp75 + _tmp115 * _tmp117 * _tmp75 -
                         _tmp125 * _tmp126 - _tmp163 * _tmp77;
  const Scalar _tmp165 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp166 = _tmp122 * _tmp165;
  const Scalar _tmp167 = _tmp132 * _tmp136;
  const Scalar _tmp168 = _tmp109 * _tmp144 * _tmp82 + _tmp117 * _tmp150 * _tmp82 +
                         _tmp118 * _tmp141 + _tmp126 * _tmp152 * _tmp82 - _tmp137 * _tmp167 +
                         _tmp146 * _tmp148;
  const Scalar _tmp169 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp170 = std::asinh(_tmp168 * _tmp169);
  const Scalar _tmp171 = Scalar(1.0) * _tmp170;
  const Scalar _tmp172 = _tmp136 * _tmp19;
  const Scalar _tmp173 =
      (-_tmp166 * _tmp168 + _tmp169 * (-_tmp137 * _tmp172 - _tmp141 + _tmp148 * _tmp156)) /
      std::sqrt(Scalar(_tmp165 * std::pow(_tmp168, Scalar(2)) + 1));
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp175 =
      -_tmp170 * _tmp174 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp57), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp53 - 1), Scalar(2))));
  const Scalar _tmp176 = Scalar(9.6622558468725703) * _tmp121;
  const Scalar _tmp177 = _tmp176 * _tmp77;
  const Scalar _tmp178 = Scalar(0.1034955) * _tmp169;
  const Scalar _tmp179 = _tmp175 * _tmp178;
  const Scalar _tmp180 = _tmp102 * _tmp109 + _tmp114 * _tmp117 + _tmp124 * _tmp126 + _tmp163;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = -_tmp100 * _tmp109 * _tmp135 - _tmp113 * _tmp117 * _tmp135 +
                         _tmp118 * _tmp139 - _tmp126 * _tmp151 - _tmp146 * _tmp147 + _tmp167;
  const Scalar _tmp183 = std::asinh(_tmp181 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp185 =
      -_tmp183 * _tmp184 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp40), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp36 - 1), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp183;
  const Scalar _tmp189 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp190 = _tmp121 * _tmp189;
  const Scalar _tmp191 = (_tmp181 * (-_tmp139 - _tmp147 * _tmp156 + _tmp172) + _tmp182 * _tmp190) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp189 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp32 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp20),
                                           Scalar(2)) +
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp31),
                                               Scalar(2)))))));
  _res(1, 0) =
      -_tmp158 * (-Scalar(4.967784) * _tmp129 + Scalar(1.0) * _tmp157 * std::sinh(_tmp155) -
                  (-Scalar(0.1034955) * _tmp129 * _tmp159 +
                   _tmp161 * (-_tmp154 * _tmp160 - _tmp157 * _tmp158)) *
                      std::sinh(_tmp162)) -
      _tmp160 * (Scalar(4.967784) * _tmp130 + std::cosh(_tmp155) - std::cosh(_tmp162));
  _res(2, 0) =
      -_tmp174 * (-Scalar(4.967784) * _tmp166 + Scalar(1.0) * _tmp173 * std::sinh(_tmp171) -
                  (-Scalar(0.1034955) * _tmp166 * _tmp175 +
                   _tmp178 * (-_tmp170 * _tmp177 - _tmp173 * _tmp174)) *
                      std::sinh(_tmp179)) -
      _tmp177 * (Scalar(4.967784) * _tmp169 + std::cosh(_tmp171) - std::cosh(_tmp179));
  _res(3, 0) = _tmp176 * (Scalar(4.967784) * _tmp181 - std::cosh(_tmp187) + std::cosh(_tmp188)) -
               _tmp184 * (Scalar(4.967784) * _tmp190 + Scalar(1.0) * _tmp191 * std::sinh(_tmp188) -
                          (Scalar(0.1034955) * _tmp185 * _tmp190 +
                           _tmp186 * (_tmp176 * _tmp183 - _tmp184 * _tmp191)) *
                              std::sinh(_tmp187));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
