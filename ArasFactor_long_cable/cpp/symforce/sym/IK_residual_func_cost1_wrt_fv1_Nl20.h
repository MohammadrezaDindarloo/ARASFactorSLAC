// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl20
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl20(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 609

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp6;
  const Scalar _tmp14 = _tmp13 * _tmp4;
  const Scalar _tmp15 = _tmp3 * _tmp7;
  const Scalar _tmp16 = _tmp14 + _tmp15;
  const Scalar _tmp17 = -Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp12 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp10 +
                        Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999);
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp24 = _tmp13 * _tmp3;
  const Scalar _tmp25 = _tmp4 * _tmp7;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = -_tmp23 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = -_tmp9;
  const Scalar _tmp34 = _tmp12 + _tmp17;
  const Scalar _tmp35 = _tmp33 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(0, 0);
  const Scalar _tmp37 = _tmp36 + Scalar(-125.0);
  const Scalar _tmp38 = Scalar(1.0) / (_tmp37);
  const Scalar _tmp39 = -_tmp22;
  const Scalar _tmp40 = _tmp23 + _tmp39;
  const Scalar _tmp41 = _tmp28 + _tmp40;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(110.0);
  const Scalar _tmp44 = _tmp38 * _tmp43;
  const Scalar _tmp45 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp46 =
      -Scalar(0.010999999999999999) * _tmp11 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp48 = _tmp46 - _tmp47;
  const Scalar _tmp49 = _tmp45 + _tmp48;
  const Scalar _tmp50 = _tmp44 * _tmp49;
  const Scalar _tmp51 = -_tmp45;
  const Scalar _tmp52 = _tmp48 + _tmp51;
  const Scalar _tmp53 = _tmp29 + _tmp39;
  const Scalar _tmp54 = _tmp53 + position_vector(1, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(110.0);
  const Scalar _tmp56 = _tmp18 + _tmp33;
  const Scalar _tmp57 = _tmp56 + position_vector(0, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(125.0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp55, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp55 * _tmp59;
  const Scalar _tmp61 = _tmp58 * _tmp59;
  const Scalar _tmp62 = -_tmp50 * _tmp61 + _tmp52 * _tmp60;
  const Scalar _tmp63 = Scalar(1.0) / (_tmp44 * _tmp61 - _tmp60);
  const Scalar _tmp64 = _tmp44 * _tmp63;
  const Scalar _tmp65 = _tmp50 + _tmp62 * _tmp64;
  const Scalar _tmp66 = Scalar(1.0) * _tmp41;
  const Scalar _tmp67 = -_tmp66;
  const Scalar _tmp68 = Scalar(1.0) / (_tmp53 + _tmp67);
  const Scalar _tmp69 = Scalar(1.0) * _tmp35;
  const Scalar _tmp70 = -_tmp56 + _tmp69;
  const Scalar _tmp71 = _tmp68 * _tmp70;
  const Scalar _tmp72 = _tmp49 * _tmp61 - _tmp52 * _tmp61;
  const Scalar _tmp73 = -_tmp49 + _tmp64 * _tmp72 - _tmp65 * _tmp71;
  const Scalar _tmp74 = _tmp22 + _tmp23 + _tmp28;
  const Scalar _tmp75 = _tmp74 + position_vector(1, 0);
  const Scalar _tmp76 = _tmp75 + Scalar(-110.0);
  const Scalar _tmp77 = _tmp34 + _tmp9;
  const Scalar _tmp78 = _tmp77 + position_vector(0, 0);
  const Scalar _tmp79 = _tmp78 + Scalar(-125.0);
  const Scalar _tmp80 = std::pow(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp79, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp81 = _tmp79 * _tmp80;
  const Scalar _tmp82 = _tmp76 * _tmp80;
  const Scalar _tmp83 =
      std::sqrt(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp43, Scalar(2))));
  const Scalar _tmp84 = Scalar(1.0) / (_tmp83);
  const Scalar _tmp85 = _tmp38 * _tmp83;
  const Scalar _tmp86 = _tmp85 * (_tmp35 * _tmp43 * _tmp84 - _tmp37 * _tmp41 * _tmp84);
  const Scalar _tmp87 = _tmp53 * _tmp61 - _tmp56 * _tmp60 + _tmp61 * _tmp86;
  const Scalar _tmp88 = _tmp44 * _tmp81 - _tmp82;
  const Scalar _tmp89 = _tmp63 * _tmp88;
  const Scalar _tmp90 = _tmp74 * _tmp81 - _tmp77 * _tmp82 + _tmp81 * _tmp86 - _tmp87 * _tmp89;
  const Scalar _tmp91 = _tmp46 + _tmp47;
  const Scalar _tmp92 = _tmp45 + _tmp91;
  const Scalar _tmp93 = _tmp49 * _tmp81;
  const Scalar _tmp94 = -_tmp44 * _tmp93 - _tmp62 * _tmp89 + _tmp82 * _tmp92;
  const Scalar _tmp95 = -_tmp71 * _tmp94 - _tmp72 * _tmp89 - _tmp81 * _tmp92 + _tmp93;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp90 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp99 = _tmp95 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (_tmp64 * _tmp87 - _tmp73 * _tmp97 - _tmp86);
  const Scalar _tmp101 = _tmp100 + _tmp73;
  const Scalar _tmp102 = _tmp88 * _tmp96;
  const Scalar _tmp103 = -_tmp101 * _tmp102 - _tmp44;
  const Scalar _tmp104 = _tmp61 * _tmp63;
  const Scalar _tmp105 = _tmp81 * _tmp96;
  const Scalar _tmp106 = _tmp20 + Scalar(125.0);
  const Scalar _tmp107 = _tmp31 + Scalar(-110.0);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp112 = _tmp66 * _tmp71 + _tmp69;
  const Scalar _tmp113 = 0;
  const Scalar _tmp114 = _tmp113 * _tmp96;
  const Scalar _tmp115 = _tmp114 * _tmp89;
  const Scalar _tmp116 = _tmp85 * (_tmp114 * _tmp81 - _tmp115 * _tmp61);
  const Scalar _tmp117 = Scalar(1.0) * _tmp63;
  const Scalar _tmp118 = Scalar(1.0) * _tmp68;
  const Scalar _tmp119 = -_tmp117 * _tmp72 + _tmp118 * _tmp62 * _tmp63 * _tmp70;
  const Scalar _tmp120 = _tmp99 * (-_tmp117 * _tmp87 - _tmp119 * _tmp97);
  const Scalar _tmp121 = _tmp119 + _tmp120;
  const Scalar _tmp122 = -_tmp102 * _tmp121 + Scalar(1.0);
  const Scalar _tmp123 = _tmp107 * _tmp108;
  const Scalar _tmp124 = _tmp123 * fh1;
  const Scalar _tmp125 = Scalar(1.0) * _tmp98;
  const Scalar _tmp126 = fh1 * (-_tmp109 * _tmp30 + _tmp123 * _tmp19);
  const Scalar _tmp127 = -_tmp110 * _tmp85 * (_tmp101 * _tmp105 + _tmp103 * _tmp104 + Scalar(1.0)) -
                         _tmp111 * _tmp116 -
                         _tmp124 * _tmp85 * (_tmp104 * _tmp122 + _tmp105 * _tmp121) -
                         _tmp126 * _tmp85 * (-_tmp125 * _tmp61 * _tmp89 + _tmp125 * _tmp81);
  const Scalar _tmp128 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp129 = _tmp116 * _tmp128;
  const Scalar _tmp130 = _tmp67 + _tmp74;
  const Scalar _tmp131 = _tmp130 * _tmp71;
  const Scalar _tmp132 = Scalar(1.0) / (-_tmp131 + _tmp69 - _tmp77);
  const Scalar _tmp133 = Scalar(1.0) * _tmp132;
  const Scalar _tmp134 = _tmp112 * _tmp132;
  const Scalar _tmp135 = _tmp94 * _tmp96;
  const Scalar _tmp136 = _tmp68 * (-_tmp113 * _tmp135 - _tmp130 * _tmp134 + _tmp67);
  const Scalar _tmp137 = -Scalar(1.0) * _tmp112 * _tmp133 - Scalar(1.0) * _tmp136 + Scalar(1.0);
  const Scalar _tmp138 = fh1 * (_tmp51 + _tmp91);
  const Scalar _tmp139 = -_tmp123 * _tmp138 - Scalar(2.3544) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp140 = _tmp133 * _tmp71;
  const Scalar _tmp141 = _tmp131 * _tmp133 + Scalar(1.0);
  const Scalar _tmp142 = -Scalar(1.0) * _tmp118 * _tmp141 + Scalar(1.0) * _tmp140;
  const Scalar _tmp143 = _tmp130 * _tmp132;
  const Scalar _tmp144 = -_tmp117 * _tmp62 + _tmp120 * _tmp143 - _tmp121 * _tmp135;
  const Scalar _tmp145 = _tmp100 * _tmp143 - _tmp101 * _tmp135 + _tmp65;
  const Scalar _tmp146 = _tmp109 * _tmp138 + Scalar(2.3544) * _tmp16 + _tmp19 * fv1;
  const Scalar _tmp147 = _tmp130 * _tmp68;
  const Scalar _tmp148 = Scalar(1.0) * _tmp133 * _tmp147 - Scalar(1.0) * _tmp133;
  const Scalar _tmp149 = _tmp133 * _tmp99;
  const Scalar _tmp150 = -_tmp125 * _tmp94 + _tmp130 * _tmp149;
  const Scalar _tmp151 = Scalar(1.0) * _tmp110 * (_tmp100 * _tmp133 - _tmp118 * _tmp145) +
                         _tmp111 * _tmp137 +
                         Scalar(1.0) * _tmp124 * (-_tmp118 * _tmp144 + _tmp120 * _tmp133) +
                         Scalar(1.0) * _tmp126 * (-_tmp118 * _tmp150 + _tmp149) +
                         _tmp139 * _tmp142 + _tmp146 * _tmp148;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp153 = std::asinh(_tmp151 * _tmp152);
  const Scalar _tmp154 = Scalar(1.0) * _tmp153;
  const Scalar _tmp155 = _tmp27 + _tmp40;
  const Scalar _tmp156 =
      (-_tmp129 * _tmp151 + _tmp152 * (-_tmp137 + _tmp142 * _tmp155 + _tmp148 * _tmp19)) /
      std::sqrt(Scalar(_tmp128 * std::pow(_tmp151, Scalar(2)) + 1));
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp158 =
      -_tmp153 * _tmp157 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp36), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp159 = Scalar(0.1034955) * _tmp152;
  const Scalar _tmp160 = _tmp158 * _tmp159;
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp116;
  const Scalar _tmp162 = _tmp111 * _tmp114;
  const Scalar _tmp163 = _tmp125 * _tmp126;
  const Scalar _tmp164 =
      _tmp103 * _tmp110 * _tmp63 + _tmp122 * _tmp124 * _tmp63 - _tmp162 * _tmp89 - _tmp163 * _tmp89;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp141 * _tmp68;
  const Scalar _tmp167 = _tmp133 * _tmp146;
  const Scalar _tmp168 = _tmp110 * _tmp145 * _tmp68 + _tmp111 * _tmp136 +
                         _tmp124 * _tmp144 * _tmp68 + _tmp126 * _tmp150 * _tmp68 +
                         _tmp139 * _tmp166 - _tmp147 * _tmp167;
  const Scalar _tmp169 = std::asinh(_tmp165 * _tmp168);
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp54 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp57 - 1), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp114;
  const Scalar _tmp175 = _tmp174 * _tmp89;
  const Scalar _tmp176 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp177 = _tmp115 * _tmp176;
  const Scalar _tmp178 = _tmp133 * _tmp19;
  const Scalar _tmp179 =
      (_tmp165 * (-_tmp136 - _tmp147 * _tmp178 + _tmp155 * _tmp166) - _tmp168 * _tmp177) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp176 + 1));
  const Scalar _tmp180 = Scalar(1.0) * _tmp169;
  const Scalar _tmp181 =
      _tmp101 * _tmp110 * _tmp96 + _tmp121 * _tmp124 * _tmp96 + _tmp162 + _tmp163;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = -_tmp100 * _tmp110 * _tmp132 + _tmp111 * _tmp134 -
                         _tmp120 * _tmp124 * _tmp132 - _tmp126 * _tmp149 - _tmp139 * _tmp140 +
                         _tmp167;
  const Scalar _tmp184 = std::asinh(_tmp182 * _tmp183);
  const Scalar _tmp185 = Scalar(1.0) * _tmp184;
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp187 =
      -_tmp184 * _tmp186 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp75), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp78), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp191 = _tmp114 * _tmp190;
  const Scalar _tmp192 = (_tmp182 * (-_tmp134 - _tmp140 * _tmp155 + _tmp178) + _tmp183 * _tmp191) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp190 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp32 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp31),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp20 - 1),
                                           Scalar(2)))))));
  _res(1, 0) =
      -_tmp157 * (-Scalar(4.967784) * _tmp129 + Scalar(1.0) * _tmp156 * std::sinh(_tmp154) -
                  (-Scalar(0.1034955) * _tmp129 * _tmp158 +
                   _tmp159 * (-_tmp153 * _tmp161 - _tmp156 * _tmp157)) *
                      std::sinh(_tmp160)) -
      _tmp161 * (Scalar(4.967784) * _tmp152 + std::cosh(_tmp154) - std::cosh(_tmp160));
  _res(2, 0) =
      -_tmp170 * (-Scalar(4.967784) * _tmp177 + Scalar(1.0) * _tmp179 * std::sinh(_tmp180) -
                  (-Scalar(0.1034955) * _tmp171 * _tmp177 +
                   _tmp172 * (-_tmp169 * _tmp175 - _tmp170 * _tmp179)) *
                      std::sinh(_tmp173)) -
      _tmp175 * (Scalar(4.967784) * _tmp165 - std::cosh(_tmp173) + std::cosh(_tmp180));
  _res(3, 0) = _tmp174 * (Scalar(4.967784) * _tmp182 + std::cosh(_tmp185) - std::cosh(_tmp189)) -
               _tmp186 * (Scalar(4.967784) * _tmp191 + Scalar(1.0) * _tmp192 * std::sinh(_tmp185) -
                          (Scalar(0.1034955) * _tmp187 * _tmp191 +
                           _tmp188 * (_tmp174 * _tmp184 - _tmp186 * _tmp192)) *
                              std::sinh(_tmp189));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
