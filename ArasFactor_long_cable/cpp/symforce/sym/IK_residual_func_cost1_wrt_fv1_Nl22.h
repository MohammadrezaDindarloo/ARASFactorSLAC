// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl22
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl22(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 605

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (191)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp10 = 2 * _tmp5 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp3;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp3;
  const Scalar _tmp16 = _tmp15 * _tmp9;
  const Scalar _tmp17 = _tmp11 * _tmp5;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp14 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp8;
  const Scalar _tmp21 = _tmp20 + position_vector(0, 0);
  const Scalar _tmp22 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp25 = _tmp15 * _tmp5;
  const Scalar _tmp26 = _tmp11 * _tmp9;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = -_tmp28;
  const Scalar _tmp30 = -_tmp24 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(1, 0);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = _tmp19 + _tmp7;
  const Scalar _tmp35 = _tmp34 + position_vector(0, 0);
  const Scalar _tmp36 = _tmp23 + _tmp24 + _tmp29;
  const Scalar _tmp37 = _tmp36 + position_vector(1, 0);
  const Scalar _tmp38 = -_tmp23;
  const Scalar _tmp39 = _tmp24 + _tmp38;
  const Scalar _tmp40 = _tmp29 + _tmp39;
  const Scalar _tmp41 = _tmp40 + position_vector(1, 0);
  const Scalar _tmp42 = _tmp41 + Scalar(110.0);
  const Scalar _tmp43 = _tmp14 - _tmp18;
  const Scalar _tmp44 = _tmp43 + _tmp7;
  const Scalar _tmp45 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-125.0);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp42, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp46 * _tmp47;
  const Scalar _tmp49 = _tmp37 + Scalar(-110.0);
  const Scalar _tmp50 = _tmp35 + Scalar(-125.0);
  const Scalar _tmp51 =
      std::sqrt(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp50, Scalar(2))));
  const Scalar _tmp52 = Scalar(1.0) / (_tmp51);
  const Scalar _tmp53 = Scalar(1.0) / (_tmp50);
  const Scalar _tmp54 = _tmp51 * _tmp53;
  const Scalar _tmp55 = _tmp54 * (_tmp34 * _tmp49 * _tmp52 - _tmp36 * _tmp50 * _tmp52);
  const Scalar _tmp56 = _tmp30 + _tmp38;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(110.0);
  const Scalar _tmp59 = _tmp43 + _tmp8;
  const Scalar _tmp60 = _tmp59 + position_vector(0, 0);
  const Scalar _tmp61 = _tmp60 + Scalar(125.0);
  const Scalar _tmp62 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp61, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp63 = _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp58 * _tmp62;
  const Scalar _tmp65 = _tmp55 * _tmp63 + _tmp56 * _tmp63 - _tmp59 * _tmp64;
  const Scalar _tmp66 = _tmp49 * _tmp53;
  const Scalar _tmp67 = Scalar(1.0) / (_tmp63 * _tmp66 - _tmp64);
  const Scalar _tmp68 = _tmp42 * _tmp47;
  const Scalar _tmp69 = _tmp48 * _tmp66 - _tmp68;
  const Scalar _tmp70 = _tmp67 * _tmp69;
  const Scalar _tmp71 = _tmp40 * _tmp48 - _tmp44 * _tmp68 + _tmp48 * _tmp55 - _tmp65 * _tmp70;
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = Scalar(1.0) * _tmp72;
  const Scalar _tmp74 = _tmp63 * _tmp70;
  const Scalar _tmp75 = _tmp32 + Scalar(-110.0);
  const Scalar _tmp76 = _tmp21 + Scalar(125.0);
  const Scalar _tmp77 = std::pow(Scalar(std::pow(_tmp75, Scalar(2)) + std::pow(_tmp76, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp78 = _tmp75 * _tmp77;
  const Scalar _tmp79 = _tmp76 * _tmp77;
  const Scalar _tmp80 = fh1 * (_tmp20 * _tmp78 - _tmp31 * _tmp79);
  const Scalar _tmp81 = _tmp66 * _tmp67;
  const Scalar _tmp82 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp83 = -Scalar(0.010999999999999999) * _tmp22 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp84 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp85 = _tmp83 + _tmp84;
  const Scalar _tmp86 = _tmp82 + _tmp85;
  const Scalar _tmp87 = _tmp66 * _tmp86;
  const Scalar _tmp88 = -_tmp82;
  const Scalar _tmp89 = _tmp83 - _tmp84;
  const Scalar _tmp90 = _tmp88 + _tmp89;
  const Scalar _tmp91 = _tmp63 * _tmp86;
  const Scalar _tmp92 = _tmp64 * _tmp90 - _tmp66 * _tmp91;
  const Scalar _tmp93 = _tmp81 * _tmp92 + _tmp87;
  const Scalar _tmp94 = Scalar(1.0) * _tmp36;
  const Scalar _tmp95 = -_tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp56 + _tmp95);
  const Scalar _tmp97 = Scalar(1.0) * _tmp34;
  const Scalar _tmp98 = _tmp96 * (-_tmp59 + _tmp97);
  const Scalar _tmp99 = -_tmp63 * _tmp90 + _tmp91;
  const Scalar _tmp100 = _tmp81 * _tmp99 - _tmp86 - _tmp93 * _tmp98;
  const Scalar _tmp101 = _tmp82 + _tmp89;
  const Scalar _tmp102 = _tmp101 * _tmp68 - _tmp48 * _tmp87 - _tmp70 * _tmp92;
  const Scalar _tmp103 = -_tmp101 * _tmp48 - _tmp102 * _tmp98 + _tmp48 * _tmp86 - _tmp70 * _tmp99;
  const Scalar _tmp104 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp105 = _tmp104 * _tmp71;
  const Scalar _tmp106 = _tmp103 * _tmp72;
  const Scalar _tmp107 = _tmp106 * (-_tmp100 * _tmp105 - _tmp55 + _tmp65 * _tmp81);
  const Scalar _tmp108 = _tmp100 + _tmp107;
  const Scalar _tmp109 = _tmp104 * _tmp48;
  const Scalar _tmp110 = _tmp104 * _tmp69;
  const Scalar _tmp111 = -_tmp108 * _tmp110 - _tmp66;
  const Scalar _tmp112 = _tmp63 * _tmp67;
  const Scalar _tmp113 = _tmp79 * fh1;
  const Scalar _tmp114 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp115 = _tmp94 * _tmp98 + _tmp97;
  const Scalar _tmp116 = 0;
  const Scalar _tmp117 = _tmp54 * (_tmp116 * _tmp48 - _tmp116 * _tmp74);
  const Scalar _tmp118 = Scalar(1.0) * _tmp67;
  const Scalar _tmp119 = _tmp118 * _tmp92;
  const Scalar _tmp120 = -_tmp118 * _tmp99 + _tmp119 * _tmp98;
  const Scalar _tmp121 = _tmp106 * (-_tmp105 * _tmp120 - _tmp118 * _tmp65);
  const Scalar _tmp122 = _tmp120 + _tmp121;
  const Scalar _tmp123 = -_tmp110 * _tmp122 + Scalar(1.0);
  const Scalar _tmp124 = _tmp78 * fh1;
  const Scalar _tmp125 = -_tmp113 * _tmp54 * (_tmp108 * _tmp109 + _tmp111 * _tmp112 + Scalar(1.0)) -
                         _tmp114 * _tmp117 -
                         _tmp124 * _tmp54 * (_tmp109 * _tmp122 + _tmp112 * _tmp123) -
                         _tmp54 * _tmp80 * (_tmp48 * _tmp73 - _tmp73 * _tmp74);
  const Scalar _tmp126 = Scalar(1.0) / (_tmp125);
  const Scalar _tmp127 = _tmp40 + _tmp95;
  const Scalar _tmp128 = _tmp127 * _tmp98;
  const Scalar _tmp129 = Scalar(1.0) / (-_tmp128 - _tmp44 + _tmp97);
  const Scalar _tmp130 = Scalar(1.0) * _tmp129;
  const Scalar _tmp131 = _tmp106 * _tmp130;
  const Scalar _tmp132 = -_tmp102 * _tmp73 + _tmp127 * _tmp131;
  const Scalar _tmp133 = Scalar(1.0) * _tmp96;
  const Scalar _tmp134 = fh1 * (_tmp85 + _tmp88);
  const Scalar _tmp135 = Scalar(2.3544) * _tmp13 + _tmp134 * _tmp79 + _tmp20 * fv1;
  const Scalar _tmp136 = _tmp127 * _tmp96;
  const Scalar _tmp137 = Scalar(1.0) * _tmp130 * _tmp136 - Scalar(1.0) * _tmp130;
  const Scalar _tmp138 = _tmp127 * _tmp129;
  const Scalar _tmp139 = _tmp102 * _tmp104;
  const Scalar _tmp140 = -_tmp119 + _tmp121 * _tmp138 - _tmp122 * _tmp139;
  const Scalar _tmp141 = _tmp115 * _tmp129;
  const Scalar _tmp142 = _tmp96 * (-_tmp102 * _tmp116 - _tmp127 * _tmp141 + _tmp95);
  const Scalar _tmp143 = -Scalar(1.0) * _tmp115 * _tmp130 - Scalar(1.0) * _tmp142 + Scalar(1.0);
  const Scalar _tmp144 = -_tmp134 * _tmp78 - Scalar(2.3544) * _tmp27 - _tmp31 * fv1;
  const Scalar _tmp145 = _tmp96 * (_tmp128 * _tmp130 + Scalar(1.0));
  const Scalar _tmp146 = _tmp130 * _tmp98;
  const Scalar _tmp147 = -Scalar(1.0) * _tmp145 + Scalar(1.0) * _tmp146;
  const Scalar _tmp148 = _tmp107 * _tmp138 - _tmp108 * _tmp139 + _tmp93;
  const Scalar _tmp149 =
      Scalar(1.0) * _tmp113 * (_tmp107 * _tmp130 - _tmp133 * _tmp148) + _tmp114 * _tmp143 +
      Scalar(1.0) * _tmp124 * (_tmp121 * _tmp130 - _tmp133 * _tmp140) + _tmp135 * _tmp137 +
      _tmp144 * _tmp147 + Scalar(1.0) * _tmp80 * (_tmp131 - _tmp132 * _tmp133);
  const Scalar _tmp150 = std::asinh(_tmp126 * _tmp149);
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp152 =
      -_tmp150 * _tmp151 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp35), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp37), Scalar(2))));
  const Scalar _tmp153 = Scalar(0.1034955) * _tmp126;
  const Scalar _tmp154 = _tmp152 * _tmp153;
  const Scalar _tmp155 = std::pow(_tmp125, Scalar(-2));
  const Scalar _tmp156 = _tmp117 * _tmp155;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp117;
  const Scalar _tmp158 = _tmp28 + _tmp39;
  const Scalar _tmp159 =
      (_tmp126 * (_tmp137 * _tmp20 - _tmp143 + _tmp147 * _tmp158) - _tmp149 * _tmp156) /
      std::sqrt(Scalar(std::pow(_tmp149, Scalar(2)) * _tmp155 + 1));
  const Scalar _tmp160 = Scalar(1.0) * _tmp150;
  const Scalar _tmp161 = _tmp114 * _tmp116;
  const Scalar _tmp162 = _tmp73 * _tmp80;
  const Scalar _tmp163 =
      _tmp111 * _tmp113 * _tmp67 + _tmp123 * _tmp124 * _tmp67 - _tmp161 * _tmp70 - _tmp162 * _tmp70;
  const Scalar _tmp164 = Scalar(1.0) / (_tmp163);
  const Scalar _tmp165 = _tmp130 * _tmp135;
  const Scalar _tmp166 = _tmp113 * _tmp148 * _tmp96 + _tmp114 * _tmp142 +
                         _tmp124 * _tmp140 * _tmp96 + _tmp132 * _tmp80 * _tmp96 -
                         _tmp136 * _tmp165 + _tmp144 * _tmp145;
  const Scalar _tmp167 = std::asinh(_tmp164 * _tmp166);
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp169 =
      -_tmp167 * _tmp168 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp57 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp60 - 1), Scalar(2))));
  const Scalar _tmp170 = Scalar(0.1034955) * _tmp164;
  const Scalar _tmp171 = _tmp169 * _tmp170;
  const Scalar _tmp172 = std::pow(_tmp163, Scalar(-2));
  const Scalar _tmp173 = _tmp116 * _tmp172 * _tmp70;
  const Scalar _tmp174 = _tmp130 * _tmp20;
  const Scalar _tmp175 =
      (_tmp164 * (-_tmp136 * _tmp174 - _tmp142 + _tmp145 * _tmp158) - _tmp166 * _tmp173) /
      std::sqrt(Scalar(std::pow(_tmp166, Scalar(2)) * _tmp172 + 1));
  const Scalar _tmp176 = Scalar(9.6622558468725703) * _tmp116;
  const Scalar _tmp177 = _tmp176 * _tmp70;
  const Scalar _tmp178 = Scalar(1.0) * _tmp167;
  const Scalar _tmp179 = -_tmp107 * _tmp113 * _tmp129 + _tmp114 * _tmp141 -
                         _tmp121 * _tmp124 * _tmp129 - _tmp131 * _tmp80 - _tmp144 * _tmp146 +
                         _tmp165;
  const Scalar _tmp180 =
      _tmp104 * _tmp108 * _tmp113 + _tmp104 * _tmp122 * _tmp124 + _tmp161 + _tmp162;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = std::asinh(_tmp179 * _tmp181);
  const Scalar _tmp183 = Scalar(1.0) * _tmp182;
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp185 =
      -_tmp182 * _tmp184 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp45), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp41 - 1), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp189 = _tmp116 * _tmp188;
  const Scalar _tmp190 = (_tmp179 * _tmp189 + _tmp181 * (-_tmp141 - _tmp146 * _tmp158 + _tmp174)) /
                         std::sqrt(Scalar(std::pow(_tmp179, Scalar(2)) * _tmp188 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp32),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp21 - 1),
                                           Scalar(2)))))));
  _res(1, 0) =
      -_tmp151 * (-Scalar(4.967784) * _tmp156 + Scalar(1.0) * _tmp159 * std::sinh(_tmp160) -
                  (-Scalar(0.1034955) * _tmp152 * _tmp156 +
                   _tmp153 * (-_tmp150 * _tmp157 - _tmp151 * _tmp159)) *
                      std::sinh(_tmp154)) -
      _tmp157 * (Scalar(4.967784) * _tmp126 - std::cosh(_tmp154) + std::cosh(_tmp160));
  _res(2, 0) =
      -_tmp168 * (-Scalar(4.967784) * _tmp173 + Scalar(1.0) * _tmp175 * std::sinh(_tmp178) -
                  (-Scalar(0.1034955) * _tmp169 * _tmp173 +
                   _tmp170 * (-_tmp167 * _tmp177 - _tmp168 * _tmp175)) *
                      std::sinh(_tmp171)) -
      _tmp177 * (Scalar(4.967784) * _tmp164 - std::cosh(_tmp171) + std::cosh(_tmp178));
  _res(3, 0) = _tmp176 * (Scalar(4.967784) * _tmp181 + std::cosh(_tmp183) - std::cosh(_tmp187)) -
               _tmp184 * (Scalar(4.967784) * _tmp189 + Scalar(1.0) * _tmp190 * std::sinh(_tmp183) -
                          (Scalar(0.1034955) * _tmp185 * _tmp189 +
                           _tmp186 * (_tmp176 * _tmp182 - _tmp184 * _tmp190)) *
                              std::sinh(_tmp187));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
