// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl8
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl8(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 608

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp6;
  const Scalar _tmp14 = _tmp13 * _tmp3;
  const Scalar _tmp15 = _tmp4 * _tmp7;
  const Scalar _tmp16 = _tmp14 - _tmp15;
  const Scalar _tmp17 = Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp17;
  const Scalar _tmp19 = -_tmp12 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp9;
  const Scalar _tmp21 = _tmp20 + position_vector(1, 0);
  const Scalar _tmp22 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp24 = _tmp13 * _tmp4;
  const Scalar _tmp25 = _tmp3 * _tmp7;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = _tmp12 + _tmp18 + _tmp9;
  const Scalar _tmp34 = _tmp33 + position_vector(1, 0);
  const Scalar _tmp35 = _tmp34 + Scalar(-110.0);
  const Scalar _tmp36 = _tmp27 + _tmp28;
  const Scalar _tmp37 = _tmp23 + _tmp36;
  const Scalar _tmp38 = _tmp37 + position_vector(0, 0);
  const Scalar _tmp39 = _tmp38 + Scalar(-125.0);
  const Scalar _tmp40 = Scalar(1.0) / (_tmp39);
  const Scalar _tmp41 = _tmp35 * _tmp40;
  const Scalar _tmp42 = -_tmp9;
  const Scalar _tmp43 = _tmp19 + _tmp42;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp44 + Scalar(110.0);
  const Scalar _tmp46 = -_tmp23;
  const Scalar _tmp47 = _tmp29 + _tmp46;
  const Scalar _tmp48 = _tmp47 + position_vector(0, 0);
  const Scalar _tmp49 = _tmp48 + Scalar(125.0);
  const Scalar _tmp50 = std::pow(Scalar(std::pow(_tmp45, Scalar(2)) + std::pow(_tmp49, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp51 = _tmp49 * _tmp50;
  const Scalar _tmp52 = _tmp45 * _tmp50;
  const Scalar _tmp53 = Scalar(1.0) / (_tmp41 * _tmp51 - _tmp52);
  const Scalar _tmp54 =
      std::sqrt(Scalar(std::pow(_tmp35, Scalar(2)) + std::pow(_tmp39, Scalar(2))));
  const Scalar _tmp55 = Scalar(1.0) / (_tmp54);
  const Scalar _tmp56 = _tmp40 * _tmp54;
  const Scalar _tmp57 = _tmp56 * (-_tmp33 * _tmp39 * _tmp55 + _tmp35 * _tmp37 * _tmp55);
  const Scalar _tmp58 = _tmp53 * (_tmp43 * _tmp51 - _tmp47 * _tmp52 + _tmp51 * _tmp57);
  const Scalar _tmp59 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp60 = -_tmp59;
  const Scalar _tmp61 = -Scalar(0.010999999999999999) * _tmp10 -
                        Scalar(0.010999999999999999) * _tmp22 + Scalar(-0.010999999999999999);
  const Scalar _tmp62 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp63 = _tmp61 - _tmp62;
  const Scalar _tmp64 = _tmp60 + _tmp63;
  const Scalar _tmp65 = _tmp61 + _tmp62;
  const Scalar _tmp66 = _tmp59 + _tmp65;
  const Scalar _tmp67 = _tmp51 * _tmp66;
  const Scalar _tmp68 = _tmp53 * (-_tmp51 * _tmp64 + _tmp67);
  const Scalar _tmp69 = Scalar(1.0) * _tmp37;
  const Scalar _tmp70 = -_tmp47 + _tmp69;
  const Scalar _tmp71 = Scalar(1.0) * _tmp33;
  const Scalar _tmp72 = -_tmp71;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp43 + _tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp73;
  const Scalar _tmp75 = _tmp53 * (-_tmp41 * _tmp67 + _tmp52 * _tmp64);
  const Scalar _tmp76 = -Scalar(1.0) * _tmp68 + _tmp70 * _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp60 + _tmp65;
  const Scalar _tmp78 = _tmp36 + _tmp46;
  const Scalar _tmp79 = _tmp78 + position_vector(0, 0);
  const Scalar _tmp80 = _tmp79 + Scalar(125.0);
  const Scalar _tmp81 = _tmp12 + _tmp42;
  const Scalar _tmp82 = _tmp18 + _tmp81;
  const Scalar _tmp83 = _tmp82 + position_vector(1, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(-110.0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp80, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp80 * _tmp85;
  const Scalar _tmp87 = _tmp41 * _tmp66;
  const Scalar _tmp88 = _tmp84 * _tmp85;
  const Scalar _tmp89 = _tmp41 * _tmp86 - _tmp88;
  const Scalar _tmp90 = -_tmp75 * _tmp89 + _tmp77 * _tmp88 - _tmp86 * _tmp87;
  const Scalar _tmp91 = _tmp70 * _tmp73;
  const Scalar _tmp92 = _tmp66 * _tmp86 - _tmp68 * _tmp89 - _tmp77 * _tmp86 - _tmp90 * _tmp91;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 = _tmp57 * _tmp86 - _tmp58 * _tmp89 - _tmp78 * _tmp88 + _tmp82 * _tmp86;
  const Scalar _tmp95 = _tmp93 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp97 = _tmp92 * _tmp96;
  const Scalar _tmp98 = _tmp97 * (-Scalar(1.0) * _tmp58 - _tmp76 * _tmp95);
  const Scalar _tmp99 = _tmp76 + _tmp98;
  const Scalar _tmp100 = _tmp86 * _tmp93;
  const Scalar _tmp101 = _tmp89 * _tmp93;
  const Scalar _tmp102 = -_tmp101 * _tmp99 + Scalar(1.0);
  const Scalar _tmp103 = _tmp51 * _tmp53;
  const Scalar _tmp104 = _tmp21 + Scalar(110.0);
  const Scalar _tmp105 = _tmp31 + Scalar(-125.0);
  const Scalar _tmp106 =
      std::pow(Scalar(std::pow(_tmp104, Scalar(2)) + std::pow(_tmp105, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp107 = _tmp104 * _tmp106;
  const Scalar _tmp108 = _tmp107 * fh1;
  const Scalar _tmp109 = Scalar(1.0) * _tmp96;
  const Scalar _tmp110 = _tmp103 * _tmp89;
  const Scalar _tmp111 = _tmp105 * _tmp106;
  const Scalar _tmp112 = fh1 * (_tmp107 * _tmp30 - _tmp111 * _tmp20);
  const Scalar _tmp113 = _tmp41 * _tmp75 + _tmp87;
  const Scalar _tmp114 = -_tmp113 * _tmp91 + _tmp41 * _tmp68 - _tmp66;
  const Scalar _tmp115 = _tmp97 * (-_tmp114 * _tmp95 + _tmp41 * _tmp58 - _tmp57);
  const Scalar _tmp116 = _tmp114 + _tmp115;
  const Scalar _tmp117 = -_tmp101 * _tmp116 - _tmp41;
  const Scalar _tmp118 = _tmp111 * fh1;
  const Scalar _tmp119 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp120 = _tmp69 + _tmp71 * _tmp91;
  const Scalar _tmp121 = 0;
  const Scalar _tmp122 = _tmp56 * (-_tmp110 * _tmp121 + _tmp121 * _tmp86);
  const Scalar _tmp123 = -_tmp108 * _tmp56 * (_tmp100 * _tmp99 + _tmp102 * _tmp103) -
                         _tmp112 * _tmp56 * (-_tmp109 * _tmp110 + _tmp109 * _tmp86) -
                         _tmp118 * _tmp56 * (_tmp100 * _tmp116 + _tmp103 * _tmp117 + Scalar(1.0)) -
                         _tmp119 * _tmp122;
  const Scalar _tmp124 = Scalar(1.0) / (_tmp123);
  const Scalar _tmp125 = _tmp90 * _tmp93;
  const Scalar _tmp126 = _tmp72 + _tmp82;
  const Scalar _tmp127 = _tmp126 * _tmp91;
  const Scalar _tmp128 = Scalar(1.0) / (-_tmp127 + _tmp69 - _tmp78);
  const Scalar _tmp129 = _tmp126 * _tmp128;
  const Scalar _tmp130 = -_tmp125 * _tmp99 + _tmp129 * _tmp98 - Scalar(1.0) * _tmp75;
  const Scalar _tmp131 = Scalar(1.0) * _tmp128;
  const Scalar _tmp132 = _tmp113 + _tmp115 * _tmp129 - _tmp116 * _tmp125;
  const Scalar _tmp133 = fh1 * (_tmp59 + _tmp63);
  const Scalar _tmp134 = _tmp111 * _tmp133 + Scalar(2.3544) * _tmp26 + _tmp30 * fv1;
  const Scalar _tmp135 = _tmp126 * _tmp73;
  const Scalar _tmp136 = Scalar(1.0) * _tmp131 * _tmp135 - Scalar(1.0) * _tmp131;
  const Scalar _tmp137 = _tmp131 * _tmp97;
  const Scalar _tmp138 = -_tmp109 * _tmp90 + _tmp126 * _tmp137;
  const Scalar _tmp139 = -_tmp107 * _tmp133 - Scalar(2.3544) * _tmp16 - _tmp20 * fv1;
  const Scalar _tmp140 = _tmp127 * _tmp131 + Scalar(1.0);
  const Scalar _tmp141 = _tmp131 * _tmp91;
  const Scalar _tmp142 = -Scalar(1.0) * _tmp140 * _tmp74 + Scalar(1.0) * _tmp141;
  const Scalar _tmp143 = _tmp120 * _tmp128;
  const Scalar _tmp144 = _tmp73 * (-_tmp121 * _tmp90 - _tmp126 * _tmp143 + _tmp72);
  const Scalar _tmp145 = -Scalar(1.0) * _tmp120 * _tmp131 - Scalar(1.0) * _tmp144 + Scalar(1.0);
  const Scalar _tmp146 = Scalar(1.0) * _tmp108 * (-_tmp130 * _tmp74 + _tmp131 * _tmp98) +
                         Scalar(1.0) * _tmp112 * (_tmp137 - _tmp138 * _tmp74) +
                         Scalar(1.0) * _tmp118 * (_tmp115 * _tmp131 - _tmp132 * _tmp74) +
                         _tmp119 * _tmp145 + _tmp134 * _tmp136 + _tmp139 * _tmp142;
  const Scalar _tmp147 = std::asinh(_tmp124 * _tmp146);
  const Scalar _tmp148 = Scalar(1.0) * _tmp147;
  const Scalar _tmp149 = Scalar(9.6622558468725703) * _tmp123;
  const Scalar _tmp150 =
      -_tmp147 * _tmp149 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp34), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp38), Scalar(2))));
  const Scalar _tmp151 = Scalar(0.1034955) * _tmp124;
  const Scalar _tmp152 = _tmp150 * _tmp151;
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp122;
  const Scalar _tmp154 = std::pow(_tmp123, Scalar(-2));
  const Scalar _tmp155 = _tmp122 * _tmp154;
  const Scalar _tmp156 = _tmp17 + _tmp81;
  const Scalar _tmp157 =
      (_tmp124 * (_tmp136 * _tmp30 + _tmp142 * _tmp156 - _tmp145) - _tmp146 * _tmp155) /
      std::sqrt(Scalar(std::pow(_tmp146, Scalar(2)) * _tmp154 + 1));
  const Scalar _tmp158 = _tmp119 * _tmp121;
  const Scalar _tmp159 = _tmp53 * _tmp89;
  const Scalar _tmp160 = _tmp109 * _tmp112;
  const Scalar _tmp161 = _tmp102 * _tmp108 * _tmp53 + _tmp117 * _tmp118 * _tmp53 -
                         _tmp158 * _tmp159 - _tmp159 * _tmp160;
  const Scalar _tmp162 = std::pow(_tmp161, Scalar(-2));
  const Scalar _tmp163 = _tmp121 * _tmp159 * _tmp162;
  const Scalar _tmp164 = _tmp140 * _tmp73;
  const Scalar _tmp165 = _tmp131 * _tmp134;
  const Scalar _tmp166 = _tmp108 * _tmp130 * _tmp73 + _tmp112 * _tmp138 * _tmp73 +
                         _tmp118 * _tmp132 * _tmp73 + _tmp119 * _tmp144 - _tmp135 * _tmp165 +
                         _tmp139 * _tmp164;
  const Scalar _tmp167 = Scalar(1.0) / (_tmp161);
  const Scalar _tmp168 = std::asinh(_tmp166 * _tmp167);
  const Scalar _tmp169 = Scalar(1.0) * _tmp168;
  const Scalar _tmp170 = _tmp131 * _tmp30;
  const Scalar _tmp171 =
      (-_tmp163 * _tmp166 + _tmp167 * (-_tmp135 * _tmp170 - _tmp144 + _tmp156 * _tmp164)) /
      std::sqrt(Scalar(_tmp162 * std::pow(_tmp166, Scalar(2)) + 1));
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp121;
  const Scalar _tmp173 = _tmp159 * _tmp172;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp161;
  const Scalar _tmp175 = Scalar(0.1034955) * _tmp167;
  const Scalar _tmp176 =
      -_tmp168 * _tmp174 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp44 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp48 - 1), Scalar(2))));
  const Scalar _tmp177 = _tmp175 * _tmp176;
  const Scalar _tmp178 = _tmp108 * _tmp93 * _tmp99 + _tmp116 * _tmp118 * _tmp93 + _tmp158 + _tmp160;
  const Scalar _tmp179 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp180 = _tmp121 * _tmp179;
  const Scalar _tmp181 = -_tmp108 * _tmp128 * _tmp98 - _tmp112 * _tmp137 -
                         _tmp115 * _tmp118 * _tmp128 + _tmp119 * _tmp143 - _tmp139 * _tmp141 +
                         _tmp165;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp183 = std::asinh(_tmp181 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp185 =
      -_tmp183 * _tmp184 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp83), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp79 - 1), Scalar(2))));
  const Scalar _tmp186 = (_tmp180 * _tmp181 + _tmp182 * (-_tmp141 * _tmp156 - _tmp143 + _tmp170)) /
                         std::sqrt(Scalar(_tmp179 * std::pow(_tmp181, Scalar(2)) + 1));
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp188 = _tmp185 * _tmp187;
  const Scalar _tmp189 = Scalar(1.0) * _tmp183;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp32 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp31),
                                           Scalar(2)) +
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp21 - 1),
                                               Scalar(2)))))));
  _res(1, 0) =
      -_tmp149 * (-Scalar(4.967784) * _tmp155 + Scalar(1.0) * _tmp157 * std::sinh(_tmp148) -
                  (-Scalar(0.1034955) * _tmp150 * _tmp155 +
                   _tmp151 * (-_tmp147 * _tmp153 - _tmp149 * _tmp157)) *
                      std::sinh(_tmp152)) -
      _tmp153 * (Scalar(4.967784) * _tmp124 + std::cosh(_tmp148) - std::cosh(_tmp152));
  _res(2, 0) = -_tmp173 * (Scalar(4.967784) * _tmp167 + std::cosh(_tmp169) - std::cosh(_tmp177)) -
               _tmp174 * (-Scalar(4.967784) * _tmp163 + Scalar(1.0) * _tmp171 * std::sinh(_tmp169) -
                          (-Scalar(0.1034955) * _tmp163 * _tmp176 +
                           _tmp175 * (-_tmp168 * _tmp173 - _tmp171 * _tmp174)) *
                              std::sinh(_tmp177));
  _res(3, 0) = _tmp172 * (Scalar(4.967784) * _tmp182 - std::cosh(_tmp188) + std::cosh(_tmp189)) -
               _tmp184 * (Scalar(4.967784) * _tmp180 + Scalar(1.0) * _tmp186 * std::sinh(_tmp189) -
                          (Scalar(0.1034955) * _tmp180 * _tmp185 +
                           _tmp187 * (_tmp172 * _tmp183 - _tmp184 * _tmp186)) *
                              std::sinh(_tmp188));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
