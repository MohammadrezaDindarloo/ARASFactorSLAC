// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl19
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl19(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 642

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp8 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp1;
  const Scalar _tmp14 = _tmp13 * _tmp7;
  const Scalar _tmp15 = _tmp3 * _tmp9;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(0, 0);
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = _tmp13 * _tmp3;
  const Scalar _tmp23 = _tmp7 * _tmp9;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp26 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp28 = _tmp25 + _tmp27;
  const Scalar _tmp29 = _tmp21 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(1, 0);
  const Scalar _tmp31 = _tmp0 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -Scalar(0.1034955) * _tmp33 * fh1 -
      Scalar(12.936937500000001) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp30), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp19 - 1), Scalar(2))));
  const Scalar _tmp35 = _tmp0 * _tmp34;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(1.0) * _tmp32;
  const Scalar _tmp39 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp40 = _tmp25 - _tmp27;
  const Scalar _tmp41 = _tmp20 + _tmp40;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(110.0);
  const Scalar _tmp44 = _tmp12 - _tmp16;
  const Scalar _tmp45 = _tmp44 + _tmp5;
  const Scalar _tmp46 = _tmp45 + position_vector(0, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(-125.0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp51 = -_tmp50;
  const Scalar _tmp52 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp53 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp54 = _tmp52 - _tmp53;
  const Scalar _tmp55 = _tmp51 + _tmp54;
  const Scalar _tmp56 = _tmp20 + _tmp28;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(-110.0);
  const Scalar _tmp59 = _tmp17 + _tmp5;
  const Scalar _tmp60 = _tmp59 + position_vector(0, 0);
  const Scalar _tmp61 = _tmp60 + Scalar(-125.0);
  const Scalar _tmp62 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp61, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp63 = _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp52 + _tmp53;
  const Scalar _tmp65 = _tmp50 + _tmp64;
  const Scalar _tmp66 = _tmp55 * _tmp63 - _tmp63 * _tmp65;
  const Scalar _tmp67 = _tmp21 + _tmp40;
  const Scalar _tmp68 = _tmp67 + position_vector(1, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(110.0);
  const Scalar _tmp70 = _tmp44 + _tmp6;
  const Scalar _tmp71 = _tmp70 + position_vector(0, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(125.0);
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = _tmp69 * _tmp73;
  const Scalar _tmp75 = _tmp58 * _tmp62;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp63 * _tmp74 - _tmp75);
  const Scalar _tmp77 = _tmp43 * _tmp48;
  const Scalar _tmp78 = _tmp49 * _tmp74 - _tmp77;
  const Scalar _tmp79 = _tmp76 * _tmp78;
  const Scalar _tmp80 = _tmp51 + _tmp64;
  const Scalar _tmp81 = _tmp55 * _tmp74;
  const Scalar _tmp82 = -_tmp63 * _tmp81 + _tmp65 * _tmp75;
  const Scalar _tmp83 = -_tmp49 * _tmp81 + _tmp77 * _tmp80 - _tmp79 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) * _tmp67;
  const Scalar _tmp85 = -_tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp56 + _tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp70;
  const Scalar _tmp88 = _tmp86 * (-_tmp59 + _tmp87);
  const Scalar _tmp89 = _tmp49 * _tmp55 - _tmp49 * _tmp80 - _tmp66 * _tmp79 - _tmp83 * _tmp88;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 = _tmp84 * _tmp88 + _tmp87;
  const Scalar _tmp92 = 0;
  const Scalar _tmp93 = _tmp90 * _tmp92;
  const Scalar _tmp94 = _tmp63 * _tmp79;
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp69, Scalar(2)) + std::pow(_tmp72, Scalar(2))));
  const Scalar _tmp96 = _tmp73 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) * _tmp76;
  const Scalar _tmp98 = _tmp82 * _tmp97;
  const Scalar _tmp99 = -_tmp66 * _tmp97 + _tmp88 * _tmp98;
  const Scalar _tmp100 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp101 = _tmp96 * (-_tmp100 * _tmp67 * _tmp72 + _tmp100 * _tmp69 * _tmp70);
  const Scalar _tmp102 = _tmp101 * _tmp63 + _tmp56 * _tmp63 - _tmp59 * _tmp75;
  const Scalar _tmp103 = _tmp101 * _tmp49 - _tmp102 * _tmp79 + _tmp41 * _tmp49 - _tmp45 * _tmp77;
  const Scalar _tmp104 = _tmp103 * _tmp90;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp106 = _tmp105 * _tmp89;
  const Scalar _tmp107 = _tmp106 * (-_tmp102 * _tmp97 - _tmp104 * _tmp99);
  const Scalar _tmp108 = _tmp107 + _tmp99;
  const Scalar _tmp109 = _tmp108 * _tmp90;
  const Scalar _tmp110 = _tmp78 * _tmp90;
  const Scalar _tmp111 = -_tmp108 * _tmp110 + Scalar(1.0);
  const Scalar _tmp112 = _tmp63 * _tmp76;
  const Scalar _tmp113 = _tmp19 + Scalar(125.0);
  const Scalar _tmp114 = _tmp30 + Scalar(-110.0);
  const Scalar _tmp115 =
      std::pow(Scalar(std::pow(_tmp113, Scalar(2)) + std::pow(_tmp114, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp116 = _tmp114 * _tmp115;
  const Scalar _tmp117 = _tmp116 * _tmp96 * (_tmp109 * _tmp49 + _tmp111 * _tmp112);
  const Scalar _tmp118 = _tmp113 * _tmp115;
  const Scalar _tmp119 = _tmp116 * _tmp18 - _tmp118 * _tmp29;
  const Scalar _tmp120 = Scalar(1.0) * _tmp105;
  const Scalar _tmp121 = _tmp119 * _tmp96 * (_tmp120 * _tmp49 - _tmp120 * _tmp94);
  const Scalar _tmp122 = _tmp74 * _tmp76;
  const Scalar _tmp123 = _tmp122 * _tmp82 + _tmp81;
  const Scalar _tmp124 = _tmp122 * _tmp66 - _tmp123 * _tmp88 - _tmp55;
  const Scalar _tmp125 = _tmp106 * (-_tmp101 + _tmp102 * _tmp122 - _tmp104 * _tmp124);
  const Scalar _tmp126 = _tmp124 + _tmp125;
  const Scalar _tmp127 = _tmp126 * _tmp90;
  const Scalar _tmp128 = -_tmp110 * _tmp126 - _tmp74;
  const Scalar _tmp129 = _tmp118 * _tmp96 * (_tmp112 * _tmp128 + _tmp127 * _tmp49 + Scalar(1.0));
  const Scalar _tmp130 = -_tmp117 * fh1 - _tmp121 * fh1 - _tmp129 * fh1 -
                         _tmp39 * _tmp96 * (_tmp49 * _tmp93 - _tmp93 * _tmp94);
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp41 + _tmp85;
  const Scalar _tmp133 = _tmp132 * _tmp88;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 - _tmp45 + _tmp87);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp106 * _tmp135;
  const Scalar _tmp137 = -_tmp120 * _tmp83 + _tmp132 * _tmp136;
  const Scalar _tmp138 = Scalar(1.0) * _tmp86;
  const Scalar _tmp139 = Scalar(1.0) * _tmp119 * (_tmp136 - _tmp137 * _tmp138);
  const Scalar _tmp140 = _tmp50 + _tmp54;
  const Scalar _tmp141 = _tmp140 * fh1;
  const Scalar _tmp142 = Scalar(2.3544) * _tmp11 + _tmp118 * _tmp141 + _tmp18 * fv1;
  const Scalar _tmp143 = _tmp132 * _tmp86;
  const Scalar _tmp144 = Scalar(1.0) * _tmp135 * _tmp143 - Scalar(1.0) * _tmp135;
  const Scalar _tmp145 = _tmp83 * _tmp90;
  const Scalar _tmp146 = _tmp132 * _tmp134;
  const Scalar _tmp147 = _tmp107 * _tmp146 - _tmp108 * _tmp145 - _tmp98;
  const Scalar _tmp148 = Scalar(1.0) * _tmp116 * (_tmp107 * _tmp135 - _tmp138 * _tmp147);
  const Scalar _tmp149 = _tmp134 * _tmp91;
  const Scalar _tmp150 = _tmp86 * (-_tmp132 * _tmp149 - _tmp145 * _tmp92 + _tmp85);
  const Scalar _tmp151 = -_tmp116 * _tmp141 - Scalar(2.3544) * _tmp24 - _tmp29 * fv1;
  const Scalar _tmp152 = _tmp133 * _tmp135 + Scalar(1.0);
  const Scalar _tmp153 = _tmp135 * _tmp88;
  const Scalar _tmp154 = -Scalar(1.0) * _tmp138 * _tmp152 + Scalar(1.0) * _tmp153;
  const Scalar _tmp155 = _tmp123 + _tmp125 * _tmp146 - _tmp126 * _tmp145;
  const Scalar _tmp156 = Scalar(1.0) * _tmp118 * (_tmp125 * _tmp135 - _tmp138 * _tmp155);
  const Scalar _tmp157 =
      _tmp139 * fh1 + _tmp142 * _tmp144 + _tmp148 * fh1 + _tmp151 * _tmp154 + _tmp156 * fh1 +
      Scalar(1.0) * _tmp39 * (-_tmp135 * _tmp91 - Scalar(1.0) * _tmp150 + Scalar(1.0));
  const Scalar _tmp158 = std::asinh(_tmp131 * _tmp157);
  const Scalar _tmp159 = Scalar(1.0) * _tmp158;
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp161 =
      -_tmp158 * _tmp160 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp68 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp71 - 1), Scalar(2))));
  const Scalar _tmp162 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp163 = _tmp161 * _tmp162;
  const Scalar _tmp164 = -_tmp117 - _tmp121 - _tmp129;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp166 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp167 = _tmp164 * _tmp166;
  const Scalar _tmp168 = _tmp118 * _tmp140;
  const Scalar _tmp169 = _tmp116 * _tmp140;
  const Scalar _tmp170 =
      (_tmp131 * (_tmp139 + _tmp144 * _tmp168 + _tmp148 - _tmp154 * _tmp169 + _tmp156) -
       _tmp157 * _tmp167) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp166 + 1));
  const Scalar _tmp171 = _tmp152 * _tmp86;
  const Scalar _tmp172 = _tmp116 * _tmp147 * _tmp86;
  const Scalar _tmp173 = _tmp135 * _tmp142;
  const Scalar _tmp174 = _tmp119 * _tmp137 * _tmp86;
  const Scalar _tmp175 = _tmp118 * _tmp86;
  const Scalar _tmp176 = _tmp155 * _tmp175;
  const Scalar _tmp177 = -_tmp143 * _tmp173 + _tmp150 * _tmp39 + _tmp151 * _tmp171 + _tmp172 * fh1 +
                         _tmp174 * fh1 + _tmp176 * fh1;
  const Scalar _tmp178 = _tmp39 * _tmp93;
  const Scalar _tmp179 = _tmp118 * _tmp128 * _tmp76;
  const Scalar _tmp180 = _tmp119 * _tmp120;
  const Scalar _tmp181 = _tmp180 * fh1;
  const Scalar _tmp182 = _tmp111 * _tmp116 * _tmp76;
  const Scalar _tmp183 = -_tmp178 * _tmp79 + _tmp179 * fh1 - _tmp181 * _tmp79 + _tmp182 * fh1;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp177 * _tmp184);
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp187 =
      -_tmp185 * _tmp186 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp57), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp60), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(1.0) * _tmp185;
  const Scalar _tmp191 = _tmp179 - _tmp180 * _tmp79 + _tmp182;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp191;
  const Scalar _tmp193 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp194 = _tmp191 * _tmp193;
  const Scalar _tmp195 =
      (-_tmp177 * _tmp194 + _tmp184 * (-_tmp132 * _tmp135 * _tmp140 * _tmp175 - _tmp169 * _tmp171 +
                                       _tmp172 + _tmp174 + _tmp176)) /
      std::sqrt(Scalar(std::pow(_tmp177, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp196 = _tmp109 * _tmp116;
  const Scalar _tmp197 = _tmp118 * _tmp127;
  const Scalar _tmp198 = _tmp178 + _tmp181 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = _tmp107 * _tmp116 * _tmp134;
  const Scalar _tmp201 = _tmp119 * _tmp136;
  const Scalar _tmp202 = _tmp118 * _tmp125 * _tmp134;
  const Scalar _tmp203 = _tmp149 * _tmp39 - _tmp151 * _tmp153 + _tmp173 - _tmp200 * fh1 -
                         _tmp201 * fh1 - _tmp202 * fh1;
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp198;
  const Scalar _tmp207 =
      -_tmp204 * _tmp206 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp46), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = _tmp180 + _tmp196 + _tmp197;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp210;
  const Scalar _tmp212 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp213 = _tmp210 * _tmp212;
  const Scalar _tmp214 =
      (_tmp199 * (_tmp135 * _tmp168 + _tmp153 * _tmp169 - _tmp200 - _tmp201 - _tmp202) -
       _tmp203 * _tmp213) /
      std::sqrt(Scalar(std::pow(_tmp203, Scalar(2)) * _tmp212 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp38) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp31 * _tmp37 - _tmp33) -
            _tmp34 * _tmp36) *
               std::cosh(_tmp35)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35) -
      Scalar(9.6622558468725703) * std::sinh(_tmp38);
  _res(1, 0) = _tmp160 * (-Scalar(1.0) * _tmp170 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp161 * _tmp167 +
                           _tmp162 * (-_tmp158 * _tmp165 - _tmp160 * _tmp170)) *
                              std::cosh(_tmp163)) +
               _tmp165 * (-std::sinh(_tmp159) - std::sinh(_tmp163));
  _res(2, 0) = _tmp186 * (-Scalar(1.0) * _tmp195 * std::cosh(_tmp190) -
                          (-Scalar(0.1034955) * _tmp187 * _tmp194 +
                           _tmp188 * (-_tmp185 * _tmp192 - _tmp186 * _tmp195)) *
                              std::cosh(_tmp189)) +
               _tmp192 * (-std::sinh(_tmp189) - std::sinh(_tmp190));
  _res(3, 0) = _tmp206 * (-Scalar(1.0) * _tmp214 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp207 * _tmp213 +
                           _tmp208 * (-_tmp204 * _tmp211 - _tmp206 * _tmp214)) *
                              std::cosh(_tmp209)) +
               _tmp211 * (-std::sinh(_tmp205) - std::sinh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
