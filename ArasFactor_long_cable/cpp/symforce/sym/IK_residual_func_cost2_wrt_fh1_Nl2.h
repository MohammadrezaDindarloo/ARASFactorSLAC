// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl2
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl2(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 640

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (211)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp3 = 2 * _tmp2;
  const Scalar _tmp4 = _tmp1 * _tmp3;
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp7 = _tmp5 * _tmp6;
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -_tmp8;
  const Scalar _tmp10 = _tmp3 * _tmp5;
  const Scalar _tmp11 = _tmp1 * _tmp6;
  const Scalar _tmp12 = _tmp10 - _tmp11;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp15 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 - _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp9;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp20;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp5;
  const Scalar _tmp24 = _tmp2 * _tmp6;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp4 - Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = _tmp0 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -Scalar(0.1034955) * _tmp33 * fh1 -
      Scalar(12.936937500000001) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp19 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp30 - 1), Scalar(2))));
  const Scalar _tmp35 = _tmp0 * _tmp34;
  const Scalar _tmp36 = Scalar(1.0) * _tmp32;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp40 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp41 = -Scalar(0.010999999999999999) * _tmp14 -
                        Scalar(0.010999999999999999) * _tmp20 + Scalar(-0.010999999999999999);
  const Scalar _tmp42 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp43 = _tmp41 - _tmp42;
  const Scalar _tmp44 = _tmp40 + _tmp43;
  const Scalar _tmp45 = _tmp26 + _tmp27;
  const Scalar _tmp46 = _tmp22 + _tmp45;
  const Scalar _tmp47 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp48 = _tmp47 + Scalar(125.0);
  const Scalar _tmp49 = _tmp13 + _tmp16;
  const Scalar _tmp50 = _tmp49 + _tmp9;
  const Scalar _tmp51 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(-110.0);
  const Scalar _tmp53 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp52, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp54 = _tmp48 * _tmp53;
  const Scalar _tmp55 = -_tmp40;
  const Scalar _tmp56 = _tmp41 + _tmp42;
  const Scalar _tmp57 = _tmp55 + _tmp56;
  const Scalar _tmp58 = _tmp17 + _tmp8;
  const Scalar _tmp59 = _tmp58 + position_vector(1, 0);
  const Scalar _tmp60 = _tmp59 + Scalar(110.0);
  const Scalar _tmp61 = _tmp21 + _tmp28;
  const Scalar _tmp62 = _tmp61 + position_vector(0, 0);
  const Scalar _tmp63 = _tmp62 + Scalar(-125.0);
  const Scalar _tmp64 = std::pow(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp63, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp65 = _tmp60 * _tmp64;
  const Scalar _tmp66 = _tmp49 + _tmp8;
  const Scalar _tmp67 = _tmp66 + position_vector(1, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(-110.0);
  const Scalar _tmp69 = _tmp21 + _tmp45;
  const Scalar _tmp70 = _tmp69 + position_vector(0, 0);
  const Scalar _tmp71 = _tmp70 + Scalar(-125.0);
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = _tmp68 * _tmp72;
  const Scalar _tmp74 = _tmp40 + _tmp56;
  const Scalar _tmp75 = _tmp63 * _tmp64;
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp57 * _tmp65 - _tmp73 * _tmp76;
  const Scalar _tmp78 = Scalar(1.0) / (-_tmp65 + _tmp73 * _tmp75);
  const Scalar _tmp79 = _tmp52 * _tmp53;
  const Scalar _tmp80 = _tmp54 * _tmp73 - _tmp79;
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp54 * _tmp74;
  const Scalar _tmp83 = _tmp44 * _tmp79 - _tmp73 * _tmp82 - _tmp77 * _tmp81;
  const Scalar _tmp84 = Scalar(1.0) * _tmp66;
  const Scalar _tmp85 = -_tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp58 + _tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp69;
  const Scalar _tmp88 = _tmp86 * (-_tmp61 + _tmp87);
  const Scalar _tmp89 = -_tmp57 * _tmp75 + _tmp76;
  const Scalar _tmp90 = -_tmp44 * _tmp54 - _tmp81 * _tmp89 + _tmp82 - _tmp83 * _tmp88;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp84 * _tmp88 + _tmp87;
  const Scalar _tmp93 = 0;
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp68, Scalar(2)) + std::pow(_tmp71, Scalar(2))));
  const Scalar _tmp95 = _tmp72 * _tmp94;
  const Scalar _tmp96 = _tmp19 + Scalar(110.0);
  const Scalar _tmp97 = _tmp30 + Scalar(125.0);
  const Scalar _tmp98 = std::pow(Scalar(std::pow(_tmp96, Scalar(2)) + std::pow(_tmp97, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp99 = _tmp96 * _tmp98;
  const Scalar _tmp100 = _tmp97 * _tmp98;
  const Scalar _tmp101 = -_tmp100 * _tmp18 + _tmp29 * _tmp99;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp103 = _tmp95 * (-_tmp102 * _tmp66 * _tmp71 + _tmp102 * _tmp68 * _tmp69);
  const Scalar _tmp104 = _tmp103 * _tmp75 + _tmp58 * _tmp75 - _tmp61 * _tmp65;
  const Scalar _tmp105 = _tmp103 * _tmp54 - _tmp104 * _tmp81 - _tmp46 * _tmp79 + _tmp50 * _tmp54;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp105);
  const Scalar _tmp107 = Scalar(1.0) * _tmp106;
  const Scalar _tmp108 = Scalar(1.0) * _tmp78;
  const Scalar _tmp109 = _tmp106 * _tmp108 * _tmp80;
  const Scalar _tmp110 = _tmp101 * _tmp95 * (_tmp107 * _tmp54 - _tmp109 * _tmp75);
  const Scalar _tmp111 = _tmp73 * _tmp78;
  const Scalar _tmp112 = _tmp111 * _tmp77 + _tmp73 * _tmp74;
  const Scalar _tmp113 = _tmp111 * _tmp89 - _tmp112 * _tmp88 - _tmp74;
  const Scalar _tmp114 = _tmp105 * _tmp91;
  const Scalar _tmp115 = _tmp106 * _tmp90;
  const Scalar _tmp116 = _tmp115 * (-_tmp103 + _tmp104 * _tmp111 - _tmp113 * _tmp114);
  const Scalar _tmp117 = _tmp91 * (_tmp113 + _tmp116);
  const Scalar _tmp118 = _tmp78 * (-_tmp117 * _tmp80 - _tmp73);
  const Scalar _tmp119 = _tmp100 * _tmp95 * (_tmp117 * _tmp54 + _tmp118 * _tmp75 + Scalar(1.0));
  const Scalar _tmp120 = _tmp108 * _tmp77;
  const Scalar _tmp121 = -_tmp108 * _tmp89 + _tmp120 * _tmp88;
  const Scalar _tmp122 = _tmp115 * (-_tmp104 * _tmp108 - _tmp114 * _tmp121);
  const Scalar _tmp123 = _tmp91 * (_tmp121 + _tmp122);
  const Scalar _tmp124 = _tmp78 * (-_tmp123 * _tmp80 + Scalar(1.0));
  const Scalar _tmp125 = _tmp95 * _tmp99 * (_tmp123 * _tmp54 + _tmp124 * _tmp75);
  const Scalar _tmp126 = -_tmp110 * fh1 - _tmp119 * fh1 - _tmp125 * fh1 -
                         _tmp39 * _tmp95 * (_tmp54 * _tmp93 - _tmp75 * _tmp81 * _tmp93);
  const Scalar _tmp127 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp128 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp129 = _tmp50 + _tmp85;
  const Scalar _tmp130 = _tmp129 * _tmp88;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp46 + _tmp87);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp115 * _tmp132;
  const Scalar _tmp134 = _tmp86 * (-_tmp107 * _tmp83 + _tmp129 * _tmp133);
  const Scalar _tmp135 = Scalar(1.0) * _tmp101;
  const Scalar _tmp136 = _tmp135 * (_tmp133 - Scalar(1.0) * _tmp134);
  const Scalar _tmp137 = _tmp129 * _tmp131;
  const Scalar _tmp138 = -_tmp120 + _tmp122 * _tmp137 - _tmp123 * _tmp83;
  const Scalar _tmp139 = Scalar(1.0) * _tmp86;
  const Scalar _tmp140 = Scalar(1.0) * _tmp99 * (_tmp122 * _tmp132 - _tmp138 * _tmp139);
  const Scalar _tmp141 = _tmp132 * _tmp88;
  const Scalar _tmp142 = _tmp130 * _tmp132 + Scalar(1.0);
  const Scalar _tmp143 = -Scalar(1.0) * _tmp139 * _tmp142 + Scalar(1.0) * _tmp141;
  const Scalar _tmp144 = _tmp43 + _tmp55;
  const Scalar _tmp145 = _tmp144 * _tmp99;
  const Scalar _tmp146 = _tmp129 * _tmp86;
  const Scalar _tmp147 = Scalar(1.0) * _tmp132 * _tmp146 - Scalar(1.0) * _tmp132;
  const Scalar _tmp148 = _tmp100 * _tmp144;
  const Scalar _tmp149 = _tmp112 + _tmp116 * _tmp137 - _tmp117 * _tmp83;
  const Scalar _tmp150 = Scalar(1.0) * _tmp100 * (_tmp116 * _tmp132 - _tmp139 * _tmp149);
  const Scalar _tmp151 = _tmp144 * fh1;
  const Scalar _tmp152 = _tmp100 * _tmp151 + Scalar(2.3544) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp153 = _tmp131 * _tmp92;
  const Scalar _tmp154 = -_tmp129 * _tmp153 - _tmp83 * _tmp93 + _tmp85;
  const Scalar _tmp155 = -Scalar(2.3544) * _tmp12 - _tmp151 * _tmp99 - _tmp18 * fv1;
  const Scalar _tmp156 =
      _tmp136 * fh1 + _tmp140 * fh1 + _tmp143 * _tmp155 + _tmp147 * _tmp152 + _tmp150 * fh1 +
      Scalar(1.0) * _tmp39 * (-_tmp132 * _tmp92 - _tmp139 * _tmp154 + Scalar(1.0));
  const Scalar _tmp157 = std::pow(_tmp126, Scalar(-2));
  const Scalar _tmp158 = -_tmp110 - _tmp119 - _tmp125;
  const Scalar _tmp159 = _tmp157 * _tmp158;
  const Scalar _tmp160 =
      (_tmp128 * (_tmp136 + _tmp140 - _tmp143 * _tmp145 + _tmp147 * _tmp148 + _tmp150) -
       _tmp156 * _tmp159) /
      std::sqrt(Scalar(std::pow(_tmp156, Scalar(2)) * _tmp157 + 1));
  const Scalar _tmp161 = std::asinh(_tmp128 * _tmp156);
  const Scalar _tmp162 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp128;
  const Scalar _tmp164 =
      -_tmp127 * _tmp161 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp67), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp70), Scalar(2))));
  const Scalar _tmp165 = _tmp163 * _tmp164;
  const Scalar _tmp166 = Scalar(1.0) * _tmp161;
  const Scalar _tmp167 = _tmp100 * _tmp118;
  const Scalar _tmp168 = _tmp39 * _tmp93;
  const Scalar _tmp169 = _tmp124 * _tmp99;
  const Scalar _tmp170 = _tmp101 * _tmp109;
  const Scalar _tmp171 = _tmp167 * fh1 - _tmp168 * _tmp81 + _tmp169 * fh1 - _tmp170 * fh1;
  const Scalar _tmp172 = Scalar(1.0) / (_tmp171);
  const Scalar _tmp173 = _tmp100 * _tmp86;
  const Scalar _tmp174 = _tmp149 * _tmp173;
  const Scalar _tmp175 = _tmp132 * _tmp152;
  const Scalar _tmp176 = _tmp142 * _tmp86;
  const Scalar _tmp177 = _tmp138 * _tmp86 * _tmp99;
  const Scalar _tmp178 = _tmp101 * _tmp134;
  const Scalar _tmp179 = -_tmp146 * _tmp175 + _tmp154 * _tmp39 * _tmp86 + _tmp155 * _tmp176 +
                         _tmp174 * fh1 + _tmp177 * fh1 + _tmp178 * fh1;
  const Scalar _tmp180 = std::asinh(_tmp172 * _tmp179);
  const Scalar _tmp181 = Scalar(1.0) * _tmp180;
  const Scalar _tmp182 = std::pow(_tmp171, Scalar(-2));
  const Scalar _tmp183 = _tmp167 + _tmp169 - _tmp170;
  const Scalar _tmp184 = _tmp182 * _tmp183;
  const Scalar _tmp185 = (_tmp172 * (-_tmp129 * _tmp132 * _tmp144 * _tmp173 - _tmp145 * _tmp176 +
                                     _tmp174 + _tmp177 + _tmp178) -
                          _tmp179 * _tmp184) /
                         std::sqrt(Scalar(std::pow(_tmp179, Scalar(2)) * _tmp182 + 1));
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp171;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp172;
  const Scalar _tmp189 =
      -_tmp180 * _tmp186 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp62), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp59 - 1), Scalar(2))));
  const Scalar _tmp190 = _tmp188 * _tmp189;
  const Scalar _tmp191 = _tmp100 * _tmp117;
  const Scalar _tmp192 = _tmp106 * _tmp135;
  const Scalar _tmp193 = _tmp123 * _tmp99;
  const Scalar _tmp194 = _tmp168 + _tmp191 * fh1 + _tmp192 * fh1 + _tmp193 * fh1;
  const Scalar _tmp195 = Scalar(1.0) / (_tmp194);
  const Scalar _tmp196 = _tmp100 * _tmp116 * _tmp131;
  const Scalar _tmp197 = _tmp122 * _tmp131 * _tmp99;
  const Scalar _tmp198 = _tmp101 * _tmp133;
  const Scalar _tmp199 = -_tmp141 * _tmp155 + _tmp153 * _tmp39 + _tmp175 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = std::asinh(_tmp195 * _tmp199);
  const Scalar _tmp201 = Scalar(1.0) * _tmp200;
  const Scalar _tmp202 = Scalar(9.6622558468725703) * _tmp194;
  const Scalar _tmp203 =
      -_tmp200 * _tmp202 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp51), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp47 - 1), Scalar(2))));
  const Scalar _tmp204 = Scalar(0.1034955) * _tmp195;
  const Scalar _tmp205 = _tmp203 * _tmp204;
  const Scalar _tmp206 = _tmp191 + _tmp192 + _tmp193;
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp206;
  const Scalar _tmp208 = std::pow(_tmp194, Scalar(-2));
  const Scalar _tmp209 = _tmp206 * _tmp208;
  const Scalar _tmp210 =
      (_tmp195 * (_tmp132 * _tmp148 + _tmp141 * _tmp145 - _tmp196 - _tmp197 - _tmp198) -
       _tmp199 * _tmp209) /
      std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp208 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp36) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp31 * _tmp38 - _tmp33) -
            _tmp34 * _tmp37) *
               std::cosh(_tmp35)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35) -
      Scalar(9.6622558468725703) * std::sinh(_tmp36);
  _res(1, 0) = _tmp127 * (-Scalar(1.0) * _tmp160 * std::cosh(_tmp166) -
                          (-Scalar(0.1034955) * _tmp159 * _tmp164 +
                           _tmp163 * (-_tmp127 * _tmp160 - _tmp161 * _tmp162)) *
                              std::cosh(_tmp165)) +
               _tmp162 * (-std::sinh(_tmp165) - std::sinh(_tmp166));
  _res(2, 0) = _tmp186 * (-Scalar(1.0) * _tmp185 * std::cosh(_tmp181) -
                          (-Scalar(0.1034955) * _tmp184 * _tmp189 +
                           _tmp188 * (-_tmp180 * _tmp187 - _tmp185 * _tmp186)) *
                              std::cosh(_tmp190)) +
               _tmp187 * (-std::sinh(_tmp181) - std::sinh(_tmp190));
  _res(3, 0) = _tmp202 * (-Scalar(1.0) * _tmp210 * std::cosh(_tmp201) -
                          (-Scalar(0.1034955) * _tmp203 * _tmp209 +
                           _tmp204 * (-_tmp200 * _tmp207 - _tmp202 * _tmp210)) *
                              std::cosh(_tmp205)) +
               _tmp207 * (-std::sinh(_tmp201) - std::sinh(_tmp205));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
