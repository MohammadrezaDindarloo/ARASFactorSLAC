// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl9
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl9(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 594

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp11 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 +
                        Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999);
  const Scalar _tmp13 = 2 * _tmp6;
  const Scalar _tmp14 = _tmp13 * _tmp3;
  const Scalar _tmp15 = _tmp4 * _tmp7;
  const Scalar _tmp16 = _tmp14 - _tmp15;
  const Scalar _tmp17 = Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp17;
  const Scalar _tmp19 = -_tmp12 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp9;
  const Scalar _tmp21 = _tmp20 + position_vector(1, 0);
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp26 = _tmp13 * _tmp4;
  const Scalar _tmp27 = _tmp3 * _tmp7;
  const Scalar _tmp28 = _tmp26 + _tmp27;
  const Scalar _tmp29 = -Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = _tmp25 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(0, 0);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = _tmp12 + _tmp18 + _tmp9;
  const Scalar _tmp35 = _tmp34 + position_vector(1, 0);
  const Scalar _tmp36 = _tmp35 + Scalar(-110.0);
  const Scalar _tmp37 = _tmp22 + _tmp30;
  const Scalar _tmp38 = _tmp37 + position_vector(0, 0);
  const Scalar _tmp39 = _tmp38 + Scalar(-125.0);
  const Scalar _tmp40 =
      std::sqrt(Scalar(std::pow(_tmp36, Scalar(2)) + std::pow(_tmp39, Scalar(2))));
  const Scalar _tmp41 = Scalar(1.0) / (_tmp40);
  const Scalar _tmp42 = Scalar(1.0) / (_tmp39);
  const Scalar _tmp43 = _tmp40 * _tmp42;
  const Scalar _tmp44 = _tmp43 * (-_tmp34 * _tmp39 * _tmp41 + _tmp36 * _tmp37 * _tmp41);
  const Scalar _tmp45 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp46 =
      -Scalar(0.010999999999999999) * _tmp10 - Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp26 - Scalar(0.20999999999999999) * _tmp27;
  const Scalar _tmp48 = _tmp46 - _tmp47;
  const Scalar _tmp49 = _tmp45 + _tmp48;
  const Scalar _tmp50 = -_tmp25 + _tmp29;
  const Scalar _tmp51 = _tmp22 + _tmp50;
  const Scalar _tmp52 = _tmp51 + position_vector(0, 0);
  const Scalar _tmp53 = _tmp52 + Scalar(125.0);
  const Scalar _tmp54 = -_tmp9;
  const Scalar _tmp55 = _tmp12 + _tmp54;
  const Scalar _tmp56 = _tmp18 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 + Scalar(-110.0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp53 * _tmp59;
  const Scalar _tmp61 = _tmp46 + _tmp47;
  const Scalar _tmp62 = _tmp45 + _tmp61;
  const Scalar _tmp63 = -_tmp49 * _tmp60 + _tmp60 * _tmp62;
  const Scalar _tmp64 = _tmp58 * _tmp59;
  const Scalar _tmp65 = _tmp36 * _tmp42;
  const Scalar _tmp66 = Scalar(1.0) / (_tmp60 * _tmp65 - _tmp64);
  const Scalar _tmp67 = _tmp65 * _tmp66;
  const Scalar _tmp68 = _tmp62 * _tmp65;
  const Scalar _tmp69 = _tmp49 * _tmp64 - _tmp60 * _tmp68;
  const Scalar _tmp70 = _tmp67 * _tmp69 + _tmp68;
  const Scalar _tmp71 = Scalar(1.0) * _tmp34;
  const Scalar _tmp72 = -_tmp71;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp56 + _tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp37;
  const Scalar _tmp75 = _tmp73 * (-_tmp51 + _tmp74);
  const Scalar _tmp76 = -_tmp62 + _tmp63 * _tmp67 - _tmp70 * _tmp75;
  const Scalar _tmp77 = -_tmp45;
  const Scalar _tmp78 = _tmp48 + _tmp77;
  const Scalar _tmp79 = _tmp19 + _tmp54;
  const Scalar _tmp80 = _tmp79 + position_vector(1, 0);
  const Scalar _tmp81 = _tmp80 + Scalar(110.0);
  const Scalar _tmp82 = _tmp23 + _tmp50;
  const Scalar _tmp83 = _tmp82 + position_vector(0, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(125.0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp81 * _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp85;
  const Scalar _tmp88 = _tmp65 * _tmp87 - _tmp86;
  const Scalar _tmp89 = _tmp66 * _tmp88;
  const Scalar _tmp90 = -_tmp68 * _tmp87 - _tmp69 * _tmp89 + _tmp78 * _tmp86;
  const Scalar _tmp91 = _tmp62 * _tmp87 - _tmp63 * _tmp89 - _tmp75 * _tmp90 - _tmp78 * _tmp87;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp44 * _tmp60 - _tmp51 * _tmp64 + _tmp56 * _tmp60;
  const Scalar _tmp94 = _tmp44 * _tmp87 + _tmp79 * _tmp87 - _tmp82 * _tmp86 - _tmp89 * _tmp93;
  const Scalar _tmp95 = _tmp92 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp97 = _tmp91 * _tmp96;
  const Scalar _tmp98 = _tmp97 * (-_tmp44 + _tmp67 * _tmp93 - _tmp76 * _tmp95);
  const Scalar _tmp99 = _tmp72 + _tmp79;
  const Scalar _tmp100 = _tmp75 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (-_tmp100 + _tmp74 - _tmp82);
  const Scalar _tmp102 = _tmp101 * _tmp99;
  const Scalar _tmp103 = _tmp92 * (_tmp76 + _tmp98);
  const Scalar _tmp104 = _tmp102 * _tmp98 - _tmp103 * _tmp90 + _tmp70;
  const Scalar _tmp105 = Scalar(1.0) * _tmp73;
  const Scalar _tmp106 = Scalar(1.0) * _tmp101;
  const Scalar _tmp107 = _tmp21 + Scalar(110.0);
  const Scalar _tmp108 = _tmp32 + Scalar(-125.0);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp108 * _tmp109;
  const Scalar _tmp111 = _tmp110 * fh1;
  const Scalar _tmp112 = Scalar(1.0) * _tmp66;
  const Scalar _tmp113 = _tmp112 * _tmp69;
  const Scalar _tmp114 = -_tmp112 * _tmp63 + _tmp113 * _tmp75;
  const Scalar _tmp115 = _tmp97 * (-_tmp112 * _tmp93 - _tmp114 * _tmp95);
  const Scalar _tmp116 = _tmp114 + _tmp115;
  const Scalar _tmp117 = _tmp90 * _tmp92;
  const Scalar _tmp118 = _tmp102 * _tmp115 - _tmp113 - _tmp116 * _tmp117;
  const Scalar _tmp119 = _tmp107 * _tmp109;
  const Scalar _tmp120 = _tmp119 * fh1;
  const Scalar _tmp121 = fh1 * (_tmp61 + _tmp77);
  const Scalar _tmp122 = _tmp110 * _tmp121 + Scalar(2.3544) * _tmp28 + _tmp31 * fv1;
  const Scalar _tmp123 = _tmp73 * _tmp99;
  const Scalar _tmp124 = Scalar(1.0) * _tmp106 * _tmp123 - Scalar(1.0) * _tmp106;
  const Scalar _tmp125 = -_tmp119 * _tmp121 - Scalar(2.3544) * _tmp16 - _tmp20 * fv1;
  const Scalar _tmp126 = _tmp73 * (_tmp100 * _tmp106 + Scalar(1.0));
  const Scalar _tmp127 = _tmp106 * _tmp75;
  const Scalar _tmp128 = -Scalar(1.0) * _tmp126 + Scalar(1.0) * _tmp127;
  const Scalar _tmp129 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp130 = _tmp71 * _tmp75 + _tmp74;
  const Scalar _tmp131 = _tmp101 * _tmp130;
  const Scalar _tmp132 = 0;
  const Scalar _tmp133 = _tmp73 * (-_tmp117 * _tmp132 - _tmp131 * _tmp99 + _tmp72);
  const Scalar _tmp134 = -Scalar(1.0) * _tmp131 - Scalar(1.0) * _tmp133 + Scalar(1.0);
  const Scalar _tmp135 = _tmp106 * _tmp97;
  const Scalar _tmp136 = Scalar(1.0) * _tmp96;
  const Scalar _tmp137 = _tmp135 * _tmp99 - _tmp136 * _tmp90;
  const Scalar _tmp138 = fh1 * (-_tmp110 * _tmp20 + _tmp119 * _tmp31);
  const Scalar _tmp139 = Scalar(1.0) * _tmp111 * (-_tmp104 * _tmp105 + _tmp106 * _tmp98) +
                         Scalar(1.0) * _tmp120 * (-_tmp105 * _tmp118 + _tmp106 * _tmp115) +
                         _tmp122 * _tmp124 + _tmp125 * _tmp128 + _tmp129 * _tmp134 +
                         Scalar(1.0) * _tmp138 * (-_tmp105 * _tmp137 + _tmp135);
  const Scalar _tmp140 = _tmp132 * _tmp92;
  const Scalar _tmp141 = _tmp60 * _tmp89;
  const Scalar _tmp142 = _tmp43 * (-_tmp140 * _tmp141 + _tmp140 * _tmp87);
  const Scalar _tmp143 = _tmp116 * _tmp92;
  const Scalar _tmp144 = -_tmp143 * _tmp88 + Scalar(1.0);
  const Scalar _tmp145 = _tmp60 * _tmp66;
  const Scalar _tmp146 = -_tmp103 * _tmp88 - _tmp65;
  const Scalar _tmp147 = -_tmp111 * _tmp43 * (_tmp103 * _tmp87 + _tmp145 * _tmp146 + Scalar(1.0)) -
                         _tmp120 * _tmp43 * (_tmp143 * _tmp87 + _tmp144 * _tmp145) -
                         _tmp129 * _tmp142 -
                         _tmp138 * _tmp43 * (-_tmp136 * _tmp141 + _tmp136 * _tmp87);
  const Scalar _tmp148 = Scalar(1.0) / (_tmp147);
  const Scalar _tmp149 = std::asinh(_tmp139 * _tmp148);
  const Scalar _tmp150 = Scalar(1.0) * _tmp149;
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp147;
  const Scalar _tmp152 =
      -_tmp149 * _tmp151 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp35), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp38), Scalar(2))));
  const Scalar _tmp153 = Scalar(0.1034955) * _tmp148;
  const Scalar _tmp154 = _tmp152 * _tmp153;
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp142;
  const Scalar _tmp156 = std::pow(_tmp147, Scalar(-2));
  const Scalar _tmp157 = _tmp17 + _tmp55;
  const Scalar _tmp158 = _tmp142 * _tmp156;
  const Scalar _tmp159 =
      (-_tmp139 * _tmp158 + _tmp148 * (_tmp124 * _tmp31 + _tmp128 * _tmp157 - _tmp134)) /
      std::sqrt(Scalar(std::pow(_tmp139, Scalar(2)) * _tmp156 + 1));
  const Scalar _tmp160 = _tmp136 * _tmp138;
  const Scalar _tmp161 = _tmp129 * _tmp140;
  const Scalar _tmp162 =
      _tmp111 * _tmp146 * _tmp66 + _tmp120 * _tmp144 * _tmp66 - _tmp160 * _tmp89 - _tmp161 * _tmp89;
  const Scalar _tmp163 = Scalar(1.0) / (_tmp162);
  const Scalar _tmp164 = _tmp106 * _tmp122;
  const Scalar _tmp165 = _tmp104 * _tmp111 * _tmp73 + _tmp118 * _tmp120 * _tmp73 -
                         _tmp123 * _tmp164 + _tmp125 * _tmp126 + _tmp129 * _tmp133 +
                         _tmp137 * _tmp138 * _tmp73;
  const Scalar _tmp166 = std::asinh(_tmp163 * _tmp165);
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp162;
  const Scalar _tmp168 =
      -_tmp166 * _tmp167 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp57), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp52 - 1), Scalar(2))));
  const Scalar _tmp169 = Scalar(0.1034955) * _tmp163;
  const Scalar _tmp170 = _tmp168 * _tmp169;
  const Scalar _tmp171 = Scalar(1.0) * _tmp166;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp140;
  const Scalar _tmp173 = _tmp172 * _tmp89;
  const Scalar _tmp174 = std::pow(_tmp162, Scalar(-2));
  const Scalar _tmp175 = _tmp106 * _tmp31;
  const Scalar _tmp176 = _tmp140 * _tmp174 * _tmp89;
  const Scalar _tmp177 =
      (_tmp163 * (-_tmp123 * _tmp175 + _tmp126 * _tmp157 - _tmp133) - _tmp165 * _tmp176) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp174 + 1));
  const Scalar _tmp178 = -_tmp101 * _tmp111 * _tmp98 - _tmp101 * _tmp115 * _tmp120 -
                         _tmp125 * _tmp127 + _tmp129 * _tmp131 - _tmp135 * _tmp138 + _tmp164;
  const Scalar _tmp179 = _tmp103 * _tmp111 + _tmp120 * _tmp143 + _tmp160 + _tmp161;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = std::asinh(_tmp178 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp184 =
      -_tmp181 * _tmp183 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp80 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp83 - 1), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp188 = _tmp140 * _tmp187;
  const Scalar _tmp189 = (_tmp178 * _tmp188 + _tmp180 * (-_tmp127 * _tmp157 - _tmp131 + _tmp175)) /
                         std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp187 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp32),
                                           Scalar(2)) +
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp21 - 1),
                                               Scalar(2)))))));
  _res(1, 0) = _tmp151 * (-Scalar(1.0) * _tmp159 * std::cosh(_tmp150) -
                          (-Scalar(0.1034955) * _tmp152 * _tmp158 +
                           _tmp153 * (-_tmp149 * _tmp155 - _tmp151 * _tmp159)) *
                              std::cosh(_tmp154)) +
               _tmp155 * (-std::sinh(_tmp150) - std::sinh(_tmp154));
  _res(2, 0) = _tmp167 * (-Scalar(1.0) * _tmp177 * std::cosh(_tmp171) -
                          (-Scalar(0.1034955) * _tmp168 * _tmp176 +
                           _tmp169 * (-_tmp166 * _tmp173 - _tmp167 * _tmp177)) *
                              std::cosh(_tmp170)) +
               _tmp173 * (-std::sinh(_tmp170) - std::sinh(_tmp171));
  _res(3, 0) = -_tmp172 * (-std::sinh(_tmp182) - std::sinh(_tmp186)) +
               _tmp183 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp182) -
                          (Scalar(0.1034955) * _tmp184 * _tmp188 +
                           _tmp185 * (_tmp172 * _tmp181 - _tmp183 * _tmp189)) *
                              std::cosh(_tmp186));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
