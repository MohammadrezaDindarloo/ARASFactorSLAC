// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl20
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl20(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 655

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (217)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = 2 * _tmp1 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp5 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = 2 * _tmp4;
  const Scalar _tmp9 = _tmp2 * _tmp8;
  const Scalar _tmp10 = _tmp1 * _tmp5;
  const Scalar _tmp11 = _tmp10 + _tmp9;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp14 = 1 - 2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp7;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp1 * _tmp8;
  const Scalar _tmp24 = _tmp2 * _tmp5;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = _tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(1, 0);
  const Scalar _tmp30 = _tmp0 * fv1;
  const Scalar _tmp31 = std::asinh(_tmp30);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * _tmp31;
  const Scalar _tmp33 =
      -_tmp32 * fh1 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp29), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp18 - 1), Scalar(2))));
  const Scalar _tmp34 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp35 = _tmp33 * _tmp34;
  const Scalar _tmp36 = Scalar(1.0) * _tmp31;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = -_tmp7;
  const Scalar _tmp40 = _tmp12 + _tmp15;
  const Scalar _tmp41 = _tmp39 + _tmp40;
  const Scalar _tmp42 = Scalar(1.0) * _tmp41;
  const Scalar _tmp43 = -_tmp22 + _tmp26;
  const Scalar _tmp44 = _tmp19 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp20 + _tmp43;
  const Scalar _tmp48 = Scalar(1.0) / (_tmp46 + _tmp47);
  const Scalar _tmp49 = _tmp16 + _tmp39;
  const Scalar _tmp50 = _tmp48 * (_tmp42 - _tmp49);
  const Scalar _tmp51 = _tmp42 + _tmp45 * _tmp50;
  const Scalar _tmp52 = _tmp19 + _tmp27;
  const Scalar _tmp53 = _tmp46 + _tmp52;
  const Scalar _tmp54 = _tmp50 * _tmp53;
  const Scalar _tmp55 = _tmp40 + _tmp7;
  const Scalar _tmp56 = Scalar(1.0) / (_tmp42 - _tmp54 - _tmp55);
  const Scalar _tmp57 = Scalar(1.0) * _tmp56;
  const Scalar _tmp58 = _tmp51 * _tmp56;
  const Scalar _tmp59 = 0;
  const Scalar _tmp60 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp61 = -Scalar(0.010999999999999999) * _tmp13 -
                        Scalar(0.010999999999999999) * _tmp21 + Scalar(-0.010999999999999999);
  const Scalar _tmp62 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp63 = _tmp61 + _tmp62;
  const Scalar _tmp64 = _tmp60 + _tmp63;
  const Scalar _tmp65 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-110.0);
  const Scalar _tmp67 = _tmp55 + position_vector(0, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(-125.0);
  const Scalar _tmp69 = std::pow(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp68, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp70 = _tmp68 * _tmp69;
  const Scalar _tmp71 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(-125.0);
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp75 = _tmp74 + Scalar(110.0);
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp61 - _tmp62;
  const Scalar _tmp78 = _tmp60 + _tmp77;
  const Scalar _tmp79 = _tmp70 * _tmp78;
  const Scalar _tmp80 = -_tmp60;
  const Scalar _tmp81 = _tmp77 + _tmp80;
  const Scalar _tmp82 = _tmp47 + position_vector(1, 0);
  const Scalar _tmp83 = _tmp82 + Scalar(110.0);
  const Scalar _tmp84 = _tmp49 + position_vector(0, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(125.0);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp83, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp83 * _tmp86;
  const Scalar _tmp88 = _tmp85 * _tmp86;
  const Scalar _tmp89 = _tmp78 * _tmp88;
  const Scalar _tmp90 = -_tmp76 * _tmp89 + _tmp81 * _tmp87;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp76 * _tmp88 - _tmp87);
  const Scalar _tmp92 = _tmp66 * _tmp69;
  const Scalar _tmp93 = _tmp70 * _tmp76 - _tmp92;
  const Scalar _tmp94 = _tmp91 * _tmp93;
  const Scalar _tmp95 = _tmp64 * _tmp92 - _tmp76 * _tmp79 - _tmp90 * _tmp94;
  const Scalar _tmp96 = -_tmp81 * _tmp88 + _tmp89;
  const Scalar _tmp97 = -_tmp50 * _tmp95 - _tmp64 * _tmp70 + _tmp79 - _tmp94 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp99 = _tmp95 * _tmp98;
  const Scalar _tmp100 = _tmp48 * (_tmp46 - _tmp53 * _tmp58 - _tmp59 * _tmp99);
  const Scalar _tmp101 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp102 = _tmp18 + Scalar(125.0);
  const Scalar _tmp103 = _tmp29 + Scalar(-110.0);
  const Scalar _tmp104 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp103, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp105 = _tmp103 * _tmp104;
  const Scalar _tmp106 = _tmp63 + _tmp80;
  const Scalar _tmp107 = _tmp106 * fh1;
  const Scalar _tmp108 = -_tmp105 * _tmp107 - Scalar(2.3544) * _tmp25 - _tmp28 * fv1;
  const Scalar _tmp109 = _tmp50 * _tmp57;
  const Scalar _tmp110 = _tmp48 * (_tmp54 * _tmp57 + Scalar(1.0));
  const Scalar _tmp111 = Scalar(1.0) * _tmp109 - Scalar(1.0) * _tmp110;
  const Scalar _tmp112 =
      std::sqrt(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp75, Scalar(2))));
  const Scalar _tmp113 = Scalar(1.0) / (_tmp112);
  const Scalar _tmp114 = _tmp112 * _tmp73;
  const Scalar _tmp115 = _tmp114 * (_tmp113 * _tmp41 * _tmp75 - _tmp113 * _tmp44 * _tmp72);
  const Scalar _tmp116 = _tmp115 * _tmp88 + _tmp47 * _tmp88 - _tmp49 * _tmp87;
  const Scalar _tmp117 = Scalar(1.0) * _tmp91;
  const Scalar _tmp118 = _tmp117 * _tmp90;
  const Scalar _tmp119 = -_tmp117 * _tmp96 + _tmp118 * _tmp50;
  const Scalar _tmp120 = _tmp115 * _tmp70 - _tmp116 * _tmp94 + _tmp52 * _tmp70 - _tmp55 * _tmp92;
  const Scalar _tmp121 = _tmp120 * _tmp98;
  const Scalar _tmp122 = Scalar(1.0) / (_tmp120);
  const Scalar _tmp123 = _tmp122 * _tmp97;
  const Scalar _tmp124 = _tmp123 * (-_tmp116 * _tmp117 - _tmp119 * _tmp121);
  const Scalar _tmp125 = _tmp53 * _tmp56;
  const Scalar _tmp126 = _tmp119 + _tmp124;
  const Scalar _tmp127 = -_tmp118 + _tmp124 * _tmp125 - _tmp126 * _tmp99;
  const Scalar _tmp128 = Scalar(1.0) * _tmp48;
  const Scalar _tmp129 = Scalar(1.0) * _tmp105 * (_tmp124 * _tmp57 - _tmp127 * _tmp128);
  const Scalar _tmp130 = _tmp76 * _tmp91;
  const Scalar _tmp131 = _tmp130 * _tmp90 + _tmp76 * _tmp78;
  const Scalar _tmp132 = _tmp130 * _tmp96 - _tmp131 * _tmp50 - _tmp78;
  const Scalar _tmp133 = _tmp123 * (-_tmp115 + _tmp116 * _tmp130 - _tmp121 * _tmp132);
  const Scalar _tmp134 = _tmp132 + _tmp133;
  const Scalar _tmp135 = _tmp125 * _tmp133 + _tmp131 - _tmp134 * _tmp99;
  const Scalar _tmp136 = _tmp102 * _tmp104;
  const Scalar _tmp137 = Scalar(1.0) * _tmp136 * (-_tmp128 * _tmp135 + _tmp133 * _tmp57);
  const Scalar _tmp138 = _tmp107 * _tmp136 + Scalar(2.3544) * _tmp11 + _tmp17 * fv1;
  const Scalar _tmp139 = _tmp48 * _tmp53;
  const Scalar _tmp140 = Scalar(1.0) * _tmp139 * _tmp57 - Scalar(1.0) * _tmp57;
  const Scalar _tmp141 = _tmp123 * _tmp57;
  const Scalar _tmp142 = Scalar(1.0) * _tmp122;
  const Scalar _tmp143 = _tmp141 * _tmp53 - _tmp142 * _tmp95;
  const Scalar _tmp144 = _tmp105 * _tmp17 - _tmp136 * _tmp28;
  const Scalar _tmp145 = Scalar(1.0) * _tmp144;
  const Scalar _tmp146 = _tmp145 * (-_tmp128 * _tmp143 + _tmp141);
  const Scalar _tmp147 =
      Scalar(1.0) * _tmp101 * (-Scalar(1.0) * _tmp100 - _tmp51 * _tmp57 + Scalar(1.0)) +
      _tmp108 * _tmp111 + _tmp129 * fh1 + _tmp137 * fh1 + _tmp138 * _tmp140 + _tmp146 * fh1;
  const Scalar _tmp148 = _tmp93 * _tmp98;
  const Scalar _tmp149 = -_tmp134 * _tmp148 - _tmp76;
  const Scalar _tmp150 = _tmp88 * _tmp91;
  const Scalar _tmp151 = _tmp134 * _tmp98;
  const Scalar _tmp152 = _tmp114 * _tmp136 * (_tmp149 * _tmp150 + _tmp151 * _tmp70 + Scalar(1.0));
  const Scalar _tmp153 = _tmp59 * _tmp98;
  const Scalar _tmp154 = _tmp88 * _tmp94;
  const Scalar _tmp155 = -_tmp126 * _tmp148 + Scalar(1.0);
  const Scalar _tmp156 = _tmp126 * _tmp98;
  const Scalar _tmp157 = _tmp105 * _tmp114 * (_tmp150 * _tmp155 + _tmp156 * _tmp70);
  const Scalar _tmp158 = _tmp114 * _tmp144 * (-_tmp142 * _tmp154 + _tmp142 * _tmp70);
  const Scalar _tmp159 = -_tmp101 * _tmp114 * (-_tmp153 * _tmp154 + _tmp153 * _tmp70) -
                         _tmp152 * fh1 - _tmp157 * fh1 - _tmp158 * fh1;
  const Scalar _tmp160 = Scalar(1.0) / (_tmp159);
  const Scalar _tmp161 = std::asinh(_tmp147 * _tmp160);
  const Scalar _tmp162 = Scalar(1.0) * _tmp161;
  const Scalar _tmp163 = std::pow(_tmp159, Scalar(-2));
  const Scalar _tmp164 = _tmp105 * _tmp106;
  const Scalar _tmp165 = _tmp106 * _tmp136;
  const Scalar _tmp166 = -_tmp152 - _tmp157 - _tmp158;
  const Scalar _tmp167 = _tmp163 * _tmp166;
  const Scalar _tmp168 = (-_tmp147 * _tmp167 + _tmp160 * (-_tmp111 * _tmp164 + _tmp129 + _tmp137 +
                                                          _tmp140 * _tmp165 + _tmp146)) /
                         std::sqrt(Scalar(std::pow(_tmp147, Scalar(2)) * _tmp163 + 1));
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp166;
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp160;
  const Scalar _tmp172 =
      -_tmp161 * _tmp170 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp71), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp74 - 1), Scalar(2))));
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = _tmp136 * _tmp149 * _tmp91;
  const Scalar _tmp175 = _tmp101 * _tmp153;
  const Scalar _tmp176 = _tmp122 * _tmp145;
  const Scalar _tmp177 = _tmp176 * fh1;
  const Scalar _tmp178 = _tmp105 * _tmp155 * _tmp91;
  const Scalar _tmp179 = _tmp174 * fh1 - _tmp175 * _tmp94 - _tmp177 * _tmp94 + _tmp178 * fh1;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = _tmp105 * _tmp127 * _tmp48;
  const Scalar _tmp182 = _tmp136 * _tmp48;
  const Scalar _tmp183 = _tmp135 * _tmp182;
  const Scalar _tmp184 = _tmp143 * _tmp144 * _tmp48;
  const Scalar _tmp185 = _tmp138 * _tmp57;
  const Scalar _tmp186 = _tmp100 * _tmp101 + _tmp108 * _tmp110 - _tmp139 * _tmp185 + _tmp181 * fh1 +
                         _tmp183 * fh1 + _tmp184 * fh1;
  const Scalar _tmp187 = std::asinh(_tmp180 * _tmp186);
  const Scalar _tmp188 = Scalar(1.0) * _tmp187;
  const Scalar _tmp189 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp190 =
      -_tmp187 * _tmp189 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp82 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp84 - 1), Scalar(2))));
  const Scalar _tmp191 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp192 = _tmp190 * _tmp191;
  const Scalar _tmp193 = _tmp174 - _tmp176 * _tmp94 + _tmp178;
  const Scalar _tmp194 = Scalar(9.6622558468725703) * _tmp193;
  const Scalar _tmp195 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp196 = _tmp193 * _tmp195;
  const Scalar _tmp197 = (_tmp180 * (-_tmp106 * _tmp182 * _tmp53 * _tmp57 - _tmp110 * _tmp164 +
                                     _tmp181 + _tmp183 + _tmp184) -
                          _tmp186 * _tmp196) /
                         std::sqrt(Scalar(std::pow(_tmp186, Scalar(2)) * _tmp195 + 1));
  const Scalar _tmp198 = _tmp105 * _tmp156;
  const Scalar _tmp199 = _tmp136 * _tmp151;
  const Scalar _tmp200 = _tmp175 + _tmp177 + _tmp198 * fh1 + _tmp199 * fh1;
  const Scalar _tmp201 = Scalar(1.0) / (_tmp200);
  const Scalar _tmp202 = _tmp133 * _tmp136 * _tmp56;
  const Scalar _tmp203 = _tmp105 * _tmp124 * _tmp56;
  const Scalar _tmp204 = _tmp141 * _tmp144;
  const Scalar _tmp205 = _tmp101 * _tmp58 - _tmp108 * _tmp109 + _tmp185 - _tmp202 * fh1 -
                         _tmp203 * fh1 - _tmp204 * fh1;
  const Scalar _tmp206 = std::asinh(_tmp201 * _tmp205);
  const Scalar _tmp207 = Scalar(1.0) * _tmp206;
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp200;
  const Scalar _tmp209 =
      -_tmp206 * _tmp208 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp65), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp67), Scalar(2))));
  const Scalar _tmp210 = Scalar(0.1034955) * _tmp201;
  const Scalar _tmp211 = _tmp209 * _tmp210;
  const Scalar _tmp212 = _tmp176 + _tmp198 + _tmp199;
  const Scalar _tmp213 = Scalar(9.6622558468725703) * _tmp212;
  const Scalar _tmp214 = std::pow(_tmp200, Scalar(-2));
  const Scalar _tmp215 = _tmp212 * _tmp214;
  const Scalar _tmp216 =
      (_tmp201 * (_tmp109 * _tmp164 + _tmp165 * _tmp57 - _tmp202 - _tmp203 - _tmp204) -
       _tmp205 * _tmp215) /
      std::sqrt(Scalar(std::pow(_tmp205, Scalar(2)) * _tmp214 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(48.000000000000007) * _tmp0 -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::sinh(_tmp36) - Scalar(4.967784) * _tmp37 -
           (-Scalar(0.1034955) * _tmp33 * _tmp37 +
            _tmp34 * (Scalar(9.6622558468725703) * _tmp30 * _tmp38 - _tmp32)) *
               std::sinh(_tmp35)) +
      Scalar(9.6622558468725703) * std::cosh(_tmp35) -
      Scalar(9.6622558468725703) * std::cosh(_tmp36);
  _res(1, 0) = -_tmp169 * (Scalar(4.967784) * _tmp160 + std::cosh(_tmp162) - std::cosh(_tmp173)) -
               _tmp170 * (-Scalar(4.967784) * _tmp167 + Scalar(1.0) * _tmp168 * std::sinh(_tmp162) -
                          (-Scalar(0.1034955) * _tmp167 * _tmp172 +
                           _tmp171 * (-_tmp161 * _tmp169 - _tmp168 * _tmp170)) *
                              std::sinh(_tmp173));
  _res(2, 0) =
      -_tmp189 * (-Scalar(4.967784) * _tmp196 + Scalar(1.0) * _tmp197 * std::sinh(_tmp188) -
                  (-Scalar(0.1034955) * _tmp190 * _tmp196 +
                   _tmp191 * (-_tmp187 * _tmp194 - _tmp189 * _tmp197)) *
                      std::sinh(_tmp192)) -
      _tmp194 * (Scalar(4.967784) * _tmp180 + std::cosh(_tmp188) - std::cosh(_tmp192));
  _res(3, 0) =
      -_tmp208 * (-Scalar(4.967784) * _tmp215 + Scalar(1.0) * _tmp216 * std::sinh(_tmp207) -
                  (-Scalar(0.1034955) * _tmp209 * _tmp215 +
                   _tmp210 * (-_tmp206 * _tmp213 - _tmp208 * _tmp216)) *
                      std::sinh(_tmp211)) -
      _tmp213 * (Scalar(4.967784) * _tmp201 + std::cosh(_tmp207) - std::cosh(_tmp211));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
