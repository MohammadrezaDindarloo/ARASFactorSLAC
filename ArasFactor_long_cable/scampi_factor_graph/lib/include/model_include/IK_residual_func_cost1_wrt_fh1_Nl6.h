// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 656

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (216)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp3 = 2 * _tmp2;
  const Scalar _tmp4 = _tmp1 * _tmp3;
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp7 = _tmp5 * _tmp6;
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp10 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp11 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp12 = _tmp3 * _tmp5;
  const Scalar _tmp13 = _tmp1 * _tmp6;
  const Scalar _tmp14 = _tmp12 - _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp11 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp8;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp19;
  const Scalar _tmp21 = 2 * _tmp1 * _tmp5;
  const Scalar _tmp22 = _tmp2 * _tmp6;
  const Scalar _tmp23 = _tmp21 + _tmp22;
  const Scalar _tmp24 = -Scalar(0.010999999999999999) * _tmp23;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp4 - Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(0, 0);
  const Scalar _tmp29 = _tmp0 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -_tmp31 * fh1 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp28), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp18 - 1), Scalar(2))));
  const Scalar _tmp33 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp34 = _tmp32 * _tmp33;
  const Scalar _tmp35 = Scalar(1.0) * _tmp30;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = -_tmp20;
  const Scalar _tmp39 = _tmp26 + _tmp38;
  const Scalar _tmp40 = Scalar(1.0) * _tmp39;
  const Scalar _tmp41 = _tmp24 + _tmp25;
  const Scalar _tmp42 = _tmp20 + _tmp41;
  const Scalar _tmp43 = -_tmp8;
  const Scalar _tmp44 = _tmp16 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp11 + _tmp15;
  const Scalar _tmp48 = _tmp47 + _tmp8;
  const Scalar _tmp49 = Scalar(1.0) / (_tmp46 + _tmp48);
  const Scalar _tmp50 = _tmp49 * (_tmp40 - _tmp42);
  const Scalar _tmp51 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp52 = -Scalar(0.010999999999999999) * _tmp19 -
                        Scalar(0.010999999999999999) * _tmp9 + Scalar(-0.010999999999999999);
  const Scalar _tmp53 = Scalar(0.20999999999999999) * _tmp21 - Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp54 = _tmp52 + _tmp53;
  const Scalar _tmp55 = _tmp51 + _tmp54;
  const Scalar _tmp56 = _tmp48 + position_vector(1, 0);
  const Scalar _tmp57 = _tmp56 + Scalar(-110.0);
  const Scalar _tmp58 = _tmp42 + position_vector(0, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(-125.0);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp57, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp57 * _tmp60;
  const Scalar _tmp62 = -_tmp51;
  const Scalar _tmp63 = _tmp52 - _tmp53;
  const Scalar _tmp64 = _tmp62 + _tmp63;
  const Scalar _tmp65 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(110.0);
  const Scalar _tmp67 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(125.0);
  const Scalar _tmp69 = Scalar(1.0) / (_tmp68);
  const Scalar _tmp70 = _tmp66 * _tmp69;
  const Scalar _tmp71 = _tmp64 * _tmp70;
  const Scalar _tmp72 = _tmp59 * _tmp60;
  const Scalar _tmp73 = _tmp55 * _tmp61 - _tmp71 * _tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (-_tmp61 + _tmp70 * _tmp72);
  const Scalar _tmp75 = Scalar(1.0) * _tmp74;
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = -_tmp55 * _tmp72 + _tmp64 * _tmp72;
  const Scalar _tmp78 = _tmp50 * _tmp76 - _tmp75 * _tmp77;
  const Scalar _tmp79 = _tmp51 + _tmp63;
  const Scalar _tmp80 = _tmp38 + _tmp41;
  const Scalar _tmp81 = _tmp80 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(125.0);
  const Scalar _tmp83 = _tmp43 + _tmp47;
  const Scalar _tmp84 = _tmp83 + position_vector(1, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(-110.0);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp82 * _tmp86;
  const Scalar _tmp88 = _tmp85 * _tmp86;
  const Scalar _tmp89 = _tmp70 * _tmp87 - _tmp88;
  const Scalar _tmp90 = _tmp74 * _tmp89;
  const Scalar _tmp91 = -_tmp71 * _tmp87 - _tmp73 * _tmp90 + _tmp79 * _tmp88;
  const Scalar _tmp92 = -_tmp50 * _tmp91 + _tmp64 * _tmp87 - _tmp77 * _tmp90 - _tmp79 * _tmp87;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp68, Scalar(2))));
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 = _tmp69 * _tmp94;
  const Scalar _tmp97 = _tmp96 * (_tmp39 * _tmp66 * _tmp95 - _tmp44 * _tmp68 * _tmp95);
  const Scalar _tmp98 = -_tmp42 * _tmp61 + _tmp48 * _tmp72 + _tmp72 * _tmp97;
  const Scalar _tmp99 = -_tmp80 * _tmp88 + _tmp83 * _tmp87 + _tmp87 * _tmp97 - _tmp90 * _tmp98;
  const Scalar _tmp100 = _tmp93 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp102 = _tmp101 * _tmp92;
  const Scalar _tmp103 = _tmp102 * (-_tmp100 * _tmp78 - _tmp75 * _tmp98);
  const Scalar _tmp104 = _tmp103 + _tmp78;
  const Scalar _tmp105 = _tmp87 * _tmp93;
  const Scalar _tmp106 = _tmp89 * _tmp93;
  const Scalar _tmp107 = -_tmp104 * _tmp106 + Scalar(1.0);
  const Scalar _tmp108 = _tmp72 * _tmp74;
  const Scalar _tmp109 = _tmp18 + Scalar(110.0);
  const Scalar _tmp110 = _tmp28 + Scalar(-125.0);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp109 * _tmp111;
  const Scalar _tmp113 = _tmp112 * _tmp96 * (_tmp104 * _tmp105 + _tmp107 * _tmp108);
  const Scalar _tmp114 = _tmp110 * _tmp111;
  const Scalar _tmp115 = _tmp112 * _tmp27 - _tmp114 * _tmp17;
  const Scalar _tmp116 = Scalar(1.0) * _tmp101;
  const Scalar _tmp117 = _tmp72 * _tmp90;
  const Scalar _tmp118 = _tmp115 * _tmp96 * (-_tmp116 * _tmp117 + _tmp116 * _tmp87);
  const Scalar _tmp119 = _tmp70 * _tmp74;
  const Scalar _tmp120 = _tmp119 * _tmp73 + _tmp71;
  const Scalar _tmp121 = _tmp119 * _tmp77 - _tmp120 * _tmp50 - _tmp64;
  const Scalar _tmp122 = _tmp102 * (-_tmp100 * _tmp121 + _tmp119 * _tmp98 - _tmp97);
  const Scalar _tmp123 = _tmp121 + _tmp122;
  const Scalar _tmp124 = -_tmp106 * _tmp123 - _tmp70;
  const Scalar _tmp125 = _tmp114 * _tmp96 * (_tmp105 * _tmp123 + _tmp108 * _tmp124 + Scalar(1.0));
  const Scalar _tmp126 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp127 = _tmp40 + _tmp45 * _tmp50;
  const Scalar _tmp128 = 0;
  const Scalar _tmp129 = _tmp128 * _tmp93;
  const Scalar _tmp130 = -_tmp113 * fh1 - _tmp118 * fh1 - _tmp125 * fh1 -
                         _tmp126 * _tmp96 * (-_tmp117 * _tmp129 + _tmp129 * _tmp87);
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp46 + _tmp83;
  const Scalar _tmp133 = _tmp132 * _tmp50;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp40 - _tmp80);
  const Scalar _tmp135 = _tmp132 * _tmp134;
  const Scalar _tmp136 = _tmp91 * _tmp93;
  const Scalar _tmp137 = _tmp120 + _tmp122 * _tmp135 - _tmp123 * _tmp136;
  const Scalar _tmp138 = Scalar(1.0) * _tmp49;
  const Scalar _tmp139 = Scalar(1.0) * _tmp134;
  const Scalar _tmp140 = Scalar(1.0) * _tmp114 * (_tmp122 * _tmp139 - _tmp137 * _tmp138);
  const Scalar _tmp141 = _tmp54 + _tmp62;
  const Scalar _tmp142 = _tmp141 * fh1;
  const Scalar _tmp143 = _tmp114 * _tmp142 + Scalar(2.3544) * _tmp23 + _tmp27 * fv1;
  const Scalar _tmp144 = _tmp132 * _tmp49;
  const Scalar _tmp145 = _tmp139 * _tmp144;
  const Scalar _tmp146 = -Scalar(1.0) * _tmp139 + Scalar(1.0) * _tmp145;
  const Scalar _tmp147 = -_tmp112 * _tmp142 - Scalar(2.3544) * _tmp14 - _tmp17 * fv1;
  const Scalar _tmp148 = _tmp133 * _tmp139 + Scalar(1.0);
  const Scalar _tmp149 = _tmp139 * _tmp50;
  const Scalar _tmp150 = -Scalar(1.0) * _tmp138 * _tmp148 + Scalar(1.0) * _tmp149;
  const Scalar _tmp151 = _tmp102 * _tmp139;
  const Scalar _tmp152 = -_tmp116 * _tmp91 + _tmp132 * _tmp151;
  const Scalar _tmp153 = Scalar(1.0) * _tmp115;
  const Scalar _tmp154 = _tmp153 * (-_tmp138 * _tmp152 + _tmp151);
  const Scalar _tmp155 = _tmp127 * _tmp134;
  const Scalar _tmp156 = _tmp49 * (-_tmp128 * _tmp136 - _tmp132 * _tmp155 + _tmp46);
  const Scalar _tmp157 = _tmp103 * _tmp135 - _tmp104 * _tmp136 - _tmp76;
  const Scalar _tmp158 = Scalar(1.0) * _tmp112 * (_tmp103 * _tmp139 - _tmp138 * _tmp157);
  const Scalar _tmp159 =
      Scalar(1.0) * _tmp126 * (-_tmp127 * _tmp139 - Scalar(1.0) * _tmp156 + Scalar(1.0)) +
      _tmp140 * fh1 + _tmp143 * _tmp146 + _tmp147 * _tmp150 + _tmp154 * fh1 + _tmp158 * fh1;
  const Scalar _tmp160 = std::asinh(_tmp131 * _tmp159);
  const Scalar _tmp161 = Scalar(1.0) * _tmp160;
  const Scalar _tmp162 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp163 = _tmp112 * _tmp141;
  const Scalar _tmp164 = _tmp114 * _tmp141;
  const Scalar _tmp165 = -_tmp113 - _tmp118 - _tmp125;
  const Scalar _tmp166 = _tmp162 * _tmp165;
  const Scalar _tmp167 =
      (_tmp131 * (_tmp140 + _tmp146 * _tmp164 - _tmp150 * _tmp163 + _tmp154 + _tmp158) -
       _tmp159 * _tmp166) /
      std::sqrt(Scalar(std::pow(_tmp159, Scalar(2)) * _tmp162 + 1));
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp169 =
      -_tmp160 * _tmp168 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp65 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp67 - 1), Scalar(2))));
  const Scalar _tmp170 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp171 = _tmp169 * _tmp170;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp173 = _tmp114 * _tmp124 * _tmp74;
  const Scalar _tmp174 = _tmp101 * _tmp153;
  const Scalar _tmp175 = _tmp174 * fh1;
  const Scalar _tmp176 = _tmp107 * _tmp112 * _tmp74;
  const Scalar _tmp177 = _tmp126 * _tmp129;
  const Scalar _tmp178 = _tmp173 * fh1 - _tmp175 * _tmp90 + _tmp176 * fh1 - _tmp177 * _tmp90;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = _tmp112 * _tmp157 * _tmp49;
  const Scalar _tmp181 = _tmp148 * _tmp49;
  const Scalar _tmp182 = _tmp114 * _tmp137 * _tmp49;
  const Scalar _tmp183 = _tmp139 * _tmp143;
  const Scalar _tmp184 = _tmp115 * _tmp152 * _tmp49;
  const Scalar _tmp185 = _tmp126 * _tmp156 - _tmp144 * _tmp183 + _tmp147 * _tmp181 + _tmp180 * fh1 +
                         _tmp182 * fh1 + _tmp184 * fh1;
  const Scalar _tmp186 = std::asinh(_tmp179 * _tmp185);
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp186;
  const Scalar _tmp188 =
      -_tmp178 * _tmp187 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp56), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp58), Scalar(2))));
  const Scalar _tmp189 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp190 = _tmp173 - _tmp174 * _tmp90 + _tmp176;
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp193 =
      (_tmp179 * (-_tmp145 * _tmp164 - _tmp163 * _tmp181 + _tmp180 + _tmp182 + _tmp184) -
       _tmp185 * _tmp191) /
      std::sqrt(Scalar(std::pow(_tmp185, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp194 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp195 = _tmp188 * _tmp194;
  const Scalar _tmp196 = Scalar(1.0) * _tmp186;
  const Scalar _tmp197 = _tmp103 * _tmp112 * _tmp134;
  const Scalar _tmp198 = _tmp114 * _tmp122 * _tmp134;
  const Scalar _tmp199 = _tmp115 * _tmp151;
  const Scalar _tmp200 = _tmp126 * _tmp155 - _tmp147 * _tmp149 + _tmp183 - _tmp197 * fh1 -
                         _tmp198 * fh1 - _tmp199 * fh1;
  const Scalar _tmp201 = _tmp104 * _tmp112 * _tmp93;
  const Scalar _tmp202 = _tmp114 * _tmp123 * _tmp93;
  const Scalar _tmp203 = _tmp175 + _tmp177 + _tmp201 * fh1 + _tmp202 * fh1;
  const Scalar _tmp204 = Scalar(1.0) / (_tmp203);
  const Scalar _tmp205 = std::asinh(_tmp200 * _tmp204);
  const Scalar _tmp206 = Scalar(1.0) * _tmp205;
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp203;
  const Scalar _tmp208 =
      -_tmp205 * _tmp207 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp84), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp81 - 1), Scalar(2))));
  const Scalar _tmp209 = Scalar(0.1034955) * _tmp204;
  const Scalar _tmp210 = _tmp208 * _tmp209;
  const Scalar _tmp211 = _tmp174 + _tmp201 + _tmp202;
  const Scalar _tmp212 = Scalar(9.6622558468725703) * _tmp211;
  const Scalar _tmp213 = std::pow(_tmp203, Scalar(-2));
  const Scalar _tmp214 = _tmp211 * _tmp213;
  const Scalar _tmp215 = (-_tmp200 * _tmp214 + _tmp204 * (_tmp139 * _tmp164 + _tmp149 * _tmp163 -
                                                          _tmp197 - _tmp198 - _tmp199)) /
                         std::sqrt(Scalar(std::pow(_tmp200, Scalar(2)) * _tmp213 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(48.000000000000007) * _tmp0 -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::sinh(_tmp35) - Scalar(4.967784) * _tmp36 -
           (-Scalar(0.1034955) * _tmp32 * _tmp36 +
            _tmp33 * (Scalar(9.6622558468725703) * _tmp29 * _tmp37 - _tmp31)) *
               std::sinh(_tmp34)) +
      Scalar(9.6622558468725703) * std::cosh(_tmp34) -
      Scalar(9.6622558468725703) * std::cosh(_tmp35);
  _res(1, 0) =
      -_tmp168 * (-Scalar(4.967784) * _tmp166 + Scalar(1.0) * _tmp167 * std::sinh(_tmp161) -
                  (-Scalar(0.1034955) * _tmp166 * _tmp169 +
                   _tmp170 * (-_tmp160 * _tmp172 - _tmp167 * _tmp168)) *
                      std::sinh(_tmp171)) -
      _tmp172 * (Scalar(4.967784) * _tmp131 + std::cosh(_tmp161) - std::cosh(_tmp171));
  _res(2, 0) = -Scalar(9.6622558468725703) * _tmp190 *
                   (Scalar(4.967784) * _tmp179 - std::cosh(_tmp195) + std::cosh(_tmp196)) -
               _tmp192 * (-Scalar(4.967784) * _tmp191 + Scalar(1.0) * _tmp193 * std::sinh(_tmp196) -
                          (-Scalar(0.1034955) * _tmp188 * _tmp191 +
                           _tmp194 * (-_tmp187 * _tmp190 - _tmp192 * _tmp193)) *
                              std::sinh(_tmp195));
  _res(3, 0) =
      -_tmp207 * (-Scalar(4.967784) * _tmp214 + Scalar(1.0) * _tmp215 * std::sinh(_tmp206) -
                  (-Scalar(0.1034955) * _tmp208 * _tmp214 +
                   _tmp209 * (-_tmp205 * _tmp212 - _tmp207 * _tmp215)) *
                      std::sinh(_tmp210)) -
      _tmp212 * (Scalar(4.967784) * _tmp204 + std::cosh(_tmp206) - std::cosh(_tmp210));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
