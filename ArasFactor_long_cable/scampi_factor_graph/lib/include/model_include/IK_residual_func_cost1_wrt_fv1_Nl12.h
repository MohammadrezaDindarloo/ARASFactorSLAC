// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl12
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl12(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 606

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp9 = 2 * _tmp8;
  const Scalar _tmp10 = _tmp5 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp3;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp3 * _tmp9;
  const Scalar _tmp16 = _tmp11 * _tmp5;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp7;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp22 = -2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp24 = 2 * _tmp3 * _tmp5;
  const Scalar _tmp25 = _tmp11 * _tmp8;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = _tmp23 + _tmp28;
  const Scalar _tmp30 = _tmp21 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = _tmp31 + Scalar(-110.0);
  const Scalar _tmp34 = _tmp20 + Scalar(-125.0);
  const Scalar _tmp35 = std::pow(Scalar(std::pow(_tmp33, Scalar(2)) + std::pow(_tmp34, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp38 = -Scalar(0.010999999999999999) * _tmp22 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp39 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp40 = _tmp38 + _tmp39;
  const Scalar _tmp41 = fh1 * (_tmp37 + _tmp40);
  const Scalar _tmp42 = Scalar(2.3544) * _tmp13 + _tmp19 * fv1 + _tmp36 * _tmp41;
  const Scalar _tmp43 = -_tmp7;
  const Scalar _tmp44 = _tmp18 + _tmp43;
  const Scalar _tmp45 = -_tmp21;
  const Scalar _tmp46 = -_tmp23;
  const Scalar _tmp47 = _tmp28 + _tmp46;
  const Scalar _tmp48 = _tmp45 + _tmp47;
  const Scalar _tmp49 = Scalar(1.0) * _tmp48;
  const Scalar _tmp50 = -_tmp49;
  const Scalar _tmp51 = _tmp29 + _tmp45;
  const Scalar _tmp52 = _tmp50 + _tmp51;
  const Scalar _tmp53 = _tmp21 + _tmp47;
  const Scalar _tmp54 = Scalar(1.0) / (_tmp50 + _tmp53);
  const Scalar _tmp55 = _tmp14 - _tmp17;
  const Scalar _tmp56 = _tmp55 + _tmp7;
  const Scalar _tmp57 = _tmp43 + _tmp55;
  const Scalar _tmp58 = Scalar(1.0) * _tmp57;
  const Scalar _tmp59 = -_tmp56 + _tmp58;
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp52 * _tmp60;
  const Scalar _tmp62 = Scalar(1.0) / (-_tmp44 + _tmp58 - _tmp61);
  const Scalar _tmp63 = Scalar(1.0) * _tmp62;
  const Scalar _tmp64 = _tmp52 * _tmp54;
  const Scalar _tmp65 = Scalar(1.0) * _tmp63 * _tmp64 - Scalar(1.0) * _tmp63;
  const Scalar _tmp66 = _tmp53 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 + Scalar(110.0);
  const Scalar _tmp68 = _tmp56 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(-125.0);
  const Scalar _tmp70 = std::pow(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp69, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp71 = _tmp69 * _tmp70;
  const Scalar _tmp72 = _tmp48 + position_vector(1, 0);
  const Scalar _tmp73 = _tmp72 + Scalar(110.0);
  const Scalar _tmp74 = _tmp57 + position_vector(0, 0);
  const Scalar _tmp75 = _tmp74 + Scalar(125.0);
  const Scalar _tmp76 =
      std::sqrt(Scalar(std::pow(_tmp73, Scalar(2)) + std::pow(_tmp75, Scalar(2))));
  const Scalar _tmp77 = Scalar(1.0) / (_tmp76);
  const Scalar _tmp78 = Scalar(1.0) / (_tmp75);
  const Scalar _tmp79 = _tmp76 * _tmp78;
  const Scalar _tmp80 = _tmp79 * (-_tmp48 * _tmp75 * _tmp77 + _tmp57 * _tmp73 * _tmp77);
  const Scalar _tmp81 = _tmp67 * _tmp70;
  const Scalar _tmp82 = _tmp53 * _tmp71 - _tmp56 * _tmp81 + _tmp71 * _tmp80;
  const Scalar _tmp83 = _tmp73 * _tmp78;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp71 * _tmp83 - _tmp81);
  const Scalar _tmp85 = Scalar(1.0) * _tmp84;
  const Scalar _tmp86 = -_tmp37;
  const Scalar _tmp87 = _tmp40 + _tmp86;
  const Scalar _tmp88 = _tmp38 - _tmp39;
  const Scalar _tmp89 = _tmp86 + _tmp88;
  const Scalar _tmp90 = _tmp71 * _tmp89;
  const Scalar _tmp91 = _tmp81 * _tmp87 - _tmp83 * _tmp90;
  const Scalar _tmp92 = Scalar(1.0) * _tmp54;
  const Scalar _tmp93 = -_tmp71 * _tmp87 + _tmp90;
  const Scalar _tmp94 = _tmp59 * _tmp84 * _tmp91 * _tmp92 - _tmp85 * _tmp93;
  const Scalar _tmp95 = _tmp37 + _tmp88;
  const Scalar _tmp96 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp97 = _tmp96 + Scalar(125.0);
  const Scalar _tmp98 = _tmp51 + position_vector(1, 0);
  const Scalar _tmp99 = _tmp98 + Scalar(-110.0);
  const Scalar _tmp100 = std::pow(Scalar(std::pow(_tmp97, Scalar(2)) + std::pow(_tmp99, Scalar(2))),
                                  Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp101 = _tmp100 * _tmp97;
  const Scalar _tmp102 = _tmp101 * _tmp89;
  const Scalar _tmp103 = _tmp100 * _tmp99;
  const Scalar _tmp104 = _tmp101 * _tmp83 - _tmp103;
  const Scalar _tmp105 = _tmp104 * _tmp84;
  const Scalar _tmp106 = -_tmp102 * _tmp83 + _tmp103 * _tmp95 - _tmp105 * _tmp91;
  const Scalar _tmp107 = -_tmp101 * _tmp95 + _tmp102 - _tmp105 * _tmp93 - _tmp106 * _tmp60;
  const Scalar _tmp108 = Scalar(1.0) / (_tmp107);
  const Scalar _tmp109 = _tmp101 * _tmp51 + _tmp101 * _tmp80 - _tmp103 * _tmp44 - _tmp105 * _tmp82;
  const Scalar _tmp110 = _tmp108 * _tmp109;
  const Scalar _tmp111 = Scalar(1.0) / (_tmp109);
  const Scalar _tmp112 = _tmp107 * _tmp111;
  const Scalar _tmp113 = _tmp112 * (-_tmp110 * _tmp94 - _tmp82 * _tmp85);
  const Scalar _tmp114 = _tmp52 * _tmp62;
  const Scalar _tmp115 = _tmp108 * (_tmp113 + _tmp94);
  const Scalar _tmp116 = -_tmp106 * _tmp115 + _tmp113 * _tmp114 - _tmp85 * _tmp91;
  const Scalar _tmp117 = _tmp33 * _tmp35;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = _tmp112 * _tmp63;
  const Scalar _tmp120 = Scalar(1.0) * _tmp111;
  const Scalar _tmp121 = -_tmp106 * _tmp120 + _tmp119 * _tmp52;
  const Scalar _tmp122 = fh1 * (_tmp117 * _tmp19 - _tmp30 * _tmp36);
  const Scalar _tmp123 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp124 = _tmp49 * _tmp60 + _tmp58;
  const Scalar _tmp125 = 0;
  const Scalar _tmp126 = _tmp124 * _tmp62;
  const Scalar _tmp127 = _tmp54 * (-_tmp106 * _tmp125 - _tmp126 * _tmp52 + _tmp50);
  const Scalar _tmp128 = -Scalar(1.0) * _tmp124 * _tmp63 - Scalar(1.0) * _tmp127 + Scalar(1.0);
  const Scalar _tmp129 = -_tmp117 * _tmp41 - Scalar(2.3544) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp130 = _tmp60 * _tmp63;
  const Scalar _tmp131 = _tmp61 * _tmp63 + Scalar(1.0);
  const Scalar _tmp132 = Scalar(1.0) * _tmp130 - Scalar(1.0) * _tmp131 * _tmp92;
  const Scalar _tmp133 = _tmp83 * _tmp84;
  const Scalar _tmp134 = _tmp133 * _tmp91 + _tmp83 * _tmp89;
  const Scalar _tmp135 = _tmp133 * _tmp93 - _tmp134 * _tmp60 - _tmp89;
  const Scalar _tmp136 = _tmp112 * (-_tmp110 * _tmp135 + _tmp133 * _tmp82 - _tmp80);
  const Scalar _tmp137 = _tmp108 * (_tmp135 + _tmp136);
  const Scalar _tmp138 = -_tmp106 * _tmp137 + _tmp114 * _tmp136 + _tmp134;
  const Scalar _tmp139 = _tmp36 * fh1;
  const Scalar _tmp140 =
      Scalar(1.0) * _tmp118 * (_tmp113 * _tmp63 - _tmp116 * _tmp92) +
      Scalar(1.0) * _tmp122 * (_tmp119 - _tmp121 * _tmp92) + _tmp123 * _tmp128 + _tmp129 * _tmp132 +
      Scalar(1.0) * _tmp139 * (_tmp136 * _tmp63 - _tmp138 * _tmp92) + _tmp42 * _tmp65;
  const Scalar _tmp141 = _tmp105 * _tmp125;
  const Scalar _tmp142 = _tmp79 * (_tmp101 * _tmp125 - _tmp141 * _tmp71);
  const Scalar _tmp143 = -_tmp104 * _tmp115 + Scalar(1.0);
  const Scalar _tmp144 = _tmp71 * _tmp84;
  const Scalar _tmp145 = -_tmp104 * _tmp137 - _tmp83;
  const Scalar _tmp146 = -_tmp118 * _tmp79 * (_tmp101 * _tmp115 + _tmp143 * _tmp144) -
                         _tmp122 * _tmp79 * (_tmp101 * _tmp120 - _tmp105 * _tmp120 * _tmp71) -
                         _tmp123 * _tmp142 -
                         _tmp139 * _tmp79 * (_tmp101 * _tmp137 + _tmp144 * _tmp145 + Scalar(1.0));
  const Scalar _tmp147 = Scalar(1.0) / (_tmp146);
  const Scalar _tmp148 = std::asinh(_tmp140 * _tmp147);
  const Scalar _tmp149 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp150 =
      -_tmp148 * _tmp149 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp72 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp74 - 1), Scalar(2))));
  const Scalar _tmp151 = std::pow(_tmp146, Scalar(-2));
  const Scalar _tmp152 = _tmp142 * _tmp151;
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp142;
  const Scalar _tmp154 = _tmp27 + _tmp45 + _tmp46;
  const Scalar _tmp155 =
      (-_tmp140 * _tmp152 + _tmp147 * (-_tmp128 + _tmp132 * _tmp154 + _tmp19 * _tmp65)) /
      std::sqrt(Scalar(std::pow(_tmp140, Scalar(2)) * _tmp151 + 1));
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp147;
  const Scalar _tmp157 = _tmp150 * _tmp156;
  const Scalar _tmp158 = Scalar(1.0) * _tmp148;
  const Scalar _tmp159 = _tmp123 * _tmp125;
  const Scalar _tmp160 = _tmp120 * _tmp122;
  const Scalar _tmp161 = -_tmp105 * _tmp159 - _tmp105 * _tmp160 + _tmp118 * _tmp143 * _tmp84 +
                         _tmp139 * _tmp145 * _tmp84;
  const Scalar _tmp162 = Scalar(1.0) / (_tmp161);
  const Scalar _tmp163 = _tmp131 * _tmp54;
  const Scalar _tmp164 = _tmp42 * _tmp63;
  const Scalar _tmp165 = _tmp116 * _tmp118 * _tmp54 + _tmp121 * _tmp122 * _tmp54 +
                         _tmp123 * _tmp127 + _tmp129 * _tmp163 + _tmp138 * _tmp139 * _tmp54 -
                         _tmp164 * _tmp64;
  const Scalar _tmp166 = std::asinh(_tmp162 * _tmp165);
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp161;
  const Scalar _tmp168 =
      -_tmp166 * _tmp167 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp68), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp66 - 1), Scalar(2))));
  const Scalar _tmp169 = Scalar(0.1034955) * _tmp162;
  const Scalar _tmp170 = _tmp168 * _tmp169;
  const Scalar _tmp171 = Scalar(1.0) * _tmp166;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp173 = _tmp105 * _tmp172;
  const Scalar _tmp174 = std::pow(_tmp161, Scalar(-2));
  const Scalar _tmp175 = _tmp141 * _tmp174;
  const Scalar _tmp176 = _tmp19 * _tmp63;
  const Scalar _tmp177 =
      (_tmp162 * (-_tmp127 + _tmp154 * _tmp163 - _tmp176 * _tmp64) - _tmp165 * _tmp175) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp174 + 1));
  const Scalar _tmp178 = _tmp115 * _tmp118 + _tmp137 * _tmp139 + _tmp159 + _tmp160;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = -_tmp113 * _tmp118 * _tmp62 - _tmp119 * _tmp122 + _tmp123 * _tmp126 -
                         _tmp129 * _tmp130 - _tmp136 * _tmp139 * _tmp62 + _tmp164;
  const Scalar _tmp181 = std::asinh(_tmp179 * _tmp180);
  const Scalar _tmp182 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp183 =
      -_tmp181 * _tmp182 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp98), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp96 - 1), Scalar(2))));
  const Scalar _tmp184 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp185 = _tmp183 * _tmp184;
  const Scalar _tmp186 = Scalar(1.0) * _tmp181;
  const Scalar _tmp187 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp188 = _tmp125 * _tmp187;
  const Scalar _tmp189 = (_tmp179 * (-_tmp126 - _tmp130 * _tmp154 + _tmp176) + _tmp180 * _tmp188) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp187 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp32 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp20),
                                           Scalar(2)) +
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp31),
                                               Scalar(2)))))));
  _res(1, 0) =
      -_tmp149 * (-Scalar(4.967784) * _tmp152 + Scalar(1.0) * _tmp155 * std::sinh(_tmp158) -
                  (-Scalar(0.1034955) * _tmp150 * _tmp152 +
                   _tmp156 * (-_tmp148 * _tmp153 - _tmp149 * _tmp155)) *
                      std::sinh(_tmp157)) -
      _tmp153 * (Scalar(4.967784) * _tmp147 - std::cosh(_tmp157) + std::cosh(_tmp158));
  _res(2, 0) =
      -_tmp167 * (-Scalar(4.967784) * _tmp175 + Scalar(1.0) * _tmp177 * std::sinh(_tmp171) -
                  (-Scalar(0.1034955) * _tmp168 * _tmp175 +
                   _tmp169 * (-_tmp166 * _tmp173 - _tmp167 * _tmp177)) *
                      std::sinh(_tmp170)) -
      _tmp173 * (Scalar(4.967784) * _tmp162 - std::cosh(_tmp170) + std::cosh(_tmp171));
  _res(3, 0) = _tmp172 * (Scalar(4.967784) * _tmp179 - std::cosh(_tmp185) + std::cosh(_tmp186)) -
               _tmp182 * (Scalar(4.967784) * _tmp188 + Scalar(1.0) * _tmp189 * std::sinh(_tmp186) -
                          (Scalar(0.1034955) * _tmp183 * _tmp188 +
                           _tmp184 * (_tmp172 * _tmp181 - _tmp182 * _tmp189)) *
                              std::sinh(_tmp185));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
