// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 532

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = 2 * _tmp1 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp5 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp9 = 1 - 2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp8 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp11 = 2 * _tmp4;
  const Scalar _tmp12 = _tmp1 * _tmp11;
  const Scalar _tmp13 = _tmp2 * _tmp5;
  const Scalar _tmp14 = _tmp12 - _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp10 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp7;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp23 = _tmp11 * _tmp2;
  const Scalar _tmp24 = _tmp1 * _tmp5;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = _tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 =
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp29), Scalar(2)) +
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp18 - 1), Scalar(2));
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp34 = -_tmp33;
  const Scalar _tmp35 = -Scalar(0.010999999999999999) * _tmp21 -
                        Scalar(0.010999999999999999) * _tmp8 + Scalar(-0.010999999999999999);
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp37 = _tmp35 + _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp40 = _tmp10 + _tmp15;
  const Scalar _tmp41 = _tmp40 + _tmp7;
  const Scalar _tmp42 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(-110.0);
  const Scalar _tmp44 = _tmp19 + _tmp27;
  const Scalar _tmp45 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-125.0);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp46 * _tmp47;
  const Scalar _tmp49 = _tmp33 + _tmp37;
  const Scalar _tmp50 = _tmp35 - _tmp36;
  const Scalar _tmp51 = _tmp33 + _tmp50;
  const Scalar _tmp52 = -_tmp7;
  const Scalar _tmp53 = _tmp16 + _tmp52;
  const Scalar _tmp54 = _tmp53 + position_vector(1, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(110.0);
  const Scalar _tmp56 = -_tmp22 + _tmp26;
  const Scalar _tmp57 = _tmp20 + _tmp56;
  const Scalar _tmp58 = _tmp57 + position_vector(0, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(125.0);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp55, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp59 * _tmp60;
  const Scalar _tmp62 = _tmp34 + _tmp50;
  const Scalar _tmp63 = _tmp51 * _tmp61 - _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp40 + _tmp52;
  const Scalar _tmp65 = _tmp64 + position_vector(1, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-110.0);
  const Scalar _tmp67 = _tmp19 + _tmp56;
  const Scalar _tmp68 = _tmp67 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(125.0);
  const Scalar _tmp70 = Scalar(1.0) / (_tmp69);
  const Scalar _tmp71 = _tmp66 * _tmp70;
  const Scalar _tmp72 = _tmp55 * _tmp60;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp61 * _tmp71 - _tmp72);
  const Scalar _tmp74 = _tmp43 * _tmp47;
  const Scalar _tmp75 = _tmp48 * _tmp71 - _tmp74;
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp48 * _tmp51;
  const Scalar _tmp78 = _tmp51 * _tmp71;
  const Scalar _tmp79 = -_tmp61 * _tmp78 + _tmp62 * _tmp72;
  const Scalar _tmp80 = _tmp49 * _tmp74 - _tmp71 * _tmp77 - _tmp76 * _tmp79;
  const Scalar _tmp81 = Scalar(1.0) * _tmp64;
  const Scalar _tmp82 = -_tmp81;
  const Scalar _tmp83 = Scalar(1.0) / (_tmp53 + _tmp82);
  const Scalar _tmp84 = Scalar(1.0) * _tmp67;
  const Scalar _tmp85 = _tmp83 * (-_tmp57 + _tmp84);
  const Scalar _tmp86 = -_tmp48 * _tmp49 - _tmp63 * _tmp76 + _tmp77 - _tmp80 * _tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp86);
  const Scalar _tmp88 = _tmp81 * _tmp85 + _tmp84;
  const Scalar _tmp89 = 0;
  const Scalar _tmp90 = _tmp87 * _tmp89;
  const Scalar _tmp91 =
      std::sqrt(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp69, Scalar(2))));
  const Scalar _tmp92 = _tmp70 * _tmp91;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp94 = _tmp92 * (-_tmp64 * _tmp69 * _tmp93 + _tmp66 * _tmp67 * _tmp93);
  const Scalar _tmp95 = _tmp53 * _tmp61 - _tmp57 * _tmp72 + _tmp61 * _tmp94;
  const Scalar _tmp96 = _tmp41 * _tmp48 - _tmp44 * _tmp74 + _tmp48 * _tmp94 - _tmp76 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp98 = Scalar(1.0) * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) * _tmp73;
  const Scalar _tmp100 = _tmp75 * _tmp97 * _tmp99;
  const Scalar _tmp101 = _tmp18 + Scalar(110.0);
  const Scalar _tmp102 = _tmp29 + Scalar(-125.0);
  const Scalar _tmp103 =
      std::pow(Scalar(std::pow(_tmp101, Scalar(2)) + std::pow(_tmp102, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp104 = _tmp101 * _tmp103;
  const Scalar _tmp105 = _tmp102 * _tmp103;
  const Scalar _tmp106 = fh1 * (_tmp104 * _tmp28 - _tmp105 * _tmp17);
  const Scalar _tmp107 = _tmp79 * _tmp99;
  const Scalar _tmp108 = _tmp107 * _tmp85 - _tmp63 * _tmp99;
  const Scalar _tmp109 = _tmp87 * _tmp96;
  const Scalar _tmp110 = _tmp86 * _tmp97;
  const Scalar _tmp111 = _tmp110 * (-_tmp108 * _tmp109 - _tmp95 * _tmp99);
  const Scalar _tmp112 = _tmp108 + _tmp111;
  const Scalar _tmp113 = _tmp75 * _tmp87;
  const Scalar _tmp114 = -_tmp112 * _tmp113 + Scalar(1.0);
  const Scalar _tmp115 = _tmp61 * _tmp73;
  const Scalar _tmp116 = _tmp48 * _tmp87;
  const Scalar _tmp117 = _tmp104 * fh1;
  const Scalar _tmp118 = _tmp71 * _tmp73;
  const Scalar _tmp119 = _tmp118 * _tmp79 + _tmp78;
  const Scalar _tmp120 = _tmp118 * _tmp63 - _tmp119 * _tmp85 - _tmp51;
  const Scalar _tmp121 = _tmp110 * (-_tmp109 * _tmp120 + _tmp118 * _tmp95 - _tmp94);
  const Scalar _tmp122 = _tmp120 + _tmp121;
  const Scalar _tmp123 = -_tmp113 * _tmp122 - _tmp71;
  const Scalar _tmp124 = _tmp105 * fh1;
  const Scalar _tmp125 = -_tmp106 * _tmp92 * (-_tmp100 * _tmp61 + _tmp48 * _tmp98) -
                         _tmp117 * _tmp92 * (_tmp112 * _tmp116 + _tmp114 * _tmp115) -
                         _tmp124 * _tmp92 * (_tmp115 * _tmp123 + _tmp116 * _tmp122 + Scalar(1.0)) -
                         _tmp39 * _tmp92 * (_tmp48 * _tmp90 - _tmp61 * _tmp76 * _tmp90);
  const Scalar _tmp126 = Scalar(1.0) / (_tmp125);
  const Scalar _tmp127 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp65), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp68 - 1), Scalar(2));
  const Scalar _tmp128 = _tmp41 + _tmp82;
  const Scalar _tmp129 = _tmp128 * _tmp85;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp44 + _tmp84);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp131 * _tmp85;
  const Scalar _tmp133 = _tmp83 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp134 = _tmp38 * fh1;
  const Scalar _tmp135 = -_tmp104 * _tmp134 - Scalar(2.3544) * _tmp14 - _tmp17 * fv1;
  const Scalar _tmp136 = _tmp110 * _tmp131;
  const Scalar _tmp137 = _tmp128 * _tmp136 - _tmp80 * _tmp98;
  const Scalar _tmp138 = Scalar(1.0) * _tmp83;
  const Scalar _tmp139 = _tmp128 * _tmp130;
  const Scalar _tmp140 = _tmp80 * _tmp87;
  const Scalar _tmp141 = _tmp119 + _tmp121 * _tmp139 - _tmp122 * _tmp140;
  const Scalar _tmp142 = _tmp128 * _tmp83;
  const Scalar _tmp143 = _tmp105 * _tmp134 + Scalar(2.3544) * _tmp25 + _tmp28 * fv1;
  const Scalar _tmp144 = _tmp130 * _tmp88;
  const Scalar _tmp145 = -_tmp128 * _tmp144 - _tmp140 * _tmp89 + _tmp82;
  const Scalar _tmp146 = -_tmp107 + _tmp111 * _tmp139 - _tmp112 * _tmp140;
  const Scalar _tmp147 = std::asinh(
      _tmp126 * (Scalar(1.0) * _tmp106 * (_tmp136 - _tmp137 * _tmp138) +
                 Scalar(1.0) * _tmp117 * (_tmp111 * _tmp131 - _tmp138 * _tmp146) +
                 Scalar(1.0) * _tmp124 * (_tmp121 * _tmp131 - _tmp138 * _tmp141) +
                 Scalar(1.0) * _tmp135 * (_tmp132 - Scalar(1.0) * _tmp133) +
                 Scalar(1.0) * _tmp143 * (_tmp131 * _tmp142 - _tmp131) +
                 Scalar(1.0) * _tmp39 * (-_tmp131 * _tmp88 - _tmp138 * _tmp145 + Scalar(1.0))));
  const Scalar _tmp148 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp149 = _tmp39 * _tmp90;
  const Scalar _tmp150 = -_tmp100 * _tmp106 + _tmp114 * _tmp117 * _tmp73 +
                         _tmp123 * _tmp124 * _tmp73 - _tmp149 * _tmp76;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = _tmp131 * _tmp143;
  const Scalar _tmp153 =
      std::asinh(_tmp151 * (_tmp106 * _tmp137 * _tmp83 + _tmp117 * _tmp146 * _tmp83 +
                            _tmp124 * _tmp141 * _tmp83 + _tmp133 * _tmp135 - _tmp142 * _tmp152 +
                            _tmp145 * _tmp39 * _tmp83));
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp150;
  const Scalar _tmp155 =
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp54 - 1), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp58 - 1), Scalar(2));
  const Scalar _tmp156 =
      _tmp106 * _tmp98 + _tmp112 * _tmp117 * _tmp87 + _tmp122 * _tmp124 * _tmp87 + _tmp149;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 = std::asinh(_tmp157 * (-_tmp106 * _tmp136 - _tmp111 * _tmp117 * _tmp130 -
                                               _tmp121 * _tmp124 * _tmp130 - _tmp132 * _tmp135 +
                                               _tmp144 * _tmp39 + _tmp152));
  const Scalar _tmp159 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp42), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp45), Scalar(2));
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp156;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 * (-std::sinh(Scalar(1.0) * _tmp31) -
                std::sinh(Scalar(0.1034955) * _tmp0 * (-std::sqrt(_tmp30) - _tmp31 * _tmp32))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp30 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp38 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(1, 0) =
      _tmp148 *
          (-std::sinh(Scalar(1.0) * _tmp147) -
           std::sinh(Scalar(0.1034955) * _tmp126 * (-std::sqrt(_tmp127) - _tmp147 * _tmp148))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp127 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp51 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(2, 0) =
      _tmp154 *
          (-std::sinh(Scalar(1.0) * _tmp153) -
           std::sinh(Scalar(0.1034955) * _tmp151 * (-_tmp153 * _tmp154 - std::sqrt(_tmp155)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp155 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp62 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(3, 0) =
      _tmp160 *
          (-std::sinh(Scalar(1.0) * _tmp158) -
           std::sinh(Scalar(0.1034955) * _tmp157 * (-_tmp158 * _tmp160 - std::sqrt(_tmp159)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp159 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp49 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
