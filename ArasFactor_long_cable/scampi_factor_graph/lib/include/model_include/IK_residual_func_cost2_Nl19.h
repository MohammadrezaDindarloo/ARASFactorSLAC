// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl19
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl19(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 534

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp8 = 2 * _tmp3;
  const Scalar _tmp9 = _tmp7 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = _tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp1 * _tmp7;
  const Scalar _tmp15 = _tmp10 * _tmp3;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(0, 0);
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = _tmp1 * _tmp8;
  const Scalar _tmp23 = _tmp10 * _tmp7;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp26 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp28 = _tmp25 + _tmp27;
  const Scalar _tmp29 = _tmp21 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(1, 0);
  const Scalar _tmp31 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp30), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp19 - 1), Scalar(2));
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp35 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp36 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp37 = _tmp35 - _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = -_tmp34;
  const Scalar _tmp40 = _tmp37 + _tmp39;
  const Scalar _tmp41 = _tmp25 - _tmp27;
  const Scalar _tmp42 = _tmp21 + _tmp41;
  const Scalar _tmp43 = _tmp42 + position_vector(1, 0);
  const Scalar _tmp44 = _tmp13 - _tmp16;
  const Scalar _tmp45 = _tmp44 + _tmp6;
  const Scalar _tmp46 = _tmp45 + position_vector(0, 0);
  const Scalar _tmp47 =
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp43 - 1), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp46 - 1), Scalar(2));
  const Scalar _tmp48 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp49 = _tmp20 + _tmp41;
  const Scalar _tmp50 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(110.0);
  const Scalar _tmp52 = _tmp44 + _tmp5;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(-125.0);
  const Scalar _tmp55 = std::pow(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp54, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp56 = _tmp54 * _tmp55;
  const Scalar _tmp57 = _tmp20 + _tmp28;
  const Scalar _tmp58 = _tmp57 + position_vector(1, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(-110.0);
  const Scalar _tmp60 = _tmp17 + _tmp5;
  const Scalar _tmp61 = _tmp60 + position_vector(0, 0);
  const Scalar _tmp62 = _tmp61 + Scalar(-125.0);
  const Scalar _tmp63 = std::pow(Scalar(std::pow(_tmp59, Scalar(2)) + std::pow(_tmp62, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp64 = _tmp62 * _tmp63;
  const Scalar _tmp65 = _tmp35 + _tmp36;
  const Scalar _tmp66 = _tmp34 + _tmp65;
  const Scalar _tmp67 = _tmp40 * _tmp64 - _tmp64 * _tmp66;
  const Scalar _tmp68 = _tmp51 * _tmp55;
  const Scalar _tmp69 = _tmp43 + Scalar(110.0);
  const Scalar _tmp70 = _tmp46 + Scalar(125.0);
  const Scalar _tmp71 = Scalar(1.0) / (_tmp70);
  const Scalar _tmp72 = _tmp69 * _tmp71;
  const Scalar _tmp73 = _tmp56 * _tmp72 - _tmp68;
  const Scalar _tmp74 = _tmp59 * _tmp63;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp64 * _tmp72 - _tmp74);
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp39 + _tmp65;
  const Scalar _tmp78 = _tmp40 * _tmp56;
  const Scalar _tmp79 = _tmp40 * _tmp72;
  const Scalar _tmp80 = -_tmp64 * _tmp79 + _tmp66 * _tmp74;
  const Scalar _tmp81 = _tmp68 * _tmp77 - _tmp72 * _tmp78 - _tmp76 * _tmp80;
  const Scalar _tmp82 = Scalar(1.0) * _tmp42;
  const Scalar _tmp83 = -_tmp82;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp57 + _tmp83);
  const Scalar _tmp85 = Scalar(1.0) * _tmp45;
  const Scalar _tmp86 = -_tmp60 + _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp86;
  const Scalar _tmp88 = -_tmp56 * _tmp77 - _tmp67 * _tmp76 + _tmp78 - _tmp81 * _tmp87;
  const Scalar _tmp89 = Scalar(1.0) / (_tmp88);
  const Scalar _tmp90 = _tmp82 * _tmp87 + _tmp85;
  const Scalar _tmp91 = 0;
  const Scalar _tmp92 = _tmp89 * _tmp91;
  const Scalar _tmp93 = _tmp64 * _tmp75;
  const Scalar _tmp94 = _tmp73 * _tmp89;
  const Scalar _tmp95 = _tmp91 * _tmp94;
  const Scalar _tmp96 =
      std::sqrt(Scalar(std::pow(_tmp69, Scalar(2)) + std::pow(_tmp70, Scalar(2))));
  const Scalar _tmp97 = _tmp71 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) * _tmp84;
  const Scalar _tmp99 = Scalar(1.0) * _tmp75;
  const Scalar _tmp100 = -_tmp67 * _tmp99 + _tmp75 * _tmp80 * _tmp86 * _tmp98;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp102 = _tmp97 * (-_tmp101 * _tmp42 * _tmp70 + _tmp101 * _tmp45 * _tmp69);
  const Scalar _tmp103 = _tmp102 * _tmp64 + _tmp57 * _tmp64 - _tmp60 * _tmp74;
  const Scalar _tmp104 = _tmp102 * _tmp56 - _tmp103 * _tmp76 + _tmp49 * _tmp56 - _tmp52 * _tmp68;
  const Scalar _tmp105 = _tmp104 * _tmp89;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp107 = _tmp106 * _tmp88;
  const Scalar _tmp108 = _tmp107 * (-_tmp100 * _tmp105 - _tmp103 * _tmp99);
  const Scalar _tmp109 = _tmp100 + _tmp108;
  const Scalar _tmp110 = _tmp56 * _tmp89;
  const Scalar _tmp111 = -_tmp109 * _tmp94 + Scalar(1.0);
  const Scalar _tmp112 = _tmp30 + Scalar(-110.0);
  const Scalar _tmp113 = _tmp19 + Scalar(125.0);
  const Scalar _tmp114 =
      std::pow(Scalar(std::pow(_tmp112, Scalar(2)) + std::pow(_tmp113, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp115 = _tmp112 * _tmp114;
  const Scalar _tmp116 = _tmp115 * fh1;
  const Scalar _tmp117 = Scalar(1.0) * _tmp106;
  const Scalar _tmp118 = _tmp113 * _tmp114;
  const Scalar _tmp119 = fh1 * (_tmp115 * _tmp18 - _tmp118 * _tmp29);
  const Scalar _tmp120 = _tmp72 * _tmp75;
  const Scalar _tmp121 = _tmp120 * _tmp80 + _tmp79;
  const Scalar _tmp122 = _tmp120 * _tmp67 - _tmp121 * _tmp87 - _tmp40;
  const Scalar _tmp123 = _tmp107 * (-_tmp102 + _tmp103 * _tmp120 - _tmp105 * _tmp122);
  const Scalar _tmp124 = _tmp122 + _tmp123;
  const Scalar _tmp125 = -_tmp124 * _tmp94 - _tmp72;
  const Scalar _tmp126 = _tmp118 * fh1;
  const Scalar _tmp127 = -_tmp116 * _tmp97 * (_tmp109 * _tmp110 + _tmp111 * _tmp93) -
                         _tmp119 * _tmp97 * (_tmp117 * _tmp56 - _tmp117 * _tmp64 * _tmp76) -
                         _tmp126 * _tmp97 * (_tmp110 * _tmp124 + _tmp125 * _tmp93 + Scalar(1.0)) -
                         _tmp48 * _tmp97 * (_tmp56 * _tmp92 - _tmp93 * _tmp95);
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = _tmp49 + _tmp83;
  const Scalar _tmp130 = _tmp129 * _tmp87;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp52 + _tmp85);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp107 * _tmp132;
  const Scalar _tmp134 = -_tmp117 * _tmp81 + _tmp129 * _tmp133;
  const Scalar _tmp135 = _tmp38 * fh1;
  const Scalar _tmp136 = _tmp118 * _tmp135 + Scalar(2.3544) * _tmp12 + _tmp18 * fv1;
  const Scalar _tmp137 = _tmp129 * _tmp84;
  const Scalar _tmp138 = _tmp81 * _tmp89;
  const Scalar _tmp139 = _tmp129 * _tmp131;
  const Scalar _tmp140 = _tmp108 * _tmp139 - _tmp109 * _tmp138 - _tmp80 * _tmp99;
  const Scalar _tmp141 = _tmp131 * _tmp90;
  const Scalar _tmp142 = -_tmp129 * _tmp141 - _tmp138 * _tmp91 + _tmp83;
  const Scalar _tmp143 = _tmp130 * _tmp132 + Scalar(1.0);
  const Scalar _tmp144 = _tmp132 * _tmp87;
  const Scalar _tmp145 = -_tmp115 * _tmp135 - Scalar(2.3544) * _tmp24 - _tmp29 * fv1;
  const Scalar _tmp146 = _tmp121 + _tmp123 * _tmp139 - _tmp124 * _tmp138;
  const Scalar _tmp147 = std::asinh(
      _tmp128 * (Scalar(1.0) * _tmp116 * (_tmp108 * _tmp132 - _tmp140 * _tmp98) +
                 Scalar(1.0) * _tmp119 * (_tmp133 - _tmp134 * _tmp98) +
                 Scalar(1.0) * _tmp126 * (_tmp123 * _tmp132 - _tmp146 * _tmp98) +
                 Scalar(1.0) * _tmp136 * (_tmp132 * _tmp137 - _tmp132) +
                 Scalar(1.0) * _tmp145 * (-_tmp143 * _tmp98 + _tmp144) +
                 Scalar(1.0) * _tmp48 * (-_tmp132 * _tmp90 - _tmp142 * _tmp98 + Scalar(1.0))));
  const Scalar _tmp148 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp149 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp58), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp61), Scalar(2));
  const Scalar _tmp150 = _tmp132 * _tmp136;
  const Scalar _tmp151 = _tmp117 * _tmp119;
  const Scalar _tmp152 = _tmp111 * _tmp116 * _tmp75 + _tmp125 * _tmp126 * _tmp75 -
                         _tmp151 * _tmp76 - _tmp48 * _tmp75 * _tmp95;
  const Scalar _tmp153 = Scalar(1.0) / (_tmp152);
  const Scalar _tmp154 =
      std::asinh(_tmp153 * (_tmp116 * _tmp140 * _tmp84 + _tmp119 * _tmp134 * _tmp84 +
                            _tmp126 * _tmp146 * _tmp84 - _tmp137 * _tmp150 +
                            _tmp142 * _tmp48 * _tmp84 + _tmp143 * _tmp145 * _tmp84));
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp152;
  const Scalar _tmp156 =
      _tmp109 * _tmp116 * _tmp89 + _tmp124 * _tmp126 * _tmp89 + _tmp151 + _tmp48 * _tmp92;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 = std::asinh(_tmp157 * (-_tmp108 * _tmp116 * _tmp131 - _tmp119 * _tmp133 -
                                               _tmp123 * _tmp126 * _tmp131 + _tmp141 * _tmp48 -
                                               _tmp144 * _tmp145 + _tmp150));
  const Scalar _tmp159 =
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp53), Scalar(2)) +
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp50 - 1), Scalar(2));
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp156;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 * (-std::sinh(Scalar(1.0) * _tmp32) -
                std::sinh(Scalar(0.1034955) * _tmp0 * (-std::sqrt(_tmp31) - _tmp32 * _tmp33))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp31 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp38 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(1, 0) =
      _tmp148 *
          (-std::sinh(Scalar(1.0) * _tmp147) -
           std::sinh(Scalar(0.1034955) * _tmp128 * (-_tmp147 * _tmp148 - std::sqrt(_tmp47)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp47 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp40 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(2, 0) =
      _tmp155 *
          (-std::sinh(Scalar(1.0) * _tmp154) -
           std::sinh(Scalar(0.1034955) * _tmp153 * (-std::sqrt(_tmp149) - _tmp154 * _tmp155))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp149 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp66 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(3, 0) =
      _tmp160 *
          (-std::sinh(Scalar(1.0) * _tmp158) -
           std::sinh(Scalar(0.1034955) * _tmp157 * (-_tmp158 * _tmp160 - std::sqrt(_tmp159)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp159 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp77 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
