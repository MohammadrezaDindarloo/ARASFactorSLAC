// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 527

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (157)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = -_tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp3 * _tmp8;
  const Scalar _tmp15 = _tmp1 * _tmp10;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp20 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp22 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp23 = _tmp10 * _tmp7;
  const Scalar _tmp24 = _tmp22 + _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 =
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp29), Scalar(2)) +
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp19 - 1), Scalar(2));
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp34 = -_tmp33;
  const Scalar _tmp35 =
      -Scalar(0.010999999999999999) * _tmp20 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp37 = _tmp35 + _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = _tmp13 - _tmp16;
  const Scalar _tmp40 = _tmp39 + _tmp6;
  const Scalar _tmp41 = _tmp40 + position_vector(1, 0);
  const Scalar _tmp42 = -_tmp21;
  const Scalar _tmp43 = _tmp27 + _tmp42;
  const Scalar _tmp44 = _tmp43 + position_vector(0, 0);
  const Scalar _tmp45 =
      Scalar(12100.0) * std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp41 - 1), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp44 - 1), Scalar(2));
  const Scalar _tmp46 = _tmp25 + _tmp26;
  const Scalar _tmp47 = _tmp42 + _tmp46;
  const Scalar _tmp48 = _tmp47 + position_vector(0, 0);
  const Scalar _tmp49 = _tmp48 + Scalar(125.0);
  const Scalar _tmp50 = _tmp39 + _tmp5;
  const Scalar _tmp51 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(-110.0);
  const Scalar _tmp53 = std::pow(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp52, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp54 = _tmp49 * _tmp53;
  const Scalar _tmp55 = _tmp35 - _tmp36;
  const Scalar _tmp56 = _tmp33 + _tmp55;
  const Scalar _tmp57 = _tmp34 + _tmp55;
  const Scalar _tmp58 = _tmp41 + Scalar(110.0);
  const Scalar _tmp59 = _tmp44 + Scalar(125.0);
  const Scalar _tmp60 = Scalar(1.0) / (_tmp59);
  const Scalar _tmp61 = _tmp58 * _tmp60;
  const Scalar _tmp62 = _tmp57 * _tmp61;
  const Scalar _tmp63 = _tmp33 + _tmp37;
  const Scalar _tmp64 = _tmp17 + _tmp5;
  const Scalar _tmp65 = _tmp64 + position_vector(1, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-110.0);
  const Scalar _tmp67 = _tmp21 + _tmp46;
  const Scalar _tmp68 = _tmp67 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(-125.0);
  const Scalar _tmp70 = std::pow(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp69, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp71 = _tmp66 * _tmp70;
  const Scalar _tmp72 = _tmp69 * _tmp70;
  const Scalar _tmp73 = -_tmp62 * _tmp72 + _tmp63 * _tmp71;
  const Scalar _tmp74 = _tmp52 * _tmp53;
  const Scalar _tmp75 = _tmp54 * _tmp61 - _tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp61 * _tmp72 - _tmp71);
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = -_tmp54 * _tmp62 + _tmp56 * _tmp74 - _tmp73 * _tmp77;
  const Scalar _tmp79 = Scalar(1.0) * _tmp43;
  const Scalar _tmp80 = Scalar(1.0) * _tmp40;
  const Scalar _tmp81 = -_tmp80;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp64 + _tmp81);
  const Scalar _tmp83 = _tmp82 * (-_tmp67 + _tmp79);
  const Scalar _tmp84 = _tmp57 * _tmp72 - _tmp63 * _tmp72;
  const Scalar _tmp85 = -_tmp54 * _tmp56 + _tmp54 * _tmp57 - _tmp77 * _tmp84 - _tmp78 * _tmp83;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp76;
  const Scalar _tmp88 = _tmp73 * _tmp87;
  const Scalar _tmp89 = _tmp83 * _tmp88 - _tmp84 * _tmp87;
  const Scalar _tmp90 =
      std::sqrt(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp59, Scalar(2))));
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp60 * _tmp90;
  const Scalar _tmp93 = _tmp92 * (-_tmp40 * _tmp59 * _tmp91 + _tmp43 * _tmp58 * _tmp91);
  const Scalar _tmp94 = _tmp64 * _tmp72 - _tmp67 * _tmp71 + _tmp72 * _tmp93;
  const Scalar _tmp95 = -_tmp47 * _tmp74 + _tmp50 * _tmp54 + _tmp54 * _tmp93 - _tmp77 * _tmp94;
  const Scalar _tmp96 = _tmp86 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp85 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp87 * _tmp94 - _tmp89 * _tmp96);
  const Scalar _tmp100 = _tmp86 * (_tmp89 + _tmp99);
  const Scalar _tmp101 = -_tmp100 * _tmp75 + Scalar(1.0);
  const Scalar _tmp102 = _tmp72 * _tmp76;
  const Scalar _tmp103 = _tmp19 + Scalar(110.0);
  const Scalar _tmp104 = _tmp29 + Scalar(-125.0);
  const Scalar _tmp105 =
      std::pow(Scalar(std::pow(_tmp103, Scalar(2)) + std::pow(_tmp104, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp106 = _tmp103 * _tmp105;
  const Scalar _tmp107 = _tmp106 * fh1;
  const Scalar _tmp108 = Scalar(1.0) * _tmp97;
  const Scalar _tmp109 = _tmp72 * _tmp77;
  const Scalar _tmp110 = _tmp104 * _tmp105;
  const Scalar _tmp111 = fh1 * (_tmp106 * _tmp28 - _tmp110 * _tmp18);
  const Scalar _tmp112 = _tmp61 * _tmp76;
  const Scalar _tmp113 = _tmp112 * _tmp73 + _tmp62;
  const Scalar _tmp114 = _tmp112 * _tmp84 - _tmp113 * _tmp83 - _tmp57;
  const Scalar _tmp115 = _tmp98 * (_tmp112 * _tmp94 - _tmp114 * _tmp96 - _tmp93);
  const Scalar _tmp116 = _tmp86 * (_tmp114 + _tmp115);
  const Scalar _tmp117 = -_tmp116 * _tmp75 - _tmp61;
  const Scalar _tmp118 = _tmp110 * fh1;
  const Scalar _tmp119 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp120 = _tmp79 + _tmp80 * _tmp83;
  const Scalar _tmp121 = 0;
  const Scalar _tmp122 = -_tmp107 * _tmp92 * (_tmp100 * _tmp54 + _tmp101 * _tmp102) -
                         _tmp111 * _tmp92 * (-_tmp108 * _tmp109 + _tmp108 * _tmp54) -
                         _tmp118 * _tmp92 * (_tmp102 * _tmp117 + _tmp116 * _tmp54 + Scalar(1.0)) -
                         _tmp119 * _tmp92 * (-_tmp109 * _tmp121 + _tmp121 * _tmp54);
  const Scalar _tmp123 = Scalar(1.0) / (_tmp122);
  const Scalar _tmp124 = _tmp50 + _tmp81;
  const Scalar _tmp125 = _tmp124 * _tmp83;
  const Scalar _tmp126 = Scalar(1.0) / (-_tmp125 - _tmp47 + _tmp79);
  const Scalar _tmp127 = _tmp124 * _tmp126;
  const Scalar _tmp128 = _tmp113 + _tmp115 * _tmp127 - _tmp116 * _tmp78;
  const Scalar _tmp129 = Scalar(1.0) * _tmp82;
  const Scalar _tmp130 = Scalar(1.0) * _tmp126;
  const Scalar _tmp131 = _tmp124 * _tmp130;
  const Scalar _tmp132 = _tmp38 * fh1;
  const Scalar _tmp133 = _tmp110 * _tmp132 + Scalar(2.3544) * _tmp24 + _tmp28 * fv1;
  const Scalar _tmp134 = _tmp125 * _tmp130 + Scalar(1.0);
  const Scalar _tmp135 = _tmp130 * _tmp83;
  const Scalar _tmp136 = -_tmp106 * _tmp132 - Scalar(2.3544) * _tmp12 - _tmp18 * fv1;
  const Scalar _tmp137 = -_tmp108 * _tmp78 + _tmp131 * _tmp98;
  const Scalar _tmp138 = _tmp130 * _tmp98;
  const Scalar _tmp139 = _tmp120 * _tmp126;
  const Scalar _tmp140 = -_tmp121 * _tmp78 - _tmp124 * _tmp139 + _tmp81;
  const Scalar _tmp141 = -_tmp100 * _tmp78 + _tmp127 * _tmp99 - _tmp88;
  const Scalar _tmp142 = std::asinh(
      _tmp123 * (Scalar(1.0) * _tmp107 * (-_tmp129 * _tmp141 + _tmp130 * _tmp99) +
                 Scalar(1.0) * _tmp111 * (-_tmp129 * _tmp137 + _tmp138) +
                 Scalar(1.0) * _tmp118 * (_tmp115 * _tmp130 - _tmp128 * _tmp129) +
                 Scalar(1.0) * _tmp119 * (-_tmp120 * _tmp130 - _tmp129 * _tmp140 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp133 * (-_tmp130 + _tmp131 * _tmp82) +
                 Scalar(1.0) * _tmp136 * (-_tmp129 * _tmp134 + _tmp135)));
  const Scalar _tmp143 = Scalar(9.6622558468725703) * _tmp122;
  const Scalar _tmp144 = _tmp108 * _tmp111;
  const Scalar _tmp145 = _tmp119 * _tmp121;
  const Scalar _tmp146 =
      _tmp101 * _tmp107 * _tmp76 + _tmp117 * _tmp118 * _tmp76 - _tmp144 * _tmp77 - _tmp145 * _tmp77;
  const Scalar _tmp147 = Scalar(1.0) / (_tmp146);
  const Scalar _tmp148 = _tmp130 * _tmp133;
  const Scalar _tmp149 =
      std::asinh(_tmp147 * (_tmp107 * _tmp141 * _tmp82 + _tmp111 * _tmp137 * _tmp82 +
                            _tmp118 * _tmp128 * _tmp82 + _tmp119 * _tmp140 * _tmp82 -
                            _tmp124 * _tmp148 * _tmp82 + _tmp134 * _tmp136 * _tmp82));
  const Scalar _tmp150 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp65), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp68), Scalar(2));
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp152 =
      Scalar(12100.0) * std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp51), Scalar(2)) +
      Scalar(15625.0) * std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp48 - 1), Scalar(2));
  const Scalar _tmp153 = _tmp100 * _tmp107 + _tmp116 * _tmp118 + _tmp144 + _tmp145;
  const Scalar _tmp154 = Scalar(1.0) / (_tmp153);
  const Scalar _tmp155 = std::asinh(_tmp154 * (-_tmp107 * _tmp126 * _tmp99 - _tmp111 * _tmp138 -
                                               _tmp115 * _tmp118 * _tmp126 + _tmp119 * _tmp139 -
                                               _tmp135 * _tmp136 + _tmp148));
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp153;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 * (-std::sinh(Scalar(1.0) * _tmp31) -
                std::sinh(Scalar(0.1034955) * _tmp0 * (-std::sqrt(_tmp30) - _tmp31 * _tmp32))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp30 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp38 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(1, 0) =
      _tmp143 *
          (-std::sinh(Scalar(1.0) * _tmp142) -
           std::sinh(Scalar(0.1034955) * _tmp123 * (-_tmp142 * _tmp143 - std::sqrt(_tmp45)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp45 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp57 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(2, 0) =
      _tmp151 *
          (-std::sinh(Scalar(1.0) * _tmp149) -
           std::sinh(Scalar(0.1034955) * _tmp147 * (-_tmp149 * _tmp151 - std::sqrt(_tmp150)))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp150 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp63 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));
  _res(3, 0) =
      _tmp156 *
          (-std::sinh(Scalar(1.0) * _tmp155) -
           std::sinh(Scalar(0.1034955) * _tmp154 * (-std::sqrt(_tmp152) - _tmp155 * _tmp156))) -
      Scalar(48.0) * std::sqrt(Scalar(
                         Scalar(0.00043402777777777775) * _tmp152 +
                         std::pow(Scalar(-Scalar(0.020833333333333332) * _tmp56 -
                                         Scalar(0.020833333333333332) * position_vector(2, 0) + 1),
                                  Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
