// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl1(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 599

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp10 = 2 * _tmp5;
  const Scalar _tmp11 = _tmp10 * _tmp9;
  const Scalar _tmp12 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp13 = _tmp12 * _tmp3;
  const Scalar _tmp14 = _tmp11 - _tmp13;
  const Scalar _tmp15 = Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp15;
  const Scalar _tmp17 = 2 * _tmp3 * _tmp9;
  const Scalar _tmp18 = _tmp12 * _tmp5;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp16 - _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = 1 - 2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp25 = -_tmp24;
  const Scalar _tmp26 = _tmp10 * _tmp3;
  const Scalar _tmp27 = _tmp12 * _tmp9;
  const Scalar _tmp28 = _tmp26 + _tmp27;
  const Scalar _tmp29 = -Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = _tmp25 + _tmp31;
  const Scalar _tmp33 = _tmp32 + position_vector(0, 0);
  const Scalar _tmp34 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp35 = _tmp33 + Scalar(125.0);
  const Scalar _tmp36 = _tmp22 + Scalar(110.0);
  const Scalar _tmp37 = std::pow(Scalar(std::pow(_tmp35, Scalar(2)) + std::pow(_tmp36, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = _tmp35 * _tmp37;
  const Scalar _tmp39 = Scalar(0.20999999999999999) * _tmp26 - Scalar(0.20999999999999999) * _tmp27;
  const Scalar _tmp40 = -_tmp39;
  const Scalar _tmp41 =
      -Scalar(0.010999999999999999) * _tmp23 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp42 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp43 = _tmp41 - _tmp42;
  const Scalar _tmp44 = fh1 * (_tmp40 + _tmp43);
  const Scalar _tmp45 = Scalar(2.3544) * _tmp28 + _tmp32 * fv1 + _tmp38 * _tmp44;
  const Scalar _tmp46 = _tmp24 + _tmp31;
  const Scalar _tmp47 = Scalar(1.0) * _tmp46;
  const Scalar _tmp48 = _tmp16 + _tmp19 + _tmp8;
  const Scalar _tmp49 = Scalar(1.0) * _tmp48;
  const Scalar _tmp50 = -_tmp49;
  const Scalar _tmp51 = _tmp19 + _tmp7;
  const Scalar _tmp52 = _tmp16 + _tmp51;
  const Scalar _tmp53 = _tmp50 + _tmp52;
  const Scalar _tmp54 = _tmp20 + _tmp7;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp50 + _tmp54);
  const Scalar _tmp56 = _tmp29 + _tmp30;
  const Scalar _tmp57 = _tmp25 + _tmp56;
  const Scalar _tmp58 = _tmp55 * (_tmp47 - _tmp57);
  const Scalar _tmp59 = _tmp53 * _tmp58;
  const Scalar _tmp60 = _tmp24 + _tmp56;
  const Scalar _tmp61 = Scalar(1.0) / (_tmp47 - _tmp59 - _tmp60);
  const Scalar _tmp62 = Scalar(1.0) * _tmp61;
  const Scalar _tmp63 = _tmp53 * _tmp55;
  const Scalar _tmp64 = Scalar(1.0) * _tmp62 * _tmp63 - Scalar(1.0) * _tmp62;
  const Scalar _tmp65 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp66 = _tmp47 + _tmp49 * _tmp58;
  const Scalar _tmp67 = 0;
  const Scalar _tmp68 = _tmp41 + _tmp42;
  const Scalar _tmp69 = _tmp39 + _tmp68;
  const Scalar _tmp70 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp71 = _tmp70 + Scalar(-110.0);
  const Scalar _tmp72 = _tmp60 + position_vector(0, 0);
  const Scalar _tmp73 = _tmp72 + Scalar(-125.0);
  const Scalar _tmp74 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp73, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp77 = _tmp76 + Scalar(-125.0);
  const Scalar _tmp78 = Scalar(1.0) / (_tmp77);
  const Scalar _tmp79 = _tmp48 + position_vector(1, 0);
  const Scalar _tmp80 = _tmp79 + Scalar(110.0);
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp71 * _tmp74;
  const Scalar _tmp83 = _tmp75 * _tmp81 - _tmp82;
  const Scalar _tmp84 = _tmp40 + _tmp68;
  const Scalar _tmp85 = _tmp57 + position_vector(0, 0);
  const Scalar _tmp86 = _tmp85 + Scalar(125.0);
  const Scalar _tmp87 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp88 = _tmp87 + Scalar(-110.0);
  const Scalar _tmp89 = std::pow(Scalar(std::pow(_tmp86, Scalar(2)) + std::pow(_tmp88, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp90 = _tmp86 * _tmp89;
  const Scalar _tmp91 = _tmp39 + _tmp43;
  const Scalar _tmp92 = _tmp88 * _tmp89;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp81 * _tmp90 - _tmp92);
  const Scalar _tmp94 = _tmp93 * (-_tmp84 * _tmp90 + _tmp90 * _tmp91);
  const Scalar _tmp95 = _tmp81 * _tmp91;
  const Scalar _tmp96 = _tmp93 * (_tmp84 * _tmp92 - _tmp90 * _tmp95);
  const Scalar _tmp97 = _tmp69 * _tmp82 - _tmp75 * _tmp95 - _tmp83 * _tmp96;
  const Scalar _tmp98 = -_tmp58 * _tmp97 - _tmp69 * _tmp75 + _tmp75 * _tmp91 - _tmp83 * _tmp94;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp100 = _tmp97 * _tmp99;
  const Scalar _tmp101 = _tmp61 * _tmp66;
  const Scalar _tmp102 = _tmp55 * (-_tmp100 * _tmp67 - _tmp101 * _tmp53 + _tmp50);
  const Scalar _tmp103 = -Scalar(1.0) * _tmp102 - Scalar(1.0) * _tmp62 * _tmp66 + Scalar(1.0);
  const Scalar _tmp104 = _tmp81 * _tmp96 + _tmp95;
  const Scalar _tmp105 = -_tmp104 * _tmp58 + _tmp81 * _tmp94 - _tmp91;
  const Scalar _tmp106 =
      std::sqrt(Scalar(std::pow(_tmp77, Scalar(2)) + std::pow(_tmp80, Scalar(2))));
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp106 * _tmp78;
  const Scalar _tmp109 = _tmp108 * (_tmp107 * _tmp46 * _tmp80 - _tmp107 * _tmp48 * _tmp77);
  const Scalar _tmp110 = _tmp93 * (_tmp109 * _tmp90 + _tmp54 * _tmp90 - _tmp57 * _tmp92);
  const Scalar _tmp111 = _tmp109 * _tmp75 - _tmp110 * _tmp83 + _tmp52 * _tmp75 - _tmp60 * _tmp82;
  const Scalar _tmp112 = _tmp111 * _tmp99;
  const Scalar _tmp113 = Scalar(1.0) / (_tmp111);
  const Scalar _tmp114 = _tmp113 * _tmp98;
  const Scalar _tmp115 = _tmp114 * (-_tmp105 * _tmp112 - _tmp109 + _tmp110 * _tmp81);
  const Scalar _tmp116 = _tmp105 + _tmp115;
  const Scalar _tmp117 = _tmp53 * _tmp61;
  const Scalar _tmp118 = -_tmp100 * _tmp116 + _tmp104 + _tmp115 * _tmp117;
  const Scalar _tmp119 = Scalar(1.0) * _tmp55;
  const Scalar _tmp120 = _tmp38 * fh1;
  const Scalar _tmp121 = _tmp114 * _tmp62;
  const Scalar _tmp122 = Scalar(1.0) * _tmp113;
  const Scalar _tmp123 = _tmp121 * _tmp53 - _tmp122 * _tmp97;
  const Scalar _tmp124 = _tmp36 * _tmp37;
  const Scalar _tmp125 = fh1 * (_tmp124 * _tmp32 - _tmp21 * _tmp38);
  const Scalar _tmp126 = -_tmp124 * _tmp44 - Scalar(2.3544) * _tmp14 - _tmp21 * fv1;
  const Scalar _tmp127 = _tmp59 * _tmp62 + Scalar(1.0);
  const Scalar _tmp128 = _tmp58 * _tmp62;
  const Scalar _tmp129 = -Scalar(1.0) * _tmp119 * _tmp127 + Scalar(1.0) * _tmp128;
  const Scalar _tmp130 = Scalar(1.0) * _tmp96;
  const Scalar _tmp131 = _tmp130 * _tmp58 - Scalar(1.0) * _tmp94;
  const Scalar _tmp132 = _tmp114 * (-Scalar(1.0) * _tmp110 - _tmp112 * _tmp131);
  const Scalar _tmp133 = _tmp131 + _tmp132;
  const Scalar _tmp134 = -_tmp100 * _tmp133 + _tmp117 * _tmp132 - _tmp130;
  const Scalar _tmp135 = _tmp124 * fh1;
  const Scalar _tmp136 =
      _tmp103 * _tmp65 + Scalar(1.0) * _tmp120 * (_tmp115 * _tmp62 - _tmp118 * _tmp119) +
      Scalar(1.0) * _tmp125 * (-_tmp119 * _tmp123 + _tmp121) + _tmp126 * _tmp129 +
      Scalar(1.0) * _tmp135 * (-_tmp119 * _tmp134 + _tmp132 * _tmp62) + _tmp45 * _tmp64;
  const Scalar _tmp137 = _tmp67 * _tmp99;
  const Scalar _tmp138 = _tmp90 * _tmp93;
  const Scalar _tmp139 = _tmp138 * _tmp83;
  const Scalar _tmp140 = _tmp108 * (-_tmp137 * _tmp139 + _tmp137 * _tmp75);
  const Scalar _tmp141 = _tmp83 * _tmp99;
  const Scalar _tmp142 = -_tmp133 * _tmp141 + Scalar(1.0);
  const Scalar _tmp143 = _tmp75 * _tmp99;
  const Scalar _tmp144 = -_tmp116 * _tmp141 - _tmp81;
  const Scalar _tmp145 =
      -_tmp108 * _tmp120 * (_tmp116 * _tmp143 + _tmp138 * _tmp144 + Scalar(1.0)) -
      _tmp108 * _tmp125 * (-_tmp122 * _tmp139 + _tmp122 * _tmp75) -
      _tmp108 * _tmp135 * (_tmp133 * _tmp143 + _tmp138 * _tmp142) - _tmp140 * _tmp65;
  const Scalar _tmp146 = Scalar(1.0) / (_tmp145);
  const Scalar _tmp147 = std::asinh(_tmp136 * _tmp146);
  const Scalar _tmp148 = Scalar(1.0) * _tmp147;
  const Scalar _tmp149 = Scalar(9.6622558468725703) * _tmp145;
  const Scalar _tmp150 =
      -_tmp147 * _tmp149 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp76), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp79 - 1), Scalar(2))));
  const Scalar _tmp151 = Scalar(0.1034955) * _tmp146;
  const Scalar _tmp152 = _tmp150 * _tmp151;
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp140;
  const Scalar _tmp154 = std::pow(_tmp145, Scalar(-2));
  const Scalar _tmp155 = _tmp140 * _tmp154;
  const Scalar _tmp156 = _tmp15 + _tmp51;
  const Scalar _tmp157 =
      (-_tmp136 * _tmp155 + _tmp146 * (-_tmp103 + _tmp129 * _tmp156 + _tmp32 * _tmp64)) /
      std::sqrt(Scalar(std::pow(_tmp136, Scalar(2)) * _tmp154 + 1));
  const Scalar _tmp158 = _tmp137 * _tmp65;
  const Scalar _tmp159 = _tmp83 * _tmp93;
  const Scalar _tmp160 = _tmp122 * _tmp125;
  const Scalar _tmp161 = _tmp120 * _tmp144 * _tmp93 + _tmp135 * _tmp142 * _tmp93 -
                         _tmp158 * _tmp159 - _tmp159 * _tmp160;
  const Scalar _tmp162 = Scalar(1.0) / (_tmp161);
  const Scalar _tmp163 = _tmp45 * _tmp62;
  const Scalar _tmp164 = _tmp127 * _tmp55;
  const Scalar _tmp165 = _tmp102 * _tmp65 + _tmp118 * _tmp120 * _tmp55 +
                         _tmp123 * _tmp125 * _tmp55 + _tmp126 * _tmp164 +
                         _tmp134 * _tmp135 * _tmp55 - _tmp163 * _tmp63;
  const Scalar _tmp166 = std::asinh(_tmp162 * _tmp165);
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp161;
  const Scalar _tmp168 =
      -_tmp166 * _tmp167 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp87), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp85 - 1), Scalar(2))));
  const Scalar _tmp169 = Scalar(0.1034955) * _tmp162;
  const Scalar _tmp170 = _tmp168 * _tmp169;
  const Scalar _tmp171 = std::pow(_tmp161, Scalar(-2));
  const Scalar _tmp172 = _tmp137 * _tmp159 * _tmp171;
  const Scalar _tmp173 = _tmp32 * _tmp62;
  const Scalar _tmp174 =
      (_tmp162 * (-_tmp102 + _tmp156 * _tmp164 - _tmp173 * _tmp63) - _tmp165 * _tmp172) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp171 + 1));
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp137;
  const Scalar _tmp176 = _tmp159 * _tmp175;
  const Scalar _tmp177 = Scalar(1.0) * _tmp166;
  const Scalar _tmp178 =
      _tmp116 * _tmp120 * _tmp99 + _tmp133 * _tmp135 * _tmp99 + _tmp158 + _tmp160;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = _tmp101 * _tmp65 - _tmp115 * _tmp120 * _tmp61 - _tmp121 * _tmp125 -
                         _tmp126 * _tmp128 - _tmp132 * _tmp135 * _tmp61 + _tmp163;
  const Scalar _tmp181 = std::asinh(_tmp179 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp184 =
      -_tmp181 * _tmp183 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp70), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp72), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp188 = _tmp137 * _tmp187;
  const Scalar _tmp189 = (_tmp179 * (-_tmp101 - _tmp128 * _tmp156 + _tmp173) + _tmp180 * _tmp188) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp187 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp34 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp22 - 1),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp33 - 1),
                                           Scalar(2)))))));
  _res(1, 0) = _tmp149 * (-Scalar(1.0) * _tmp157 * std::cosh(_tmp148) -
                          (-Scalar(0.1034955) * _tmp150 * _tmp155 +
                           _tmp151 * (-_tmp147 * _tmp153 - _tmp149 * _tmp157)) *
                              std::cosh(_tmp152)) +
               _tmp153 * (-std::sinh(_tmp148) - std::sinh(_tmp152));
  _res(2, 0) = _tmp167 * (-Scalar(1.0) * _tmp174 * std::cosh(_tmp177) -
                          (-Scalar(0.1034955) * _tmp168 * _tmp172 +
                           _tmp169 * (-_tmp166 * _tmp176 - _tmp167 * _tmp174)) *
                              std::cosh(_tmp170)) +
               _tmp176 * (-std::sinh(_tmp170) - std::sinh(_tmp177));
  _res(3, 0) = -_tmp175 * (-std::sinh(_tmp182) - std::sinh(_tmp186)) +
               _tmp183 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp182) -
                          (Scalar(0.1034955) * _tmp184 * _tmp188 +
                           _tmp185 * (_tmp175 * _tmp181 - _tmp183 * _tmp189)) *
                              std::cosh(_tmp186));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
