// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl20
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl20(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 596

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (192)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp6;
  const Scalar _tmp14 = _tmp13 * _tmp4;
  const Scalar _tmp15 = _tmp3 * _tmp7;
  const Scalar _tmp16 = _tmp14 + _tmp15;
  const Scalar _tmp17 = -Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp12 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp10 +
                        Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999);
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp24 = _tmp13 * _tmp3;
  const Scalar _tmp25 = _tmp4 * _tmp7;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = -_tmp23 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = -_tmp22;
  const Scalar _tmp34 = _tmp23 + _tmp33;
  const Scalar _tmp35 = _tmp28 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = -_tmp9;
  const Scalar _tmp38 = _tmp12 + _tmp17;
  const Scalar _tmp39 = _tmp37 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = Scalar(19.620000000000001) - fv1;
  const Scalar _tmp42 = Scalar(1.0) * _tmp39;
  const Scalar _tmp43 = Scalar(1.0) * _tmp35;
  const Scalar _tmp44 = -_tmp43;
  const Scalar _tmp45 = _tmp29 + _tmp33;
  const Scalar _tmp46 = Scalar(1.0) / (_tmp44 + _tmp45);
  const Scalar _tmp47 = _tmp18 + _tmp37;
  const Scalar _tmp48 = _tmp46 * (_tmp42 - _tmp47);
  const Scalar _tmp49 = _tmp42 + _tmp43 * _tmp48;
  const Scalar _tmp50 = _tmp22 + _tmp23 + _tmp28;
  const Scalar _tmp51 = _tmp44 + _tmp50;
  const Scalar _tmp52 = _tmp48 * _tmp51;
  const Scalar _tmp53 = _tmp38 + _tmp9;
  const Scalar _tmp54 = Scalar(1.0) / (_tmp42 - _tmp52 - _tmp53);
  const Scalar _tmp55 = Scalar(1.0) * _tmp54;
  const Scalar _tmp56 = _tmp49 * _tmp54;
  const Scalar _tmp57 = 0;
  const Scalar _tmp58 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp59 =
      -Scalar(0.010999999999999999) * _tmp11 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp60 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp61 = _tmp59 + _tmp60;
  const Scalar _tmp62 = _tmp58 + _tmp61;
  const Scalar _tmp63 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp64 = _tmp63 + Scalar(-110.0);
  const Scalar _tmp65 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-125.0);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp66 * _tmp67;
  const Scalar _tmp69 = _tmp40 + Scalar(-125.0);
  const Scalar _tmp70 = Scalar(1.0) / (_tmp69);
  const Scalar _tmp71 = _tmp36 + Scalar(110.0);
  const Scalar _tmp72 = _tmp70 * _tmp71;
  const Scalar _tmp73 = -_tmp58;
  const Scalar _tmp74 = _tmp61 + _tmp73;
  const Scalar _tmp75 = _tmp68 * _tmp74;
  const Scalar _tmp76 = _tmp59 - _tmp60;
  const Scalar _tmp77 = _tmp73 + _tmp76;
  const Scalar _tmp78 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp79 = _tmp78 + Scalar(110.0);
  const Scalar _tmp80 = _tmp47 + position_vector(0, 0);
  const Scalar _tmp81 = _tmp80 + Scalar(125.0);
  const Scalar _tmp82 = std::pow(Scalar(std::pow(_tmp79, Scalar(2)) + std::pow(_tmp81, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp83 = _tmp79 * _tmp82;
  const Scalar _tmp84 = _tmp81 * _tmp82;
  const Scalar _tmp85 = _tmp72 * _tmp74;
  const Scalar _tmp86 = _tmp77 * _tmp83 - _tmp84 * _tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp72 * _tmp84 - _tmp83);
  const Scalar _tmp88 = _tmp64 * _tmp67;
  const Scalar _tmp89 = _tmp68 * _tmp72 - _tmp88;
  const Scalar _tmp90 = _tmp87 * _tmp89;
  const Scalar _tmp91 = _tmp62 * _tmp88 - _tmp72 * _tmp75 - _tmp86 * _tmp90;
  const Scalar _tmp92 = _tmp74 * _tmp84 - _tmp77 * _tmp84;
  const Scalar _tmp93 = -_tmp48 * _tmp91 - _tmp62 * _tmp68 + _tmp75 - _tmp90 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp91 * _tmp94;
  const Scalar _tmp96 = _tmp46 * (_tmp44 - _tmp51 * _tmp56 - _tmp57 * _tmp95);
  const Scalar _tmp97 = -Scalar(1.0) * _tmp49 * _tmp55 - Scalar(1.0) * _tmp96 + Scalar(1.0);
  const Scalar _tmp98 = _tmp31 + Scalar(-110.0);
  const Scalar _tmp99 = _tmp20 + Scalar(125.0);
  const Scalar _tmp100 = std::pow(Scalar(std::pow(_tmp98, Scalar(2)) + std::pow(_tmp99, Scalar(2))),
                                  Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp101 = _tmp100 * _tmp98;
  const Scalar _tmp102 = fh1 * (_tmp58 + _tmp76);
  const Scalar _tmp103 = -_tmp101 * _tmp102 - Scalar(2.3544) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp104 = _tmp48 * _tmp55;
  const Scalar _tmp105 = _tmp52 * _tmp55 + Scalar(1.0);
  const Scalar _tmp106 = Scalar(1.0) * _tmp46;
  const Scalar _tmp107 = Scalar(1.0) * _tmp104 - Scalar(1.0) * _tmp105 * _tmp106;
  const Scalar _tmp108 =
      std::sqrt(Scalar(std::pow(_tmp69, Scalar(2)) + std::pow(_tmp71, Scalar(2))));
  const Scalar _tmp109 = Scalar(1.0) / (_tmp108);
  const Scalar _tmp110 = _tmp108 * _tmp70;
  const Scalar _tmp111 = _tmp110 * (-_tmp109 * _tmp35 * _tmp69 + _tmp109 * _tmp39 * _tmp71);
  const Scalar _tmp112 = _tmp111 * _tmp84 + _tmp45 * _tmp84 - _tmp47 * _tmp83;
  const Scalar _tmp113 = Scalar(1.0) * _tmp87;
  const Scalar _tmp114 = _tmp113 * _tmp86;
  const Scalar _tmp115 = -_tmp113 * _tmp92 + _tmp114 * _tmp48;
  const Scalar _tmp116 = _tmp111 * _tmp68 - _tmp112 * _tmp90 + _tmp50 * _tmp68 - _tmp53 * _tmp88;
  const Scalar _tmp117 = _tmp116 * _tmp94;
  const Scalar _tmp118 = Scalar(1.0) / (_tmp116);
  const Scalar _tmp119 = _tmp118 * _tmp93;
  const Scalar _tmp120 = _tmp119 * (-_tmp112 * _tmp113 - _tmp115 * _tmp117);
  const Scalar _tmp121 = _tmp51 * _tmp54;
  const Scalar _tmp122 = _tmp115 + _tmp120;
  const Scalar _tmp123 = -_tmp114 + _tmp120 * _tmp121 - _tmp122 * _tmp95;
  const Scalar _tmp124 = _tmp101 * fh1;
  const Scalar _tmp125 = _tmp72 * _tmp87;
  const Scalar _tmp126 = _tmp125 * _tmp86 + _tmp85;
  const Scalar _tmp127 = _tmp125 * _tmp92 - _tmp126 * _tmp48 - _tmp74;
  const Scalar _tmp128 = _tmp119 * (-_tmp111 + _tmp112 * _tmp125 - _tmp117 * _tmp127);
  const Scalar _tmp129 = _tmp94 * (_tmp127 + _tmp128);
  const Scalar _tmp130 = _tmp121 * _tmp128 + _tmp126 - _tmp129 * _tmp91;
  const Scalar _tmp131 = _tmp100 * _tmp99;
  const Scalar _tmp132 = _tmp131 * fh1;
  const Scalar _tmp133 = _tmp102 * _tmp131 + Scalar(2.3544) * _tmp16 + _tmp19 * fv1;
  const Scalar _tmp134 = _tmp46 * _tmp51;
  const Scalar _tmp135 = Scalar(1.0) * _tmp134 * _tmp55 - Scalar(1.0) * _tmp55;
  const Scalar _tmp136 = _tmp119 * _tmp55;
  const Scalar _tmp137 = Scalar(1.0) * _tmp118;
  const Scalar _tmp138 = _tmp136 * _tmp51 - _tmp137 * _tmp91;
  const Scalar _tmp139 = fh1 * (_tmp101 * _tmp19 - _tmp131 * _tmp30);
  const Scalar _tmp140 =
      _tmp103 * _tmp107 + Scalar(1.0) * _tmp124 * (-_tmp106 * _tmp123 + _tmp120 * _tmp55) +
      Scalar(1.0) * _tmp132 * (-_tmp106 * _tmp130 + _tmp128 * _tmp55) + _tmp133 * _tmp135 +
      Scalar(1.0) * _tmp139 * (-_tmp106 * _tmp138 + _tmp136) + _tmp41 * _tmp97;
  const Scalar _tmp141 = -_tmp129 * _tmp89 - _tmp72;
  const Scalar _tmp142 = _tmp84 * _tmp87;
  const Scalar _tmp143 = _tmp57 * _tmp94;
  const Scalar _tmp144 = _tmp143 * _tmp90;
  const Scalar _tmp145 = _tmp110 * (_tmp143 * _tmp68 - _tmp144 * _tmp84);
  const Scalar _tmp146 = _tmp122 * _tmp94;
  const Scalar _tmp147 = -_tmp146 * _tmp89 + Scalar(1.0);
  const Scalar _tmp148 = -_tmp110 * _tmp124 * (_tmp142 * _tmp147 + _tmp146 * _tmp68) -
                         _tmp110 * _tmp132 * (_tmp129 * _tmp68 + _tmp141 * _tmp142 + Scalar(1.0)) -
                         _tmp110 * _tmp139 * (_tmp137 * _tmp68 - _tmp137 * _tmp84 * _tmp90) -
                         _tmp145 * _tmp41;
  const Scalar _tmp149 = Scalar(1.0) / (_tmp148);
  const Scalar _tmp150 = std::asinh(_tmp140 * _tmp149);
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp148;
  const Scalar _tmp152 =
      -_tmp150 * _tmp151 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp40), Scalar(2)) +
                     Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp36 - 1), Scalar(2))));
  const Scalar _tmp153 = Scalar(0.1034955) * _tmp149;
  const Scalar _tmp154 = _tmp152 * _tmp153;
  const Scalar _tmp155 = Scalar(1.0) * _tmp150;
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp145;
  const Scalar _tmp157 = std::pow(_tmp148, Scalar(-2));
  const Scalar _tmp158 = _tmp145 * _tmp157;
  const Scalar _tmp159 = _tmp27 + _tmp34;
  const Scalar _tmp160 =
      (-_tmp140 * _tmp158 + _tmp149 * (_tmp107 * _tmp159 + _tmp135 * _tmp19 - _tmp97)) /
      std::sqrt(Scalar(std::pow(_tmp140, Scalar(2)) * _tmp157 + 1));
  const Scalar _tmp161 = _tmp143 * _tmp41;
  const Scalar _tmp162 = _tmp137 * _tmp139;
  const Scalar _tmp163 =
      _tmp124 * _tmp147 * _tmp87 + _tmp132 * _tmp141 * _tmp87 - _tmp161 * _tmp90 - _tmp162 * _tmp90;
  const Scalar _tmp164 = Scalar(1.0) / (_tmp163);
  const Scalar _tmp165 = _tmp105 * _tmp46;
  const Scalar _tmp166 = _tmp133 * _tmp55;
  const Scalar _tmp167 = _tmp103 * _tmp165 + _tmp123 * _tmp124 * _tmp46 +
                         _tmp130 * _tmp132 * _tmp46 - _tmp134 * _tmp166 +
                         _tmp138 * _tmp139 * _tmp46 + _tmp41 * _tmp96;
  const Scalar _tmp168 = std::asinh(_tmp164 * _tmp167);
  const Scalar _tmp169 = Scalar(1.0) * _tmp168;
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp171 =
      -_tmp168 * _tmp170 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(-Scalar(0.0090909090909090905) * _tmp78 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp80 - 1), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp164;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp143;
  const Scalar _tmp175 = _tmp174 * _tmp90;
  const Scalar _tmp176 = std::pow(_tmp163, Scalar(-2));
  const Scalar _tmp177 = _tmp144 * _tmp176;
  const Scalar _tmp178 = _tmp19 * _tmp55;
  const Scalar _tmp179 =
      (_tmp164 * (-_tmp134 * _tmp178 + _tmp159 * _tmp165 - _tmp96) - _tmp167 * _tmp177) /
      std::sqrt(Scalar(std::pow(_tmp167, Scalar(2)) * _tmp176 + 1));
  const Scalar _tmp180 = _tmp124 * _tmp146 + _tmp129 * _tmp132 + _tmp161 + _tmp162;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = -_tmp103 * _tmp104 - _tmp120 * _tmp124 * _tmp54 -
                         _tmp128 * _tmp132 * _tmp54 - _tmp136 * _tmp139 + _tmp166 + _tmp41 * _tmp56;
  const Scalar _tmp183 = std::asinh(_tmp181 * _tmp182);
  const Scalar _tmp184 = Scalar(1.0) * _tmp183;
  const Scalar _tmp185 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp186 = _tmp143 * _tmp185;
  const Scalar _tmp187 = (_tmp181 * (-_tmp104 * _tmp159 + _tmp178 - _tmp56) + _tmp182 * _tmp186) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp185 + 1));
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp189 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp190 =
      -_tmp183 * _tmp188 -
      Scalar(125.0) *
          std::sqrt(
              Scalar(Scalar(0.77439999999999998) *
                         std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp63), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.0080000000000000002) * _tmp65), Scalar(2))));
  const Scalar _tmp191 = _tmp189 * _tmp190;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          Scalar(125.0) *
                              std::sqrt(Scalar(
                                  Scalar(0.77439999999999998) *
                                      std::pow(Scalar(1 - Scalar(0.0090909090909090905) * _tmp31),
                                               Scalar(2)) +
                                  std::pow(Scalar(-Scalar(0.0080000000000000002) * _tmp20 - 1),
                                           Scalar(2)))))));
  _res(1, 0) = _tmp151 * (-Scalar(1.0) * _tmp160 * std::cosh(_tmp155) -
                          (-Scalar(0.1034955) * _tmp152 * _tmp158 +
                           _tmp153 * (-_tmp150 * _tmp156 - _tmp151 * _tmp160)) *
                              std::cosh(_tmp154)) +
               _tmp156 * (-std::sinh(_tmp154) - std::sinh(_tmp155));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp179 * std::cosh(_tmp169) -
                          (-Scalar(0.1034955) * _tmp171 * _tmp177 +
                           _tmp172 * (-_tmp168 * _tmp175 - _tmp170 * _tmp179)) *
                              std::cosh(_tmp173)) +
               _tmp175 * (-std::sinh(_tmp169) - std::sinh(_tmp173));
  _res(3, 0) = -_tmp174 * (-std::sinh(_tmp184) - std::sinh(_tmp191)) +
               _tmp188 * (-Scalar(1.0) * _tmp187 * std::cosh(_tmp184) -
                          (Scalar(0.1034955) * _tmp186 * _tmp190 +
                           _tmp189 * (_tmp174 * _tmp183 - _tmp187 * _tmp188)) *
                              std::cosh(_tmp191));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
