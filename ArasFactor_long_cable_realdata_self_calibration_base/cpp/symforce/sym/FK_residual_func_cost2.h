// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/pose3.h>
#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost2
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     TransformationMatrix: Pose3
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost2(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const sym::Pose3<Scalar>& TransformationMatrix, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar epsilon) {
  // Total ops: 492

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 7, 1>& _TransformationMatrix = TransformationMatrix.Data();

  // Intermediate terms (158)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 =
      _DeltaRot[0] * _TransformationMatrix[3] - _DeltaRot[1] * _TransformationMatrix[2] +
      _DeltaRot[2] * _TransformationMatrix[1] + _DeltaRot[3] * _TransformationMatrix[0];
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 =
      -_DeltaRot[0] * _TransformationMatrix[1] + _DeltaRot[1] * _TransformationMatrix[0] +
      _DeltaRot[2] * _TransformationMatrix[3] + _DeltaRot[3] * _TransformationMatrix[2];
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp5 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp7 = -_tmp6;
  const Scalar _tmp8 =
      _DeltaRot[0] * _TransformationMatrix[2] + _DeltaRot[1] * _TransformationMatrix[3] -
      _DeltaRot[2] * _TransformationMatrix[0] + _DeltaRot[3] * _TransformationMatrix[1];
  const Scalar _tmp9 = 2 * _tmp4;
  const Scalar _tmp10 = _tmp8 * _tmp9;
  const Scalar _tmp11 =
      -2 * _DeltaRot[0] * _TransformationMatrix[0] - 2 * _DeltaRot[1] * _TransformationMatrix[1] -
      2 * _DeltaRot[2] * _TransformationMatrix[2] + 2 * _DeltaRot[3] * _TransformationMatrix[3];
  const Scalar _tmp12 = _tmp11 * _tmp2;
  const Scalar _tmp13 = _tmp10 - _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp2 * _tmp8;
  const Scalar _tmp16 = _tmp11 * _tmp4;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 - _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp7;
  const Scalar _tmp20 = _TransformationMatrix[5] + _tmp19;
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp5;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = _tmp2 * _tmp9;
  const Scalar _tmp25 = _tmp11 * _tmp8;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _TransformationMatrix[4] + _tmp30;
  const Scalar _tmp32 = std::pow(Scalar(-_tmp20 + p_a(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp31 + p_a(0, 0)), Scalar(2));
  const Scalar _tmp33 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 =
      -Scalar(0.010999999999999999) * _tmp21 - Scalar(0.010999999999999999) * _tmp3;
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = _tmp36 + _tmp37;
  const Scalar _tmp41 = _tmp35 + _tmp40;
  const Scalar _tmp42 = _tmp22 + _tmp29;
  const Scalar _tmp43 = _TransformationMatrix[4] + _tmp42;
  const Scalar _tmp44 = _tmp14 + _tmp17;
  const Scalar _tmp45 = _tmp44 + _tmp7;
  const Scalar _tmp46 = _TransformationMatrix[5] + _tmp45;
  const Scalar _tmp47 = std::pow(Scalar(-_tmp43 + p_b(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp46 + p_b(1, 0)), Scalar(2));
  const Scalar _tmp48 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp49 = _tmp34 + _tmp38;
  const Scalar _tmp50 = _tmp18 + _tmp6;
  const Scalar _tmp51 = _TransformationMatrix[5] + _tmp50;
  const Scalar _tmp52 = _tmp51 - p_d(1, 0);
  const Scalar _tmp53 = _tmp27 + _tmp28;
  const Scalar _tmp54 = _tmp23 + _tmp53;
  const Scalar _tmp55 = _TransformationMatrix[4] + _tmp54;
  const Scalar _tmp56 = _tmp55 - p_d(0, 0);
  const Scalar _tmp57 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp56, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp58 = _tmp56 * _tmp57;
  const Scalar _tmp59 = _tmp34 + _tmp40;
  const Scalar _tmp60 = _tmp22 + _tmp53;
  const Scalar _tmp61 = _TransformationMatrix[4] + _tmp60;
  const Scalar _tmp62 = _tmp61 - p_c(0, 0);
  const Scalar _tmp63 = _tmp44 + _tmp6;
  const Scalar _tmp64 = _TransformationMatrix[5] + _tmp63;
  const Scalar _tmp65 = _tmp64 - p_c(1, 0);
  const Scalar _tmp66 = std::pow(Scalar(std::pow(_tmp62, Scalar(2)) + std::pow(_tmp65, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp67 = _tmp62 * _tmp66;
  const Scalar _tmp68 = _tmp41 * _tmp67 - _tmp59 * _tmp67;
  const Scalar _tmp69 = _tmp52 * _tmp57;
  const Scalar _tmp70 = _tmp46 - p_b(1, 0);
  const Scalar _tmp71 = _tmp43 - p_b(0, 0);
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = _tmp70 * _tmp72;
  const Scalar _tmp74 = _tmp58 * _tmp73 - _tmp69;
  const Scalar _tmp75 = _tmp65 * _tmp66;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp67 * _tmp73 - _tmp75);
  const Scalar _tmp77 = _tmp74 * _tmp76;
  const Scalar _tmp78 = _tmp41 * _tmp73;
  const Scalar _tmp79 = _tmp76 * (_tmp59 * _tmp75 - _tmp67 * _tmp78);
  const Scalar _tmp80 = _tmp49 * _tmp69 - _tmp58 * _tmp78 - _tmp74 * _tmp79;
  const Scalar _tmp81 = Scalar(1.0) * _tmp45;
  const Scalar _tmp82 = -_tmp81;
  const Scalar _tmp83 = Scalar(1.0) / (_tmp63 + _tmp82);
  const Scalar _tmp84 = Scalar(1.0) * _tmp42;
  const Scalar _tmp85 = _tmp83 * (-_tmp60 + _tmp84);
  const Scalar _tmp86 = _tmp41 * _tmp58 - _tmp49 * _tmp58 - _tmp68 * _tmp77 - _tmp80 * _tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp86);
  const Scalar _tmp88 = _tmp81 * _tmp85 + _tmp84;
  const Scalar _tmp89 = 0;
  const Scalar _tmp90 = _tmp67 * _tmp77;
  const Scalar _tmp91 =
      std::sqrt(Scalar(std::pow(_tmp70, Scalar(2)) + std::pow(_tmp71, Scalar(2))));
  const Scalar _tmp92 = _tmp72 * _tmp91;
  const Scalar _tmp93 = Scalar(1.0) * _tmp76;
  const Scalar _tmp94 = Scalar(1.0) * _tmp79;
  const Scalar _tmp95 = -_tmp68 * _tmp93 + _tmp85 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp97 = _tmp92 * (_tmp42 * _tmp70 * _tmp96 - _tmp45 * _tmp71 * _tmp96);
  const Scalar _tmp98 = -_tmp60 * _tmp75 + _tmp63 * _tmp67 + _tmp67 * _tmp97;
  const Scalar _tmp99 = _tmp50 * _tmp58 - _tmp54 * _tmp69 + _tmp58 * _tmp97 - _tmp77 * _tmp98;
  const Scalar _tmp100 = _tmp87 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp102 = _tmp101 * _tmp86;
  const Scalar _tmp103 = _tmp102 * (-_tmp100 * _tmp95 - _tmp93 * _tmp98);
  const Scalar _tmp104 = _tmp87 * (_tmp103 + _tmp95);
  const Scalar _tmp105 = -_tmp104 * _tmp74 + Scalar(1.0);
  const Scalar _tmp106 = _tmp67 * _tmp76;
  const Scalar _tmp107 = _tmp31 - p_a(0, 0);
  const Scalar _tmp108 = _tmp20 - p_a(1, 0);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp108 * _tmp109;
  const Scalar _tmp111 = _tmp110 * fh1;
  const Scalar _tmp112 = _tmp73 * _tmp76;
  const Scalar _tmp113 = _tmp73 * _tmp79 + _tmp78;
  const Scalar _tmp114 = _tmp112 * _tmp68 - _tmp113 * _tmp85 - _tmp41;
  const Scalar _tmp115 = _tmp102 * (-_tmp100 * _tmp114 + _tmp112 * _tmp98 - _tmp97);
  const Scalar _tmp116 = _tmp87 * (_tmp114 + _tmp115);
  const Scalar _tmp117 = -_tmp116 * _tmp74 - _tmp73;
  const Scalar _tmp118 = _tmp107 * _tmp109;
  const Scalar _tmp119 = _tmp118 * fh1;
  const Scalar _tmp120 = Scalar(1.0) * _tmp101;
  const Scalar _tmp121 = fh1 * (_tmp110 * _tmp30 - _tmp118 * _tmp19);
  const Scalar _tmp122 = -_tmp111 * _tmp92 * (_tmp104 * _tmp58 + _tmp105 * _tmp106) -
                         _tmp119 * _tmp92 * (_tmp106 * _tmp117 + _tmp116 * _tmp58 + Scalar(1.0)) -
                         _tmp121 * _tmp92 * (_tmp120 * _tmp58 - _tmp120 * _tmp90) -
                         _tmp48 * _tmp92 * (_tmp58 * _tmp89 - _tmp89 * _tmp90);
  const Scalar _tmp123 = Scalar(1.0) / (_tmp122);
  const Scalar _tmp124 = _tmp39 * fh1;
  const Scalar _tmp125 = -_tmp110 * _tmp124 - Scalar(40.024799999999999) * _tmp13 - _tmp19 * fv1;
  const Scalar _tmp126 = _tmp50 + _tmp82;
  const Scalar _tmp127 = _tmp126 * _tmp85;
  const Scalar _tmp128 = Scalar(1.0) / (-_tmp127 - _tmp54 + _tmp84);
  const Scalar _tmp129 = Scalar(1.0) * _tmp128;
  const Scalar _tmp130 = _tmp129 * _tmp85;
  const Scalar _tmp131 = _tmp127 * _tmp129 + Scalar(1.0);
  const Scalar _tmp132 = Scalar(1.0) * _tmp83;
  const Scalar _tmp133 = _tmp102 * _tmp129;
  const Scalar _tmp134 = -_tmp120 * _tmp80 + _tmp126 * _tmp133;
  const Scalar _tmp135 = _tmp126 * _tmp128;
  const Scalar _tmp136 = _tmp113 + _tmp115 * _tmp135 - _tmp116 * _tmp80;
  const Scalar _tmp137 = _tmp103 * _tmp135 - _tmp104 * _tmp80 - _tmp94;
  const Scalar _tmp138 = _tmp126 * _tmp83;
  const Scalar _tmp139 = _tmp118 * _tmp124 + Scalar(40.024799999999999) * _tmp26 + _tmp30 * fv1;
  const Scalar _tmp140 = _tmp128 * _tmp88;
  const Scalar _tmp141 = -_tmp126 * _tmp140 - _tmp80 * _tmp89 + _tmp82;
  const Scalar _tmp142 = std::asinh(
      _tmp123 * (Scalar(1.0) * _tmp111 * (_tmp103 * _tmp129 - _tmp132 * _tmp137) +
                 Scalar(1.0) * _tmp119 * (_tmp115 * _tmp129 - _tmp132 * _tmp136) +
                 Scalar(1.0) * _tmp121 * (-_tmp132 * _tmp134 + _tmp133) +
                 Scalar(1.0) * _tmp125 * (_tmp130 - _tmp131 * _tmp132) +
                 Scalar(1.0) * _tmp139 * (_tmp129 * _tmp138 - _tmp129) +
                 Scalar(1.0) * _tmp48 * (-_tmp129 * _tmp88 - _tmp132 * _tmp141 + Scalar(1.0))));
  const Scalar _tmp143 = Scalar(1.4083112389913199) * _tmp122;
  const Scalar _tmp144 = std::pow(Scalar(-_tmp61 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp64 + p_c(1, 0)), Scalar(2));
  const Scalar _tmp145 = _tmp83 * fh1;
  const Scalar _tmp146 = _tmp129 * _tmp139;
  const Scalar _tmp147 = _tmp120 * _tmp121;
  const Scalar _tmp148 = _tmp48 * _tmp89;
  const Scalar _tmp149 =
      _tmp105 * _tmp111 * _tmp76 + _tmp117 * _tmp119 * _tmp76 - _tmp147 * _tmp77 - _tmp148 * _tmp77;
  const Scalar _tmp150 = Scalar(1.0) / (_tmp149);
  const Scalar _tmp151 =
      std::asinh(_tmp150 * (_tmp110 * _tmp137 * _tmp145 + _tmp118 * _tmp136 * _tmp145 +
                            _tmp121 * _tmp134 * _tmp83 + _tmp125 * _tmp131 * _tmp83 -
                            _tmp138 * _tmp146 + _tmp141 * _tmp48 * _tmp83));
  const Scalar _tmp152 = Scalar(1.4083112389913199) * _tmp149;
  const Scalar _tmp153 = std::pow(Scalar(-_tmp51 + p_d(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp55 + p_d(0, 0)), Scalar(2));
  const Scalar _tmp154 = _tmp104 * _tmp111 + _tmp116 * _tmp119 + _tmp147 + _tmp148;
  const Scalar _tmp155 = Scalar(1.0) / (_tmp154);
  const Scalar _tmp156 =
      std::asinh(_tmp155 * (-_tmp103 * _tmp111 * _tmp128 - _tmp115 * _tmp119 * _tmp128 -
                            _tmp121 * _tmp133 - _tmp125 * _tmp130 + _tmp140 * _tmp48 + _tmp146));
  const Scalar _tmp157 = Scalar(1.4083112389913199) * _tmp154;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
          (-std::sinh(Scalar(1.0) * _tmp1) -
           std::sinh(Scalar(0.71007031138673404) * _tmp0 * (-_tmp1 * _tmp33 - std::sqrt(_tmp32)))) -
      std::sqrt(Scalar(
          _tmp32 + std::pow(Scalar(-_TransformationMatrix[6] - _tmp39 + p_a(2, 0)), Scalar(2))));
  _res(1, 0) =
      _tmp143 * (-std::sinh(Scalar(1.0) * _tmp142) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp123 *
                           (-_tmp142 * _tmp143 - std::sqrt(_tmp47)))) -
      std::sqrt(Scalar(
          _tmp47 + std::pow(Scalar(-_TransformationMatrix[6] - _tmp41 + p_b(2, 0)), Scalar(2))));
  _res(2, 0) =
      _tmp152 * (-std::sinh(Scalar(1.0) * _tmp151) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp150 *
                           (-std::sqrt(_tmp144) - _tmp151 * _tmp152))) -
      std::sqrt(Scalar(
          _tmp144 + std::pow(Scalar(-_TransformationMatrix[6] - _tmp59 + p_c(2, 0)), Scalar(2))));
  _res(3, 0) =
      _tmp157 * (-std::sinh(Scalar(1.0) * _tmp156) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp155 *
                           (-std::sqrt(_tmp153) - _tmp156 * _tmp157))) -
      std::sqrt(Scalar(
          _tmp153 + std::pow(Scalar(-_TransformationMatrix[6] - _tmp49 + p_d(2, 0)), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
