// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/pose3.h>
#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost4_wrt_fv1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     TransformationMatrix: Pose3
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost4WrtFv1(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const sym::Pose3<Scalar>& TransformationMatrix, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar epsilon) {
  // Total ops: 460

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 7, 1>& _TransformationMatrix = TransformationMatrix.Data();

  // Intermediate terms (153)
  const Scalar _tmp0 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp1 =
      _DeltaRot[0] * _TransformationMatrix[3] - _DeltaRot[1] * _TransformationMatrix[2] +
      _DeltaRot[2] * _TransformationMatrix[1] + _DeltaRot[3] * _TransformationMatrix[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 =
      -_DeltaRot[0] * _TransformationMatrix[1] + _DeltaRot[1] * _TransformationMatrix[0] +
      _DeltaRot[2] * _TransformationMatrix[3] + _DeltaRot[3] * _TransformationMatrix[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 =
      _DeltaRot[0] * _TransformationMatrix[2] + _DeltaRot[1] * _TransformationMatrix[3] -
      _DeltaRot[2] * _TransformationMatrix[0] + _DeltaRot[3] * _TransformationMatrix[1];
  const Scalar _tmp8 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp9 =
      -2 * _DeltaRot[0] * _TransformationMatrix[0] - 2 * _DeltaRot[1] * _TransformationMatrix[1] -
      2 * _DeltaRot[2] * _TransformationMatrix[2] + 2 * _DeltaRot[3] * _TransformationMatrix[3];
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = -_tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp1;
  const Scalar _tmp14 = _tmp13 * _tmp7;
  const Scalar _tmp15 = _tmp3 * _tmp9;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _TransformationMatrix[5] + _tmp18;
  const Scalar _tmp20 = _tmp19 - p_b(1, 0);
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp23 = _tmp13 * _tmp3;
  const Scalar _tmp24 = _tmp7 * _tmp9;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _TransformationMatrix[4] + _tmp29;
  const Scalar _tmp31 = _tmp30 - p_b(0, 0);
  const Scalar _tmp32 = Scalar(1.0) / (_tmp31);
  const Scalar _tmp33 = _tmp20 * _tmp32;
  const Scalar _tmp34 = _tmp26 + _tmp27;
  const Scalar _tmp35 = _tmp22 + _tmp34;
  const Scalar _tmp36 = _TransformationMatrix[4] + _tmp35;
  const Scalar _tmp37 = _tmp36 - p_c(0, 0);
  const Scalar _tmp38 = _tmp17 + _tmp5;
  const Scalar _tmp39 = _TransformationMatrix[5] + _tmp38;
  const Scalar _tmp40 = _tmp39 - p_c(1, 0);
  const Scalar _tmp41 = std::pow(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp40, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp42 = _tmp37 * _tmp41;
  const Scalar _tmp43 = _tmp40 * _tmp41;
  const Scalar _tmp44 = Scalar(1.0) / (_tmp33 * _tmp42 - _tmp43);
  const Scalar _tmp45 = _tmp42 * _tmp44;
  const Scalar _tmp46 = Scalar(1.0) * _tmp29;
  const Scalar _tmp47 = Scalar(1.0) * _tmp18;
  const Scalar _tmp48 = -_tmp47;
  const Scalar _tmp49 = Scalar(1.0) / (_tmp38 + _tmp48);
  const Scalar _tmp50 = _tmp49 * (-_tmp35 + _tmp46);
  const Scalar _tmp51 = _tmp46 + _tmp47 * _tmp50;
  const Scalar _tmp52 = 0;
  const Scalar _tmp53 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp54 = -_tmp53;
  const Scalar _tmp55 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp56 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp57 = _tmp55 + _tmp56;
  const Scalar _tmp58 = _tmp54 + _tmp57;
  const Scalar _tmp59 = _tmp12 - _tmp16;
  const Scalar _tmp60 = _tmp5 + _tmp59;
  const Scalar _tmp61 = _TransformationMatrix[5] + _tmp60;
  const Scalar _tmp62 = _tmp61 - p_d(1, 0);
  const Scalar _tmp63 = -_tmp22;
  const Scalar _tmp64 = _tmp34 + _tmp63;
  const Scalar _tmp65 = _TransformationMatrix[4] + _tmp64;
  const Scalar _tmp66 = _tmp65 - p_d(0, 0);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp62, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp66 * _tmp67;
  const Scalar _tmp69 = _tmp55 - _tmp56;
  const Scalar _tmp70 = _tmp53 + _tmp69;
  const Scalar _tmp71 = _tmp53 + _tmp57;
  const Scalar _tmp72 = _tmp42 * _tmp70 - _tmp42 * _tmp71;
  const Scalar _tmp73 = _tmp62 * _tmp67;
  const Scalar _tmp74 = _tmp33 * _tmp68 - _tmp73;
  const Scalar _tmp75 = _tmp44 * _tmp74;
  const Scalar _tmp76 = _tmp33 * _tmp70;
  const Scalar _tmp77 = _tmp44 * (-_tmp42 * _tmp76 + _tmp43 * _tmp71);
  const Scalar _tmp78 = _tmp58 * _tmp73 - _tmp68 * _tmp76 - _tmp74 * _tmp77;
  const Scalar _tmp79 = -_tmp50 * _tmp78 - _tmp58 * _tmp68 + _tmp68 * _tmp70 - _tmp72 * _tmp75;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp79);
  const Scalar _tmp81 = _tmp74 * _tmp80;
  const Scalar _tmp82 = _tmp52 * _tmp81;
  const Scalar _tmp83 = _tmp52 * _tmp80;
  const Scalar _tmp84 =
      std::sqrt(Scalar(std::pow(_tmp20, Scalar(2)) + std::pow(_tmp31, Scalar(2))));
  const Scalar _tmp85 = _tmp32 * _tmp84;
  const Scalar _tmp86 = _tmp85 * (-_tmp45 * _tmp82 + _tmp68 * _tmp83);
  const Scalar _tmp87 = Scalar(1.0) * _tmp44;
  const Scalar _tmp88 = Scalar(1.0) * _tmp77;
  const Scalar _tmp89 = _tmp50 * _tmp88 - _tmp72 * _tmp87;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp84);
  const Scalar _tmp91 = _tmp85 * (-_tmp18 * _tmp31 * _tmp90 + _tmp20 * _tmp29 * _tmp90);
  const Scalar _tmp92 = -_tmp35 * _tmp43 + _tmp38 * _tmp42 + _tmp42 * _tmp91;
  const Scalar _tmp93 = _tmp60 * _tmp68 - _tmp64 * _tmp73 + _tmp68 * _tmp91 - _tmp75 * _tmp92;
  const Scalar _tmp94 = _tmp80 * _tmp93;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp96 = _tmp79 * _tmp95;
  const Scalar _tmp97 = _tmp96 * (-_tmp87 * _tmp92 - _tmp89 * _tmp94);
  const Scalar _tmp98 = _tmp89 + _tmp97;
  const Scalar _tmp99 = -_tmp81 * _tmp98 + Scalar(1.0);
  const Scalar _tmp100 = _tmp68 * _tmp80;
  const Scalar _tmp101 = _tmp28 + _tmp63;
  const Scalar _tmp102 = _TransformationMatrix[4] + _tmp101 - p_a(0, 0);
  const Scalar _tmp103 = _tmp59 + _tmp6;
  const Scalar _tmp104 = _TransformationMatrix[5] + _tmp103 - p_a(1, 0);
  const Scalar _tmp105 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp104, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp106 = _tmp104 * _tmp105;
  const Scalar _tmp107 = _tmp106 * fh1;
  const Scalar _tmp108 = _tmp33 * _tmp44;
  const Scalar _tmp109 = _tmp33 * _tmp77 + _tmp76;
  const Scalar _tmp110 = _tmp108 * _tmp72 - _tmp109 * _tmp50 - _tmp70;
  const Scalar _tmp111 = _tmp96 * (_tmp108 * _tmp92 - _tmp110 * _tmp94 - _tmp91);
  const Scalar _tmp112 = _tmp110 + _tmp111;
  const Scalar _tmp113 = -_tmp112 * _tmp81 - _tmp33;
  const Scalar _tmp114 = _tmp102 * _tmp105;
  const Scalar _tmp115 = _tmp114 * fh1;
  const Scalar _tmp116 = Scalar(1.0) * _tmp95;
  const Scalar _tmp117 = fh1 * (_tmp101 * _tmp106 - _tmp103 * _tmp114);
  const Scalar _tmp118 = -_tmp0 * _tmp86 - _tmp107 * _tmp85 * (_tmp100 * _tmp98 + _tmp45 * _tmp99) -
                         _tmp115 * _tmp85 * (_tmp100 * _tmp112 + _tmp113 * _tmp45 + Scalar(1.0)) -
                         _tmp117 * _tmp85 * (-_tmp116 * _tmp42 * _tmp75 + _tmp116 * _tmp68);
  const Scalar _tmp119 = Scalar(1.0) / (_tmp118);
  const Scalar _tmp120 = Scalar(0.71007031138673404) * _tmp119;
  const Scalar _tmp121 = fh1 * (_tmp54 + _tmp69);
  const Scalar _tmp122 = -_tmp103 * fv1 - _tmp106 * _tmp121 - Scalar(40.024799999999999) * _tmp11;
  const Scalar _tmp123 = _tmp48 + _tmp60;
  const Scalar _tmp124 = _tmp123 * _tmp50;
  const Scalar _tmp125 = Scalar(1.0) / (-_tmp124 + _tmp46 - _tmp64);
  const Scalar _tmp126 = Scalar(1.0) * _tmp125;
  const Scalar _tmp127 = _tmp126 * _tmp50;
  const Scalar _tmp128 = _tmp124 * _tmp126 + Scalar(1.0);
  const Scalar _tmp129 = Scalar(1.0) * _tmp49;
  const Scalar _tmp130 = _tmp126 * _tmp96;
  const Scalar _tmp131 = _tmp123 * _tmp126;
  const Scalar _tmp132 = -_tmp116 * _tmp78 + _tmp131 * _tmp96;
  const Scalar _tmp133 = _tmp78 * _tmp80;
  const Scalar _tmp134 = _tmp123 * _tmp125;
  const Scalar _tmp135 = _tmp109 + _tmp111 * _tmp134 - _tmp112 * _tmp133;
  const Scalar _tmp136 = -_tmp133 * _tmp98 + _tmp134 * _tmp97 - _tmp88;
  const Scalar _tmp137 = _tmp101 * fv1 + _tmp114 * _tmp121 + Scalar(40.024799999999999) * _tmp25;
  const Scalar _tmp138 = _tmp125 * _tmp51;
  const Scalar _tmp139 = -_tmp123 * _tmp138 - _tmp133 * _tmp52 + _tmp48;
  const Scalar _tmp140 = std::cosh(
      _tmp120 *
      (-Scalar(1.4083112389913199) * _tmp118 *
           std::asinh(_tmp119 *
                      (Scalar(1.0) * _tmp0 * (-_tmp126 * _tmp51 - _tmp129 * _tmp139 + Scalar(1.0)) +
                       Scalar(1.0) * _tmp107 * (_tmp126 * _tmp97 - _tmp129 * _tmp136) +
                       Scalar(1.0) * _tmp115 * (_tmp111 * _tmp126 - _tmp129 * _tmp135) +
                       Scalar(1.0) * _tmp117 * (-_tmp129 * _tmp132 + _tmp130) +
                       Scalar(1.0) * _tmp122 * (_tmp127 - _tmp128 * _tmp129) +
                       Scalar(1.0) * _tmp137 * (-_tmp126 + _tmp131 * _tmp49))) -
       std::sqrt(Scalar(std::pow(Scalar(-_tmp19 + p_b(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp30 + p_b(0, 0)), Scalar(2))))));
  const Scalar _tmp141 = _tmp116 * _tmp117;
  const Scalar _tmp142 = _tmp44 * _tmp82;
  const Scalar _tmp143 =
      -_tmp0 * _tmp142 + _tmp107 * _tmp44 * _tmp99 + _tmp113 * _tmp115 * _tmp44 - _tmp141 * _tmp75;
  const Scalar _tmp144 = Scalar(1.0) / (_tmp143);
  const Scalar _tmp145 = Scalar(0.71007031138673404) * _tmp144;
  const Scalar _tmp146 = _tmp49 * fh1;
  const Scalar _tmp147 = _tmp126 * _tmp137;
  const Scalar _tmp148 = std::cosh(
      _tmp145 *
      (-Scalar(1.4083112389913199) * _tmp143 *
           std::asinh(_tmp144 * (_tmp0 * _tmp139 * _tmp49 + _tmp106 * _tmp136 * _tmp146 +
                                 _tmp114 * _tmp135 * _tmp146 + _tmp117 * _tmp132 * _tmp49 +
                                 _tmp122 * _tmp128 * _tmp49 - _tmp123 * _tmp147 * _tmp49)) -
       std::sqrt(Scalar(std::pow(Scalar(-_tmp36 + p_c(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp39 + p_c(1, 0)), Scalar(2))))));
  const Scalar _tmp149 =
      _tmp0 * _tmp83 + _tmp107 * _tmp80 * _tmp98 + _tmp112 * _tmp115 * _tmp80 + _tmp141;
  const Scalar _tmp150 = Scalar(1.0) / (_tmp149);
  const Scalar _tmp151 = Scalar(0.71007031138673404) * _tmp150;
  const Scalar _tmp152 = std::cosh(
      _tmp151 * (-Scalar(1.4083112389913199) * _tmp149 *
                     std::asinh(_tmp150 * (_tmp0 * _tmp138 - _tmp107 * _tmp125 * _tmp97 -
                                           _tmp111 * _tmp115 * _tmp125 - _tmp117 * _tmp130 -
                                           _tmp122 * _tmp127 + _tmp147)) -
                 std::sqrt(Scalar(std::pow(Scalar(-_tmp61 + p_d(1, 0)), Scalar(2)) +
                                  std::pow(Scalar(-_tmp65 + p_d(0, 0)), Scalar(2))))));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = 0;
  _res(1, 0) = Scalar(1.0) * _tmp119 * _tmp86 * p_b(2, 0) -
               Scalar(1.4083112389913199) * _tmp120 * _tmp86 * p_b(2, 0);
  _res(2, 0) = -Scalar(1.4083112389913199) * _tmp142 * _tmp145 * p_c(2, 0) +
               _tmp144 * _tmp82 * _tmp87 * p_c(2, 0);
  _res(3, 0) = -Scalar(1.0) * _tmp150 * _tmp83 * p_d(2, 0) +
               Scalar(1.4083112389913199) * _tmp151 * _tmp83 * p_d(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
