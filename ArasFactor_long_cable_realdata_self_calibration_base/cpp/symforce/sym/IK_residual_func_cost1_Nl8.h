// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_Nl8
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1Nl8(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 486

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (163)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(0.71007031138673404) * _tmp0;
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp6;
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = _tmp4 * _tmp7;
  const Scalar _tmp13 = _tmp11 - _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 - _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp25 = _tmp10 * _tmp4;
  const Scalar _tmp26 = _tmp3 * _tmp7;
  const Scalar _tmp27 = _tmp25 + _tmp26;
  const Scalar _tmp28 = -Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = _tmp24 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp34 = -_tmp33;
  const Scalar _tmp35 = -Scalar(0.010999999999999999) * _tmp15 -
                        Scalar(0.010999999999999999) * _tmp23 + Scalar(-0.010999999999999999);
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp25 - Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp37 = _tmp35 + _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = _tmp21 + _tmp29;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 - p_c(0, 0);
  const Scalar _tmp42 = _tmp14 + _tmp17;
  const Scalar _tmp43 = _tmp42 + _tmp9;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp44 - p_c(1, 0);
  const Scalar _tmp46 =
      std::sqrt(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp45, Scalar(2))));
  const Scalar _tmp47 = Scalar(1.0) / (_tmp46);
  const Scalar _tmp48 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp49 = _tmp46 * _tmp48;
  const Scalar _tmp50 = _tmp49 * (_tmp39 * _tmp45 * _tmp47 - _tmp41 * _tmp43 * _tmp47);
  const Scalar _tmp51 = -_tmp24 + _tmp28;
  const Scalar _tmp52 = _tmp21 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 - p_d(0, 0);
  const Scalar _tmp55 = -_tmp9;
  const Scalar _tmp56 = _tmp42 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 - p_d(1, 0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp58 * _tmp59;
  const Scalar _tmp62 = _tmp22 + _tmp51;
  const Scalar _tmp63 = _tmp62 + position_vector(0, 0);
  const Scalar _tmp64 = _tmp63 - p_a(0, 0);
  const Scalar _tmp65 = _tmp18 + _tmp55;
  const Scalar _tmp66 = _tmp65 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 - p_a(1, 0);
  const Scalar _tmp68 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp67, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp69 = _tmp64 * _tmp68;
  const Scalar _tmp70 = _tmp67 * _tmp68;
  const Scalar _tmp71 = _tmp50 * _tmp69 - _tmp62 * _tmp70 + _tmp65 * _tmp69;
  const Scalar _tmp72 = _tmp45 * _tmp48;
  const Scalar _tmp73 = _tmp60 * _tmp72 - _tmp61;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp69 * _tmp72 - _tmp70);
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp50 * _tmp60 - _tmp52 * _tmp61 + _tmp56 * _tmp60 - _tmp71 * _tmp75;
  const Scalar _tmp77 = Scalar(1.0) / (_tmp76);
  const Scalar _tmp78 = Scalar(1.0) * _tmp77;
  const Scalar _tmp79 = _tmp69 * _tmp75;
  const Scalar _tmp80 = _tmp31 - p_b(0, 0);
  const Scalar _tmp81 = _tmp20 - p_b(1, 0);
  const Scalar _tmp82 = std::pow(Scalar(std::pow(_tmp80, Scalar(2)) + std::pow(_tmp81, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp83 = _tmp81 * _tmp82;
  const Scalar _tmp84 = _tmp80 * _tmp82;
  const Scalar _tmp85 = fh1 * (-_tmp19 * _tmp84 + _tmp30 * _tmp83);
  const Scalar _tmp86 = _tmp33 + _tmp37;
  const Scalar _tmp87 = _tmp72 * _tmp86;
  const Scalar _tmp88 = _tmp35 - _tmp36;
  const Scalar _tmp89 = _tmp34 + _tmp88;
  const Scalar _tmp90 = -_tmp69 * _tmp87 + _tmp70 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) * _tmp39;
  const Scalar _tmp92 = -_tmp62 + _tmp91;
  const Scalar _tmp93 = Scalar(1.0) * _tmp43;
  const Scalar _tmp94 = -_tmp93;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp65 + _tmp94);
  const Scalar _tmp96 = Scalar(1.0) * _tmp95;
  const Scalar _tmp97 = _tmp69 * _tmp86 - _tmp69 * _tmp89;
  const Scalar _tmp98 = Scalar(1.0) * _tmp74;
  const Scalar _tmp99 = _tmp74 * _tmp90 * _tmp92 * _tmp96 - _tmp97 * _tmp98;
  const Scalar _tmp100 = _tmp60 * _tmp86;
  const Scalar _tmp101 = _tmp33 + _tmp88;
  const Scalar _tmp102 = -_tmp100 * _tmp72 + _tmp101 * _tmp61 - _tmp75 * _tmp90;
  const Scalar _tmp103 = _tmp92 * _tmp95;
  const Scalar _tmp104 = _tmp100 - _tmp101 * _tmp60 - _tmp102 * _tmp103 - _tmp75 * _tmp97;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp106 = _tmp105 * _tmp76;
  const Scalar _tmp107 = _tmp104 * _tmp77;
  const Scalar _tmp108 = _tmp107 * (-_tmp106 * _tmp99 - _tmp71 * _tmp98);
  const Scalar _tmp109 = _tmp108 + _tmp99;
  const Scalar _tmp110 = _tmp105 * _tmp73;
  const Scalar _tmp111 = -_tmp109 * _tmp110 + Scalar(1.0);
  const Scalar _tmp112 = _tmp69 * _tmp74;
  const Scalar _tmp113 = _tmp105 * _tmp60;
  const Scalar _tmp114 = _tmp83 * fh1;
  const Scalar _tmp115 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp116 = _tmp103 * _tmp93 + _tmp91;
  const Scalar _tmp117 = 0;
  const Scalar _tmp118 = _tmp105 * _tmp117;
  const Scalar _tmp119 = _tmp72 * _tmp74;
  const Scalar _tmp120 = _tmp119 * _tmp90 + _tmp87;
  const Scalar _tmp121 = -_tmp103 * _tmp120 + _tmp119 * _tmp97 - _tmp86;
  const Scalar _tmp122 = _tmp107 * (-_tmp106 * _tmp121 + _tmp119 * _tmp71 - _tmp50);
  const Scalar _tmp123 = _tmp121 + _tmp122;
  const Scalar _tmp124 = -_tmp110 * _tmp123 - _tmp72;
  const Scalar _tmp125 = _tmp84 * fh1;
  const Scalar _tmp126 = -_tmp114 * _tmp49 * (_tmp109 * _tmp113 + _tmp111 * _tmp112) -
                         _tmp115 * _tmp49 * (_tmp118 * _tmp60 - _tmp118 * _tmp79) -
                         _tmp125 * _tmp49 * (_tmp112 * _tmp124 + _tmp113 * _tmp123 + Scalar(1.0)) -
                         _tmp49 * _tmp85 * (_tmp60 * _tmp78 - _tmp78 * _tmp79);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = Scalar(0.71007031138673404) * _tmp127;
  const Scalar _tmp129 = _tmp102 * _tmp105;
  const Scalar _tmp130 = _tmp56 + _tmp94;
  const Scalar _tmp131 = _tmp103 * _tmp130;
  const Scalar _tmp132 = Scalar(1.0) / (-_tmp131 - _tmp52 + _tmp91);
  const Scalar _tmp133 = _tmp116 * _tmp132;
  const Scalar _tmp134 = -_tmp117 * _tmp129 - _tmp130 * _tmp133 + _tmp94;
  const Scalar _tmp135 = Scalar(1.0) * _tmp132;
  const Scalar _tmp136 = _tmp130 * _tmp132;
  const Scalar _tmp137 = _tmp108 * _tmp136 - _tmp109 * _tmp129 - _tmp90 * _tmp98;
  const Scalar _tmp138 = _tmp107 * _tmp135;
  const Scalar _tmp139 = -_tmp102 * _tmp78 + _tmp130 * _tmp138;
  const Scalar _tmp140 = _tmp130 * _tmp95;
  const Scalar _tmp141 = _tmp38 * fh1;
  const Scalar _tmp142 = _tmp141 * _tmp84 + Scalar(40.024799999999999) * _tmp27 + _tmp30 * fv1;
  const Scalar _tmp143 = _tmp120 + _tmp122 * _tmp136 - _tmp123 * _tmp129;
  const Scalar _tmp144 = -Scalar(40.024799999999999) * _tmp13 - _tmp141 * _tmp83 - _tmp19 * fv1;
  const Scalar _tmp145 = _tmp131 * _tmp135 + Scalar(1.0);
  const Scalar _tmp146 = std::asinh(
      _tmp127 * (Scalar(1.0) * _tmp114 * (_tmp108 * _tmp135 - _tmp137 * _tmp96) +
                 Scalar(1.0) * _tmp115 * (-_tmp116 * _tmp135 - _tmp134 * _tmp96 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp125 * (_tmp122 * _tmp135 - _tmp143 * _tmp96) +
                 Scalar(1.0) * _tmp142 * (_tmp135 * _tmp140 - _tmp135) +
                 Scalar(1.0) * _tmp144 * (_tmp103 * _tmp135 - _tmp145 * _tmp96) +
                 Scalar(1.0) * _tmp85 * (_tmp138 - _tmp139 * _tmp96)));
  const Scalar _tmp147 = Scalar(1.4083112389913199) * _tmp126;
  const Scalar _tmp148 = _tmp78 * _tmp85;
  const Scalar _tmp149 = _tmp115 * _tmp118;
  const Scalar _tmp150 =
      _tmp111 * _tmp114 * _tmp74 + _tmp124 * _tmp125 * _tmp74 - _tmp148 * _tmp75 - _tmp149 * _tmp75;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = Scalar(0.71007031138673404) * _tmp151;
  const Scalar _tmp153 = _tmp135 * _tmp142;
  const Scalar _tmp154 = _tmp144 * _tmp95;
  const Scalar _tmp155 = _tmp95 * fh1;
  const Scalar _tmp156 =
      std::asinh(_tmp151 * (_tmp115 * _tmp134 * _tmp95 + _tmp137 * _tmp155 * _tmp83 +
                            _tmp139 * _tmp85 * _tmp95 - _tmp140 * _tmp153 +
                            _tmp143 * _tmp155 * _tmp84 + _tmp145 * _tmp154));
  const Scalar _tmp157 = Scalar(1.4083112389913199) * _tmp150;
  const Scalar _tmp158 =
      _tmp105 * _tmp109 * _tmp114 + _tmp105 * _tmp123 * _tmp125 + _tmp148 + _tmp149;
  const Scalar _tmp159 = Scalar(1.0) / (_tmp158);
  const Scalar _tmp160 = std::asinh(
      _tmp159 * (-_tmp108 * _tmp114 * _tmp132 + _tmp115 * _tmp133 - _tmp122 * _tmp125 * _tmp132 -
                 _tmp135 * _tmp154 * _tmp92 - _tmp138 * _tmp85 + _tmp153));
  const Scalar _tmp161 = Scalar(1.4083112389913199) * _tmp158;
  const Scalar _tmp162 = Scalar(0.71007031138673404) * _tmp159;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp32 * (_tmp2 * p_b(2, 0) + std::cosh(Scalar(1.0) * _tmp1) -
                 std::cosh(_tmp2 *
                           (-_tmp1 * _tmp32 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_b(1, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp31 + p_b(0, 0)), Scalar(2))))))) +
      _tmp38 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp147 *
          (_tmp128 * p_c(2, 0) + std::cosh(Scalar(1.0) * _tmp146) -
           std::cosh(_tmp128 *
                     (-_tmp146 * _tmp147 -
                      std::sqrt(Scalar(std::pow(Scalar(-_tmp40 + p_c(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp44 + p_c(1, 0)), Scalar(2))))))) +
      _tmp86 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp157 *
          (_tmp152 * p_a(2, 0) + std::cosh(Scalar(1.0) * _tmp156) -
           std::cosh(_tmp152 *
                     (-_tmp156 * _tmp157 -
                      std::sqrt(Scalar(std::pow(Scalar(-_tmp63 + p_a(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp66 + p_a(1, 0)), Scalar(2))))))) +
      _tmp89 + position_vector(2, 0);
  _res(3, 0) =
      _tmp101 -
      _tmp161 * (_tmp162 * p_d(2, 0) + std::cosh(Scalar(1.0) * _tmp160) -
                 std::cosh(_tmp162 *
                           (-_tmp160 * _tmp161 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp53 + p_d(0, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp57 + p_d(1, 0)), Scalar(2))))))) +
      position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
