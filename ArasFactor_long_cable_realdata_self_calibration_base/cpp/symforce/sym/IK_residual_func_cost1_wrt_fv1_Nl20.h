// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl20
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl20(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 587

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (196)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -_tmp9;
  const Scalar _tmp11 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp12 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp11 +
                        Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999);
  const Scalar _tmp14 = 2 * _tmp6;
  const Scalar _tmp15 = _tmp14 * _tmp3;
  const Scalar _tmp16 = _tmp4 * _tmp7;
  const Scalar _tmp17 = _tmp15 - _tmp16;
  const Scalar _tmp18 = Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = _tmp13 + _tmp19;
  const Scalar _tmp21 = _tmp10 + _tmp20;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp24 = _tmp14 * _tmp4;
  const Scalar _tmp25 = _tmp3 * _tmp7;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp30 = _tmp27 - _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(0, 0);
  const Scalar _tmp33 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 =
      -Scalar(0.010999999999999999) * _tmp11 - Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = -_tmp23;
  const Scalar _tmp41 = _tmp30 + _tmp40;
  const Scalar _tmp42 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp43 = _tmp42 - p_a(0, 0);
  const Scalar _tmp44 = -_tmp13;
  const Scalar _tmp45 = _tmp10 + _tmp19 + _tmp44;
  const Scalar _tmp46 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp47 = _tmp46 - p_a(1, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp43 * _tmp48;
  const Scalar _tmp50 = _tmp36 + _tmp37;
  const Scalar _tmp51 = _tmp35 + _tmp50;
  const Scalar _tmp52 = _tmp49 * _tmp51;
  const Scalar _tmp53 = -_tmp39 * _tmp49 + _tmp52;
  const Scalar _tmp54 = _tmp27 + _tmp29;
  const Scalar _tmp55 = _tmp40 + _tmp54;
  const Scalar _tmp56 = _tmp55 + position_vector(0, 0);
  const Scalar _tmp57 = _tmp56 - p_b(0, 0);
  const Scalar _tmp58 = Scalar(1.0) / (_tmp57);
  const Scalar _tmp59 = _tmp44 + _tmp9;
  const Scalar _tmp60 = _tmp19 + _tmp59;
  const Scalar _tmp61 = _tmp60 + position_vector(1, 0);
  const Scalar _tmp62 = _tmp61 - p_b(1, 0);
  const Scalar _tmp63 = _tmp58 * _tmp62;
  const Scalar _tmp64 = _tmp47 * _tmp48;
  const Scalar _tmp65 = Scalar(1.0) / (_tmp49 * _tmp63 - _tmp64);
  const Scalar _tmp66 = Scalar(1.0) * _tmp65;
  const Scalar _tmp67 = Scalar(1.0) * _tmp60;
  const Scalar _tmp68 = -_tmp67;
  const Scalar _tmp69 = Scalar(1.0) / (_tmp45 + _tmp68);
  const Scalar _tmp70 = Scalar(1.0) * _tmp55;
  const Scalar _tmp71 = _tmp69 * (-_tmp41 + _tmp70);
  const Scalar _tmp72 = _tmp39 * _tmp64 - _tmp52 * _tmp63;
  const Scalar _tmp73 = _tmp66 * _tmp72;
  const Scalar _tmp74 = -_tmp53 * _tmp66 + _tmp71 * _tmp73;
  const Scalar _tmp75 = _tmp20 + _tmp9;
  const Scalar _tmp76 = _tmp23 + _tmp54;
  const Scalar _tmp77 = _tmp76 + position_vector(0, 0);
  const Scalar _tmp78 = _tmp77 - p_c(0, 0);
  const Scalar _tmp79 = _tmp75 + position_vector(1, 0);
  const Scalar _tmp80 = _tmp79 - p_c(1, 0);
  const Scalar _tmp81 = std::pow(Scalar(std::pow(_tmp78, Scalar(2)) + std::pow(_tmp80, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp82 = _tmp78 * _tmp81;
  const Scalar _tmp83 = _tmp80 * _tmp81;
  const Scalar _tmp84 =
      std::sqrt(Scalar(std::pow(_tmp57, Scalar(2)) + std::pow(_tmp62, Scalar(2))));
  const Scalar _tmp85 = Scalar(1.0) / (_tmp84);
  const Scalar _tmp86 = _tmp58 * _tmp84;
  const Scalar _tmp87 = _tmp86 * (_tmp55 * _tmp62 * _tmp85 - _tmp57 * _tmp60 * _tmp85);
  const Scalar _tmp88 = -_tmp41 * _tmp64 + _tmp45 * _tmp49 + _tmp49 * _tmp87;
  const Scalar _tmp89 = _tmp63 * _tmp82 - _tmp83;
  const Scalar _tmp90 = _tmp65 * _tmp89;
  const Scalar _tmp91 = _tmp75 * _tmp82 - _tmp76 * _tmp83 + _tmp82 * _tmp87 - _tmp88 * _tmp90;
  const Scalar _tmp92 = _tmp51 * _tmp82;
  const Scalar _tmp93 = _tmp34 + _tmp50;
  const Scalar _tmp94 = -_tmp63 * _tmp92 - _tmp72 * _tmp90 + _tmp83 * _tmp93;
  const Scalar _tmp95 = -_tmp53 * _tmp90 - _tmp71 * _tmp94 - _tmp82 * _tmp93 + _tmp92;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp91 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp99 = _tmp95 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp66 * _tmp88 - _tmp74 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp74;
  const Scalar _tmp102 = _tmp82 * _tmp96;
  const Scalar _tmp103 = _tmp89 * _tmp96;
  const Scalar _tmp104 = -_tmp101 * _tmp103 + Scalar(1.0);
  const Scalar _tmp105 = _tmp49 * _tmp65;
  const Scalar _tmp106 = _tmp32 - p_d(0, 0);
  const Scalar _tmp107 = _tmp22 - p_d(1, 0);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp107 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = _tmp63 * _tmp65;
  const Scalar _tmp112 = _tmp111 * _tmp72 + _tmp51 * _tmp63;
  const Scalar _tmp113 = _tmp111 * _tmp53 - _tmp112 * _tmp71 - _tmp51;
  const Scalar _tmp114 = _tmp99 * (_tmp111 * _tmp88 - _tmp113 * _tmp97 - _tmp87);
  const Scalar _tmp115 = _tmp113 + _tmp114;
  const Scalar _tmp116 = -_tmp103 * _tmp115 - _tmp63;
  const Scalar _tmp117 = _tmp106 * _tmp108;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = Scalar(1.0) * _tmp98;
  const Scalar _tmp120 = fh1 * (_tmp109 * _tmp31 - _tmp117 * _tmp21);
  const Scalar _tmp121 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp122 = _tmp67 * _tmp71 + _tmp70;
  const Scalar _tmp123 = 0;
  const Scalar _tmp124 = _tmp123 * _tmp96;
  const Scalar _tmp125 = _tmp124 * _tmp90;
  const Scalar _tmp126 = _tmp86 * (_tmp124 * _tmp82 - _tmp125 * _tmp49);
  const Scalar _tmp127 = -_tmp110 * _tmp86 * (_tmp101 * _tmp102 + _tmp104 * _tmp105) -
                         _tmp118 * _tmp86 * (_tmp102 * _tmp115 + _tmp105 * _tmp116 + Scalar(1.0)) -
                         _tmp120 * _tmp86 * (-_tmp119 * _tmp49 * _tmp90 + _tmp119 * _tmp82) -
                         _tmp121 * _tmp126;
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = _tmp68 + _tmp75;
  const Scalar _tmp130 = _tmp129 * _tmp71;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 + _tmp70 - _tmp76);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp94 * _tmp96;
  const Scalar _tmp134 = _tmp129 * _tmp131;
  const Scalar _tmp135 = _tmp100 * _tmp134 - _tmp101 * _tmp133 - _tmp73;
  const Scalar _tmp136 = Scalar(1.0) * _tmp69;
  const Scalar _tmp137 = fh1 * (_tmp34 + _tmp38);
  const Scalar _tmp138 = _tmp117 * _tmp137 + Scalar(40.024799999999999) * _tmp26 + _tmp31 * fv1;
  const Scalar _tmp139 = _tmp129 * _tmp69;
  const Scalar _tmp140 = Scalar(1.0) * _tmp132 * _tmp139 - Scalar(1.0) * _tmp132;
  const Scalar _tmp141 = _tmp112 + _tmp114 * _tmp134 - _tmp115 * _tmp133;
  const Scalar _tmp142 = -_tmp109 * _tmp137 - Scalar(40.024799999999999) * _tmp17 - _tmp21 * fv1;
  const Scalar _tmp143 = _tmp132 * _tmp71;
  const Scalar _tmp144 = _tmp69 * (_tmp130 * _tmp132 + Scalar(1.0));
  const Scalar _tmp145 = Scalar(1.0) * _tmp143 - Scalar(1.0) * _tmp144;
  const Scalar _tmp146 = _tmp122 * _tmp131;
  const Scalar _tmp147 = _tmp69 * (-_tmp123 * _tmp133 - _tmp129 * _tmp146 + _tmp68);
  const Scalar _tmp148 = -Scalar(1.0) * _tmp122 * _tmp132 - Scalar(1.0) * _tmp147 + Scalar(1.0);
  const Scalar _tmp149 = _tmp132 * _tmp99;
  const Scalar _tmp150 = -_tmp119 * _tmp94 + _tmp129 * _tmp149;
  const Scalar _tmp151 = Scalar(1.0) * _tmp110 * (_tmp100 * _tmp132 - _tmp135 * _tmp136) +
                         Scalar(1.0) * _tmp118 * (_tmp114 * _tmp132 - _tmp136 * _tmp141) +
                         Scalar(1.0) * _tmp120 * (-_tmp136 * _tmp150 + _tmp149) +
                         _tmp121 * _tmp148 + _tmp138 * _tmp140 + _tmp142 * _tmp145;
  const Scalar _tmp152 = std::asinh(_tmp128 * _tmp151);
  const Scalar _tmp153 = Scalar(1.0) * _tmp152;
  const Scalar _tmp154 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp155 = _tmp126 * _tmp154;
  const Scalar _tmp156 = _tmp18 + _tmp59;
  const Scalar _tmp157 =
      (_tmp128 * (_tmp140 * _tmp31 + _tmp145 * _tmp156 - _tmp148) - _tmp151 * _tmp155) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp154 + 1));
  const Scalar _tmp158 = Scalar(0.71007031138673404) * _tmp155;
  const Scalar _tmp159 = Scalar(1.4083112389913199) * _tmp127;
  const Scalar _tmp160 =
      -_tmp152 * _tmp159 - std::sqrt(Scalar(std::pow(Scalar(-_tmp56 + p_b(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp61 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp161 = Scalar(0.71007031138673404) * _tmp128;
  const Scalar _tmp162 = _tmp160 * _tmp161;
  const Scalar _tmp163 = Scalar(1.4083112389913199) * _tmp126;
  const Scalar _tmp164 = _tmp132 * _tmp138;
  const Scalar _tmp165 = _tmp110 * _tmp135 * _tmp69 + _tmp118 * _tmp141 * _tmp69 +
                         _tmp120 * _tmp150 * _tmp69 + _tmp121 * _tmp147 - _tmp139 * _tmp164 +
                         _tmp142 * _tmp144;
  const Scalar _tmp166 = _tmp119 * _tmp120;
  const Scalar _tmp167 = _tmp121 * _tmp124;
  const Scalar _tmp168 =
      _tmp104 * _tmp110 * _tmp65 + _tmp116 * _tmp118 * _tmp65 - _tmp166 * _tmp90 - _tmp167 * _tmp90;
  const Scalar _tmp169 = Scalar(1.0) / (_tmp168);
  const Scalar _tmp170 = std::asinh(_tmp165 * _tmp169);
  const Scalar _tmp171 = Scalar(1.4083112389913199) * _tmp168;
  const Scalar _tmp172 =
      -_tmp170 * _tmp171 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp173 = std::pow(_tmp168, Scalar(-2));
  const Scalar _tmp174 = _tmp125 * _tmp173;
  const Scalar _tmp175 = Scalar(0.71007031138673404) * _tmp174;
  const Scalar _tmp176 = Scalar(1.4083112389913199) * _tmp124;
  const Scalar _tmp177 = _tmp176 * _tmp90;
  const Scalar _tmp178 = _tmp132 * _tmp31;
  const Scalar _tmp179 =
      (-_tmp165 * _tmp174 + _tmp169 * (-_tmp139 * _tmp178 + _tmp144 * _tmp156 - _tmp147)) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp173 + 1));
  const Scalar _tmp180 = Scalar(0.71007031138673404) * _tmp169;
  const Scalar _tmp181 = _tmp172 * _tmp180;
  const Scalar _tmp182 = Scalar(1.0) * _tmp170;
  const Scalar _tmp183 = -_tmp100 * _tmp110 * _tmp131 - _tmp114 * _tmp118 * _tmp131 -
                         _tmp120 * _tmp149 + _tmp121 * _tmp146 - _tmp142 * _tmp143 + _tmp164;
  const Scalar _tmp184 =
      _tmp101 * _tmp110 * _tmp96 + _tmp115 * _tmp118 * _tmp96 + _tmp166 + _tmp167;
  const Scalar _tmp185 = Scalar(1.0) / (_tmp184);
  const Scalar _tmp186 = std::asinh(_tmp183 * _tmp185);
  const Scalar _tmp187 = Scalar(1.4083112389913199) * _tmp184;
  const Scalar _tmp188 = std::pow(_tmp184, Scalar(-2));
  const Scalar _tmp189 = _tmp124 * _tmp188;
  const Scalar _tmp190 = (_tmp183 * _tmp189 + _tmp185 * (-_tmp143 * _tmp156 - _tmp146 + _tmp178)) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp188 + 1));
  const Scalar _tmp191 = Scalar(0.71007031138673404) * _tmp185;
  const Scalar _tmp192 =
      -_tmp186 * _tmp187 - std::sqrt(Scalar(std::pow(Scalar(-_tmp77 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp79 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp193 = Scalar(0.71007031138673404) * _tmp189;
  const Scalar _tmp194 = _tmp191 * _tmp192;
  const Scalar _tmp195 = Scalar(1.0) * _tmp186;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.71007031138673404) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_d(1, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp32 + p_d(0, 0)), Scalar(2)))))));
  _res(1, 0) =
      -_tmp159 * (Scalar(1.0) * _tmp157 * std::sinh(_tmp153) - _tmp158 * p_b(2, 0) -
                  (-_tmp158 * _tmp160 + _tmp161 * (-_tmp152 * _tmp163 - _tmp157 * _tmp159)) *
                      std::sinh(_tmp162)) -
      _tmp163 * (_tmp161 * p_b(2, 0) + std::cosh(_tmp153) - std::cosh(_tmp162));
  _res(2, 0) =
      -_tmp171 * (-_tmp175 * p_a(2, 0) + Scalar(1.0) * _tmp179 * std::sinh(_tmp182) -
                  (-_tmp172 * _tmp175 + _tmp180 * (-_tmp170 * _tmp177 - _tmp171 * _tmp179)) *
                      std::sinh(_tmp181)) -
      _tmp177 * (_tmp180 * p_a(2, 0) - std::cosh(_tmp181) + std::cosh(_tmp182));
  _res(3, 0) = _tmp176 * (_tmp191 * p_c(2, 0) - std::cosh(_tmp194) + std::cosh(_tmp195)) -
               _tmp187 * (Scalar(1.0) * _tmp190 * std::sinh(_tmp195) + _tmp193 * p_c(2, 0) -
                          (_tmp191 * (_tmp176 * _tmp186 - _tmp187 * _tmp190) + _tmp192 * _tmp193) *
                              std::sinh(_tmp194));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
