// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost4_wrt_fv1_Nl0
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost4WrtFv1Nl0(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 578

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp10 = 2 * _tmp3 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp5;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp5;
  const Scalar _tmp15 = _tmp14 * _tmp9;
  const Scalar _tmp16 = _tmp11 * _tmp3;
  const Scalar _tmp17 = _tmp15 - _tmp16;
  const Scalar _tmp18 = Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = -_tmp13 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + position_vector(1, 0);
  const Scalar _tmp23 = 1 - 2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp25 = -_tmp24;
  const Scalar _tmp26 = _tmp14 * _tmp3;
  const Scalar _tmp27 = _tmp11 * _tmp9;
  const Scalar _tmp28 = _tmp26 + _tmp27;
  const Scalar _tmp29 = -Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = _tmp25 + _tmp31;
  const Scalar _tmp33 = _tmp32 + position_vector(0, 0);
  const Scalar _tmp34 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp35 = _tmp13 + _tmp19 + _tmp8;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp24 + _tmp31;
  const Scalar _tmp38 = _tmp37 + position_vector(0, 0);
  const Scalar _tmp39 = _tmp38 - p_b(0, 0);
  const Scalar _tmp40 = _tmp36 - p_b(1, 0);
  const Scalar _tmp41 =
      std::sqrt(Scalar(std::pow(_tmp39, Scalar(2)) + std::pow(_tmp40, Scalar(2))));
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = Scalar(1.0) / (_tmp39);
  const Scalar _tmp44 = _tmp41 * _tmp43;
  const Scalar _tmp45 = _tmp44 * (-_tmp35 * _tmp39 * _tmp42 + _tmp37 * _tmp40 * _tmp42);
  const Scalar _tmp46 = Scalar(0.20999999999999999) * _tmp26 - Scalar(0.20999999999999999) * _tmp27;
  const Scalar _tmp47 =
      -Scalar(0.010999999999999999) * _tmp23 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp48 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp49 = _tmp47 + _tmp48;
  const Scalar _tmp50 = _tmp46 + _tmp49;
  const Scalar _tmp51 = _tmp29 + _tmp30;
  const Scalar _tmp52 = _tmp24 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 - p_c(0, 0);
  const Scalar _tmp55 = _tmp13 + _tmp7;
  const Scalar _tmp56 = _tmp19 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 - p_c(1, 0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp47 - _tmp48;
  const Scalar _tmp62 = _tmp46 + _tmp61;
  const Scalar _tmp63 = _tmp60 * _tmp62;
  const Scalar _tmp64 = -_tmp50 * _tmp60 + _tmp63;
  const Scalar _tmp65 = _tmp40 * _tmp43;
  const Scalar _tmp66 = _tmp58 * _tmp59;
  const Scalar _tmp67 = Scalar(1.0) / (_tmp60 * _tmp65 - _tmp66);
  const Scalar _tmp68 = _tmp65 * _tmp67;
  const Scalar _tmp69 = _tmp50 * _tmp66 - _tmp63 * _tmp65;
  const Scalar _tmp70 = _tmp62 * _tmp65 + _tmp68 * _tmp69;
  const Scalar _tmp71 = Scalar(1.0) * _tmp35;
  const Scalar _tmp72 = -_tmp71;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp56 + _tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp37;
  const Scalar _tmp75 = _tmp73 * (-_tmp52 + _tmp74);
  const Scalar _tmp76 = -_tmp62 + _tmp64 * _tmp68 - _tmp70 * _tmp75;
  const Scalar _tmp77 = -_tmp46;
  const Scalar _tmp78 = _tmp49 + _tmp77;
  const Scalar _tmp79 = _tmp25 + _tmp51;
  const Scalar _tmp80 = _tmp79 + position_vector(0, 0);
  const Scalar _tmp81 = _tmp80 - p_d(0, 0);
  const Scalar _tmp82 = _tmp20 + _tmp7;
  const Scalar _tmp83 = _tmp82 + position_vector(1, 0);
  const Scalar _tmp84 = _tmp83 - p_d(1, 0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp81 * _tmp85;
  const Scalar _tmp87 = _tmp62 * _tmp86;
  const Scalar _tmp88 = _tmp84 * _tmp85;
  const Scalar _tmp89 = _tmp65 * _tmp86 - _tmp88;
  const Scalar _tmp90 = _tmp67 * _tmp89;
  const Scalar _tmp91 = -_tmp65 * _tmp87 - _tmp69 * _tmp90 + _tmp78 * _tmp88;
  const Scalar _tmp92 = -_tmp64 * _tmp90 - _tmp75 * _tmp91 - _tmp78 * _tmp86 + _tmp87;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 = _tmp45 * _tmp60 - _tmp52 * _tmp66 + _tmp56 * _tmp60;
  const Scalar _tmp95 = _tmp45 * _tmp86 - _tmp79 * _tmp88 + _tmp82 * _tmp86 - _tmp90 * _tmp94;
  const Scalar _tmp96 = _tmp93 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp92 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp45 + _tmp68 * _tmp94 - _tmp76 * _tmp96);
  const Scalar _tmp100 = _tmp76 + _tmp99;
  const Scalar _tmp101 = _tmp89 * _tmp93;
  const Scalar _tmp102 = -_tmp100 * _tmp101 - _tmp65;
  const Scalar _tmp103 = _tmp60 * _tmp67;
  const Scalar _tmp104 = _tmp86 * _tmp93;
  const Scalar _tmp105 = _tmp33 - p_a(0, 0);
  const Scalar _tmp106 = _tmp22 - p_a(1, 0);
  const Scalar _tmp107 =
      std::pow(Scalar(std::pow(_tmp105, Scalar(2)) + std::pow(_tmp106, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp108 * fh1;
  const Scalar _tmp110 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp111 = _tmp71 * _tmp75 + _tmp74;
  const Scalar _tmp112 = 0;
  const Scalar _tmp113 = _tmp112 * _tmp93;
  const Scalar _tmp114 = _tmp113 * _tmp90;
  const Scalar _tmp115 = _tmp44 * (_tmp113 * _tmp86 - _tmp114 * _tmp60);
  const Scalar _tmp116 = Scalar(1.0) * _tmp67;
  const Scalar _tmp117 = _tmp116 * _tmp69;
  const Scalar _tmp118 = -_tmp116 * _tmp64 + _tmp117 * _tmp75;
  const Scalar _tmp119 = _tmp98 * (-_tmp116 * _tmp94 - _tmp118 * _tmp96);
  const Scalar _tmp120 = _tmp118 + _tmp119;
  const Scalar _tmp121 = -_tmp101 * _tmp120 + Scalar(1.0);
  const Scalar _tmp122 = _tmp106 * _tmp107;
  const Scalar _tmp123 = _tmp122 * fh1;
  const Scalar _tmp124 = Scalar(1.0) * _tmp97;
  const Scalar _tmp125 = _tmp116 * _tmp89 * _tmp97;
  const Scalar _tmp126 = fh1 * (-_tmp108 * _tmp21 + _tmp122 * _tmp32);
  const Scalar _tmp127 = -_tmp109 * _tmp44 * (_tmp100 * _tmp104 + _tmp102 * _tmp103 + Scalar(1.0)) -
                         _tmp110 * _tmp115 -
                         _tmp123 * _tmp44 * (_tmp103 * _tmp121 + _tmp104 * _tmp120) -
                         _tmp126 * _tmp44 * (_tmp124 * _tmp86 - _tmp125 * _tmp60);
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = fh1 * (_tmp61 + _tmp77);
  const Scalar _tmp130 = _tmp108 * _tmp129 + Scalar(40.024799999999999) * _tmp28 + _tmp32 * fv1;
  const Scalar _tmp131 = _tmp72 + _tmp82;
  const Scalar _tmp132 = _tmp131 * _tmp75;
  const Scalar _tmp133 = Scalar(1.0) / (-_tmp132 + _tmp74 - _tmp79);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp131 * _tmp134;
  const Scalar _tmp136 = -Scalar(1.0) * _tmp134 + Scalar(1.0) * _tmp135 * _tmp73;
  const Scalar _tmp137 = _tmp111 * _tmp133;
  const Scalar _tmp138 = _tmp91 * _tmp93;
  const Scalar _tmp139 = _tmp73 * (-_tmp112 * _tmp138 - _tmp131 * _tmp137 + _tmp72);
  const Scalar _tmp140 = -Scalar(1.0) * _tmp137 - Scalar(1.0) * _tmp139 + Scalar(1.0);
  const Scalar _tmp141 = -_tmp122 * _tmp129 - Scalar(40.024799999999999) * _tmp17 - _tmp21 * fv1;
  const Scalar _tmp142 = _tmp73 * (_tmp132 * _tmp134 + Scalar(1.0));
  const Scalar _tmp143 = _tmp134 * _tmp75;
  const Scalar _tmp144 = -Scalar(1.0) * _tmp142 + Scalar(1.0) * _tmp143;
  const Scalar _tmp145 = -_tmp124 * _tmp91 + _tmp135 * _tmp98;
  const Scalar _tmp146 = Scalar(1.0) * _tmp73;
  const Scalar _tmp147 = _tmp134 * _tmp98;
  const Scalar _tmp148 = _tmp131 * _tmp133;
  const Scalar _tmp149 = -_tmp117 + _tmp119 * _tmp148 - _tmp120 * _tmp138;
  const Scalar _tmp150 = -_tmp100 * _tmp138 + _tmp148 * _tmp99 + _tmp70;
  const Scalar _tmp151 = Scalar(1.0) * _tmp109 * (_tmp134 * _tmp99 - _tmp146 * _tmp150) +
                         _tmp110 * _tmp140 +
                         Scalar(1.0) * _tmp123 * (_tmp119 * _tmp134 - _tmp146 * _tmp149) +
                         Scalar(1.0) * _tmp126 * (-_tmp145 * _tmp146 + _tmp147) +
                         _tmp130 * _tmp136 + _tmp141 * _tmp144;
  const Scalar _tmp152 = std::asinh(_tmp128 * _tmp151);
  const Scalar _tmp153 = Scalar(1.4083112389913199) * _tmp127;
  const Scalar _tmp154 =
      -_tmp152 * _tmp153 - std::sqrt(Scalar(std::pow(Scalar(-_tmp36 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp38 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp155 = Scalar(0.71007031138673404) * _tmp128;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = Scalar(1.0) * _tmp152;
  const Scalar _tmp158 = Scalar(1.4083112389913199) * _tmp115;
  const Scalar _tmp159 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp160 = _tmp18 + _tmp55;
  const Scalar _tmp161 = _tmp115 * _tmp159;
  const Scalar _tmp162 =
      (_tmp128 * (_tmp136 * _tmp32 - _tmp140 + _tmp144 * _tmp160) - _tmp151 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp159 + 1));
  const Scalar _tmp163 = _tmp110 * _tmp113;
  const Scalar _tmp164 = _tmp102 * _tmp109 * _tmp67 + _tmp121 * _tmp123 * _tmp67 -
                         _tmp125 * _tmp126 - _tmp163 * _tmp90;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp130 * _tmp134;
  const Scalar _tmp167 = _tmp131 * _tmp73;
  const Scalar _tmp168 = _tmp109 * _tmp150 * _tmp73 + _tmp110 * _tmp139 +
                         _tmp123 * _tmp149 * _tmp73 + _tmp126 * _tmp145 * _tmp73 +
                         _tmp141 * _tmp142 - _tmp166 * _tmp167;
  const Scalar _tmp169 = std::asinh(_tmp165 * _tmp168);
  const Scalar _tmp170 = Scalar(1.4083112389913199) * _tmp164;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 - std::sqrt(Scalar(std::pow(Scalar(-_tmp53 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp57 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.71007031138673404) * _tmp165;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(1.0) * _tmp169;
  const Scalar _tmp175 = Scalar(1.4083112389913199) * _tmp113;
  const Scalar _tmp176 = _tmp175 * _tmp90;
  const Scalar _tmp177 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp178 = _tmp114 * _tmp177;
  const Scalar _tmp179 = _tmp134 * _tmp32;
  const Scalar _tmp180 =
      (_tmp165 * (-_tmp139 + _tmp142 * _tmp160 - _tmp167 * _tmp179) - _tmp168 * _tmp178) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp177 + 1));
  const Scalar _tmp181 =
      _tmp100 * _tmp109 * _tmp93 + _tmp120 * _tmp123 * _tmp93 + _tmp124 * _tmp126 + _tmp163;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = -_tmp109 * _tmp133 * _tmp99 + _tmp110 * _tmp137 -
                         _tmp119 * _tmp123 * _tmp133 - _tmp126 * _tmp147 - _tmp141 * _tmp143 +
                         _tmp166;
  const Scalar _tmp184 = std::asinh(_tmp182 * _tmp183);
  const Scalar _tmp185 = Scalar(1.4083112389913199) * _tmp181;
  const Scalar _tmp186 =
      -_tmp184 * _tmp185 - std::sqrt(Scalar(std::pow(Scalar(-_tmp80 + p_d(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp83 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp187 = Scalar(0.71007031138673404) * _tmp182;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp190 = _tmp113 * _tmp189;
  const Scalar _tmp191 = (_tmp182 * (-_tmp137 - _tmp143 * _tmp160 + _tmp179) + _tmp183 * _tmp190) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp192 = Scalar(1.0) * _tmp184;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp34 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.71007031138673404) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_a(1, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp33 + p_a(0, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-Scalar(1.0) * _tmp162 * std::cosh(_tmp157) -
                          (-Scalar(0.71007031138673404) * _tmp154 * _tmp161 +
                           _tmp155 * (-_tmp152 * _tmp158 - _tmp153 * _tmp162)) *
                              std::cosh(_tmp156)) +
               _tmp158 * (-std::sinh(_tmp156) - std::sinh(_tmp157));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp180 * std::cosh(_tmp174) -
                          (-Scalar(0.71007031138673404) * _tmp171 * _tmp178 +
                           _tmp172 * (-_tmp169 * _tmp176 - _tmp170 * _tmp180)) *
                              std::cosh(_tmp173)) +
               _tmp176 * (-std::sinh(_tmp173) - std::sinh(_tmp174));
  _res(3, 0) = -_tmp175 * (-std::sinh(_tmp188) - std::sinh(_tmp192)) +
               _tmp185 * (-Scalar(1.0) * _tmp191 * std::cosh(_tmp192) -
                          (Scalar(0.71007031138673404) * _tmp186 * _tmp190 +
                           _tmp187 * (_tmp175 * _tmp184 - _tmp185 * _tmp191)) *
                              std::cosh(_tmp188));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
