// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost4_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost4Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 474

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (152)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = 2 * _tmp1 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp5 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = 2 * _tmp4;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = _tmp2 * _tmp5;
  const Scalar _tmp11 = -_tmp10 + _tmp9;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp14 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 +
                        Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999);
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp7;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp2 * _tmp8;
  const Scalar _tmp24 = _tmp1 * _tmp5;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = _tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp31 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp32 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp33 = -_tmp22 + _tmp26;
  const Scalar _tmp34 = _tmp19 + _tmp33;
  const Scalar _tmp35 = Scalar(1.0) * _tmp34;
  const Scalar _tmp36 = -_tmp7;
  const Scalar _tmp37 = _tmp12 + _tmp15;
  const Scalar _tmp38 = _tmp36 + _tmp37;
  const Scalar _tmp39 = Scalar(1.0) * _tmp38;
  const Scalar _tmp40 = -_tmp39;
  const Scalar _tmp41 = _tmp16 + _tmp36;
  const Scalar _tmp42 = Scalar(1.0) / (_tmp40 + _tmp41);
  const Scalar _tmp43 = _tmp20 + _tmp33;
  const Scalar _tmp44 = _tmp35 - _tmp43;
  const Scalar _tmp45 = _tmp42 * _tmp44;
  const Scalar _tmp46 = _tmp35 + _tmp39 * _tmp45;
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp48 =
      -Scalar(0.010999999999999999) * _tmp13 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp49 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp50 = _tmp48 + _tmp49;
  const Scalar _tmp51 = _tmp47 + _tmp50;
  const Scalar _tmp52 = _tmp19 + _tmp27;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 - p_c(0, 0);
  const Scalar _tmp55 = _tmp37 + _tmp7;
  const Scalar _tmp56 = _tmp55 + position_vector(1, 0);
  const Scalar _tmp57 = _tmp56 - p_c(1, 0);
  const Scalar _tmp58 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp57, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp59 = _tmp54 * _tmp58;
  const Scalar _tmp60 = _tmp34 + position_vector(0, 0);
  const Scalar _tmp61 = _tmp60 - p_d(0, 0);
  const Scalar _tmp62 = Scalar(1.0) / (_tmp61);
  const Scalar _tmp63 = _tmp38 + position_vector(1, 0);
  const Scalar _tmp64 = _tmp63 - p_d(1, 0);
  const Scalar _tmp65 = _tmp62 * _tmp64;
  const Scalar _tmp66 = _tmp57 * _tmp58;
  const Scalar _tmp67 = _tmp59 * _tmp65 - _tmp66;
  const Scalar _tmp68 = -_tmp47;
  const Scalar _tmp69 = _tmp48 - _tmp49;
  const Scalar _tmp70 = _tmp68 + _tmp69;
  const Scalar _tmp71 = _tmp43 + position_vector(0, 0);
  const Scalar _tmp72 = _tmp71 - p_a(0, 0);
  const Scalar _tmp73 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp74 = _tmp73 - p_a(1, 0);
  const Scalar _tmp75 = std::pow(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp74, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp76 = _tmp72 * _tmp75;
  const Scalar _tmp77 = _tmp47 + _tmp69;
  const Scalar _tmp78 = _tmp76 * _tmp77;
  const Scalar _tmp79 = _tmp74 * _tmp75;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp65 * _tmp76 - _tmp79);
  const Scalar _tmp81 = _tmp80 * (-_tmp70 * _tmp76 + _tmp78);
  const Scalar _tmp82 = _tmp59 * _tmp77;
  const Scalar _tmp83 = _tmp80 * (-_tmp65 * _tmp78 + _tmp70 * _tmp79);
  const Scalar _tmp84 = _tmp51 * _tmp66 - _tmp65 * _tmp82 - _tmp67 * _tmp83;
  const Scalar _tmp85 = -_tmp45 * _tmp84 - _tmp51 * _tmp59 - _tmp67 * _tmp81 + _tmp82;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 = 0;
  const Scalar _tmp88 = _tmp76 * _tmp80;
  const Scalar _tmp89 = _tmp67 * _tmp88;
  const Scalar _tmp90 =
      std::sqrt(Scalar(std::pow(_tmp61, Scalar(2)) + std::pow(_tmp64, Scalar(2))));
  const Scalar _tmp91 = _tmp62 * _tmp90;
  const Scalar _tmp92 = Scalar(1.0) * _tmp42;
  const Scalar _tmp93 = _tmp44 * _tmp83 * _tmp92 - Scalar(1.0) * _tmp81;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp95 = _tmp91 * (_tmp34 * _tmp64 * _tmp94 - _tmp38 * _tmp61 * _tmp94);
  const Scalar _tmp96 = _tmp80 * (_tmp41 * _tmp76 - _tmp43 * _tmp79 + _tmp76 * _tmp95);
  const Scalar _tmp97 = -_tmp52 * _tmp66 + _tmp55 * _tmp59 + _tmp59 * _tmp95 - _tmp67 * _tmp96;
  const Scalar _tmp98 = _tmp86 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp100 = _tmp85 * _tmp99;
  const Scalar _tmp101 = _tmp100 * (-_tmp93 * _tmp98 - Scalar(1.0) * _tmp96);
  const Scalar _tmp102 = _tmp86 * (_tmp101 + _tmp93);
  const Scalar _tmp103 = -_tmp102 * _tmp67 + Scalar(1.0);
  const Scalar _tmp104 = _tmp29 - p_b(0, 0);
  const Scalar _tmp105 = _tmp18 - p_b(1, 0);
  const Scalar _tmp106 =
      std::pow(Scalar(std::pow(_tmp104, Scalar(2)) + std::pow(_tmp105, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp107 = _tmp105 * _tmp106;
  const Scalar _tmp108 = _tmp107 * fh1;
  const Scalar _tmp109 = _tmp65 * _tmp77 + _tmp65 * _tmp83;
  const Scalar _tmp110 = -_tmp109 * _tmp45 + _tmp65 * _tmp81 - _tmp77;
  const Scalar _tmp111 = _tmp100 * (-_tmp110 * _tmp98 + _tmp65 * _tmp96 - _tmp95);
  const Scalar _tmp112 = _tmp86 * (_tmp110 + _tmp111);
  const Scalar _tmp113 = -_tmp112 * _tmp67 - _tmp65;
  const Scalar _tmp114 = _tmp104 * _tmp106;
  const Scalar _tmp115 = _tmp114 * fh1;
  const Scalar _tmp116 = Scalar(1.0) * _tmp99;
  const Scalar _tmp117 = fh1 * (_tmp107 * _tmp28 - _tmp114 * _tmp17);
  const Scalar _tmp118 = -_tmp108 * _tmp91 * (_tmp102 * _tmp59 + _tmp103 * _tmp88) -
                         _tmp115 * _tmp91 * (_tmp112 * _tmp59 + _tmp113 * _tmp88 + Scalar(1.0)) -
                         _tmp117 * _tmp91 * (_tmp116 * _tmp59 - _tmp116 * _tmp89) -
                         _tmp32 * _tmp91 * (_tmp59 * _tmp87 - _tmp87 * _tmp89);
  const Scalar _tmp119 = Scalar(1.0) / (_tmp118);
  const Scalar _tmp120 = _tmp40 + _tmp55;
  const Scalar _tmp121 = _tmp120 * _tmp45;
  const Scalar _tmp122 = Scalar(1.0) / (-_tmp121 + _tmp35 - _tmp52);
  const Scalar _tmp123 = Scalar(1.0) * _tmp122;
  const Scalar _tmp124 = _tmp120 * _tmp123;
  const Scalar _tmp125 = fh1 * (_tmp50 + _tmp68);
  const Scalar _tmp126 = _tmp114 * _tmp125 + Scalar(40.024799999999999) * _tmp25 + _tmp28 * fv1;
  const Scalar _tmp127 = -_tmp107 * _tmp125 - Scalar(40.024799999999999) * _tmp11 - _tmp17 * fv1;
  const Scalar _tmp128 = _tmp123 * _tmp45;
  const Scalar _tmp129 = _tmp121 * _tmp123 + Scalar(1.0);
  const Scalar _tmp130 = _tmp100 * _tmp123;
  const Scalar _tmp131 = _tmp100 * _tmp124 - _tmp116 * _tmp84;
  const Scalar _tmp132 = _tmp122 * _tmp46;
  const Scalar _tmp133 = -_tmp120 * _tmp132 + _tmp40 - _tmp84 * _tmp87;
  const Scalar _tmp134 = _tmp120 * _tmp122;
  const Scalar _tmp135 = _tmp109 + _tmp111 * _tmp134 - _tmp112 * _tmp84;
  const Scalar _tmp136 = Scalar(1.0) * fh1;
  const Scalar _tmp137 = _tmp101 * _tmp134 - _tmp102 * _tmp84 - Scalar(1.0) * _tmp83;
  const Scalar _tmp138 = std::asinh(
      _tmp119 * (_tmp107 * _tmp136 * (_tmp101 * _tmp123 - _tmp137 * _tmp92) +
                 _tmp114 * _tmp136 * (_tmp111 * _tmp123 - _tmp135 * _tmp92) +
                 Scalar(1.0) * _tmp117 * (_tmp130 - _tmp131 * _tmp92) +
                 Scalar(1.0) * _tmp126 * (-_tmp123 + _tmp124 * _tmp42) +
                 Scalar(1.0) * _tmp127 * (_tmp128 - _tmp129 * _tmp92) +
                 Scalar(1.0) * _tmp32 * (-_tmp123 * _tmp46 - _tmp133 * _tmp92 + Scalar(1.0))));
  const Scalar _tmp139 = Scalar(1.4083112389913199) * _tmp118;
  const Scalar _tmp140 = _tmp116 * _tmp117;
  const Scalar _tmp141 = _tmp67 * _tmp80;
  const Scalar _tmp142 = _tmp32 * _tmp87;
  const Scalar _tmp143 = _tmp103 * _tmp108 * _tmp80 + _tmp113 * _tmp115 * _tmp80 -
                         _tmp140 * _tmp141 - _tmp141 * _tmp142;
  const Scalar _tmp144 = Scalar(1.0) / (_tmp143);
  const Scalar _tmp145 = _tmp123 * _tmp126;
  const Scalar _tmp146 =
      std::asinh(_tmp144 * (_tmp108 * _tmp137 * _tmp42 + _tmp115 * _tmp135 * _tmp42 +
                            _tmp117 * _tmp131 * _tmp42 - _tmp120 * _tmp145 * _tmp42 +
                            _tmp127 * _tmp129 * _tmp42 + _tmp133 * _tmp32 * _tmp42));
  const Scalar _tmp147 = Scalar(1.4083112389913199) * _tmp143;
  const Scalar _tmp148 = _tmp102 * _tmp108 + _tmp112 * _tmp115 + _tmp140 + _tmp142;
  const Scalar _tmp149 = Scalar(1.0) / (_tmp148);
  const Scalar _tmp150 =
      std::asinh(_tmp149 * (-_tmp101 * _tmp108 * _tmp122 - _tmp111 * _tmp115 * _tmp122 -
                            _tmp117 * _tmp130 - _tmp127 * _tmp128 + _tmp132 * _tmp32 + _tmp145));
  const Scalar _tmp151 = Scalar(1.4083112389913199) * _tmp148;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp31 * (-std::sinh(Scalar(1.0) * _tmp30) -
                std::sinh(Scalar(0.71007031138673404) * _tmp0 *
                          (-_tmp30 * _tmp31 -
                           std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp29 + p_b(0, 0)), Scalar(2))))))) -
      encoder(0, 0);
  _res(1, 0) =
      _tmp139 * (-std::sinh(Scalar(1.0) * _tmp138) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp119 *
                           (-_tmp138 * _tmp139 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp60 + p_d(0, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp63 + p_d(1, 0)), Scalar(2))))))) -
      encoder(1, 0);
  _res(2, 0) =
      _tmp147 * (-std::sinh(Scalar(1.0) * _tmp146) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp144 *
                           (-_tmp146 * _tmp147 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp71 + p_a(0, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp73 + p_a(1, 0)), Scalar(2))))))) -
      encoder(2, 0);
  _res(3, 0) =
      _tmp151 * (-std::sinh(Scalar(1.0) * _tmp150) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp149 *
                           (-_tmp150 * _tmp151 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp53 + p_c(0, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp56 + p_c(1, 0)), Scalar(2))))))) -
      encoder(3, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
