// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost4_wrt_fv1_Nl13
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost4WrtFv1Nl13(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 579

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp8 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp9 = 2 * _tmp5;
  const Scalar _tmp10 = _tmp8 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp12 = _tmp11 * _tmp3;
  const Scalar _tmp13 = _tmp10 - _tmp12;
  const Scalar _tmp14 = Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -_tmp14;
  const Scalar _tmp16 = 2 * _tmp3 * _tmp8;
  const Scalar _tmp17 = _tmp11 * _tmp5;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp15 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp7;
  const Scalar _tmp21 = _tmp20 + position_vector(1, 0);
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp23 = _tmp3 * _tmp9;
  const Scalar _tmp24 = _tmp11 * _tmp8;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = 1 - 2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp29 = _tmp26 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp33 = -_tmp7;
  const Scalar _tmp34 = -_tmp18;
  const Scalar _tmp35 = _tmp33 + _tmp34;
  const Scalar _tmp36 = _tmp15 + _tmp35;
  const Scalar _tmp37 = Scalar(1.0) * _tmp36;
  const Scalar _tmp38 = -_tmp37;
  const Scalar _tmp39 = _tmp15 + _tmp34 + _tmp7;
  const Scalar _tmp40 = Scalar(1.0) / (_tmp38 + _tmp39);
  const Scalar _tmp41 = _tmp26 - _tmp28;
  const Scalar _tmp42 = _tmp22 + _tmp41;
  const Scalar _tmp43 = -_tmp22;
  const Scalar _tmp44 = _tmp41 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = _tmp40 * (-_tmp42 + _tmp45);
  const Scalar _tmp47 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp48 =
      -Scalar(0.010999999999999999) * _tmp27 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp49 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp50 = _tmp48 - _tmp49;
  const Scalar _tmp51 = _tmp47 + _tmp50;
  const Scalar _tmp52 = _tmp42 + position_vector(0, 0);
  const Scalar _tmp53 = _tmp52 - p_d(0, 0);
  const Scalar _tmp54 = _tmp39 + position_vector(1, 0);
  const Scalar _tmp55 = _tmp54 - p_d(1, 0);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp55 * _tmp56;
  const Scalar _tmp58 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp59 = _tmp58 - p_a(0, 0);
  const Scalar _tmp60 = Scalar(1.0) / (_tmp59);
  const Scalar _tmp61 = _tmp36 + position_vector(1, 0);
  const Scalar _tmp62 = _tmp61 - p_a(1, 0);
  const Scalar _tmp63 = _tmp60 * _tmp62;
  const Scalar _tmp64 = -_tmp47;
  const Scalar _tmp65 = _tmp50 + _tmp64;
  const Scalar _tmp66 = _tmp53 * _tmp56;
  const Scalar _tmp67 = _tmp65 * _tmp66;
  const Scalar _tmp68 = _tmp51 * _tmp57 - _tmp63 * _tmp67;
  const Scalar _tmp69 = Scalar(1.0) / (-_tmp57 + _tmp63 * _tmp66);
  const Scalar _tmp70 = Scalar(1.0) * _tmp69;
  const Scalar _tmp71 = _tmp68 * _tmp70;
  const Scalar _tmp72 = -_tmp51 * _tmp66 + _tmp67;
  const Scalar _tmp73 = _tmp46 * _tmp71 - _tmp70 * _tmp72;
  const Scalar _tmp74 = _tmp48 + _tmp49;
  const Scalar _tmp75 = _tmp64 + _tmp74;
  const Scalar _tmp76 = _tmp29 + _tmp43;
  const Scalar _tmp77 = _tmp76 + position_vector(0, 0);
  const Scalar _tmp78 = _tmp77 - p_b(0, 0);
  const Scalar _tmp79 = _tmp19 + _tmp33;
  const Scalar _tmp80 = _tmp79 + position_vector(1, 0);
  const Scalar _tmp81 = _tmp80 - p_b(1, 0);
  const Scalar _tmp82 = std::pow(Scalar(std::pow(_tmp78, Scalar(2)) + std::pow(_tmp81, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp83 = _tmp81 * _tmp82;
  const Scalar _tmp84 = _tmp78 * _tmp82;
  const Scalar _tmp85 = _tmp63 * _tmp84 - _tmp83;
  const Scalar _tmp86 = _tmp69 * _tmp85;
  const Scalar _tmp87 = _tmp65 * _tmp84;
  const Scalar _tmp88 = -_tmp63 * _tmp87 - _tmp68 * _tmp86 + _tmp75 * _tmp83;
  const Scalar _tmp89 = -_tmp46 * _tmp88 - _tmp72 * _tmp86 - _tmp75 * _tmp84 + _tmp87;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 =
      std::sqrt(Scalar(std::pow(_tmp59, Scalar(2)) + std::pow(_tmp62, Scalar(2))));
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp60 * _tmp91;
  const Scalar _tmp94 = _tmp93 * (-_tmp36 * _tmp59 * _tmp92 + _tmp44 * _tmp62 * _tmp92);
  const Scalar _tmp95 = _tmp39 * _tmp66 - _tmp42 * _tmp57 + _tmp66 * _tmp94;
  const Scalar _tmp96 = -_tmp76 * _tmp83 + _tmp79 * _tmp84 + _tmp84 * _tmp94 - _tmp86 * _tmp95;
  const Scalar _tmp97 = _tmp90 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp89 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp70 * _tmp95 - _tmp73 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp73;
  const Scalar _tmp102 = _tmp84 * _tmp90;
  const Scalar _tmp103 = _tmp85 * _tmp90;
  const Scalar _tmp104 = -_tmp101 * _tmp103 + Scalar(1.0);
  const Scalar _tmp105 = _tmp66 * _tmp69;
  const Scalar _tmp106 = _tmp21 - p_c(1, 0);
  const Scalar _tmp107 = _tmp31 - p_c(0, 0);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = _tmp63 * _tmp69;
  const Scalar _tmp112 = _tmp111 * _tmp68 + _tmp63 * _tmp65;
  const Scalar _tmp113 = _tmp111 * _tmp72 - _tmp112 * _tmp46 - _tmp65;
  const Scalar _tmp114 = _tmp99 * (_tmp111 * _tmp95 - _tmp113 * _tmp97 - _tmp94);
  const Scalar _tmp115 = _tmp113 + _tmp114;
  const Scalar _tmp116 = -_tmp103 * _tmp115 - _tmp63;
  const Scalar _tmp117 = _tmp107 * _tmp108;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp120 = _tmp37 * _tmp46 + _tmp45;
  const Scalar _tmp121 = 0;
  const Scalar _tmp122 = _tmp121 * _tmp90;
  const Scalar _tmp123 = _tmp122 * _tmp86;
  const Scalar _tmp124 = _tmp93 * (_tmp122 * _tmp84 - _tmp123 * _tmp66);
  const Scalar _tmp125 = _tmp70 * _tmp85 * _tmp98;
  const Scalar _tmp126 = Scalar(1.0) * _tmp98;
  const Scalar _tmp127 = fh1 * (_tmp109 * _tmp30 - _tmp117 * _tmp20);
  const Scalar _tmp128 = -_tmp110 * _tmp93 * (_tmp101 * _tmp102 + _tmp104 * _tmp105) -
                         _tmp118 * _tmp93 * (_tmp102 * _tmp115 + _tmp105 * _tmp116 + Scalar(1.0)) -
                         _tmp119 * _tmp124 -
                         _tmp127 * _tmp93 * (-_tmp125 * _tmp66 + _tmp126 * _tmp84);
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = _tmp38 + _tmp79;
  const Scalar _tmp131 = _tmp130 * _tmp46;
  const Scalar _tmp132 = Scalar(1.0) / (-_tmp131 + _tmp45 - _tmp76);
  const Scalar _tmp133 = Scalar(1.0) * _tmp132;
  const Scalar _tmp134 = _tmp133 * _tmp99;
  const Scalar _tmp135 = -_tmp126 * _tmp88 + _tmp130 * _tmp134;
  const Scalar _tmp136 = Scalar(1.0) * _tmp40;
  const Scalar _tmp137 = _tmp130 * _tmp132;
  const Scalar _tmp138 = _tmp88 * _tmp90;
  const Scalar _tmp139 = _tmp100 * _tmp137 - _tmp101 * _tmp138 - _tmp71;
  const Scalar _tmp140 = fh1 * (_tmp47 + _tmp74);
  const Scalar _tmp141 = -_tmp109 * _tmp140 - Scalar(40.024799999999999) * _tmp13 - _tmp20 * fv1;
  const Scalar _tmp142 = _tmp133 * _tmp46;
  const Scalar _tmp143 = _tmp40 * (_tmp131 * _tmp133 + Scalar(1.0));
  const Scalar _tmp144 = Scalar(1.0) * _tmp142 - Scalar(1.0) * _tmp143;
  const Scalar _tmp145 = _tmp117 * _tmp140 + Scalar(40.024799999999999) * _tmp25 + _tmp30 * fv1;
  const Scalar _tmp146 = _tmp130 * _tmp40;
  const Scalar _tmp147 = Scalar(1.0) * _tmp133 * _tmp146 - Scalar(1.0) * _tmp133;
  const Scalar _tmp148 = _tmp112 + _tmp114 * _tmp137 - _tmp115 * _tmp138;
  const Scalar _tmp149 = _tmp120 * _tmp132;
  const Scalar _tmp150 = _tmp40 * (-_tmp121 * _tmp138 - _tmp130 * _tmp149 + _tmp38);
  const Scalar _tmp151 = -Scalar(1.0) * _tmp149 - Scalar(1.0) * _tmp150 + Scalar(1.0);
  const Scalar _tmp152 = Scalar(1.0) * _tmp110 * (_tmp100 * _tmp133 - _tmp136 * _tmp139) +
                         Scalar(1.0) * _tmp118 * (_tmp114 * _tmp133 - _tmp136 * _tmp148) +
                         _tmp119 * _tmp151 + Scalar(1.0) * _tmp127 * (_tmp134 - _tmp135 * _tmp136) +
                         _tmp141 * _tmp144 + _tmp145 * _tmp147;
  const Scalar _tmp153 = std::asinh(_tmp129 * _tmp152);
  const Scalar _tmp154 = Scalar(1.4083112389913199) * _tmp128;
  const Scalar _tmp155 =
      -_tmp153 * _tmp154 - std::sqrt(Scalar(std::pow(Scalar(-_tmp58 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp61 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp156 = Scalar(0.71007031138673404) * _tmp129;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = Scalar(1.0) * _tmp153;
  const Scalar _tmp159 = Scalar(1.4083112389913199) * _tmp124;
  const Scalar _tmp160 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp161 = _tmp124 * _tmp160;
  const Scalar _tmp162 = _tmp14 + _tmp35;
  const Scalar _tmp163 =
      (_tmp129 * (_tmp144 * _tmp162 + _tmp147 * _tmp30 - _tmp151) - _tmp152 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp152, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp164 = _tmp69 * fh1;
  const Scalar _tmp165 = _tmp119 * _tmp122;
  const Scalar _tmp166 = _tmp104 * _tmp109 * _tmp164 + _tmp116 * _tmp117 * _tmp164 -
                         _tmp125 * _tmp127 - _tmp165 * _tmp86;
  const Scalar _tmp167 = Scalar(1.0) / (_tmp166);
  const Scalar _tmp168 = _tmp133 * _tmp145;
  const Scalar _tmp169 = _tmp110 * _tmp139 * _tmp40 + _tmp118 * _tmp148 * _tmp40 +
                         _tmp119 * _tmp150 + _tmp127 * _tmp135 * _tmp40 + _tmp141 * _tmp143 -
                         _tmp146 * _tmp168;
  const Scalar _tmp170 = std::asinh(_tmp167 * _tmp169);
  const Scalar _tmp171 = Scalar(1.0) * _tmp170;
  const Scalar _tmp172 = Scalar(1.4083112389913199) * _tmp166;
  const Scalar _tmp173 =
      -_tmp170 * _tmp172 - std::sqrt(Scalar(std::pow(Scalar(-_tmp52 + p_d(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp54 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp174 = Scalar(0.71007031138673404) * _tmp167;
  const Scalar _tmp175 = _tmp173 * _tmp174;
  const Scalar _tmp176 = Scalar(1.4083112389913199) * _tmp122;
  const Scalar _tmp177 = _tmp176 * _tmp86;
  const Scalar _tmp178 = std::pow(_tmp166, Scalar(-2));
  const Scalar _tmp179 = _tmp133 * _tmp30;
  const Scalar _tmp180 = _tmp123 * _tmp178;
  const Scalar _tmp181 =
      (_tmp167 * (_tmp143 * _tmp162 - _tmp146 * _tmp179 - _tmp150) - _tmp169 * _tmp180) /
      std::sqrt(Scalar(std::pow(_tmp169, Scalar(2)) * _tmp178 + 1));
  const Scalar _tmp182 = -_tmp100 * _tmp110 * _tmp132 - _tmp114 * _tmp118 * _tmp132 +
                         _tmp119 * _tmp149 - _tmp127 * _tmp134 - _tmp141 * _tmp142 + _tmp168;
  const Scalar _tmp183 =
      _tmp101 * _tmp110 * _tmp90 + _tmp115 * _tmp118 * _tmp90 + _tmp126 * _tmp127 + _tmp165;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp182 * _tmp184);
  const Scalar _tmp186 = Scalar(1.4083112389913199) * _tmp183;
  const Scalar _tmp187 =
      -_tmp185 * _tmp186 - std::sqrt(Scalar(std::pow(Scalar(-_tmp77 + p_b(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp80 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.71007031138673404) * _tmp184;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(1.0) * _tmp185;
  const Scalar _tmp191 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp192 = _tmp122 * _tmp191;
  const Scalar _tmp193 = (_tmp182 * _tmp192 + _tmp184 * (-_tmp142 * _tmp162 - _tmp149 + _tmp179)) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp191 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.71007031138673404) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp21 + p_c(1, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp31 + p_c(0, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp154 * (-Scalar(1.0) * _tmp163 * std::cosh(_tmp158) -
                          (-Scalar(0.71007031138673404) * _tmp155 * _tmp161 +
                           _tmp156 * (-_tmp153 * _tmp159 - _tmp154 * _tmp163)) *
                              std::cosh(_tmp157)) +
               _tmp159 * (-std::sinh(_tmp157) - std::sinh(_tmp158));
  _res(2, 0) = _tmp172 * (-Scalar(1.0) * _tmp181 * std::cosh(_tmp171) -
                          (-Scalar(0.71007031138673404) * _tmp173 * _tmp180 +
                           _tmp174 * (-_tmp170 * _tmp177 - _tmp172 * _tmp181)) *
                              std::cosh(_tmp175)) +
               _tmp177 * (-std::sinh(_tmp171) - std::sinh(_tmp175));
  _res(3, 0) = -_tmp176 * (-std::sinh(_tmp189) - std::sinh(_tmp190)) +
               _tmp186 * (-Scalar(1.0) * _tmp193 * std::cosh(_tmp190) -
                          (Scalar(0.71007031138673404) * _tmp187 * _tmp192 +
                           _tmp188 * (_tmp176 * _tmp185 - _tmp186 * _tmp193)) *
                              std::cosh(_tmp189));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
