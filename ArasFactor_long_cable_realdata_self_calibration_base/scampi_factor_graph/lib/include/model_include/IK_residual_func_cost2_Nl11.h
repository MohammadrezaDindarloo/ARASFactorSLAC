// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl11
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl11(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 499

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (163)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = -_tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp1 * _tmp8;
  const Scalar _tmp15 = _tmp10 * _tmp3;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp23 = _tmp10 * _tmp7;
  const Scalar _tmp24 = _tmp22 + _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = std::pow(Scalar(-_tmp19 + p_b(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp29 + p_b(0, 0)), Scalar(2));
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp34 = -_tmp33;
  const Scalar _tmp35 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp36 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp37 = _tmp35 + _tmp36;
  const Scalar _tmp38 = _tmp34 + _tmp37;
  const Scalar _tmp39 = -_tmp21;
  const Scalar _tmp40 = _tmp25 + _tmp26;
  const Scalar _tmp41 = _tmp39 + _tmp40;
  const Scalar _tmp42 = Scalar(1.0) * _tmp41;
  const Scalar _tmp43 = _tmp13 - _tmp16;
  const Scalar _tmp44 = _tmp43 + _tmp5;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp43 + _tmp6;
  const Scalar _tmp48 = _tmp46 + _tmp47;
  const Scalar _tmp49 = _tmp17 + _tmp5;
  const Scalar _tmp50 = Scalar(1.0) / (_tmp46 + _tmp49);
  const Scalar _tmp51 = _tmp21 + _tmp40;
  const Scalar _tmp52 = _tmp50 * (_tmp42 - _tmp51);
  const Scalar _tmp53 = _tmp48 * _tmp52;
  const Scalar _tmp54 = _tmp27 + _tmp39;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp42 - _tmp53 - _tmp54);
  const Scalar _tmp56 = Scalar(1.0) * _tmp55;
  const Scalar _tmp57 = _tmp48 * _tmp50;
  const Scalar _tmp58 = _tmp29 - p_b(0, 0);
  const Scalar _tmp59 = _tmp19 - p_b(1, 0);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp58 * _tmp60;
  const Scalar _tmp62 = _tmp38 * fh1;
  const Scalar _tmp63 = Scalar(40.024799999999999) * _tmp24 + _tmp28 * fv1 + _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp33 + _tmp37;
  const Scalar _tmp65 = _tmp51 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 - p_c(0, 0);
  const Scalar _tmp67 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp68 = _tmp67 - p_c(1, 0);
  const Scalar _tmp69 = std::pow(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp68, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp70 = _tmp66 * _tmp69;
  const Scalar _tmp71 = _tmp35 - _tmp36;
  const Scalar _tmp72 = _tmp33 + _tmp71;
  const Scalar _tmp73 = _tmp70 * _tmp72;
  const Scalar _tmp74 = -_tmp64 * _tmp70 + _tmp73;
  const Scalar _tmp75 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp76 = _tmp75 - p_d(0, 0);
  const Scalar _tmp77 = Scalar(1.0) / (_tmp76);
  const Scalar _tmp78 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp79 = _tmp78 - p_d(1, 0);
  const Scalar _tmp80 = _tmp77 * _tmp79;
  const Scalar _tmp81 = _tmp68 * _tmp69;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp70 * _tmp80 - _tmp81);
  const Scalar _tmp83 = _tmp80 * _tmp82;
  const Scalar _tmp84 = _tmp82 * (_tmp64 * _tmp81 - _tmp73 * _tmp80);
  const Scalar _tmp85 = _tmp72 * _tmp80 + _tmp80 * _tmp84;
  const Scalar _tmp86 = -_tmp52 * _tmp85 - _tmp72 + _tmp74 * _tmp83;
  const Scalar _tmp87 = _tmp34 + _tmp71;
  const Scalar _tmp88 = _tmp54 + position_vector(0, 0);
  const Scalar _tmp89 = _tmp88 - p_a(0, 0);
  const Scalar _tmp90 = _tmp47 + position_vector(1, 0);
  const Scalar _tmp91 = _tmp90 - p_a(1, 0);
  const Scalar _tmp92 = std::pow(Scalar(std::pow(_tmp89, Scalar(2)) + std::pow(_tmp91, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp93 = _tmp89 * _tmp92;
  const Scalar _tmp94 = _tmp91 * _tmp92;
  const Scalar _tmp95 = _tmp80 * _tmp93 - _tmp94;
  const Scalar _tmp96 = _tmp72 * _tmp93;
  const Scalar _tmp97 = -_tmp80 * _tmp96 - _tmp84 * _tmp95 + _tmp87 * _tmp94;
  const Scalar _tmp98 = _tmp82 * _tmp95;
  const Scalar _tmp99 = -_tmp52 * _tmp97 - _tmp74 * _tmp98 - _tmp87 * _tmp93 + _tmp96;
  const Scalar _tmp100 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp101 =
      std::sqrt(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp79, Scalar(2))));
  const Scalar _tmp102 = Scalar(1.0) / (_tmp101);
  const Scalar _tmp103 = _tmp101 * _tmp77;
  const Scalar _tmp104 = _tmp103 * (_tmp102 * _tmp41 * _tmp79 - _tmp102 * _tmp44 * _tmp76);
  const Scalar _tmp105 = _tmp104 * _tmp70 + _tmp49 * _tmp70 - _tmp51 * _tmp81;
  const Scalar _tmp106 = _tmp104 * _tmp93 - _tmp105 * _tmp98 + _tmp47 * _tmp93 - _tmp54 * _tmp94;
  const Scalar _tmp107 = _tmp100 * _tmp106;
  const Scalar _tmp108 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp109 = _tmp108 * _tmp99;
  const Scalar _tmp110 = _tmp109 * (-_tmp104 + _tmp105 * _tmp83 - _tmp107 * _tmp86);
  const Scalar _tmp111 = _tmp48 * _tmp55;
  const Scalar _tmp112 = _tmp110 + _tmp86;
  const Scalar _tmp113 = _tmp100 * _tmp97;
  const Scalar _tmp114 = _tmp110 * _tmp111 - _tmp112 * _tmp113 + _tmp85;
  const Scalar _tmp115 = Scalar(1.0) * _tmp50;
  const Scalar _tmp116 = Scalar(1.0) * fh1;
  const Scalar _tmp117 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp118 = _tmp42 + _tmp45 * _tmp52;
  const Scalar _tmp119 = _tmp118 * _tmp55;
  const Scalar _tmp120 = 0;
  const Scalar _tmp121 = -_tmp113 * _tmp120 - _tmp119 * _tmp48 + _tmp46;
  const Scalar _tmp122 = Scalar(1.0) * _tmp84;
  const Scalar _tmp123 = Scalar(1.0) * _tmp82;
  const Scalar _tmp124 = _tmp122 * _tmp52 - _tmp123 * _tmp74;
  const Scalar _tmp125 = _tmp109 * (-_tmp105 * _tmp123 - _tmp107 * _tmp124);
  const Scalar _tmp126 = _tmp124 + _tmp125;
  const Scalar _tmp127 = _tmp111 * _tmp125 - _tmp113 * _tmp126 - _tmp122;
  const Scalar _tmp128 = _tmp59 * _tmp60;
  const Scalar _tmp129 = -Scalar(40.024799999999999) * _tmp12 - _tmp128 * _tmp62 - _tmp18 * fv1;
  const Scalar _tmp130 = _tmp52 * _tmp56;
  const Scalar _tmp131 = _tmp53 * _tmp56 + Scalar(1.0);
  const Scalar _tmp132 = _tmp109 * _tmp56;
  const Scalar _tmp133 = Scalar(1.0) * _tmp108;
  const Scalar _tmp134 = _tmp132 * _tmp48 - _tmp133 * _tmp97;
  const Scalar _tmp135 = fh1 * (_tmp128 * _tmp28 - _tmp18 * _tmp61);
  const Scalar _tmp136 = _tmp100 * _tmp95;
  const Scalar _tmp137 = -_tmp126 * _tmp136 + Scalar(1.0);
  const Scalar _tmp138 = _tmp70 * _tmp82;
  const Scalar _tmp139 = _tmp100 * _tmp93;
  const Scalar _tmp140 = _tmp128 * fh1;
  const Scalar _tmp141 = -_tmp112 * _tmp136 - _tmp80;
  const Scalar _tmp142 = _tmp61 * fh1;
  const Scalar _tmp143 = _tmp100 * _tmp120;
  const Scalar _tmp144 = _tmp108 * _tmp123 * _tmp95;
  const Scalar _tmp145 = -_tmp103 * _tmp117 * (-_tmp143 * _tmp70 * _tmp98 + _tmp143 * _tmp93) -
                         _tmp103 * _tmp135 * (_tmp133 * _tmp93 - _tmp144 * _tmp70) -
                         _tmp103 * _tmp140 * (_tmp126 * _tmp139 + _tmp137 * _tmp138) -
                         _tmp103 * _tmp142 * (_tmp112 * _tmp139 + _tmp138 * _tmp141 + Scalar(1.0));
  const Scalar _tmp146 = Scalar(1.0) / (_tmp145);
  const Scalar _tmp147 = std::asinh(
      _tmp146 * (_tmp116 * _tmp128 * (-_tmp115 * _tmp127 + _tmp125 * _tmp56) +
                 _tmp116 * _tmp61 * (_tmp110 * _tmp56 - _tmp114 * _tmp115) +
                 Scalar(1.0) * _tmp117 * (-_tmp115 * _tmp121 - _tmp118 * _tmp56 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp129 * (-_tmp115 * _tmp131 + _tmp130) +
                 Scalar(1.0) * _tmp135 * (-_tmp115 * _tmp134 + _tmp132) +
                 Scalar(1.0) * _tmp63 * (_tmp56 * _tmp57 - _tmp56)));
  const Scalar _tmp148 = Scalar(1.4083112389913199) * _tmp145;
  const Scalar _tmp149 = std::pow(Scalar(-_tmp75 + p_d(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp78 + p_d(1, 0)), Scalar(2));
  const Scalar _tmp150 = std::pow(Scalar(-_tmp65 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp67 + p_c(1, 0)), Scalar(2));
  const Scalar _tmp151 = _tmp82 * fh1;
  const Scalar _tmp152 = _tmp117 * _tmp143;
  const Scalar _tmp153 = _tmp128 * _tmp137 * _tmp151 - _tmp135 * _tmp144 +
                         _tmp141 * _tmp151 * _tmp61 - _tmp152 * _tmp98;
  const Scalar _tmp154 = Scalar(1.0) / (_tmp153);
  const Scalar _tmp155 = _tmp56 * _tmp63;
  const Scalar _tmp156 =
      std::asinh(_tmp154 * (_tmp114 * _tmp142 * _tmp50 + _tmp117 * _tmp121 * _tmp50 +
                            _tmp127 * _tmp140 * _tmp50 + _tmp129 * _tmp131 * _tmp50 +
                            _tmp134 * _tmp135 * _tmp50 - _tmp155 * _tmp57));
  const Scalar _tmp157 = Scalar(1.4083112389913199) * _tmp153;
  const Scalar _tmp158 = std::pow(Scalar(-_tmp88 + p_a(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp90 + p_a(1, 0)), Scalar(2));
  const Scalar _tmp159 =
      _tmp100 * _tmp112 * _tmp142 + _tmp100 * _tmp126 * _tmp140 + _tmp133 * _tmp135 + _tmp152;
  const Scalar _tmp160 = Scalar(1.0) / (_tmp159);
  const Scalar _tmp161 = std::asinh(_tmp160 * (-_tmp110 * _tmp142 * _tmp55 + _tmp117 * _tmp119 -
                                               _tmp125 * _tmp140 * _tmp55 - _tmp129 * _tmp130 -
                                               _tmp132 * _tmp135 + _tmp155));
  const Scalar _tmp162 = Scalar(1.4083112389913199) * _tmp159;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
          (-std::sinh(Scalar(1.0) * _tmp31) - std::sinh(Scalar(0.71007031138673404) * _tmp0 *
                                                        (-std::sqrt(_tmp30) - _tmp31 * _tmp32))) -
      std::sqrt(Scalar(_tmp30 +
                       std::pow(Scalar(-_tmp38 + p_b(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(1, 0) =
      _tmp148 * (-std::sinh(Scalar(1.0) * _tmp147) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp146 *
                           (-_tmp147 * _tmp148 - std::sqrt(_tmp149)))) -
      std::sqrt(Scalar(_tmp149 +
                       std::pow(Scalar(-_tmp72 + p_d(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(2, 0) =
      _tmp157 * (-std::sinh(Scalar(1.0) * _tmp156) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp154 *
                           (-std::sqrt(_tmp150) - _tmp156 * _tmp157))) -
      std::sqrt(Scalar(_tmp150 +
                       std::pow(Scalar(-_tmp64 + p_c(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(3, 0) =
      _tmp162 * (-std::sinh(Scalar(1.0) * _tmp161) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp160 *
                           (-std::sqrt(_tmp158) - _tmp161 * _tmp162))) -
      std::sqrt(Scalar(_tmp158 +
                       std::pow(Scalar(-_tmp87 + p_a(2, 0) - position_vector(2, 0)), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
