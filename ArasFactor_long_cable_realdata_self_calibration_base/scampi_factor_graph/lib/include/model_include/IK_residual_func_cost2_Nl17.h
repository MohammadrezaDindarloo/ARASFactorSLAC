// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl17
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl17(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 498

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = 2 * _tmp0 * _tmp1;
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp5 = _tmp3 * _tmp4;
  const Scalar _tmp6 = Scalar(0.20999999999999999) * _tmp2 - Scalar(0.20999999999999999) * _tmp5;
  const Scalar _tmp7 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp8 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp9 = -Scalar(0.010999999999999999) * _tmp7 - Scalar(0.010999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp3;
  const Scalar _tmp11 = _tmp0 * _tmp10;
  const Scalar _tmp12 = _tmp1 * _tmp4;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp13 + _tmp9;
  const Scalar _tmp15 = _tmp14 + _tmp6;
  const Scalar _tmp16 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp7 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp18 = _tmp11 - _tmp12;
  const Scalar _tmp19 = -Scalar(0.010999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp1 * _tmp10;
  const Scalar _tmp21 = _tmp0 * _tmp4;
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp19 + _tmp22;
  const Scalar _tmp24 = _tmp17 + _tmp23;
  const Scalar _tmp25 = _tmp24 + position_vector(1, 0);
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp20 - Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp28 = _tmp2 + _tmp5;
  const Scalar _tmp29 = -Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = _tmp27 + _tmp29;
  const Scalar _tmp31 = _tmp26 + _tmp30;
  const Scalar _tmp32 = _tmp31 + position_vector(0, 0);
  const Scalar _tmp33 = std::pow(Scalar(-_tmp25 + p_c(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp32 + p_c(0, 0)), Scalar(2));
  const Scalar _tmp34 = Scalar(1.0) / (fh1);
  const Scalar _tmp35 = std::asinh(_tmp34 * fv1);
  const Scalar _tmp36 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp37 = -_tmp27 + _tmp29;
  const Scalar _tmp38 = _tmp26 + _tmp37;
  const Scalar _tmp39 = Scalar(1.0) * _tmp38;
  const Scalar _tmp40 = -_tmp26;
  const Scalar _tmp41 = _tmp30 + _tmp40;
  const Scalar _tmp42 = _tmp39 - _tmp41;
  const Scalar _tmp43 = _tmp38 + position_vector(0, 0);
  const Scalar _tmp44 = _tmp43 - p_d(0, 0);
  const Scalar _tmp45 = Scalar(1.0) / (_tmp44);
  const Scalar _tmp46 = _tmp19 - _tmp22;
  const Scalar _tmp47 = _tmp17 + _tmp46;
  const Scalar _tmp48 = _tmp47 + position_vector(1, 0);
  const Scalar _tmp49 = _tmp48 - p_d(1, 0);
  const Scalar _tmp50 = _tmp45 * _tmp49;
  const Scalar _tmp51 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp52 = _tmp51 - p_b(0, 0);
  const Scalar _tmp53 = -_tmp17;
  const Scalar _tmp54 = _tmp23 + _tmp53;
  const Scalar _tmp55 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp56 = _tmp55 - p_b(1, 0);
  const Scalar _tmp57 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp56, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp58 = _tmp52 * _tmp57;
  const Scalar _tmp59 = _tmp56 * _tmp57;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp50 * _tmp58 - _tmp59);
  const Scalar _tmp61 = -_tmp13 + _tmp9;
  const Scalar _tmp62 = _tmp6 + _tmp61;
  const Scalar _tmp63 = -_tmp6;
  const Scalar _tmp64 = _tmp14 + _tmp63;
  const Scalar _tmp65 = _tmp58 * _tmp64;
  const Scalar _tmp66 = -_tmp50 * _tmp65 + _tmp59 * _tmp62;
  const Scalar _tmp67 = Scalar(1.0) * _tmp47;
  const Scalar _tmp68 = -_tmp67;
  const Scalar _tmp69 = Scalar(1.0) / (_tmp54 + _tmp68);
  const Scalar _tmp70 = Scalar(1.0) * _tmp69;
  const Scalar _tmp71 = -_tmp58 * _tmp62 + _tmp65;
  const Scalar _tmp72 = Scalar(1.0) * _tmp60;
  const Scalar _tmp73 = _tmp42 * _tmp60 * _tmp66 * _tmp70 - _tmp71 * _tmp72;
  const Scalar _tmp74 = _tmp37 + _tmp40;
  const Scalar _tmp75 = _tmp74 + position_vector(0, 0);
  const Scalar _tmp76 = _tmp75 - p_a(0, 0);
  const Scalar _tmp77 = _tmp46 + _tmp53;
  const Scalar _tmp78 = _tmp77 + position_vector(1, 0);
  const Scalar _tmp79 = _tmp78 - p_a(1, 0);
  const Scalar _tmp80 = std::pow(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp79, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp81 = _tmp76 * _tmp80;
  const Scalar _tmp82 =
      std::sqrt(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp49, Scalar(2))));
  const Scalar _tmp83 = Scalar(1.0) / (_tmp82);
  const Scalar _tmp84 = _tmp45 * _tmp82;
  const Scalar _tmp85 = _tmp84 * (_tmp38 * _tmp49 * _tmp83 - _tmp44 * _tmp47 * _tmp83);
  const Scalar _tmp86 = -_tmp41 * _tmp59 + _tmp54 * _tmp58 + _tmp58 * _tmp85;
  const Scalar _tmp87 = _tmp79 * _tmp80;
  const Scalar _tmp88 = _tmp50 * _tmp81 - _tmp87;
  const Scalar _tmp89 = _tmp60 * _tmp88;
  const Scalar _tmp90 = -_tmp74 * _tmp87 + _tmp77 * _tmp81 + _tmp81 * _tmp85 - _tmp86 * _tmp89;
  const Scalar _tmp91 = _tmp61 + _tmp63;
  const Scalar _tmp92 = _tmp64 * _tmp81;
  const Scalar _tmp93 = -_tmp50 * _tmp92 - _tmp66 * _tmp89 + _tmp87 * _tmp91;
  const Scalar _tmp94 = _tmp42 * _tmp69;
  const Scalar _tmp95 = -_tmp71 * _tmp89 - _tmp81 * _tmp91 + _tmp92 - _tmp93 * _tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp90 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp99 = _tmp95 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp72 * _tmp86 - _tmp73 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp73;
  const Scalar _tmp102 = _tmp88 * _tmp96;
  const Scalar _tmp103 = -_tmp101 * _tmp102 + Scalar(1.0);
  const Scalar _tmp104 = _tmp58 * _tmp60;
  const Scalar _tmp105 = _tmp81 * _tmp96;
  const Scalar _tmp106 = _tmp25 - p_c(1, 0);
  const Scalar _tmp107 = _tmp32 - p_c(0, 0);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp112 = _tmp39 + _tmp67 * _tmp94;
  const Scalar _tmp113 = 0;
  const Scalar _tmp114 = _tmp102 * _tmp113;
  const Scalar _tmp115 = _tmp113 * _tmp96;
  const Scalar _tmp116 = _tmp50 * _tmp60;
  const Scalar _tmp117 = _tmp116 * _tmp66 + _tmp50 * _tmp64;
  const Scalar _tmp118 = _tmp116 * _tmp71 - _tmp117 * _tmp94 - _tmp64;
  const Scalar _tmp119 = _tmp99 * (_tmp116 * _tmp86 - _tmp118 * _tmp97 - _tmp85);
  const Scalar _tmp120 = _tmp118 + _tmp119;
  const Scalar _tmp121 = -_tmp102 * _tmp120 - _tmp50;
  const Scalar _tmp122 = _tmp107 * _tmp108;
  const Scalar _tmp123 = _tmp122 * fh1;
  const Scalar _tmp124 = Scalar(1.0) * _tmp98;
  const Scalar _tmp125 = fh1 * (_tmp109 * _tmp31 - _tmp122 * _tmp24);
  const Scalar _tmp126 = -_tmp110 * _tmp84 * (_tmp101 * _tmp105 + _tmp103 * _tmp104) -
                         _tmp111 * _tmp84 * (-_tmp104 * _tmp114 + _tmp115 * _tmp81) -
                         _tmp123 * _tmp84 * (_tmp104 * _tmp121 + _tmp105 * _tmp120 + Scalar(1.0)) -
                         _tmp125 * _tmp84 * (-_tmp124 * _tmp58 * _tmp89 + _tmp124 * _tmp81);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = std::pow(Scalar(-_tmp43 + p_d(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp48 + p_d(1, 0)), Scalar(2));
  const Scalar _tmp129 = _tmp68 + _tmp77;
  const Scalar _tmp130 = _tmp129 * _tmp94;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 + _tmp39 - _tmp74);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp93 * _tmp96;
  const Scalar _tmp134 = _tmp129 * _tmp131;
  const Scalar _tmp135 = _tmp100 * _tmp134 - _tmp101 * _tmp133 - _tmp66 * _tmp72;
  const Scalar _tmp136 = _tmp132 * _tmp99;
  const Scalar _tmp137 = -_tmp124 * _tmp93 + _tmp129 * _tmp136;
  const Scalar _tmp138 = _tmp112 * _tmp131;
  const Scalar _tmp139 = -_tmp113 * _tmp133 - _tmp129 * _tmp138 + _tmp68;
  const Scalar _tmp140 = _tmp117 + _tmp119 * _tmp134 - _tmp120 * _tmp133;
  const Scalar _tmp141 = _tmp15 * fh1;
  const Scalar _tmp142 = -_tmp109 * _tmp141 - Scalar(40.024799999999999) * _tmp18 - _tmp24 * fv1;
  const Scalar _tmp143 = _tmp130 * _tmp132 + Scalar(1.0);
  const Scalar _tmp144 = _tmp132 * _tmp94;
  const Scalar _tmp145 = _tmp122 * _tmp141 + Scalar(40.024799999999999) * _tmp28 + _tmp31 * fv1;
  const Scalar _tmp146 = _tmp129 * _tmp69;
  const Scalar _tmp147 = std::asinh(
      _tmp127 * (Scalar(1.0) * _tmp110 * (_tmp100 * _tmp132 - _tmp135 * _tmp70) +
                 Scalar(1.0) * _tmp111 * (-_tmp112 * _tmp132 - _tmp139 * _tmp70 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp123 * (_tmp119 * _tmp132 - _tmp140 * _tmp70) +
                 Scalar(1.0) * _tmp125 * (_tmp136 - _tmp137 * _tmp70) +
                 Scalar(1.0) * _tmp142 * (-_tmp143 * _tmp70 + _tmp144) +
                 Scalar(1.0) * _tmp145 * (_tmp132 * _tmp146 - _tmp132)));
  const Scalar _tmp148 = Scalar(1.4083112389913199) * _tmp126;
  const Scalar _tmp149 = std::pow(Scalar(-_tmp51 + p_b(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp55 + p_b(1, 0)), Scalar(2));
  const Scalar _tmp150 = _tmp132 * _tmp145;
  const Scalar _tmp151 = _tmp124 * _tmp125;
  const Scalar _tmp152 = _tmp103 * _tmp110 * _tmp60 - _tmp111 * _tmp114 * _tmp60 +
                         _tmp121 * _tmp123 * _tmp60 - _tmp151 * _tmp89;
  const Scalar _tmp153 = Scalar(1.0) / (_tmp152);
  const Scalar _tmp154 =
      std::asinh(_tmp153 * (_tmp110 * _tmp135 * _tmp69 + _tmp111 * _tmp139 * _tmp69 +
                            _tmp123 * _tmp140 * _tmp69 + _tmp125 * _tmp137 * _tmp69 +
                            _tmp142 * _tmp143 * _tmp69 - _tmp146 * _tmp150));
  const Scalar _tmp155 = Scalar(1.4083112389913199) * _tmp152;
  const Scalar _tmp156 =
      _tmp101 * _tmp110 * _tmp96 + _tmp111 * _tmp115 + _tmp120 * _tmp123 * _tmp96 + _tmp151;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 = std::asinh(_tmp157 * (-_tmp100 * _tmp110 * _tmp131 + _tmp111 * _tmp138 -
                                               _tmp119 * _tmp123 * _tmp131 - _tmp125 * _tmp136 -
                                               _tmp142 * _tmp144 + _tmp150));
  const Scalar _tmp159 = std::pow(Scalar(-_tmp75 + p_a(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp78 + p_a(1, 0)), Scalar(2));
  const Scalar _tmp160 = Scalar(1.4083112389913199) * _tmp156;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp36 *
          (-std::sinh(Scalar(1.0) * _tmp35) - std::sinh(Scalar(0.71007031138673404) * _tmp34 *
                                                        (-std::sqrt(_tmp33) - _tmp35 * _tmp36))) -
      std::sqrt(Scalar(_tmp33 +
                       std::pow(Scalar(-_tmp15 + p_c(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(1, 0) =
      _tmp148 * (-std::sinh(Scalar(1.0) * _tmp147) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp127 *
                           (-std::sqrt(_tmp128) - _tmp147 * _tmp148))) -
      std::sqrt(Scalar(_tmp128 +
                       std::pow(Scalar(-_tmp64 + p_d(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(2, 0) =
      _tmp155 * (-std::sinh(Scalar(1.0) * _tmp154) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp153 *
                           (-std::sqrt(_tmp149) - _tmp154 * _tmp155))) -
      std::sqrt(Scalar(_tmp149 +
                       std::pow(Scalar(-_tmp62 + p_b(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(3, 0) =
      _tmp160 * (-std::sinh(Scalar(1.0) * _tmp158) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp157 *
                           (-_tmp158 * _tmp160 - std::sqrt(_tmp159)))) -
      std::sqrt(Scalar(_tmp159 +
                       std::pow(Scalar(-_tmp91 + p_a(2, 0) - position_vector(2, 0)), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
