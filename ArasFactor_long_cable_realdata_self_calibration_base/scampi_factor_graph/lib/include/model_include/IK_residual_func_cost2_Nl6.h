// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 494

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (156)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = 2 * _tmp1 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp5 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = 2 * _tmp4;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = _tmp2 * _tmp5;
  const Scalar _tmp11 = -_tmp10 + _tmp9;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp14 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 +
                        Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999);
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp7;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp2 * _tmp8;
  const Scalar _tmp24 = _tmp1 * _tmp5;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = _tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = std::pow(Scalar(-_tmp18 + p_b(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp29 + p_b(0, 0)), Scalar(2));
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(1.4083112389913199) * fh1;
  const Scalar _tmp33 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp34 =
      -Scalar(0.010999999999999999) * _tmp13 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp35 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp36 = _tmp34 - _tmp35;
  const Scalar _tmp37 = _tmp33 + _tmp36;
  const Scalar _tmp38 = -_tmp22 + _tmp26;
  const Scalar _tmp39 = _tmp20 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 - p_a(0, 0);
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = -_tmp7;
  const Scalar _tmp44 = _tmp16 + _tmp43;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 - p_a(1, 0);
  const Scalar _tmp47 = _tmp42 * _tmp46;
  const Scalar _tmp48 = _tmp19 + _tmp38;
  const Scalar _tmp49 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp50 = _tmp49 - p_d(0, 0);
  const Scalar _tmp51 = _tmp12 + _tmp15;
  const Scalar _tmp52 = _tmp43 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(1, 0);
  const Scalar _tmp54 = _tmp53 - p_d(1, 0);
  const Scalar _tmp55 = std::pow(Scalar(std::pow(_tmp50, Scalar(2)) + std::pow(_tmp54, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp56 = _tmp54 * _tmp55;
  const Scalar _tmp57 = _tmp50 * _tmp55;
  const Scalar _tmp58 = _tmp47 * _tmp57 - _tmp56;
  const Scalar _tmp59 = -_tmp33;
  const Scalar _tmp60 = _tmp36 + _tmp59;
  const Scalar _tmp61 = _tmp34 + _tmp35;
  const Scalar _tmp62 = _tmp59 + _tmp61;
  const Scalar _tmp63 = _tmp33 + _tmp61;
  const Scalar _tmp64 = _tmp51 + _tmp7;
  const Scalar _tmp65 = _tmp64 + position_vector(1, 0);
  const Scalar _tmp66 = _tmp65 - p_c(1, 0);
  const Scalar _tmp67 = _tmp19 + _tmp27;
  const Scalar _tmp68 = _tmp67 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 - p_c(0, 0);
  const Scalar _tmp70 = std::pow(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp69, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp71 = _tmp66 * _tmp70;
  const Scalar _tmp72 = _tmp47 * _tmp60;
  const Scalar _tmp73 = _tmp69 * _tmp70;
  const Scalar _tmp74 = _tmp63 * _tmp71 - _tmp72 * _tmp73;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp47 * _tmp73 - _tmp71);
  const Scalar _tmp76 = _tmp58 * _tmp75;
  const Scalar _tmp77 = _tmp56 * _tmp62 - _tmp57 * _tmp72 - _tmp74 * _tmp76;
  const Scalar _tmp78 = Scalar(1.0) * _tmp44;
  const Scalar _tmp79 = -_tmp78;
  const Scalar _tmp80 = Scalar(1.0) / (_tmp64 + _tmp79);
  const Scalar _tmp81 = Scalar(1.0) * _tmp39;
  const Scalar _tmp82 = -_tmp67 + _tmp81;
  const Scalar _tmp83 = _tmp80 * _tmp82;
  const Scalar _tmp84 = _tmp60 * _tmp73 - _tmp63 * _tmp73;
  const Scalar _tmp85 = _tmp57 * _tmp60 - _tmp57 * _tmp62 - _tmp76 * _tmp84 - _tmp77 * _tmp83;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp85);
  const Scalar _tmp87 =
      std::sqrt(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp46, Scalar(2))));
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = _tmp42 * _tmp87;
  const Scalar _tmp90 = _tmp89 * (_tmp39 * _tmp46 * _tmp88 - _tmp41 * _tmp44 * _tmp88);
  const Scalar _tmp91 = _tmp47 * _tmp75;
  const Scalar _tmp92 = _tmp72 + _tmp74 * _tmp91;
  const Scalar _tmp93 = -_tmp60 - _tmp83 * _tmp92 + _tmp84 * _tmp91;
  const Scalar _tmp94 = _tmp64 * _tmp73 - _tmp67 * _tmp71 + _tmp73 * _tmp90;
  const Scalar _tmp95 = -_tmp48 * _tmp56 + _tmp52 * _tmp57 + _tmp57 * _tmp90 - _tmp76 * _tmp94;
  const Scalar _tmp96 = _tmp86 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp85 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp90 + _tmp91 * _tmp94 - _tmp93 * _tmp96);
  const Scalar _tmp100 = _tmp86 * (_tmp93 + _tmp99);
  const Scalar _tmp101 = -_tmp100 * _tmp58 - _tmp47;
  const Scalar _tmp102 = _tmp73 * _tmp75;
  const Scalar _tmp103 = _tmp29 - p_b(0, 0);
  const Scalar _tmp104 = _tmp18 - p_b(1, 0);
  const Scalar _tmp105 =
      std::pow(Scalar(std::pow(_tmp103, Scalar(2)) + std::pow(_tmp104, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp106 = _tmp103 * _tmp105;
  const Scalar _tmp107 = _tmp106 * fh1;
  const Scalar _tmp108 = Scalar(1.0) * _tmp75;
  const Scalar _tmp109 = Scalar(1.0) * _tmp80;
  const Scalar _tmp110 = -_tmp108 * _tmp84 + _tmp109 * _tmp74 * _tmp75 * _tmp82;
  const Scalar _tmp111 = _tmp98 * (-_tmp108 * _tmp94 - _tmp110 * _tmp96);
  const Scalar _tmp112 = _tmp86 * (_tmp110 + _tmp111);
  const Scalar _tmp113 = -_tmp112 * _tmp58 + Scalar(1.0);
  const Scalar _tmp114 = _tmp104 * _tmp105;
  const Scalar _tmp115 = _tmp114 * fh1;
  const Scalar _tmp116 = Scalar(333.54000000000002) - fv1;
  const Scalar _tmp117 = _tmp78 * _tmp83 + _tmp81;
  const Scalar _tmp118 = 0;
  const Scalar _tmp119 = Scalar(1.0) * _tmp97;
  const Scalar _tmp120 = _tmp108 * _tmp58 * _tmp97;
  const Scalar _tmp121 = fh1 * (-_tmp106 * _tmp17 + _tmp114 * _tmp28);
  const Scalar _tmp122 = -_tmp107 * _tmp89 * (_tmp100 * _tmp57 + _tmp101 * _tmp102 + Scalar(1.0)) -
                         _tmp115 * _tmp89 * (_tmp102 * _tmp113 + _tmp112 * _tmp57) -
                         _tmp116 * _tmp89 * (_tmp118 * _tmp57 - _tmp118 * _tmp73 * _tmp76) -
                         _tmp121 * _tmp89 * (_tmp119 * _tmp57 - _tmp120 * _tmp73);
  const Scalar _tmp123 = Scalar(1.0) / (_tmp122);
  const Scalar _tmp124 = _tmp52 + _tmp79;
  const Scalar _tmp125 = _tmp124 * _tmp83;
  const Scalar _tmp126 = Scalar(1.0) / (-_tmp125 - _tmp48 + _tmp81);
  const Scalar _tmp127 = _tmp117 * _tmp126;
  const Scalar _tmp128 = -_tmp118 * _tmp77 - _tmp124 * _tmp127 + _tmp79;
  const Scalar _tmp129 = Scalar(1.0) * _tmp126;
  const Scalar _tmp130 = _tmp124 * _tmp126;
  const Scalar _tmp131 = -_tmp100 * _tmp77 + _tmp130 * _tmp99 + _tmp92;
  const Scalar _tmp132 = _tmp124 * _tmp80;
  const Scalar _tmp133 = _tmp37 * fh1;
  const Scalar _tmp134 = _tmp106 * _tmp133 + Scalar(40.024799999999999) * _tmp25 + _tmp28 * fv1;
  const Scalar _tmp135 = _tmp129 * _tmp98;
  const Scalar _tmp136 = -_tmp119 * _tmp77 + _tmp124 * _tmp135;
  const Scalar _tmp137 = -_tmp108 * _tmp74 + _tmp111 * _tmp130 - _tmp112 * _tmp77;
  const Scalar _tmp138 = -Scalar(40.024799999999999) * _tmp11 - _tmp114 * _tmp133 - _tmp17 * fv1;
  const Scalar _tmp139 = _tmp125 * _tmp129 + Scalar(1.0);
  const Scalar _tmp140 = _tmp129 * _tmp83;
  const Scalar _tmp141 = std::asinh(
      _tmp123 * (Scalar(1.0) * _tmp107 * (-_tmp109 * _tmp131 + _tmp129 * _tmp99) +
                 Scalar(1.0) * _tmp115 * (-_tmp109 * _tmp137 + _tmp111 * _tmp129) +
                 Scalar(1.0) * _tmp116 * (-_tmp109 * _tmp128 - _tmp117 * _tmp129 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp121 * (-_tmp109 * _tmp136 + _tmp135) +
                 Scalar(1.0) * _tmp134 * (_tmp129 * _tmp132 - _tmp129) +
                 Scalar(1.0) * _tmp138 * (-_tmp109 * _tmp139 + _tmp140)));
  const Scalar _tmp142 = Scalar(1.4083112389913199) * _tmp122;
  const Scalar _tmp143 = std::pow(Scalar(-_tmp40 + p_a(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp45 + p_a(1, 0)), Scalar(2));
  const Scalar _tmp144 = std::pow(Scalar(-_tmp65 + p_c(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp68 + p_c(0, 0)), Scalar(2));
  const Scalar _tmp145 = _tmp129 * _tmp134;
  const Scalar _tmp146 = _tmp116 * _tmp118;
  const Scalar _tmp147 = _tmp101 * _tmp107 * _tmp75 + _tmp113 * _tmp115 * _tmp75 -
                         _tmp120 * _tmp121 - _tmp146 * _tmp76;
  const Scalar _tmp148 = Scalar(1.0) / (_tmp147);
  const Scalar _tmp149 =
      std::asinh(_tmp148 * (_tmp107 * _tmp131 * _tmp80 + _tmp115 * _tmp137 * _tmp80 +
                            _tmp116 * _tmp128 * _tmp80 + _tmp121 * _tmp136 * _tmp80 -
                            _tmp132 * _tmp145 + _tmp138 * _tmp139 * _tmp80));
  const Scalar _tmp150 = Scalar(1.4083112389913199) * _tmp147;
  const Scalar _tmp151 = _tmp100 * _tmp107 + _tmp112 * _tmp115 + _tmp119 * _tmp121 + _tmp146;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 =
      std::asinh(_tmp152 * (-_tmp107 * _tmp126 * _tmp99 - _tmp111 * _tmp115 * _tmp126 +
                            _tmp116 * _tmp127 - _tmp121 * _tmp135 - _tmp138 * _tmp140 + _tmp145));
  const Scalar _tmp154 = std::pow(Scalar(-_tmp49 + p_d(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp53 + p_d(1, 0)), Scalar(2));
  const Scalar _tmp155 = Scalar(1.4083112389913199) * _tmp151;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
          (-std::sinh(Scalar(1.0) * _tmp31) - std::sinh(Scalar(0.71007031138673404) * _tmp0 *
                                                        (-std::sqrt(_tmp30) - _tmp31 * _tmp32))) -
      std::sqrt(Scalar(_tmp30 +
                       std::pow(Scalar(-_tmp37 + p_b(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(1, 0) =
      _tmp142 * (-std::sinh(Scalar(1.0) * _tmp141) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp123 *
                           (-_tmp141 * _tmp142 - std::sqrt(_tmp143)))) -
      std::sqrt(Scalar(_tmp143 +
                       std::pow(Scalar(-_tmp60 + p_a(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(2, 0) =
      _tmp150 * (-std::sinh(Scalar(1.0) * _tmp149) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp148 *
                           (-std::sqrt(_tmp144) - _tmp149 * _tmp150))) -
      std::sqrt(Scalar(_tmp144 +
                       std::pow(Scalar(-_tmp63 + p_c(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(3, 0) =
      _tmp155 * (-std::sinh(Scalar(1.0) * _tmp153) -
                 std::sinh(Scalar(0.71007031138673404) * _tmp152 *
                           (-_tmp153 * _tmp155 - std::sqrt(_tmp154)))) -
      std::sqrt(Scalar(_tmp154 +
                       std::pow(Scalar(-_tmp62 + p_d(2, 0) - position_vector(2, 0)), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
