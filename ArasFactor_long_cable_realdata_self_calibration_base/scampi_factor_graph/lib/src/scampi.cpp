                              ...
                              int inner_interval, Eigen::Matrix<double, 4, 1> cable_length, VectorXi reorder_idx, gtsam::Values *oprimization_result_LM)
{
    ...
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, 0.1/3.0); // 0.02/3.0
    ...
    auto prior_noiseModel_delta_rot = noiseModel::Diagonal::Sigmas((gtsam::Vector(3)<< 5.0 * M_PI/180.0,  5.0 * M_PI/180.0,  5.0 * M_PI/180.0).finished()); // 2.0e-1
    auto prior_noiseModel_pulley = noiseModel::Diagonal::Sigmas((gtsam::Vector(3)<< 11.0/sqrt(3.0)/3.0,  11.0/sqrt(3.0)/3.0,  11.0/sqrt(3.0)/3.0).finished());
                                            ...
                                            gtsam::Values *oprimization_result_LM)
{
    ...
    auto prior_noiseModel_delta_rot = noiseModel::Diagonal::Sigmas((gtsam::Vector(3)<< (5.0e0 * M_PI/180.0) / 3.0, (5.0e0 * M_PI/180.0) / 3.0, (5.0e0 * M_PI/180.0) / 3.0).finished()); // 1.0e-2
    ...
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, 0.2/3.0 ); // z     1.5
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, 1.0/3.0); // l -||b-a||   
    auto Sensor_noiseModel_cost3 = gtsam::noiseModel::Isotropic::Sigma(4, 0.15/3.0); // encoder   0.35
