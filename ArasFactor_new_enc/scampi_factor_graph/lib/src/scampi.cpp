                              ...
                              double init_estimate_h1, double init_estimate_v1, gtsam::Rot3 init_estimate_rot, gtsam::Values *oprimization_result_LM)
{
    ...
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(1.0/5000.0));
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(100.0/1.0));
    auto Sensor_noiseModel_cost3 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(1.0));
                                            ...
                                            gtsam::Values *oprimization_result_LM)
{
    ...
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10.0/1.0));
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10.0/1.0));
    auto prior_noiseModel_pulley = noiseModel::Diagonal::Sigmas((gtsam::Vector(12)<<pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise, pulley_noise).finished());

    for (size_t i = 0; i < p_platform_collection.size(); i++)
    {
        graph.add(std::make_shared<FK_factor_graoh_cost1>(Symbol('h', i), Symbol('v', i), Symbol('r', i), Symbol('t', i), Symbol('p', 0), cable_length_collection[i], rot_init_platform_collection[i], Sensor_noiseModel_cost1));
        graph.add(std::make_shared<FK_factor_graoh_cost2>(Symbol('h', i), Symbol('v', i), Symbol('r', i), Symbol('t', i), Symbol('p', 0), cable_length_collection[i], rot_init_platform_collection[i], Sensor_noiseModel_cost2));
    ...
    // graph.add(gtsam::PriorFactor<gtsam::Matrix43>(Symbol('p', 0), pulley_position_estimate, prior_noiseModel_pulley));

    initial_estimate.insert(Symbol('p', 0), pulley_position_estimate);
...
{  
    ...
    for (size_t j = 0; j < p_platform_collection.size(); j++)
    {
        ...
        state_init.rot_platform = rot_init_platform_collection[j];
    ...
    // optimization_result.print();
    //harvest the results
    Eigen::Matrix<double, 4, 3> pulley_optimized = optimization_result.at<gtsam::Matrix43>(Symbol('p', 0));
