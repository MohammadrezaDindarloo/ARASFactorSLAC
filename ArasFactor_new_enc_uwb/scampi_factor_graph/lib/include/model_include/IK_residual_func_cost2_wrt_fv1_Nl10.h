// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 601

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp13 = _tmp3 * _tmp8;
  const Scalar _tmp14 = _tmp1 * _tmp10;
  const Scalar _tmp15 = _tmp13 - _tmp14;
  const Scalar _tmp16 = Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp16;
  const Scalar _tmp18 = _tmp12 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp29 = _tmp26 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(1.0) * _tmp0 /
                        std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp35 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp36 = _tmp26 - _tmp28;
  const Scalar _tmp37 = _tmp21 + _tmp36;
  const Scalar _tmp38 = Scalar(1.0) * _tmp37;
  const Scalar _tmp39 = -_tmp12;
  const Scalar _tmp40 = _tmp39 + _tmp5;
  const Scalar _tmp41 = _tmp17 + _tmp40;
  const Scalar _tmp42 = Scalar(1.0) * _tmp41;
  const Scalar _tmp43 = -_tmp42;
  const Scalar _tmp44 = _tmp17 + _tmp39 + _tmp6;
  const Scalar _tmp45 = Scalar(1.0) / (_tmp43 + _tmp44);
  const Scalar _tmp46 = _tmp22 + _tmp36;
  const Scalar _tmp47 = _tmp38 - _tmp46;
  const Scalar _tmp48 = _tmp45 * _tmp47;
  const Scalar _tmp49 = _tmp38 + _tmp42 * _tmp48;
  const Scalar _tmp50 = _tmp18 + _tmp5;
  const Scalar _tmp51 = _tmp43 + _tmp50;
  const Scalar _tmp52 = _tmp48 * _tmp51;
  const Scalar _tmp53 = _tmp21 + _tmp29;
  const Scalar _tmp54 = Scalar(1.0) / (_tmp38 - _tmp52 - _tmp53);
  const Scalar _tmp55 = Scalar(1.0) * _tmp54;
  const Scalar _tmp56 = _tmp49 * _tmp54;
  const Scalar _tmp57 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp58 = -_tmp57;
  const Scalar _tmp59 = -Scalar(0.010999999999999999) * _tmp2 -
                        Scalar(0.010999999999999999) * _tmp27 + Scalar(-0.010999999999999999);
  const Scalar _tmp60 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp61 = _tmp59 - _tmp60;
  const Scalar _tmp62 = _tmp58 + _tmp61;
  const Scalar _tmp63 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp64 = _tmp63 + Scalar(8.3196563720703107);
  const Scalar _tmp65 = _tmp46 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(1.9874742031097401);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp64 * _tmp67;
  const Scalar _tmp69 = _tmp41 + position_vector(1, 0);
  const Scalar _tmp70 = _tmp69 + Scalar(-4.83288938413423);
  const Scalar _tmp71 = _tmp37 + position_vector(0, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(1.7965602546229);
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = _tmp70 * _tmp73;
  const Scalar _tmp75 = _tmp57 + _tmp61;
  const Scalar _tmp76 = _tmp66 * _tmp67;
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = _tmp62 * _tmp68 - _tmp74 * _tmp77;
  const Scalar _tmp79 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp80 = _tmp79 + Scalar(-4.7744369927459998);
  const Scalar _tmp81 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(-2.7171519410699099);
  const Scalar _tmp83 = std::pow(Scalar(std::pow(_tmp80, Scalar(2)) + std::pow(_tmp82, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp84 = _tmp82 * _tmp83;
  const Scalar _tmp85 = _tmp80 * _tmp83;
  const Scalar _tmp86 = _tmp74 * _tmp84 - _tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (-_tmp68 + _tmp74 * _tmp76);
  const Scalar _tmp88 = _tmp86 * _tmp87;
  const Scalar _tmp89 = _tmp59 + _tmp60;
  const Scalar _tmp90 = _tmp57 + _tmp89;
  const Scalar _tmp91 = _tmp75 * _tmp84;
  const Scalar _tmp92 = -_tmp74 * _tmp91 - _tmp78 * _tmp88 + _tmp85 * _tmp90;
  const Scalar _tmp93 = -_tmp62 * _tmp76 + _tmp77;
  const Scalar _tmp94 = -_tmp48 * _tmp92 - _tmp84 * _tmp90 - _tmp88 * _tmp93 + _tmp91;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 = 0;
  const Scalar _tmp97 = _tmp45 * (_tmp43 - _tmp51 * _tmp56 - _tmp92 * _tmp96);
  const Scalar _tmp98 = -Scalar(1.0) * _tmp49 * _tmp55 - Scalar(1.0) * _tmp97 + Scalar(1.0);
  const Scalar _tmp99 = _tmp20 + Scalar(8.3885017487099702);
  const Scalar _tmp100 = _tmp31 + Scalar(-2.5193355532036801);
  const Scalar _tmp101 =
      std::pow(Scalar(std::pow(_tmp100, Scalar(2)) + std::pow(_tmp99, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp102 = _tmp101 * _tmp99;
  const Scalar _tmp103 = fh1 * (_tmp58 + _tmp89);
  const Scalar _tmp104 = -_tmp102 * _tmp103 - Scalar(5.1796800000000003) * _tmp15 - _tmp19 * fv1;
  const Scalar _tmp105 = _tmp48 * _tmp55;
  const Scalar _tmp106 = _tmp52 * _tmp55 + Scalar(1.0);
  const Scalar _tmp107 = Scalar(1.0) * _tmp45;
  const Scalar _tmp108 = Scalar(1.0) * _tmp105 - Scalar(1.0) * _tmp106 * _tmp107;
  const Scalar _tmp109 =
      std::sqrt(Scalar(std::pow(_tmp70, Scalar(2)) + std::pow(_tmp72, Scalar(2))));
  const Scalar _tmp110 = Scalar(1.0) / (_tmp109);
  const Scalar _tmp111 = _tmp109 * _tmp73;
  const Scalar _tmp112 = _tmp111 * (_tmp110 * _tmp37 * _tmp70 - _tmp110 * _tmp41 * _tmp72);
  const Scalar _tmp113 = _tmp112 * _tmp76 + _tmp44 * _tmp76 - _tmp46 * _tmp68;
  const Scalar _tmp114 = Scalar(1.0) * _tmp87;
  const Scalar _tmp115 = _tmp107 * _tmp47 * _tmp78 * _tmp87 - _tmp114 * _tmp93;
  const Scalar _tmp116 = _tmp112 * _tmp84 - _tmp113 * _tmp88 + _tmp50 * _tmp84 - _tmp53 * _tmp85;
  const Scalar _tmp117 = _tmp116 * _tmp95;
  const Scalar _tmp118 = Scalar(1.0) / (_tmp116);
  const Scalar _tmp119 = _tmp118 * _tmp94;
  const Scalar _tmp120 = _tmp119 * (-_tmp113 * _tmp114 - _tmp115 * _tmp117);
  const Scalar _tmp121 = _tmp51 * _tmp54;
  const Scalar _tmp122 = _tmp115 + _tmp120;
  const Scalar _tmp123 = _tmp92 * _tmp95;
  const Scalar _tmp124 = -_tmp114 * _tmp78 + _tmp120 * _tmp121 - _tmp122 * _tmp123;
  const Scalar _tmp125 = _tmp102 * fh1;
  const Scalar _tmp126 = _tmp74 * _tmp87;
  const Scalar _tmp127 = _tmp126 * _tmp78 + _tmp74 * _tmp75;
  const Scalar _tmp128 = _tmp126 * _tmp93 - _tmp127 * _tmp48 - _tmp75;
  const Scalar _tmp129 = _tmp119 * (-_tmp112 + _tmp113 * _tmp126 - _tmp117 * _tmp128);
  const Scalar _tmp130 = _tmp128 + _tmp129;
  const Scalar _tmp131 = _tmp121 * _tmp129 - _tmp123 * _tmp130 + _tmp127;
  const Scalar _tmp132 = _tmp100 * _tmp101;
  const Scalar _tmp133 = _tmp132 * fh1;
  const Scalar _tmp134 = _tmp103 * _tmp132 + Scalar(5.1796800000000003) * _tmp25 + _tmp30 * fv1;
  const Scalar _tmp135 = _tmp45 * _tmp51;
  const Scalar _tmp136 = Scalar(1.0) * _tmp135 * _tmp55 - Scalar(1.0) * _tmp55;
  const Scalar _tmp137 = _tmp119 * _tmp55;
  const Scalar _tmp138 = Scalar(1.0) * _tmp118;
  const Scalar _tmp139 = _tmp137 * _tmp51 - _tmp138 * _tmp92;
  const Scalar _tmp140 = fh1 * (_tmp102 * _tmp30 - _tmp132 * _tmp19);
  const Scalar _tmp141 =
      _tmp104 * _tmp108 + Scalar(1.0) * _tmp125 * (-_tmp107 * _tmp124 + _tmp120 * _tmp55) +
      Scalar(1.0) * _tmp133 * (-_tmp107 * _tmp131 + _tmp129 * _tmp55) + _tmp134 * _tmp136 +
      Scalar(1.0) * _tmp140 * (-_tmp107 * _tmp139 + _tmp137) + _tmp35 * _tmp98;
  const Scalar _tmp142 = _tmp76 * _tmp88;
  const Scalar _tmp143 = _tmp111 * (-_tmp142 * _tmp96 + _tmp84 * _tmp96);
  const Scalar _tmp144 = _tmp84 * _tmp95;
  const Scalar _tmp145 = _tmp86 * _tmp95;
  const Scalar _tmp146 = -_tmp130 * _tmp145 - _tmp74;
  const Scalar _tmp147 = _tmp76 * _tmp87;
  const Scalar _tmp148 = -_tmp122 * _tmp145 + Scalar(1.0);
  const Scalar _tmp149 = -_tmp111 * _tmp125 * (_tmp122 * _tmp144 + _tmp147 * _tmp148) -
                         _tmp111 * _tmp133 * (_tmp130 * _tmp144 + _tmp146 * _tmp147 + Scalar(1.0)) -
                         _tmp111 * _tmp140 * (-_tmp138 * _tmp142 + _tmp138 * _tmp84) -
                         _tmp143 * _tmp35;
  const Scalar _tmp150 = Scalar(1.0) / (_tmp149);
  const Scalar _tmp151 = std::asinh(_tmp141 * _tmp150);
  const Scalar _tmp152 = Scalar(1.0) * _tmp151;
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp149;
  const Scalar _tmp154 =
      -_tmp151 * _tmp153 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp69), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp71 - 1), Scalar(2))));
  const Scalar _tmp155 = Scalar(0.1034955) * _tmp150;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp143;
  const Scalar _tmp158 = _tmp16 + _tmp40;
  const Scalar _tmp159 = std::pow(_tmp149, Scalar(-2));
  const Scalar _tmp160 = _tmp143 * _tmp159;
  const Scalar _tmp161 =
      (-_tmp141 * _tmp160 + _tmp150 * (_tmp108 * _tmp158 + _tmp136 * _tmp30 - _tmp98)) /
      std::sqrt(Scalar(std::pow(_tmp141, Scalar(2)) * _tmp159 + 1));
  const Scalar _tmp162 = _tmp35 * _tmp96;
  const Scalar _tmp163 = _tmp138 * _tmp140;
  const Scalar _tmp164 =
      _tmp125 * _tmp148 * _tmp87 + _tmp133 * _tmp146 * _tmp87 - _tmp162 * _tmp88 - _tmp163 * _tmp88;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp45 * fh1;
  const Scalar _tmp167 = _tmp134 * _tmp55;
  const Scalar _tmp168 = _tmp106 * _tmp45;
  const Scalar _tmp169 = _tmp102 * _tmp124 * _tmp166 + _tmp104 * _tmp168 +
                         _tmp131 * _tmp132 * _tmp166 - _tmp135 * _tmp167 +
                         _tmp139 * _tmp140 * _tmp45 + _tmp35 * _tmp97;
  const Scalar _tmp170 = std::asinh(_tmp165 * _tmp169);
  const Scalar _tmp171 = Scalar(1.0) * _tmp170;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp173 =
      -_tmp170 * _tmp172 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp63 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp65 - 1), Scalar(2))));
  const Scalar _tmp174 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp175 = _tmp173 * _tmp174;
  const Scalar _tmp176 = Scalar(9.6622558468725703) * _tmp96;
  const Scalar _tmp177 = _tmp176 * _tmp88;
  const Scalar _tmp178 = _tmp30 * _tmp55;
  const Scalar _tmp179 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp180 = _tmp179 * _tmp88 * _tmp96;
  const Scalar _tmp181 =
      (_tmp165 * (-_tmp135 * _tmp178 + _tmp158 * _tmp168 - _tmp97) - _tmp169 * _tmp180) /
      std::sqrt(Scalar(std::pow(_tmp169, Scalar(2)) * _tmp179 + 1));
  const Scalar _tmp182 = -_tmp104 * _tmp105 - _tmp120 * _tmp125 * _tmp54 -
                         _tmp129 * _tmp133 * _tmp54 - _tmp137 * _tmp140 + _tmp167 + _tmp35 * _tmp56;
  const Scalar _tmp183 =
      _tmp122 * _tmp125 * _tmp95 + _tmp130 * _tmp133 * _tmp95 + _tmp162 + _tmp163;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp182 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp188 =
      -_tmp185 * _tmp187 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp79), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp81), Scalar(2))));
  const Scalar _tmp189 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp190 = _tmp188 * _tmp189;
  const Scalar _tmp191 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp192 = _tmp191 * _tmp96;
  const Scalar _tmp193 = (_tmp182 * _tmp192 + _tmp184 * (-_tmp105 * _tmp158 + _tmp178 - _tmp56)) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp191 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
      (-_tmp34 * std::cosh(Scalar(1.0) * _tmp32) +
       _tmp34 *
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp32 * _tmp33 -
                Scalar(8.3885017487099702) *
                    std::sqrt(Scalar(
                        Scalar(0.090199313518583735) *
                            std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp31), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp20 - 1), Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-Scalar(1.0) * _tmp161 * std::cosh(_tmp152) -
                          (-Scalar(0.1034955) * _tmp154 * _tmp160 +
                           _tmp155 * (-_tmp151 * _tmp157 - _tmp153 * _tmp161)) *
                              std::cosh(_tmp156)) +
               _tmp157 * (-std::sinh(_tmp152) - std::sinh(_tmp156));
  _res(2, 0) = _tmp172 * (-Scalar(1.0) * _tmp181 * std::cosh(_tmp171) -
                          (-Scalar(0.1034955) * _tmp173 * _tmp180 +
                           _tmp174 * (-_tmp170 * _tmp177 - _tmp172 * _tmp181)) *
                              std::cosh(_tmp175)) +
               _tmp177 * (-std::sinh(_tmp171) - std::sinh(_tmp175));
  _res(3, 0) = -_tmp176 * (-std::sinh(_tmp186) - std::sinh(_tmp190)) +
               _tmp187 * (-Scalar(1.0) * _tmp193 * std::cosh(_tmp186) -
                          (Scalar(0.1034955) * _tmp188 * _tmp192 +
                           _tmp189 * (_tmp176 * _tmp185 - _tmp187 * _tmp193)) *
                              std::cosh(_tmp190));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
