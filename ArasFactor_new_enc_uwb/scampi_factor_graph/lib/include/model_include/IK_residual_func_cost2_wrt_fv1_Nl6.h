// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 602

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (191)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp1;
  const Scalar _tmp9 = _tmp7 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp13 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp14 = _tmp1 * _tmp10;
  const Scalar _tmp15 = _tmp13 - _tmp14;
  const Scalar _tmp16 = Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp16;
  const Scalar _tmp18 = _tmp12 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp23 = _tmp3 * _tmp8;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(1.0) * _tmp0 /
                        std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp34 = -_tmp12;
  const Scalar _tmp35 = _tmp17 + _tmp34;
  const Scalar _tmp36 = _tmp35 + _tmp6;
  const Scalar _tmp37 = _tmp36 + position_vector(1, 0);
  const Scalar _tmp38 = _tmp37 + Scalar(8.3196563720703107);
  const Scalar _tmp39 = -_tmp22;
  const Scalar _tmp40 = _tmp28 + _tmp39;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp41 + Scalar(1.9874742031097401);
  const Scalar _tmp43 =
      std::sqrt(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp42, Scalar(2))));
  const Scalar _tmp44 = Scalar(1.0) / (_tmp43);
  const Scalar _tmp45 = Scalar(1.0) / (_tmp42);
  const Scalar _tmp46 = _tmp43 * _tmp45;
  const Scalar _tmp47 = _tmp46 * (-_tmp36 * _tmp42 * _tmp44 + _tmp38 * _tmp40 * _tmp44);
  const Scalar _tmp48 = _tmp38 * _tmp45;
  const Scalar _tmp49 = _tmp18 + _tmp5;
  const Scalar _tmp50 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(-4.7744369927459998);
  const Scalar _tmp52 = _tmp26 + _tmp27;
  const Scalar _tmp53 = _tmp22 + _tmp52;
  const Scalar _tmp54 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(-2.7171519410699099);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp51 * _tmp56;
  const Scalar _tmp58 = _tmp55 * _tmp56;
  const Scalar _tmp59 = Scalar(1.0) / (_tmp48 * _tmp58 - _tmp57);
  const Scalar _tmp60 = _tmp59 * (_tmp47 * _tmp58 + _tmp49 * _tmp58 - _tmp53 * _tmp57);
  const Scalar _tmp61 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp62 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp63 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp64 = _tmp62 + _tmp63;
  const Scalar _tmp65 = _tmp61 + _tmp64;
  const Scalar _tmp66 = -_tmp61;
  const Scalar _tmp67 = _tmp62 - _tmp63;
  const Scalar _tmp68 = _tmp66 + _tmp67;
  const Scalar _tmp69 = _tmp48 * _tmp68;
  const Scalar _tmp70 = _tmp59 * (_tmp57 * _tmp65 - _tmp58 * _tmp69);
  const Scalar _tmp71 = _tmp48 * _tmp70 + _tmp69;
  const Scalar _tmp72 = Scalar(1.0) * _tmp36;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp49 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp40;
  const Scalar _tmp76 = -_tmp53 + _tmp75;
  const Scalar _tmp77 = _tmp74 * _tmp76;
  const Scalar _tmp78 = _tmp59 * (-_tmp58 * _tmp65 + _tmp58 * _tmp68);
  const Scalar _tmp79 = _tmp48 * _tmp78 - _tmp68 - _tmp71 * _tmp77;
  const Scalar _tmp80 = _tmp39 + _tmp52;
  const Scalar _tmp81 = _tmp80 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(1.7965602546229);
  const Scalar _tmp83 = _tmp35 + _tmp5;
  const Scalar _tmp84 = _tmp83 + position_vector(1, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(-4.83288938413423);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp82 * _tmp86;
  const Scalar _tmp88 = _tmp68 * _tmp87;
  const Scalar _tmp89 = _tmp64 + _tmp66;
  const Scalar _tmp90 = _tmp85 * _tmp86;
  const Scalar _tmp91 = _tmp48 * _tmp87 - _tmp90;
  const Scalar _tmp92 = -_tmp48 * _tmp88 - _tmp70 * _tmp91 + _tmp89 * _tmp90;
  const Scalar _tmp93 = -_tmp77 * _tmp92 - _tmp78 * _tmp91 - _tmp87 * _tmp89 + _tmp88;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp47 * _tmp87 - _tmp60 * _tmp91 - _tmp80 * _tmp90 + _tmp83 * _tmp87;
  const Scalar _tmp96 = _tmp94 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp93 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp47 + _tmp48 * _tmp60 - _tmp79 * _tmp96);
  const Scalar _tmp100 = _tmp79 + _tmp99;
  const Scalar _tmp101 = _tmp92 * _tmp94;
  const Scalar _tmp102 = _tmp73 + _tmp83;
  const Scalar _tmp103 = _tmp102 * _tmp77;
  const Scalar _tmp104 = Scalar(1.0) / (-_tmp103 + _tmp75 - _tmp80);
  const Scalar _tmp105 = _tmp102 * _tmp104;
  const Scalar _tmp106 = -_tmp100 * _tmp101 + _tmp105 * _tmp99 + _tmp71;
  const Scalar _tmp107 = Scalar(1.0) * _tmp74;
  const Scalar _tmp108 = Scalar(1.0) * _tmp104;
  const Scalar _tmp109 = _tmp30 + Scalar(-2.5193355532036801);
  const Scalar _tmp110 = _tmp20 + Scalar(8.3885017487099702);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp109 * _tmp111;
  const Scalar _tmp113 = _tmp112 * fh1;
  const Scalar _tmp114 = fh1 * (_tmp61 + _tmp67);
  const Scalar _tmp115 = _tmp112 * _tmp114 + Scalar(5.1796800000000003) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp116 = _tmp102 * _tmp74;
  const Scalar _tmp117 = Scalar(1.0) * _tmp108 * _tmp116 - Scalar(1.0) * _tmp108;
  const Scalar _tmp118 = _tmp110 * _tmp111;
  const Scalar _tmp119 = -_tmp114 * _tmp118 - Scalar(5.1796800000000003) * _tmp15 - _tmp19 * fv1;
  const Scalar _tmp120 = _tmp103 * _tmp108 + Scalar(1.0);
  const Scalar _tmp121 = _tmp108 * _tmp77;
  const Scalar _tmp122 = -Scalar(1.0) * _tmp107 * _tmp120 + Scalar(1.0) * _tmp121;
  const Scalar _tmp123 = _tmp108 * _tmp98;
  const Scalar _tmp124 = Scalar(1.0) * _tmp97;
  const Scalar _tmp125 = _tmp102 * _tmp123 - _tmp124 * _tmp92;
  const Scalar _tmp126 = fh1 * (-_tmp112 * _tmp19 + _tmp118 * _tmp29);
  const Scalar _tmp127 = _tmp107 * _tmp70 * _tmp76 - Scalar(1.0) * _tmp78;
  const Scalar _tmp128 = _tmp98 * (-_tmp127 * _tmp96 - Scalar(1.0) * _tmp60);
  const Scalar _tmp129 = _tmp127 + _tmp128;
  const Scalar _tmp130 = -_tmp101 * _tmp129 + _tmp105 * _tmp128 - Scalar(1.0) * _tmp70;
  const Scalar _tmp131 = _tmp118 * fh1;
  const Scalar _tmp132 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp133 = _tmp72 * _tmp77 + _tmp75;
  const Scalar _tmp134 = 0;
  const Scalar _tmp135 = _tmp134 * _tmp94;
  const Scalar _tmp136 = _tmp104 * _tmp133;
  const Scalar _tmp137 = _tmp74 * (-_tmp102 * _tmp136 - _tmp135 * _tmp92 + _tmp73);
  const Scalar _tmp138 = -Scalar(1.0) * _tmp108 * _tmp133 - Scalar(1.0) * _tmp137 + Scalar(1.0);
  const Scalar _tmp139 =
      Scalar(1.0) * _tmp113 * (-_tmp106 * _tmp107 + _tmp108 * _tmp99) + _tmp115 * _tmp117 +
      _tmp119 * _tmp122 + Scalar(1.0) * _tmp126 * (-_tmp107 * _tmp125 + _tmp123) +
      Scalar(1.0) * _tmp131 * (-_tmp107 * _tmp130 + _tmp108 * _tmp128) + _tmp132 * _tmp138;
  const Scalar _tmp140 = _tmp58 * _tmp59;
  const Scalar _tmp141 = _tmp87 * _tmp94;
  const Scalar _tmp142 = _tmp91 * _tmp94;
  const Scalar _tmp143 = -_tmp100 * _tmp142 - _tmp48;
  const Scalar _tmp144 = -_tmp129 * _tmp142 + Scalar(1.0);
  const Scalar _tmp145 = _tmp134 * _tmp142;
  const Scalar _tmp146 = _tmp46 * (_tmp135 * _tmp87 - _tmp140 * _tmp145);
  const Scalar _tmp147 = -_tmp113 * _tmp46 * (_tmp100 * _tmp141 + _tmp140 * _tmp143 + Scalar(1.0)) -
                         _tmp126 * _tmp46 * (-_tmp124 * _tmp140 * _tmp91 + _tmp124 * _tmp87) -
                         _tmp131 * _tmp46 * (_tmp129 * _tmp141 + _tmp140 * _tmp144) -
                         _tmp132 * _tmp146;
  const Scalar _tmp148 = Scalar(1.0) / (_tmp147);
  const Scalar _tmp149 = std::asinh(_tmp139 * _tmp148);
  const Scalar _tmp150 = Scalar(9.6622558468725703) * _tmp147;
  const Scalar _tmp151 =
      -_tmp149 * _tmp150 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp37 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp41 - 1), Scalar(2))));
  const Scalar _tmp152 = std::pow(_tmp147, Scalar(-2));
  const Scalar _tmp153 = _tmp146 * _tmp152;
  const Scalar _tmp154 = _tmp16 + _tmp34 + _tmp5;
  const Scalar _tmp155 =
      (-_tmp139 * _tmp153 + _tmp148 * (_tmp117 * _tmp29 + _tmp122 * _tmp154 - _tmp138)) /
      std::sqrt(Scalar(std::pow(_tmp139, Scalar(2)) * _tmp152 + 1));
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp157 = Scalar(0.1034955) * _tmp148;
  const Scalar _tmp158 = _tmp151 * _tmp157;
  const Scalar _tmp159 = Scalar(1.0) * _tmp149;
  const Scalar _tmp160 = _tmp145 * _tmp59;
  const Scalar _tmp161 = _tmp124 * _tmp126;
  const Scalar _tmp162 = _tmp113 * _tmp143 * _tmp59 + _tmp131 * _tmp144 * _tmp59 -
                         _tmp132 * _tmp160 - _tmp161 * _tmp59 * _tmp91;
  const Scalar _tmp163 = Scalar(1.0) / (_tmp162);
  const Scalar _tmp164 = _tmp108 * _tmp115;
  const Scalar _tmp165 = _tmp120 * _tmp74;
  const Scalar _tmp166 = _tmp106 * _tmp113 * _tmp74 - _tmp116 * _tmp164 + _tmp119 * _tmp165 +
                         _tmp125 * _tmp126 * _tmp74 + _tmp130 * _tmp131 * _tmp74 +
                         _tmp132 * _tmp137;
  const Scalar _tmp167 = std::asinh(_tmp163 * _tmp166);
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp162;
  const Scalar _tmp169 =
      -_tmp167 * _tmp168 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp50), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp54), Scalar(2))));
  const Scalar _tmp170 = std::pow(_tmp162, Scalar(-2));
  const Scalar _tmp171 = _tmp160 * _tmp170;
  const Scalar _tmp172 = _tmp108 * _tmp29;
  const Scalar _tmp173 =
      (_tmp163 * (-_tmp116 * _tmp172 - _tmp137 + _tmp154 * _tmp165) - _tmp166 * _tmp171) /
      std::sqrt(Scalar(std::pow(_tmp166, Scalar(2)) * _tmp170 + 1));
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp160;
  const Scalar _tmp175 = Scalar(0.1034955) * _tmp163;
  const Scalar _tmp176 = _tmp169 * _tmp175;
  const Scalar _tmp177 = Scalar(1.0) * _tmp167;
  const Scalar _tmp178 = -_tmp104 * _tmp113 * _tmp99 - _tmp104 * _tmp128 * _tmp131 -
                         _tmp119 * _tmp121 - _tmp123 * _tmp126 + _tmp132 * _tmp136 + _tmp164;
  const Scalar _tmp179 =
      _tmp100 * _tmp113 * _tmp94 + _tmp129 * _tmp131 * _tmp94 + _tmp132 * _tmp135 + _tmp161;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = std::asinh(_tmp178 * _tmp180);
  const Scalar _tmp182 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp183 =
      -_tmp181 * _tmp182 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp84), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp81 - 1), Scalar(2))));
  const Scalar _tmp184 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp185 = _tmp183 * _tmp184;
  const Scalar _tmp186 = Scalar(1.0) * _tmp181;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp135;
  const Scalar _tmp188 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp189 = _tmp135 * _tmp188;
  const Scalar _tmp190 = (_tmp178 * _tmp189 + _tmp180 * (-_tmp121 * _tmp154 - _tmp136 + _tmp172)) /
                         std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp188 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp33 * std::cosh(Scalar(1.0) * _tmp31) +
       _tmp33 *
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp31 * _tmp32 -
                Scalar(8.3885017487099702) *
                    std::sqrt(Scalar(
                        Scalar(0.090199313518583735) *
                            std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp30), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp20 - 1), Scalar(2)))))));
  _res(1, 0) = _tmp150 * (-Scalar(1.0) * _tmp155 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp151 * _tmp153 +
                           _tmp157 * (-_tmp149 * _tmp156 - _tmp150 * _tmp155)) *
                              std::cosh(_tmp158)) +
               _tmp156 * (-std::sinh(_tmp158) - std::sinh(_tmp159));
  _res(2, 0) = _tmp168 * (-Scalar(1.0) * _tmp173 * std::cosh(_tmp177) -
                          (-Scalar(0.1034955) * _tmp169 * _tmp171 +
                           _tmp175 * (-_tmp167 * _tmp174 - _tmp168 * _tmp173)) *
                              std::cosh(_tmp176)) +
               _tmp174 * (-std::sinh(_tmp176) - std::sinh(_tmp177));
  _res(3, 0) = _tmp182 * (-Scalar(1.0) * _tmp190 * std::cosh(_tmp186) -
                          (Scalar(0.1034955) * _tmp183 * _tmp189 +
                           _tmp184 * (_tmp181 * _tmp187 - _tmp182 * _tmp190)) *
                              std::cosh(_tmp185)) -
               _tmp187 * (-std::sinh(_tmp185) - std::sinh(_tmp186));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
