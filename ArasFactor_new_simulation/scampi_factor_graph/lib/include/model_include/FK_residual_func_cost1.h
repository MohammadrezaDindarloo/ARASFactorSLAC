// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/pose3.h>
#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     TransformationMatrix: Pose3
 *     encoder: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost1(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const sym::Pose3<Scalar>& TransformationMatrix, const Eigen::Matrix<Scalar, 4, 1>& encoder,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar epsilon) {
  // Total ops: 487

  // Unused inputs
  (void)encoder;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 7, 1>& _TransformationMatrix = TransformationMatrix.Data();

  // Intermediate terms (158)
  const Scalar _tmp0 =
      -_DeltaRot[0] * _TransformationMatrix[1] + _DeltaRot[1] * _TransformationMatrix[0] +
      _DeltaRot[2] * _TransformationMatrix[3] + _DeltaRot[3] * _TransformationMatrix[2];
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 =
      _DeltaRot[0] * _TransformationMatrix[3] - _DeltaRot[1] * _TransformationMatrix[2] +
      _DeltaRot[2] * _TransformationMatrix[1] + _DeltaRot[3] * _TransformationMatrix[0];
  const Scalar _tmp3 = 1 - 2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 =
      _DeltaRot[0] * _TransformationMatrix[2] + _DeltaRot[1] * _TransformationMatrix[3] -
      _DeltaRot[2] * _TransformationMatrix[0] + _DeltaRot[3] * _TransformationMatrix[1];
  const Scalar _tmp7 = 2 * _tmp2 * _tmp6;
  const Scalar _tmp8 =
      -2 * _DeltaRot[0] * _TransformationMatrix[0] - 2 * _DeltaRot[1] * _TransformationMatrix[1] -
      2 * _DeltaRot[2] * _TransformationMatrix[2] + 2 * _DeltaRot[3] * _TransformationMatrix[3];
  const Scalar _tmp9 = _tmp0 * _tmp8;
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp7 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp11 = 2 * _tmp0;
  const Scalar _tmp12 = _tmp11 * _tmp6;
  const Scalar _tmp13 = _tmp2 * _tmp8;
  const Scalar _tmp14 = _tmp12 - _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp10 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _TransformationMatrix[5] + _tmp17;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp7 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp21 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp23 = _tmp11 * _tmp2;
  const Scalar _tmp24 = _tmp6 * _tmp8;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp22 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _TransformationMatrix[4] + _tmp28;
  const Scalar _tmp30 = Scalar(1.0) / (fh1);
  const Scalar _tmp31 = std::asinh(_tmp30 * fv1);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(0.1034955) * _tmp30;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 =
      -Scalar(0.010999999999999999) * _tmp21 - Scalar(0.010999999999999999) * _tmp3;
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp41 = _tmp10 + _tmp15;
  const Scalar _tmp42 = _tmp41 + _tmp5;
  const Scalar _tmp43 = _TransformationMatrix[5] + _tmp42;
  const Scalar _tmp44 = _tmp43 - p_b(1, 0);
  const Scalar _tmp45 = _tmp22 + _tmp26;
  const Scalar _tmp46 = _tmp20 + _tmp45;
  const Scalar _tmp47 = _TransformationMatrix[4] + _tmp46;
  const Scalar _tmp48 = _tmp47 - p_b(0, 0);
  const Scalar _tmp49 = Scalar(1.0) / (_tmp48);
  const Scalar _tmp50 = _tmp44 * _tmp49;
  const Scalar _tmp51 = _tmp19 + _tmp45;
  const Scalar _tmp52 = _TransformationMatrix[4] + _tmp51;
  const Scalar _tmp53 = _tmp52 - p_c(0, 0);
  const Scalar _tmp54 = _tmp4 + _tmp41;
  const Scalar _tmp55 = _TransformationMatrix[5] + _tmp54;
  const Scalar _tmp56 = _tmp55 - p_c(1, 0);
  const Scalar _tmp57 = std::pow(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp56, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp58 = _tmp53 * _tmp57;
  const Scalar _tmp59 = _tmp56 * _tmp57;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp50 * _tmp58 - _tmp59);
  const Scalar _tmp61 = _tmp58 * _tmp60;
  const Scalar _tmp62 = Scalar(1.0) * _tmp46;
  const Scalar _tmp63 = Scalar(1.0) * _tmp42;
  const Scalar _tmp64 = -_tmp63;
  const Scalar _tmp65 = Scalar(1.0) / (_tmp54 + _tmp64);
  const Scalar _tmp66 = _tmp65 * (-_tmp51 + _tmp62);
  const Scalar _tmp67 = _tmp62 + _tmp63 * _tmp66;
  const Scalar _tmp68 = 0;
  const Scalar _tmp69 = _tmp16 + _tmp4;
  const Scalar _tmp70 = _TransformationMatrix[5] + _tmp69;
  const Scalar _tmp71 = _tmp70 - p_d(1, 0);
  const Scalar _tmp72 = _tmp19 + _tmp27;
  const Scalar _tmp73 = _TransformationMatrix[4] + _tmp72;
  const Scalar _tmp74 = _tmp73 - p_d(0, 0);
  const Scalar _tmp75 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp74, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp76 = _tmp71 * _tmp75;
  const Scalar _tmp77 = _tmp74 * _tmp75;
  const Scalar _tmp78 = _tmp50 * _tmp77 - _tmp76;
  const Scalar _tmp79 = _tmp36 + _tmp37;
  const Scalar _tmp80 = _tmp35 + _tmp79;
  const Scalar _tmp81 = _tmp34 + _tmp38;
  const Scalar _tmp82 = _tmp34 + _tmp79;
  const Scalar _tmp83 = _tmp60 * (_tmp58 * _tmp81 - _tmp58 * _tmp82);
  const Scalar _tmp84 = _tmp50 * _tmp81;
  const Scalar _tmp85 = _tmp60 * (-_tmp58 * _tmp84 + _tmp59 * _tmp82);
  const Scalar _tmp86 = _tmp76 * _tmp80 - _tmp77 * _tmp84 - _tmp78 * _tmp85;
  const Scalar _tmp87 = -_tmp66 * _tmp86 - _tmp77 * _tmp80 + _tmp77 * _tmp81 - _tmp78 * _tmp83;
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = _tmp78 * _tmp88;
  const Scalar _tmp90 = _tmp68 * _tmp89;
  const Scalar _tmp91 = _tmp68 * _tmp88;
  const Scalar _tmp92 =
      std::sqrt(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp48, Scalar(2))));
  const Scalar _tmp93 = _tmp49 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) * _tmp85;
  const Scalar _tmp95 = _tmp66 * _tmp94 - Scalar(1.0) * _tmp83;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp97 = _tmp93 * (-_tmp42 * _tmp48 * _tmp96 + _tmp44 * _tmp46 * _tmp96);
  const Scalar _tmp98 = _tmp60 * (-_tmp51 * _tmp59 + _tmp54 * _tmp58 + _tmp58 * _tmp97);
  const Scalar _tmp99 = _tmp69 * _tmp77 - _tmp72 * _tmp76 + _tmp77 * _tmp97 - _tmp78 * _tmp98;
  const Scalar _tmp100 = _tmp88 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp102 = _tmp101 * _tmp87;
  const Scalar _tmp103 = _tmp102 * (-_tmp100 * _tmp95 - Scalar(1.0) * _tmp98);
  const Scalar _tmp104 = _tmp103 + _tmp95;
  const Scalar _tmp105 = -_tmp104 * _tmp89 + Scalar(1.0);
  const Scalar _tmp106 = _tmp77 * _tmp88;
  const Scalar _tmp107 = _tmp29 - p_a(0, 0);
  const Scalar _tmp108 = _tmp18 - p_a(1, 0);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp108 * _tmp109;
  const Scalar _tmp111 = _tmp110 * fh1;
  const Scalar _tmp112 = _tmp50 * _tmp85 + _tmp84;
  const Scalar _tmp113 = -_tmp112 * _tmp66 + _tmp50 * _tmp83 - _tmp81;
  const Scalar _tmp114 = _tmp102 * (-_tmp100 * _tmp113 + _tmp50 * _tmp98 - _tmp97);
  const Scalar _tmp115 = _tmp113 + _tmp114;
  const Scalar _tmp116 = -_tmp115 * _tmp89 - _tmp50;
  const Scalar _tmp117 = _tmp107 * _tmp109;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = Scalar(1.0) * _tmp101;
  const Scalar _tmp120 = fh1 * (_tmp110 * _tmp28 - _tmp117 * _tmp17);
  const Scalar _tmp121 = -_tmp111 * _tmp93 * (_tmp104 * _tmp106 + _tmp105 * _tmp61) -
                         _tmp118 * _tmp93 * (_tmp106 * _tmp115 + _tmp116 * _tmp61 + Scalar(1.0)) -
                         _tmp120 * _tmp93 * (-_tmp119 * _tmp61 * _tmp78 + _tmp119 * _tmp77) -
                         _tmp40 * _tmp93 * (-_tmp61 * _tmp90 + _tmp77 * _tmp91);
  const Scalar _tmp122 = Scalar(1.0) / (_tmp121);
  const Scalar _tmp123 = Scalar(0.1034955) * _tmp122;
  const Scalar _tmp124 = _tmp64 + _tmp69;
  const Scalar _tmp125 = _tmp124 * _tmp66;
  const Scalar _tmp126 = Scalar(1.0) / (-_tmp125 + _tmp62 - _tmp72);
  const Scalar _tmp127 = Scalar(1.0) * _tmp126;
  const Scalar _tmp128 = _tmp102 * _tmp127;
  const Scalar _tmp129 = _tmp124 * _tmp127;
  const Scalar _tmp130 = _tmp102 * _tmp129 - _tmp119 * _tmp86;
  const Scalar _tmp131 = Scalar(1.0) * _tmp65;
  const Scalar _tmp132 = _tmp86 * _tmp88;
  const Scalar _tmp133 = _tmp124 * _tmp126;
  const Scalar _tmp134 = _tmp112 + _tmp114 * _tmp133 - _tmp115 * _tmp132;
  const Scalar _tmp135 = _tmp39 * fh1;
  const Scalar _tmp136 = -_tmp110 * _tmp135 - Scalar(5.1796800000000003) * _tmp14 - _tmp17 * fv1;
  const Scalar _tmp137 = _tmp127 * _tmp66;
  const Scalar _tmp138 = _tmp125 * _tmp127 + Scalar(1.0);
  const Scalar _tmp139 = _tmp103 * _tmp133 - _tmp104 * _tmp132 - _tmp94;
  const Scalar _tmp140 = _tmp126 * _tmp67;
  const Scalar _tmp141 = -_tmp124 * _tmp140 - _tmp132 * _tmp68 + _tmp64;
  const Scalar _tmp142 = _tmp117 * _tmp135 + Scalar(5.1796800000000003) * _tmp25 + _tmp28 * fv1;
  const Scalar _tmp143 = std::asinh(
      _tmp122 * (Scalar(1.0) * _tmp111 * (_tmp103 * _tmp127 - _tmp131 * _tmp139) +
                 Scalar(1.0) * _tmp118 * (_tmp114 * _tmp127 - _tmp131 * _tmp134) +
                 Scalar(1.0) * _tmp120 * (_tmp128 - _tmp130 * _tmp131) +
                 Scalar(1.0) * _tmp136 * (-_tmp131 * _tmp138 + _tmp137) +
                 Scalar(1.0) * _tmp142 * (-_tmp127 + _tmp129 * _tmp65) +
                 Scalar(1.0) * _tmp40 * (-_tmp127 * _tmp67 - _tmp131 * _tmp141 + Scalar(1.0))));
  const Scalar _tmp144 = Scalar(9.6622558468725703) * _tmp121;
  const Scalar _tmp145 = _tmp119 * _tmp120;
  const Scalar _tmp146 = _tmp105 * _tmp111 * _tmp60 + _tmp116 * _tmp118 * _tmp60 -
                         _tmp145 * _tmp60 * _tmp78 - _tmp40 * _tmp60 * _tmp90;
  const Scalar _tmp147 = Scalar(1.0) / (_tmp146);
  const Scalar _tmp148 = _tmp127 * _tmp142;
  const Scalar _tmp149 = _tmp65 * fh1;
  const Scalar _tmp150 =
      std::asinh(_tmp147 * (_tmp110 * _tmp139 * _tmp149 + _tmp117 * _tmp134 * _tmp149 +
                            _tmp120 * _tmp130 * _tmp65 - _tmp124 * _tmp148 * _tmp65 +
                            _tmp136 * _tmp138 * _tmp65 + _tmp141 * _tmp40 * _tmp65));
  const Scalar _tmp151 = Scalar(0.1034955) * _tmp147;
  const Scalar _tmp152 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp153 =
      _tmp104 * _tmp111 * _tmp88 + _tmp115 * _tmp118 * _tmp88 + _tmp145 + _tmp40 * _tmp91;
  const Scalar _tmp154 = Scalar(1.0) / (_tmp153);
  const Scalar _tmp155 = Scalar(0.1034955) * _tmp154;
  const Scalar _tmp156 =
      std::asinh(_tmp154 * (-_tmp103 * _tmp111 * _tmp126 - _tmp114 * _tmp118 * _tmp126 -
                            _tmp120 * _tmp128 - _tmp136 * _tmp137 + _tmp140 * _tmp40 + _tmp148));
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp153;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _TransformationMatrix[6] -
      _tmp32 * (_tmp33 * p_a(2, 0) + std::cosh(Scalar(1.0) * _tmp31) -
                std::cosh(_tmp33 *
                          (-_tmp31 * _tmp32 -
                           std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_a(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp29 + p_a(0, 0)), Scalar(2))))))) +
      _tmp39;
  _res(1, 0) =
      _TransformationMatrix[6] -
      _tmp144 * (_tmp123 * p_b(2, 0) + std::cosh(Scalar(1.0) * _tmp143) -
                 std::cosh(_tmp123 *
                           (-_tmp143 * _tmp144 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp43 + p_b(1, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp47 + p_b(0, 0)), Scalar(2))))))) +
      _tmp81;
  _res(2, 0) =
      _TransformationMatrix[6] -
      _tmp152 * (_tmp151 * p_c(2, 0) + std::cosh(Scalar(1.0) * _tmp150) -
                 std::cosh(_tmp151 *
                           (-_tmp150 * _tmp152 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp52 + p_c(0, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp55 + p_c(1, 0)), Scalar(2))))))) +
      _tmp82;
  _res(3, 0) =
      _TransformationMatrix[6] -
      _tmp157 * (_tmp155 * p_d(2, 0) + std::cosh(Scalar(1.0) * _tmp156) -
                 std::cosh(_tmp155 *
                           (-_tmp156 * _tmp157 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp70 + p_d(1, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp73 + p_d(0, 0)), Scalar(2))))))) +
      _tmp80;

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
