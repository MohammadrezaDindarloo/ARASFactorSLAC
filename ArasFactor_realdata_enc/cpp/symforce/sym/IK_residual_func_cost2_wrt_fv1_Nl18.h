// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl18
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl18(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 577

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp10 = 2 * _tmp9;
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = -_DeltaRot[0] * rot_init_x - _DeltaRot[1] * rot_init_y -
                        _DeltaRot[2] * rot_init_z + _DeltaRot[3] * rot_init_w;
  const Scalar _tmp13 = 2 * _tmp12;
  const Scalar _tmp14 = _tmp13 * _tmp5;
  const Scalar _tmp15 = _tmp11 + _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp10 * _tmp5;
  const Scalar _tmp18 = _tmp13 * _tmp3;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp16 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _tmp21 + p_init0;
  const Scalar _tmp23 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp4 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp25 = 2 * _tmp3 * _tmp5;
  const Scalar _tmp26 = _tmp10 * _tmp12;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp29 = -_tmp28;
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = _tmp24 + _tmp31;
  const Scalar _tmp33 = _tmp32 + p_init1;
  const Scalar _tmp34 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp35 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp36 = _tmp20 + _tmp7;
  const Scalar _tmp37 = _tmp36 + p_init0;
  const Scalar _tmp38 = _tmp37 - p_c(0, 0);
  const Scalar _tmp39 = _tmp29 + _tmp30;
  const Scalar _tmp40 = _tmp24 + _tmp39;
  const Scalar _tmp41 = _tmp40 + p_init1;
  const Scalar _tmp42 = _tmp41 - p_c(1, 0);
  const Scalar _tmp43 = std::pow(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp42, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp44 = _tmp38 * _tmp43;
  const Scalar _tmp45 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 =
      -Scalar(0.010999999999999999) * _tmp23 - Scalar(0.010999999999999999) * _tmp6;
  const Scalar _tmp48 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp26;
  const Scalar _tmp49 = _tmp47 - _tmp48;
  const Scalar _tmp50 = _tmp46 + _tmp49;
  const Scalar _tmp51 = -_tmp24;
  const Scalar _tmp52 = _tmp39 + _tmp51;
  const Scalar _tmp53 = _tmp52 + p_init1;
  const Scalar _tmp54 = _tmp53 - p_b(1, 0);
  const Scalar _tmp55 = _tmp16 - _tmp19;
  const Scalar _tmp56 = _tmp55 + _tmp7;
  const Scalar _tmp57 = _tmp56 + p_init0;
  const Scalar _tmp58 = _tmp57 - p_b(0, 0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp58 * _tmp59;
  const Scalar _tmp61 = _tmp50 * _tmp60;
  const Scalar _tmp62 = _tmp45 + _tmp49;
  const Scalar _tmp63 = -_tmp60 * _tmp62 + _tmp61;
  const Scalar _tmp64 = _tmp31 + _tmp51;
  const Scalar _tmp65 = _tmp64 + p_init1;
  const Scalar _tmp66 = _tmp65 - p_a(1, 0);
  const Scalar _tmp67 = _tmp55 + _tmp8;
  const Scalar _tmp68 = _tmp67 + p_init0;
  const Scalar _tmp69 = _tmp68 - p_a(0, 0);
  const Scalar _tmp70 = Scalar(1.0) / (_tmp69);
  const Scalar _tmp71 = _tmp66 * _tmp70;
  const Scalar _tmp72 = _tmp42 * _tmp43;
  const Scalar _tmp73 = _tmp44 * _tmp71 - _tmp72;
  const Scalar _tmp74 = _tmp54 * _tmp59;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp60 * _tmp71 - _tmp74);
  const Scalar _tmp76 = _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp44 * _tmp50;
  const Scalar _tmp78 = _tmp47 + _tmp48;
  const Scalar _tmp79 = _tmp45 + _tmp78;
  const Scalar _tmp80 = -_tmp61 * _tmp71 + _tmp62 * _tmp74;
  const Scalar _tmp81 = -_tmp71 * _tmp77 + _tmp72 * _tmp79 - _tmp76 * _tmp80;
  const Scalar _tmp82 = Scalar(1.0) * _tmp64;
  const Scalar _tmp83 = -_tmp82;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp52 + _tmp83);
  const Scalar _tmp85 = Scalar(1.0) * _tmp67;
  const Scalar _tmp86 = -_tmp56 + _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp86;
  const Scalar _tmp88 = -_tmp44 * _tmp79 - _tmp63 * _tmp76 + _tmp77 - _tmp81 * _tmp87;
  const Scalar _tmp89 = Scalar(1.0) / (_tmp88);
  const Scalar _tmp90 = _tmp82 * _tmp87 + _tmp85;
  const Scalar _tmp91 = 0;
  const Scalar _tmp92 = _tmp89 * _tmp91;
  const Scalar _tmp93 = _tmp76 * _tmp92;
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp69, Scalar(2))));
  const Scalar _tmp95 = _tmp70 * _tmp94;
  const Scalar _tmp96 = _tmp95 * (_tmp44 * _tmp92 - _tmp60 * _tmp93);
  const Scalar _tmp97 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp98 = _tmp95 * (-_tmp64 * _tmp69 * _tmp97 + _tmp66 * _tmp67 * _tmp97);
  const Scalar _tmp99 = _tmp52 * _tmp60 - _tmp56 * _tmp74 + _tmp60 * _tmp98;
  const Scalar _tmp100 = _tmp71 * _tmp75;
  const Scalar _tmp101 = _tmp100 * _tmp80 + _tmp50 * _tmp71;
  const Scalar _tmp102 = _tmp100 * _tmp63 - _tmp101 * _tmp87 - _tmp50;
  const Scalar _tmp103 = -_tmp36 * _tmp72 + _tmp40 * _tmp44 + _tmp44 * _tmp98 - _tmp76 * _tmp99;
  const Scalar _tmp104 = _tmp103 * _tmp89;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp106 = _tmp105 * _tmp88;
  const Scalar _tmp107 = _tmp106 * (_tmp100 * _tmp99 - _tmp102 * _tmp104 - _tmp98);
  const Scalar _tmp108 = _tmp102 + _tmp107;
  const Scalar _tmp109 = _tmp44 * _tmp89;
  const Scalar _tmp110 = _tmp73 * _tmp89;
  const Scalar _tmp111 = -_tmp108 * _tmp110 - _tmp71;
  const Scalar _tmp112 = _tmp60 * _tmp75;
  const Scalar _tmp113 = _tmp33 - p_d(1, 0);
  const Scalar _tmp114 = _tmp22 - p_d(0, 0);
  const Scalar _tmp115 =
      std::pow(Scalar(std::pow(_tmp113, Scalar(2)) + std::pow(_tmp114, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp116 = _tmp114 * _tmp115;
  const Scalar _tmp117 = _tmp116 * fh1;
  const Scalar _tmp118 = Scalar(1.0) * _tmp105;
  const Scalar _tmp119 = _tmp113 * _tmp115;
  const Scalar _tmp120 = fh1 * (-_tmp116 * _tmp32 + _tmp119 * _tmp21);
  const Scalar _tmp121 = Scalar(1.0) * _tmp75;
  const Scalar _tmp122 = Scalar(1.0) * _tmp84;
  const Scalar _tmp123 = -_tmp121 * _tmp63 + _tmp122 * _tmp75 * _tmp80 * _tmp86;
  const Scalar _tmp124 = _tmp106 * (-_tmp104 * _tmp123 - _tmp121 * _tmp99);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = -_tmp110 * _tmp125 + Scalar(1.0);
  const Scalar _tmp127 = _tmp119 * fh1;
  const Scalar _tmp128 = -_tmp117 * _tmp95 * (_tmp108 * _tmp109 + _tmp111 * _tmp112 + Scalar(1.0)) -
                         _tmp120 * _tmp95 * (_tmp118 * _tmp44 - _tmp118 * _tmp60 * _tmp76) -
                         _tmp127 * _tmp95 * (_tmp109 * _tmp125 + _tmp112 * _tmp126) -
                         _tmp35 * _tmp96;
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = fh1 * (_tmp46 + _tmp78);
  const Scalar _tmp131 = -_tmp119 * _tmp130 - Scalar(5.1796800000000003) * _tmp27 - _tmp32 * fv1;
  const Scalar _tmp132 = _tmp40 + _tmp83;
  const Scalar _tmp133 = _tmp132 * _tmp87;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 - _tmp36 + _tmp85);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp84 * (_tmp133 * _tmp135 + Scalar(1.0));
  const Scalar _tmp137 = _tmp135 * _tmp87;
  const Scalar _tmp138 = -Scalar(1.0) * _tmp136 + Scalar(1.0) * _tmp137;
  const Scalar _tmp139 = _tmp116 * _tmp130 + Scalar(5.1796800000000003) * _tmp15 + _tmp21 * fv1;
  const Scalar _tmp140 = _tmp132 * _tmp84;
  const Scalar _tmp141 = Scalar(1.0) * _tmp135 * _tmp140 - Scalar(1.0) * _tmp135;
  const Scalar _tmp142 = _tmp81 * _tmp89;
  const Scalar _tmp143 = _tmp132 * _tmp134;
  const Scalar _tmp144 = _tmp101 + _tmp107 * _tmp143 - _tmp108 * _tmp142;
  const Scalar _tmp145 = _tmp106 * _tmp135;
  const Scalar _tmp146 = -_tmp118 * _tmp81 + _tmp132 * _tmp145;
  const Scalar _tmp147 = -_tmp121 * _tmp80 + _tmp124 * _tmp143 - _tmp125 * _tmp142;
  const Scalar _tmp148 = _tmp134 * _tmp90;
  const Scalar _tmp149 = _tmp84 * (-_tmp132 * _tmp148 - _tmp142 * _tmp91 + _tmp83);
  const Scalar _tmp150 = -Scalar(1.0) * _tmp135 * _tmp90 - Scalar(1.0) * _tmp149 + Scalar(1.0);
  const Scalar _tmp151 = Scalar(1.0) * _tmp117 * (_tmp107 * _tmp135 - _tmp122 * _tmp144) +
                         Scalar(1.0) * _tmp120 * (-_tmp122 * _tmp146 + _tmp145) +
                         Scalar(1.0) * _tmp127 * (-_tmp122 * _tmp147 + _tmp124 * _tmp135) +
                         _tmp131 * _tmp138 + _tmp139 * _tmp141 + _tmp150 * _tmp35;
  const Scalar _tmp152 = std::asinh(_tmp129 * _tmp151);
  const Scalar _tmp153 = Scalar(1.0) * _tmp152;
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp155 =
      -_tmp152 * _tmp154 - std::sqrt(Scalar(std::pow(Scalar(-_tmp65 + p_a(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp68 + p_a(0, 0)), Scalar(2))));
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp129;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp96;
  const Scalar _tmp159 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp160 = _tmp159 * _tmp96;
  const Scalar _tmp161 = _tmp28 + _tmp30 + _tmp51;
  const Scalar _tmp162 =
      (_tmp129 * (_tmp138 * _tmp161 + _tmp141 * _tmp21 - _tmp150) - _tmp151 * _tmp160) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp159 + 1));
  const Scalar _tmp163 = _tmp135 * _tmp139;
  const Scalar _tmp164 = _tmp117 * _tmp144 * _tmp84 + _tmp120 * _tmp146 * _tmp84 +
                         _tmp127 * _tmp147 * _tmp84 + _tmp131 * _tmp136 - _tmp140 * _tmp163 +
                         _tmp149 * _tmp35;
  const Scalar _tmp165 = _tmp118 * _tmp120;
  const Scalar _tmp166 = _tmp35 * _tmp92;
  const Scalar _tmp167 =
      _tmp111 * _tmp117 * _tmp75 + _tmp126 * _tmp127 * _tmp75 - _tmp165 * _tmp76 - _tmp166 * _tmp76;
  const Scalar _tmp168 = Scalar(1.0) / (_tmp167);
  const Scalar _tmp169 = std::asinh(_tmp164 * _tmp168);
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp167;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 - std::sqrt(Scalar(std::pow(Scalar(-_tmp53 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp57 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp168;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = std::pow(_tmp167, Scalar(-2));
  const Scalar _tmp175 = _tmp174 * _tmp93;
  const Scalar _tmp176 = _tmp135 * _tmp21;
  const Scalar _tmp177 =
      (-_tmp164 * _tmp175 + _tmp168 * (_tmp136 * _tmp161 - _tmp140 * _tmp176 - _tmp149)) /
      std::sqrt(Scalar(std::pow(_tmp164, Scalar(2)) * _tmp174 + 1));
  const Scalar _tmp178 = Scalar(9.6622558468725703) * _tmp92;
  const Scalar _tmp179 = _tmp178 * _tmp76;
  const Scalar _tmp180 = Scalar(1.0) * _tmp169;
  const Scalar _tmp181 = -_tmp107 * _tmp117 * _tmp134 - _tmp120 * _tmp145 -
                         _tmp124 * _tmp127 * _tmp134 - _tmp131 * _tmp137 + _tmp148 * _tmp35 +
                         _tmp163;
  const Scalar _tmp182 = _tmp89 * fh1;
  const Scalar _tmp183 =
      _tmp108 * _tmp116 * _tmp182 + _tmp119 * _tmp125 * _tmp182 + _tmp165 + _tmp166;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp181 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp188 = _tmp187 * _tmp92;
  const Scalar _tmp189 = (_tmp181 * _tmp188 + _tmp184 * (-_tmp137 * _tmp161 - _tmp148 + _tmp176)) /
                         std::sqrt(Scalar(std::pow(_tmp181, Scalar(2)) * _tmp187 + 1));
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp191 =
      -_tmp185 * _tmp190 - std::sqrt(Scalar(std::pow(Scalar(-_tmp37 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp41 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp193 = _tmp191 * _tmp192;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp34 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_d(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp33 + p_d(1, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp154 * (-Scalar(1.0) * _tmp162 * std::cosh(_tmp153) -
                          (-Scalar(0.1034955) * _tmp155 * _tmp160 +
                           _tmp156 * (-_tmp152 * _tmp158 - _tmp154 * _tmp162)) *
                              std::cosh(_tmp157)) +
               _tmp158 * (-std::sinh(_tmp153) - std::sinh(_tmp157));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp177 * std::cosh(_tmp180) -
                          (-Scalar(0.1034955) * _tmp171 * _tmp175 +
                           _tmp172 * (-_tmp169 * _tmp179 - _tmp170 * _tmp177)) *
                              std::cosh(_tmp173)) +
               _tmp179 * (-std::sinh(_tmp173) - std::sinh(_tmp180));
  _res(3, 0) = -_tmp178 * (-std::sinh(_tmp186) - std::sinh(_tmp193)) +
               _tmp190 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp186) -
                          (Scalar(0.1034955) * _tmp188 * _tmp191 +
                           _tmp192 * (_tmp178 * _tmp185 - _tmp189 * _tmp190)) *
                              std::cosh(_tmp193));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
