// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl21
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl21(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 580

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = 2 * _tmp4;
  const Scalar _tmp6 = _tmp3 * _tmp5;
  const Scalar _tmp7 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp8 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp9 = _tmp7 * _tmp8;
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp6 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp11 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp12 = 1 - 2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp15 = _tmp4 * _tmp8;
  const Scalar _tmp16 = _tmp14 + _tmp15;
  const Scalar _tmp17 = -Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp13 + _tmp17;
  const Scalar _tmp19 = _tmp10 + _tmp18;
  const Scalar _tmp20 = _tmp19 + p_init0;
  const Scalar _tmp21 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp11 +
                        Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999);
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp6 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp24 = _tmp5 * _tmp7;
  const Scalar _tmp25 = _tmp3 * _tmp8;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = -_tmp23 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + p_init1;
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = -_tmp10;
  const Scalar _tmp34 = _tmp13 + _tmp17;
  const Scalar _tmp35 = _tmp33 + _tmp34;
  const Scalar _tmp36 = _tmp35 + p_init0;
  const Scalar _tmp37 = -_tmp22;
  const Scalar _tmp38 = _tmp23 + _tmp37;
  const Scalar _tmp39 = _tmp28 + _tmp38;
  const Scalar _tmp40 = _tmp39 + p_init1;
  const Scalar _tmp41 = _tmp18 + _tmp33;
  const Scalar _tmp42 = _tmp41 + p_init0;
  const Scalar _tmp43 = _tmp42 - p_a(0, 0);
  const Scalar _tmp44 = _tmp29 + _tmp37;
  const Scalar _tmp45 = _tmp44 + p_init1;
  const Scalar _tmp46 = _tmp45 - p_a(1, 0);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp43 * _tmp47;
  const Scalar _tmp49 = _tmp10 + _tmp34;
  const Scalar _tmp50 = _tmp49 + p_init0;
  const Scalar _tmp51 = _tmp50 - p_c(0, 0);
  const Scalar _tmp52 = _tmp22 + _tmp23 + _tmp28;
  const Scalar _tmp53 = _tmp52 + p_init1;
  const Scalar _tmp54 = _tmp53 - p_c(1, 0);
  const Scalar _tmp55 = std::pow(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp54, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp56 = _tmp54 * _tmp55;
  const Scalar _tmp57 = _tmp51 * _tmp55;
  const Scalar _tmp58 = _tmp40 - p_b(1, 0);
  const Scalar _tmp59 = _tmp36 - p_b(0, 0);
  const Scalar _tmp60 =
      std::sqrt(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp59, Scalar(2))));
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = Scalar(1.0) / (_tmp59);
  const Scalar _tmp63 = _tmp60 * _tmp62;
  const Scalar _tmp64 = _tmp63 * (_tmp35 * _tmp58 * _tmp61 - _tmp39 * _tmp59 * _tmp61);
  const Scalar _tmp65 = -_tmp49 * _tmp56 + _tmp52 * _tmp57 + _tmp57 * _tmp64;
  const Scalar _tmp66 = _tmp58 * _tmp62;
  const Scalar _tmp67 = Scalar(1.0) / (-_tmp56 + _tmp57 * _tmp66);
  const Scalar _tmp68 = _tmp46 * _tmp47;
  const Scalar _tmp69 = _tmp48 * _tmp66 - _tmp68;
  const Scalar _tmp70 = _tmp67 * _tmp69;
  const Scalar _tmp71 = -_tmp41 * _tmp68 + _tmp44 * _tmp48 + _tmp48 * _tmp64 - _tmp65 * _tmp70;
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = Scalar(1.0) * _tmp72;
  const Scalar _tmp74 = Scalar(1.0) * _tmp67;
  const Scalar _tmp75 = _tmp69 * _tmp72 * _tmp74;
  const Scalar _tmp76 = _tmp31 - p_d(1, 0);
  const Scalar _tmp77 = _tmp20 - p_d(0, 0);
  const Scalar _tmp78 = std::pow(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp77, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp79 = _tmp77 * _tmp78;
  const Scalar _tmp80 = _tmp76 * _tmp78;
  const Scalar _tmp81 = fh1 * (_tmp19 * _tmp80 - _tmp30 * _tmp79);
  const Scalar _tmp82 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp83 =
      -Scalar(0.010999999999999999) * _tmp12 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp84 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp85 = _tmp83 - _tmp84;
  const Scalar _tmp86 = _tmp82 + _tmp85;
  const Scalar _tmp87 = _tmp57 * _tmp86;
  const Scalar _tmp88 = _tmp83 + _tmp84;
  const Scalar _tmp89 = _tmp82 + _tmp88;
  const Scalar _tmp90 = -_tmp57 * _tmp89 + _tmp87;
  const Scalar _tmp91 = _tmp56 * _tmp89 - _tmp66 * _tmp87;
  const Scalar _tmp92 = _tmp74 * _tmp91;
  const Scalar _tmp93 = Scalar(1.0) * _tmp39;
  const Scalar _tmp94 = -_tmp93;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp52 + _tmp94);
  const Scalar _tmp96 = Scalar(1.0) * _tmp35;
  const Scalar _tmp97 = _tmp95 * (-_tmp49 + _tmp96);
  const Scalar _tmp98 = -_tmp74 * _tmp90 + _tmp92 * _tmp97;
  const Scalar _tmp99 = -_tmp82;
  const Scalar _tmp100 = _tmp85 + _tmp99;
  const Scalar _tmp101 = _tmp48 * _tmp86;
  const Scalar _tmp102 = _tmp100 * _tmp68 - _tmp101 * _tmp66 - _tmp70 * _tmp91;
  const Scalar _tmp103 = -_tmp100 * _tmp48 + _tmp101 - _tmp102 * _tmp97 - _tmp70 * _tmp90;
  const Scalar _tmp104 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp105 = _tmp104 * _tmp71;
  const Scalar _tmp106 = _tmp103 * _tmp72;
  const Scalar _tmp107 = _tmp106 * (-_tmp105 * _tmp98 - _tmp65 * _tmp74);
  const Scalar _tmp108 = _tmp107 + _tmp98;
  const Scalar _tmp109 = _tmp104 * _tmp48;
  const Scalar _tmp110 = _tmp104 * _tmp69;
  const Scalar _tmp111 = -_tmp108 * _tmp110 + Scalar(1.0);
  const Scalar _tmp112 = _tmp57 * _tmp67;
  const Scalar _tmp113 = _tmp80 * fh1;
  const Scalar _tmp114 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp115 = _tmp93 * _tmp97 + _tmp96;
  const Scalar _tmp116 = 0;
  const Scalar _tmp117 = _tmp110 * _tmp116;
  const Scalar _tmp118 = _tmp104 * _tmp116;
  const Scalar _tmp119 = _tmp63 * (-_tmp112 * _tmp117 + _tmp118 * _tmp48);
  const Scalar _tmp120 = _tmp66 * _tmp67;
  const Scalar _tmp121 = _tmp120 * _tmp91 + _tmp66 * _tmp86;
  const Scalar _tmp122 = _tmp120 * _tmp90 - _tmp121 * _tmp97 - _tmp86;
  const Scalar _tmp123 = _tmp106 * (-_tmp105 * _tmp122 + _tmp120 * _tmp65 - _tmp64);
  const Scalar _tmp124 = _tmp122 + _tmp123;
  const Scalar _tmp125 = -_tmp110 * _tmp124 - _tmp66;
  const Scalar _tmp126 = _tmp79 * fh1;
  const Scalar _tmp127 = -_tmp113 * _tmp63 * (_tmp108 * _tmp109 + _tmp111 * _tmp112) -
                         _tmp114 * _tmp119 -
                         _tmp126 * _tmp63 * (_tmp109 * _tmp124 + _tmp112 * _tmp125 + Scalar(1.0)) -
                         _tmp63 * _tmp81 * (_tmp48 * _tmp73 - _tmp57 * _tmp75);
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = _tmp44 + _tmp94;
  const Scalar _tmp130 = _tmp129 * _tmp97;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp41 + _tmp96);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp102 * _tmp104;
  const Scalar _tmp134 = _tmp129 * _tmp131;
  const Scalar _tmp135 = _tmp121 + _tmp123 * _tmp134 - _tmp124 * _tmp133;
  const Scalar _tmp136 = Scalar(1.0) * _tmp95;
  const Scalar _tmp137 = _tmp115 * _tmp131;
  const Scalar _tmp138 = _tmp95 * (-_tmp116 * _tmp133 - _tmp129 * _tmp137 + _tmp94);
  const Scalar _tmp139 = -Scalar(1.0) * _tmp115 * _tmp132 - Scalar(1.0) * _tmp138 + Scalar(1.0);
  const Scalar _tmp140 = _tmp107 * _tmp134 - _tmp108 * _tmp133 - _tmp92;
  const Scalar _tmp141 = fh1 * (_tmp88 + _tmp99);
  const Scalar _tmp142 = _tmp141 * _tmp79 + Scalar(5.1796800000000003) * _tmp16 + _tmp19 * fv1;
  const Scalar _tmp143 = _tmp129 * _tmp132;
  const Scalar _tmp144 = -Scalar(1.0) * _tmp132 + Scalar(1.0) * _tmp143 * _tmp95;
  const Scalar _tmp145 = -_tmp141 * _tmp80 - Scalar(5.1796800000000003) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp146 = _tmp132 * _tmp97;
  const Scalar _tmp147 = _tmp95 * (_tmp130 * _tmp132 + Scalar(1.0));
  const Scalar _tmp148 = Scalar(1.0) * _tmp146 - Scalar(1.0) * _tmp147;
  const Scalar _tmp149 = -_tmp102 * _tmp73 + _tmp106 * _tmp143;
  const Scalar _tmp150 = _tmp106 * _tmp132;
  const Scalar _tmp151 =
      Scalar(1.0) * _tmp113 * (_tmp107 * _tmp132 - _tmp136 * _tmp140) + _tmp114 * _tmp139 +
      Scalar(1.0) * _tmp126 * (_tmp123 * _tmp132 - _tmp135 * _tmp136) + _tmp142 * _tmp144 +
      _tmp145 * _tmp148 + Scalar(1.0) * _tmp81 * (-_tmp136 * _tmp149 + _tmp150);
  const Scalar _tmp152 = std::asinh(_tmp128 * _tmp151);
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp154 =
      -_tmp152 * _tmp153 - std::sqrt(Scalar(std::pow(Scalar(-_tmp36 + p_b(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp40 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp155 = Scalar(0.1034955) * _tmp128;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = Scalar(1.0) * _tmp152;
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp119;
  const Scalar _tmp159 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp160 = _tmp119 * _tmp159;
  const Scalar _tmp161 = _tmp27 + _tmp38;
  const Scalar _tmp162 =
      (_tmp128 * (-_tmp139 + _tmp144 * _tmp19 + _tmp148 * _tmp161) - _tmp151 * _tmp160) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp159 + 1));
  const Scalar _tmp163 = _tmp117 * _tmp67;
  const Scalar _tmp164 =
      _tmp111 * _tmp113 * _tmp67 - _tmp114 * _tmp163 + _tmp125 * _tmp126 * _tmp67 - _tmp75 * _tmp81;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp132 * _tmp142;
  const Scalar _tmp167 = _tmp129 * _tmp95;
  const Scalar _tmp168 = _tmp113 * _tmp140 * _tmp95 + _tmp114 * _tmp138 +
                         _tmp126 * _tmp135 * _tmp95 + _tmp145 * _tmp147 +
                         _tmp149 * _tmp81 * _tmp95 - _tmp166 * _tmp167;
  const Scalar _tmp169 = std::asinh(_tmp165 * _tmp168);
  const Scalar _tmp170 = Scalar(1.0) * _tmp169;
  const Scalar _tmp171 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp172 =
      -_tmp169 * _tmp171 - std::sqrt(Scalar(std::pow(Scalar(-_tmp50 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp53 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp173 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp174 = _tmp172 * _tmp173;
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp176 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp177 = _tmp132 * _tmp19;
  const Scalar _tmp178 = _tmp163 * _tmp176;
  const Scalar _tmp179 =
      (_tmp165 * (-_tmp138 + _tmp147 * _tmp161 - _tmp167 * _tmp177) - _tmp168 * _tmp178) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp176 + 1));
  const Scalar _tmp180 = -_tmp107 * _tmp113 * _tmp131 + _tmp114 * _tmp137 -
                         _tmp123 * _tmp126 * _tmp131 - _tmp145 * _tmp146 - _tmp150 * _tmp81 +
                         _tmp166;
  const Scalar _tmp181 = _tmp104 * _tmp108 * _tmp113 + _tmp104 * _tmp124 * _tmp126 +
                         _tmp114 * _tmp118 + _tmp73 * _tmp81;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = std::asinh(_tmp180 * _tmp182);
  const Scalar _tmp184 = Scalar(1.0) * _tmp183;
  const Scalar _tmp185 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp186 = _tmp118 * _tmp185;
  const Scalar _tmp187 = (_tmp180 * _tmp186 + _tmp182 * (-_tmp137 - _tmp146 * _tmp161 + _tmp177)) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp185 + 1));
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp189 =
      -_tmp183 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp45 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp118;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp32 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_d(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp31 + p_d(1, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-Scalar(1.0) * _tmp162 * std::cosh(_tmp157) -
                          (-Scalar(0.1034955) * _tmp154 * _tmp160 +
                           _tmp155 * (-_tmp152 * _tmp158 - _tmp153 * _tmp162)) *
                              std::cosh(_tmp156)) +
               _tmp158 * (-std::sinh(_tmp156) - std::sinh(_tmp157));
  _res(2, 0) = _tmp171 * (-Scalar(1.0) * _tmp179 * std::cosh(_tmp170) -
                          (-Scalar(0.1034955) * _tmp172 * _tmp178 +
                           _tmp173 * (-_tmp169 * _tmp175 - _tmp171 * _tmp179)) *
                              std::cosh(_tmp174)) +
               _tmp175 * (-std::sinh(_tmp170) - std::sinh(_tmp174));
  _res(3, 0) = _tmp188 * (-Scalar(1.0) * _tmp187 * std::cosh(_tmp184) -
                          (Scalar(0.1034955) * _tmp186 * _tmp189 +
                           _tmp190 * (_tmp183 * _tmp192 - _tmp187 * _tmp188)) *
                              std::cosh(_tmp191)) -
               _tmp192 * (-std::sinh(_tmp184) - std::sinh(_tmp191));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
