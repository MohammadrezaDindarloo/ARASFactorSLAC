// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl3
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl3(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 637

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (219)
  const Scalar _tmp0 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp1 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp2 = Scalar(1.0) / (fh1);
  const Scalar _tmp3 = _tmp2 * fv1;
  const Scalar _tmp4 = std::asinh(_tmp3);
  const Scalar _tmp5 = Scalar(9.6622558468725703) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp7 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp9 = -2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp7 + Scalar(0.20999999999999999) * _tmp9 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp11 = -_tmp10;
  const Scalar _tmp12 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                        _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp13 = 2 * _tmp8;
  const Scalar _tmp14 = _tmp12 * _tmp13;
  const Scalar _tmp15 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp16 = _tmp15 * _tmp6;
  const Scalar _tmp17 = _tmp14 + _tmp16;
  const Scalar _tmp18 = -Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = 2 * _tmp12 * _tmp6;
  const Scalar _tmp20 = _tmp15 * _tmp8;
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp19 - Scalar(0.20999999999999999) * _tmp20;
  const Scalar _tmp22 = _tmp18 - _tmp21;
  const Scalar _tmp23 = _tmp11 + _tmp22;
  const Scalar _tmp24 = _tmp23 + p_init0;
  const Scalar _tmp25 = 1 - 2 * std::pow(_tmp12, Scalar(2));
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp27 = -_tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp20;
  const Scalar _tmp29 = _tmp13 * _tmp6;
  const Scalar _tmp30 = _tmp12 * _tmp15;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = -Scalar(0.010999999999999999) * _tmp31;
  const Scalar _tmp33 = -_tmp28 + _tmp32;
  const Scalar _tmp34 = _tmp27 + _tmp33;
  const Scalar _tmp35 = _tmp34 + p_init1;
  const Scalar _tmp36 =
      -_tmp5 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp24 + p_a(0, 0)), Scalar(2)) +
                                      std::pow(Scalar(-_tmp35 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp37 = Scalar(0.1034955) * _tmp2;
  const Scalar _tmp38 = _tmp36 * _tmp37;
  const Scalar _tmp39 =
      std::pow(Scalar(_tmp0 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = Scalar(1.0) * _tmp4;
  const Scalar _tmp41 = _tmp24 - p_a(0, 0);
  const Scalar _tmp42 = _tmp35 - p_a(1, 0);
  const Scalar _tmp43 = std::pow(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp42, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp44 = _tmp42 * _tmp43;
  const Scalar _tmp45 = _tmp41 * _tmp43;
  const Scalar _tmp46 = _tmp23 * _tmp44 - _tmp34 * _tmp45;
  const Scalar _tmp47 = _tmp28 + _tmp32;
  const Scalar _tmp48 = _tmp27 + _tmp47;
  const Scalar _tmp49 = _tmp48 + p_init1;
  const Scalar _tmp50 = _tmp49 - p_b(1, 0);
  const Scalar _tmp51 = _tmp10 + _tmp22;
  const Scalar _tmp52 = _tmp51 + p_init0;
  const Scalar _tmp53 = _tmp52 - p_b(0, 0);
  const Scalar _tmp54 = std::pow(Scalar(std::pow(_tmp50, Scalar(2)) + std::pow(_tmp53, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp55 = _tmp53 * _tmp54;
  const Scalar _tmp56 = _tmp50 * _tmp54;
  const Scalar _tmp57 = _tmp18 + _tmp21;
  const Scalar _tmp58 = _tmp10 + _tmp57;
  const Scalar _tmp59 = _tmp58 + p_init0;
  const Scalar _tmp60 = _tmp59 - p_c(0, 0);
  const Scalar _tmp61 = _tmp26 + _tmp47;
  const Scalar _tmp62 = _tmp61 + p_init1;
  const Scalar _tmp63 = _tmp62 - p_c(1, 0);
  const Scalar _tmp64 =
      std::sqrt(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp63, Scalar(2))));
  const Scalar _tmp65 = Scalar(1.0) / (_tmp64);
  const Scalar _tmp66 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp67 = _tmp64 * _tmp66;
  const Scalar _tmp68 = _tmp67 * (_tmp58 * _tmp63 * _tmp65 - _tmp60 * _tmp61 * _tmp65);
  const Scalar _tmp69 = _tmp26 + _tmp33;
  const Scalar _tmp70 = _tmp69 + p_init1;
  const Scalar _tmp71 = _tmp70 - p_d(1, 0);
  const Scalar _tmp72 = _tmp11 + _tmp57;
  const Scalar _tmp73 = _tmp72 + p_init0;
  const Scalar _tmp74 = _tmp73 - p_d(0, 0);
  const Scalar _tmp75 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp74, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp71 * _tmp75;
  const Scalar _tmp78 = _tmp68 * _tmp76 + _tmp69 * _tmp76 - _tmp72 * _tmp77;
  const Scalar _tmp79 = _tmp63 * _tmp66;
  const Scalar _tmp80 = _tmp55 * _tmp79 - _tmp56;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp76 * _tmp79 - _tmp77);
  const Scalar _tmp82 = _tmp80 * _tmp81;
  const Scalar _tmp83 = _tmp48 * _tmp55 - _tmp51 * _tmp56 + _tmp55 * _tmp68 - _tmp78 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) / (_tmp83);
  const Scalar _tmp85 = Scalar(1.0) * _tmp84;
  const Scalar _tmp86 = _tmp46 * _tmp67 * (_tmp55 * _tmp85 - _tmp76 * _tmp82 * _tmp85);
  const Scalar _tmp87 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp88 =
      -Scalar(0.010999999999999999) * _tmp25 - Scalar(0.010999999999999999) * _tmp7;
  const Scalar _tmp89 = Scalar(0.20999999999999999) * _tmp29 + Scalar(0.20999999999999999) * _tmp30;
  const Scalar _tmp90 = _tmp88 + _tmp89;
  const Scalar _tmp91 = _tmp87 + _tmp90;
  const Scalar _tmp92 = _tmp55 * _tmp91;
  const Scalar _tmp93 = _tmp88 - _tmp89;
  const Scalar _tmp94 = _tmp87 + _tmp93;
  const Scalar _tmp95 = _tmp76 * _tmp91;
  const Scalar _tmp96 = -_tmp87;
  const Scalar _tmp97 = _tmp90 + _tmp96;
  const Scalar _tmp98 = -_tmp76 * _tmp97 + _tmp95;
  const Scalar _tmp99 = _tmp77 * _tmp97 - _tmp79 * _tmp95;
  const Scalar _tmp100 = _tmp56 * _tmp94 - _tmp79 * _tmp92 - _tmp82 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) * _tmp61;
  const Scalar _tmp102 = -_tmp101;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp102 + _tmp69);
  const Scalar _tmp104 = Scalar(1.0) * _tmp58;
  const Scalar _tmp105 = _tmp103 * (_tmp104 - _tmp72);
  const Scalar _tmp106 = -_tmp100 * _tmp105 - _tmp55 * _tmp94 - _tmp82 * _tmp98 + _tmp92;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp79 * _tmp81;
  const Scalar _tmp109 = _tmp108 * _tmp99 + _tmp79 * _tmp91;
  const Scalar _tmp110 = -_tmp105 * _tmp109 + _tmp108 * _tmp98 - _tmp91;
  const Scalar _tmp111 = _tmp107 * _tmp83;
  const Scalar _tmp112 = _tmp106 * _tmp84;
  const Scalar _tmp113 = _tmp112 * (_tmp108 * _tmp78 - _tmp110 * _tmp111 - _tmp68);
  const Scalar _tmp114 = _tmp110 + _tmp113;
  const Scalar _tmp115 = _tmp107 * _tmp114;
  const Scalar _tmp116 = _tmp107 * _tmp80;
  const Scalar _tmp117 = _tmp81 * (-_tmp114 * _tmp116 - _tmp79);
  const Scalar _tmp118 = _tmp45 * _tmp67 * (_tmp115 * _tmp55 + _tmp117 * _tmp76 + Scalar(1.0));
  const Scalar _tmp119 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp120 = _tmp101 * _tmp105 + _tmp104;
  const Scalar _tmp121 = 0;
  const Scalar _tmp122 = _tmp116 * _tmp121 * _tmp81;
  const Scalar _tmp123 = _tmp107 * _tmp121;
  const Scalar _tmp124 = Scalar(1.0) * _tmp81;
  const Scalar _tmp125 = _tmp124 * _tmp99;
  const Scalar _tmp126 = _tmp105 * _tmp125 - _tmp124 * _tmp98;
  const Scalar _tmp127 = _tmp112 * (-_tmp111 * _tmp126 - _tmp124 * _tmp78);
  const Scalar _tmp128 = _tmp126 + _tmp127;
  const Scalar _tmp129 = _tmp107 * _tmp128;
  const Scalar _tmp130 = _tmp81 * (-_tmp116 * _tmp128 + Scalar(1.0));
  const Scalar _tmp131 = _tmp44 * _tmp67 * (_tmp129 * _tmp55 + _tmp130 * _tmp76);
  const Scalar _tmp132 = -_tmp118 * fh1 -
                         _tmp119 * _tmp67 * (-_tmp122 * _tmp76 + _tmp123 * _tmp55) - _tmp131 * fh1 -
                         _tmp86 * fh1;
  const Scalar _tmp133 = Scalar(1.0) / (_tmp132);
  const Scalar _tmp134 = _tmp102 + _tmp48;
  const Scalar _tmp135 = _tmp105 * _tmp134;
  const Scalar _tmp136 = Scalar(1.0) / (_tmp104 - _tmp135 - _tmp51);
  const Scalar _tmp137 = Scalar(1.0) * _tmp136;
  const Scalar _tmp138 = _tmp100 * _tmp107;
  const Scalar _tmp139 = _tmp120 * _tmp136;
  const Scalar _tmp140 = _tmp102 - _tmp121 * _tmp138 - _tmp134 * _tmp139;
  const Scalar _tmp141 = Scalar(1.0) * _tmp103;
  const Scalar _tmp142 = _tmp93 + _tmp96;
  const Scalar _tmp143 = _tmp142 * fh1;
  const Scalar _tmp144 = _tmp143 * _tmp45 + Scalar(5.1796800000000003) * _tmp17 + _tmp23 * fv1;
  const Scalar _tmp145 = _tmp134 * _tmp137;
  const Scalar _tmp146 = _tmp103 * _tmp145;
  const Scalar _tmp147 = -Scalar(1.0) * _tmp137 + Scalar(1.0) * _tmp146;
  const Scalar _tmp148 = -_tmp143 * _tmp44 - Scalar(5.1796800000000003) * _tmp31 - _tmp34 * fv1;
  const Scalar _tmp149 = _tmp135 * _tmp137 + Scalar(1.0);
  const Scalar _tmp150 = _tmp105 * _tmp137;
  const Scalar _tmp151 = -Scalar(1.0) * _tmp141 * _tmp149 + Scalar(1.0) * _tmp150;
  const Scalar _tmp152 = _tmp134 * _tmp136;
  const Scalar _tmp153 = -_tmp125 + _tmp127 * _tmp152 - _tmp128 * _tmp138;
  const Scalar _tmp154 = Scalar(1.0) * _tmp44 * (_tmp127 * _tmp137 - _tmp141 * _tmp153);
  const Scalar _tmp155 = _tmp109 + _tmp113 * _tmp152 - _tmp114 * _tmp138;
  const Scalar _tmp156 = Scalar(1.0) * _tmp45 * (_tmp113 * _tmp137 - _tmp141 * _tmp155);
  const Scalar _tmp157 = _tmp103 * (-_tmp100 * _tmp85 + _tmp112 * _tmp145);
  const Scalar _tmp158 = _tmp112 * _tmp137;
  const Scalar _tmp159 = Scalar(1.0) * _tmp46 * (-Scalar(1.0) * _tmp157 + _tmp158);
  const Scalar _tmp160 =
      Scalar(1.0) * _tmp119 * (-_tmp120 * _tmp137 - _tmp140 * _tmp141 + Scalar(1.0)) +
      _tmp144 * _tmp147 + _tmp148 * _tmp151 + _tmp154 * fh1 + _tmp156 * fh1 + _tmp159 * fh1;
  const Scalar _tmp161 = std::asinh(_tmp133 * _tmp160);
  const Scalar _tmp162 = Scalar(1.0) * _tmp161;
  const Scalar _tmp163 = Scalar(9.6622558468725703) * _tmp132;
  const Scalar _tmp164 =
      -_tmp161 * _tmp163 - std::sqrt(Scalar(std::pow(Scalar(-_tmp59 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp62 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp165 = Scalar(0.1034955) * _tmp133;
  const Scalar _tmp166 = _tmp164 * _tmp165;
  const Scalar _tmp167 = -_tmp118 - _tmp131 - _tmp86;
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp167;
  const Scalar _tmp169 = std::pow(_tmp132, Scalar(-2));
  const Scalar _tmp170 = _tmp167 * _tmp169;
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp170;
  const Scalar _tmp172 = _tmp142 * _tmp45;
  const Scalar _tmp173 = _tmp142 * _tmp44;
  const Scalar _tmp174 =
      (_tmp133 * (_tmp147 * _tmp172 - _tmp151 * _tmp173 + _tmp154 + _tmp156 + _tmp159) -
       _tmp160 * _tmp170) /
      std::sqrt(Scalar(std::pow(_tmp160, Scalar(2)) * _tmp169 + 1));
  const Scalar _tmp175 = _tmp46 * _tmp85;
  const Scalar _tmp176 = _tmp175 * fh1;
  const Scalar _tmp177 = _tmp130 * _tmp44;
  const Scalar _tmp178 = _tmp117 * _tmp45;
  const Scalar _tmp179 = -_tmp119 * _tmp122 - _tmp176 * _tmp82 + _tmp177 * fh1 + _tmp178 * fh1;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = _tmp103 * _tmp153 * _tmp44;
  const Scalar _tmp182 = _tmp137 * _tmp144;
  const Scalar _tmp183 = _tmp157 * _tmp46;
  const Scalar _tmp184 = _tmp103 * _tmp149;
  const Scalar _tmp185 = _tmp103 * _tmp155 * _tmp45;
  const Scalar _tmp186 = _tmp103 * _tmp119 * _tmp140 - _tmp103 * _tmp134 * _tmp182 +
                         _tmp148 * _tmp184 + _tmp181 * fh1 + _tmp183 * fh1 + _tmp185 * fh1;
  const Scalar _tmp187 = std::asinh(_tmp180 * _tmp186);
  const Scalar _tmp188 = Scalar(1.0) * _tmp187;
  const Scalar _tmp189 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp190 = -_tmp175 * _tmp82 + _tmp177 + _tmp178;
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 =
      (_tmp180 * (-_tmp146 * _tmp172 - _tmp173 * _tmp184 + _tmp181 + _tmp183 + _tmp185) -
       _tmp186 * _tmp191) /
      std::sqrt(Scalar(std::pow(_tmp186, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp193 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp194 =
      -_tmp187 * _tmp193 - std::sqrt(Scalar(std::pow(Scalar(-_tmp70 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp73 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp195 = Scalar(0.1034955) * _tmp191;
  const Scalar _tmp196 = Scalar(9.6622558468725703) * _tmp190;
  const Scalar _tmp197 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp198 = _tmp194 * _tmp197;
  const Scalar _tmp199 = _tmp115 * _tmp45;
  const Scalar _tmp200 = _tmp129 * _tmp44;
  const Scalar _tmp201 = _tmp119 * _tmp123 + _tmp176 + _tmp199 * fh1 + _tmp200 * fh1;
  const Scalar _tmp202 = Scalar(1.0) / (_tmp201);
  const Scalar _tmp203 = _tmp127 * _tmp136 * _tmp44;
  const Scalar _tmp204 = _tmp158 * _tmp46;
  const Scalar _tmp205 = _tmp113 * _tmp136 * _tmp45;
  const Scalar _tmp206 = _tmp119 * _tmp139 - _tmp148 * _tmp150 + _tmp182 - _tmp203 * fh1 -
                         _tmp204 * fh1 - _tmp205 * fh1;
  const Scalar _tmp207 = std::asinh(_tmp202 * _tmp206);
  const Scalar _tmp208 = Scalar(1.0) * _tmp207;
  const Scalar _tmp209 = std::pow(_tmp201, Scalar(-2));
  const Scalar _tmp210 = _tmp175 + _tmp199 + _tmp200;
  const Scalar _tmp211 = _tmp209 * _tmp210;
  const Scalar _tmp212 =
      (_tmp202 * (_tmp137 * _tmp172 + _tmp150 * _tmp173 - _tmp203 - _tmp204 - _tmp205) -
       _tmp206 * _tmp211) /
      std::sqrt(Scalar(std::pow(_tmp206, Scalar(2)) * _tmp209 + 1));
  const Scalar _tmp213 = Scalar(0.1034955) * _tmp211;
  const Scalar _tmp214 = Scalar(9.6622558468725703) * _tmp201;
  const Scalar _tmp215 =
      -_tmp207 * _tmp214 - std::sqrt(Scalar(std::pow(Scalar(-_tmp49 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp52 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp216 = Scalar(0.1034955) * _tmp202;
  const Scalar _tmp217 = _tmp215 * _tmp216;
  const Scalar _tmp218 = Scalar(9.6622558468725703) * _tmp210;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(9.6622558468725703) * _tmp37 * p_a(2, 0) -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp0 * _tmp39 * fv1 * std::sinh(_tmp40) - _tmp1 * p_a(2, 0) -
           (-_tmp1 * _tmp36 + _tmp37 * (Scalar(9.6622558468725703) * _tmp3 * _tmp39 - _tmp5)) *
               std::sinh(_tmp38)) +
      Scalar(9.6622558468725703) * std::cosh(_tmp38) -
      Scalar(9.6622558468725703) * std::cosh(_tmp40);
  _res(1, 0) =
      -_tmp163 * (-_tmp171 * p_c(2, 0) + Scalar(1.0) * _tmp174 * std::sinh(_tmp162) -
                  (-_tmp164 * _tmp171 + _tmp165 * (-_tmp161 * _tmp168 - _tmp163 * _tmp174)) *
                      std::sinh(_tmp166)) -
      _tmp168 * (_tmp165 * p_c(2, 0) + std::cosh(_tmp162) - std::cosh(_tmp166));
  _res(2, 0) =
      -_tmp193 * (Scalar(1.0) * _tmp192 * std::sinh(_tmp188) - _tmp195 * p_d(2, 0) -
                  (-_tmp194 * _tmp195 + _tmp197 * (-_tmp187 * _tmp196 - _tmp192 * _tmp193)) *
                      std::sinh(_tmp198)) -
      _tmp196 * (_tmp197 * p_d(2, 0) + std::cosh(_tmp188) - std::cosh(_tmp198));
  _res(3, 0) =
      -_tmp214 * (Scalar(1.0) * _tmp212 * std::sinh(_tmp208) - _tmp213 * p_b(2, 0) -
                  (-_tmp213 * _tmp215 + _tmp216 * (-_tmp207 * _tmp218 - _tmp212 * _tmp214)) *
                      std::sinh(_tmp217)) -
      _tmp218 * (_tmp216 * p_b(2, 0) + std::cosh(_tmp208) - std::cosh(_tmp217));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
