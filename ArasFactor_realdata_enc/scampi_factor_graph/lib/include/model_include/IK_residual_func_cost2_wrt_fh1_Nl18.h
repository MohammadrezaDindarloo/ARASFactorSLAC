// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl18
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl18(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 624

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp2 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp3 = 2 * _tmp2;
  const Scalar _tmp4 = _tmp1 * _tmp3;
  const Scalar _tmp5 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp6 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp7 = _tmp5 * _tmp6;
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp4 - Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp10 = 1 - 2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp11 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp12 = 2 * _tmp1 * _tmp5;
  const Scalar _tmp13 = _tmp2 * _tmp6;
  const Scalar _tmp14 = _tmp12 + _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp11 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp8;
  const Scalar _tmp18 = _tmp17 + p_init0;
  const Scalar _tmp19 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp9 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp22 = _tmp3 * _tmp5;
  const Scalar _tmp23 = _tmp1 * _tmp6;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = -_tmp21 + _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + p_init1;
  const Scalar _tmp29 = _tmp0 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -Scalar(0.1034955) * _tmp31 * fh1 -
      Scalar(0.1034955) * std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_d(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp28 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp33 = _tmp0 * _tmp32;
  const Scalar _tmp34 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp35 =
      std::pow(Scalar(_tmp34 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp36 = Scalar(1.0) * _tmp30;
  const Scalar _tmp37 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp38 = _tmp11 + _tmp15;
  const Scalar _tmp39 = _tmp38 + _tmp8;
  const Scalar _tmp40 = _tmp39 + p_init0;
  const Scalar _tmp41 = _tmp40 - p_c(0, 0);
  const Scalar _tmp42 = _tmp21 + _tmp25;
  const Scalar _tmp43 = _tmp20 + _tmp42;
  const Scalar _tmp44 = _tmp43 + p_init1;
  const Scalar _tmp45 = _tmp44 - p_c(1, 0);
  const Scalar _tmp46 = std::pow(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp45, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp47 = _tmp41 * _tmp46;
  const Scalar _tmp48 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp49 = -_tmp48;
  const Scalar _tmp50 =
      -Scalar(0.010999999999999999) * _tmp10 - Scalar(0.010999999999999999) * _tmp19;
  const Scalar _tmp51 = Scalar(0.20999999999999999) * _tmp12 - Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp52 = _tmp50 - _tmp51;
  const Scalar _tmp53 = _tmp49 + _tmp52;
  const Scalar _tmp54 = -_tmp20;
  const Scalar _tmp55 = _tmp42 + _tmp54;
  const Scalar _tmp56 = _tmp55 + p_init1;
  const Scalar _tmp57 = _tmp56 - p_b(1, 0);
  const Scalar _tmp58 = -_tmp8;
  const Scalar _tmp59 = _tmp38 + _tmp58;
  const Scalar _tmp60 = _tmp59 + p_init0;
  const Scalar _tmp61 = _tmp60 - p_b(0, 0);
  const Scalar _tmp62 = std::pow(Scalar(std::pow(_tmp57, Scalar(2)) + std::pow(_tmp61, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp63 = _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp53 * _tmp63;
  const Scalar _tmp65 = _tmp50 + _tmp51;
  const Scalar _tmp66 = _tmp49 + _tmp65;
  const Scalar _tmp67 = -_tmp63 * _tmp66 + _tmp64;
  const Scalar _tmp68 = _tmp26 + _tmp54;
  const Scalar _tmp69 = _tmp68 + p_init1;
  const Scalar _tmp70 = _tmp69 - p_a(1, 0);
  const Scalar _tmp71 = _tmp16 + _tmp58;
  const Scalar _tmp72 = _tmp71 + p_init0;
  const Scalar _tmp73 = _tmp72 - p_a(0, 0);
  const Scalar _tmp74 = Scalar(1.0) / (_tmp73);
  const Scalar _tmp75 = _tmp70 * _tmp74;
  const Scalar _tmp76 = _tmp45 * _tmp46;
  const Scalar _tmp77 = _tmp47 * _tmp75 - _tmp76;
  const Scalar _tmp78 = _tmp57 * _tmp62;
  const Scalar _tmp79 = Scalar(1.0) / (_tmp63 * _tmp75 - _tmp78);
  const Scalar _tmp80 = _tmp77 * _tmp79;
  const Scalar _tmp81 = _tmp47 * _tmp53;
  const Scalar _tmp82 = _tmp48 + _tmp65;
  const Scalar _tmp83 = -_tmp64 * _tmp75 + _tmp66 * _tmp78;
  const Scalar _tmp84 = -_tmp75 * _tmp81 + _tmp76 * _tmp82 - _tmp80 * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) * _tmp68;
  const Scalar _tmp86 = -_tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp55 + _tmp86);
  const Scalar _tmp88 = Scalar(1.0) * _tmp71;
  const Scalar _tmp89 = _tmp87 * (-_tmp59 + _tmp88);
  const Scalar _tmp90 = -_tmp47 * _tmp82 - _tmp67 * _tmp80 + _tmp81 - _tmp84 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp85 * _tmp89 + _tmp88;
  const Scalar _tmp93 = 0;
  const Scalar _tmp94 = _tmp91 * _tmp93;
  const Scalar _tmp95 = _tmp77 * _tmp91;
  const Scalar _tmp96 = _tmp79 * _tmp93 * _tmp95;
  const Scalar _tmp97 =
      std::sqrt(Scalar(std::pow(_tmp70, Scalar(2)) + std::pow(_tmp73, Scalar(2))));
  const Scalar _tmp98 = _tmp74 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp100 = _tmp98 * (-_tmp68 * _tmp73 * _tmp99 + _tmp70 * _tmp71 * _tmp99);
  const Scalar _tmp101 = _tmp100 * _tmp63 + _tmp55 * _tmp63 - _tmp59 * _tmp78;
  const Scalar _tmp102 = _tmp75 * _tmp79;
  const Scalar _tmp103 = _tmp102 * _tmp83 + _tmp53 * _tmp75;
  const Scalar _tmp104 = _tmp102 * _tmp67 - _tmp103 * _tmp89 - _tmp53;
  const Scalar _tmp105 = _tmp100 * _tmp47 - _tmp101 * _tmp80 - _tmp39 * _tmp76 + _tmp43 * _tmp47;
  const Scalar _tmp106 = _tmp105 * _tmp91;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp105);
  const Scalar _tmp108 = _tmp107 * _tmp90;
  const Scalar _tmp109 = _tmp108 * (-_tmp100 + _tmp101 * _tmp102 - _tmp104 * _tmp106);
  const Scalar _tmp110 = _tmp104 + _tmp109;
  const Scalar _tmp111 = _tmp110 * _tmp91;
  const Scalar _tmp112 = _tmp79 * (-_tmp110 * _tmp95 - _tmp75);
  const Scalar _tmp113 = _tmp28 - p_d(1, 0);
  const Scalar _tmp114 = _tmp18 - p_d(0, 0);
  const Scalar _tmp115 =
      std::pow(Scalar(std::pow(_tmp113, Scalar(2)) + std::pow(_tmp114, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp116 = _tmp114 * _tmp115;
  const Scalar _tmp117 = _tmp116 * _tmp98 * (_tmp111 * _tmp47 + _tmp112 * _tmp63 + Scalar(1.0));
  const Scalar _tmp118 = _tmp113 * _tmp115;
  const Scalar _tmp119 = -_tmp116 * _tmp27 + _tmp118 * _tmp17;
  const Scalar _tmp120 = Scalar(1.0) * _tmp107;
  const Scalar _tmp121 = _tmp119 * _tmp98 * (_tmp120 * _tmp47 - _tmp120 * _tmp63 * _tmp80);
  const Scalar _tmp122 = Scalar(1.0) * _tmp79;
  const Scalar _tmp123 = _tmp122 * _tmp83;
  const Scalar _tmp124 = -_tmp122 * _tmp67 + _tmp123 * _tmp89;
  const Scalar _tmp125 = _tmp108 * (-_tmp101 * _tmp122 - _tmp106 * _tmp124);
  const Scalar _tmp126 = _tmp124 + _tmp125;
  const Scalar _tmp127 = _tmp126 * _tmp91;
  const Scalar _tmp128 = _tmp79 * (-_tmp126 * _tmp95 + Scalar(1.0));
  const Scalar _tmp129 = _tmp118 * _tmp98 * (_tmp127 * _tmp47 + _tmp128 * _tmp63);
  const Scalar _tmp130 = -_tmp117 * fh1 - _tmp121 * fh1 - _tmp129 * fh1 -
                         _tmp37 * _tmp98 * (_tmp47 * _tmp94 - _tmp63 * _tmp96);
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp48 + _tmp52;
  const Scalar _tmp133 = _tmp132 * fh1;
  const Scalar _tmp134 = -_tmp118 * _tmp133 - Scalar(5.1796800000000003) * _tmp24 - _tmp27 * fv1;
  const Scalar _tmp135 = _tmp43 + _tmp86;
  const Scalar _tmp136 = _tmp135 * _tmp89;
  const Scalar _tmp137 = Scalar(1.0) / (-_tmp136 - _tmp39 + _tmp88);
  const Scalar _tmp138 = Scalar(1.0) * _tmp137;
  const Scalar _tmp139 = _tmp136 * _tmp138 + Scalar(1.0);
  const Scalar _tmp140 = Scalar(1.0) * _tmp87;
  const Scalar _tmp141 = _tmp138 * _tmp89;
  const Scalar _tmp142 = -Scalar(1.0) * _tmp139 * _tmp140 + Scalar(1.0) * _tmp141;
  const Scalar _tmp143 = _tmp116 * _tmp133 + Scalar(5.1796800000000003) * _tmp14 + _tmp17 * fv1;
  const Scalar _tmp144 = _tmp135 * _tmp87;
  const Scalar _tmp145 = Scalar(1.0) * _tmp138 * _tmp144 - Scalar(1.0) * _tmp138;
  const Scalar _tmp146 = _tmp84 * _tmp91;
  const Scalar _tmp147 = _tmp135 * _tmp137;
  const Scalar _tmp148 = _tmp103 + _tmp109 * _tmp147 - _tmp110 * _tmp146;
  const Scalar _tmp149 = Scalar(1.0) * _tmp116 * (_tmp109 * _tmp138 - _tmp140 * _tmp148);
  const Scalar _tmp150 = _tmp108 * _tmp138;
  const Scalar _tmp151 = _tmp87 * (-_tmp120 * _tmp84 + _tmp135 * _tmp150);
  const Scalar _tmp152 = Scalar(1.0) * _tmp119;
  const Scalar _tmp153 = _tmp152 * (_tmp150 - Scalar(1.0) * _tmp151);
  const Scalar _tmp154 = -_tmp123 + _tmp125 * _tmp147 - _tmp126 * _tmp146;
  const Scalar _tmp155 = Scalar(1.0) * _tmp118 * (_tmp125 * _tmp138 - _tmp140 * _tmp154);
  const Scalar _tmp156 = _tmp137 * _tmp92;
  const Scalar _tmp157 = -_tmp135 * _tmp156 - _tmp146 * _tmp93 + _tmp86;
  const Scalar _tmp158 =
      _tmp134 * _tmp142 + _tmp143 * _tmp145 + _tmp149 * fh1 + _tmp153 * fh1 + _tmp155 * fh1 +
      Scalar(1.0) * _tmp37 * (-_tmp138 * _tmp92 - _tmp140 * _tmp157 + Scalar(1.0));
  const Scalar _tmp159 = std::asinh(_tmp131 * _tmp158);
  const Scalar _tmp160 = Scalar(1.0) * _tmp159;
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp162 =
      -_tmp159 * _tmp161 - std::sqrt(Scalar(std::pow(Scalar(-_tmp69 + p_a(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp72 + p_a(0, 0)), Scalar(2))));
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = -_tmp117 - _tmp121 - _tmp129;
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp167 = _tmp118 * _tmp132;
  const Scalar _tmp168 = _tmp116 * _tmp132;
  const Scalar _tmp169 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp170 = _tmp165 * _tmp169;
  const Scalar _tmp171 =
      (_tmp131 * (-_tmp142 * _tmp167 + _tmp145 * _tmp168 + _tmp149 + _tmp153 + _tmp155) -
       _tmp158 * _tmp170) /
      std::sqrt(Scalar(std::pow(_tmp158, Scalar(2)) * _tmp169 + 1));
  const Scalar _tmp172 = _tmp118 * _tmp154 * _tmp87;
  const Scalar _tmp173 = _tmp139 * _tmp87;
  const Scalar _tmp174 = _tmp116 * _tmp87;
  const Scalar _tmp175 = _tmp148 * _tmp174;
  const Scalar _tmp176 = _tmp119 * _tmp151;
  const Scalar _tmp177 = _tmp138 * _tmp143;
  const Scalar _tmp178 = _tmp134 * _tmp173 - _tmp144 * _tmp177 + _tmp157 * _tmp37 * _tmp87 +
                         _tmp172 * fh1 + _tmp175 * fh1 + _tmp176 * fh1;
  const Scalar _tmp179 = _tmp118 * _tmp128;
  const Scalar _tmp180 = _tmp107 * _tmp152;
  const Scalar _tmp181 = _tmp180 * fh1;
  const Scalar _tmp182 = _tmp112 * _tmp116;
  const Scalar _tmp183 = _tmp179 * fh1 - _tmp181 * _tmp80 + _tmp182 * fh1 - _tmp37 * _tmp96;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp178 * _tmp184);
  const Scalar _tmp186 = _tmp179 - _tmp180 * _tmp80 + _tmp182;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp186;
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp189 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp190 = _tmp186 * _tmp189;
  const Scalar _tmp191 =
      (-_tmp178 * _tmp190 + _tmp184 * (-_tmp132 * _tmp135 * _tmp138 * _tmp174 - _tmp167 * _tmp173 +
                                       _tmp172 + _tmp175 + _tmp176)) /
      std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp193 =
      -_tmp185 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp56 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp60 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp194 = _tmp192 * _tmp193;
  const Scalar _tmp195 = Scalar(1.0) * _tmp185;
  const Scalar _tmp196 = _tmp119 * _tmp150;
  const Scalar _tmp197 = _tmp118 * _tmp125 * _tmp137;
  const Scalar _tmp198 = _tmp109 * _tmp116 * _tmp137;
  const Scalar _tmp199 = -_tmp134 * _tmp141 + _tmp156 * _tmp37 + _tmp177 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = _tmp118 * _tmp127;
  const Scalar _tmp201 = _tmp111 * _tmp116;
  const Scalar _tmp202 = _tmp181 + _tmp200 * fh1 + _tmp201 * fh1 + _tmp37 * _tmp94;
  const Scalar _tmp203 = Scalar(1.0) / (_tmp202);
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = std::pow(_tmp202, Scalar(-2));
  const Scalar _tmp207 = _tmp180 + _tmp200 + _tmp201;
  const Scalar _tmp208 = _tmp206 * _tmp207;
  const Scalar _tmp209 = (-_tmp199 * _tmp208 + _tmp203 * (_tmp138 * _tmp168 + _tmp141 * _tmp167 -
                                                          _tmp196 - _tmp197 - _tmp198)) /
                         std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp206 + 1));
  const Scalar _tmp210 = Scalar(9.6622558468725703) * _tmp202;
  const Scalar _tmp211 =
      -_tmp204 * _tmp210 - std::sqrt(Scalar(std::pow(Scalar(-_tmp40 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp44 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp212 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp213 = Scalar(0.1034955) * _tmp203;
  const Scalar _tmp214 = _tmp211 * _tmp213;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp34 * _tmp35 * fv1 * std::cosh(_tmp36) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp29 * _tmp35 - _tmp31) -
            _tmp32 * _tmp34) *
               std::cosh(_tmp33)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp33) -
      Scalar(9.6622558468725703) * std::sinh(_tmp36);
  _res(1, 0) = _tmp161 * (-Scalar(1.0) * _tmp171 * std::cosh(_tmp160) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp170 +
                           _tmp163 * (-_tmp159 * _tmp166 - _tmp161 * _tmp171)) *
                              std::cosh(_tmp164)) +
               _tmp166 * (-std::sinh(_tmp160) - std::sinh(_tmp164));
  _res(2, 0) = _tmp187 * (-std::sinh(_tmp194) - std::sinh(_tmp195)) +
               _tmp188 * (-Scalar(1.0) * _tmp191 * std::cosh(_tmp195) -
                          (-Scalar(0.1034955) * _tmp190 * _tmp193 +
                           _tmp192 * (-_tmp185 * _tmp187 - _tmp188 * _tmp191)) *
                              std::cosh(_tmp194));
  _res(3, 0) = _tmp210 * (-Scalar(1.0) * _tmp209 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp208 * _tmp211 +
                           _tmp213 * (-_tmp204 * _tmp212 - _tmp209 * _tmp210)) *
                              std::cosh(_tmp214)) +
               _tmp212 * (-std::sinh(_tmp205) - std::sinh(_tmp214));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
