// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl8
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl8(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 625

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (212)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp5 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _tmp2 * _tmp4;
  const Scalar _tmp10 = _tmp0 * _tmp5;
  const Scalar _tmp11 = _tmp10 + _tmp9;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp14 = 1 - 2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp8;
  const Scalar _tmp18 = _tmp17 + p_init0;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp3 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp20 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp20;
  const Scalar _tmp22 = 2 * _tmp0 * _tmp4;
  const Scalar _tmp23 = _tmp1 * _tmp5;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = -_tmp21 + _tmp25;
  const Scalar _tmp27 = _tmp19 + _tmp26;
  const Scalar _tmp28 = _tmp27 + p_init1;
  const Scalar _tmp29 = Scalar(1.0) / (fh1);
  const Scalar _tmp30 = _tmp29 * fv1;
  const Scalar _tmp31 = std::asinh(_tmp30);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * _tmp31;
  const Scalar _tmp33 =
      -_tmp32 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_b(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp28 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp34 = Scalar(0.1034955) * _tmp29;
  const Scalar _tmp35 = _tmp33 * _tmp34;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(1.0) * _tmp31;
  const Scalar _tmp39 = -_tmp19;
  const Scalar _tmp40 = _tmp21 + _tmp25;
  const Scalar _tmp41 = _tmp39 + _tmp40;
  const Scalar _tmp42 = _tmp41 + p_init1;
  const Scalar _tmp43 = _tmp42 - p_d(1, 0);
  const Scalar _tmp44 = _tmp12 - _tmp15;
  const Scalar _tmp45 = _tmp44 + _tmp7;
  const Scalar _tmp46 = _tmp45 + p_init0;
  const Scalar _tmp47 = _tmp46 - p_d(0, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp51 = -Scalar(0.010999999999999999) * _tmp13 -
                        Scalar(0.010999999999999999) * _tmp20 + Scalar(-0.010999999999999999);
  const Scalar _tmp52 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp53 = _tmp51 + _tmp52;
  const Scalar _tmp54 = _tmp50 + _tmp53;
  const Scalar _tmp55 = _tmp49 * _tmp54;
  const Scalar _tmp56 = _tmp51 - _tmp52;
  const Scalar _tmp57 = _tmp50 + _tmp56;
  const Scalar _tmp58 = _tmp43 * _tmp48;
  const Scalar _tmp59 = _tmp19 + _tmp40;
  const Scalar _tmp60 = _tmp59 + p_init1;
  const Scalar _tmp61 = _tmp60 - p_c(1, 0);
  const Scalar _tmp62 = _tmp16 + _tmp7;
  const Scalar _tmp63 = _tmp62 + p_init0;
  const Scalar _tmp64 = _tmp63 - p_c(0, 0);
  const Scalar _tmp65 = Scalar(1.0) / (_tmp64);
  const Scalar _tmp66 = _tmp61 * _tmp65;
  const Scalar _tmp67 = _tmp49 * _tmp66 - _tmp58;
  const Scalar _tmp68 = _tmp44 + _tmp8;
  const Scalar _tmp69 = _tmp68 + p_init0;
  const Scalar _tmp70 = _tmp69 - p_a(0, 0);
  const Scalar _tmp71 = _tmp26 + _tmp39;
  const Scalar _tmp72 = _tmp71 + p_init1;
  const Scalar _tmp73 = _tmp72 - p_a(1, 0);
  const Scalar _tmp74 = std::pow(Scalar(std::pow(_tmp70, Scalar(2)) + std::pow(_tmp73, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp75 = _tmp70 * _tmp74;
  const Scalar _tmp76 = _tmp73 * _tmp74;
  const Scalar _tmp77 = Scalar(1.0) / (_tmp66 * _tmp75 - _tmp76);
  const Scalar _tmp78 = -_tmp50;
  const Scalar _tmp79 = _tmp56 + _tmp78;
  const Scalar _tmp80 = _tmp54 * _tmp75;
  const Scalar _tmp81 = _tmp77 * (-_tmp75 * _tmp79 + _tmp80);
  const Scalar _tmp82 = _tmp77 * (-_tmp66 * _tmp80 + _tmp76 * _tmp79);
  const Scalar _tmp83 = -_tmp55 * _tmp66 + _tmp57 * _tmp58 - _tmp67 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) * _tmp59;
  const Scalar _tmp85 = -_tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp71 + _tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp62;
  const Scalar _tmp88 = -_tmp68 + _tmp87;
  const Scalar _tmp89 = _tmp86 * _tmp88;
  const Scalar _tmp90 = -_tmp49 * _tmp57 + _tmp55 - _tmp67 * _tmp81 - _tmp83 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp54 * _tmp66 + _tmp66 * _tmp82;
  const Scalar _tmp93 = -_tmp54 + _tmp66 * _tmp81 - _tmp89 * _tmp92;
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp61, Scalar(2)) + std::pow(_tmp64, Scalar(2))));
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 = _tmp65 * _tmp94;
  const Scalar _tmp97 = _tmp96 * (-_tmp59 * _tmp64 * _tmp95 + _tmp61 * _tmp62 * _tmp95);
  const Scalar _tmp98 = _tmp77 * (-_tmp68 * _tmp76 + _tmp71 * _tmp75 + _tmp75 * _tmp97);
  const Scalar _tmp99 = _tmp41 * _tmp49 - _tmp45 * _tmp58 + _tmp49 * _tmp97 - _tmp67 * _tmp98;
  const Scalar _tmp100 = _tmp91 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp102 = _tmp101 * _tmp90;
  const Scalar _tmp103 = _tmp102 * (-_tmp100 * _tmp93 + _tmp66 * _tmp98 - _tmp97);
  const Scalar _tmp104 = _tmp103 + _tmp93;
  const Scalar _tmp105 = _tmp104 * _tmp91;
  const Scalar _tmp106 = _tmp67 * _tmp91;
  const Scalar _tmp107 = _tmp77 * (-_tmp104 * _tmp106 - _tmp66);
  const Scalar _tmp108 = _tmp28 - p_b(1, 0);
  const Scalar _tmp109 = _tmp18 - p_b(0, 0);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp109 * _tmp110;
  const Scalar _tmp112 = _tmp111 * _tmp96 * (_tmp105 * _tmp49 + _tmp107 * _tmp75 + Scalar(1.0));
  const Scalar _tmp113 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp114 = _tmp84 * _tmp89 + _tmp87;
  const Scalar _tmp115 = 0;
  const Scalar _tmp116 = _tmp106 * _tmp115 * _tmp77;
  const Scalar _tmp117 = _tmp115 * _tmp91;
  const Scalar _tmp118 = _tmp108 * _tmp110;
  const Scalar _tmp119 = -_tmp111 * _tmp27 + _tmp118 * _tmp17;
  const Scalar _tmp120 = Scalar(1.0) * _tmp101;
  const Scalar _tmp121 = _tmp67 * _tmp77;
  const Scalar _tmp122 = _tmp119 * _tmp96 * (-_tmp120 * _tmp121 * _tmp75 + _tmp120 * _tmp49);
  const Scalar _tmp123 = Scalar(1.0) * _tmp86;
  const Scalar _tmp124 = _tmp123 * _tmp82 * _tmp88 - Scalar(1.0) * _tmp81;
  const Scalar _tmp125 = _tmp102 * (-_tmp100 * _tmp124 - Scalar(1.0) * _tmp98);
  const Scalar _tmp126 = _tmp124 + _tmp125;
  const Scalar _tmp127 = _tmp77 * (-_tmp106 * _tmp126 + Scalar(1.0));
  const Scalar _tmp128 = _tmp126 * _tmp91;
  const Scalar _tmp129 = _tmp118 * _tmp96 * (_tmp127 * _tmp75 + _tmp128 * _tmp49);
  const Scalar _tmp130 = -_tmp112 * fh1 -
                         _tmp113 * _tmp96 * (-_tmp116 * _tmp75 + _tmp117 * _tmp49) - _tmp122 * fh1 -
                         _tmp129 * fh1;
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp83 * _tmp91;
  const Scalar _tmp133 = _tmp41 + _tmp85;
  const Scalar _tmp134 = _tmp133 * _tmp89;
  const Scalar _tmp135 = Scalar(1.0) / (-_tmp134 - _tmp45 + _tmp87);
  const Scalar _tmp136 = _tmp133 * _tmp135;
  const Scalar _tmp137 = _tmp103 * _tmp136 - _tmp104 * _tmp132 + _tmp92;
  const Scalar _tmp138 = Scalar(1.0) * _tmp135;
  const Scalar _tmp139 = Scalar(1.0) * _tmp111 * (_tmp103 * _tmp138 - _tmp123 * _tmp137);
  const Scalar _tmp140 = _tmp114 * _tmp135;
  const Scalar _tmp141 = -_tmp115 * _tmp132 - _tmp133 * _tmp140 + _tmp85;
  const Scalar _tmp142 = _tmp102 * _tmp138;
  const Scalar _tmp143 = -_tmp120 * _tmp83 + _tmp133 * _tmp142;
  const Scalar _tmp144 = Scalar(1.0) * _tmp119 * (-_tmp123 * _tmp143 + _tmp142);
  const Scalar _tmp145 = _tmp125 * _tmp136 - _tmp126 * _tmp132 - Scalar(1.0) * _tmp82;
  const Scalar _tmp146 = Scalar(1.0) * _tmp118 * (-_tmp123 * _tmp145 + _tmp125 * _tmp138);
  const Scalar _tmp147 = _tmp53 + _tmp78;
  const Scalar _tmp148 = _tmp147 * fh1;
  const Scalar _tmp149 = Scalar(5.1796800000000003) * _tmp11 + _tmp111 * _tmp148 + _tmp17 * fv1;
  const Scalar _tmp150 = _tmp133 * _tmp86;
  const Scalar _tmp151 = Scalar(1.0) * _tmp138 * _tmp150 - Scalar(1.0) * _tmp138;
  const Scalar _tmp152 = -_tmp118 * _tmp148 - Scalar(5.1796800000000003) * _tmp24 - _tmp27 * fv1;
  const Scalar _tmp153 = _tmp138 * _tmp89;
  const Scalar _tmp154 = _tmp86 * (_tmp134 * _tmp138 + Scalar(1.0));
  const Scalar _tmp155 = Scalar(1.0) * _tmp153 - Scalar(1.0) * _tmp154;
  const Scalar _tmp156 =
      Scalar(1.0) * _tmp113 * (-_tmp114 * _tmp138 - _tmp123 * _tmp141 + Scalar(1.0)) +
      _tmp139 * fh1 + _tmp144 * fh1 + _tmp146 * fh1 + _tmp149 * _tmp151 + _tmp152 * _tmp155;
  const Scalar _tmp157 = std::asinh(_tmp131 * _tmp156);
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp159 =
      -_tmp157 * _tmp158 - std::sqrt(Scalar(std::pow(Scalar(-_tmp60 + p_c(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp63 + p_c(0, 0)), Scalar(2))));
  const Scalar _tmp160 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp161 = _tmp159 * _tmp160;
  const Scalar _tmp162 = -_tmp112 - _tmp122 - _tmp129;
  const Scalar _tmp163 = Scalar(9.6622558468725703) * _tmp162;
  const Scalar _tmp164 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp165 = _tmp162 * _tmp164;
  const Scalar _tmp166 = _tmp118 * _tmp147;
  const Scalar _tmp167 = _tmp111 * _tmp147;
  const Scalar _tmp168 =
      (_tmp131 * (_tmp139 + _tmp144 + _tmp146 + _tmp151 * _tmp167 - _tmp155 * _tmp166) -
       _tmp156 * _tmp165) /
      std::sqrt(Scalar(std::pow(_tmp156, Scalar(2)) * _tmp164 + 1));
  const Scalar _tmp169 = Scalar(1.0) * _tmp157;
  const Scalar _tmp170 = _tmp119 * _tmp120;
  const Scalar _tmp171 = _tmp170 * fh1;
  const Scalar _tmp172 = _tmp107 * _tmp111;
  const Scalar _tmp173 = _tmp118 * _tmp127;
  const Scalar _tmp174 = -_tmp113 * _tmp116 - _tmp121 * _tmp171 + _tmp172 * fh1 + _tmp173 * fh1;
  const Scalar _tmp175 = _tmp138 * _tmp149;
  const Scalar _tmp176 = _tmp118 * _tmp145 * _tmp86;
  const Scalar _tmp177 = _tmp119 * _tmp143 * _tmp86;
  const Scalar _tmp178 = _tmp111 * _tmp86;
  const Scalar _tmp179 = _tmp137 * _tmp178;
  const Scalar _tmp180 = _tmp113 * _tmp141 * _tmp86 - _tmp150 * _tmp175 + _tmp152 * _tmp154 +
                         _tmp176 * fh1 + _tmp177 * fh1 + _tmp179 * fh1;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp174);
  const Scalar _tmp182 = std::asinh(_tmp180 * _tmp181);
  const Scalar _tmp183 = Scalar(9.6622558468725703) * _tmp182;
  const Scalar _tmp184 =
      -_tmp174 * _tmp183 - std::sqrt(Scalar(std::pow(Scalar(-_tmp69 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp72 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp185 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp174, Scalar(-2));
  const Scalar _tmp188 = -_tmp121 * _tmp170 + _tmp172 + _tmp173;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp174;
  const Scalar _tmp191 =
      (-_tmp180 * _tmp189 + _tmp181 * (-_tmp133 * _tmp138 * _tmp147 * _tmp178 - _tmp154 * _tmp166 +
                                       _tmp176 + _tmp177 + _tmp179)) /
      std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp187 + 1));
  const Scalar _tmp192 = Scalar(1.0) * _tmp182;
  const Scalar _tmp193 = _tmp105 * _tmp111;
  const Scalar _tmp194 = _tmp118 * _tmp128;
  const Scalar _tmp195 = _tmp113 * _tmp117 + _tmp171 + _tmp193 * fh1 + _tmp194 * fh1;
  const Scalar _tmp196 = Scalar(1.0) / (_tmp195);
  const Scalar _tmp197 = _tmp118 * _tmp125 * _tmp135;
  const Scalar _tmp198 = _tmp119 * _tmp142;
  const Scalar _tmp199 = _tmp103 * _tmp111 * _tmp135;
  const Scalar _tmp200 = _tmp113 * _tmp140 - _tmp152 * _tmp153 + _tmp175 - _tmp197 * fh1 -
                         _tmp198 * fh1 - _tmp199 * fh1;
  const Scalar _tmp201 = std::asinh(_tmp196 * _tmp200);
  const Scalar _tmp202 = Scalar(1.0) * _tmp201;
  const Scalar _tmp203 = Scalar(9.6622558468725703) * _tmp195;
  const Scalar _tmp204 =
      -_tmp201 * _tmp203 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp205 = Scalar(0.1034955) * _tmp196;
  const Scalar _tmp206 = _tmp204 * _tmp205;
  const Scalar _tmp207 = _tmp170 + _tmp193 + _tmp194;
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp209 = std::pow(_tmp195, Scalar(-2));
  const Scalar _tmp210 = _tmp207 * _tmp209;
  const Scalar _tmp211 =
      (_tmp196 * (_tmp138 * _tmp167 + _tmp153 * _tmp166 - _tmp197 - _tmp198 - _tmp199) -
       _tmp200 * _tmp210) /
      std::sqrt(Scalar(std::pow(_tmp200, Scalar(2)) * _tmp209 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp38) -
                    (-Scalar(0.1034955) * _tmp33 * _tmp36 +
                     _tmp34 * (Scalar(9.6622558468725703) * _tmp30 * _tmp37 - _tmp32)) *
                        std::cosh(_tmp35)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp35) -
               Scalar(9.6622558468725703) * std::sinh(_tmp38);
  _res(1, 0) = _tmp158 * (-Scalar(1.0) * _tmp168 * std::cosh(_tmp169) -
                          (-Scalar(0.1034955) * _tmp159 * _tmp165 +
                           _tmp160 * (-_tmp157 * _tmp163 - _tmp158 * _tmp168)) *
                              std::cosh(_tmp161)) +
               _tmp163 * (-std::sinh(_tmp161) - std::sinh(_tmp169));
  _res(2, 0) = Scalar(9.6622558468725703) * _tmp188 * (-std::sinh(_tmp186) - std::sinh(_tmp192)) +
               _tmp190 * (-Scalar(1.0) * _tmp191 * std::cosh(_tmp192) -
                          (-Scalar(0.1034955) * _tmp184 * _tmp189 +
                           _tmp185 * (-_tmp183 * _tmp188 - _tmp190 * _tmp191)) *
                              std::cosh(_tmp186));
  _res(3, 0) = _tmp203 * (-Scalar(1.0) * _tmp211 * std::cosh(_tmp202) -
                          (-Scalar(0.1034955) * _tmp204 * _tmp210 +
                           _tmp205 * (-_tmp201 * _tmp208 - _tmp203 * _tmp211)) *
                              std::cosh(_tmp206)) +
               _tmp208 * (-std::sinh(_tmp202) - std::sinh(_tmp206));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
