// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl3
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl3(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 494

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp1 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -_DeltaRot[0] * rot_init_x - _DeltaRot[1] * rot_init_y -
                       _DeltaRot[2] * rot_init_z + _DeltaRot[3] * rot_init_w;
  const Scalar _tmp6 = 2 * _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp10 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp11 = -Scalar(0.010999999999999999) * _tmp10 -
                        Scalar(0.010999999999999999) * _tmp9 + Scalar(-0.010999999999999999);
  const Scalar _tmp12 = _tmp2 * _tmp4;
  const Scalar _tmp13 = 2 * _tmp0;
  const Scalar _tmp14 = _tmp13 * _tmp5;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp11 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp8;
  const Scalar _tmp18 = 1 - 2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp18 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = _tmp3 + _tmp6;
  const Scalar _tmp22 = -Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp13 * _tmp4;
  const Scalar _tmp24 = _tmp2 * _tmp5;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp26 = _tmp22 - _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + p_init0;
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp30 = -_tmp29;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp32 = _tmp12 - _tmp14;
  const Scalar _tmp33 = -Scalar(0.010999999999999999) * _tmp32;
  const Scalar _tmp34 = -_tmp31 + _tmp33;
  const Scalar _tmp35 = _tmp30 + _tmp34;
  const Scalar _tmp36 = _tmp35 + p_init1;
  const Scalar _tmp37 = std::pow(Scalar(-_tmp28 + p_a(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp36 + p_a(1, 0)), Scalar(2));
  const Scalar _tmp38 = Scalar(1.0) / (fh1);
  const Scalar _tmp39 = std::asinh(_tmp38 * fv1);
  const Scalar _tmp40 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp41 = _tmp31 + _tmp33;
  const Scalar _tmp42 = _tmp30 + _tmp41;
  const Scalar _tmp43 = _tmp42 + p_init1;
  const Scalar _tmp44 = _tmp43 - p_b(1, 0);
  const Scalar _tmp45 = _tmp19 + _tmp26;
  const Scalar _tmp46 = _tmp45 + p_init0;
  const Scalar _tmp47 = _tmp46 - p_b(0, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = _tmp44 * _tmp48;
  const Scalar _tmp51 = _tmp22 + _tmp25;
  const Scalar _tmp52 = _tmp19 + _tmp51;
  const Scalar _tmp53 = _tmp52 + p_init0;
  const Scalar _tmp54 = _tmp53 - p_c(0, 0);
  const Scalar _tmp55 = _tmp29 + _tmp41;
  const Scalar _tmp56 = _tmp55 + p_init1;
  const Scalar _tmp57 = _tmp56 - p_c(1, 0);
  const Scalar _tmp58 =
      std::sqrt(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp57, Scalar(2))));
  const Scalar _tmp59 = Scalar(1.0) / (_tmp58);
  const Scalar _tmp60 = Scalar(1.0) / (_tmp54);
  const Scalar _tmp61 = _tmp58 * _tmp60;
  const Scalar _tmp62 = _tmp61 * (_tmp52 * _tmp57 * _tmp59 - _tmp54 * _tmp55 * _tmp59);
  const Scalar _tmp63 = _tmp29 + _tmp34;
  const Scalar _tmp64 = _tmp63 + p_init1;
  const Scalar _tmp65 = _tmp64 - p_d(1, 0);
  const Scalar _tmp66 = _tmp20 + _tmp51;
  const Scalar _tmp67 = _tmp66 + p_init0;
  const Scalar _tmp68 = _tmp67 - p_d(0, 0);
  const Scalar _tmp69 = std::pow(Scalar(std::pow(_tmp65, Scalar(2)) + std::pow(_tmp68, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp70 = _tmp68 * _tmp69;
  const Scalar _tmp71 = _tmp65 * _tmp69;
  const Scalar _tmp72 = _tmp62 * _tmp70 + _tmp63 * _tmp70 - _tmp66 * _tmp71;
  const Scalar _tmp73 = _tmp57 * _tmp60;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp70 * _tmp73 - _tmp71);
  const Scalar _tmp75 = _tmp49 * _tmp73 - _tmp50;
  const Scalar _tmp76 = _tmp74 * _tmp75;
  const Scalar _tmp77 = _tmp42 * _tmp49 - _tmp45 * _tmp50 + _tmp49 * _tmp62 - _tmp72 * _tmp76;
  const Scalar _tmp78 = Scalar(1.0) / (_tmp77);
  const Scalar _tmp79 = Scalar(1.0) * _tmp78;
  const Scalar _tmp80 = Scalar(1.0) * _tmp74;
  const Scalar _tmp81 = _tmp75 * _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp28 - p_a(0, 0);
  const Scalar _tmp83 = _tmp36 - p_a(1, 0);
  const Scalar _tmp84 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp83, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp85 = _tmp83 * _tmp84;
  const Scalar _tmp86 = _tmp82 * _tmp84;
  const Scalar _tmp87 = fh1 * (_tmp27 * _tmp85 - _tmp35 * _tmp86);
  const Scalar _tmp88 = _tmp73 * _tmp74;
  const Scalar _tmp89 = _tmp11 + _tmp15;
  const Scalar _tmp90 = _tmp7 + _tmp89;
  const Scalar _tmp91 = _tmp73 * _tmp90;
  const Scalar _tmp92 = _tmp8 + _tmp89;
  const Scalar _tmp93 = -_tmp70 * _tmp91 + _tmp71 * _tmp92;
  const Scalar _tmp94 = _tmp88 * _tmp93 + _tmp91;
  const Scalar _tmp95 = Scalar(1.0) * _tmp55;
  const Scalar _tmp96 = -_tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp63 + _tmp96);
  const Scalar _tmp98 = Scalar(1.0) * _tmp52;
  const Scalar _tmp99 = _tmp97 * (-_tmp66 + _tmp98);
  const Scalar _tmp100 = _tmp70 * _tmp90 - _tmp70 * _tmp92;
  const Scalar _tmp101 = _tmp100 * _tmp88 - _tmp90 - _tmp94 * _tmp99;
  const Scalar _tmp102 = _tmp16 + _tmp7;
  const Scalar _tmp103 = _tmp102 * _tmp50 - _tmp49 * _tmp91 - _tmp76 * _tmp93;
  const Scalar _tmp104 = -_tmp100 * _tmp76 - _tmp102 * _tmp49 - _tmp103 * _tmp99 + _tmp49 * _tmp90;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp106 = _tmp105 * _tmp77;
  const Scalar _tmp107 = _tmp104 * _tmp78;
  const Scalar _tmp108 = _tmp107 * (-_tmp101 * _tmp106 - _tmp62 + _tmp72 * _tmp88);
  const Scalar _tmp109 = _tmp101 + _tmp108;
  const Scalar _tmp110 = _tmp105 * _tmp49;
  const Scalar _tmp111 = _tmp105 * _tmp75;
  const Scalar _tmp112 = -_tmp109 * _tmp111 - _tmp73;
  const Scalar _tmp113 = _tmp70 * _tmp74;
  const Scalar _tmp114 = _tmp86 * fh1;
  const Scalar _tmp115 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp116 = _tmp95 * _tmp99 + _tmp98;
  const Scalar _tmp117 = 0;
  const Scalar _tmp118 = _tmp111 * _tmp117;
  const Scalar _tmp119 = _tmp105 * _tmp117;
  const Scalar _tmp120 = _tmp80 * _tmp93;
  const Scalar _tmp121 = -_tmp100 * _tmp80 + _tmp120 * _tmp99;
  const Scalar _tmp122 = _tmp107 * (-_tmp106 * _tmp121 - _tmp72 * _tmp80);
  const Scalar _tmp123 = _tmp121 + _tmp122;
  const Scalar _tmp124 = -_tmp111 * _tmp123 + Scalar(1.0);
  const Scalar _tmp125 = _tmp85 * fh1;
  const Scalar _tmp126 = -_tmp114 * _tmp61 * (_tmp109 * _tmp110 + _tmp112 * _tmp113 + Scalar(1.0)) -
                         _tmp115 * _tmp61 * (-_tmp113 * _tmp118 + _tmp119 * _tmp49) -
                         _tmp125 * _tmp61 * (_tmp110 * _tmp123 + _tmp113 * _tmp124) -
                         _tmp61 * _tmp87 * (_tmp49 * _tmp79 - _tmp70 * _tmp81);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = _tmp42 + _tmp96;
  const Scalar _tmp129 = _tmp128 * _tmp99;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp45 + _tmp98);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp103 * _tmp105;
  const Scalar _tmp133 = _tmp116 * _tmp130;
  const Scalar _tmp134 = -_tmp117 * _tmp132 - _tmp128 * _tmp133 + _tmp96;
  const Scalar _tmp135 = Scalar(1.0) * _tmp97;
  const Scalar _tmp136 = _tmp128 * _tmp130;
  const Scalar _tmp137 = -_tmp120 + _tmp122 * _tmp136 - _tmp123 * _tmp132;
  const Scalar _tmp138 = _tmp17 * fh1;
  const Scalar _tmp139 = -_tmp138 * _tmp85 - Scalar(3.29616) * _tmp32 - _tmp35 * fv1;
  const Scalar _tmp140 = _tmp97 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp141 = _tmp131 * _tmp99;
  const Scalar _tmp142 = _tmp108 * _tmp136 - _tmp109 * _tmp132 + _tmp94;
  const Scalar _tmp143 = _tmp138 * _tmp86 + Scalar(3.29616) * _tmp21 + _tmp27 * fv1;
  const Scalar _tmp144 = _tmp128 * _tmp97;
  const Scalar _tmp145 = _tmp107 * _tmp131;
  const Scalar _tmp146 = -_tmp103 * _tmp79 + _tmp128 * _tmp145;
  const Scalar _tmp147 = std::asinh(
      _tmp127 * (Scalar(1.0) * _tmp114 * (_tmp108 * _tmp131 - _tmp135 * _tmp142) +
                 Scalar(1.0) * _tmp115 * (-_tmp116 * _tmp131 - _tmp134 * _tmp135 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp125 * (_tmp122 * _tmp131 - _tmp135 * _tmp137) +
                 Scalar(1.0) * _tmp139 * (-Scalar(1.0) * _tmp140 + _tmp141) +
                 Scalar(1.0) * _tmp143 * (_tmp131 * _tmp144 - _tmp131) +
                 Scalar(1.0) * _tmp87 * (-_tmp135 * _tmp146 + _tmp145)));
  const Scalar _tmp148 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp149 = std::pow(Scalar(-_tmp53 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp56 + p_c(1, 0)), Scalar(2));
  const Scalar _tmp150 = _tmp112 * _tmp114 * _tmp74 - _tmp115 * _tmp118 * _tmp74 +
                         _tmp124 * _tmp125 * _tmp74 - _tmp81 * _tmp87;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = std::pow(Scalar(-_tmp64 + p_d(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp67 + p_d(0, 0)), Scalar(2));
  const Scalar _tmp153 = _tmp131 * _tmp143;
  const Scalar _tmp154 =
      std::asinh(_tmp151 * (_tmp114 * _tmp142 * _tmp97 + _tmp115 * _tmp134 * _tmp97 +
                            _tmp125 * _tmp137 * _tmp97 + _tmp139 * _tmp140 - _tmp144 * _tmp153 +
                            _tmp146 * _tmp87 * _tmp97));
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp150;
  const Scalar _tmp156 = std::pow(Scalar(-_tmp43 + p_b(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp46 + p_b(0, 0)), Scalar(2));
  const Scalar _tmp157 = _tmp105 * _tmp109 * _tmp114 + _tmp105 * _tmp123 * _tmp125 +
                         _tmp115 * _tmp119 + _tmp79 * _tmp87;
  const Scalar _tmp158 = Scalar(1.0) / (_tmp157);
  const Scalar _tmp159 = std::asinh(_tmp158 * (-_tmp108 * _tmp114 * _tmp130 + _tmp115 * _tmp133 -
                                               _tmp122 * _tmp125 * _tmp130 - _tmp139 * _tmp141 -
                                               _tmp145 * _tmp87 + _tmp153));
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp157;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp40 * (-std::sinh(Scalar(1.0) * _tmp39) -
                std::sinh(Scalar(0.1034955) * _tmp38 * (-std::sqrt(_tmp37) - _tmp39 * _tmp40))) -
      std::sqrt(Scalar(_tmp37 + std::pow(Scalar(-_tmp17 + p_a(2, 0) - p_init2), Scalar(2))));
  _res(1, 0) =
      _tmp148 *
          (-std::sinh(Scalar(1.0) * _tmp147) -
           std::sinh(Scalar(0.1034955) * _tmp127 * (-_tmp147 * _tmp148 - std::sqrt(_tmp149)))) -
      std::sqrt(Scalar(_tmp149 + std::pow(Scalar(-_tmp90 + p_c(2, 0) - p_init2), Scalar(2))));
  _res(2, 0) =
      _tmp155 *
          (-std::sinh(Scalar(1.0) * _tmp154) -
           std::sinh(Scalar(0.1034955) * _tmp151 * (-std::sqrt(_tmp152) - _tmp154 * _tmp155))) -
      std::sqrt(Scalar(_tmp152 + std::pow(Scalar(-_tmp92 + p_d(2, 0) - p_init2), Scalar(2))));
  _res(3, 0) =
      _tmp160 *
          (-std::sinh(Scalar(1.0) * _tmp159) -
           std::sinh(Scalar(0.1034955) * _tmp158 * (-std::sqrt(_tmp156) - _tmp159 * _tmp160))) -
      std::sqrt(Scalar(_tmp156 + std::pow(Scalar(-_tmp102 + p_b(2, 0) - p_init2), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
