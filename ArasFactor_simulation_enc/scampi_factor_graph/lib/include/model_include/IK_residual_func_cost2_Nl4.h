// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl4
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl4(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 495

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp1 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp10 = 1 - 2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp11 =
      -Scalar(0.010999999999999999) * _tmp10 - Scalar(0.010999999999999999) * _tmp9;
  const Scalar _tmp12 = _tmp2 * _tmp4;
  const Scalar _tmp13 = _tmp0 * _tmp5;
  const Scalar _tmp14 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp11 - _tmp14;
  const Scalar _tmp16 = _tmp15 + _tmp8;
  const Scalar _tmp17 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = _tmp3 + _tmp6;
  const Scalar _tmp21 = -Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp22 = 2 * _tmp0 * _tmp4;
  const Scalar _tmp23 = _tmp1 * _tmp5;
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp25 = _tmp21 - _tmp24;
  const Scalar _tmp26 = _tmp19 + _tmp25;
  const Scalar _tmp27 = _tmp26 + p_init0;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp29 = -_tmp28;
  const Scalar _tmp30 = _tmp12 - _tmp13;
  const Scalar _tmp31 = -Scalar(0.010999999999999999) * _tmp30;
  const Scalar _tmp32 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp9 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp33 = _tmp31 - _tmp32;
  const Scalar _tmp34 = _tmp29 + _tmp33;
  const Scalar _tmp35 = _tmp34 + p_init1;
  const Scalar _tmp36 = std::pow(Scalar(-_tmp27 + p_a(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp35 + p_a(1, 0)), Scalar(2));
  const Scalar _tmp37 = Scalar(1.0) / (fh1);
  const Scalar _tmp38 = std::asinh(_tmp37 * fv1);
  const Scalar _tmp39 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp40 = _tmp11 + _tmp14;
  const Scalar _tmp41 = _tmp40 + _tmp8;
  const Scalar _tmp42 = _tmp21 + _tmp24;
  const Scalar _tmp43 = _tmp19 + _tmp42;
  const Scalar _tmp44 = _tmp43 + p_init0;
  const Scalar _tmp45 = _tmp31 + _tmp32;
  const Scalar _tmp46 = _tmp29 + _tmp45;
  const Scalar _tmp47 = _tmp46 + p_init1;
  const Scalar _tmp48 = std::pow(Scalar(-_tmp44 + p_d(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp47 + p_d(1, 0)), Scalar(2));
  const Scalar _tmp49 = Scalar(1.0) * _tmp43;
  const Scalar _tmp50 = _tmp18 + _tmp42;
  const Scalar _tmp51 = Scalar(1.0) * _tmp46;
  const Scalar _tmp52 = -_tmp51;
  const Scalar _tmp53 = _tmp28 + _tmp45;
  const Scalar _tmp54 = _tmp52 + _tmp53;
  const Scalar _tmp55 = _tmp28 + _tmp33;
  const Scalar _tmp56 = Scalar(1.0) / (_tmp52 + _tmp55);
  const Scalar _tmp57 = _tmp18 + _tmp25;
  const Scalar _tmp58 = _tmp49 - _tmp57;
  const Scalar _tmp59 = _tmp56 * _tmp58;
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = Scalar(1.0) / (_tmp49 - _tmp50 - _tmp60);
  const Scalar _tmp62 = Scalar(1.0) * _tmp61;
  const Scalar _tmp63 = _tmp50 + p_init0;
  const Scalar _tmp64 = _tmp63 - p_c(0, 0);
  const Scalar _tmp65 = _tmp53 + p_init1;
  const Scalar _tmp66 = _tmp65 - p_c(1, 0);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp66 * _tmp67;
  const Scalar _tmp69 = _tmp64 * _tmp67;
  const Scalar _tmp70 = _tmp55 + p_init1;
  const Scalar _tmp71 = _tmp70 - p_b(1, 0);
  const Scalar _tmp72 = _tmp57 + p_init0;
  const Scalar _tmp73 = _tmp72 - p_b(0, 0);
  const Scalar _tmp74 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp73, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp47 - p_d(1, 0);
  const Scalar _tmp77 = _tmp44 - p_d(0, 0);
  const Scalar _tmp78 =
      std::sqrt(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp77, Scalar(2))));
  const Scalar _tmp79 = Scalar(1.0) / (_tmp78);
  const Scalar _tmp80 = Scalar(1.0) / (_tmp77);
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp81 * (_tmp43 * _tmp76 * _tmp79 - _tmp46 * _tmp77 * _tmp79);
  const Scalar _tmp83 = _tmp71 * _tmp74;
  const Scalar _tmp84 = _tmp55 * _tmp75 - _tmp57 * _tmp83 + _tmp75 * _tmp82;
  const Scalar _tmp85 = _tmp76 * _tmp80;
  const Scalar _tmp86 = -_tmp68 + _tmp69 * _tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp75 * _tmp85 - _tmp83);
  const Scalar _tmp88 = _tmp86 * _tmp87;
  const Scalar _tmp89 = -_tmp50 * _tmp68 + _tmp53 * _tmp69 + _tmp69 * _tmp82 - _tmp84 * _tmp88;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 = _tmp40 + _tmp7;
  const Scalar _tmp92 = _tmp41 * _tmp85;
  const Scalar _tmp93 = _tmp15 + _tmp7;
  const Scalar _tmp94 = _tmp41 * _tmp75;
  const Scalar _tmp95 = _tmp83 * _tmp93 - _tmp85 * _tmp94;
  const Scalar _tmp96 = _tmp68 * _tmp91 - _tmp69 * _tmp92 - _tmp88 * _tmp95;
  const Scalar _tmp97 = -_tmp75 * _tmp93 + _tmp94;
  const Scalar _tmp98 = _tmp41 * _tmp69 - _tmp59 * _tmp96 - _tmp69 * _tmp91 - _tmp88 * _tmp97;
  const Scalar _tmp99 = _tmp90 * _tmp98;
  const Scalar _tmp100 = _tmp62 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) * _tmp90;
  const Scalar _tmp102 = _tmp100 * _tmp54 - _tmp101 * _tmp96;
  const Scalar _tmp103 = Scalar(1.0) * _tmp56;
  const Scalar _tmp104 = _tmp27 - p_a(0, 0);
  const Scalar _tmp105 = _tmp35 - p_a(1, 0);
  const Scalar _tmp106 =
      std::pow(Scalar(std::pow(_tmp104, Scalar(2)) + std::pow(_tmp105, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp107 = _tmp105 * _tmp106;
  const Scalar _tmp108 = _tmp104 * _tmp106;
  const Scalar _tmp109 = fh1 * (_tmp107 * _tmp26 - _tmp108 * _tmp34);
  const Scalar _tmp110 = _tmp49 + _tmp51 * _tmp59;
  const Scalar _tmp111 = _tmp110 * _tmp61;
  const Scalar _tmp112 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp113 = 0;
  const Scalar _tmp114 = -_tmp111 * _tmp54 - _tmp113 * _tmp96 + _tmp52;
  const Scalar _tmp115 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp116 = _tmp16 * fh1;
  const Scalar _tmp117 = _tmp108 * _tmp116 + Scalar(3.29616) * _tmp20 + _tmp26 * fv1;
  const Scalar _tmp118 = _tmp54 * _tmp56;
  const Scalar _tmp119 = Scalar(1.0) * _tmp87;
  const Scalar _tmp120 = _tmp103 * _tmp58 * _tmp87 * _tmp95 - _tmp119 * _tmp97;
  const Scalar _tmp121 = _tmp112 * _tmp89;
  const Scalar _tmp122 = _tmp99 * (-_tmp119 * _tmp84 - _tmp120 * _tmp121);
  const Scalar _tmp123 = _tmp120 + _tmp122;
  const Scalar _tmp124 = _tmp112 * _tmp96;
  const Scalar _tmp125 = _tmp54 * _tmp61;
  const Scalar _tmp126 = -_tmp119 * _tmp95 + _tmp122 * _tmp125 - _tmp123 * _tmp124;
  const Scalar _tmp127 = _tmp107 * fh1;
  const Scalar _tmp128 = -_tmp107 * _tmp116 - Scalar(3.29616) * _tmp30 - _tmp34 * fv1;
  const Scalar _tmp129 = _tmp60 * _tmp62 + Scalar(1.0);
  const Scalar _tmp130 = _tmp59 * _tmp62;
  const Scalar _tmp131 = _tmp85 * _tmp87;
  const Scalar _tmp132 = _tmp131 * _tmp95 + _tmp92;
  const Scalar _tmp133 = _tmp131 * _tmp97 - _tmp132 * _tmp59 - _tmp41;
  const Scalar _tmp134 = _tmp99 * (-_tmp121 * _tmp133 + _tmp131 * _tmp84 - _tmp82);
  const Scalar _tmp135 = _tmp133 + _tmp134;
  const Scalar _tmp136 = -_tmp124 * _tmp135 + _tmp125 * _tmp134 + _tmp132;
  const Scalar _tmp137 = _tmp108 * fh1;
  const Scalar _tmp138 = _tmp112 * _tmp69;
  const Scalar _tmp139 = _tmp112 * _tmp86;
  const Scalar _tmp140 = -_tmp135 * _tmp139 - _tmp85;
  const Scalar _tmp141 = _tmp75 * _tmp87;
  const Scalar _tmp142 = -_tmp123 * _tmp139 + Scalar(1.0);
  const Scalar _tmp143 = _tmp75 * _tmp88;
  const Scalar _tmp144 = -_tmp109 * _tmp81 * (-_tmp101 * _tmp143 + _tmp101 * _tmp69) -
                         _tmp115 * _tmp81 * (-_tmp113 * _tmp143 + _tmp113 * _tmp69) -
                         _tmp127 * _tmp81 * (_tmp123 * _tmp138 + _tmp141 * _tmp142) -
                         _tmp137 * _tmp81 * (_tmp135 * _tmp138 + _tmp140 * _tmp141 + Scalar(1.0));
  const Scalar _tmp145 = Scalar(1.0) / (_tmp144);
  const Scalar _tmp146 = std::asinh(
      _tmp145 * (Scalar(1.0) * _tmp109 * (_tmp100 - _tmp102 * _tmp103) +
                 Scalar(1.0) * _tmp115 * (-_tmp103 * _tmp114 - _tmp110 * _tmp62 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp117 * (_tmp118 * _tmp62 - _tmp62) +
                 Scalar(1.0) * _tmp127 * (-_tmp103 * _tmp126 + _tmp122 * _tmp62) +
                 Scalar(1.0) * _tmp128 * (-_tmp103 * _tmp129 + _tmp130) +
                 Scalar(1.0) * _tmp137 * (-_tmp103 * _tmp136 + _tmp134 * _tmp62)));
  const Scalar _tmp147 = Scalar(9.6622558468725703) * _tmp144;
  const Scalar _tmp148 = _tmp101 * _tmp109;
  const Scalar _tmp149 = _tmp113 * _tmp115;
  const Scalar _tmp150 =
      _tmp127 * _tmp142 * _tmp87 + _tmp137 * _tmp140 * _tmp87 - _tmp148 * _tmp88 - _tmp149 * _tmp88;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = _tmp117 * _tmp62;
  const Scalar _tmp153 =
      std::asinh(_tmp151 * (_tmp102 * _tmp109 * _tmp56 + _tmp114 * _tmp115 * _tmp56 -
                            _tmp118 * _tmp152 + _tmp126 * _tmp127 * _tmp56 +
                            _tmp128 * _tmp129 * _tmp56 + _tmp136 * _tmp137 * _tmp56));
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp150;
  const Scalar _tmp155 = std::pow(Scalar(-_tmp70 + p_b(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp72 + p_b(0, 0)), Scalar(2));
  const Scalar _tmp156 = std::pow(Scalar(-_tmp63 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp65 + p_c(1, 0)), Scalar(2));
  const Scalar _tmp157 =
      _tmp112 * _tmp123 * _tmp127 + _tmp112 * _tmp135 * _tmp137 + _tmp148 + _tmp149;
  const Scalar _tmp158 = Scalar(1.0) / (_tmp157);
  const Scalar _tmp159 =
      std::asinh(_tmp158 * (-_tmp100 * _tmp109 + _tmp111 * _tmp115 - _tmp122 * _tmp127 * _tmp61 -
                            _tmp128 * _tmp130 - _tmp134 * _tmp137 * _tmp61 + _tmp152));
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp157;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp39 * (-std::sinh(Scalar(1.0) * _tmp38) -
                std::sinh(Scalar(0.1034955) * _tmp37 * (-std::sqrt(_tmp36) - _tmp38 * _tmp39))) -
      std::sqrt(Scalar(_tmp36 + std::pow(Scalar(-_tmp16 + p_a(2, 0) - p_init2), Scalar(2))));
  _res(1, 0) =
      _tmp147 *
          (-std::sinh(Scalar(1.0) * _tmp146) -
           std::sinh(Scalar(0.1034955) * _tmp145 * (-_tmp146 * _tmp147 - std::sqrt(_tmp48)))) -
      std::sqrt(Scalar(_tmp48 + std::pow(Scalar(-_tmp41 + p_d(2, 0) - p_init2), Scalar(2))));
  _res(2, 0) =
      _tmp154 *
          (-std::sinh(Scalar(1.0) * _tmp153) -
           std::sinh(Scalar(0.1034955) * _tmp151 * (-_tmp153 * _tmp154 - std::sqrt(_tmp155)))) -
      std::sqrt(Scalar(_tmp155 + std::pow(Scalar(-_tmp93 + p_b(2, 0) - p_init2), Scalar(2))));
  _res(3, 0) =
      _tmp160 *
          (-std::sinh(Scalar(1.0) * _tmp159) -
           std::sinh(Scalar(0.1034955) * _tmp158 * (-std::sqrt(_tmp156) - _tmp159 * _tmp160))) -
      std::sqrt(Scalar(_tmp156 + std::pow(Scalar(-_tmp91 + p_c(2, 0) - p_init2), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
