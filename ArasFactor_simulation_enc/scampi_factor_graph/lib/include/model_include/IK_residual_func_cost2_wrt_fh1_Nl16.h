// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl16
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl16(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 622

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _tmp0 * fv1;
  const Scalar _tmp2 = std::asinh(_tmp1);
  const Scalar _tmp3 = Scalar(9.6622558468725703) * _tmp2;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp10 = 2 * _tmp6 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp12 = _tmp11 * _tmp4;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp4;
  const Scalar _tmp16 = _tmp15 * _tmp9;
  const Scalar _tmp17 = _tmp11 * _tmp6;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp14 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp8;
  const Scalar _tmp21 = _tmp20 + p_init0;
  const Scalar _tmp22 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp24 = _tmp15 * _tmp6;
  const Scalar _tmp25 = _tmp11 * _tmp9;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + p_init1;
  const Scalar _tmp32 =
      -_tmp3 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp21 + p_c(0, 0)), Scalar(2)) +
                                      std::pow(Scalar(-_tmp31 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp33 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp34 = _tmp32 * _tmp33;
  const Scalar _tmp35 = Scalar(1.0) * _tmp2;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp39 = -_tmp38;
  const Scalar _tmp40 = -Scalar(0.010999999999999999) * _tmp22 -
                        Scalar(0.010999999999999999) * _tmp5 + Scalar(-0.010999999999999999);
  const Scalar _tmp41 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp42 = _tmp40 + _tmp41;
  const Scalar _tmp43 = _tmp39 + _tmp42;
  const Scalar _tmp44 = -_tmp8;
  const Scalar _tmp45 = _tmp14 - _tmp18;
  const Scalar _tmp46 = _tmp44 + _tmp45;
  const Scalar _tmp47 = _tmp46 + p_init0;
  const Scalar _tmp48 = _tmp47 - p_a(0, 0);
  const Scalar _tmp49 = -_tmp23;
  const Scalar _tmp50 = _tmp27 - _tmp28;
  const Scalar _tmp51 = _tmp49 + _tmp50;
  const Scalar _tmp52 = _tmp51 + p_init1;
  const Scalar _tmp53 = _tmp52 - p_a(1, 0);
  const Scalar _tmp54 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp53, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp55 = _tmp48 * _tmp54;
  const Scalar _tmp56 = _tmp43 * _tmp55;
  const Scalar _tmp57 = _tmp40 - _tmp41;
  const Scalar _tmp58 = _tmp39 + _tmp57;
  const Scalar _tmp59 = -_tmp55 * _tmp58 + _tmp56;
  const Scalar _tmp60 = _tmp23 + _tmp50;
  const Scalar _tmp61 = _tmp60 + p_init1;
  const Scalar _tmp62 = _tmp61 - p_d(1, 0);
  const Scalar _tmp63 = _tmp19 + _tmp44;
  const Scalar _tmp64 = _tmp63 + p_init0;
  const Scalar _tmp65 = _tmp64 - p_d(0, 0);
  const Scalar _tmp66 = Scalar(1.0) / (_tmp65);
  const Scalar _tmp67 = _tmp62 * _tmp66;
  const Scalar _tmp68 = _tmp53 * _tmp54;
  const Scalar _tmp69 = Scalar(1.0) / (_tmp55 * _tmp67 - _tmp68);
  const Scalar _tmp70 = Scalar(1.0) * _tmp69;
  const Scalar _tmp71 = -_tmp56 * _tmp67 + _tmp58 * _tmp68;
  const Scalar _tmp72 = _tmp70 * _tmp71;
  const Scalar _tmp73 = Scalar(1.0) * _tmp60;
  const Scalar _tmp74 = -_tmp73;
  const Scalar _tmp75 = Scalar(1.0) / (_tmp51 + _tmp74);
  const Scalar _tmp76 = Scalar(1.0) * _tmp63;
  const Scalar _tmp77 = _tmp75 * (-_tmp46 + _tmp76);
  const Scalar _tmp78 = -_tmp59 * _tmp70 + _tmp72 * _tmp77;
  const Scalar _tmp79 = _tmp29 + _tmp49;
  const Scalar _tmp80 = _tmp79 + p_init1;
  const Scalar _tmp81 = _tmp80 - p_b(1, 0);
  const Scalar _tmp82 = _tmp45 + _tmp8;
  const Scalar _tmp83 = _tmp82 + p_init0;
  const Scalar _tmp84 = _tmp83 - p_b(0, 0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp84 * _tmp85;
  const Scalar _tmp87 = _tmp81 * _tmp85;
  const Scalar _tmp88 = _tmp67 * _tmp86 - _tmp87;
  const Scalar _tmp89 = _tmp69 * _tmp88;
  const Scalar _tmp90 = _tmp38 + _tmp57;
  const Scalar _tmp91 = _tmp43 * _tmp67;
  const Scalar _tmp92 = -_tmp71 * _tmp89 - _tmp86 * _tmp91 + _tmp87 * _tmp90;
  const Scalar _tmp93 = _tmp43 * _tmp86 - _tmp59 * _tmp89 - _tmp77 * _tmp92 - _tmp86 * _tmp90;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp62, Scalar(2)) + std::pow(_tmp65, Scalar(2))));
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp66 * _tmp95;
  const Scalar _tmp98 = _tmp97 * (-_tmp60 * _tmp65 * _tmp96 + _tmp62 * _tmp63 * _tmp96);
  const Scalar _tmp99 = -_tmp46 * _tmp68 + _tmp51 * _tmp55 + _tmp55 * _tmp98;
  const Scalar _tmp100 = _tmp79 * _tmp86 - _tmp82 * _tmp87 + _tmp86 * _tmp98 - _tmp89 * _tmp99;
  const Scalar _tmp101 = _tmp100 * _tmp94;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp93;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp78 - _tmp70 * _tmp99);
  const Scalar _tmp105 = _tmp74 + _tmp79;
  const Scalar _tmp106 = _tmp105 * _tmp77;
  const Scalar _tmp107 = Scalar(1.0) / (-_tmp106 + _tmp76 - _tmp82);
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp104 + _tmp78;
  const Scalar _tmp110 = _tmp92 * _tmp94;
  const Scalar _tmp111 = _tmp104 * _tmp108 - _tmp109 * _tmp110 - _tmp72;
  const Scalar _tmp112 = Scalar(1.0) * _tmp75;
  const Scalar _tmp113 = Scalar(1.0) * _tmp107;
  const Scalar _tmp114 = _tmp21 - p_c(0, 0);
  const Scalar _tmp115 = _tmp31 - p_c(1, 0);
  const Scalar _tmp116 =
      std::pow(Scalar(std::pow(_tmp114, Scalar(2)) + std::pow(_tmp115, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp117 = _tmp115 * _tmp116;
  const Scalar _tmp118 = Scalar(1.0) * _tmp117 * (_tmp104 * _tmp113 - _tmp111 * _tmp112);
  const Scalar _tmp119 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp120 = _tmp73 * _tmp77 + _tmp76;
  const Scalar _tmp121 = _tmp107 * _tmp120;
  const Scalar _tmp122 = 0;
  const Scalar _tmp123 = -_tmp105 * _tmp121 - _tmp110 * _tmp122 + _tmp74;
  const Scalar _tmp124 = _tmp38 + _tmp42;
  const Scalar _tmp125 = _tmp124 * fh1;
  const Scalar _tmp126 = -_tmp117 * _tmp125 - Scalar(3.29616) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp127 = _tmp75 * (_tmp106 * _tmp113 + Scalar(1.0));
  const Scalar _tmp128 = _tmp113 * _tmp77;
  const Scalar _tmp129 = -Scalar(1.0) * _tmp127 + Scalar(1.0) * _tmp128;
  const Scalar _tmp130 = _tmp67 * _tmp69;
  const Scalar _tmp131 = _tmp130 * _tmp71 + _tmp91;
  const Scalar _tmp132 = _tmp130 * _tmp59 - _tmp131 * _tmp77 - _tmp43;
  const Scalar _tmp133 = _tmp103 * (-_tmp101 * _tmp132 + _tmp130 * _tmp99 - _tmp98);
  const Scalar _tmp134 = _tmp132 + _tmp133;
  const Scalar _tmp135 = _tmp108 * _tmp133 - _tmp110 * _tmp134 + _tmp131;
  const Scalar _tmp136 = _tmp114 * _tmp116;
  const Scalar _tmp137 = Scalar(1.0) * _tmp136 * (-_tmp112 * _tmp135 + _tmp113 * _tmp133);
  const Scalar _tmp138 = _tmp125 * _tmp136 + Scalar(3.29616) * _tmp13 + _tmp20 * fv1;
  const Scalar _tmp139 = _tmp105 * _tmp113;
  const Scalar _tmp140 = _tmp139 * _tmp75;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp113 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp103 * _tmp113;
  const Scalar _tmp143 = Scalar(1.0) * _tmp102;
  const Scalar _tmp144 = _tmp75 * (_tmp103 * _tmp139 - _tmp143 * _tmp92);
  const Scalar _tmp145 = _tmp117 * _tmp20 - _tmp136 * _tmp30;
  const Scalar _tmp146 = Scalar(1.0) * _tmp145;
  const Scalar _tmp147 = _tmp146 * (_tmp142 - Scalar(1.0) * _tmp144);
  const Scalar _tmp148 =
      _tmp118 * fh1 +
      Scalar(1.0) * _tmp119 * (-_tmp112 * _tmp123 - _tmp113 * _tmp120 + Scalar(1.0)) +
      _tmp126 * _tmp129 + _tmp137 * fh1 + _tmp138 * _tmp141 + _tmp147 * fh1;
  const Scalar _tmp149 = _tmp134 * _tmp94;
  const Scalar _tmp150 = _tmp88 * _tmp94;
  const Scalar _tmp151 = _tmp69 * (-_tmp134 * _tmp150 - _tmp67);
  const Scalar _tmp152 = _tmp136 * _tmp97 * (_tmp149 * _tmp86 + _tmp151 * _tmp55 + Scalar(1.0));
  const Scalar _tmp153 =
      _tmp145 * _tmp97 * (-_tmp102 * _tmp55 * _tmp70 * _tmp88 + _tmp143 * _tmp86);
  const Scalar _tmp154 = _tmp109 * _tmp94;
  const Scalar _tmp155 = _tmp69 * (-_tmp109 * _tmp150 + Scalar(1.0));
  const Scalar _tmp156 = _tmp117 * _tmp97 * (_tmp154 * _tmp86 + _tmp155 * _tmp55);
  const Scalar _tmp157 = _tmp122 * _tmp94;
  const Scalar _tmp158 = -_tmp119 * _tmp97 * (-_tmp157 * _tmp55 * _tmp89 + _tmp157 * _tmp86) -
                         _tmp152 * fh1 - _tmp153 * fh1 - _tmp156 * fh1;
  const Scalar _tmp159 = Scalar(1.0) / (_tmp158);
  const Scalar _tmp160 = std::asinh(_tmp148 * _tmp159);
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp162 =
      -_tmp160 * _tmp161 - std::sqrt(Scalar(std::pow(Scalar(-_tmp61 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp64 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = -_tmp152 - _tmp153 - _tmp156;
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp167 = _tmp117 * _tmp124;
  const Scalar _tmp168 = _tmp124 * _tmp136;
  const Scalar _tmp169 = std::pow(_tmp158, Scalar(-2));
  const Scalar _tmp170 = _tmp165 * _tmp169;
  const Scalar _tmp171 = (-_tmp148 * _tmp170 + _tmp159 * (_tmp118 - _tmp129 * _tmp167 + _tmp137 +
                                                          _tmp141 * _tmp168 + _tmp147)) /
                         std::sqrt(Scalar(std::pow(_tmp148, Scalar(2)) * _tmp169 + 1));
  const Scalar _tmp172 = Scalar(1.0) * _tmp160;
  const Scalar _tmp173 = _tmp111 * _tmp117 * _tmp75;
  const Scalar _tmp174 = _tmp144 * _tmp145;
  const Scalar _tmp175 = _tmp135 * _tmp136 * _tmp75;
  const Scalar _tmp176 = _tmp113 * _tmp138;
  const Scalar _tmp177 = -_tmp105 * _tmp176 * _tmp75 + _tmp119 * _tmp123 * _tmp75 +
                         _tmp126 * _tmp127 + _tmp173 * fh1 + _tmp174 * fh1 + _tmp175 * fh1;
  const Scalar _tmp178 = _tmp136 * _tmp151;
  const Scalar _tmp179 = _tmp117 * _tmp155;
  const Scalar _tmp180 = _tmp119 * _tmp157;
  const Scalar _tmp181 = _tmp102 * _tmp146;
  const Scalar _tmp182 = _tmp181 * fh1;
  const Scalar _tmp183 = _tmp178 * fh1 + _tmp179 * fh1 - _tmp180 * _tmp89 - _tmp182 * _tmp89;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp177 * _tmp184);
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp187 =
      -_tmp185 * _tmp186 - std::sqrt(Scalar(std::pow(Scalar(-_tmp47 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp52 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = _tmp178 + _tmp179 - _tmp181 * _tmp89;
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp190;
  const Scalar _tmp192 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp193 = _tmp190 * _tmp192;
  const Scalar _tmp194 = (-_tmp177 * _tmp193 + _tmp184 * (-_tmp127 * _tmp167 - _tmp140 * _tmp168 +
                                                          _tmp173 + _tmp174 + _tmp175)) /
                         std::sqrt(Scalar(std::pow(_tmp177, Scalar(2)) * _tmp192 + 1));
  const Scalar _tmp195 = Scalar(1.0) * _tmp185;
  const Scalar _tmp196 = _tmp117 * _tmp154;
  const Scalar _tmp197 = _tmp136 * _tmp149;
  const Scalar _tmp198 = _tmp181 + _tmp196 + _tmp197;
  const Scalar _tmp199 = _tmp180 + _tmp182 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp200 = Scalar(1.0) / (_tmp199);
  const Scalar _tmp201 = _tmp107 * _tmp133 * _tmp136;
  const Scalar _tmp202 = _tmp142 * _tmp145;
  const Scalar _tmp203 = _tmp104 * _tmp107 * _tmp117;
  const Scalar _tmp204 = _tmp119 * _tmp121 - _tmp126 * _tmp128 + _tmp176 - _tmp201 * fh1 -
                         _tmp202 * fh1 - _tmp203 * fh1;
  const Scalar _tmp205 = std::asinh(_tmp200 * _tmp204);
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp205;
  const Scalar _tmp207 =
      -_tmp199 * _tmp206 - std::sqrt(Scalar(std::pow(Scalar(-_tmp80 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp83 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp200;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = Scalar(1.0) * _tmp205;
  const Scalar _tmp211 = std::pow(_tmp199, Scalar(-2));
  const Scalar _tmp212 = _tmp198 * _tmp211;
  const Scalar _tmp213 =
      (_tmp200 * (_tmp113 * _tmp168 + _tmp128 * _tmp167 - _tmp201 - _tmp202 - _tmp203) -
       _tmp204 * _tmp212) /
      std::sqrt(Scalar(std::pow(_tmp204, Scalar(2)) * _tmp211 + 1));
  const Scalar _tmp214 = Scalar(9.6622558468725703) * _tmp199;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp35) -
                    (-Scalar(0.1034955) * _tmp32 * _tmp36 +
                     _tmp33 * (Scalar(9.6622558468725703) * _tmp1 * _tmp37 - _tmp3)) *
                        std::cosh(_tmp34)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp34) -
               Scalar(9.6622558468725703) * std::sinh(_tmp35);
  _res(1, 0) = _tmp161 * (-Scalar(1.0) * _tmp171 * std::cosh(_tmp172) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp170 +
                           _tmp163 * (-_tmp160 * _tmp166 - _tmp161 * _tmp171)) *
                              std::cosh(_tmp164)) +
               _tmp166 * (-std::sinh(_tmp164) - std::sinh(_tmp172));
  _res(2, 0) = _tmp186 * (-Scalar(1.0) * _tmp194 * std::cosh(_tmp195) -
                          (-Scalar(0.1034955) * _tmp187 * _tmp193 +
                           _tmp188 * (-_tmp185 * _tmp191 - _tmp186 * _tmp194)) *
                              std::cosh(_tmp189)) +
               _tmp191 * (-std::sinh(_tmp189) - std::sinh(_tmp195));
  _res(3, 0) = Scalar(9.6622558468725703) * _tmp198 * (-std::sinh(_tmp209) - std::sinh(_tmp210)) +
               _tmp214 * (-Scalar(1.0) * _tmp213 * std::cosh(_tmp210) -
                          (-Scalar(0.1034955) * _tmp207 * _tmp212 +
                           _tmp208 * (-_tmp198 * _tmp206 - _tmp213 * _tmp214)) *
                              std::cosh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
