// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl7
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl7(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 499

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (161)
  const Scalar _tmp0 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp1 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp2 = 2 * _tmp1;
  const Scalar _tmp3 = _tmp0 * _tmp2;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp6 = _tmp4 * _tmp5;
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp3 - Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp9 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp10 = -Scalar(0.010999999999999999) * _tmp8 -
                        Scalar(0.010999999999999999) * _tmp9 + Scalar(-0.010999999999999999);
  const Scalar _tmp11 = _tmp2 * _tmp4;
  const Scalar _tmp12 = _tmp0 * _tmp5;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp10 - _tmp13;
  const Scalar _tmp15 = _tmp14 + _tmp7;
  const Scalar _tmp16 = 2 * _tmp0 * _tmp4;
  const Scalar _tmp17 = _tmp1 * _tmp5;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = _tmp3 + _tmp6;
  const Scalar _tmp21 = -Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp22 = 1 - 2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp24 = _tmp21 + _tmp23;
  const Scalar _tmp25 = _tmp19 + _tmp24;
  const Scalar _tmp26 = _tmp25 + p_init0;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = _tmp11 - _tmp12;
  const Scalar _tmp30 = -Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp32 = _tmp30 + _tmp31;
  const Scalar _tmp33 = _tmp28 + _tmp32;
  const Scalar _tmp34 = _tmp33 + p_init1;
  const Scalar _tmp35 = std::pow(Scalar(-_tmp26 + p_b(0, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp34 + p_b(1, 0)), Scalar(2));
  const Scalar _tmp36 = Scalar(1.0) / (fh1);
  const Scalar _tmp37 = std::asinh(_tmp36 * fv1);
  const Scalar _tmp38 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp39 = _tmp21 - _tmp23;
  const Scalar _tmp40 = _tmp19 + _tmp39;
  const Scalar _tmp41 = Scalar(1.0) * _tmp40;
  const Scalar _tmp42 = _tmp18 + _tmp24;
  const Scalar _tmp43 = _tmp30 - _tmp31;
  const Scalar _tmp44 = _tmp28 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp27 + _tmp32;
  const Scalar _tmp48 = _tmp46 + _tmp47;
  const Scalar _tmp49 = _tmp27 + _tmp43;
  const Scalar _tmp50 = Scalar(1.0) / (_tmp46 + _tmp49);
  const Scalar _tmp51 = _tmp18 + _tmp39;
  const Scalar _tmp52 = _tmp41 - _tmp51;
  const Scalar _tmp53 = _tmp50 * _tmp52;
  const Scalar _tmp54 = _tmp48 * _tmp53;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp41 - _tmp42 - _tmp54);
  const Scalar _tmp56 = Scalar(1.0) * _tmp55;
  const Scalar _tmp57 = _tmp54 * _tmp56 + Scalar(1.0);
  const Scalar _tmp58 = Scalar(1.0) * _tmp50;
  const Scalar _tmp59 = _tmp53 * _tmp56;
  const Scalar _tmp60 = _tmp34 - p_b(1, 0);
  const Scalar _tmp61 = _tmp26 - p_b(0, 0);
  const Scalar _tmp62 = std::pow(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp61, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp63 = _tmp60 * _tmp62;
  const Scalar _tmp64 = _tmp15 * fh1;
  const Scalar _tmp65 = -Scalar(3.29616) * _tmp29 - _tmp33 * fv1 - _tmp63 * _tmp64;
  const Scalar _tmp66 = _tmp48 * _tmp50;
  const Scalar _tmp67 = _tmp61 * _tmp62;
  const Scalar _tmp68 = Scalar(3.29616) * _tmp20 + _tmp25 * fv1 + _tmp64 * _tmp67;
  const Scalar _tmp69 = -_tmp7;
  const Scalar _tmp70 = _tmp10 + _tmp13;
  const Scalar _tmp71 = _tmp69 + _tmp70;
  const Scalar _tmp72 = _tmp49 + p_init1;
  const Scalar _tmp73 = _tmp72 - p_d(1, 0);
  const Scalar _tmp74 = _tmp51 + p_init0;
  const Scalar _tmp75 = _tmp74 - p_d(0, 0);
  const Scalar _tmp76 = std::pow(Scalar(std::pow(_tmp73, Scalar(2)) + std::pow(_tmp75, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp77 = _tmp73 * _tmp76;
  const Scalar _tmp78 = _tmp75 * _tmp76;
  const Scalar _tmp79 = _tmp14 + _tmp69;
  const Scalar _tmp80 = _tmp44 + p_init1;
  const Scalar _tmp81 = _tmp80 - p_a(1, 0);
  const Scalar _tmp82 = _tmp40 + p_init0;
  const Scalar _tmp83 = _tmp82 - p_a(0, 0);
  const Scalar _tmp84 = Scalar(1.0) / (_tmp83);
  const Scalar _tmp85 = _tmp81 * _tmp84;
  const Scalar _tmp86 = _tmp79 * _tmp85;
  const Scalar _tmp87 = _tmp71 * _tmp77 - _tmp78 * _tmp86;
  const Scalar _tmp88 = Scalar(1.0) / (-_tmp77 + _tmp78 * _tmp85);
  const Scalar _tmp89 = Scalar(1.0) * _tmp88;
  const Scalar _tmp90 = -_tmp71 * _tmp78 + _tmp78 * _tmp79;
  const Scalar _tmp91 = _tmp52 * _tmp58 * _tmp87 * _tmp88 - _tmp89 * _tmp90;
  const Scalar _tmp92 = _tmp42 + p_init0;
  const Scalar _tmp93 = _tmp92 - p_c(0, 0);
  const Scalar _tmp94 = _tmp47 + p_init1;
  const Scalar _tmp95 = _tmp94 - p_c(1, 0);
  const Scalar _tmp96 = std::pow(Scalar(std::pow(_tmp93, Scalar(2)) + std::pow(_tmp95, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp97 = _tmp93 * _tmp96;
  const Scalar _tmp98 = _tmp95 * _tmp96;
  const Scalar _tmp99 = _tmp85 * _tmp97 - _tmp98;
  const Scalar _tmp100 = _tmp88 * _tmp99;
  const Scalar _tmp101 = _tmp79 * _tmp97;
  const Scalar _tmp102 = _tmp7 + _tmp70;
  const Scalar _tmp103 = -_tmp100 * _tmp87 - _tmp101 * _tmp85 + _tmp102 * _tmp98;
  const Scalar _tmp104 = -_tmp100 * _tmp90 + _tmp101 - _tmp102 * _tmp97 - _tmp103 * _tmp53;
  const Scalar _tmp105 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp106 =
      std::sqrt(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp83, Scalar(2))));
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp106 * _tmp84;
  const Scalar _tmp109 = _tmp108 * (_tmp107 * _tmp40 * _tmp81 - _tmp107 * _tmp44 * _tmp83);
  const Scalar _tmp110 = _tmp109 * _tmp78 + _tmp49 * _tmp78 - _tmp51 * _tmp77;
  const Scalar _tmp111 = -_tmp100 * _tmp110 + _tmp109 * _tmp97 - _tmp42 * _tmp98 + _tmp47 * _tmp97;
  const Scalar _tmp112 = _tmp105 * _tmp111;
  const Scalar _tmp113 = Scalar(1.0) / (_tmp111);
  const Scalar _tmp114 = _tmp104 * _tmp113;
  const Scalar _tmp115 = _tmp114 * (-_tmp110 * _tmp89 - _tmp112 * _tmp91);
  const Scalar _tmp116 = _tmp48 * _tmp55;
  const Scalar _tmp117 = _tmp115 + _tmp91;
  const Scalar _tmp118 = _tmp103 * _tmp105;
  const Scalar _tmp119 = _tmp115 * _tmp116 - _tmp117 * _tmp118 - _tmp87 * _tmp89;
  const Scalar _tmp120 = _tmp63 * fh1;
  const Scalar _tmp121 = _tmp85 * _tmp88;
  const Scalar _tmp122 = _tmp121 * _tmp87 + _tmp86;
  const Scalar _tmp123 = _tmp121 * _tmp90 - _tmp122 * _tmp53 - _tmp79;
  const Scalar _tmp124 = _tmp114 * (-_tmp109 + _tmp110 * _tmp121 - _tmp112 * _tmp123);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = _tmp116 * _tmp124 - _tmp118 * _tmp125 + _tmp122;
  const Scalar _tmp127 = _tmp67 * fh1;
  const Scalar _tmp128 = _tmp114 * _tmp56;
  const Scalar _tmp129 = Scalar(1.0) * _tmp113;
  const Scalar _tmp130 = -_tmp103 * _tmp129 + _tmp128 * _tmp48;
  const Scalar _tmp131 = fh1 * (_tmp25 * _tmp63 - _tmp33 * _tmp67);
  const Scalar _tmp132 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp133 = _tmp41 + _tmp45 * _tmp53;
  const Scalar _tmp134 = 0;
  const Scalar _tmp135 = _tmp133 * _tmp55;
  const Scalar _tmp136 = _tmp50 * (-_tmp118 * _tmp134 - _tmp135 * _tmp48 + _tmp46);
  const Scalar _tmp137 = _tmp113 * _tmp89 * _tmp99;
  const Scalar _tmp138 = _tmp105 * _tmp99;
  const Scalar _tmp139 = -_tmp125 * _tmp138 - _tmp85;
  const Scalar _tmp140 = _tmp78 * _tmp88;
  const Scalar _tmp141 = _tmp105 * _tmp97;
  const Scalar _tmp142 = _tmp134 * _tmp138;
  const Scalar _tmp143 = _tmp105 * _tmp134;
  const Scalar _tmp144 = -_tmp117 * _tmp138 + Scalar(1.0);
  const Scalar _tmp145 = -_tmp108 * _tmp120 * (_tmp117 * _tmp141 + _tmp140 * _tmp144) -
                         _tmp108 * _tmp127 * (_tmp125 * _tmp141 + _tmp139 * _tmp140 + Scalar(1.0)) -
                         _tmp108 * _tmp131 * (_tmp129 * _tmp97 - _tmp137 * _tmp78) -
                         _tmp108 * _tmp132 * (-_tmp140 * _tmp142 + _tmp143 * _tmp97);
  const Scalar _tmp146 = Scalar(1.0) / (_tmp145);
  const Scalar _tmp147 = std::asinh(
      _tmp146 * (Scalar(1.0) * _tmp120 * (_tmp115 * _tmp56 - _tmp119 * _tmp58) +
                 Scalar(1.0) * _tmp127 * (_tmp124 * _tmp56 - _tmp126 * _tmp58) +
                 Scalar(1.0) * _tmp131 * (_tmp128 - _tmp130 * _tmp58) +
                 Scalar(1.0) * _tmp132 * (-_tmp133 * _tmp56 - Scalar(1.0) * _tmp136 + Scalar(1.0)) +
                 Scalar(1.0) * _tmp65 * (-_tmp57 * _tmp58 + _tmp59) +
                 Scalar(1.0) * _tmp68 * (_tmp56 * _tmp66 - _tmp56)));
  const Scalar _tmp148 = Scalar(9.6622558468725703) * _tmp145;
  const Scalar _tmp149 = std::pow(Scalar(-_tmp80 + p_a(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp82 + p_a(0, 0)), Scalar(2));
  const Scalar _tmp150 = std::pow(Scalar(-_tmp72 + p_d(1, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp74 + p_d(0, 0)), Scalar(2));
  const Scalar _tmp151 = _tmp120 * _tmp144 * _tmp88 + _tmp127 * _tmp139 * _tmp88 -
                         _tmp131 * _tmp137 - _tmp132 * _tmp142 * _tmp88;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 = _tmp56 * _tmp68;
  const Scalar _tmp154 =
      std::asinh(_tmp152 * (_tmp119 * _tmp120 * _tmp50 + _tmp126 * _tmp127 * _tmp50 +
                            _tmp130 * _tmp131 * _tmp50 + _tmp132 * _tmp136 - _tmp153 * _tmp66 +
                            _tmp50 * _tmp57 * _tmp65));
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp151;
  const Scalar _tmp156 = _tmp105 * _tmp117 * _tmp120 + _tmp105 * _tmp125 * _tmp127 +
                         _tmp129 * _tmp131 + _tmp132 * _tmp143;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 =
      std::asinh(_tmp157 * (-_tmp115 * _tmp120 * _tmp55 - _tmp124 * _tmp127 * _tmp55 -
                            _tmp128 * _tmp131 + _tmp132 * _tmp135 + _tmp153 - _tmp59 * _tmp65));
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp156;
  const Scalar _tmp160 = std::pow(Scalar(-_tmp92 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp94 + p_c(1, 0)), Scalar(2));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp38 * (-std::sinh(Scalar(1.0) * _tmp37) -
                std::sinh(Scalar(0.1034955) * _tmp36 * (-std::sqrt(_tmp35) - _tmp37 * _tmp38))) -
      std::sqrt(Scalar(_tmp35 + std::pow(Scalar(-_tmp15 + p_b(2, 0) - p_init2), Scalar(2))));
  _res(1, 0) =
      _tmp148 *
          (-std::sinh(Scalar(1.0) * _tmp147) -
           std::sinh(Scalar(0.1034955) * _tmp146 * (-_tmp147 * _tmp148 - std::sqrt(_tmp149)))) -
      std::sqrt(Scalar(_tmp149 + std::pow(Scalar(-_tmp79 + p_a(2, 0) - p_init2), Scalar(2))));
  _res(2, 0) =
      _tmp155 *
          (-std::sinh(Scalar(1.0) * _tmp154) -
           std::sinh(Scalar(0.1034955) * _tmp152 * (-std::sqrt(_tmp150) - _tmp154 * _tmp155))) -
      std::sqrt(Scalar(_tmp150 + std::pow(Scalar(-_tmp71 + p_d(2, 0) - p_init2), Scalar(2))));
  _res(3, 0) =
      _tmp159 *
          (-std::sinh(Scalar(1.0) * _tmp158) -
           std::sinh(Scalar(0.1034955) * _tmp157 * (-_tmp158 * _tmp159 - std::sqrt(_tmp160)))) -
      std::sqrt(Scalar(_tmp160 + std::pow(Scalar(-_tmp102 + p_c(2, 0) - p_init2), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
