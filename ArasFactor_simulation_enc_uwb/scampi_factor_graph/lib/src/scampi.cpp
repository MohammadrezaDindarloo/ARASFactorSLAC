                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    std::uniform_real_distribution<double> cable_uwb_noise(-0.05, 0.05);
    std::uniform_real_distribution<double> cable_encoder_noise(-0.001, 0.001);
    ...
    double translationnoise = 0.001/3; //in meter 
    double orientationnoise = (0.05 * M_PI / 180.0)/3; // in degree and convert to radian
    ...
    auto Sensor_noiseModel_cost5 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(20)); 
    ...
    for (size_t i = 0; i < p_platform_collection.size()-1; i++)
    {
        ...
        double uwb_data_2 = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + cable_uwb_noise(generator);
        double uwb_data_3 = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + cable_uwb_noise(generator);
        double uwb_data_4 = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + cable_uwb_noise(generator);
        ...
        double encoder_data_2 = cable_length_collection[i][1] + cable_encoder_noise(generator);
        double encoder_data_3 = cable_length_collection[i][2] + cable_encoder_noise(generator);
        double encoder_data_4 = cable_length_collection[i][3] + cable_encoder_noise(generator);
        ...
        graph.add(std::make_shared<FK_factor_graph_cost4>(Symbol('o', 0), Symbol('o', 1), Symbol('o', 2), Symbol('o', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('h', i), Symbol('v', i), Symbol('X', i), encodr_data, DeltaRot, Sensor_noiseModel_cost4));
        graph.add(std::make_shared<FK_factor_graph_cost5>(Symbol('w', 0), Symbol('w', 1), Symbol('w', 2), Symbol('w', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('h', i), Symbol('v', i), Symbol('X', i), uwb_data, DeltaRot, Sensor_noiseModel_cost5));
    ...
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 0), 0.0, Sensor_noiseModel_prior));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 1), 0.0, Sensor_noiseModel_prior));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 2), 0.0, Sensor_noiseModel_prior));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 3), 0.0, Sensor_noiseModel_prior));
