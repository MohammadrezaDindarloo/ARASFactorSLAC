// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl11
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl11(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 636

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (218)
  const Scalar _tmp0 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp1 = Scalar(0.1034955) * p_b(2, 0);
  const Scalar _tmp2 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp4 = 2 * _tmp3;
  const Scalar _tmp5 = _tmp2 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -_tmp9;
  const Scalar _tmp11 = 2 * _tmp2 * _tmp6;
  const Scalar _tmp12 = _tmp3 * _tmp7;
  const Scalar _tmp13 = _tmp11 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp10 + _tmp18;
  const Scalar _tmp20 = _tmp19 + p_init0;
  const Scalar _tmp21 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = _tmp4 * _tmp6;
  const Scalar _tmp25 = _tmp2 * _tmp7;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + p_init1;
  const Scalar _tmp32 = Scalar(1.0) / (fh1);
  const Scalar _tmp33 = _tmp32 * fv1;
  const Scalar _tmp34 = std::asinh(_tmp33);
  const Scalar _tmp35 = Scalar(9.6622558468725703) * _tmp34;
  const Scalar _tmp36 =
      -_tmp35 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_b(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp31 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp37 = Scalar(0.1034955) * _tmp32;
  const Scalar _tmp38 = _tmp36 * _tmp37;
  const Scalar _tmp39 =
      std::pow(Scalar(_tmp0 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = Scalar(1.0) * _tmp34;
  const Scalar _tmp41 = _tmp14 - _tmp17;
  const Scalar _tmp42 = _tmp41 + _tmp9;
  const Scalar _tmp43 = _tmp42 + p_init0;
  const Scalar _tmp44 = _tmp27 - _tmp28;
  const Scalar _tmp45 = _tmp22 + _tmp44;
  const Scalar _tmp46 = _tmp45 + p_init1;
  const Scalar _tmp47 = _tmp31 - p_b(1, 0);
  const Scalar _tmp48 = _tmp20 - p_b(0, 0);
  const Scalar _tmp49 = std::pow(Scalar(std::pow(_tmp47, Scalar(2)) + std::pow(_tmp48, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp50 = _tmp48 * _tmp49;
  const Scalar _tmp51 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp52 = -Scalar(0.010999999999999999) * _tmp15 -
                        Scalar(0.010999999999999999) * _tmp21 + Scalar(-0.010999999999999999);
  const Scalar _tmp53 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp54 = _tmp52 - _tmp53;
  const Scalar _tmp55 = _tmp51 + _tmp54;
  const Scalar _tmp56 = _tmp55 * fh1;
  const Scalar _tmp57 = Scalar(3.29616) * _tmp13 + _tmp19 * fv1 + _tmp50 * _tmp56;
  const Scalar _tmp58 = Scalar(1.0) * _tmp42;
  const Scalar _tmp59 = _tmp10 + _tmp41;
  const Scalar _tmp60 = Scalar(1.0) * _tmp45;
  const Scalar _tmp61 = -_tmp60;
  const Scalar _tmp62 = _tmp23 + _tmp44;
  const Scalar _tmp63 = _tmp61 + _tmp62;
  const Scalar _tmp64 = _tmp22 + _tmp29;
  const Scalar _tmp65 = Scalar(1.0) / (_tmp61 + _tmp64);
  const Scalar _tmp66 = _tmp18 + _tmp9;
  const Scalar _tmp67 = _tmp65 * (_tmp58 - _tmp66);
  const Scalar _tmp68 = _tmp63 * _tmp67;
  const Scalar _tmp69 = Scalar(1.0) / (_tmp58 - _tmp59 - _tmp68);
  const Scalar _tmp70 = Scalar(1.0) * _tmp69;
  const Scalar _tmp71 = _tmp63 * _tmp65;
  const Scalar _tmp72 = _tmp70 * _tmp71;
  const Scalar _tmp73 = -Scalar(1.0) * _tmp70 + Scalar(1.0) * _tmp72;
  const Scalar _tmp74 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp75 = -_tmp51;
  const Scalar _tmp76 = _tmp54 + _tmp75;
  const Scalar _tmp77 = _tmp59 + p_init0;
  const Scalar _tmp78 = _tmp77 - p_a(0, 0);
  const Scalar _tmp79 = _tmp62 + p_init1;
  const Scalar _tmp80 = _tmp79 - p_a(1, 0);
  const Scalar _tmp81 = std::pow(Scalar(std::pow(_tmp78, Scalar(2)) + std::pow(_tmp80, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp82 = _tmp80 * _tmp81;
  const Scalar _tmp83 = _tmp46 - p_d(1, 0);
  const Scalar _tmp84 = _tmp43 - p_d(0, 0);
  const Scalar _tmp85 = Scalar(1.0) / (_tmp84);
  const Scalar _tmp86 = _tmp83 * _tmp85;
  const Scalar _tmp87 = _tmp52 + _tmp53;
  const Scalar _tmp88 = _tmp75 + _tmp87;
  const Scalar _tmp89 = _tmp78 * _tmp81;
  const Scalar _tmp90 = _tmp88 * _tmp89;
  const Scalar _tmp91 = _tmp51 + _tmp87;
  const Scalar _tmp92 = _tmp66 + p_init0;
  const Scalar _tmp93 = _tmp92 - p_c(0, 0);
  const Scalar _tmp94 = _tmp64 + p_init1;
  const Scalar _tmp95 = _tmp94 - p_c(1, 0);
  const Scalar _tmp96 = std::pow(Scalar(std::pow(_tmp93, Scalar(2)) + std::pow(_tmp95, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp97 = _tmp95 * _tmp96;
  const Scalar _tmp98 = _tmp93 * _tmp96;
  const Scalar _tmp99 = _tmp86 * _tmp88;
  const Scalar _tmp100 = _tmp91 * _tmp97 - _tmp98 * _tmp99;
  const Scalar _tmp101 = -_tmp82 + _tmp86 * _tmp89;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp86 * _tmp98 - _tmp97);
  const Scalar _tmp103 = _tmp101 * _tmp102;
  const Scalar _tmp104 = -_tmp100 * _tmp103 + _tmp76 * _tmp82 - _tmp86 * _tmp90;
  const Scalar _tmp105 = _tmp88 * _tmp98 - _tmp91 * _tmp98;
  const Scalar _tmp106 = -_tmp103 * _tmp105 - _tmp104 * _tmp67 - _tmp76 * _tmp89 + _tmp90;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp58 + _tmp60 * _tmp67;
  const Scalar _tmp109 = 0;
  const Scalar _tmp110 = _tmp108 * _tmp69;
  const Scalar _tmp111 = -_tmp104 * _tmp109 - _tmp110 * _tmp63 + _tmp61;
  const Scalar _tmp112 = Scalar(1.0) * _tmp65;
  const Scalar _tmp113 = _tmp47 * _tmp49;
  const Scalar _tmp114 = _tmp113 * _tmp19 - _tmp30 * _tmp50;
  const Scalar _tmp115 =
      std::sqrt(Scalar(std::pow(_tmp83, Scalar(2)) + std::pow(_tmp84, Scalar(2))));
  const Scalar _tmp116 = Scalar(1.0) / (_tmp115);
  const Scalar _tmp117 = _tmp115 * _tmp85;
  const Scalar _tmp118 = _tmp117 * (_tmp116 * _tmp42 * _tmp83 - _tmp116 * _tmp45 * _tmp84);
  const Scalar _tmp119 = _tmp118 * _tmp98 + _tmp64 * _tmp98 - _tmp66 * _tmp97;
  const Scalar _tmp120 = -_tmp103 * _tmp119 + _tmp118 * _tmp89 - _tmp59 * _tmp82 + _tmp62 * _tmp89;
  const Scalar _tmp121 = Scalar(1.0) / (_tmp120);
  const Scalar _tmp122 = Scalar(1.0) * _tmp121;
  const Scalar _tmp123 = _tmp106 * _tmp121;
  const Scalar _tmp124 = _tmp123 * _tmp70;
  const Scalar _tmp125 = _tmp65 * (-_tmp104 * _tmp122 + _tmp124 * _tmp63);
  const Scalar _tmp126 = Scalar(1.0) * _tmp114 * (_tmp124 - Scalar(1.0) * _tmp125);
  const Scalar _tmp127 = Scalar(1.0) * _tmp102;
  const Scalar _tmp128 = _tmp100 * _tmp127;
  const Scalar _tmp129 = -_tmp105 * _tmp127 + _tmp128 * _tmp67;
  const Scalar _tmp130 = _tmp107 * _tmp120;
  const Scalar _tmp131 = _tmp123 * (-_tmp119 * _tmp127 - _tmp129 * _tmp130);
  const Scalar _tmp132 = _tmp63 * _tmp69;
  const Scalar _tmp133 = _tmp129 + _tmp131;
  const Scalar _tmp134 = _tmp104 * _tmp107;
  const Scalar _tmp135 = -_tmp128 + _tmp131 * _tmp132 - _tmp133 * _tmp134;
  const Scalar _tmp136 = Scalar(1.0) * _tmp113 * (-_tmp112 * _tmp135 + _tmp131 * _tmp70);
  const Scalar _tmp137 = -_tmp113 * _tmp56 - Scalar(3.29616) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp138 = _tmp67 * _tmp70;
  const Scalar _tmp139 = _tmp68 * _tmp70 + Scalar(1.0);
  const Scalar _tmp140 = -Scalar(1.0) * _tmp112 * _tmp139 + Scalar(1.0) * _tmp138;
  const Scalar _tmp141 = _tmp102 * _tmp86;
  const Scalar _tmp142 = _tmp100 * _tmp141 + _tmp99;
  const Scalar _tmp143 = _tmp105 * _tmp141 - _tmp142 * _tmp67 - _tmp88;
  const Scalar _tmp144 = _tmp123 * (-_tmp118 + _tmp119 * _tmp141 - _tmp130 * _tmp143);
  const Scalar _tmp145 = _tmp143 + _tmp144;
  const Scalar _tmp146 = _tmp132 * _tmp144 - _tmp134 * _tmp145 + _tmp142;
  const Scalar _tmp147 = Scalar(1.0) * _tmp50 * (-_tmp112 * _tmp146 + _tmp144 * _tmp70);
  const Scalar _tmp148 =
      _tmp126 * fh1 + _tmp136 * fh1 + _tmp137 * _tmp140 + _tmp147 * fh1 + _tmp57 * _tmp73 +
      Scalar(1.0) * _tmp74 * (-_tmp108 * _tmp70 - _tmp111 * _tmp112 + Scalar(1.0));
  const Scalar _tmp149 = _tmp103 * _tmp98;
  const Scalar _tmp150 = _tmp114 * _tmp117 * (-_tmp122 * _tmp149 + _tmp122 * _tmp89);
  const Scalar _tmp151 = _tmp107 * _tmp89;
  const Scalar _tmp152 = _tmp101 * _tmp107;
  const Scalar _tmp153 = _tmp102 * (-_tmp133 * _tmp152 + Scalar(1.0));
  const Scalar _tmp154 = _tmp113 * _tmp117 * (_tmp133 * _tmp151 + _tmp153 * _tmp98);
  const Scalar _tmp155 = _tmp102 * (-_tmp145 * _tmp152 - _tmp86);
  const Scalar _tmp156 = _tmp117 * _tmp50 * (_tmp145 * _tmp151 + _tmp155 * _tmp98 + Scalar(1.0));
  const Scalar _tmp157 = -_tmp117 * _tmp74 * (-_tmp109 * _tmp149 + _tmp109 * _tmp89) -
                         _tmp150 * fh1 - _tmp154 * fh1 - _tmp156 * fh1;
  const Scalar _tmp158 = Scalar(1.0) / (_tmp157);
  const Scalar _tmp159 = std::asinh(_tmp148 * _tmp158);
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp157;
  const Scalar _tmp161 =
      -_tmp159 * _tmp160 - std::sqrt(Scalar(std::pow(Scalar(-_tmp43 + p_d(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp162 = std::pow(_tmp157, Scalar(-2));
  const Scalar _tmp163 = -_tmp150 - _tmp154 - _tmp156;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = Scalar(0.1034955) * _tmp164;
  const Scalar _tmp166 = _tmp113 * _tmp55;
  const Scalar _tmp167 = _tmp50 * _tmp55;
  const Scalar _tmp168 = (-_tmp148 * _tmp164 + _tmp158 * (_tmp126 + _tmp136 - _tmp140 * _tmp166 +
                                                          _tmp147 + _tmp167 * _tmp73)) /
                         std::sqrt(Scalar(std::pow(_tmp148, Scalar(2)) * _tmp162 + 1));
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp170 = Scalar(0.1034955) * _tmp158;
  const Scalar _tmp171 = _tmp161 * _tmp170;
  const Scalar _tmp172 = Scalar(1.0) * _tmp159;
  const Scalar _tmp173 = _tmp113 * _tmp153;
  const Scalar _tmp174 = _tmp109 * _tmp74;
  const Scalar _tmp175 = _tmp155 * _tmp50;
  const Scalar _tmp176 = _tmp114 * _tmp122;
  const Scalar _tmp177 = _tmp176 * fh1;
  const Scalar _tmp178 = -_tmp103 * _tmp174 - _tmp103 * _tmp177 + _tmp173 * fh1 + _tmp175 * fh1;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp181 = _tmp113 * _tmp135 * _tmp65;
  const Scalar _tmp182 = _tmp146 * _tmp50 * _tmp65;
  const Scalar _tmp183 = _tmp139 * _tmp65;
  const Scalar _tmp184 = _tmp57 * _tmp70;
  const Scalar _tmp185 = _tmp114 * _tmp125;
  const Scalar _tmp186 = _tmp111 * _tmp65 * _tmp74 + _tmp137 * _tmp183 + _tmp181 * fh1 +
                         _tmp182 * fh1 - _tmp184 * _tmp71 + _tmp185 * fh1;
  const Scalar _tmp187 = std::asinh(_tmp179 * _tmp186);
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp187;
  const Scalar _tmp189 =
      -_tmp178 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp92 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp94 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp190 = _tmp180 * _tmp189;
  const Scalar _tmp191 = Scalar(1.0) * _tmp187;
  const Scalar _tmp192 = -_tmp103 * _tmp176 + _tmp173 + _tmp175;
  const Scalar _tmp193 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp194 = _tmp192 * _tmp193;
  const Scalar _tmp195 = Scalar(0.1034955) * _tmp194;
  const Scalar _tmp196 =
      (_tmp179 * (-_tmp166 * _tmp183 - _tmp167 * _tmp72 + _tmp181 + _tmp182 + _tmp185) -
       _tmp186 * _tmp194) /
      std::sqrt(Scalar(std::pow(_tmp186, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp197 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp198 = _tmp107 * _tmp145 * _tmp50;
  const Scalar _tmp199 = _tmp107 * _tmp113 * _tmp133;
  const Scalar _tmp200 = _tmp174 + _tmp177 + _tmp198 * fh1 + _tmp199 * fh1;
  const Scalar _tmp201 = Scalar(1.0) / (_tmp200);
  const Scalar _tmp202 = _tmp114 * _tmp124;
  const Scalar _tmp203 = _tmp144 * _tmp50 * _tmp69;
  const Scalar _tmp204 = _tmp113 * _tmp131 * _tmp69;
  const Scalar _tmp205 = _tmp110 * _tmp74 - _tmp137 * _tmp138 + _tmp184 - _tmp202 * fh1 -
                         _tmp203 * fh1 - _tmp204 * fh1;
  const Scalar _tmp206 = std::asinh(_tmp201 * _tmp205);
  const Scalar _tmp207 = Scalar(1.0) * _tmp206;
  const Scalar _tmp208 = std::pow(_tmp200, Scalar(-2));
  const Scalar _tmp209 = _tmp176 + _tmp198 + _tmp199;
  const Scalar _tmp210 = _tmp208 * _tmp209;
  const Scalar _tmp211 =
      (_tmp201 * (_tmp138 * _tmp166 + _tmp167 * _tmp70 - _tmp202 - _tmp203 - _tmp204) -
       _tmp205 * _tmp210) /
      std::sqrt(Scalar(std::pow(_tmp205, Scalar(2)) * _tmp208 + 1));
  const Scalar _tmp212 = Scalar(0.1034955) * _tmp210;
  const Scalar _tmp213 = Scalar(9.6622558468725703) * _tmp200;
  const Scalar _tmp214 =
      -_tmp206 * _tmp213 - std::sqrt(Scalar(std::pow(Scalar(-_tmp77 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp79 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp215 = Scalar(9.6622558468725703) * _tmp209;
  const Scalar _tmp216 = Scalar(0.1034955) * _tmp201;
  const Scalar _tmp217 = _tmp214 * _tmp216;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = -Scalar(9.6622558468725703) * _tmp1 * _tmp32 -
               Scalar(9.6622558468725703) * fh1 *
                   (-_tmp0 * _tmp1 - Scalar(1.0) * _tmp0 * _tmp39 * fv1 * std::sinh(_tmp40) -
                    (-Scalar(0.1034955) * _tmp0 * _tmp36 +
                     _tmp37 * (Scalar(9.6622558468725703) * _tmp33 * _tmp39 - _tmp35)) *
                        std::sinh(_tmp38)) +
               Scalar(9.6622558468725703) * std::cosh(_tmp38) -
               Scalar(9.6622558468725703) * std::cosh(_tmp40);
  _res(1, 0) =
      -_tmp160 * (-_tmp165 * p_d(2, 0) + Scalar(1.0) * _tmp168 * std::sinh(_tmp172) -
                  (-_tmp161 * _tmp165 + _tmp170 * (-_tmp159 * _tmp169 - _tmp160 * _tmp168)) *
                      std::sinh(_tmp171)) -
      _tmp169 * (_tmp170 * p_d(2, 0) - std::cosh(_tmp171) + std::cosh(_tmp172));
  _res(2, 0) = -Scalar(9.6622558468725703) * _tmp192 *
                   (_tmp180 * p_c(2, 0) - std::cosh(_tmp190) + std::cosh(_tmp191)) -
               _tmp197 * (-_tmp195 * p_c(2, 0) + Scalar(1.0) * _tmp196 * std::sinh(_tmp191) -
                          (_tmp180 * (-_tmp188 * _tmp192 - _tmp196 * _tmp197) - _tmp189 * _tmp195) *
                              std::sinh(_tmp190));
  _res(3, 0) =
      -_tmp213 * (Scalar(1.0) * _tmp211 * std::sinh(_tmp207) - _tmp212 * p_a(2, 0) -
                  (-_tmp212 * _tmp214 + _tmp216 * (-_tmp206 * _tmp215 - _tmp211 * _tmp213)) *
                      std::sinh(_tmp217)) -
      _tmp215 * (_tmp216 * p_a(2, 0) + std::cosh(_tmp207) - std::cosh(_tmp217));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
