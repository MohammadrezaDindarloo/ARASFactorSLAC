// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost4_wrt_fv1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     ofset0: Scalar
 *     ofset1: Scalar
 *     ofset2: Scalar
 *     ofset3: Scalar
 *     encoder_data: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost4WrtFv1(
    const Scalar fh1, const Scalar fv1, const Scalar ofset0, const Scalar ofset1,
    const Scalar ofset2, const Scalar ofset3, const Eigen::Matrix<Scalar, 4, 1>& encoder_data,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& DeltaRot, const Eigen::Matrix<Scalar, 3, 1>& position_vector,
    const sym::Rot3<Scalar>& Rot_init, const Scalar epsilon) {
  // Total ops: 580

  // Unused inputs
  (void)ofset0;
  (void)ofset1;
  (void)ofset2;
  (void)ofset3;
  (void)encoder_data;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = -_tmp11 + _tmp9;
  const Scalar _tmp13 = Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = -_tmp13;
  const Scalar _tmp15 = _tmp1 * _tmp8;
  const Scalar _tmp16 = _tmp10 * _tmp3;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 - _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp25 = _tmp10 * _tmp7;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(1.0) * _tmp0 /
                        std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp35 = _tmp22 + _tmp29;
  const Scalar _tmp36 = _tmp35 + position_vector(0, 0);
  const Scalar _tmp37 = _tmp36 - p_b(0, 0);
  const Scalar _tmp38 = _tmp14 + _tmp17 + _tmp6;
  const Scalar _tmp39 = _tmp38 + position_vector(1, 0);
  const Scalar _tmp40 = _tmp39 - p_b(1, 0);
  const Scalar _tmp41 =
      std::sqrt(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp40, Scalar(2))));
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = Scalar(1.0) / (_tmp37);
  const Scalar _tmp44 = _tmp41 * _tmp43;
  const Scalar _tmp45 = _tmp44 * (_tmp35 * _tmp40 * _tmp42 - _tmp37 * _tmp38 * _tmp42);
  const Scalar _tmp46 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp47 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp48 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp49 = _tmp47 + _tmp48;
  const Scalar _tmp50 = _tmp46 + _tmp49;
  const Scalar _tmp51 = _tmp27 + _tmp28;
  const Scalar _tmp52 = _tmp22 + _tmp51;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 - p_c(0, 0);
  const Scalar _tmp55 = _tmp17 + _tmp5;
  const Scalar _tmp56 = _tmp14 + _tmp55;
  const Scalar _tmp57 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp58 = _tmp57 - p_c(1, 0);
  const Scalar _tmp59 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp58, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp60 = _tmp54 * _tmp59;
  const Scalar _tmp61 = _tmp47 - _tmp48;
  const Scalar _tmp62 = _tmp46 + _tmp61;
  const Scalar _tmp63 = _tmp60 * _tmp62;
  const Scalar _tmp64 = -_tmp50 * _tmp60 + _tmp63;
  const Scalar _tmp65 = _tmp40 * _tmp43;
  const Scalar _tmp66 = _tmp58 * _tmp59;
  const Scalar _tmp67 = Scalar(1.0) / (_tmp60 * _tmp65 - _tmp66);
  const Scalar _tmp68 = _tmp65 * _tmp67;
  const Scalar _tmp69 = _tmp62 * _tmp65;
  const Scalar _tmp70 = _tmp50 * _tmp66 - _tmp63 * _tmp65;
  const Scalar _tmp71 = _tmp68 * _tmp70 + _tmp69;
  const Scalar _tmp72 = Scalar(1.0) * _tmp38;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp56 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp35;
  const Scalar _tmp76 = -_tmp52 + _tmp75;
  const Scalar _tmp77 = _tmp74 * _tmp76;
  const Scalar _tmp78 = -_tmp62 + _tmp64 * _tmp68 - _tmp71 * _tmp77;
  const Scalar _tmp79 = -_tmp46;
  const Scalar _tmp80 = _tmp49 + _tmp79;
  const Scalar _tmp81 = _tmp23 + _tmp51;
  const Scalar _tmp82 = _tmp81 + position_vector(0, 0);
  const Scalar _tmp83 = _tmp82 - p_d(0, 0);
  const Scalar _tmp84 = _tmp18 + _tmp5;
  const Scalar _tmp85 = _tmp84 + position_vector(1, 0);
  const Scalar _tmp86 = _tmp85 - p_d(1, 0);
  const Scalar _tmp87 = std::pow(Scalar(std::pow(_tmp83, Scalar(2)) + std::pow(_tmp86, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp88 = _tmp83 * _tmp87;
  const Scalar _tmp89 = _tmp86 * _tmp87;
  const Scalar _tmp90 = _tmp65 * _tmp88 - _tmp89;
  const Scalar _tmp91 = _tmp67 * _tmp90;
  const Scalar _tmp92 = -_tmp69 * _tmp88 - _tmp70 * _tmp91 + _tmp80 * _tmp89;
  const Scalar _tmp93 = _tmp62 * _tmp88 - _tmp64 * _tmp91 - _tmp77 * _tmp92 - _tmp80 * _tmp88;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp45 * _tmp60 - _tmp52 * _tmp66 + _tmp56 * _tmp60;
  const Scalar _tmp96 = _tmp45 * _tmp88 - _tmp81 * _tmp89 + _tmp84 * _tmp88 - _tmp91 * _tmp95;
  const Scalar _tmp97 = _tmp94 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp93 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp45 + _tmp68 * _tmp95 - _tmp78 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp78;
  const Scalar _tmp102 = _tmp90 * _tmp94;
  const Scalar _tmp103 = -_tmp101 * _tmp102 - _tmp65;
  const Scalar _tmp104 = _tmp60 * _tmp67;
  const Scalar _tmp105 = _tmp88 * _tmp94;
  const Scalar _tmp106 = _tmp31 - p_a(0, 0);
  const Scalar _tmp107 = _tmp20 - p_a(1, 0);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = Scalar(1.0) * _tmp67;
  const Scalar _tmp112 = Scalar(1.0) * _tmp74;
  const Scalar _tmp113 = -_tmp111 * _tmp64 + _tmp112 * _tmp67 * _tmp70 * _tmp76;
  const Scalar _tmp114 = _tmp99 * (-_tmp111 * _tmp95 - _tmp113 * _tmp97);
  const Scalar _tmp115 = _tmp113 + _tmp114;
  const Scalar _tmp116 = -_tmp102 * _tmp115 + Scalar(1.0);
  const Scalar _tmp117 = _tmp107 * _tmp108;
  const Scalar _tmp118 = _tmp117 * fh1;
  const Scalar _tmp119 = Scalar(1.0) * _tmp98;
  const Scalar _tmp120 = _tmp60 * _tmp91;
  const Scalar _tmp121 = fh1 * (-_tmp109 * _tmp19 + _tmp117 * _tmp30);
  const Scalar _tmp122 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp123 = _tmp72 * _tmp77 + _tmp75;
  const Scalar _tmp124 = 0;
  const Scalar _tmp125 = _tmp124 * _tmp94;
  const Scalar _tmp126 = _tmp44 * (-_tmp120 * _tmp125 + _tmp125 * _tmp88);
  const Scalar _tmp127 = -_tmp110 * _tmp44 * (_tmp101 * _tmp105 + _tmp103 * _tmp104 + Scalar(1.0)) -
                         _tmp118 * _tmp44 * (_tmp104 * _tmp116 + _tmp105 * _tmp115) -
                         _tmp121 * _tmp44 * (-_tmp119 * _tmp120 + _tmp119 * _tmp88) -
                         _tmp122 * _tmp126;
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = fh1 * (_tmp61 + _tmp79);
  const Scalar _tmp130 = _tmp109 * _tmp129 + Scalar(5.1796800000000003) * _tmp26 + _tmp30 * fv1;
  const Scalar _tmp131 = _tmp73 + _tmp84;
  const Scalar _tmp132 = _tmp131 * _tmp77;
  const Scalar _tmp133 = Scalar(1.0) / (-_tmp132 + _tmp75 - _tmp81);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp131 * _tmp74;
  const Scalar _tmp136 = Scalar(1.0) * _tmp134 * _tmp135 - Scalar(1.0) * _tmp134;
  const Scalar _tmp137 = _tmp92 * _tmp94;
  const Scalar _tmp138 = _tmp123 * _tmp133;
  const Scalar _tmp139 = _tmp74 * (-_tmp124 * _tmp137 - _tmp131 * _tmp138 + _tmp73);
  const Scalar _tmp140 = -Scalar(1.0) * _tmp123 * _tmp134 - Scalar(1.0) * _tmp139 + Scalar(1.0);
  const Scalar _tmp141 = _tmp134 * _tmp99;
  const Scalar _tmp142 = -_tmp119 * _tmp92 + _tmp131 * _tmp141;
  const Scalar _tmp143 = -_tmp117 * _tmp129 - Scalar(5.1796800000000003) * _tmp12 - _tmp19 * fv1;
  const Scalar _tmp144 = _tmp132 * _tmp134 + Scalar(1.0);
  const Scalar _tmp145 = _tmp134 * _tmp77;
  const Scalar _tmp146 = -Scalar(1.0) * _tmp112 * _tmp144 + Scalar(1.0) * _tmp145;
  const Scalar _tmp147 = _tmp131 * _tmp133;
  const Scalar _tmp148 = -_tmp111 * _tmp70 + _tmp114 * _tmp147 - _tmp115 * _tmp137;
  const Scalar _tmp149 = _tmp100 * _tmp147 - _tmp101 * _tmp137 + _tmp71;
  const Scalar _tmp150 = Scalar(1.0) * _tmp110 * (_tmp100 * _tmp134 - _tmp112 * _tmp149) +
                         Scalar(1.0) * _tmp118 * (-_tmp112 * _tmp148 + _tmp114 * _tmp134) +
                         Scalar(1.0) * _tmp121 * (-_tmp112 * _tmp142 + _tmp141) +
                         _tmp122 * _tmp140 + _tmp130 * _tmp136 + _tmp143 * _tmp146;
  const Scalar _tmp151 = std::asinh(_tmp128 * _tmp150);
  const Scalar _tmp152 = Scalar(1.0) * _tmp151;
  const Scalar _tmp153 = _tmp13 + _tmp55;
  const Scalar _tmp154 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp155 = _tmp126 * _tmp154;
  const Scalar _tmp156 =
      (_tmp128 * (_tmp136 * _tmp30 - _tmp140 + _tmp146 * _tmp153) - _tmp150 * _tmp155) /
      std::sqrt(Scalar(std::pow(_tmp150, Scalar(2)) * _tmp154 + 1));
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp159 = Scalar(0.1034955) * _tmp128;
  const Scalar _tmp160 =
      -_tmp151 * _tmp157 - std::sqrt(Scalar(std::pow(Scalar(-_tmp36 + p_b(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp39 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp161 = _tmp159 * _tmp160;
  const Scalar _tmp162 = _tmp119 * _tmp121;
  const Scalar _tmp163 = _tmp122 * _tmp125;
  const Scalar _tmp164 =
      _tmp103 * _tmp110 * _tmp67 + _tmp116 * _tmp118 * _tmp67 - _tmp162 * _tmp91 - _tmp163 * _tmp91;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp144 * _tmp74;
  const Scalar _tmp167 = _tmp130 * _tmp134;
  const Scalar _tmp168 = _tmp110 * _tmp149 * _tmp74 + _tmp118 * _tmp148 * _tmp74 +
                         _tmp121 * _tmp142 * _tmp74 + _tmp122 * _tmp139 - _tmp135 * _tmp167 +
                         _tmp143 * _tmp166;
  const Scalar _tmp169 = std::asinh(_tmp165 * _tmp168);
  const Scalar _tmp170 = Scalar(1.0) * _tmp169;
  const Scalar _tmp171 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp172 = _tmp134 * _tmp30;
  const Scalar _tmp173 = _tmp125 * _tmp171 * _tmp91;
  const Scalar _tmp174 =
      (_tmp165 * (-_tmp135 * _tmp172 - _tmp139 + _tmp153 * _tmp166) - _tmp168 * _tmp173) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp171 + 1));
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp176 =
      -_tmp169 * _tmp175 - std::sqrt(Scalar(std::pow(Scalar(-_tmp53 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp57 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp177 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp178 = _tmp176 * _tmp177;
  const Scalar _tmp179 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp180 = _tmp179 * _tmp91;
  const Scalar _tmp181 =
      _tmp101 * _tmp110 * _tmp94 + _tmp115 * _tmp118 * _tmp94 + _tmp162 + _tmp163;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = -_tmp100 * _tmp110 * _tmp133 - _tmp114 * _tmp118 * _tmp133 -
                         _tmp121 * _tmp141 + _tmp122 * _tmp138 - _tmp143 * _tmp145 + _tmp167;
  const Scalar _tmp184 = std::asinh(_tmp182 * _tmp183);
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp186 =
      -_tmp184 * _tmp185 - std::sqrt(Scalar(std::pow(Scalar(-_tmp82 + p_d(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp85 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp187 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp188 = _tmp125 * _tmp187;
  const Scalar _tmp189 = (_tmp182 * (-_tmp138 - _tmp145 * _tmp153 + _tmp172) + _tmp183 * _tmp188) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp187 + 1));
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp191 = _tmp186 * _tmp190;
  const Scalar _tmp192 = Scalar(1.0) * _tmp184;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
      (-_tmp34 * std::cosh(Scalar(1.0) * _tmp32) +
       _tmp34 * std::cosh(Scalar(0.1034955) * _tmp0 *
                          (-_tmp32 * _tmp33 -
                           std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_a(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp31 + p_a(0, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp157 * (-Scalar(1.0) * _tmp156 * std::cosh(_tmp152) -
                          (-Scalar(0.1034955) * _tmp155 * _tmp160 +
                           _tmp159 * (-_tmp151 * _tmp158 - _tmp156 * _tmp157)) *
                              std::cosh(_tmp161)) +
               _tmp158 * (-std::sinh(_tmp152) - std::sinh(_tmp161));
  _res(2, 0) = _tmp175 * (-Scalar(1.0) * _tmp174 * std::cosh(_tmp170) -
                          (-Scalar(0.1034955) * _tmp173 * _tmp176 +
                           _tmp177 * (-_tmp169 * _tmp180 - _tmp174 * _tmp175)) *
                              std::cosh(_tmp178)) +
               _tmp180 * (-std::sinh(_tmp170) - std::sinh(_tmp178));
  _res(3, 0) = -_tmp179 * (-std::sinh(_tmp191) - std::sinh(_tmp192)) +
               _tmp185 * (-Scalar(1.0) * _tmp189 * std::cosh(_tmp192) -
                          (Scalar(0.1034955) * _tmp186 * _tmp188 +
                           _tmp190 * (_tmp179 * _tmp184 - _tmp185 * _tmp189)) *
                              std::cosh(_tmp191));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
