// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost1(
    const Scalar fh1, const Scalar fv1, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 483

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (163)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp2 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = 2 * _tmp3;
  const Scalar _tmp5 = _tmp2 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -_tmp9;
  const Scalar _tmp11 = _tmp4 * _tmp6;
  const Scalar _tmp12 = _tmp2 * _tmp7;
  const Scalar _tmp13 = _tmp11 - _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 - _tmp17;
  const Scalar _tmp19 = _tmp10 + _tmp18;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = 2 * _tmp2 * _tmp6;
  const Scalar _tmp25 = _tmp3 * _tmp7;
  const Scalar _tmp26 = _tmp24 + _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(0, 0);
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp24 - Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 = -Scalar(0.010999999999999999) * _tmp15 -
                        Scalar(0.010999999999999999) * _tmp21 + Scalar(-0.010999999999999999);
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = _tmp31 - p_a(0, 0);
  const Scalar _tmp41 = _tmp20 - p_a(1, 0);
  const Scalar _tmp42 = std::pow(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp41, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp43 = _tmp40 * _tmp42;
  const Scalar _tmp44 = _tmp39 * fh1;
  const Scalar _tmp45 = Scalar(3.29616) * _tmp26 + _tmp30 * fv1 + _tmp43 * _tmp44;
  const Scalar _tmp46 = _tmp27 + _tmp28;
  const Scalar _tmp47 = _tmp23 + _tmp46;
  const Scalar _tmp48 = _tmp22 + _tmp29;
  const Scalar _tmp49 = Scalar(1.0) * _tmp48;
  const Scalar _tmp50 = _tmp18 + _tmp9;
  const Scalar _tmp51 = Scalar(1.0) * _tmp50;
  const Scalar _tmp52 = -_tmp51;
  const Scalar _tmp53 = _tmp14 + _tmp17;
  const Scalar _tmp54 = _tmp10 + _tmp53;
  const Scalar _tmp55 = _tmp52 + _tmp54;
  const Scalar _tmp56 = _tmp53 + _tmp9;
  const Scalar _tmp57 = Scalar(1.0) / (_tmp52 + _tmp56);
  const Scalar _tmp58 = _tmp22 + _tmp46;
  const Scalar _tmp59 = _tmp49 - _tmp58;
  const Scalar _tmp60 = _tmp57 * _tmp59;
  const Scalar _tmp61 = _tmp55 * _tmp60;
  const Scalar _tmp62 = Scalar(1.0) / (-_tmp47 + _tmp49 - _tmp61);
  const Scalar _tmp63 = Scalar(1.0) * _tmp62;
  const Scalar _tmp64 = _tmp55 * _tmp57;
  const Scalar _tmp65 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp66 = _tmp49 + _tmp51 * _tmp60;
  const Scalar _tmp67 = 0;
  const Scalar _tmp68 = _tmp36 + _tmp37;
  const Scalar _tmp69 = _tmp35 + _tmp68;
  const Scalar _tmp70 = _tmp47 + position_vector(0, 0);
  const Scalar _tmp71 = _tmp70 - p_d(0, 0);
  const Scalar _tmp72 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp73 = _tmp72 - p_d(1, 0);
  const Scalar _tmp74 = std::pow(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp73, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp77 = _tmp76 - p_b(0, 0);
  const Scalar _tmp78 = Scalar(1.0) / (_tmp77);
  const Scalar _tmp79 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp80 = _tmp79 - p_b(1, 0);
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = _tmp34 + _tmp38;
  const Scalar _tmp83 = _tmp58 + position_vector(0, 0);
  const Scalar _tmp84 = _tmp83 - p_c(0, 0);
  const Scalar _tmp85 = _tmp56 + position_vector(1, 0);
  const Scalar _tmp86 = _tmp85 - p_c(1, 0);
  const Scalar _tmp87 = std::pow(Scalar(std::pow(_tmp84, Scalar(2)) + std::pow(_tmp86, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp88 = _tmp84 * _tmp87;
  const Scalar _tmp89 = _tmp82 * _tmp88;
  const Scalar _tmp90 = _tmp34 + _tmp68;
  const Scalar _tmp91 = _tmp86 * _tmp87;
  const Scalar _tmp92 = -_tmp81 * _tmp89 + _tmp90 * _tmp91;
  const Scalar _tmp93 = _tmp71 * _tmp74;
  const Scalar _tmp94 = -_tmp75 + _tmp81 * _tmp93;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp81 * _tmp88 - _tmp91);
  const Scalar _tmp96 = _tmp94 * _tmp95;
  const Scalar _tmp97 = _tmp81 * _tmp82;
  const Scalar _tmp98 = _tmp69 * _tmp75 - _tmp92 * _tmp96 - _tmp93 * _tmp97;
  const Scalar _tmp99 = -_tmp88 * _tmp90 + _tmp89;
  const Scalar _tmp100 = -_tmp60 * _tmp98 - _tmp69 * _tmp93 + _tmp82 * _tmp93 - _tmp96 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp102 = _tmp101 * _tmp98;
  const Scalar _tmp103 = _tmp62 * _tmp66;
  const Scalar _tmp104 = -_tmp102 * _tmp67 - _tmp103 * _tmp55 + _tmp52;
  const Scalar _tmp105 = Scalar(1.0) * _tmp57;
  const Scalar _tmp106 =
      std::sqrt(Scalar(std::pow(_tmp77, Scalar(2)) + std::pow(_tmp80, Scalar(2))));
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp106 * _tmp78;
  const Scalar _tmp109 = _tmp108 * (_tmp107 * _tmp48 * _tmp80 - _tmp107 * _tmp50 * _tmp77);
  const Scalar _tmp110 = _tmp109 * _tmp88 + _tmp56 * _tmp88 - _tmp58 * _tmp91;
  const Scalar _tmp111 = _tmp109 * _tmp93 - _tmp110 * _tmp96 - _tmp47 * _tmp75 + _tmp54 * _tmp93;
  const Scalar _tmp112 = Scalar(1.0) / (_tmp111);
  const Scalar _tmp113 = Scalar(1.0) * _tmp112;
  const Scalar _tmp114 = _tmp100 * _tmp112;
  const Scalar _tmp115 = _tmp114 * _tmp63;
  const Scalar _tmp116 = -_tmp113 * _tmp98 + _tmp115 * _tmp55;
  const Scalar _tmp117 = _tmp41 * _tmp42;
  const Scalar _tmp118 = fh1 * (_tmp117 * _tmp30 - _tmp19 * _tmp43);
  const Scalar _tmp119 = -_tmp117 * _tmp44 - Scalar(3.29616) * _tmp13 - _tmp19 * fv1;
  const Scalar _tmp120 = _tmp61 * _tmp63 + Scalar(1.0);
  const Scalar _tmp121 = Scalar(1.0) * _tmp95;
  const Scalar _tmp122 = _tmp121 * _tmp92;
  const Scalar _tmp123 = -_tmp121 * _tmp99 + _tmp122 * _tmp60;
  const Scalar _tmp124 = _tmp101 * _tmp111;
  const Scalar _tmp125 = _tmp114 * (-_tmp110 * _tmp121 - _tmp123 * _tmp124);
  const Scalar _tmp126 = _tmp55 * _tmp62;
  const Scalar _tmp127 = _tmp123 + _tmp125;
  const Scalar _tmp128 = -_tmp102 * _tmp127 - _tmp122 + _tmp125 * _tmp126;
  const Scalar _tmp129 = _tmp117 * fh1;
  const Scalar _tmp130 = _tmp81 * _tmp95;
  const Scalar _tmp131 = _tmp130 * _tmp92 + _tmp97;
  const Scalar _tmp132 = _tmp130 * _tmp99 - _tmp131 * _tmp60 - _tmp82;
  const Scalar _tmp133 = _tmp114 * (-_tmp109 + _tmp110 * _tmp130 - _tmp124 * _tmp132);
  const Scalar _tmp134 = _tmp132 + _tmp133;
  const Scalar _tmp135 = -_tmp102 * _tmp134 + _tmp126 * _tmp133 + _tmp131;
  const Scalar _tmp136 = _tmp43 * fh1;
  const Scalar _tmp137 = _tmp101 * _tmp94;
  const Scalar _tmp138 = -_tmp134 * _tmp137 - _tmp81;
  const Scalar _tmp139 = _tmp88 * _tmp95;
  const Scalar _tmp140 = _tmp101 * _tmp93;
  const Scalar _tmp141 = -_tmp127 * _tmp137 + Scalar(1.0);
  const Scalar _tmp142 = _tmp88 * _tmp96;
  const Scalar _tmp143 = _tmp101 * _tmp67;
  const Scalar _tmp144 = -_tmp108 * _tmp118 * (-_tmp113 * _tmp142 + _tmp113 * _tmp93) -
                         _tmp108 * _tmp129 * (_tmp127 * _tmp140 + _tmp139 * _tmp141) -
                         _tmp108 * _tmp136 * (_tmp134 * _tmp140 + _tmp138 * _tmp139 + Scalar(1.0)) -
                         _tmp108 * _tmp65 * (-_tmp142 * _tmp143 + _tmp143 * _tmp93);
  const Scalar _tmp145 = Scalar(1.0) / (_tmp144);
  const Scalar _tmp146 = std::asinh(
      _tmp145 * (Scalar(1.0) * _tmp118 * (-_tmp105 * _tmp116 + _tmp115) +
                 Scalar(1.0) * _tmp119 * (-_tmp105 * _tmp120 + _tmp60 * _tmp63) +
                 Scalar(1.0) * _tmp129 * (-_tmp105 * _tmp128 + _tmp125 * _tmp63) +
                 Scalar(1.0) * _tmp136 * (-_tmp105 * _tmp135 + _tmp133 * _tmp63) +
                 Scalar(1.0) * _tmp45 * (_tmp63 * _tmp64 - _tmp63) +
                 Scalar(1.0) * _tmp65 * (-_tmp104 * _tmp105 - _tmp63 * _tmp66 + Scalar(1.0))));
  const Scalar _tmp147 = Scalar(9.6622558468725703) * _tmp144;
  const Scalar _tmp148 = Scalar(0.1034955) * _tmp145;
  const Scalar _tmp149 = _tmp113 * _tmp118;
  const Scalar _tmp150 = _tmp143 * _tmp65;
  const Scalar _tmp151 =
      _tmp129 * _tmp141 * _tmp95 + _tmp136 * _tmp138 * _tmp95 - _tmp149 * _tmp96 - _tmp150 * _tmp96;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 = _tmp119 * _tmp57;
  const Scalar _tmp154 = _tmp45 * _tmp63;
  const Scalar _tmp155 = std::asinh(
      _tmp152 * (_tmp104 * _tmp57 * _tmp65 + _tmp116 * _tmp118 * _tmp57 + _tmp120 * _tmp153 +
                 _tmp128 * _tmp129 * _tmp57 + _tmp135 * _tmp136 * _tmp57 - _tmp154 * _tmp64));
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp152;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp151;
  const Scalar _tmp158 =
      _tmp101 * _tmp127 * _tmp129 + _tmp101 * _tmp134 * _tmp136 + _tmp149 + _tmp150;
  const Scalar _tmp159 = Scalar(1.0) / (_tmp158);
  const Scalar _tmp160 =
      std::asinh(_tmp159 * (_tmp103 * _tmp65 - _tmp115 * _tmp118 - _tmp125 * _tmp129 * _tmp62 -
                            _tmp133 * _tmp136 * _tmp62 - _tmp153 * _tmp59 * _tmp63 + _tmp154));
  const Scalar _tmp161 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp162 = Scalar(9.6622558468725703) * _tmp158;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 * (_tmp1 * p_a(2, 0) + std::cosh(Scalar(1.0) * _tmp32) -
                 std::cosh(_tmp1 *
                           (-_tmp32 * _tmp33 -
                            std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_a(1, 0)), Scalar(2)) +
                                             std::pow(Scalar(-_tmp31 + p_a(0, 0)), Scalar(2))))))) +
      _tmp39 + position_vector(2, 0);
  _res(1, 0) =
      -_tmp147 *
          (_tmp148 * p_b(2, 0) + std::cosh(Scalar(1.0) * _tmp146) -
           std::cosh(_tmp148 *
                     (-_tmp146 * _tmp147 -
                      std::sqrt(Scalar(std::pow(Scalar(-_tmp76 + p_b(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp79 + p_b(1, 0)), Scalar(2))))))) +
      _tmp82 + position_vector(2, 0);
  _res(2, 0) =
      -_tmp157 *
          (_tmp156 * p_c(2, 0) + std::cosh(Scalar(1.0) * _tmp155) -
           std::cosh(_tmp156 *
                     (-_tmp155 * _tmp157 -
                      std::sqrt(Scalar(std::pow(Scalar(-_tmp83 + p_c(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp85 + p_c(1, 0)), Scalar(2))))))) +
      _tmp90 + position_vector(2, 0);
  _res(3, 0) =
      -_tmp162 *
          (_tmp161 * p_d(2, 0) + std::cosh(Scalar(1.0) * _tmp160) -
           std::cosh(_tmp161 *
                     (-_tmp160 * _tmp162 -
                      std::sqrt(Scalar(std::pow(Scalar(-_tmp70 + p_d(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp72 + p_d(1, 0)), Scalar(2))))))) +
      _tmp69 + position_vector(2, 0);

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
