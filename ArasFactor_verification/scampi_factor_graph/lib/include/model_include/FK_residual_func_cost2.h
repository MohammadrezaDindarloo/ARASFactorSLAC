// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost2
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost2(
    const Scalar fh1, const Scalar fv1, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 494

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (162)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp13 = _tmp3 * _tmp8;
  const Scalar _tmp14 = _tmp1 * _tmp10;
  const Scalar _tmp15 = _tmp13 - _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = std::pow(Scalar(-_tmp19 + p_a(1, 0)), Scalar(2)) +
                        std::pow(Scalar(-_tmp30 + p_a(0, 0)), Scalar(2));
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 = -Scalar(0.010999999999999999) * _tmp2 -
                        Scalar(0.010999999999999999) * _tmp20 + Scalar(-0.010999999999999999);
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = _tmp21 + _tmp28;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp41 - p_b(0, 0);
  const Scalar _tmp43 = _tmp12 + _tmp16;
  const Scalar _tmp44 = _tmp43 + _tmp6;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 - p_b(1, 0);
  const Scalar _tmp47 =
      std::sqrt(Scalar(std::pow(_tmp42, Scalar(2)) + std::pow(_tmp46, Scalar(2))));
  const Scalar _tmp48 = Scalar(1.0) / (_tmp47);
  const Scalar _tmp49 = Scalar(1.0) / (_tmp42);
  const Scalar _tmp50 = _tmp47 * _tmp49;
  const Scalar _tmp51 = _tmp50 * (_tmp40 * _tmp46 * _tmp48 - _tmp42 * _tmp44 * _tmp48);
  const Scalar _tmp52 = _tmp36 + _tmp37;
  const Scalar _tmp53 = _tmp34 + _tmp52;
  const Scalar _tmp54 = _tmp26 + _tmp27;
  const Scalar _tmp55 = _tmp21 + _tmp54;
  const Scalar _tmp56 = _tmp55 + position_vector(0, 0);
  const Scalar _tmp57 = _tmp56 - p_c(0, 0);
  const Scalar _tmp58 = _tmp43 + _tmp5;
  const Scalar _tmp59 = _tmp58 + position_vector(1, 0);
  const Scalar _tmp60 = _tmp59 - p_c(1, 0);
  const Scalar _tmp61 = std::pow(Scalar(std::pow(_tmp57, Scalar(2)) + std::pow(_tmp60, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp62 = _tmp57 * _tmp61;
  const Scalar _tmp63 = _tmp34 + _tmp38;
  const Scalar _tmp64 = _tmp62 * _tmp63;
  const Scalar _tmp65 = -_tmp53 * _tmp62 + _tmp64;
  const Scalar _tmp66 = _tmp46 * _tmp49;
  const Scalar _tmp67 = _tmp60 * _tmp61;
  const Scalar _tmp68 = Scalar(1.0) / (_tmp62 * _tmp66 - _tmp67);
  const Scalar _tmp69 = _tmp66 * _tmp68;
  const Scalar _tmp70 = _tmp53 * _tmp67 - _tmp64 * _tmp66;
  const Scalar _tmp71 = _tmp63 * _tmp66 + _tmp69 * _tmp70;
  const Scalar _tmp72 = Scalar(1.0) * _tmp44;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp58 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp40;
  const Scalar _tmp76 = _tmp74 * (-_tmp55 + _tmp75);
  const Scalar _tmp77 = -_tmp63 + _tmp65 * _tmp69 - _tmp71 * _tmp76;
  const Scalar _tmp78 = _tmp35 + _tmp52;
  const Scalar _tmp79 = _tmp22 + _tmp54;
  const Scalar _tmp80 = _tmp79 + position_vector(0, 0);
  const Scalar _tmp81 = _tmp80 - p_d(0, 0);
  const Scalar _tmp82 = _tmp17 + _tmp5;
  const Scalar _tmp83 = _tmp82 + position_vector(1, 0);
  const Scalar _tmp84 = _tmp83 - p_d(1, 0);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp81 * _tmp85;
  const Scalar _tmp87 = _tmp63 * _tmp86;
  const Scalar _tmp88 = _tmp84 * _tmp85;
  const Scalar _tmp89 = _tmp66 * _tmp86 - _tmp88;
  const Scalar _tmp90 = _tmp68 * _tmp89;
  const Scalar _tmp91 = -_tmp66 * _tmp87 - _tmp70 * _tmp90 + _tmp78 * _tmp88;
  const Scalar _tmp92 = -_tmp65 * _tmp90 - _tmp76 * _tmp91 - _tmp78 * _tmp86 + _tmp87;
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 = _tmp51 * _tmp62 - _tmp55 * _tmp67 + _tmp58 * _tmp62;
  const Scalar _tmp95 = _tmp51 * _tmp86 - _tmp79 * _tmp88 + _tmp82 * _tmp86 - _tmp90 * _tmp94;
  const Scalar _tmp96 = _tmp93 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp92 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp51 + _tmp69 * _tmp94 - _tmp77 * _tmp96);
  const Scalar _tmp100 = _tmp77 + _tmp99;
  const Scalar _tmp101 = _tmp89 * _tmp93;
  const Scalar _tmp102 = -_tmp100 * _tmp101 - _tmp66;
  const Scalar _tmp103 = _tmp62 * _tmp68;
  const Scalar _tmp104 = _tmp86 * _tmp93;
  const Scalar _tmp105 = _tmp30 - p_a(0, 0);
  const Scalar _tmp106 = _tmp19 - p_a(1, 0);
  const Scalar _tmp107 =
      std::pow(Scalar(std::pow(_tmp105, Scalar(2)) + std::pow(_tmp106, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp108 * fh1;
  const Scalar _tmp110 = Scalar(1.0) * _tmp68;
  const Scalar _tmp111 = _tmp110 * _tmp70;
  const Scalar _tmp112 = -_tmp110 * _tmp65 + _tmp111 * _tmp76;
  const Scalar _tmp113 = _tmp98 * (-_tmp110 * _tmp94 - _tmp112 * _tmp96);
  const Scalar _tmp114 = _tmp112 + _tmp113;
  const Scalar _tmp115 = -_tmp101 * _tmp114 + Scalar(1.0);
  const Scalar _tmp116 = _tmp106 * _tmp107;
  const Scalar _tmp117 = _tmp116 * fh1;
  const Scalar _tmp118 = Scalar(1.0) * _tmp97;
  const Scalar _tmp119 = _tmp62 * _tmp90;
  const Scalar _tmp120 = fh1 * (-_tmp108 * _tmp18 + _tmp116 * _tmp29);
  const Scalar _tmp121 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp122 = _tmp72 * _tmp76 + _tmp75;
  const Scalar _tmp123 = 0;
  const Scalar _tmp124 = _tmp123 * _tmp93;
  const Scalar _tmp125 = -_tmp109 * _tmp50 * (_tmp100 * _tmp104 + _tmp102 * _tmp103 + Scalar(1.0)) -
                         _tmp117 * _tmp50 * (_tmp103 * _tmp115 + _tmp104 * _tmp114) -
                         _tmp120 * _tmp50 * (-_tmp118 * _tmp119 + _tmp118 * _tmp86) -
                         _tmp121 * _tmp50 * (-_tmp119 * _tmp124 + _tmp124 * _tmp86);
  const Scalar _tmp126 = Scalar(1.0) / (_tmp125);
  const Scalar _tmp127 = std::pow(Scalar(-_tmp41 + p_b(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp45 + p_b(1, 0)), Scalar(2));
  const Scalar _tmp128 = _tmp39 * fh1;
  const Scalar _tmp129 = _tmp108 * _tmp128 + Scalar(3.29616) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp130 = _tmp73 + _tmp82;
  const Scalar _tmp131 = _tmp130 * _tmp76;
  const Scalar _tmp132 = Scalar(1.0) / (-_tmp131 + _tmp75 - _tmp79);
  const Scalar _tmp133 = Scalar(1.0) * _tmp132;
  const Scalar _tmp134 = _tmp130 * _tmp74;
  const Scalar _tmp135 = _tmp91 * _tmp93;
  const Scalar _tmp136 = _tmp122 * _tmp132;
  const Scalar _tmp137 = _tmp74 * (-_tmp123 * _tmp135 - _tmp130 * _tmp136 + _tmp73);
  const Scalar _tmp138 = _tmp133 * _tmp98;
  const Scalar _tmp139 = -_tmp118 * _tmp91 + _tmp130 * _tmp138;
  const Scalar _tmp140 = Scalar(1.0) * _tmp74;
  const Scalar _tmp141 = -_tmp116 * _tmp128 - Scalar(3.29616) * _tmp15 - _tmp18 * fv1;
  const Scalar _tmp142 = _tmp131 * _tmp133 + Scalar(1.0);
  const Scalar _tmp143 = _tmp133 * _tmp76;
  const Scalar _tmp144 = _tmp130 * _tmp132;
  const Scalar _tmp145 = -_tmp111 + _tmp113 * _tmp144 - _tmp114 * _tmp135;
  const Scalar _tmp146 = -_tmp100 * _tmp135 + _tmp144 * _tmp99 + _tmp71;
  const Scalar _tmp147 =
      std::asinh(_tmp126 * (Scalar(1.0) * _tmp109 * (_tmp133 * _tmp99 - _tmp140 * _tmp146) +
                            Scalar(1.0) * _tmp117 * (_tmp113 * _tmp133 - _tmp140 * _tmp145) +
                            Scalar(1.0) * _tmp120 * (_tmp138 - _tmp139 * _tmp140) +
                            Scalar(1.0) * _tmp121 *
                                (-_tmp122 * _tmp133 - Scalar(1.0) * _tmp137 + Scalar(1.0)) +
                            Scalar(1.0) * _tmp129 * (_tmp133 * _tmp134 - _tmp133) +
                            Scalar(1.0) * _tmp141 * (-_tmp140 * _tmp142 + _tmp143)));
  const Scalar _tmp148 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp149 = _tmp118 * _tmp120;
  const Scalar _tmp150 = _tmp121 * _tmp124;
  const Scalar _tmp151 =
      _tmp102 * _tmp109 * _tmp68 + _tmp115 * _tmp117 * _tmp68 - _tmp149 * _tmp90 - _tmp150 * _tmp90;
  const Scalar _tmp152 = Scalar(1.0) / (_tmp151);
  const Scalar _tmp153 = _tmp129 * _tmp133;
  const Scalar _tmp154 =
      std::asinh(_tmp152 * (_tmp109 * _tmp146 * _tmp74 + _tmp117 * _tmp145 * _tmp74 +
                            _tmp120 * _tmp139 * _tmp74 + _tmp121 * _tmp137 - _tmp134 * _tmp153 +
                            _tmp141 * _tmp142 * _tmp74));
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp151;
  const Scalar _tmp156 = std::pow(Scalar(-_tmp56 + p_c(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp59 + p_c(1, 0)), Scalar(2));
  const Scalar _tmp157 =
      _tmp100 * _tmp109 * _tmp93 + _tmp114 * _tmp117 * _tmp93 + _tmp149 + _tmp150;
  const Scalar _tmp158 = Scalar(1.0) / (_tmp157);
  const Scalar _tmp159 =
      std::asinh(_tmp158 * (-_tmp109 * _tmp132 * _tmp99 - _tmp113 * _tmp117 * _tmp132 -
                            _tmp120 * _tmp138 + _tmp121 * _tmp136 - _tmp141 * _tmp143 + _tmp153));
  const Scalar _tmp160 = std::pow(Scalar(-_tmp80 + p_d(0, 0)), Scalar(2)) +
                         std::pow(Scalar(-_tmp83 + p_d(1, 0)), Scalar(2));
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp157;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 * (-std::sinh(Scalar(1.0) * _tmp32) -
                std::sinh(Scalar(0.1034955) * _tmp0 * (-std::sqrt(_tmp31) - _tmp32 * _tmp33))) -
      std::sqrt(Scalar(_tmp31 +
                       std::pow(Scalar(-_tmp39 + p_a(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(1, 0) =
      _tmp148 *
          (-std::sinh(Scalar(1.0) * _tmp147) -
           std::sinh(Scalar(0.1034955) * _tmp126 * (-std::sqrt(_tmp127) - _tmp147 * _tmp148))) -
      std::sqrt(Scalar(_tmp127 +
                       std::pow(Scalar(-_tmp63 + p_b(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(2, 0) =
      _tmp155 *
          (-std::sinh(Scalar(1.0) * _tmp154) -
           std::sinh(Scalar(0.1034955) * _tmp152 * (-_tmp154 * _tmp155 - std::sqrt(_tmp156)))) -
      std::sqrt(Scalar(_tmp156 +
                       std::pow(Scalar(-_tmp53 + p_c(2, 0) - position_vector(2, 0)), Scalar(2))));
  _res(3, 0) =
      _tmp161 *
          (-std::sinh(Scalar(1.0) * _tmp159) -
           std::sinh(Scalar(0.1034955) * _tmp158 * (-_tmp159 * _tmp161 - std::sqrt(_tmp160)))) -
      std::sqrt(Scalar(_tmp160 +
                       std::pow(Scalar(-_tmp78 + p_d(2, 0) - position_vector(2, 0)), Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
