// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/pose3.h>
#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost4_wrt_fv1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     ofset0: Scalar
 *     ofset1: Scalar
 *     ofset2: Scalar
 *     ofset3: Scalar
 *     distance_measure: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     TransformationMatrix: Pose3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost4WrtFv1(
    const Scalar fh1, const Scalar fv1, const Scalar ofset0, const Scalar ofset1,
    const Scalar ofset2, const Scalar ofset3, const Eigen::Matrix<Scalar, 4, 1>& distance_measure,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& DeltaRot, const sym::Pose3<Scalar>& TransformationMatrix,
    const Scalar epsilon) {
  // Total ops: 578

  // Unused inputs
  (void)ofset0;
  (void)ofset1;
  (void)ofset2;
  (void)ofset3;
  (void)distance_measure;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 7, 1>& _TransformationMatrix = TransformationMatrix.Data();

  // Intermediate terms (192)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 =
      _DeltaRot[0] * _TransformationMatrix[3] - _DeltaRot[1] * _TransformationMatrix[2] +
      _DeltaRot[2] * _TransformationMatrix[1] + _DeltaRot[3] * _TransformationMatrix[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 =
      -_DeltaRot[0] * _TransformationMatrix[1] + _DeltaRot[1] * _TransformationMatrix[0] +
      _DeltaRot[2] * _TransformationMatrix[3] + _DeltaRot[3] * _TransformationMatrix[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 =
      _DeltaRot[0] * _TransformationMatrix[2] + _DeltaRot[1] * _TransformationMatrix[3] -
      _DeltaRot[2] * _TransformationMatrix[0] + _DeltaRot[3] * _TransformationMatrix[1];
  const Scalar _tmp10 = 2 * _tmp3 * _tmp9;
  const Scalar _tmp11 =
      -2 * _DeltaRot[0] * _TransformationMatrix[0] - 2 * _DeltaRot[1] * _TransformationMatrix[1] -
      2 * _DeltaRot[2] * _TransformationMatrix[2] + 2 * _DeltaRot[3] * _TransformationMatrix[3];
  const Scalar _tmp12 = _tmp11 * _tmp5;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp5;
  const Scalar _tmp15 = _tmp14 * _tmp9;
  const Scalar _tmp16 = _tmp11 * _tmp3;
  const Scalar _tmp17 = _tmp15 - _tmp16;
  const Scalar _tmp18 = Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = -_tmp13 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _TransformationMatrix[5] + _tmp21;
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp24 = -_tmp23;
  const Scalar _tmp25 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp27 = _tmp14 * _tmp3;
  const Scalar _tmp28 = _tmp11 * _tmp9;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = -Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = -_tmp26 + _tmp30;
  const Scalar _tmp32 = _tmp24 + _tmp31;
  const Scalar _tmp33 = _TransformationMatrix[4] + _tmp32;
  const Scalar _tmp34 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp35 = _tmp23 + _tmp31;
  const Scalar _tmp36 = _tmp26 + _tmp30;
  const Scalar _tmp37 = _tmp24 + _tmp36;
  const Scalar _tmp38 = Scalar(1.0) * _tmp37;
  const Scalar _tmp39 = _tmp13 + _tmp19 + _tmp8;
  const Scalar _tmp40 = Scalar(1.0) * _tmp39;
  const Scalar _tmp41 = -_tmp40;
  const Scalar _tmp42 = _tmp20 + _tmp7;
  const Scalar _tmp43 = _tmp41 + _tmp42;
  const Scalar _tmp44 = _tmp13 + _tmp7;
  const Scalar _tmp45 = _tmp19 + _tmp44;
  const Scalar _tmp46 = Scalar(1.0) / (_tmp41 + _tmp45);
  const Scalar _tmp47 = _tmp23 + _tmp36;
  const Scalar _tmp48 = _tmp46 * (_tmp38 - _tmp47);
  const Scalar _tmp49 = _tmp43 * _tmp48;
  const Scalar _tmp50 = Scalar(1.0) / (-_tmp35 + _tmp38 - _tmp49);
  const Scalar _tmp51 = Scalar(1.0) * _tmp50;
  const Scalar _tmp52 = _TransformationMatrix[5] + _tmp42;
  const Scalar _tmp53 = _tmp52 - p_d(1, 0);
  const Scalar _tmp54 = _TransformationMatrix[4] + _tmp35;
  const Scalar _tmp55 = _tmp54 - p_d(0, 0);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp53 * _tmp56;
  const Scalar _tmp58 = _tmp55 * _tmp56;
  const Scalar _tmp59 = _TransformationMatrix[4] + _tmp47;
  const Scalar _tmp60 = _tmp59 - p_c(0, 0);
  const Scalar _tmp61 = _TransformationMatrix[5] + _tmp45;
  const Scalar _tmp62 = _tmp61 - p_c(1, 0);
  const Scalar _tmp63 = std::pow(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp62, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp64 = _tmp62 * _tmp63;
  const Scalar _tmp65 = _tmp60 * _tmp63;
  const Scalar _tmp66 = _TransformationMatrix[5] + _tmp39;
  const Scalar _tmp67 = _tmp66 - p_b(1, 0);
  const Scalar _tmp68 = _TransformationMatrix[4] + _tmp37;
  const Scalar _tmp69 = _tmp68 - p_b(0, 0);
  const Scalar _tmp70 =
      std::sqrt(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp69, Scalar(2))));
  const Scalar _tmp71 = Scalar(1.0) / (_tmp70);
  const Scalar _tmp72 = Scalar(1.0) / (_tmp69);
  const Scalar _tmp73 = _tmp70 * _tmp72;
  const Scalar _tmp74 = _tmp73 * (_tmp37 * _tmp67 * _tmp71 - _tmp39 * _tmp69 * _tmp71);
  const Scalar _tmp75 = _tmp45 * _tmp65 - _tmp47 * _tmp64 + _tmp65 * _tmp74;
  const Scalar _tmp76 = _tmp67 * _tmp72;
  const Scalar _tmp77 = Scalar(1.0) / (-_tmp64 + _tmp65 * _tmp76);
  const Scalar _tmp78 = -_tmp57 + _tmp58 * _tmp76;
  const Scalar _tmp79 = _tmp77 * _tmp78;
  const Scalar _tmp80 = -_tmp35 * _tmp57 + _tmp42 * _tmp58 + _tmp58 * _tmp74 - _tmp75 * _tmp79;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp80);
  const Scalar _tmp82 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp83 = -Scalar(0.010999999999999999) * _tmp25 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp84 = Scalar(0.20999999999999999) * _tmp27 - Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp85 = _tmp83 - _tmp84;
  const Scalar _tmp86 = _tmp82 + _tmp85;
  const Scalar _tmp87 = -_tmp82;
  const Scalar _tmp88 = _tmp83 + _tmp84;
  const Scalar _tmp89 = _tmp87 + _tmp88;
  const Scalar _tmp90 = _tmp58 * _tmp89;
  const Scalar _tmp91 = _tmp82 + _tmp88;
  const Scalar _tmp92 = _tmp65 * _tmp89 - _tmp65 * _tmp91;
  const Scalar _tmp93 = _tmp76 * _tmp89;
  const Scalar _tmp94 = _tmp64 * _tmp91 - _tmp65 * _tmp93;
  const Scalar _tmp95 = _tmp57 * _tmp86 - _tmp76 * _tmp90 - _tmp79 * _tmp94;
  const Scalar _tmp96 = -_tmp48 * _tmp95 - _tmp58 * _tmp86 - _tmp79 * _tmp92 + _tmp90;
  const Scalar _tmp97 = _tmp81 * _tmp96;
  const Scalar _tmp98 = _tmp51 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) * _tmp81;
  const Scalar _tmp100 = _tmp43 * _tmp98 - _tmp95 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) * _tmp46;
  const Scalar _tmp102 = _tmp33 - p_a(0, 0);
  const Scalar _tmp103 = _tmp22 - p_a(1, 0);
  const Scalar _tmp104 =
      std::pow(Scalar(std::pow(_tmp102, Scalar(2)) + std::pow(_tmp103, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp105 = _tmp103 * _tmp104;
  const Scalar _tmp106 = _tmp102 * _tmp104;
  const Scalar _tmp107 = fh1 * (_tmp105 * _tmp32 - _tmp106 * _tmp21);
  const Scalar _tmp108 = _tmp76 * _tmp77;
  const Scalar _tmp109 = _tmp108 * _tmp94 + _tmp93;
  const Scalar _tmp110 = _tmp108 * _tmp92 - _tmp109 * _tmp48 - _tmp89;
  const Scalar _tmp111 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp112 = _tmp111 * _tmp80;
  const Scalar _tmp113 = _tmp97 * (_tmp108 * _tmp75 - _tmp110 * _tmp112 - _tmp74);
  const Scalar _tmp114 = _tmp110 + _tmp113;
  const Scalar _tmp115 = _tmp111 * _tmp95;
  const Scalar _tmp116 = _tmp43 * _tmp50;
  const Scalar _tmp117 = _tmp109 + _tmp113 * _tmp116 - _tmp114 * _tmp115;
  const Scalar _tmp118 = _tmp106 * fh1;
  const Scalar _tmp119 = Scalar(1.0) * _tmp77;
  const Scalar _tmp120 = _tmp119 * _tmp94;
  const Scalar _tmp121 = -_tmp119 * _tmp92 + _tmp120 * _tmp48;
  const Scalar _tmp122 = _tmp97 * (-_tmp112 * _tmp121 - _tmp119 * _tmp75);
  const Scalar _tmp123 = _tmp121 + _tmp122;
  const Scalar _tmp124 = -_tmp115 * _tmp123 + _tmp116 * _tmp122 - _tmp120;
  const Scalar _tmp125 = _tmp105 * fh1;
  const Scalar _tmp126 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp127 = _tmp38 + _tmp40 * _tmp48;
  const Scalar _tmp128 = 0;
  const Scalar _tmp129 = _tmp127 * _tmp50;
  const Scalar _tmp130 = _tmp46 * (-_tmp115 * _tmp128 - _tmp129 * _tmp43 + _tmp41);
  const Scalar _tmp131 = -Scalar(1.0) * _tmp127 * _tmp51 - Scalar(1.0) * _tmp130 + Scalar(1.0);
  const Scalar _tmp132 = fh1 * (_tmp85 + _tmp87);
  const Scalar _tmp133 = _tmp106 * _tmp132 + Scalar(3.29616) * _tmp29 + _tmp32 * fv1;
  const Scalar _tmp134 = _tmp43 * _tmp46;
  const Scalar _tmp135 = Scalar(1.0) * _tmp134 * _tmp51 - Scalar(1.0) * _tmp51;
  const Scalar _tmp136 = -_tmp105 * _tmp132 - Scalar(3.29616) * _tmp17 - _tmp21 * fv1;
  const Scalar _tmp137 = _tmp48 * _tmp51;
  const Scalar _tmp138 = _tmp46 * (_tmp49 * _tmp51 + Scalar(1.0));
  const Scalar _tmp139 = Scalar(1.0) * _tmp137 - Scalar(1.0) * _tmp138;
  const Scalar _tmp140 = Scalar(1.0) * _tmp107 * (-_tmp100 * _tmp101 + _tmp98) +
                         Scalar(1.0) * _tmp118 * (-_tmp101 * _tmp117 + _tmp113 * _tmp51) +
                         Scalar(1.0) * _tmp125 * (-_tmp101 * _tmp124 + _tmp122 * _tmp51) +
                         _tmp126 * _tmp131 + _tmp133 * _tmp135 + _tmp136 * _tmp139;
  const Scalar _tmp141 = _tmp111 * _tmp128;
  const Scalar _tmp142 = _tmp141 * _tmp79;
  const Scalar _tmp143 = _tmp73 * (_tmp141 * _tmp58 - _tmp142 * _tmp65);
  const Scalar _tmp144 = _tmp111 * _tmp78;
  const Scalar _tmp145 = -_tmp123 * _tmp144 + Scalar(1.0);
  const Scalar _tmp146 = _tmp65 * _tmp77;
  const Scalar _tmp147 = _tmp111 * _tmp58;
  const Scalar _tmp148 = -_tmp114 * _tmp144 - _tmp76;
  const Scalar _tmp149 = _tmp119 * _tmp78 * _tmp81;
  const Scalar _tmp150 = -_tmp107 * _tmp73 * (-_tmp149 * _tmp65 + _tmp58 * _tmp99) -
                         _tmp118 * _tmp73 * (_tmp114 * _tmp147 + _tmp146 * _tmp148 + Scalar(1.0)) -
                         _tmp125 * _tmp73 * (_tmp123 * _tmp147 + _tmp145 * _tmp146) -
                         _tmp126 * _tmp143;
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = std::asinh(_tmp140 * _tmp151);
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp143;
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp150;
  const Scalar _tmp155 = _tmp18 + _tmp44;
  const Scalar _tmp156 = std::pow(_tmp150, Scalar(-2));
  const Scalar _tmp157 = _tmp143 * _tmp156;
  const Scalar _tmp158 =
      (-_tmp140 * _tmp157 + _tmp151 * (-_tmp131 + _tmp135 * _tmp32 + _tmp139 * _tmp155)) /
      std::sqrt(Scalar(std::pow(_tmp140, Scalar(2)) * _tmp156 + 1));
  const Scalar _tmp159 = Scalar(0.1034955) * _tmp151;
  const Scalar _tmp160 =
      -_tmp152 * _tmp154 - std::sqrt(Scalar(std::pow(Scalar(-_tmp66 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp68 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp161 = _tmp159 * _tmp160;
  const Scalar _tmp162 = Scalar(1.0) * _tmp152;
  const Scalar _tmp163 = _tmp133 * _tmp51;
  const Scalar _tmp164 = _tmp100 * _tmp107 * _tmp46 + _tmp117 * _tmp118 * _tmp46 +
                         _tmp124 * _tmp125 * _tmp46 + _tmp126 * _tmp130 - _tmp134 * _tmp163 +
                         _tmp136 * _tmp138;
  const Scalar _tmp165 = _tmp126 * _tmp141;
  const Scalar _tmp166 = -_tmp107 * _tmp149 + _tmp118 * _tmp148 * _tmp77 +
                         _tmp125 * _tmp145 * _tmp77 - _tmp165 * _tmp79;
  const Scalar _tmp167 = Scalar(1.0) / (_tmp166);
  const Scalar _tmp168 = std::asinh(_tmp164 * _tmp167);
  const Scalar _tmp169 = Scalar(1.0) * _tmp168;
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp166;
  const Scalar _tmp171 =
      -_tmp168 * _tmp170 - std::sqrt(Scalar(std::pow(Scalar(-_tmp59 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp61 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp167;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp141;
  const Scalar _tmp175 = _tmp174 * _tmp79;
  const Scalar _tmp176 = std::pow(_tmp166, Scalar(-2));
  const Scalar _tmp177 = _tmp32 * _tmp51;
  const Scalar _tmp178 = _tmp142 * _tmp176;
  const Scalar _tmp179 =
      (-_tmp164 * _tmp178 + _tmp167 * (-_tmp130 - _tmp134 * _tmp177 + _tmp138 * _tmp155)) /
      std::sqrt(Scalar(std::pow(_tmp164, Scalar(2)) * _tmp176 + 1));
  const Scalar _tmp180 = -_tmp107 * _tmp98 - _tmp113 * _tmp118 * _tmp50 -
                         _tmp122 * _tmp125 * _tmp50 + _tmp126 * _tmp129 - _tmp136 * _tmp137 +
                         _tmp163;
  const Scalar _tmp181 =
      _tmp107 * _tmp99 + _tmp111 * _tmp114 * _tmp118 + _tmp111 * _tmp123 * _tmp125 + _tmp165;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = std::asinh(_tmp180 * _tmp182);
  const Scalar _tmp184 = Scalar(1.0) * _tmp183;
  const Scalar _tmp185 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp186 = _tmp141 * _tmp185;
  const Scalar _tmp187 = (_tmp180 * _tmp186 + _tmp182 * (-_tmp129 - _tmp137 * _tmp155 + _tmp177)) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp185 + 1));
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp189 =
      -_tmp183 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp52 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp54 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp191 = _tmp189 * _tmp190;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp34 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_a(1, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp33 + p_a(0, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-std::sinh(_tmp161) - std::sinh(_tmp162)) +
               _tmp154 * (-Scalar(1.0) * _tmp158 * std::cosh(_tmp162) -
                          (-Scalar(0.1034955) * _tmp157 * _tmp160 +
                           _tmp159 * (-_tmp152 * _tmp153 - _tmp154 * _tmp158)) *
                              std::cosh(_tmp161));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp179 * std::cosh(_tmp169) -
                          (-Scalar(0.1034955) * _tmp171 * _tmp178 +
                           _tmp172 * (-_tmp168 * _tmp175 - _tmp170 * _tmp179)) *
                              std::cosh(_tmp173)) +
               _tmp175 * (-std::sinh(_tmp169) - std::sinh(_tmp173));
  _res(3, 0) = -_tmp174 * (-std::sinh(_tmp184) - std::sinh(_tmp191)) +
               _tmp188 * (-Scalar(1.0) * _tmp187 * std::cosh(_tmp184) -
                          (Scalar(0.1034955) * _tmp186 * _tmp189 +
                           _tmp190 * (_tmp174 * _tmp183 - _tmp187 * _tmp188)) *
                              std::cosh(_tmp191));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
