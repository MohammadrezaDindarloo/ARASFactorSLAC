// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl0
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl0(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 659

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (214)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _tmp0 * fv1;
  const Scalar _tmp2 = std::asinh(_tmp1);
  const Scalar _tmp3 = Scalar(1.0) * _tmp2;
  const Scalar _tmp4 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp5 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp6 = -2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp8 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp6 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -_tmp9;
  const Scalar _tmp11 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                        _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp12 = 2 * _tmp11;
  const Scalar _tmp13 = _tmp12 * _tmp7;
  const Scalar _tmp14 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp15 = _tmp14 * _tmp5;
  const Scalar _tmp16 = _tmp13 - _tmp15;
  const Scalar _tmp17 = -Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp12 * _tmp5;
  const Scalar _tmp19 = _tmp14 * _tmp7;
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp18 + Scalar(0.20999999999999999) * _tmp19;
  const Scalar _tmp21 = _tmp17 - _tmp20;
  const Scalar _tmp22 = _tmp10 + _tmp21;
  const Scalar _tmp23 = _tmp22 + position_vector(1, 0);
  const Scalar _tmp24 = -2 * std::pow(_tmp11, Scalar(2));
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp26 = -_tmp25;
  const Scalar _tmp27 = 2 * _tmp5 * _tmp7;
  const Scalar _tmp28 = _tmp11 * _tmp14;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = -Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp18 - Scalar(0.20999999999999999) * _tmp19;
  const Scalar _tmp32 = _tmp30 - _tmp31;
  const Scalar _tmp33 = _tmp26 + _tmp32;
  const Scalar _tmp34 = _tmp33 + position_vector(0, 0);
  const Scalar _tmp35 = Scalar(9.6622558468725703) * _tmp2;
  const Scalar _tmp36 =
      -Scalar(0.1034955) * _tmp35 * fh1 -
      Scalar(0.86104699605560286) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp23 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp34 - 1), Scalar(2))));
  const Scalar _tmp37 = _tmp0 * _tmp36;
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp4 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp40 = _tmp30 + _tmp31;
  const Scalar _tmp41 = _tmp26 + _tmp40;
  const Scalar _tmp42 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp43 = _tmp42 + Scalar(1.7965602546229);
  const Scalar _tmp44 = _tmp21 + _tmp9;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(-4.83288938413423);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp43 * _tmp47;
  const Scalar _tmp49 = _tmp17 + _tmp20;
  const Scalar _tmp50 = _tmp49 + _tmp9;
  const Scalar _tmp51 = _tmp50 + position_vector(1, 0);
  const Scalar _tmp52 = _tmp51 + Scalar(-4.7744369927459998);
  const Scalar _tmp53 = _tmp25 + _tmp40;
  const Scalar _tmp54 = _tmp53 + position_vector(0, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(-2.7171519410699099);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp55 * _tmp56;
  const Scalar _tmp58 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp59 = -_tmp58;
  const Scalar _tmp60 = -Scalar(0.010999999999999999) * _tmp24 -
                        Scalar(0.010999999999999999) * _tmp6 + Scalar(-0.010999999999999999);
  const Scalar _tmp61 = Scalar(0.20999999999999999) * _tmp27 - Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp62 = _tmp60 + _tmp61;
  const Scalar _tmp63 = _tmp59 + _tmp62;
  const Scalar _tmp64 = _tmp25 + _tmp32;
  const Scalar _tmp65 = _tmp64 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-2.5193355532036801);
  const Scalar _tmp67 = Scalar(1.0) / (_tmp66);
  const Scalar _tmp68 = _tmp10 + _tmp49;
  const Scalar _tmp69 = _tmp68 + position_vector(1, 0);
  const Scalar _tmp70 = _tmp69 + Scalar(8.3885017487099702);
  const Scalar _tmp71 = _tmp67 * _tmp70;
  const Scalar _tmp72 = _tmp63 * _tmp71;
  const Scalar _tmp73 = _tmp58 + _tmp62;
  const Scalar _tmp74 = _tmp52 * _tmp56;
  const Scalar _tmp75 = -_tmp57 * _tmp72 + _tmp73 * _tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp57 * _tmp71 - _tmp74);
  const Scalar _tmp77 = _tmp46 * _tmp47;
  const Scalar _tmp78 = _tmp48 * _tmp71 - _tmp77;
  const Scalar _tmp79 = _tmp76 * _tmp78;
  const Scalar _tmp80 = _tmp48 * _tmp63;
  const Scalar _tmp81 = _tmp60 - _tmp61;
  const Scalar _tmp82 = _tmp58 + _tmp81;
  const Scalar _tmp83 = -_tmp71 * _tmp80 - _tmp75 * _tmp79 + _tmp77 * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) * _tmp68;
  const Scalar _tmp85 = -_tmp84;
  const Scalar _tmp86 = Scalar(1.0) / (_tmp50 + _tmp85);
  const Scalar _tmp87 = Scalar(1.0) * _tmp64;
  const Scalar _tmp88 = -_tmp53 + _tmp87;
  const Scalar _tmp89 = _tmp86 * _tmp88;
  const Scalar _tmp90 = _tmp57 * _tmp63 - _tmp57 * _tmp73;
  const Scalar _tmp91 = -_tmp48 * _tmp82 - _tmp79 * _tmp90 + _tmp80 - _tmp83 * _tmp89;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp84 * _tmp89 + _tmp87;
  const Scalar _tmp94 = 0;
  const Scalar _tmp95 = _tmp92 * _tmp94;
  const Scalar _tmp96 = _tmp57 * _tmp79;
  const Scalar _tmp97 =
      std::sqrt(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp70, Scalar(2))));
  const Scalar _tmp98 = _tmp67 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp100 = _tmp98 * (_tmp64 * _tmp70 * _tmp99 - _tmp66 * _tmp68 * _tmp99);
  const Scalar _tmp101 = _tmp100 * _tmp57 + _tmp50 * _tmp57 - _tmp53 * _tmp74;
  const Scalar _tmp102 = Scalar(1.0) * _tmp76;
  const Scalar _tmp103 = Scalar(1.0) * _tmp86;
  const Scalar _tmp104 = -_tmp102 * _tmp90 + _tmp103 * _tmp75 * _tmp76 * _tmp88;
  const Scalar _tmp105 = _tmp100 * _tmp48 - _tmp101 * _tmp79 - _tmp41 * _tmp77 + _tmp44 * _tmp48;
  const Scalar _tmp106 = _tmp105 * _tmp92;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp105);
  const Scalar _tmp108 = _tmp107 * _tmp91;
  const Scalar _tmp109 = _tmp108 * (-_tmp101 * _tmp102 - _tmp104 * _tmp106);
  const Scalar _tmp110 = _tmp104 + _tmp109;
  const Scalar _tmp111 = _tmp48 * _tmp92;
  const Scalar _tmp112 = _tmp78 * _tmp92;
  const Scalar _tmp113 = _tmp76 * (-_tmp110 * _tmp112 + Scalar(1.0));
  const Scalar _tmp114 = _tmp34 + Scalar(1.9874742031097401);
  const Scalar _tmp115 = _tmp23 + Scalar(8.3196563720703107);
  const Scalar _tmp116 =
      std::pow(Scalar(std::pow(_tmp114, Scalar(2)) + std::pow(_tmp115, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp117 = _tmp115 * _tmp116;
  const Scalar _tmp118 = _tmp117 * _tmp98 * (_tmp110 * _tmp111 + _tmp113 * _tmp57);
  const Scalar _tmp119 = _tmp114 * _tmp116;
  const Scalar _tmp120 = _tmp117 * _tmp33 - _tmp119 * _tmp22;
  const Scalar _tmp121 = Scalar(1.0) * _tmp107;
  const Scalar _tmp122 = _tmp120 * _tmp98 * (_tmp121 * _tmp48 - _tmp121 * _tmp96);
  const Scalar _tmp123 = _tmp71 * _tmp76;
  const Scalar _tmp124 = _tmp123 * _tmp75 + _tmp72;
  const Scalar _tmp125 = _tmp123 * _tmp90 - _tmp124 * _tmp89 - _tmp63;
  const Scalar _tmp126 = _tmp108 * (-_tmp100 + _tmp101 * _tmp123 - _tmp106 * _tmp125);
  const Scalar _tmp127 = _tmp125 + _tmp126;
  const Scalar _tmp128 = _tmp76 * (-_tmp112 * _tmp127 - _tmp71);
  const Scalar _tmp129 = _tmp119 * _tmp98 * (_tmp111 * _tmp127 + _tmp128 * _tmp57 + Scalar(1.0));
  const Scalar _tmp130 = -_tmp118 * fh1 - _tmp122 * fh1 - _tmp129 * fh1 -
                         _tmp39 * _tmp98 * (_tmp48 * _tmp95 - _tmp95 * _tmp96);
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp59 + _tmp81;
  const Scalar _tmp133 = _tmp132 * fh1;
  const Scalar _tmp134 = _tmp119 * _tmp133 + Scalar(3.29616) * _tmp29 + _tmp33 * fv1;
  const Scalar _tmp135 = _tmp44 + _tmp85;
  const Scalar _tmp136 = _tmp135 * _tmp89;
  const Scalar _tmp137 = Scalar(1.0) / (-_tmp136 - _tmp41 + _tmp87);
  const Scalar _tmp138 = Scalar(1.0) * _tmp137;
  const Scalar _tmp139 = _tmp135 * _tmp86;
  const Scalar _tmp140 = _tmp138 * _tmp139;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp138 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp135 * _tmp137;
  const Scalar _tmp143 = _tmp83 * _tmp92;
  const Scalar _tmp144 = _tmp124 + _tmp126 * _tmp142 - _tmp127 * _tmp143;
  const Scalar _tmp145 = Scalar(1.0) * _tmp119 * (-_tmp103 * _tmp144 + _tmp126 * _tmp138);
  const Scalar _tmp146 = _tmp137 * _tmp93;
  const Scalar _tmp147 = -_tmp135 * _tmp146 - _tmp143 * _tmp94 + _tmp85;
  const Scalar _tmp148 = -_tmp102 * _tmp75 + _tmp109 * _tmp142 - _tmp110 * _tmp143;
  const Scalar _tmp149 = Scalar(1.0) * _tmp117 * (-_tmp103 * _tmp148 + _tmp109 * _tmp138);
  const Scalar _tmp150 = _tmp108 * _tmp138;
  const Scalar _tmp151 = -_tmp121 * _tmp83 + _tmp135 * _tmp150;
  const Scalar _tmp152 = Scalar(1.0) * _tmp120 * (-_tmp103 * _tmp151 + _tmp150);
  const Scalar _tmp153 = -_tmp117 * _tmp133 - Scalar(3.29616) * _tmp16 - _tmp22 * fv1;
  const Scalar _tmp154 = _tmp136 * _tmp138 + Scalar(1.0);
  const Scalar _tmp155 = _tmp138 * _tmp89;
  const Scalar _tmp156 = -Scalar(1.0) * _tmp103 * _tmp154 + Scalar(1.0) * _tmp155;
  const Scalar _tmp157 =
      _tmp134 * _tmp141 + _tmp145 * fh1 + _tmp149 * fh1 + _tmp152 * fh1 + _tmp153 * _tmp156 +
      Scalar(1.0) * _tmp39 * (-_tmp103 * _tmp147 - _tmp138 * _tmp93 + Scalar(1.0));
  const Scalar _tmp158 = std::asinh(_tmp131 * _tmp157);
  const Scalar _tmp159 = Scalar(1.0) * _tmp158;
  const Scalar _tmp160 = _tmp119 * _tmp132;
  const Scalar _tmp161 = _tmp117 * _tmp132;
  const Scalar _tmp162 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp163 = -_tmp118 - _tmp122 - _tmp129;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 =
      (_tmp131 * (_tmp141 * _tmp160 + _tmp145 + _tmp149 + _tmp152 - _tmp156 * _tmp161) -
       _tmp157 * _tmp164) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp162 + 1));
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp167 =
      -_tmp130 * _tmp166 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp65), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp69 - 1), Scalar(2))));
  const Scalar _tmp168 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp169 = _tmp167 * _tmp168;
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp171 = _tmp39 * _tmp95;
  const Scalar _tmp172 = _tmp119 * _tmp128;
  const Scalar _tmp173 = _tmp120 * _tmp121;
  const Scalar _tmp174 = _tmp173 * fh1;
  const Scalar _tmp175 = _tmp113 * _tmp117;
  const Scalar _tmp176 = -_tmp171 * _tmp79 + _tmp172 * fh1 - _tmp174 * _tmp79 + _tmp175 * fh1;
  const Scalar _tmp177 = Scalar(1.0) / (_tmp176);
  const Scalar _tmp178 = _tmp154 * _tmp86;
  const Scalar _tmp179 = _tmp134 * _tmp138;
  const Scalar _tmp180 = _tmp117 * _tmp148 * _tmp86;
  const Scalar _tmp181 = _tmp119 * _tmp144 * _tmp86;
  const Scalar _tmp182 = _tmp120 * _tmp151 * _tmp86;
  const Scalar _tmp183 = -_tmp139 * _tmp179 + _tmp147 * _tmp39 * _tmp86 + _tmp153 * _tmp178 +
                         _tmp180 * fh1 + _tmp181 * fh1 + _tmp182 * fh1;
  const Scalar _tmp184 = std::asinh(_tmp177 * _tmp183);
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp184;
  const Scalar _tmp186 =
      -_tmp176 * _tmp185 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp51), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp54), Scalar(2))));
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp177;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 = Scalar(1.0) * _tmp184;
  const Scalar _tmp190 = _tmp172 - _tmp173 * _tmp79 + _tmp175;
  const Scalar _tmp191 = std::pow(_tmp176, Scalar(-2));
  const Scalar _tmp192 = _tmp190 * _tmp191;
  const Scalar _tmp193 =
      (_tmp177 * (-_tmp140 * _tmp160 - _tmp161 * _tmp178 + _tmp180 + _tmp181 + _tmp182) -
       _tmp183 * _tmp192) /
      std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp191 + 1));
  const Scalar _tmp194 = Scalar(9.6622558468725703) * _tmp176;
  const Scalar _tmp195 = _tmp110 * _tmp117 * _tmp92;
  const Scalar _tmp196 = _tmp119 * _tmp127 * _tmp92;
  const Scalar _tmp197 = _tmp171 + _tmp174 + _tmp195 * fh1 + _tmp196 * fh1;
  const Scalar _tmp198 = _tmp109 * _tmp117 * _tmp137;
  const Scalar _tmp199 = _tmp119 * _tmp126 * _tmp137;
  const Scalar _tmp200 = _tmp120 * _tmp150;
  const Scalar _tmp201 = _tmp146 * _tmp39 - _tmp153 * _tmp155 + _tmp179 - _tmp198 * fh1 -
                         _tmp199 * fh1 - _tmp200 * fh1;
  const Scalar _tmp202 = Scalar(1.0) / (_tmp197);
  const Scalar _tmp203 = std::asinh(_tmp201 * _tmp202);
  const Scalar _tmp204 = Scalar(9.6622558468725703) * _tmp203;
  const Scalar _tmp205 =
      -_tmp197 * _tmp204 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp45), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp206 = Scalar(0.1034955) * _tmp202;
  const Scalar _tmp207 = _tmp205 * _tmp206;
  const Scalar _tmp208 = std::pow(_tmp197, Scalar(-2));
  const Scalar _tmp209 = _tmp173 + _tmp195 + _tmp196;
  const Scalar _tmp210 = _tmp208 * _tmp209;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp197;
  const Scalar _tmp212 = (-_tmp201 * _tmp210 + _tmp202 * (_tmp138 * _tmp160 + _tmp155 * _tmp161 -
                                                          _tmp198 - _tmp199 - _tmp200)) /
                         std::sqrt(Scalar(std::pow(_tmp201, Scalar(2)) * _tmp208 + 1));
  const Scalar _tmp213 = Scalar(1.0) * _tmp203;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -Scalar(8.4718465805053746) * _tmp0 -
      Scalar(9.6622558468725703) * fh1 *
          (-Scalar(1.0) * _tmp38 * _tmp4 * fv1 * std::sinh(_tmp3) -
           Scalar(0.87679799777269396) * _tmp4 -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp1 * _tmp38 - _tmp35) -
            _tmp36 * _tmp4) *
               std::sinh(_tmp37)) -
      Scalar(9.6622558468725703) * std::cosh(_tmp3) +
      Scalar(9.6622558468725703) * std::cosh(_tmp37);
  _res(1, 0) =
      -Scalar(9.6622558468725703) * _tmp163 *
          (Scalar(0.876505537412406) * _tmp131 + std::cosh(_tmp159) - std::cosh(_tmp169)) -
      _tmp170 * (-Scalar(0.876505537412406) * _tmp164 + Scalar(1.0) * _tmp165 * std::sinh(_tmp159) -
                 (-Scalar(0.1034955) * _tmp164 * _tmp167 +
                  _tmp168 * (-_tmp163 * _tmp166 - _tmp165 * _tmp170)) *
                     std::sinh(_tmp169));
  _res(2, 0) =
      -Scalar(9.6622558468725703) * _tmp190 *
          (Scalar(0.86564762886483004) * _tmp177 - std::cosh(_tmp188) + std::cosh(_tmp189)) -
      _tmp194 *
          (-Scalar(0.86564762886483004) * _tmp192 + Scalar(1.0) * _tmp193 * std::sinh(_tmp189) -
           (-Scalar(0.1034955) * _tmp186 * _tmp192 +
            _tmp187 * (-_tmp185 * _tmp190 - _tmp193 * _tmp194)) *
               std::sinh(_tmp188));
  _res(3, 0) =
      -Scalar(9.6622558468725703) * _tmp209 *
          (Scalar(0.86627065637365697) * _tmp202 - std::cosh(_tmp207) + std::cosh(_tmp213)) -
      _tmp211 *
          (-Scalar(0.86627065637365697) * _tmp210 + Scalar(1.0) * _tmp212 * std::sinh(_tmp213) -
           (-Scalar(0.1034955) * _tmp205 * _tmp210 +
            _tmp206 * (-_tmp204 * _tmp209 - _tmp211 * _tmp212)) *
               std::sinh(_tmp207));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
