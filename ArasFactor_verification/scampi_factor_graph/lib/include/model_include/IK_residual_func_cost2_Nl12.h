// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_Nl12
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2Nl12(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 528

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (157)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp1;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp3 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp3 * _tmp6;
  const Scalar _tmp14 = _tmp1 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp2 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp22 = _tmp3 * _tmp7;
  const Scalar _tmp23 = _tmp6 * _tmp9;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = _tmp21 + _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(1, 0);
  const Scalar _tmp29 = Scalar(7.3829146708599787) *
                            std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp18), Scalar(2)) +
                        Scalar(22.795248597701466) *
                            std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp28), Scalar(2));
  const Scalar _tmp30 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp32 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp33 =
      -Scalar(0.010999999999999999) * _tmp19 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp34 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp35 = _tmp33 + _tmp34;
  const Scalar _tmp36 = _tmp32 + _tmp35;
  const Scalar _tmp37 = -_tmp20;
  const Scalar _tmp38 = -_tmp21 + _tmp25;
  const Scalar _tmp39 = _tmp37 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(1, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(8.3196563720703107);
  const Scalar _tmp42 = -_tmp5;
  const Scalar _tmp43 = _tmp12 - _tmp15;
  const Scalar _tmp44 = _tmp42 + _tmp43;
  const Scalar _tmp45 = _tmp44 + position_vector(0, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(1.9874742031097401);
  const Scalar _tmp47 =
      std::sqrt(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp46, Scalar(2))));
  const Scalar _tmp48 = Scalar(1.0) / (_tmp47);
  const Scalar _tmp49 = Scalar(1.0) / (_tmp46);
  const Scalar _tmp50 = _tmp47 * _tmp49;
  const Scalar _tmp51 = _tmp50 * (-_tmp39 * _tmp46 * _tmp48 + _tmp41 * _tmp44 * _tmp48);
  const Scalar _tmp52 = _tmp16 + _tmp42;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 + Scalar(1.7965602546229);
  const Scalar _tmp55 = _tmp20 + _tmp38;
  const Scalar _tmp56 = _tmp55 + position_vector(1, 0);
  const Scalar _tmp57 = _tmp56 + Scalar(-4.83288938413423);
  const Scalar _tmp58 = std::pow(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp57, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp59 = _tmp54 * _tmp58;
  const Scalar _tmp60 = _tmp57 * _tmp58;
  const Scalar _tmp61 = _tmp26 + _tmp37;
  const Scalar _tmp62 = _tmp61 + position_vector(1, 0);
  const Scalar _tmp63 = _tmp62 + Scalar(8.3885017487099702);
  const Scalar _tmp64 = _tmp43 + _tmp5;
  const Scalar _tmp65 = _tmp64 + position_vector(0, 0);
  const Scalar _tmp66 = _tmp65 + Scalar(-2.5193355532036801);
  const Scalar _tmp67 = std::pow(Scalar(std::pow(_tmp63, Scalar(2)) + std::pow(_tmp66, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp68 = _tmp66 * _tmp67;
  const Scalar _tmp69 = _tmp63 * _tmp67;
  const Scalar _tmp70 = _tmp51 * _tmp68 + _tmp61 * _tmp68 - _tmp64 * _tmp69;
  const Scalar _tmp71 = _tmp41 * _tmp49;
  const Scalar _tmp72 = Scalar(1.0) / (_tmp68 * _tmp71 - _tmp69);
  const Scalar _tmp73 = _tmp59 * _tmp71 - _tmp60;
  const Scalar _tmp74 = _tmp72 * _tmp73;
  const Scalar _tmp75 = _tmp51 * _tmp59 - _tmp52 * _tmp60 + _tmp55 * _tmp59 - _tmp70 * _tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp75);
  const Scalar _tmp77 = Scalar(1.0) * _tmp76;
  const Scalar _tmp78 = _tmp68 * _tmp74;
  const Scalar _tmp79 = _tmp28 + Scalar(-4.7744369927459998);
  const Scalar _tmp80 = _tmp18 + Scalar(-2.7171519410699099);
  const Scalar _tmp81 = std::pow(Scalar(std::pow(_tmp79, Scalar(2)) + std::pow(_tmp80, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp82 = _tmp79 * _tmp81;
  const Scalar _tmp83 = _tmp80 * _tmp81;
  const Scalar _tmp84 = fh1 * (_tmp17 * _tmp82 - _tmp27 * _tmp83);
  const Scalar _tmp85 = -_tmp32;
  const Scalar _tmp86 = _tmp33 - _tmp34;
  const Scalar _tmp87 = _tmp85 + _tmp86;
  const Scalar _tmp88 = _tmp32 + _tmp86;
  const Scalar _tmp89 = _tmp35 + _tmp85;
  const Scalar _tmp90 = _tmp68 * _tmp87 - _tmp68 * _tmp89;
  const Scalar _tmp91 = _tmp71 * _tmp87;
  const Scalar _tmp92 = -_tmp68 * _tmp91 + _tmp69 * _tmp89;
  const Scalar _tmp93 = -_tmp59 * _tmp91 + _tmp60 * _tmp88 - _tmp74 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) * _tmp39;
  const Scalar _tmp95 = -_tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp61 + _tmp95);
  const Scalar _tmp97 = Scalar(1.0) * _tmp44;
  const Scalar _tmp98 = -_tmp64 + _tmp97;
  const Scalar _tmp99 = _tmp96 * _tmp98;
  const Scalar _tmp100 = _tmp59 * _tmp87 - _tmp59 * _tmp88 - _tmp74 * _tmp90 - _tmp93 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp102 = Scalar(1.0) * _tmp72;
  const Scalar _tmp103 = Scalar(1.0) * _tmp96;
  const Scalar _tmp104 = -_tmp102 * _tmp90 + _tmp103 * _tmp72 * _tmp92 * _tmp98;
  const Scalar _tmp105 = _tmp101 * _tmp75;
  const Scalar _tmp106 = _tmp100 * _tmp76;
  const Scalar _tmp107 = _tmp106 * (-_tmp102 * _tmp70 - _tmp104 * _tmp105);
  const Scalar _tmp108 = _tmp101 * (_tmp104 + _tmp107);
  const Scalar _tmp109 = -_tmp108 * _tmp73 + Scalar(1.0);
  const Scalar _tmp110 = _tmp68 * _tmp72;
  const Scalar _tmp111 = _tmp82 * fh1;
  const Scalar _tmp112 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp113 = _tmp94 * _tmp99 + _tmp97;
  const Scalar _tmp114 = 0;
  const Scalar _tmp115 = _tmp71 * _tmp72;
  const Scalar _tmp116 = _tmp115 * _tmp92 + _tmp91;
  const Scalar _tmp117 = _tmp115 * _tmp90 - _tmp116 * _tmp99 - _tmp87;
  const Scalar _tmp118 = _tmp106 * (-_tmp105 * _tmp117 + _tmp115 * _tmp70 - _tmp51);
  const Scalar _tmp119 = _tmp101 * (_tmp117 + _tmp118);
  const Scalar _tmp120 = -_tmp119 * _tmp73 - _tmp71;
  const Scalar _tmp121 = _tmp83 * fh1;
  const Scalar _tmp122 = -_tmp111 * _tmp50 * (_tmp108 * _tmp59 + _tmp109 * _tmp110) -
                         _tmp112 * _tmp50 * (_tmp114 * _tmp59 - _tmp114 * _tmp78) -
                         _tmp121 * _tmp50 * (_tmp110 * _tmp120 + _tmp119 * _tmp59 + Scalar(1.0)) -
                         _tmp50 * _tmp84 * (_tmp59 * _tmp77 - _tmp77 * _tmp78);
  const Scalar _tmp123 = Scalar(1.0) / (_tmp122);
  const Scalar _tmp124 = _tmp55 + _tmp95;
  const Scalar _tmp125 = _tmp124 * _tmp99;
  const Scalar _tmp126 = Scalar(1.0) / (-_tmp125 - _tmp52 + _tmp97);
  const Scalar _tmp127 = Scalar(1.0) * _tmp126;
  const Scalar _tmp128 = _tmp106 * _tmp127;
  const Scalar _tmp129 = _tmp124 * _tmp128 - _tmp77 * _tmp93;
  const Scalar _tmp130 = _tmp113 * _tmp126;
  const Scalar _tmp131 = _tmp96 * (-_tmp114 * _tmp93 - _tmp124 * _tmp130 + _tmp95);
  const Scalar _tmp132 = _tmp124 * _tmp126;
  const Scalar _tmp133 = _tmp116 + _tmp118 * _tmp132 - _tmp119 * _tmp93;
  const Scalar _tmp134 = -_tmp102 * _tmp92 + _tmp107 * _tmp132 - _tmp108 * _tmp93;
  const Scalar _tmp135 = _tmp36 * fh1;
  const Scalar _tmp136 = Scalar(3.29616) * _tmp11 + _tmp135 * _tmp83 + _tmp17 * fv1;
  const Scalar _tmp137 = _tmp124 * _tmp96;
  const Scalar _tmp138 = -_tmp135 * _tmp82 - Scalar(3.29616) * _tmp24 - _tmp27 * fv1;
  const Scalar _tmp139 = _tmp127 * _tmp99;
  const Scalar _tmp140 = _tmp96 * (_tmp125 * _tmp127 + Scalar(1.0));
  const Scalar _tmp141 =
      std::asinh(_tmp123 * (Scalar(1.0) * _tmp111 * (-_tmp103 * _tmp134 + _tmp107 * _tmp127) +
                            Scalar(1.0) * _tmp112 *
                                (-_tmp113 * _tmp127 - Scalar(1.0) * _tmp131 + Scalar(1.0)) +
                            Scalar(1.0) * _tmp121 * (-_tmp103 * _tmp133 + _tmp118 * _tmp127) +
                            Scalar(1.0) * _tmp136 * (_tmp127 * _tmp137 - _tmp127) +
                            Scalar(1.0) * _tmp138 * (_tmp139 - Scalar(1.0) * _tmp140) +
                            Scalar(1.0) * _tmp84 * (-_tmp103 * _tmp129 + _tmp128)));
  const Scalar _tmp142 =
      Scalar(69.216682149330126) *
          std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp40 - 1), Scalar(2)) +
      Scalar(3.9500537080266964) *
          std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp45 - 1), Scalar(2));
  const Scalar _tmp143 = Scalar(9.6622558468725703) * _tmp122;
  const Scalar _tmp144 = _tmp112 * _tmp114;
  const Scalar _tmp145 = _tmp77 * _tmp84;
  const Scalar _tmp146 =
      _tmp109 * _tmp111 * _tmp72 + _tmp120 * _tmp121 * _tmp72 - _tmp144 * _tmp74 - _tmp145 * _tmp74;
  const Scalar _tmp147 = Scalar(1.0) / (_tmp146);
  const Scalar _tmp148 = Scalar(6.347051629636093) *
                             std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp65), Scalar(2)) +
                         Scalar(70.366961588110229) *
                             std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp62 - 1), Scalar(2));
  const Scalar _tmp149 = _tmp127 * _tmp136;
  const Scalar _tmp150 = std::asinh(
      _tmp147 * (_tmp111 * _tmp134 * _tmp96 + _tmp112 * _tmp131 + _tmp121 * _tmp133 * _tmp96 +
                 _tmp129 * _tmp84 * _tmp96 - _tmp137 * _tmp149 + _tmp138 * _tmp140));
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp146;
  const Scalar _tmp152 = _tmp108 * _tmp111 + _tmp119 * _tmp121 + _tmp144 + _tmp145;
  const Scalar _tmp153 = Scalar(1.0) / (_tmp152);
  const Scalar _tmp154 = std::asinh(_tmp153 * (-_tmp107 * _tmp111 * _tmp126 + _tmp112 * _tmp130 -
                                               _tmp118 * _tmp121 * _tmp126 - _tmp128 * _tmp84 -
                                               _tmp138 * _tmp139 + _tmp149));
  const Scalar _tmp155 = Scalar(23.356819799277336) *
                             std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp56), Scalar(2)) +
                         Scalar(3.2276287484906994) *
                             std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp53 - 1), Scalar(2));
  const Scalar _tmp156 = Scalar(9.6622558468725703) * _tmp152;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp31 * (-std::sinh(Scalar(1.0) * _tmp30) -
                std::sinh(Scalar(0.1034955) * _tmp0 * (-std::sqrt(_tmp29) - _tmp30 * _tmp31))) -
      Scalar(8.3641088633305802) *
          std::sqrt(Scalar(Scalar(0.014294226073078949) * _tmp29 +
                           std::pow(Scalar(-Scalar(0.11955846299229073) * _tmp36 -
                                           Scalar(0.11955846299229073) * position_vector(2, 0) + 1),
                                    Scalar(2))));
  _res(1, 0) =
      _tmp143 *
          (-std::sinh(Scalar(1.0) * _tmp141) -
           std::sinh(Scalar(0.1034955) * _tmp123 * (-_tmp141 * _tmp143 - std::sqrt(_tmp142)))) -
      Scalar(8.4718465805053693) *
          std::sqrt(Scalar(Scalar(0.013932974274013004) * _tmp142 +
                           std::pow(Scalar(-Scalar(0.11803802045956635) * _tmp87 -
                                           Scalar(0.11803802045956635) * position_vector(2, 0) + 1),
                                    Scalar(2))));
  _res(2, 0) =
      _tmp151 *
          (-std::sinh(Scalar(1.0) * _tmp150) -
           std::sinh(Scalar(0.1034955) * _tmp147 * (-std::sqrt(_tmp148) - _tmp150 * _tmp151))) -
      Scalar(8.4690207536791995) *
          std::sqrt(Scalar(Scalar(0.013942273753185566) * _tmp148 +
                           std::pow(Scalar(-Scalar(0.11807740576920533) * _tmp89 -
                                           Scalar(0.11807740576920533) * position_vector(2, 0) + 1),
                                    Scalar(2))));
  _res(3, 0) =
      _tmp156 *
          (-std::sinh(Scalar(1.0) * _tmp154) -
           std::sinh(Scalar(0.1034955) * _tmp153 * (-_tmp154 * _tmp156 - std::sqrt(_tmp155)))) -
      Scalar(8.3701287145205097) *
          std::sqrt(Scalar(Scalar(0.014273672465547484) * _tmp155 +
                           std::pow(Scalar(-Scalar(0.11947247576553975) * _tmp88 -
                                           Scalar(0.11947247576553975) * position_vector(2, 0) + 1),
                                    Scalar(2))));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
