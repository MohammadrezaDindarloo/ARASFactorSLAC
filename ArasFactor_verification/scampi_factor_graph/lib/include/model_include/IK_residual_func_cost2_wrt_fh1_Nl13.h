// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl13
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl13(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 642

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp1 * _tmp6;
  const Scalar _tmp8 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = _tmp7 + _tmp9;
  const Scalar _tmp11 = -Scalar(0.010999999999999999) * _tmp10;
  const Scalar _tmp12 = 2 * _tmp3;
  const Scalar _tmp13 = _tmp12 * _tmp6;
  const Scalar _tmp14 = _tmp1 * _tmp8;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp11 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp20 = _tmp1 * _tmp12;
  const Scalar _tmp21 = _tmp6 * _tmp8;
  const Scalar _tmp22 = _tmp20 - _tmp21;
  const Scalar _tmp23 = -Scalar(0.010999999999999999) * _tmp22;
  const Scalar _tmp24 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp24 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp26 = _tmp23 + _tmp25;
  const Scalar _tmp27 = _tmp19 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(1, 0);
  const Scalar _tmp29 = _tmp0 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -Scalar(0.1034955) * _tmp31 * fh1 -
      Scalar(0.49413274378274363) *
          std::sqrt(
              Scalar(Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp18), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp28), Scalar(2))));
  const Scalar _tmp33 = _tmp0 * _tmp32;
  const Scalar _tmp34 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp35 =
      std::pow(Scalar(_tmp34 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp36 = Scalar(1.0) * _tmp30;
  const Scalar _tmp37 = _tmp28 + Scalar(-4.7744369927459998);
  const Scalar _tmp38 = _tmp18 + Scalar(-2.7171519410699099);
  const Scalar _tmp39 = std::pow(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp38, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp40 = _tmp37 * _tmp39;
  const Scalar _tmp41 = _tmp38 * _tmp39;
  const Scalar _tmp42 = _tmp17 * _tmp40 - _tmp27 * _tmp41;
  const Scalar _tmp43 = _tmp23 - _tmp25;
  const Scalar _tmp44 = _tmp19 + _tmp43;
  const Scalar _tmp45 = _tmp44 + position_vector(1, 0);
  const Scalar _tmp46 = _tmp45 + Scalar(8.3885017487099702);
  const Scalar _tmp47 = _tmp11 - _tmp15;
  const Scalar _tmp48 = _tmp47 + _tmp5;
  const Scalar _tmp49 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp50 = _tmp49 + Scalar(-2.5193355532036801);
  const Scalar _tmp51 = std::pow(Scalar(std::pow(_tmp46, Scalar(2)) + std::pow(_tmp50, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp52 = _tmp50 * _tmp51;
  const Scalar _tmp53 = -_tmp19;
  const Scalar _tmp54 = _tmp43 + _tmp53;
  const Scalar _tmp55 = _tmp54 + position_vector(1, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(8.3196563720703107);
  const Scalar _tmp57 = -_tmp5;
  const Scalar _tmp58 = _tmp47 + _tmp57;
  const Scalar _tmp59 = _tmp58 + position_vector(0, 0);
  const Scalar _tmp60 = _tmp59 + Scalar(1.9874742031097401);
  const Scalar _tmp61 =
      std::sqrt(Scalar(std::pow(_tmp56, Scalar(2)) + std::pow(_tmp60, Scalar(2))));
  const Scalar _tmp62 = Scalar(1.0) / (_tmp61);
  const Scalar _tmp63 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp64 = _tmp61 * _tmp63;
  const Scalar _tmp65 = _tmp64 * (-_tmp54 * _tmp60 * _tmp62 + _tmp56 * _tmp58 * _tmp62);
  const Scalar _tmp66 = _tmp46 * _tmp51;
  const Scalar _tmp67 = _tmp16 + _tmp57;
  const Scalar _tmp68 = _tmp67 + position_vector(0, 0);
  const Scalar _tmp69 = _tmp68 + Scalar(1.7965602546229);
  const Scalar _tmp70 = _tmp26 + _tmp53;
  const Scalar _tmp71 = _tmp70 + position_vector(1, 0);
  const Scalar _tmp72 = _tmp71 + Scalar(-4.83288938413423);
  const Scalar _tmp73 = std::pow(Scalar(std::pow(_tmp69, Scalar(2)) + std::pow(_tmp72, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp74 = _tmp69 * _tmp73;
  const Scalar _tmp75 = _tmp72 * _tmp73;
  const Scalar _tmp76 = _tmp65 * _tmp74 - _tmp67 * _tmp75 + _tmp70 * _tmp74;
  const Scalar _tmp77 = _tmp56 * _tmp63;
  const Scalar _tmp78 = Scalar(1.0) / (_tmp74 * _tmp77 - _tmp75);
  const Scalar _tmp79 = _tmp52 * _tmp77 - _tmp66;
  const Scalar _tmp80 = _tmp78 * _tmp79;
  const Scalar _tmp81 = _tmp44 * _tmp52 - _tmp48 * _tmp66 + _tmp52 * _tmp65 - _tmp76 * _tmp80;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp81);
  const Scalar _tmp83 = Scalar(1.0) * _tmp82;
  const Scalar _tmp84 = Scalar(1.0) * _tmp78;
  const Scalar _tmp85 = _tmp79 * _tmp82 * _tmp84;
  const Scalar _tmp86 = _tmp42 * _tmp64 * (_tmp52 * _tmp83 - _tmp74 * _tmp85);
  const Scalar _tmp87 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp88 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp89 = -_tmp88;
  const Scalar _tmp90 =
      -Scalar(0.010999999999999999) * _tmp24 - Scalar(0.010999999999999999) * _tmp4;
  const Scalar _tmp91 = Scalar(0.20999999999999999) * _tmp7 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp92 = _tmp90 + _tmp91;
  const Scalar _tmp93 = _tmp89 + _tmp92;
  const Scalar _tmp94 = _tmp90 - _tmp91;
  const Scalar _tmp95 = _tmp89 + _tmp94;
  const Scalar _tmp96 = _tmp88 + _tmp94;
  const Scalar _tmp97 = _tmp74 * _tmp95;
  const Scalar _tmp98 = _tmp75 * _tmp96 - _tmp77 * _tmp97;
  const Scalar _tmp99 = _tmp77 * _tmp95;
  const Scalar _tmp100 = -_tmp52 * _tmp99 + _tmp66 * _tmp93 - _tmp80 * _tmp98;
  const Scalar _tmp101 = Scalar(1.0) * _tmp54;
  const Scalar _tmp102 = -_tmp101;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp102 + _tmp70);
  const Scalar _tmp104 = Scalar(1.0) * _tmp58;
  const Scalar _tmp105 = _tmp103 * (_tmp104 - _tmp67);
  const Scalar _tmp106 = -_tmp74 * _tmp96 + _tmp97;
  const Scalar _tmp107 = -_tmp100 * _tmp105 - _tmp106 * _tmp80 - _tmp52 * _tmp93 + _tmp52 * _tmp95;
  const Scalar _tmp108 = Scalar(1.0) / (_tmp107);
  const Scalar _tmp109 = _tmp101 * _tmp105 + _tmp104;
  const Scalar _tmp110 = 0;
  const Scalar _tmp111 = _tmp84 * _tmp98;
  const Scalar _tmp112 = _tmp105 * _tmp111 - _tmp106 * _tmp84;
  const Scalar _tmp113 = _tmp108 * _tmp81;
  const Scalar _tmp114 = _tmp107 * _tmp82;
  const Scalar _tmp115 = _tmp114 * (-_tmp112 * _tmp113 - _tmp76 * _tmp84);
  const Scalar _tmp116 = _tmp112 + _tmp115;
  const Scalar _tmp117 = _tmp108 * _tmp116;
  const Scalar _tmp118 = _tmp108 * _tmp79;
  const Scalar _tmp119 = _tmp78 * (-_tmp116 * _tmp118 + Scalar(1.0));
  const Scalar _tmp120 = _tmp40 * _tmp64 * (_tmp117 * _tmp52 + _tmp119 * _tmp74);
  const Scalar _tmp121 = _tmp77 * _tmp78;
  const Scalar _tmp122 = _tmp121 * _tmp98 + _tmp99;
  const Scalar _tmp123 = -_tmp105 * _tmp122 + _tmp106 * _tmp121 - _tmp95;
  const Scalar _tmp124 = _tmp114 * (-_tmp113 * _tmp123 + _tmp121 * _tmp76 - _tmp65);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = _tmp108 * _tmp125;
  const Scalar _tmp127 = _tmp78 * (-_tmp118 * _tmp125 - _tmp77);
  const Scalar _tmp128 = _tmp41 * _tmp64 * (_tmp126 * _tmp52 + _tmp127 * _tmp74 + Scalar(1.0));
  const Scalar _tmp129 = -_tmp120 * fh1 - _tmp128 * fh1 -
                         _tmp64 * _tmp87 * (_tmp110 * _tmp52 - _tmp110 * _tmp74 * _tmp80) -
                         _tmp86 * fh1;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp100 * _tmp108;
  const Scalar _tmp132 = _tmp102 + _tmp44;
  const Scalar _tmp133 = _tmp105 * _tmp132;
  const Scalar _tmp134 = Scalar(1.0) / (_tmp104 - _tmp133 - _tmp48);
  const Scalar _tmp135 = _tmp132 * _tmp134;
  const Scalar _tmp136 = -_tmp111 + _tmp115 * _tmp135 - _tmp116 * _tmp131;
  const Scalar _tmp137 = Scalar(1.0) * _tmp103;
  const Scalar _tmp138 = Scalar(1.0) * _tmp134;
  const Scalar _tmp139 = Scalar(1.0) * _tmp40 * (_tmp115 * _tmp138 - _tmp136 * _tmp137);
  const Scalar _tmp140 = _tmp114 * _tmp138;
  const Scalar _tmp141 = -_tmp100 * _tmp83 + _tmp132 * _tmp140;
  const Scalar _tmp142 = Scalar(1.0) * _tmp42;
  const Scalar _tmp143 = _tmp142 * (-_tmp137 * _tmp141 + _tmp140);
  const Scalar _tmp144 = _tmp109 * _tmp134;
  const Scalar _tmp145 = -_tmp100 * _tmp110 + _tmp102 - _tmp132 * _tmp144;
  const Scalar _tmp146 = _tmp88 + _tmp92;
  const Scalar _tmp147 = _tmp146 * fh1;
  const Scalar _tmp148 = -_tmp147 * _tmp40 - Scalar(3.29616) * _tmp22 - _tmp27 * fv1;
  const Scalar _tmp149 = _tmp105 * _tmp138;
  const Scalar _tmp150 = _tmp133 * _tmp138 + Scalar(1.0);
  const Scalar _tmp151 = -Scalar(1.0) * _tmp137 * _tmp150 + Scalar(1.0) * _tmp149;
  const Scalar _tmp152 = _tmp122 + _tmp124 * _tmp135 - _tmp125 * _tmp131;
  const Scalar _tmp153 = Scalar(1.0) * _tmp41 * (_tmp124 * _tmp138 - _tmp137 * _tmp152);
  const Scalar _tmp154 = Scalar(3.29616) * _tmp10 + _tmp147 * _tmp41 + _tmp17 * fv1;
  const Scalar _tmp155 = _tmp103 * _tmp132;
  const Scalar _tmp156 = _tmp138 * _tmp155;
  const Scalar _tmp157 = -Scalar(1.0) * _tmp138 + Scalar(1.0) * _tmp156;
  const Scalar _tmp158 =
      _tmp139 * fh1 + _tmp143 * fh1 + _tmp148 * _tmp151 + _tmp153 * fh1 + _tmp154 * _tmp157 +
      Scalar(1.0) * _tmp87 * (-_tmp109 * _tmp138 - _tmp137 * _tmp145 + Scalar(1.0));
  const Scalar _tmp159 = std::asinh(_tmp130 * _tmp158);
  const Scalar _tmp160 = Scalar(1.0) * _tmp159;
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp162 =
      -_tmp159 * _tmp161 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp55 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp59 - 1), Scalar(2))));
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = -_tmp120 - _tmp128 - _tmp86;
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp167 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp168 = _tmp165 * _tmp167;
  const Scalar _tmp169 = _tmp146 * _tmp40;
  const Scalar _tmp170 = _tmp146 * _tmp41;
  const Scalar _tmp171 =
      (_tmp130 * (_tmp139 + _tmp143 - _tmp151 * _tmp169 + _tmp153 + _tmp157 * _tmp170) -
       _tmp158 * _tmp168) /
      std::sqrt(Scalar(std::pow(_tmp158, Scalar(2)) * _tmp167 + 1));
  const Scalar _tmp172 = _tmp119 * _tmp40;
  const Scalar _tmp173 = _tmp42 * _tmp85;
  const Scalar _tmp174 = _tmp110 * _tmp87;
  const Scalar _tmp175 = _tmp127 * _tmp41;
  const Scalar _tmp176 = _tmp172 * fh1 - _tmp173 * fh1 - _tmp174 * _tmp80 + _tmp175 * fh1;
  const Scalar _tmp177 = Scalar(1.0) / (_tmp176);
  const Scalar _tmp178 = _tmp103 * _tmp136 * _tmp40;
  const Scalar _tmp179 = _tmp103 * _tmp150;
  const Scalar _tmp180 = _tmp138 * _tmp154;
  const Scalar _tmp181 = _tmp103 * _tmp152 * _tmp41;
  const Scalar _tmp182 = _tmp103 * _tmp141 * _tmp42;
  const Scalar _tmp183 = _tmp103 * _tmp145 * _tmp87 + _tmp148 * _tmp179 - _tmp155 * _tmp180 +
                         _tmp178 * fh1 + _tmp181 * fh1 + _tmp182 * fh1;
  const Scalar _tmp184 = std::asinh(_tmp177 * _tmp183);
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp184;
  const Scalar _tmp186 =
      -_tmp176 * _tmp185 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp71), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp68 - 1), Scalar(2))));
  const Scalar _tmp187 = std::pow(_tmp176, Scalar(-2));
  const Scalar _tmp188 = _tmp172 - _tmp173 + _tmp175;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp176;
  const Scalar _tmp191 =
      (_tmp177 * (-_tmp156 * _tmp170 - _tmp169 * _tmp179 + _tmp178 + _tmp181 + _tmp182) -
       _tmp183 * _tmp189) /
      std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp187 + 1));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp177;
  const Scalar _tmp193 = _tmp186 * _tmp192;
  const Scalar _tmp194 = Scalar(1.0) * _tmp184;
  const Scalar _tmp195 = _tmp142 * _tmp82;
  const Scalar _tmp196 = _tmp117 * _tmp40;
  const Scalar _tmp197 = _tmp126 * _tmp41;
  const Scalar _tmp198 = _tmp195 + _tmp196 + _tmp197;
  const Scalar _tmp199 = _tmp174 + _tmp195 * fh1 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp200 = _tmp115 * _tmp134 * _tmp40;
  const Scalar _tmp201 = _tmp124 * _tmp134 * _tmp41;
  const Scalar _tmp202 = _tmp140 * _tmp42;
  const Scalar _tmp203 = _tmp144 * _tmp87 - _tmp148 * _tmp149 + _tmp180 - _tmp200 * fh1 -
                         _tmp201 * fh1 - _tmp202 * fh1;
  const Scalar _tmp204 = Scalar(1.0) / (_tmp199);
  const Scalar _tmp205 = std::asinh(_tmp203 * _tmp204);
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp205;
  const Scalar _tmp207 =
      -_tmp199 * _tmp206 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp49), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp45 - 1), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp204;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = Scalar(1.0) * _tmp205;
  const Scalar _tmp211 = std::pow(_tmp199, Scalar(-2));
  const Scalar _tmp212 = _tmp198 * _tmp211;
  const Scalar _tmp213 = (-_tmp203 * _tmp212 + _tmp204 * (_tmp138 * _tmp170 + _tmp149 * _tmp169 -
                                                          _tmp200 - _tmp201 - _tmp202)) /
                         std::sqrt(Scalar(std::pow(_tmp203, Scalar(2)) * _tmp211 + 1));
  const Scalar _tmp214 = Scalar(9.6622558468725703) * _tmp199;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp34 * _tmp35 * fv1 * std::cosh(_tmp36) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp29 * _tmp35 - _tmp31) -
            _tmp32 * _tmp34) *
               std::cosh(_tmp33)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp33) -
      Scalar(9.6622558468725703) * std::sinh(_tmp36);
  _res(1, 0) = _tmp161 * (-Scalar(1.0) * _tmp171 * std::cosh(_tmp160) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp168 +
                           _tmp163 * (-_tmp159 * _tmp166 - _tmp161 * _tmp171)) *
                              std::cosh(_tmp164)) +
               _tmp166 * (-std::sinh(_tmp160) - std::sinh(_tmp164));
  _res(2, 0) = Scalar(9.6622558468725703) * _tmp188 * (-std::sinh(_tmp193) - std::sinh(_tmp194)) +
               _tmp190 * (-Scalar(1.0) * _tmp191 * std::cosh(_tmp194) -
                          (-Scalar(0.1034955) * _tmp186 * _tmp189 +
                           _tmp192 * (-_tmp185 * _tmp188 - _tmp190 * _tmp191)) *
                              std::cosh(_tmp193));
  _res(3, 0) = Scalar(9.6622558468725703) * _tmp198 * (-std::sinh(_tmp209) - std::sinh(_tmp210)) +
               _tmp214 * (-Scalar(1.0) * _tmp213 * std::cosh(_tmp210) -
                          (-Scalar(0.1034955) * _tmp207 * _tmp212 +
                           _tmp208 * (-_tmp198 * _tmp206 - _tmp213 * _tmp214)) *
                              std::cosh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
