// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl15
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl15(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 644

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (213)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp7 = 2 * _tmp3;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp1 * _tmp6;
  const Scalar _tmp14 = _tmp3 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(0, 0);
  const Scalar _tmp19 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp22 = _tmp1 * _tmp7;
  const Scalar _tmp23 = _tmp6 * _tmp9;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = _tmp21 + _tmp25;
  const Scalar _tmp27 = _tmp20 + _tmp26;
  const Scalar _tmp28 = _tmp27 + position_vector(1, 0);
  const Scalar _tmp29 = _tmp0 * fv1;
  const Scalar _tmp30 = std::asinh(_tmp29);
  const Scalar _tmp31 = Scalar(9.6622558468725703) * _tmp30;
  const Scalar _tmp32 =
      -Scalar(0.1034955) * _tmp31 * fh1 -
      Scalar(0.49413274378274363) *
          std::sqrt(
              Scalar(Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp18), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp28), Scalar(2))));
  const Scalar _tmp33 = _tmp0 * _tmp32;
  const Scalar _tmp34 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp35 =
      std::pow(Scalar(_tmp34 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp36 = Scalar(1.0) * _tmp30;
  const Scalar _tmp37 = -_tmp5;
  const Scalar _tmp38 = _tmp16 + _tmp37;
  const Scalar _tmp39 = _tmp38 + position_vector(0, 0);
  const Scalar _tmp40 = _tmp39 + Scalar(1.7965602546229);
  const Scalar _tmp41 = -_tmp21 + _tmp25;
  const Scalar _tmp42 = _tmp20 + _tmp41;
  const Scalar _tmp43 = _tmp42 + position_vector(1, 0);
  const Scalar _tmp44 = _tmp43 + Scalar(-4.83288938413423);
  const Scalar _tmp45 = std::pow(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp44, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp46 = _tmp40 * _tmp45;
  const Scalar _tmp47 = _tmp12 - _tmp15;
  const Scalar _tmp48 = _tmp47 + _tmp5;
  const Scalar _tmp49 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp50 = _tmp49 + Scalar(-2.5193355532036801);
  const Scalar _tmp51 = Scalar(1.0) / (_tmp50);
  const Scalar _tmp52 = -_tmp20;
  const Scalar _tmp53 = _tmp26 + _tmp52;
  const Scalar _tmp54 = _tmp53 + position_vector(1, 0);
  const Scalar _tmp55 = _tmp54 + Scalar(8.3885017487099702);
  const Scalar _tmp56 = _tmp51 * _tmp55;
  const Scalar _tmp57 = _tmp44 * _tmp45;
  const Scalar _tmp58 = Scalar(1.0) / (_tmp46 * _tmp56 - _tmp57);
  const Scalar _tmp59 = Scalar(1.0) * _tmp48;
  const Scalar _tmp60 = Scalar(1.0) * _tmp53;
  const Scalar _tmp61 = -_tmp60;
  const Scalar _tmp62 = Scalar(1.0) / (_tmp42 + _tmp61);
  const Scalar _tmp63 = _tmp62 * (-_tmp38 + _tmp59);
  const Scalar _tmp64 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp65 = -Scalar(0.010999999999999999) * _tmp19 -
                        Scalar(0.010999999999999999) * _tmp2 + Scalar(-0.010999999999999999);
  const Scalar _tmp66 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp67 = _tmp65 - _tmp66;
  const Scalar _tmp68 = _tmp64 + _tmp67;
  const Scalar _tmp69 = _tmp56 * _tmp68;
  const Scalar _tmp70 = -_tmp64;
  const Scalar _tmp71 = _tmp65 + _tmp66;
  const Scalar _tmp72 = _tmp70 + _tmp71;
  const Scalar _tmp73 = -_tmp46 * _tmp69 + _tmp57 * _tmp72;
  const Scalar _tmp74 = Scalar(1.0) * _tmp58;
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp46 * _tmp68 - _tmp46 * _tmp72;
  const Scalar _tmp77 = _tmp63 * _tmp75 - _tmp74 * _tmp76;
  const Scalar _tmp78 = _tmp67 + _tmp70;
  const Scalar _tmp79 = _tmp37 + _tmp47;
  const Scalar _tmp80 = _tmp79 + position_vector(0, 0);
  const Scalar _tmp81 = _tmp80 + Scalar(1.9874742031097401);
  const Scalar _tmp82 = _tmp41 + _tmp52;
  const Scalar _tmp83 = _tmp82 + position_vector(1, 0);
  const Scalar _tmp84 = _tmp83 + Scalar(8.3196563720703107);
  const Scalar _tmp85 = std::pow(Scalar(std::pow(_tmp81, Scalar(2)) + std::pow(_tmp84, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp86 = _tmp81 * _tmp85;
  const Scalar _tmp87 = _tmp84 * _tmp85;
  const Scalar _tmp88 = _tmp56 * _tmp86 - _tmp87;
  const Scalar _tmp89 = _tmp58 * _tmp88;
  const Scalar _tmp90 = -_tmp69 * _tmp86 - _tmp73 * _tmp89 + _tmp78 * _tmp87;
  const Scalar _tmp91 = -_tmp63 * _tmp90 + _tmp68 * _tmp86 - _tmp76 * _tmp89 - _tmp78 * _tmp86;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 =
      std::sqrt(Scalar(std::pow(_tmp50, Scalar(2)) + std::pow(_tmp55, Scalar(2))));
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp51 * _tmp93;
  const Scalar _tmp96 = _tmp95 * (_tmp48 * _tmp55 * _tmp94 - _tmp50 * _tmp53 * _tmp94);
  const Scalar _tmp97 = -_tmp38 * _tmp57 + _tmp42 * _tmp46 + _tmp46 * _tmp96;
  const Scalar _tmp98 = -_tmp79 * _tmp87 + _tmp82 * _tmp86 + _tmp86 * _tmp96 - _tmp89 * _tmp97;
  const Scalar _tmp99 = _tmp92 * _tmp98;
  const Scalar _tmp100 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp101 = _tmp100 * _tmp91;
  const Scalar _tmp102 = _tmp101 * (-_tmp74 * _tmp97 - _tmp77 * _tmp99);
  const Scalar _tmp103 = _tmp102 + _tmp77;
  const Scalar _tmp104 = _tmp88 * _tmp92;
  const Scalar _tmp105 = _tmp58 * (-_tmp103 * _tmp104 + Scalar(1.0));
  const Scalar _tmp106 = _tmp103 * _tmp92;
  const Scalar _tmp107 = _tmp28 + Scalar(-4.7744369927459998);
  const Scalar _tmp108 = _tmp18 + Scalar(-2.7171519410699099);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp107 * _tmp109;
  const Scalar _tmp111 = _tmp110 * _tmp95 * (_tmp105 * _tmp46 + _tmp106 * _tmp86);
  const Scalar _tmp112 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp113 = _tmp59 + _tmp60 * _tmp63;
  const Scalar _tmp114 = 0;
  const Scalar _tmp115 = _tmp114 * _tmp92;
  const Scalar _tmp116 = _tmp104 * _tmp114 * _tmp58;
  const Scalar _tmp117 = _tmp56 * _tmp58;
  const Scalar _tmp118 = _tmp117 * _tmp73 + _tmp69;
  const Scalar _tmp119 = _tmp117 * _tmp76 - _tmp118 * _tmp63 - _tmp68;
  const Scalar _tmp120 = _tmp101 * (_tmp117 * _tmp97 - _tmp119 * _tmp99 - _tmp96);
  const Scalar _tmp121 = _tmp119 + _tmp120;
  const Scalar _tmp122 = _tmp58 * (-_tmp104 * _tmp121 - _tmp56);
  const Scalar _tmp123 = _tmp121 * _tmp92;
  const Scalar _tmp124 = _tmp108 * _tmp109;
  const Scalar _tmp125 = _tmp124 * _tmp95 * (_tmp122 * _tmp46 + _tmp123 * _tmp86 + Scalar(1.0));
  const Scalar _tmp126 = _tmp110 * _tmp17 - _tmp124 * _tmp27;
  const Scalar _tmp127 = Scalar(1.0) * _tmp100;
  const Scalar _tmp128 = _tmp126 * _tmp95 * (-_tmp127 * _tmp46 * _tmp89 + _tmp127 * _tmp86);
  const Scalar _tmp129 = -_tmp111 * fh1 - _tmp112 * _tmp95 * (_tmp115 * _tmp86 - _tmp116 * _tmp46) -
                         _tmp125 * fh1 - _tmp128 * fh1;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp61 + _tmp82;
  const Scalar _tmp132 = _tmp131 * _tmp63;
  const Scalar _tmp133 = Scalar(1.0) / (-_tmp132 + _tmp59 - _tmp79);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp131 * _tmp134;
  const Scalar _tmp136 = _tmp101 * _tmp135 - _tmp127 * _tmp90;
  const Scalar _tmp137 = Scalar(1.0) * _tmp62;
  const Scalar _tmp138 = _tmp101 * _tmp134;
  const Scalar _tmp139 = Scalar(1.0) * _tmp126 * (-_tmp136 * _tmp137 + _tmp138);
  const Scalar _tmp140 = _tmp64 + _tmp71;
  const Scalar _tmp141 = _tmp140 * fh1;
  const Scalar _tmp142 = Scalar(3.29616) * _tmp11 + _tmp124 * _tmp141 + _tmp17 * fv1;
  const Scalar _tmp143 = _tmp135 * _tmp62;
  const Scalar _tmp144 = -Scalar(1.0) * _tmp134 + Scalar(1.0) * _tmp143;
  const Scalar _tmp145 = _tmp113 * _tmp133;
  const Scalar _tmp146 = -_tmp115 * _tmp90 - _tmp131 * _tmp145 + _tmp61;
  const Scalar _tmp147 = -_tmp110 * _tmp141 - Scalar(3.29616) * _tmp24 - _tmp27 * fv1;
  const Scalar _tmp148 = _tmp132 * _tmp134 + Scalar(1.0);
  const Scalar _tmp149 = _tmp134 * _tmp63;
  const Scalar _tmp150 = -Scalar(1.0) * _tmp137 * _tmp148 + Scalar(1.0) * _tmp149;
  const Scalar _tmp151 = _tmp90 * _tmp92;
  const Scalar _tmp152 = _tmp131 * _tmp133;
  const Scalar _tmp153 = _tmp102 * _tmp152 - _tmp103 * _tmp151 - _tmp75;
  const Scalar _tmp154 = Scalar(1.0) * _tmp110 * (_tmp102 * _tmp134 - _tmp137 * _tmp153);
  const Scalar _tmp155 = _tmp118 + _tmp120 * _tmp152 - _tmp121 * _tmp151;
  const Scalar _tmp156 = Scalar(1.0) * _tmp124 * (_tmp120 * _tmp134 - _tmp137 * _tmp155);
  const Scalar _tmp157 =
      Scalar(1.0) * _tmp112 * (-_tmp113 * _tmp134 - _tmp137 * _tmp146 + Scalar(1.0)) +
      _tmp139 * fh1 + _tmp142 * _tmp144 + _tmp147 * _tmp150 + _tmp154 * fh1 + _tmp156 * fh1;
  const Scalar _tmp158 = std::asinh(_tmp130 * _tmp157);
  const Scalar _tmp159 = Scalar(1.0) * _tmp158;
  const Scalar _tmp160 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp161 = _tmp110 * _tmp140;
  const Scalar _tmp162 = _tmp124 * _tmp140;
  const Scalar _tmp163 = -_tmp111 - _tmp125 - _tmp128;
  const Scalar _tmp164 = _tmp160 * _tmp163;
  const Scalar _tmp165 =
      (_tmp130 * (_tmp139 + _tmp144 * _tmp162 - _tmp150 * _tmp161 + _tmp154 + _tmp156) -
       _tmp157 * _tmp164) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp167 =
      -_tmp129 * _tmp166 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp49), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp54 - 1), Scalar(2))));
  const Scalar _tmp168 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp169 = _tmp167 * _tmp168;
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp171 = _tmp126 * _tmp127;
  const Scalar _tmp172 = _tmp171 * fh1;
  const Scalar _tmp173 = _tmp122 * _tmp124;
  const Scalar _tmp174 = _tmp105 * _tmp110;
  const Scalar _tmp175 = -_tmp112 * _tmp116 - _tmp172 * _tmp89 + _tmp173 * fh1 + _tmp174 * fh1;
  const Scalar _tmp176 = _tmp124 * _tmp155 * _tmp62;
  const Scalar _tmp177 = _tmp148 * _tmp62;
  const Scalar _tmp178 = _tmp134 * _tmp142;
  const Scalar _tmp179 = _tmp126 * _tmp136 * _tmp62;
  const Scalar _tmp180 = _tmp110 * _tmp153 * _tmp62;
  const Scalar _tmp181 = _tmp112 * _tmp146 * _tmp62 - _tmp131 * _tmp178 * _tmp62 +
                         _tmp147 * _tmp177 + _tmp176 * fh1 + _tmp179 * fh1 + _tmp180 * fh1;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp175);
  const Scalar _tmp183 = std::asinh(_tmp181 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp185 =
      -_tmp175 * _tmp184 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp43), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp39 - 1), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp183;
  const Scalar _tmp189 = -_tmp171 * _tmp89 + _tmp173 + _tmp174;
  const Scalar _tmp190 = std::pow(_tmp175, Scalar(-2));
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp175;
  const Scalar _tmp193 = (-_tmp181 * _tmp191 + _tmp182 * (-_tmp143 * _tmp162 - _tmp161 * _tmp177 +
                                                          _tmp176 + _tmp179 + _tmp180)) /
                         std::sqrt(Scalar(std::pow(_tmp181, Scalar(2)) * _tmp190 + 1));
  const Scalar _tmp194 = _tmp106 * _tmp110;
  const Scalar _tmp195 = _tmp123 * _tmp124;
  const Scalar _tmp196 = _tmp112 * _tmp115 + _tmp172 + _tmp194 * fh1 + _tmp195 * fh1;
  const Scalar _tmp197 = Scalar(1.0) / (_tmp196);
  const Scalar _tmp198 = _tmp102 * _tmp110 * _tmp133;
  const Scalar _tmp199 = _tmp126 * _tmp138;
  const Scalar _tmp200 = _tmp120 * _tmp124 * _tmp133;
  const Scalar _tmp201 = _tmp112 * _tmp145 - _tmp147 * _tmp149 + _tmp178 - _tmp198 * fh1 -
                         _tmp199 * fh1 - _tmp200 * fh1;
  const Scalar _tmp202 = std::asinh(_tmp197 * _tmp201);
  const Scalar _tmp203 = Scalar(9.6622558468725703) * _tmp196;
  const Scalar _tmp204 =
      -_tmp202 * _tmp203 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp80 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp83 - 1), Scalar(2))));
  const Scalar _tmp205 = Scalar(0.1034955) * _tmp197;
  const Scalar _tmp206 = _tmp204 * _tmp205;
  const Scalar _tmp207 = Scalar(1.0) * _tmp202;
  const Scalar _tmp208 = _tmp171 + _tmp194 + _tmp195;
  const Scalar _tmp209 = Scalar(9.6622558468725703) * _tmp208;
  const Scalar _tmp210 = std::pow(_tmp196, Scalar(-2));
  const Scalar _tmp211 = _tmp208 * _tmp210;
  const Scalar _tmp212 =
      (_tmp197 * (_tmp134 * _tmp162 + _tmp149 * _tmp161 - _tmp198 - _tmp199 - _tmp200) -
       _tmp201 * _tmp211) /
      std::sqrt(Scalar(std::pow(_tmp201, Scalar(2)) * _tmp210 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp34 * _tmp35 * fv1 * std::cosh(_tmp36) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp29 * _tmp35 - _tmp31) -
            _tmp32 * _tmp34) *
               std::cosh(_tmp33)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp33) -
      Scalar(9.6622558468725703) * std::sinh(_tmp36);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp163 * (-std::sinh(_tmp159) - std::sinh(_tmp169)) +
               _tmp170 * (-Scalar(1.0) * _tmp165 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp164 * _tmp167 +
                           _tmp168 * (-_tmp163 * _tmp166 - _tmp165 * _tmp170)) *
                              std::cosh(_tmp169));
  _res(2, 0) = Scalar(9.6622558468725703) * _tmp189 * (-std::sinh(_tmp187) - std::sinh(_tmp188)) +
               _tmp192 * (-Scalar(1.0) * _tmp193 * std::cosh(_tmp188) -
                          (-Scalar(0.1034955) * _tmp185 * _tmp191 +
                           _tmp186 * (-_tmp184 * _tmp189 - _tmp192 * _tmp193)) *
                              std::cosh(_tmp187));
  _res(3, 0) = _tmp203 * (-Scalar(1.0) * _tmp212 * std::cosh(_tmp207) -
                          (-Scalar(0.1034955) * _tmp204 * _tmp211 +
                           _tmp205 * (-_tmp202 * _tmp209 - _tmp203 * _tmp212)) *
                              std::cosh(_tmp206)) +
               _tmp209 * (-std::sinh(_tmp206) - std::sinh(_tmp207));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
