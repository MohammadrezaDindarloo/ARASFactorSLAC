// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl4
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl4(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 645

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp1 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp13 = _tmp3 * _tmp8;
  const Scalar _tmp14 = _tmp1 * _tmp10;
  const Scalar _tmp15 = _tmp13 - _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = _tmp0 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -Scalar(0.1034955) * _tmp33 * fh1 -
      Scalar(0.86104699605560286) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp19 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp30 - 1), Scalar(2))));
  const Scalar _tmp35 = _tmp0 * _tmp34;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(1.0) * _tmp32;
  const Scalar _tmp39 = _tmp26 + _tmp27;
  const Scalar _tmp40 = _tmp22 + _tmp39;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp17 + _tmp5;
  const Scalar _tmp43 = _tmp42 + position_vector(1, 0);
  const Scalar _tmp44 = _tmp12 + _tmp16;
  const Scalar _tmp45 = _tmp44 + _tmp5;
  const Scalar _tmp46 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(-4.7744369927459998);
  const Scalar _tmp48 = _tmp21 + _tmp39;
  const Scalar _tmp49 = _tmp48 + position_vector(0, 0);
  const Scalar _tmp50 = _tmp49 + Scalar(-2.7171519410699099);
  const Scalar _tmp51 = std::pow(Scalar(std::pow(_tmp47, Scalar(2)) + std::pow(_tmp50, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp52 = _tmp50 * _tmp51;
  const Scalar _tmp53 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp54 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp55 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp56 = _tmp54 - _tmp55;
  const Scalar _tmp57 = _tmp53 + _tmp56;
  const Scalar _tmp58 = _tmp52 * _tmp57;
  const Scalar _tmp59 = _tmp54 + _tmp55;
  const Scalar _tmp60 = _tmp53 + _tmp59;
  const Scalar _tmp61 = -_tmp53;
  const Scalar _tmp62 = _tmp59 + _tmp61;
  const Scalar _tmp63 = _tmp44 + _tmp6;
  const Scalar _tmp64 = _tmp63 + position_vector(1, 0);
  const Scalar _tmp65 = _tmp64 + Scalar(8.3885017487099702);
  const Scalar _tmp66 = _tmp21 + _tmp28;
  const Scalar _tmp67 = _tmp66 + position_vector(0, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(-2.5193355532036801);
  const Scalar _tmp69 = std::pow(Scalar(std::pow(_tmp65, Scalar(2)) + std::pow(_tmp68, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp70 = _tmp65 * _tmp69;
  const Scalar _tmp71 = _tmp43 + Scalar(-4.83288938413423);
  const Scalar _tmp72 = _tmp41 + Scalar(1.7965602546229);
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = _tmp71 * _tmp73;
  const Scalar _tmp75 = _tmp68 * _tmp69;
  const Scalar _tmp76 = _tmp57 * _tmp75;
  const Scalar _tmp77 = _tmp62 * _tmp70 - _tmp74 * _tmp76;
  const Scalar _tmp78 = _tmp47 * _tmp51;
  const Scalar _tmp79 = _tmp52 * _tmp74 - _tmp78;
  const Scalar _tmp80 = Scalar(1.0) / (-_tmp70 + _tmp74 * _tmp75);
  const Scalar _tmp81 = _tmp79 * _tmp80;
  const Scalar _tmp82 = -_tmp58 * _tmp74 + _tmp60 * _tmp78 - _tmp77 * _tmp81;
  const Scalar _tmp83 = Scalar(1.0) * _tmp42;
  const Scalar _tmp84 = -_tmp83;
  const Scalar _tmp85 = Scalar(1.0) / (_tmp63 + _tmp84);
  const Scalar _tmp86 = Scalar(1.0) * _tmp40;
  const Scalar _tmp87 = -_tmp66 + _tmp86;
  const Scalar _tmp88 = _tmp85 * _tmp87;
  const Scalar _tmp89 = -_tmp62 * _tmp75 + _tmp76;
  const Scalar _tmp90 = -_tmp52 * _tmp60 + _tmp58 - _tmp81 * _tmp89 - _tmp82 * _tmp88;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 =
      std::sqrt(Scalar(std::pow(_tmp71, Scalar(2)) + std::pow(_tmp72, Scalar(2))));
  const Scalar _tmp93 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp94 = _tmp73 * _tmp92;
  const Scalar _tmp95 = _tmp94 * (_tmp40 * _tmp71 * _tmp93 - _tmp42 * _tmp72 * _tmp93);
  const Scalar _tmp96 = _tmp63 * _tmp75 - _tmp66 * _tmp70 + _tmp75 * _tmp95;
  const Scalar _tmp97 = _tmp74 * _tmp80;
  const Scalar _tmp98 = _tmp57 * _tmp74 + _tmp77 * _tmp97;
  const Scalar _tmp99 = -_tmp57 - _tmp88 * _tmp98 + _tmp89 * _tmp97;
  const Scalar _tmp100 = _tmp45 * _tmp52 - _tmp48 * _tmp78 + _tmp52 * _tmp95 - _tmp81 * _tmp96;
  const Scalar _tmp101 = _tmp100 * _tmp91;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp90;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp99 - _tmp95 + _tmp96 * _tmp97);
  const Scalar _tmp105 = _tmp104 + _tmp99;
  const Scalar _tmp106 = _tmp105 * _tmp91;
  const Scalar _tmp107 = _tmp79 * _tmp91;
  const Scalar _tmp108 = _tmp80 * (-_tmp105 * _tmp107 - _tmp74);
  const Scalar _tmp109 = _tmp30 + Scalar(1.9874742031097401);
  const Scalar _tmp110 = _tmp19 + Scalar(8.3196563720703107);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp109 * _tmp111;
  const Scalar _tmp113 = _tmp112 * _tmp94 * (_tmp106 * _tmp52 + _tmp108 * _tmp75 + Scalar(1.0));
  const Scalar _tmp114 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp115 = _tmp83 * _tmp88 + _tmp86;
  const Scalar _tmp116 = 0;
  const Scalar _tmp117 = _tmp116 * _tmp91;
  const Scalar _tmp118 = _tmp107 * _tmp116 * _tmp80;
  const Scalar _tmp119 = _tmp110 * _tmp111;
  const Scalar _tmp120 = -_tmp112 * _tmp18 + _tmp119 * _tmp29;
  const Scalar _tmp121 = Scalar(1.0) * _tmp102;
  const Scalar _tmp122 = _tmp120 * _tmp94 * (_tmp121 * _tmp52 - _tmp121 * _tmp75 * _tmp81);
  const Scalar _tmp123 = Scalar(1.0) * _tmp85;
  const Scalar _tmp124 = Scalar(1.0) * _tmp80;
  const Scalar _tmp125 = _tmp123 * _tmp77 * _tmp80 * _tmp87 - _tmp124 * _tmp89;
  const Scalar _tmp126 = _tmp103 * (-_tmp101 * _tmp125 - _tmp124 * _tmp96);
  const Scalar _tmp127 = _tmp125 + _tmp126;
  const Scalar _tmp128 = _tmp80 * (-_tmp107 * _tmp127 + Scalar(1.0));
  const Scalar _tmp129 = _tmp127 * _tmp91;
  const Scalar _tmp130 = _tmp119 * _tmp94 * (_tmp128 * _tmp75 + _tmp129 * _tmp52);
  const Scalar _tmp131 = -_tmp113 * fh1 - _tmp114 * _tmp94 * (_tmp117 * _tmp52 - _tmp118 * _tmp75) -
                         _tmp122 * fh1 - _tmp130 * fh1;
  const Scalar _tmp132 = Scalar(1.0) / (_tmp131);
  const Scalar _tmp133 = _tmp56 + _tmp61;
  const Scalar _tmp134 = _tmp133 * fh1;
  const Scalar _tmp135 = -_tmp119 * _tmp134 - Scalar(3.29616) * _tmp15 - _tmp18 * fv1;
  const Scalar _tmp136 = _tmp45 + _tmp84;
  const Scalar _tmp137 = _tmp136 * _tmp88;
  const Scalar _tmp138 = Scalar(1.0) / (-_tmp137 - _tmp48 + _tmp86);
  const Scalar _tmp139 = Scalar(1.0) * _tmp138;
  const Scalar _tmp140 = _tmp137 * _tmp139 + Scalar(1.0);
  const Scalar _tmp141 = _tmp139 * _tmp88;
  const Scalar _tmp142 = -Scalar(1.0) * _tmp123 * _tmp140 + Scalar(1.0) * _tmp141;
  const Scalar _tmp143 = _tmp136 * _tmp138;
  const Scalar _tmp144 = _tmp82 * _tmp91;
  const Scalar _tmp145 = -_tmp124 * _tmp77 + _tmp126 * _tmp143 - _tmp127 * _tmp144;
  const Scalar _tmp146 = Scalar(1.0) * _tmp119 * (-_tmp123 * _tmp145 + _tmp126 * _tmp139);
  const Scalar _tmp147 = _tmp112 * _tmp134 + Scalar(3.29616) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp148 = _tmp136 * _tmp139;
  const Scalar _tmp149 = _tmp148 * _tmp85;
  const Scalar _tmp150 = -Scalar(1.0) * _tmp139 + Scalar(1.0) * _tmp149;
  const Scalar _tmp151 = _tmp103 * _tmp148 - _tmp121 * _tmp82;
  const Scalar _tmp152 = _tmp103 * _tmp139;
  const Scalar _tmp153 = Scalar(1.0) * _tmp120;
  const Scalar _tmp154 = _tmp153 * (-_tmp123 * _tmp151 + _tmp152);
  const Scalar _tmp155 = _tmp104 * _tmp143 - _tmp105 * _tmp144 + _tmp98;
  const Scalar _tmp156 = Scalar(1.0) * _tmp112 * (_tmp104 * _tmp139 - _tmp123 * _tmp155);
  const Scalar _tmp157 = _tmp115 * _tmp138;
  const Scalar _tmp158 = -_tmp116 * _tmp144 - _tmp136 * _tmp157 + _tmp84;
  const Scalar _tmp159 =
      Scalar(1.0) * _tmp114 * (-_tmp115 * _tmp139 - _tmp123 * _tmp158 + Scalar(1.0)) +
      _tmp135 * _tmp142 + _tmp146 * fh1 + _tmp147 * _tmp150 + _tmp154 * fh1 + _tmp156 * fh1;
  const Scalar _tmp160 = std::asinh(_tmp132 * _tmp159);
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp160;
  const Scalar _tmp162 =
      -_tmp131 * _tmp161 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp43), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp41 - 1), Scalar(2))));
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp132;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = std::pow(_tmp131, Scalar(-2));
  const Scalar _tmp166 = -_tmp113 - _tmp122 - _tmp130;
  const Scalar _tmp167 = _tmp165 * _tmp166;
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp131;
  const Scalar _tmp169 = _tmp112 * _tmp133;
  const Scalar _tmp170 = _tmp119 * _tmp133;
  const Scalar _tmp171 =
      (_tmp132 * (-_tmp142 * _tmp170 + _tmp146 + _tmp150 * _tmp169 + _tmp154 + _tmp156) -
       _tmp159 * _tmp167) /
      std::sqrt(Scalar(std::pow(_tmp159, Scalar(2)) * _tmp165 + 1));
  const Scalar _tmp172 = Scalar(1.0) * _tmp160;
  const Scalar _tmp173 = _tmp120 * _tmp151 * _tmp85;
  const Scalar _tmp174 = _tmp140 * _tmp85;
  const Scalar _tmp175 = _tmp119 * _tmp145 * _tmp85;
  const Scalar _tmp176 = _tmp139 * _tmp147;
  const Scalar _tmp177 = _tmp112 * _tmp155 * _tmp85;
  const Scalar _tmp178 = _tmp114 * _tmp158 * _tmp85 + _tmp135 * _tmp174 -
                         _tmp136 * _tmp176 * _tmp85 + _tmp173 * fh1 + _tmp175 * fh1 + _tmp177 * fh1;
  const Scalar _tmp179 = _tmp108 * _tmp112;
  const Scalar _tmp180 = _tmp102 * _tmp153;
  const Scalar _tmp181 = _tmp180 * fh1;
  const Scalar _tmp182 = _tmp119 * _tmp128;
  const Scalar _tmp183 = -_tmp114 * _tmp118 + _tmp179 * fh1 - _tmp181 * _tmp81 + _tmp182 * fh1;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp178 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp188 =
      -_tmp185 * _tmp187 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp67), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp64 - 1), Scalar(2))));
  const Scalar _tmp189 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp190 = _tmp188 * _tmp189;
  const Scalar _tmp191 = _tmp179 - _tmp180 * _tmp81 + _tmp182;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp191;
  const Scalar _tmp193 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp194 = _tmp191 * _tmp193;
  const Scalar _tmp195 = (-_tmp178 * _tmp194 + _tmp184 * (-_tmp149 * _tmp169 - _tmp170 * _tmp174 +
                                                          _tmp173 + _tmp175 + _tmp177)) /
                         std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp196 = _tmp120 * _tmp152;
  const Scalar _tmp197 = _tmp104 * _tmp112 * _tmp138;
  const Scalar _tmp198 = _tmp119 * _tmp126 * _tmp138;
  const Scalar _tmp199 = _tmp114 * _tmp157 - _tmp135 * _tmp141 + _tmp176 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = _tmp119 * _tmp129;
  const Scalar _tmp201 = _tmp106 * _tmp112;
  const Scalar _tmp202 = _tmp114 * _tmp117 + _tmp181 + _tmp200 * fh1 + _tmp201 * fh1;
  const Scalar _tmp203 = Scalar(1.0) / (_tmp202);
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp202;
  const Scalar _tmp207 =
      -_tmp204 * _tmp206 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp46), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp49), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp203;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = _tmp180 + _tmp200 + _tmp201;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp210;
  const Scalar _tmp212 = std::pow(_tmp202, Scalar(-2));
  const Scalar _tmp213 = _tmp210 * _tmp212;
  const Scalar _tmp214 = (-_tmp199 * _tmp213 + _tmp203 * (_tmp139 * _tmp169 + _tmp141 * _tmp170 -
                                                          _tmp196 - _tmp197 - _tmp198)) /
                         std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp212 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp38) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp31 * _tmp37 - _tmp33) -
            _tmp34 * _tmp36) *
               std::cosh(_tmp35)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35) -
      Scalar(9.6622558468725703) * std::sinh(_tmp38);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp166 * (-std::sinh(_tmp164) - std::sinh(_tmp172)) +
               _tmp168 * (-Scalar(1.0) * _tmp171 * std::cosh(_tmp172) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp167 +
                           _tmp163 * (-_tmp161 * _tmp166 - _tmp168 * _tmp171)) *
                              std::cosh(_tmp164));
  _res(2, 0) = _tmp187 * (-Scalar(1.0) * _tmp195 * std::cosh(_tmp186) -
                          (-Scalar(0.1034955) * _tmp188 * _tmp194 +
                           _tmp189 * (-_tmp185 * _tmp192 - _tmp187 * _tmp195)) *
                              std::cosh(_tmp190)) +
               _tmp192 * (-std::sinh(_tmp186) - std::sinh(_tmp190));
  _res(3, 0) = _tmp206 * (-Scalar(1.0) * _tmp214 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp207 * _tmp213 +
                           _tmp208 * (-_tmp204 * _tmp211 - _tmp206 * _tmp214)) *
                              std::cosh(_tmp209)) +
               _tmp211 * (-std::sinh(_tmp205) - std::sinh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
