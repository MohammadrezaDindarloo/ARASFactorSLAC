// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl5
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl5(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 643

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (216)
  const Scalar _tmp0 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp3 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp7 = 2 * _tmp0 * _tmp6;
  const Scalar _tmp8 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp9 = _tmp2 * _tmp8;
  const Scalar _tmp10 = Scalar(0.20999999999999999) * _tmp7 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp11 = 2 * _tmp2;
  const Scalar _tmp12 = _tmp11 * _tmp6;
  const Scalar _tmp13 = _tmp0 * _tmp8;
  const Scalar _tmp14 = _tmp12 - _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = -_tmp10 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + position_vector(1, 0);
  const Scalar _tmp19 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = _tmp0 * _tmp11;
  const Scalar _tmp23 = _tmp6 * _tmp8;
  const Scalar _tmp24 = _tmp22 + _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp7 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = Scalar(1.0) / (fh1);
  const Scalar _tmp31 = _tmp30 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -_tmp33 * fh1 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp18 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp29 - 1), Scalar(2))));
  const Scalar _tmp35 = Scalar(0.1034955) * _tmp30;
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp32;
  const Scalar _tmp40 = _tmp25 + _tmp26;
  const Scalar _tmp41 = _tmp21 + _tmp40;
  const Scalar _tmp42 = _tmp41 + position_vector(0, 0);
  const Scalar _tmp43 = _tmp16 + _tmp4;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp10 + _tmp15;
  const Scalar _tmp46 = _tmp4 + _tmp45;
  const Scalar _tmp47 = _tmp46 + position_vector(1, 0);
  const Scalar _tmp48 = _tmp47 + Scalar(-4.7744369927459998);
  const Scalar _tmp49 = _tmp20 + _tmp40;
  const Scalar _tmp50 = _tmp49 + position_vector(0, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(-2.7171519410699099);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp51 * _tmp52;
  const Scalar _tmp54 = _tmp44 + Scalar(-4.83288938413423);
  const Scalar _tmp55 = _tmp42 + Scalar(1.7965602546229);
  const Scalar _tmp56 = Scalar(1.0) / (_tmp55);
  const Scalar _tmp57 = _tmp54 * _tmp56;
  const Scalar _tmp58 = _tmp48 * _tmp52;
  const Scalar _tmp59 = Scalar(1.0) / (_tmp53 * _tmp57 - _tmp58);
  const Scalar _tmp60 =
      std::sqrt(Scalar(std::pow(_tmp54, Scalar(2)) + std::pow(_tmp55, Scalar(2))));
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = _tmp56 * _tmp60;
  const Scalar _tmp63 = _tmp62 * (_tmp41 * _tmp54 * _tmp61 - _tmp43 * _tmp55 * _tmp61);
  const Scalar _tmp64 = _tmp46 * _tmp53 - _tmp49 * _tmp58 + _tmp53 * _tmp63;
  const Scalar _tmp65 = _tmp57 * _tmp59;
  const Scalar _tmp66 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp67 =
      -Scalar(0.010999999999999999) * _tmp1 - Scalar(0.010999999999999999) * _tmp19;
  const Scalar _tmp68 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp69 = _tmp67 - _tmp68;
  const Scalar _tmp70 = _tmp66 + _tmp69;
  const Scalar _tmp71 = _tmp67 + _tmp68;
  const Scalar _tmp72 = _tmp66 + _tmp71;
  const Scalar _tmp73 = _tmp53 * _tmp70;
  const Scalar _tmp74 = -_tmp57 * _tmp73 + _tmp58 * _tmp72;
  const Scalar _tmp75 = _tmp57 * _tmp70 + _tmp65 * _tmp74;
  const Scalar _tmp76 = Scalar(1.0) * _tmp43;
  const Scalar _tmp77 = -_tmp76;
  const Scalar _tmp78 = Scalar(1.0) / (_tmp46 + _tmp77);
  const Scalar _tmp79 = Scalar(1.0) * _tmp41;
  const Scalar _tmp80 = _tmp78 * (-_tmp49 + _tmp79);
  const Scalar _tmp81 = -_tmp53 * _tmp72 + _tmp73;
  const Scalar _tmp82 = _tmp65 * _tmp81 - _tmp70 - _tmp75 * _tmp80;
  const Scalar _tmp83 = _tmp45 + _tmp5;
  const Scalar _tmp84 = _tmp83 + position_vector(1, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(8.3885017487099702);
  const Scalar _tmp86 = _tmp20 + _tmp27;
  const Scalar _tmp87 = _tmp86 + position_vector(0, 0);
  const Scalar _tmp88 = _tmp87 + Scalar(-2.5193355532036801);
  const Scalar _tmp89 = std::pow(Scalar(std::pow(_tmp85, Scalar(2)) + std::pow(_tmp88, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp90 = _tmp88 * _tmp89;
  const Scalar _tmp91 = _tmp70 * _tmp90;
  const Scalar _tmp92 = -_tmp66;
  const Scalar _tmp93 = _tmp71 + _tmp92;
  const Scalar _tmp94 = _tmp85 * _tmp89;
  const Scalar _tmp95 = _tmp57 * _tmp90 - _tmp94;
  const Scalar _tmp96 = _tmp59 * _tmp95;
  const Scalar _tmp97 = -_tmp57 * _tmp91 - _tmp74 * _tmp96 + _tmp93 * _tmp94;
  const Scalar _tmp98 = -_tmp80 * _tmp97 - _tmp81 * _tmp96 - _tmp90 * _tmp93 + _tmp91;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp100 = _tmp63 * _tmp90 - _tmp64 * _tmp96 + _tmp83 * _tmp90 - _tmp86 * _tmp94;
  const Scalar _tmp101 = _tmp100 * _tmp99;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp98;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp82 - _tmp63 + _tmp64 * _tmp65);
  const Scalar _tmp105 = _tmp104 + _tmp82;
  const Scalar _tmp106 = _tmp95 * _tmp99;
  const Scalar _tmp107 = _tmp59 * (-_tmp105 * _tmp106 - _tmp57);
  const Scalar _tmp108 = _tmp90 * _tmp99;
  const Scalar _tmp109 = _tmp29 + Scalar(1.9874742031097401);
  const Scalar _tmp110 = _tmp18 + Scalar(8.3196563720703107);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp109 * _tmp111;
  const Scalar _tmp113 = _tmp112 * _tmp62 * (_tmp105 * _tmp108 + _tmp107 * _tmp53 + Scalar(1.0));
  const Scalar _tmp114 = Scalar(1.0) * _tmp59;
  const Scalar _tmp115 = _tmp114 * _tmp74;
  const Scalar _tmp116 = -_tmp114 * _tmp81 + _tmp115 * _tmp80;
  const Scalar _tmp117 = _tmp103 * (-_tmp101 * _tmp116 - _tmp114 * _tmp64);
  const Scalar _tmp118 = _tmp116 + _tmp117;
  const Scalar _tmp119 = _tmp59 * (-_tmp106 * _tmp118 + Scalar(1.0));
  const Scalar _tmp120 = _tmp110 * _tmp111;
  const Scalar _tmp121 = _tmp120 * _tmp62 * (_tmp108 * _tmp118 + _tmp119 * _tmp53);
  const Scalar _tmp122 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp123 = _tmp76 * _tmp80 + _tmp79;
  const Scalar _tmp124 = 0;
  const Scalar _tmp125 = _tmp124 * _tmp99;
  const Scalar _tmp126 = _tmp53 * _tmp96;
  const Scalar _tmp127 = -_tmp112 * _tmp17 + _tmp120 * _tmp28;
  const Scalar _tmp128 = Scalar(1.0) * _tmp102;
  const Scalar _tmp129 = _tmp127 * _tmp62 * (-_tmp126 * _tmp128 + _tmp128 * _tmp90);
  const Scalar _tmp130 = -_tmp113 * fh1 - _tmp121 * fh1 -
                         _tmp122 * _tmp62 * (-_tmp125 * _tmp126 + _tmp125 * _tmp90) - _tmp129 * fh1;
  const Scalar _tmp131 = Scalar(1.0) / (_tmp130);
  const Scalar _tmp132 = _tmp77 + _tmp83;
  const Scalar _tmp133 = _tmp132 * _tmp80;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp79 - _tmp86);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp132 * _tmp134;
  const Scalar _tmp137 = _tmp97 * _tmp99;
  const Scalar _tmp138 = -_tmp115 + _tmp117 * _tmp136 - _tmp118 * _tmp137;
  const Scalar _tmp139 = Scalar(1.0) * _tmp78;
  const Scalar _tmp140 = Scalar(1.0) * _tmp120 * (_tmp117 * _tmp135 - _tmp138 * _tmp139);
  const Scalar _tmp141 = _tmp69 + _tmp92;
  const Scalar _tmp142 = _tmp141 * fh1;
  const Scalar _tmp143 = _tmp112 * _tmp142 + Scalar(3.29616) * _tmp24 + _tmp28 * fv1;
  const Scalar _tmp144 = _tmp132 * _tmp78;
  const Scalar _tmp145 = Scalar(1.0) * _tmp135 * _tmp144 - Scalar(1.0) * _tmp135;
  const Scalar _tmp146 = _tmp104 * _tmp136 - _tmp105 * _tmp137 + _tmp75;
  const Scalar _tmp147 = Scalar(1.0) * _tmp112 * (_tmp104 * _tmp135 - _tmp139 * _tmp146);
  const Scalar _tmp148 = _tmp103 * _tmp135;
  const Scalar _tmp149 = -_tmp128 * _tmp97 + _tmp132 * _tmp148;
  const Scalar _tmp150 = Scalar(1.0) * _tmp127;
  const Scalar _tmp151 = _tmp150 * (-_tmp139 * _tmp149 + _tmp148);
  const Scalar _tmp152 = _tmp123 * _tmp134;
  const Scalar _tmp153 = -_tmp124 * _tmp137 - _tmp132 * _tmp152 + _tmp77;
  const Scalar _tmp154 = -_tmp120 * _tmp142 - Scalar(3.29616) * _tmp14 - _tmp17 * fv1;
  const Scalar _tmp155 = _tmp133 * _tmp135 + Scalar(1.0);
  const Scalar _tmp156 = _tmp135 * _tmp80;
  const Scalar _tmp157 = -Scalar(1.0) * _tmp139 * _tmp155 + Scalar(1.0) * _tmp156;
  const Scalar _tmp158 =
      Scalar(1.0) * _tmp122 * (-_tmp123 * _tmp135 - _tmp139 * _tmp153 + Scalar(1.0)) +
      _tmp140 * fh1 + _tmp143 * _tmp145 + _tmp147 * fh1 + _tmp151 * fh1 + _tmp154 * _tmp157;
  const Scalar _tmp159 = std::asinh(_tmp131 * _tmp158);
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp161 =
      -_tmp130 * _tmp160 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp44), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp42 - 1), Scalar(2))));
  const Scalar _tmp162 = std::pow(_tmp130, Scalar(-2));
  const Scalar _tmp163 = -_tmp113 - _tmp121 - _tmp129;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp130;
  const Scalar _tmp166 = _tmp112 * _tmp141;
  const Scalar _tmp167 = _tmp120 * _tmp141;
  const Scalar _tmp168 =
      (_tmp131 * (_tmp140 + _tmp145 * _tmp166 + _tmp147 + _tmp151 - _tmp157 * _tmp167) -
       _tmp158 * _tmp164) /
      std::sqrt(Scalar(std::pow(_tmp158, Scalar(2)) * _tmp162 + 1));
  const Scalar _tmp169 = Scalar(0.1034955) * _tmp131;
  const Scalar _tmp170 = _tmp161 * _tmp169;
  const Scalar _tmp171 = Scalar(1.0) * _tmp159;
  const Scalar _tmp172 = _tmp120 * _tmp138 * _tmp78;
  const Scalar _tmp173 = _tmp155 * _tmp78;
  const Scalar _tmp174 = _tmp135 * _tmp143;
  const Scalar _tmp175 = _tmp127 * _tmp149 * _tmp78;
  const Scalar _tmp176 = _tmp112 * _tmp78;
  const Scalar _tmp177 = _tmp146 * _tmp176;
  const Scalar _tmp178 = _tmp122 * _tmp153 * _tmp78 - _tmp144 * _tmp174 + _tmp154 * _tmp173 +
                         _tmp172 * fh1 + _tmp175 * fh1 + _tmp177 * fh1;
  const Scalar _tmp179 = _tmp102 * _tmp150;
  const Scalar _tmp180 = _tmp179 * fh1;
  const Scalar _tmp181 = _tmp119 * _tmp120;
  const Scalar _tmp182 = _tmp122 * _tmp125;
  const Scalar _tmp183 = _tmp107 * _tmp112;
  const Scalar _tmp184 = -_tmp180 * _tmp96 + _tmp181 * fh1 - _tmp182 * _tmp96 + _tmp183 * fh1;
  const Scalar _tmp185 = Scalar(1.0) / (_tmp184);
  const Scalar _tmp186 = std::asinh(_tmp178 * _tmp185);
  const Scalar _tmp187 = Scalar(1.0) * _tmp186;
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp184;
  const Scalar _tmp189 =
      -_tmp186 * _tmp188 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp47), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp50), Scalar(2))));
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp185;
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 = -_tmp179 * _tmp96 + _tmp181 + _tmp183;
  const Scalar _tmp193 = Scalar(9.6622558468725703) * _tmp192;
  const Scalar _tmp194 = std::pow(_tmp184, Scalar(-2));
  const Scalar _tmp195 = _tmp192 * _tmp194;
  const Scalar _tmp196 =
      (-_tmp178 * _tmp195 + _tmp185 * (-_tmp132 * _tmp135 * _tmp141 * _tmp176 - _tmp167 * _tmp173 +
                                       _tmp172 + _tmp175 + _tmp177)) /
      std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp194 + 1));
  const Scalar _tmp197 = _tmp118 * _tmp120 * _tmp99;
  const Scalar _tmp198 = _tmp105 * _tmp112 * _tmp99;
  const Scalar _tmp199 = _tmp180 + _tmp182 + _tmp197 * fh1 + _tmp198 * fh1;
  const Scalar _tmp200 = Scalar(1.0) / (_tmp199);
  const Scalar _tmp201 = _tmp104 * _tmp112 * _tmp134;
  const Scalar _tmp202 = _tmp127 * _tmp148;
  const Scalar _tmp203 = _tmp117 * _tmp120 * _tmp134;
  const Scalar _tmp204 = _tmp122 * _tmp152 - _tmp154 * _tmp156 + _tmp174 - _tmp201 * fh1 -
                         _tmp202 * fh1 - _tmp203 * fh1;
  const Scalar _tmp205 = std::asinh(_tmp200 * _tmp204);
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp199;
  const Scalar _tmp207 =
      -_tmp205 * _tmp206 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp87), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp84 - 1), Scalar(2))));
  const Scalar _tmp208 = Scalar(0.1034955) * _tmp200;
  const Scalar _tmp209 = _tmp207 * _tmp208;
  const Scalar _tmp210 = Scalar(1.0) * _tmp205;
  const Scalar _tmp211 = _tmp179 + _tmp197 + _tmp198;
  const Scalar _tmp212 = Scalar(9.6622558468725703) * _tmp211;
  const Scalar _tmp213 = std::pow(_tmp199, Scalar(-2));
  const Scalar _tmp214 = _tmp211 * _tmp213;
  const Scalar _tmp215 =
      (_tmp200 * (_tmp135 * _tmp166 + _tmp156 * _tmp167 - _tmp201 - _tmp202 - _tmp203) -
       _tmp204 * _tmp214) /
      std::sqrt(Scalar(std::pow(_tmp204, Scalar(2)) * _tmp213 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp39) -
                    (-Scalar(0.1034955) * _tmp34 * _tmp37 +
                     _tmp35 * (Scalar(9.6622558468725703) * _tmp31 * _tmp38 - _tmp33)) *
                        std::cosh(_tmp36)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp36) -
               Scalar(9.6622558468725703) * std::sinh(_tmp39);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp163 * (-std::sinh(_tmp170) - std::sinh(_tmp171)) +
               _tmp165 * (-Scalar(1.0) * _tmp168 * std::cosh(_tmp171) -
                          (-Scalar(0.1034955) * _tmp161 * _tmp164 +
                           _tmp169 * (-_tmp160 * _tmp163 - _tmp165 * _tmp168)) *
                              std::cosh(_tmp170));
  _res(2, 0) = _tmp188 * (-Scalar(1.0) * _tmp196 * std::cosh(_tmp187) -
                          (-Scalar(0.1034955) * _tmp189 * _tmp195 +
                           _tmp190 * (-_tmp186 * _tmp193 - _tmp188 * _tmp196)) *
                              std::cosh(_tmp191)) +
               _tmp193 * (-std::sinh(_tmp187) - std::sinh(_tmp191));
  _res(3, 0) = _tmp206 * (-Scalar(1.0) * _tmp215 * std::cosh(_tmp210) -
                          (-Scalar(0.1034955) * _tmp207 * _tmp214 +
                           _tmp208 * (-_tmp205 * _tmp212 - _tmp206 * _tmp215)) *
                              std::cosh(_tmp209)) +
               _tmp212 * (-std::sinh(_tmp209) - std::sinh(_tmp210));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
