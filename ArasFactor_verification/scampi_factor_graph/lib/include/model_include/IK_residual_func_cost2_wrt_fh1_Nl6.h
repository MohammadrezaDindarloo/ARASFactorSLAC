// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl6
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl6(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 626

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp7 = 2 * _tmp6;
  const Scalar _tmp8 = _tmp3 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = _tmp1 * _tmp7;
  const Scalar _tmp14 = _tmp3 * _tmp9;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp12 - _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + p_init0;
  const Scalar _tmp19 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp19 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp23 = _tmp6 * _tmp9;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp27 = _tmp25 + _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + p_init1;
  const Scalar _tmp30 = _tmp0 * fv1;
  const Scalar _tmp31 = std::asinh(_tmp30);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * _tmp31;
  const Scalar _tmp33 =
      -Scalar(0.1034955) * _tmp32 * fh1 -
      Scalar(0.1034955) * std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_b(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp29 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp34 = _tmp0 * _tmp33;
  const Scalar _tmp35 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp36 =
      std::pow(Scalar(_tmp35 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp37 = Scalar(1.0) * _tmp31;
  const Scalar _tmp38 = -_tmp5;
  const Scalar _tmp39 = _tmp12 + _tmp15;
  const Scalar _tmp40 = _tmp38 + _tmp39;
  const Scalar _tmp41 = _tmp16 + _tmp38;
  const Scalar _tmp42 = Scalar(1.0) * _tmp41;
  const Scalar _tmp43 = _tmp25 - _tmp26;
  const Scalar _tmp44 = _tmp21 + _tmp43;
  const Scalar _tmp45 = Scalar(1.0) * _tmp44;
  const Scalar _tmp46 = -_tmp45;
  const Scalar _tmp47 = _tmp20 + _tmp43;
  const Scalar _tmp48 = _tmp46 + _tmp47;
  const Scalar _tmp49 = _tmp20 + _tmp27;
  const Scalar _tmp50 = Scalar(1.0) / (_tmp46 + _tmp49);
  const Scalar _tmp51 = _tmp39 + _tmp5;
  const Scalar _tmp52 = _tmp42 - _tmp51;
  const Scalar _tmp53 = _tmp50 * _tmp52;
  const Scalar _tmp54 = _tmp48 * _tmp53;
  const Scalar _tmp55 = Scalar(1.0) / (-_tmp40 + _tmp42 - _tmp54);
  const Scalar _tmp56 = Scalar(1.0) * _tmp55;
  const Scalar _tmp57 = _tmp47 + p_init1;
  const Scalar _tmp58 = _tmp57 - p_d(1, 0);
  const Scalar _tmp59 = _tmp40 + p_init0;
  const Scalar _tmp60 = _tmp59 - p_d(0, 0);
  const Scalar _tmp61 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp60, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp62 = _tmp60 * _tmp61;
  const Scalar _tmp63 = _tmp41 + p_init0;
  const Scalar _tmp64 = _tmp63 - p_a(0, 0);
  const Scalar _tmp65 = _tmp44 + p_init1;
  const Scalar _tmp66 = _tmp65 - p_a(1, 0);
  const Scalar _tmp67 =
      std::sqrt(Scalar(std::pow(_tmp64, Scalar(2)) + std::pow(_tmp66, Scalar(2))));
  const Scalar _tmp68 = Scalar(1.0) / (_tmp67);
  const Scalar _tmp69 = Scalar(1.0) / (_tmp64);
  const Scalar _tmp70 = _tmp67 * _tmp69;
  const Scalar _tmp71 = _tmp70 * (_tmp41 * _tmp66 * _tmp68 - _tmp44 * _tmp64 * _tmp68);
  const Scalar _tmp72 = _tmp58 * _tmp61;
  const Scalar _tmp73 = _tmp66 * _tmp69;
  const Scalar _tmp74 = _tmp62 * _tmp73 - _tmp72;
  const Scalar _tmp75 = _tmp51 + p_init0;
  const Scalar _tmp76 = _tmp75 - p_c(0, 0);
  const Scalar _tmp77 = _tmp49 + p_init1;
  const Scalar _tmp78 = _tmp77 - p_c(1, 0);
  const Scalar _tmp79 = std::pow(Scalar(std::pow(_tmp76, Scalar(2)) + std::pow(_tmp78, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp80 = _tmp76 * _tmp79;
  const Scalar _tmp81 = _tmp78 * _tmp79;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp73 * _tmp80 - _tmp81);
  const Scalar _tmp83 = _tmp82 * (_tmp49 * _tmp80 - _tmp51 * _tmp81 + _tmp71 * _tmp80);
  const Scalar _tmp84 = -_tmp40 * _tmp72 + _tmp47 * _tmp62 + _tmp62 * _tmp71 - _tmp74 * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) / (_tmp84);
  const Scalar _tmp86 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp87 = -_tmp86;
  const Scalar _tmp88 = -Scalar(0.010999999999999999) * _tmp19 -
                        Scalar(0.010999999999999999) * _tmp2 + Scalar(-0.010999999999999999);
  const Scalar _tmp89 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp90 = _tmp88 - _tmp89;
  const Scalar _tmp91 = _tmp87 + _tmp90;
  const Scalar _tmp92 = _tmp62 * _tmp91;
  const Scalar _tmp93 = _tmp88 + _tmp89;
  const Scalar _tmp94 = _tmp87 + _tmp93;
  const Scalar _tmp95 = _tmp80 * _tmp91;
  const Scalar _tmp96 = _tmp86 + _tmp93;
  const Scalar _tmp97 = -_tmp73 * _tmp95 + _tmp81 * _tmp96;
  const Scalar _tmp98 = _tmp74 * _tmp82;
  const Scalar _tmp99 = _tmp72 * _tmp94 - _tmp73 * _tmp92 - _tmp97 * _tmp98;
  const Scalar _tmp100 = -_tmp80 * _tmp96 + _tmp95;
  const Scalar _tmp101 = -_tmp100 * _tmp98 - _tmp53 * _tmp99 - _tmp62 * _tmp94 + _tmp92;
  const Scalar _tmp102 = _tmp101 * _tmp85;
  const Scalar _tmp103 = _tmp102 * _tmp56;
  const Scalar _tmp104 = Scalar(1.0) * _tmp85;
  const Scalar _tmp105 = _tmp48 * _tmp56;
  const Scalar _tmp106 = _tmp102 * _tmp105 - _tmp104 * _tmp99;
  const Scalar _tmp107 = Scalar(1.0) * _tmp50;
  const Scalar _tmp108 = _tmp29 - p_b(1, 0);
  const Scalar _tmp109 = _tmp18 - p_b(0, 0);
  const Scalar _tmp110 =
      std::pow(Scalar(std::pow(_tmp108, Scalar(2)) + std::pow(_tmp109, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp111 = _tmp109 * _tmp110;
  const Scalar _tmp112 = _tmp108 * _tmp110;
  const Scalar _tmp113 = -_tmp111 * _tmp28 + _tmp112 * _tmp17;
  const Scalar _tmp114 = Scalar(1.0) * _tmp113;
  const Scalar _tmp115 = _tmp114 * (_tmp103 - _tmp106 * _tmp107);
  const Scalar _tmp116 = Scalar(1.0) * _tmp82;
  const Scalar _tmp117 = -_tmp100 * _tmp116 + _tmp107 * _tmp52 * _tmp82 * _tmp97;
  const Scalar _tmp118 = Scalar(1.0) / (_tmp101);
  const Scalar _tmp119 = _tmp118 * _tmp84;
  const Scalar _tmp120 = _tmp102 * (-_tmp117 * _tmp119 - Scalar(1.0) * _tmp83);
  const Scalar _tmp121 = _tmp48 * _tmp55;
  const Scalar _tmp122 = _tmp117 + _tmp120;
  const Scalar _tmp123 = _tmp118 * _tmp99;
  const Scalar _tmp124 = -_tmp116 * _tmp97 + _tmp120 * _tmp121 - _tmp122 * _tmp123;
  const Scalar _tmp125 = Scalar(1.0) * _tmp112 * (-_tmp107 * _tmp124 + _tmp120 * _tmp56);
  const Scalar _tmp126 = _tmp86 + _tmp90;
  const Scalar _tmp127 = _tmp126 * fh1;
  const Scalar _tmp128 = Scalar(3.29616) * _tmp11 + _tmp111 * _tmp127 + _tmp17 * fv1;
  const Scalar _tmp129 = _tmp105 * _tmp50;
  const Scalar _tmp130 = Scalar(1.0) * _tmp129 - Scalar(1.0) * _tmp56;
  const Scalar _tmp131 = _tmp73 * _tmp82;
  const Scalar _tmp132 = _tmp131 * _tmp97 + _tmp73 * _tmp91;
  const Scalar _tmp133 = _tmp100 * _tmp131 - _tmp132 * _tmp53 - _tmp91;
  const Scalar _tmp134 = _tmp102 * (-_tmp119 * _tmp133 - _tmp71 + _tmp73 * _tmp83);
  const Scalar _tmp135 = _tmp133 + _tmp134;
  const Scalar _tmp136 = _tmp121 * _tmp134 - _tmp123 * _tmp135 + _tmp132;
  const Scalar _tmp137 = Scalar(1.0) * _tmp111 * (-_tmp107 * _tmp136 + _tmp134 * _tmp56);
  const Scalar _tmp138 = -_tmp112 * _tmp127 - Scalar(3.29616) * _tmp24 - _tmp28 * fv1;
  const Scalar _tmp139 = _tmp53 * _tmp56;
  const Scalar _tmp140 = _tmp54 * _tmp56 + Scalar(1.0);
  const Scalar _tmp141 = -Scalar(1.0) * _tmp107 * _tmp140 + Scalar(1.0) * _tmp139;
  const Scalar _tmp142 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp143 = _tmp42 + _tmp45 * _tmp53;
  const Scalar _tmp144 = _tmp143 * _tmp55;
  const Scalar _tmp145 = 0;
  const Scalar _tmp146 = -_tmp123 * _tmp145 - _tmp144 * _tmp48 + _tmp46;
  const Scalar _tmp147 =
      _tmp115 * fh1 + _tmp125 * fh1 + _tmp128 * _tmp130 + _tmp137 * fh1 + _tmp138 * _tmp141 +
      Scalar(1.0) * _tmp142 * (-_tmp107 * _tmp146 - _tmp143 * _tmp56 + Scalar(1.0));
  const Scalar _tmp148 = _tmp118 * _tmp135;
  const Scalar _tmp149 = _tmp118 * _tmp74;
  const Scalar _tmp150 = _tmp82 * (-_tmp135 * _tmp149 - _tmp73);
  const Scalar _tmp151 = _tmp111 * _tmp70 * (_tmp148 * _tmp62 + _tmp150 * _tmp80 + Scalar(1.0));
  const Scalar _tmp152 = _tmp113 * _tmp70 * (_tmp104 * _tmp62 - _tmp104 * _tmp80 * _tmp98);
  const Scalar _tmp153 = _tmp118 * _tmp122;
  const Scalar _tmp154 = _tmp82 * (-_tmp122 * _tmp149 + Scalar(1.0));
  const Scalar _tmp155 = _tmp112 * _tmp70 * (_tmp153 * _tmp62 + _tmp154 * _tmp80);
  const Scalar _tmp156 = _tmp145 * _tmp149 * _tmp82;
  const Scalar _tmp157 = _tmp118 * _tmp145;
  const Scalar _tmp158 = -_tmp142 * _tmp70 * (-_tmp156 * _tmp80 + _tmp157 * _tmp62) -
                         _tmp151 * fh1 - _tmp152 * fh1 - _tmp155 * fh1;
  const Scalar _tmp159 = Scalar(1.0) / (_tmp158);
  const Scalar _tmp160 = std::asinh(_tmp147 * _tmp159);
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp162 =
      -_tmp160 * _tmp161 - std::sqrt(Scalar(std::pow(Scalar(-_tmp63 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp65 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp164 = _tmp162 * _tmp163;
  const Scalar _tmp165 = Scalar(1.0) * _tmp160;
  const Scalar _tmp166 = -_tmp151 - _tmp152 - _tmp155;
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp166;
  const Scalar _tmp168 = std::pow(_tmp158, Scalar(-2));
  const Scalar _tmp169 = _tmp112 * _tmp126;
  const Scalar _tmp170 = _tmp111 * _tmp126;
  const Scalar _tmp171 = _tmp166 * _tmp168;
  const Scalar _tmp172 = (-_tmp147 * _tmp171 + _tmp159 * (_tmp115 + _tmp125 + _tmp130 * _tmp170 +
                                                          _tmp137 - _tmp141 * _tmp169)) /
                         std::sqrt(Scalar(std::pow(_tmp147, Scalar(2)) * _tmp168 + 1));
  const Scalar _tmp173 = _tmp111 * _tmp150;
  const Scalar _tmp174 = _tmp112 * _tmp154;
  const Scalar _tmp175 = _tmp114 * _tmp85;
  const Scalar _tmp176 = _tmp175 * fh1;
  const Scalar _tmp177 = -_tmp142 * _tmp156 + _tmp173 * fh1 + _tmp174 * fh1 - _tmp176 * _tmp98;
  const Scalar _tmp178 = Scalar(1.0) / (_tmp177);
  const Scalar _tmp179 = _tmp128 * _tmp56;
  const Scalar _tmp180 = _tmp112 * _tmp124 * _tmp50;
  const Scalar _tmp181 = _tmp140 * _tmp50;
  const Scalar _tmp182 = _tmp111 * _tmp136 * _tmp50;
  const Scalar _tmp183 = _tmp106 * _tmp113 * _tmp50;
  const Scalar _tmp184 = _tmp138 * _tmp181 + _tmp142 * _tmp146 * _tmp50 -
                         _tmp179 * _tmp48 * _tmp50 + _tmp180 * fh1 + _tmp182 * fh1 + _tmp183 * fh1;
  const Scalar _tmp185 = std::asinh(_tmp178 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = std::pow(_tmp177, Scalar(-2));
  const Scalar _tmp188 = _tmp173 + _tmp174 - _tmp175 * _tmp98;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 =
      (_tmp178 * (-_tmp129 * _tmp170 - _tmp169 * _tmp181 + _tmp180 + _tmp182 + _tmp183) -
       _tmp184 * _tmp189) /
      std::sqrt(Scalar(std::pow(_tmp184, Scalar(2)) * _tmp187 + 1));
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp177;
  const Scalar _tmp192 =
      -_tmp185 * _tmp191 - std::sqrt(Scalar(std::pow(Scalar(-_tmp75 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp77 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp193 = Scalar(0.1034955) * _tmp178;
  const Scalar _tmp194 = _tmp192 * _tmp193;
  const Scalar _tmp195 = Scalar(9.6622558468725703) * _tmp188;
  const Scalar _tmp196 = _tmp112 * _tmp120 * _tmp55;
  const Scalar _tmp197 = _tmp111 * _tmp134 * _tmp55;
  const Scalar _tmp198 = _tmp103 * _tmp113;
  const Scalar _tmp199 = -_tmp138 * _tmp139 + _tmp142 * _tmp144 + _tmp179 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = _tmp111 * _tmp148;
  const Scalar _tmp201 = _tmp112 * _tmp153;
  const Scalar _tmp202 = _tmp142 * _tmp157 + _tmp176 + _tmp200 * fh1 + _tmp201 * fh1;
  const Scalar _tmp203 = Scalar(1.0) / (_tmp202);
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(9.6622558468725703) * _tmp202;
  const Scalar _tmp206 =
      -_tmp204 * _tmp205 - std::sqrt(Scalar(std::pow(Scalar(-_tmp57 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp59 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp207 = Scalar(0.1034955) * _tmp203;
  const Scalar _tmp208 = _tmp206 * _tmp207;
  const Scalar _tmp209 = Scalar(1.0) * _tmp204;
  const Scalar _tmp210 = _tmp175 + _tmp200 + _tmp201;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp210;
  const Scalar _tmp212 = std::pow(_tmp202, Scalar(-2));
  const Scalar _tmp213 = _tmp210 * _tmp212;
  const Scalar _tmp214 = (-_tmp199 * _tmp213 + _tmp203 * (_tmp139 * _tmp169 + _tmp170 * _tmp56 -
                                                          _tmp196 - _tmp197 - _tmp198)) /
                         std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp212 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp35 * _tmp36 * fv1 * std::cosh(_tmp37) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp30 * _tmp36 - _tmp32) -
            _tmp33 * _tmp35) *
               std::cosh(_tmp34)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp34) -
      Scalar(9.6622558468725703) * std::sinh(_tmp37);
  _res(1, 0) = _tmp161 * (-Scalar(1.0) * _tmp172 * std::cosh(_tmp165) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp171 +
                           _tmp163 * (-_tmp160 * _tmp167 - _tmp161 * _tmp172)) *
                              std::cosh(_tmp164)) +
               _tmp167 * (-std::sinh(_tmp164) - std::sinh(_tmp165));
  _res(2, 0) = _tmp191 * (-Scalar(1.0) * _tmp190 * std::cosh(_tmp186) -
                          (-Scalar(0.1034955) * _tmp189 * _tmp192 +
                           _tmp193 * (-_tmp185 * _tmp195 - _tmp190 * _tmp191)) *
                              std::cosh(_tmp194)) +
               _tmp195 * (-std::sinh(_tmp186) - std::sinh(_tmp194));
  _res(3, 0) = _tmp205 * (-Scalar(1.0) * _tmp214 * std::cosh(_tmp209) -
                          (-Scalar(0.1034955) * _tmp206 * _tmp213 +
                           _tmp207 * (-_tmp204 * _tmp211 - _tmp205 * _tmp214)) *
                              std::cosh(_tmp208)) +
               _tmp211 * (-std::sinh(_tmp208) - std::sinh(_tmp209));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
