// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl9
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl9(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 639

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (210)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = -_tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = _tmp1 * _tmp8;
  const Scalar _tmp15 = _tmp10 * _tmp3;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp13 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + position_vector(1, 0);
  const Scalar _tmp20 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp20 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp22 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp23 = _tmp10 * _tmp7;
  const Scalar _tmp24 = _tmp22 + _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp27 = _tmp25 - _tmp26;
  const Scalar _tmp28 = _tmp21 + _tmp27;
  const Scalar _tmp29 = _tmp28 + position_vector(0, 0);
  const Scalar _tmp30 = _tmp0 * fv1;
  const Scalar _tmp31 = std::asinh(_tmp30);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * _tmp31;
  const Scalar _tmp33 =
      -Scalar(0.1034955) * _tmp32 * fh1 -
      Scalar(0.86817218273361274) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp29), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp19 - 1), Scalar(2))));
  const Scalar _tmp34 = _tmp0 * _tmp33;
  const Scalar _tmp35 = Scalar(1.0) * _tmp31;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = -_tmp21;
  const Scalar _tmp39 = _tmp27 + _tmp38;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(1.9874742031097401);
  const Scalar _tmp42 = _tmp13 - _tmp16;
  const Scalar _tmp43 = _tmp42 + _tmp6;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp44 + Scalar(8.3196563720703107);
  const Scalar _tmp46 = std::pow(Scalar(std::pow(_tmp41, Scalar(2)) + std::pow(_tmp45, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp47 = _tmp41 * _tmp46;
  const Scalar _tmp48 = Scalar(0.20999999999999999) * _tmp22 - Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp49 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp50 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp51 = _tmp49 + _tmp50;
  const Scalar _tmp52 = _tmp48 + _tmp51;
  const Scalar _tmp53 = _tmp25 + _tmp26;
  const Scalar _tmp54 = _tmp38 + _tmp53;
  const Scalar _tmp55 = _tmp54 + position_vector(0, 0);
  const Scalar _tmp56 = _tmp55 + Scalar(1.7965602546229);
  const Scalar _tmp57 = _tmp42 + _tmp5;
  const Scalar _tmp58 = _tmp57 + position_vector(1, 0);
  const Scalar _tmp59 = _tmp58 + Scalar(-4.83288938413423);
  const Scalar _tmp60 = std::pow(Scalar(std::pow(_tmp56, Scalar(2)) + std::pow(_tmp59, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp61 = _tmp56 * _tmp60;
  const Scalar _tmp62 = -_tmp48;
  const Scalar _tmp63 = _tmp51 + _tmp62;
  const Scalar _tmp64 = _tmp52 * _tmp61 - _tmp61 * _tmp63;
  const Scalar _tmp65 = _tmp59 * _tmp60;
  const Scalar _tmp66 = _tmp17 + _tmp5;
  const Scalar _tmp67 = _tmp66 + position_vector(1, 0);
  const Scalar _tmp68 = _tmp67 + Scalar(-4.7744369927459998);
  const Scalar _tmp69 = _tmp21 + _tmp53;
  const Scalar _tmp70 = _tmp69 + position_vector(0, 0);
  const Scalar _tmp71 = _tmp70 + Scalar(-2.7171519410699099);
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = _tmp68 * _tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp61 * _tmp73 - _tmp65);
  const Scalar _tmp75 = _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp52 * _tmp73;
  const Scalar _tmp77 = -_tmp61 * _tmp76 + _tmp63 * _tmp65;
  const Scalar _tmp78 = _tmp75 * _tmp77 + _tmp76;
  const Scalar _tmp79 = Scalar(1.0) * _tmp66;
  const Scalar _tmp80 = -_tmp79;
  const Scalar _tmp81 = Scalar(1.0) / (_tmp57 + _tmp80);
  const Scalar _tmp82 = Scalar(1.0) * _tmp69;
  const Scalar _tmp83 = _tmp81 * (-_tmp54 + _tmp82);
  const Scalar _tmp84 = -_tmp52 + _tmp64 * _tmp75 - _tmp78 * _tmp83;
  const Scalar _tmp85 = _tmp49 - _tmp50;
  const Scalar _tmp86 = _tmp62 + _tmp85;
  const Scalar _tmp87 = _tmp45 * _tmp46;
  const Scalar _tmp88 = _tmp47 * _tmp73 - _tmp87;
  const Scalar _tmp89 = _tmp74 * _tmp88;
  const Scalar _tmp90 = -_tmp47 * _tmp76 - _tmp77 * _tmp89 + _tmp86 * _tmp87;
  const Scalar _tmp91 = _tmp47 * _tmp52 - _tmp47 * _tmp86 - _tmp64 * _tmp89 - _tmp83 * _tmp90;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 =
      std::sqrt(Scalar(std::pow(_tmp68, Scalar(2)) + std::pow(_tmp71, Scalar(2))));
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp72 * _tmp93;
  const Scalar _tmp96 = _tmp95 * (-_tmp66 * _tmp71 * _tmp94 + _tmp68 * _tmp69 * _tmp94);
  const Scalar _tmp97 = -_tmp54 * _tmp65 + _tmp57 * _tmp61 + _tmp61 * _tmp96;
  const Scalar _tmp98 = -_tmp39 * _tmp87 + _tmp43 * _tmp47 + _tmp47 * _tmp96 - _tmp89 * _tmp97;
  const Scalar _tmp99 = _tmp92 * _tmp98;
  const Scalar _tmp100 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp101 = _tmp100 * _tmp91;
  const Scalar _tmp102 = _tmp101 * (_tmp75 * _tmp97 - _tmp84 * _tmp99 - _tmp96);
  const Scalar _tmp103 = _tmp92 * (_tmp102 + _tmp84);
  const Scalar _tmp104 = -_tmp103 * _tmp88 - _tmp73;
  const Scalar _tmp105 = _tmp61 * _tmp74;
  const Scalar _tmp106 = _tmp19 + Scalar(8.3885017487099702);
  const Scalar _tmp107 = _tmp29 + Scalar(-2.5193355532036801);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp107 * _tmp108;
  const Scalar _tmp110 = _tmp109 * _tmp95 * (_tmp103 * _tmp47 + _tmp104 * _tmp105 + Scalar(1.0));
  const Scalar _tmp111 = Scalar(1.0) * _tmp74;
  const Scalar _tmp112 = _tmp111 * _tmp77;
  const Scalar _tmp113 = -_tmp111 * _tmp64 + _tmp112 * _tmp83;
  const Scalar _tmp114 = _tmp101 * (-_tmp111 * _tmp97 - _tmp113 * _tmp99);
  const Scalar _tmp115 = _tmp92 * (_tmp113 + _tmp114);
  const Scalar _tmp116 = -_tmp115 * _tmp88 + Scalar(1.0);
  const Scalar _tmp117 = _tmp106 * _tmp108;
  const Scalar _tmp118 = _tmp117 * _tmp95 * (_tmp105 * _tmp116 + _tmp115 * _tmp47);
  const Scalar _tmp119 = -_tmp109 * _tmp18 + _tmp117 * _tmp28;
  const Scalar _tmp120 = Scalar(1.0) * _tmp100;
  const Scalar _tmp121 = _tmp61 * _tmp89;
  const Scalar _tmp122 = _tmp119 * _tmp95 * (-_tmp120 * _tmp121 + _tmp120 * _tmp47);
  const Scalar _tmp123 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp124 = _tmp79 * _tmp83 + _tmp82;
  const Scalar _tmp125 = 0;
  const Scalar _tmp126 = -_tmp110 * fh1 - _tmp118 * fh1 - _tmp122 * fh1 -
                         _tmp123 * _tmp95 * (-_tmp121 * _tmp125 + _tmp125 * _tmp47);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = _tmp43 + _tmp80;
  const Scalar _tmp129 = _tmp128 * _tmp83;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp39 + _tmp82);
  const Scalar _tmp131 = _tmp128 * _tmp130;
  const Scalar _tmp132 = _tmp102 * _tmp131 - _tmp103 * _tmp90 + _tmp78;
  const Scalar _tmp133 = Scalar(1.0) * _tmp81;
  const Scalar _tmp134 = Scalar(1.0) * _tmp130;
  const Scalar _tmp135 = Scalar(1.0) * _tmp109 * (_tmp102 * _tmp134 - _tmp132 * _tmp133);
  const Scalar _tmp136 = _tmp124 * _tmp130;
  const Scalar _tmp137 = -_tmp125 * _tmp90 - _tmp128 * _tmp136 + _tmp80;
  const Scalar _tmp138 = _tmp48 + _tmp85;
  const Scalar _tmp139 = _tmp138 * fh1;
  const Scalar _tmp140 = _tmp109 * _tmp139 + Scalar(3.29616) * _tmp24 + _tmp28 * fv1;
  const Scalar _tmp141 = _tmp128 * _tmp134;
  const Scalar _tmp142 = -Scalar(1.0) * _tmp134 + Scalar(1.0) * _tmp141 * _tmp81;
  const Scalar _tmp143 = _tmp101 * _tmp134;
  const Scalar _tmp144 = _tmp81 * (_tmp101 * _tmp141 - _tmp120 * _tmp90);
  const Scalar _tmp145 = Scalar(1.0) * _tmp119 * (_tmp143 - Scalar(1.0) * _tmp144);
  const Scalar _tmp146 = -_tmp112 + _tmp114 * _tmp131 - _tmp115 * _tmp90;
  const Scalar _tmp147 = Scalar(1.0) * _tmp117 * (_tmp114 * _tmp134 - _tmp133 * _tmp146);
  const Scalar _tmp148 = -_tmp117 * _tmp139 - Scalar(3.29616) * _tmp12 - _tmp18 * fv1;
  const Scalar _tmp149 = _tmp129 * _tmp134 + Scalar(1.0);
  const Scalar _tmp150 = _tmp134 * _tmp83;
  const Scalar _tmp151 = -Scalar(1.0) * _tmp133 * _tmp149 + Scalar(1.0) * _tmp150;
  const Scalar _tmp152 =
      Scalar(1.0) * _tmp123 * (-_tmp124 * _tmp134 - _tmp133 * _tmp137 + Scalar(1.0)) +
      _tmp135 * fh1 + _tmp140 * _tmp142 + _tmp145 * fh1 + _tmp147 * fh1 + _tmp148 * _tmp151;
  const Scalar _tmp153 = std::asinh(_tmp127 * _tmp152);
  const Scalar _tmp154 = Scalar(1.0) * _tmp153;
  const Scalar _tmp155 = std::pow(_tmp126, Scalar(-2));
  const Scalar _tmp156 = -_tmp110 - _tmp118 - _tmp122;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = _tmp109 * _tmp138;
  const Scalar _tmp159 = _tmp117 * _tmp138;
  const Scalar _tmp160 =
      (_tmp127 * (_tmp135 + _tmp142 * _tmp158 + _tmp145 + _tmp147 - _tmp151 * _tmp159) -
       _tmp152 * _tmp157) /
      std::sqrt(Scalar(std::pow(_tmp152, Scalar(2)) * _tmp155 + 1));
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp156;
  const Scalar _tmp162 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp163 = Scalar(0.1034955) * _tmp127;
  const Scalar _tmp164 =
      -_tmp153 * _tmp162 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp67), Scalar(2)) +
                     Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp70), Scalar(2))));
  const Scalar _tmp165 = _tmp163 * _tmp164;
  const Scalar _tmp166 = _tmp149 * _tmp81;
  const Scalar _tmp167 = _tmp109 * _tmp81;
  const Scalar _tmp168 = _tmp132 * _tmp167;
  const Scalar _tmp169 = _tmp119 * _tmp144;
  const Scalar _tmp170 = _tmp117 * _tmp146 * _tmp81;
  const Scalar _tmp171 = _tmp134 * _tmp140;
  const Scalar _tmp172 = _tmp123 * _tmp137 * _tmp81 - _tmp128 * _tmp171 * _tmp81 +
                         _tmp148 * _tmp166 + _tmp168 * fh1 + _tmp169 * fh1 + _tmp170 * fh1;
  const Scalar _tmp173 = _tmp104 * _tmp109 * _tmp74;
  const Scalar _tmp174 = _tmp116 * _tmp117 * _tmp74;
  const Scalar _tmp175 = _tmp119 * _tmp120;
  const Scalar _tmp176 = _tmp175 * fh1;
  const Scalar _tmp177 = _tmp123 * _tmp125;
  const Scalar _tmp178 = _tmp173 * fh1 + _tmp174 * fh1 - _tmp176 * _tmp89 - _tmp177 * _tmp89;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = std::asinh(_tmp172 * _tmp179);
  const Scalar _tmp181 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp182 =
      -_tmp180 * _tmp181 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp58), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp55 - 1), Scalar(2))));
  const Scalar _tmp183 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp184 = _tmp182 * _tmp183;
  const Scalar _tmp185 = Scalar(1.0) * _tmp180;
  const Scalar _tmp186 = _tmp173 + _tmp174 - _tmp175 * _tmp89;
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp186;
  const Scalar _tmp188 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp189 = _tmp186 * _tmp188;
  const Scalar _tmp190 =
      (-_tmp172 * _tmp189 +
       _tmp179 * (-_tmp138 * _tmp141 * _tmp167 - _tmp159 * _tmp166 + _tmp168 + _tmp169 + _tmp170)) /
      std::sqrt(Scalar(std::pow(_tmp172, Scalar(2)) * _tmp188 + 1));
  const Scalar _tmp191 = _tmp103 * _tmp109;
  const Scalar _tmp192 = _tmp115 * _tmp117;
  const Scalar _tmp193 = _tmp176 + _tmp177 + _tmp191 * fh1 + _tmp192 * fh1;
  const Scalar _tmp194 = Scalar(1.0) / (_tmp193);
  const Scalar _tmp195 = _tmp119 * _tmp143;
  const Scalar _tmp196 = _tmp102 * _tmp109 * _tmp130;
  const Scalar _tmp197 = _tmp114 * _tmp117 * _tmp130;
  const Scalar _tmp198 = _tmp123 * _tmp136 - _tmp148 * _tmp150 + _tmp171 - _tmp195 * fh1 -
                         _tmp196 * fh1 - _tmp197 * fh1;
  const Scalar _tmp199 = std::asinh(_tmp194 * _tmp198);
  const Scalar _tmp200 = Scalar(9.6622558468725703) * _tmp193;
  const Scalar _tmp201 =
      -_tmp199 * _tmp200 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp40 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp44 - 1), Scalar(2))));
  const Scalar _tmp202 = Scalar(0.1034955) * _tmp194;
  const Scalar _tmp203 = _tmp201 * _tmp202;
  const Scalar _tmp204 = Scalar(1.0) * _tmp199;
  const Scalar _tmp205 = _tmp175 + _tmp191 + _tmp192;
  const Scalar _tmp206 = Scalar(9.6622558468725703) * _tmp205;
  const Scalar _tmp207 = std::pow(_tmp193, Scalar(-2));
  const Scalar _tmp208 = _tmp205 * _tmp207;
  const Scalar _tmp209 =
      (_tmp194 * (_tmp134 * _tmp158 + _tmp150 * _tmp159 - _tmp195 - _tmp196 - _tmp197) -
       _tmp198 * _tmp208) /
      std::sqrt(Scalar(std::pow(_tmp198, Scalar(2)) * _tmp207 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp35) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp30 * _tmp37 - _tmp32) -
            _tmp33 * _tmp36) *
               std::cosh(_tmp34)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp34) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35);
  _res(1, 0) = _tmp161 * (-std::sinh(_tmp154) - std::sinh(_tmp165)) +
               _tmp162 * (-Scalar(1.0) * _tmp160 * std::cosh(_tmp154) -
                          (-Scalar(0.1034955) * _tmp157 * _tmp164 +
                           _tmp163 * (-_tmp153 * _tmp161 - _tmp160 * _tmp162)) *
                              std::cosh(_tmp165));
  _res(2, 0) = _tmp181 * (-Scalar(1.0) * _tmp190 * std::cosh(_tmp185) -
                          (-Scalar(0.1034955) * _tmp182 * _tmp189 +
                           _tmp183 * (-_tmp180 * _tmp187 - _tmp181 * _tmp190)) *
                              std::cosh(_tmp184)) +
               _tmp187 * (-std::sinh(_tmp184) - std::sinh(_tmp185));
  _res(3, 0) = _tmp200 * (-Scalar(1.0) * _tmp209 * std::cosh(_tmp204) -
                          (-Scalar(0.1034955) * _tmp201 * _tmp208 +
                           _tmp202 * (-_tmp199 * _tmp206 - _tmp200 * _tmp209)) *
                              std::cosh(_tmp203)) +
               _tmp206 * (-std::sinh(_tmp203) - std::sinh(_tmp204));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
