// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl17
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl17(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 602

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (190)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp4 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp7 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                       2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp6;
  const Scalar _tmp11 = _tmp10 * _tmp4;
  const Scalar _tmp12 = _tmp3 * _tmp7;
  const Scalar _tmp13 = _tmp11 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp21;
  const Scalar _tmp23 = _tmp10 * _tmp3;
  const Scalar _tmp24 = _tmp4 * _tmp7;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp34 = -_tmp28;
  const Scalar _tmp35 = _tmp22 + _tmp27 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(1, 0);
  const Scalar _tmp37 = _tmp36 + Scalar(-4.83288938413423);
  const Scalar _tmp38 = _tmp14 - _tmp17;
  const Scalar _tmp39 = _tmp38 + _tmp9;
  const Scalar _tmp40 = _tmp39 + position_vector(0, 0);
  const Scalar _tmp41 = _tmp40 + Scalar(1.7965602546229);
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = _tmp37 * _tmp42;
  const Scalar _tmp44 = -_tmp22;
  const Scalar _tmp45 = _tmp29 + _tmp44;
  const Scalar _tmp46 = _tmp45 + position_vector(1, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(8.3885017487099702);
  const Scalar _tmp48 = -_tmp9;
  const Scalar _tmp49 = _tmp18 + _tmp48;
  const Scalar _tmp50 = _tmp49 + position_vector(0, 0);
  const Scalar _tmp51 = _tmp50 + Scalar(-2.5193355532036801);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp47, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp51 * _tmp52;
  const Scalar _tmp54 = _tmp47 * _tmp52;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp43 * _tmp53 - _tmp54);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = Scalar(1.0) * _tmp39;
  const Scalar _tmp58 = Scalar(1.0) * _tmp35;
  const Scalar _tmp59 = -_tmp58;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp45 + _tmp59);
  const Scalar _tmp61 = -_tmp49 + _tmp57;
  const Scalar _tmp62 = _tmp60 * _tmp61;
  const Scalar _tmp63 = _tmp57 + _tmp58 * _tmp62;
  const Scalar _tmp64 = 0;
  const Scalar _tmp65 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp66 = -_tmp65;
  const Scalar _tmp67 = -Scalar(0.010999999999999999) * _tmp15 -
                        Scalar(0.010999999999999999) * _tmp21 + Scalar(-0.010999999999999999);
  const Scalar _tmp68 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp69 = _tmp67 - _tmp68;
  const Scalar _tmp70 = _tmp66 + _tmp69;
  const Scalar _tmp71 = _tmp38 + _tmp48;
  const Scalar _tmp72 = _tmp71 + position_vector(0, 0);
  const Scalar _tmp73 = _tmp72 + Scalar(1.9874742031097401);
  const Scalar _tmp74 = _tmp34 + _tmp44;
  const Scalar _tmp75 = _tmp27 + _tmp74;
  const Scalar _tmp76 = _tmp75 + position_vector(1, 0);
  const Scalar _tmp77 = _tmp76 + Scalar(8.3196563720703107);
  const Scalar _tmp78 = std::pow(Scalar(std::pow(_tmp73, Scalar(2)) + std::pow(_tmp77, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp79 = _tmp73 * _tmp78;
  const Scalar _tmp80 = _tmp77 * _tmp78;
  const Scalar _tmp81 = _tmp43 * _tmp79 - _tmp80;
  const Scalar _tmp82 = _tmp67 + _tmp68;
  const Scalar _tmp83 = _tmp66 + _tmp82;
  const Scalar _tmp84 = _tmp65 + _tmp69;
  const Scalar _tmp85 = _tmp43 * _tmp84;
  const Scalar _tmp86 = _tmp55 * (-_tmp53 * _tmp85 + _tmp54 * _tmp83);
  const Scalar _tmp87 = _tmp70 * _tmp80 - _tmp79 * _tmp85 - _tmp81 * _tmp86;
  const Scalar _tmp88 = _tmp55 * (-_tmp53 * _tmp83 + _tmp53 * _tmp84);
  const Scalar _tmp89 = -_tmp62 * _tmp87 - _tmp70 * _tmp79 + _tmp79 * _tmp84 - _tmp81 * _tmp88;
  const Scalar _tmp90 = Scalar(1.0) / (_tmp89);
  const Scalar _tmp91 = _tmp81 * _tmp90;
  const Scalar _tmp92 = _tmp64 * _tmp91;
  const Scalar _tmp93 = _tmp64 * _tmp90;
  const Scalar _tmp94 =
      std::sqrt(Scalar(std::pow(_tmp37, Scalar(2)) + std::pow(_tmp41, Scalar(2))));
  const Scalar _tmp95 = _tmp42 * _tmp94;
  const Scalar _tmp96 = _tmp95 * (-_tmp56 * _tmp92 + _tmp79 * _tmp93);
  const Scalar _tmp97 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp98 = _tmp95 * (-_tmp35 * _tmp41 * _tmp97 + _tmp37 * _tmp39 * _tmp97);
  const Scalar _tmp99 = _tmp55 * (_tmp45 * _tmp53 - _tmp49 * _tmp54 + _tmp53 * _tmp98);
  const Scalar _tmp100 = -_tmp71 * _tmp80 + _tmp75 * _tmp79 + _tmp79 * _tmp98 - _tmp81 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp102 = Scalar(1.0) * _tmp101;
  const Scalar _tmp103 = _tmp31 + Scalar(-4.7744369927459998);
  const Scalar _tmp104 = _tmp20 + Scalar(-2.7171519410699099);
  const Scalar _tmp105 =
      std::pow(Scalar(std::pow(_tmp103, Scalar(2)) + std::pow(_tmp104, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp106 = _tmp103 * _tmp105;
  const Scalar _tmp107 = _tmp104 * _tmp105;
  const Scalar _tmp108 = fh1 * (_tmp106 * _tmp19 - _tmp107 * _tmp30);
  const Scalar _tmp109 = Scalar(1.0) * _tmp60;
  const Scalar _tmp110 = _tmp109 * _tmp61 * _tmp86 - Scalar(1.0) * _tmp88;
  const Scalar _tmp111 = _tmp100 * _tmp90;
  const Scalar _tmp112 = _tmp101 * _tmp89;
  const Scalar _tmp113 = _tmp112 * (-_tmp110 * _tmp111 - Scalar(1.0) * _tmp99);
  const Scalar _tmp114 = _tmp110 + _tmp113;
  const Scalar _tmp115 = -_tmp114 * _tmp91 + Scalar(1.0);
  const Scalar _tmp116 = _tmp79 * _tmp90;
  const Scalar _tmp117 = _tmp106 * fh1;
  const Scalar _tmp118 = _tmp43 * _tmp86 + _tmp85;
  const Scalar _tmp119 = -_tmp118 * _tmp62 + _tmp43 * _tmp88 - _tmp84;
  const Scalar _tmp120 = _tmp112 * (-_tmp111 * _tmp119 + _tmp43 * _tmp99 - _tmp98);
  const Scalar _tmp121 = _tmp119 + _tmp120;
  const Scalar _tmp122 = -_tmp121 * _tmp91 - _tmp43;
  const Scalar _tmp123 = _tmp107 * fh1;
  const Scalar _tmp124 = -_tmp108 * _tmp95 * (-_tmp102 * _tmp56 * _tmp81 + _tmp102 * _tmp79) -
                         _tmp117 * _tmp95 * (_tmp114 * _tmp116 + _tmp115 * _tmp56) -
                         _tmp123 * _tmp95 * (_tmp116 * _tmp121 + _tmp122 * _tmp56 + Scalar(1.0)) -
                         _tmp33 * _tmp96;
  const Scalar _tmp125 = Scalar(1.0) / (_tmp124);
  const Scalar _tmp126 = fh1 * (_tmp65 + _tmp82);
  const Scalar _tmp127 = _tmp107 * _tmp126 + Scalar(3.29616) * _tmp13 + _tmp19 * fv1;
  const Scalar _tmp128 = _tmp59 + _tmp75;
  const Scalar _tmp129 = _tmp128 * _tmp62;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 + _tmp57 - _tmp71);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp128 * _tmp131;
  const Scalar _tmp133 = -Scalar(1.0) * _tmp131 + Scalar(1.0) * _tmp132 * _tmp60;
  const Scalar _tmp134 = _tmp87 * _tmp90;
  const Scalar _tmp135 = _tmp130 * _tmp63;
  const Scalar _tmp136 = _tmp60 * (-_tmp128 * _tmp135 - _tmp134 * _tmp64 + _tmp59);
  const Scalar _tmp137 = -Scalar(1.0) * _tmp131 * _tmp63 - Scalar(1.0) * _tmp136 + Scalar(1.0);
  const Scalar _tmp138 = -_tmp106 * _tmp126 - Scalar(3.29616) * _tmp25 - _tmp30 * fv1;
  const Scalar _tmp139 = _tmp60 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp140 = _tmp131 * _tmp62;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp139 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp128 * _tmp130;
  const Scalar _tmp143 = _tmp113 * _tmp142 - _tmp114 * _tmp134 - Scalar(1.0) * _tmp86;
  const Scalar _tmp144 = -_tmp102 * _tmp87 + _tmp112 * _tmp132;
  const Scalar _tmp145 = _tmp112 * _tmp131;
  const Scalar _tmp146 = _tmp118 + _tmp120 * _tmp142 - _tmp121 * _tmp134;
  const Scalar _tmp147 = Scalar(1.0) * _tmp108 * (-_tmp109 * _tmp144 + _tmp145) +
                         Scalar(1.0) * _tmp117 * (-_tmp109 * _tmp143 + _tmp113 * _tmp131) +
                         Scalar(1.0) * _tmp123 * (-_tmp109 * _tmp146 + _tmp120 * _tmp131) +
                         _tmp127 * _tmp133 + _tmp137 * _tmp33 + _tmp138 * _tmp141;
  const Scalar _tmp148 = std::asinh(_tmp125 * _tmp147);
  const Scalar _tmp149 = Scalar(9.6622558468725703) * _tmp124;
  const Scalar _tmp150 =
      -_tmp148 * _tmp149 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp36), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp40 - 1), Scalar(2))));
  const Scalar _tmp151 = Scalar(0.1034955) * _tmp125;
  const Scalar _tmp152 = _tmp150 * _tmp151;
  const Scalar _tmp153 = Scalar(1.0) * _tmp148;
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp96;
  const Scalar _tmp155 = std::pow(_tmp124, Scalar(-2));
  const Scalar _tmp156 = _tmp155 * _tmp96;
  const Scalar _tmp157 = _tmp26 + _tmp74;
  const Scalar _tmp158 =
      (_tmp125 * (_tmp133 * _tmp19 - _tmp137 + _tmp141 * _tmp157) - _tmp147 * _tmp156) /
      std::sqrt(Scalar(std::pow(_tmp147, Scalar(2)) * _tmp155 + 1));
  const Scalar _tmp159 = _tmp127 * _tmp131;
  const Scalar _tmp160 = _tmp128 * _tmp60;
  const Scalar _tmp161 = _tmp108 * _tmp144 * _tmp60 + _tmp117 * _tmp143 * _tmp60 +
                         _tmp123 * _tmp146 * _tmp60 + _tmp136 * _tmp33 + _tmp138 * _tmp139 -
                         _tmp159 * _tmp160;
  const Scalar _tmp162 = _tmp102 * _tmp108;
  const Scalar _tmp163 = _tmp55 * _tmp92;
  const Scalar _tmp164 = _tmp115 * _tmp117 * _tmp55 + _tmp122 * _tmp123 * _tmp55 -
                         _tmp162 * _tmp55 * _tmp81 - _tmp163 * _tmp33;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = std::asinh(_tmp161 * _tmp165);
  const Scalar _tmp167 = Scalar(1.0) * _tmp166;
  const Scalar _tmp168 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp169 =
      -_tmp166 * _tmp168 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp50), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp46 - 1), Scalar(2))));
  const Scalar _tmp170 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp171 = _tmp169 * _tmp170;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp163;
  const Scalar _tmp173 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp174 = _tmp163 * _tmp173;
  const Scalar _tmp175 = _tmp131 * _tmp19;
  const Scalar _tmp176 =
      (-_tmp161 * _tmp174 + _tmp165 * (-_tmp136 + _tmp139 * _tmp157 - _tmp160 * _tmp175)) /
      std::sqrt(Scalar(std::pow(_tmp161, Scalar(2)) * _tmp173 + 1));
  const Scalar _tmp177 = -_tmp108 * _tmp145 - _tmp113 * _tmp117 * _tmp130 -
                         _tmp120 * _tmp123 * _tmp130 + _tmp135 * _tmp33 - _tmp138 * _tmp140 +
                         _tmp159;
  const Scalar _tmp178 =
      _tmp114 * _tmp117 * _tmp90 + _tmp121 * _tmp123 * _tmp90 + _tmp162 + _tmp33 * _tmp93;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = std::asinh(_tmp177 * _tmp179);
  const Scalar _tmp181 = Scalar(1.0) * _tmp180;
  const Scalar _tmp182 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp183 = _tmp182 * _tmp93;
  const Scalar _tmp184 = (_tmp177 * _tmp183 + _tmp179 * (-_tmp135 - _tmp140 * _tmp157 + _tmp175)) /
                         std::sqrt(Scalar(std::pow(_tmp177, Scalar(2)) * _tmp182 + 1));
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp93;
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp188 =
      -_tmp180 * _tmp186 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp72 - 1), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp76 - 1), Scalar(2))));
  const Scalar _tmp189 = _tmp187 * _tmp188;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 *
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp1 * _tmp32 -
                Scalar(4.7744369927459998) *
                    std::sqrt(Scalar(
                        Scalar(0.32387954179207445) *
                            std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp20), Scalar(2)) +
                        std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp31), Scalar(2)))))));
  _res(1, 0) = _tmp149 * (-Scalar(1.0) * _tmp158 * std::cosh(_tmp153) -
                          (-Scalar(0.1034955) * _tmp150 * _tmp156 +
                           _tmp151 * (-_tmp148 * _tmp154 - _tmp149 * _tmp158)) *
                              std::cosh(_tmp152)) +
               _tmp154 * (-std::sinh(_tmp152) - std::sinh(_tmp153));
  _res(2, 0) = _tmp168 * (-Scalar(1.0) * _tmp176 * std::cosh(_tmp167) -
                          (-Scalar(0.1034955) * _tmp169 * _tmp174 +
                           _tmp170 * (-_tmp166 * _tmp172 - _tmp168 * _tmp176)) *
                              std::cosh(_tmp171)) +
               _tmp172 * (-std::sinh(_tmp167) - std::sinh(_tmp171));
  _res(3, 0) = -_tmp185 * (-std::sinh(_tmp181) - std::sinh(_tmp189)) +
               _tmp186 * (-Scalar(1.0) * _tmp184 * std::cosh(_tmp181) -
                          (Scalar(0.1034955) * _tmp183 * _tmp188 +
                           _tmp187 * (_tmp180 * _tmp185 - _tmp184 * _tmp186)) *
                              std::cosh(_tmp189));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
