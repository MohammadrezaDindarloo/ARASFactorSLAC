// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl2
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl2(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 598

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp10 = 2 * _tmp9;
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = -_DeltaRot[0] * _Rot_init[0] - _DeltaRot[1] * _Rot_init[1] -
                        _DeltaRot[2] * _Rot_init[2] + _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp13 = 2 * _tmp5;
  const Scalar _tmp14 = _tmp12 * _tmp13;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp10 * _tmp5;
  const Scalar _tmp17 = 2 * _tmp12 * _tmp3;
  const Scalar _tmp18 = _tmp16 - _tmp17;
  const Scalar _tmp19 = Scalar(0.010999999999999999) * _tmp18;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = -_tmp15 + _tmp20;
  const Scalar _tmp22 = _tmp21 + _tmp8;
  const Scalar _tmp23 = _tmp22 + position_vector(1, 0);
  const Scalar _tmp24 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp26 = -_tmp25;
  const Scalar _tmp27 = _tmp13 * _tmp3;
  const Scalar _tmp28 = _tmp10 * _tmp12;
  const Scalar _tmp29 = _tmp27 + _tmp28;
  const Scalar _tmp30 = -Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp32 = _tmp30 - _tmp31;
  const Scalar _tmp33 = _tmp26 + _tmp32;
  const Scalar _tmp34 = _tmp33 + position_vector(0, 0);
  const Scalar _tmp35 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp36 = _tmp30 + _tmp31;
  const Scalar _tmp37 = _tmp25 + _tmp36;
  const Scalar _tmp38 = _tmp37 + position_vector(0, 0);
  const Scalar _tmp39 = _tmp15 + _tmp7;
  const Scalar _tmp40 = _tmp20 + _tmp39;
  const Scalar _tmp41 = _tmp40 + position_vector(1, 0);
  const Scalar _tmp42 = _tmp15 + _tmp20 + _tmp8;
  const Scalar _tmp43 = _tmp42 + position_vector(1, 0);
  const Scalar _tmp44 = _tmp43 + Scalar(8.3885017487099702);
  const Scalar _tmp45 = _tmp25 + _tmp32;
  const Scalar _tmp46 = _tmp45 + position_vector(0, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(-2.5193355532036801);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = _tmp44 * _tmp48;
  const Scalar _tmp51 = _tmp41 + Scalar(-4.7744369927459998);
  const Scalar _tmp52 = _tmp38 + Scalar(-2.7171519410699099);
  const Scalar _tmp53 =
      std::sqrt(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp52, Scalar(2))));
  const Scalar _tmp54 = Scalar(1.0) / (_tmp53);
  const Scalar _tmp55 = Scalar(1.0) / (_tmp52);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = _tmp56 * (_tmp37 * _tmp51 * _tmp54 - _tmp40 * _tmp52 * _tmp54);
  const Scalar _tmp58 = _tmp42 * _tmp49 - _tmp45 * _tmp50 + _tmp49 * _tmp57;
  const Scalar _tmp59 = _tmp51 * _tmp55;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp49 * _tmp59 - _tmp50);
  const Scalar _tmp61 = _tmp59 * _tmp60;
  const Scalar _tmp62 = Scalar(0.20999999999999999) * _tmp27 - Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp63 = -Scalar(0.010999999999999999) * _tmp24 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp64 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp65 = _tmp63 + _tmp64;
  const Scalar _tmp66 = _tmp62 + _tmp65;
  const Scalar _tmp67 = _tmp63 - _tmp64;
  const Scalar _tmp68 = _tmp62 + _tmp67;
  const Scalar _tmp69 = _tmp59 * _tmp66;
  const Scalar _tmp70 = -_tmp49 * _tmp69 + _tmp50 * _tmp68;
  const Scalar _tmp71 = _tmp61 * _tmp70 + _tmp69;
  const Scalar _tmp72 = Scalar(1.0) * _tmp40;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp42 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp37;
  const Scalar _tmp76 = -_tmp45 + _tmp75;
  const Scalar _tmp77 = _tmp74 * _tmp76;
  const Scalar _tmp78 = _tmp49 * _tmp66 - _tmp49 * _tmp68;
  const Scalar _tmp79 = _tmp61 * _tmp78 - _tmp66 - _tmp71 * _tmp77;
  const Scalar _tmp80 = _tmp26 + _tmp36;
  const Scalar _tmp81 = _tmp80 + position_vector(0, 0);
  const Scalar _tmp82 = _tmp81 + Scalar(1.7965602546229);
  const Scalar _tmp83 = _tmp21 + _tmp7;
  const Scalar _tmp84 = _tmp83 + position_vector(1, 0);
  const Scalar _tmp85 = _tmp84 + Scalar(-4.83288938413423);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp82 * _tmp86;
  const Scalar _tmp88 = -_tmp62;
  const Scalar _tmp89 = _tmp65 + _tmp88;
  const Scalar _tmp90 = _tmp85 * _tmp86;
  const Scalar _tmp91 = _tmp59 * _tmp87 - _tmp90;
  const Scalar _tmp92 = _tmp60 * _tmp91;
  const Scalar _tmp93 = -_tmp69 * _tmp87 - _tmp70 * _tmp92 + _tmp89 * _tmp90;
  const Scalar _tmp94 = _tmp66 * _tmp87 - _tmp77 * _tmp93 - _tmp78 * _tmp92 - _tmp87 * _tmp89;
  const Scalar _tmp95 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp96 = _tmp57 * _tmp87 - _tmp58 * _tmp92 - _tmp80 * _tmp90 + _tmp83 * _tmp87;
  const Scalar _tmp97 = _tmp95 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp94 * _tmp98;
  const Scalar _tmp100 = _tmp99 * (-_tmp57 + _tmp58 * _tmp61 - _tmp79 * _tmp97);
  const Scalar _tmp101 = _tmp100 + _tmp79;
  const Scalar _tmp102 = _tmp87 * _tmp95;
  const Scalar _tmp103 = _tmp91 * _tmp95;
  const Scalar _tmp104 = -_tmp101 * _tmp103 - _tmp59;
  const Scalar _tmp105 = _tmp49 * _tmp60;
  const Scalar _tmp106 = _tmp34 + Scalar(1.9874742031097401);
  const Scalar _tmp107 = _tmp23 + Scalar(8.3196563720703107);
  const Scalar _tmp108 =
      std::pow(Scalar(std::pow(_tmp106, Scalar(2)) + std::pow(_tmp107, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp109 = _tmp106 * _tmp108;
  const Scalar _tmp110 = _tmp109 * fh1;
  const Scalar _tmp111 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp112 = _tmp72 * _tmp77 + _tmp75;
  const Scalar _tmp113 = 0;
  const Scalar _tmp114 = _tmp113 * _tmp95;
  const Scalar _tmp115 = _tmp114 * _tmp92;
  const Scalar _tmp116 = _tmp56 * (_tmp114 * _tmp87 - _tmp115 * _tmp49);
  const Scalar _tmp117 = Scalar(1.0) * _tmp98;
  const Scalar _tmp118 = Scalar(1.0) * _tmp60;
  const Scalar _tmp119 = _tmp118 * _tmp91 * _tmp98;
  const Scalar _tmp120 = _tmp107 * _tmp108;
  const Scalar _tmp121 = fh1 * (-_tmp109 * _tmp22 + _tmp120 * _tmp33);
  const Scalar _tmp122 = Scalar(1.0) * _tmp74;
  const Scalar _tmp123 = -_tmp118 * _tmp78 + _tmp122 * _tmp60 * _tmp70 * _tmp76;
  const Scalar _tmp124 = _tmp99 * (-_tmp118 * _tmp58 - _tmp123 * _tmp97);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = -_tmp103 * _tmp125 + Scalar(1.0);
  const Scalar _tmp127 = _tmp120 * fh1;
  const Scalar _tmp128 = -_tmp110 * _tmp56 * (_tmp101 * _tmp102 + _tmp104 * _tmp105 + Scalar(1.0)) -
                         _tmp111 * _tmp116 -
                         _tmp121 * _tmp56 * (_tmp117 * _tmp87 - _tmp119 * _tmp49) -
                         _tmp127 * _tmp56 * (_tmp102 * _tmp125 + _tmp105 * _tmp126);
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = _tmp73 + _tmp83;
  const Scalar _tmp131 = _tmp130 * _tmp77;
  const Scalar _tmp132 = Scalar(1.0) / (-_tmp131 + _tmp75 - _tmp80);
  const Scalar _tmp133 = _tmp130 * _tmp132;
  const Scalar _tmp134 = _tmp93 * _tmp95;
  const Scalar _tmp135 = -_tmp118 * _tmp70 + _tmp124 * _tmp133 - _tmp125 * _tmp134;
  const Scalar _tmp136 = Scalar(1.0) * _tmp132;
  const Scalar _tmp137 = fh1 * (_tmp67 + _tmp88);
  const Scalar _tmp138 = -_tmp120 * _tmp137 - Scalar(3.29616) * _tmp18 - _tmp22 * fv1;
  const Scalar _tmp139 = _tmp136 * _tmp77;
  const Scalar _tmp140 = _tmp131 * _tmp136 + Scalar(1.0);
  const Scalar _tmp141 = -Scalar(1.0) * _tmp122 * _tmp140 + Scalar(1.0) * _tmp139;
  const Scalar _tmp142 = _tmp109 * _tmp137 + Scalar(3.29616) * _tmp29 + _tmp33 * fv1;
  const Scalar _tmp143 = _tmp130 * _tmp74;
  const Scalar _tmp144 = Scalar(1.0) * _tmp136 * _tmp143 - Scalar(1.0) * _tmp136;
  const Scalar _tmp145 = _tmp100 * _tmp133 - _tmp101 * _tmp134 + _tmp71;
  const Scalar _tmp146 = _tmp112 * _tmp132;
  const Scalar _tmp147 = _tmp74 * (-_tmp113 * _tmp134 - _tmp130 * _tmp146 + _tmp73);
  const Scalar _tmp148 = -Scalar(1.0) * _tmp112 * _tmp136 - Scalar(1.0) * _tmp147 + Scalar(1.0);
  const Scalar _tmp149 = _tmp136 * _tmp99;
  const Scalar _tmp150 = -_tmp117 * _tmp93 + _tmp130 * _tmp149;
  const Scalar _tmp151 = Scalar(1.0) * _tmp110 * (_tmp100 * _tmp136 - _tmp122 * _tmp145) +
                         _tmp111 * _tmp148 +
                         Scalar(1.0) * _tmp121 * (-_tmp122 * _tmp150 + _tmp149) +
                         Scalar(1.0) * _tmp127 * (-_tmp122 * _tmp135 + _tmp124 * _tmp136) +
                         _tmp138 * _tmp141 + _tmp142 * _tmp144;
  const Scalar _tmp152 = std::asinh(_tmp129 * _tmp151);
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp154 =
      -_tmp152 * _tmp153 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp38), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp41), Scalar(2))));
  const Scalar _tmp155 = Scalar(0.1034955) * _tmp129;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = Scalar(1.0) * _tmp152;
  const Scalar _tmp158 = Scalar(9.6622558468725703) * _tmp116;
  const Scalar _tmp159 = _tmp19 + _tmp39;
  const Scalar _tmp160 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp161 = _tmp116 * _tmp160;
  const Scalar _tmp162 =
      (_tmp129 * (_tmp141 * _tmp159 + _tmp144 * _tmp33 - _tmp148) - _tmp151 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp163 = _tmp136 * _tmp142;
  const Scalar _tmp164 = _tmp140 * _tmp74;
  const Scalar _tmp165 = _tmp110 * _tmp145 * _tmp74 + _tmp111 * _tmp147 +
                         _tmp121 * _tmp150 * _tmp74 + _tmp127 * _tmp135 * _tmp74 +
                         _tmp138 * _tmp164 - _tmp143 * _tmp163;
  const Scalar _tmp166 = _tmp111 * _tmp114;
  const Scalar _tmp167 = _tmp104 * _tmp110 * _tmp60 - _tmp119 * _tmp121 +
                         _tmp126 * _tmp127 * _tmp60 - _tmp166 * _tmp92;
  const Scalar _tmp168 = Scalar(1.0) / (_tmp167);
  const Scalar _tmp169 = std::asinh(_tmp165 * _tmp168);
  const Scalar _tmp170 = Scalar(1.0) * _tmp169;
  const Scalar _tmp171 = std::pow(_tmp167, Scalar(-2));
  const Scalar _tmp172 = _tmp115 * _tmp171;
  const Scalar _tmp173 = _tmp136 * _tmp33;
  const Scalar _tmp174 =
      (-_tmp165 * _tmp172 + _tmp168 * (-_tmp143 * _tmp173 - _tmp147 + _tmp159 * _tmp164)) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp171 + 1));
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp167;
  const Scalar _tmp176 =
      -_tmp169 * _tmp175 -
      Scalar(8.3885017487099702) *
          std::sqrt(
              Scalar(Scalar(0.090199313518583735) *
                         std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp46), Scalar(2)) +
                     std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp43 - 1), Scalar(2))));
  const Scalar _tmp177 = Scalar(9.6622558468725703) * _tmp114;
  const Scalar _tmp178 = _tmp177 * _tmp92;
  const Scalar _tmp179 = Scalar(0.1034955) * _tmp168;
  const Scalar _tmp180 = _tmp176 * _tmp179;
  const Scalar _tmp181 =
      _tmp101 * _tmp110 * _tmp95 + _tmp117 * _tmp121 + _tmp125 * _tmp127 * _tmp95 + _tmp166;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = -_tmp100 * _tmp110 * _tmp132 + _tmp111 * _tmp146 - _tmp121 * _tmp149 -
                         _tmp124 * _tmp127 * _tmp132 - _tmp138 * _tmp139 + _tmp163;
  const Scalar _tmp184 = std::asinh(_tmp182 * _tmp183);
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp186 =
      -_tmp184 * _tmp185 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp84), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp81 - 1), Scalar(2))));
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 = Scalar(1.0) * _tmp184;
  const Scalar _tmp190 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp191 = _tmp114 * _tmp190;
  const Scalar _tmp192 = (_tmp182 * (-_tmp139 * _tmp159 - _tmp146 + _tmp173) + _tmp183 * _tmp191) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp190 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp35 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(
                   Scalar(0.1034955) * _tmp0 *
                   (-_tmp1 * _tmp35 -
                    Scalar(8.3196563720703107) *
                        std::sqrt(Scalar(
                            std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp23 - 1), Scalar(2)) +
                            Scalar(0.057067943527034905) *
                                std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp34 - 1),
                                         Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-Scalar(1.0) * _tmp162 * std::cosh(_tmp157) -
                          (-Scalar(0.1034955) * _tmp154 * _tmp161 +
                           _tmp155 * (-_tmp152 * _tmp158 - _tmp153 * _tmp162)) *
                              std::cosh(_tmp156)) +
               _tmp158 * (-std::sinh(_tmp156) - std::sinh(_tmp157));
  _res(2, 0) = _tmp175 * (-Scalar(1.0) * _tmp174 * std::cosh(_tmp170) -
                          (-Scalar(0.1034955) * _tmp172 * _tmp176 +
                           _tmp179 * (-_tmp169 * _tmp178 - _tmp174 * _tmp175)) *
                              std::cosh(_tmp180)) +
               _tmp178 * (-std::sinh(_tmp170) - std::sinh(_tmp180));
  _res(3, 0) = -_tmp177 * (-std::sinh(_tmp188) - std::sinh(_tmp189)) +
               _tmp185 * (-Scalar(1.0) * _tmp192 * std::cosh(_tmp189) -
                          (Scalar(0.1034955) * _tmp186 * _tmp191 +
                           _tmp187 * (_tmp177 * _tmp184 - _tmp185 * _tmp192)) *
                              std::cosh(_tmp188));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
