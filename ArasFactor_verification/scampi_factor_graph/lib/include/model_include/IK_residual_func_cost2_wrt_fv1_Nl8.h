// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl8
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     Rot_init: Rot3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl8(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const sym::Rot3<Scalar>& Rot_init,
    const Scalar epsilon) {
  // Total ops: 601

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 4, 1>& _Rot_init = Rot_init.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * _Rot_init[3] - _DeltaRot[1] * _Rot_init[2] +
                       _DeltaRot[2] * _Rot_init[1] + _DeltaRot[3] * _Rot_init[0];
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * _Rot_init[1] + _DeltaRot[1] * _Rot_init[0] +
                       _DeltaRot[2] * _Rot_init[3] + _DeltaRot[3] * _Rot_init[2];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4 +
                       Scalar(0.20999999999999999);
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * _Rot_init[2] + _DeltaRot[1] * _Rot_init[3] -
                       _DeltaRot[2] * _Rot_init[0] + _DeltaRot[3] * _Rot_init[1];
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * _Rot_init[0] - 2 * _DeltaRot[1] * _Rot_init[1] -
                        2 * _DeltaRot[2] * _Rot_init[2] + 2 * _DeltaRot[3] * _Rot_init[3];
  const Scalar _tmp11 = _tmp1 * _tmp10;
  const Scalar _tmp12 = -_tmp11 + _tmp9;
  const Scalar _tmp13 = Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = -_tmp13;
  const Scalar _tmp15 = _tmp1 * _tmp8;
  const Scalar _tmp16 = _tmp10 * _tmp3;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(1, 0);
  const Scalar _tmp21 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp23 = 2 * _tmp1 * _tmp3;
  const Scalar _tmp24 = _tmp10 * _tmp7;
  const Scalar _tmp25 = _tmp23 + _tmp24;
  const Scalar _tmp26 = -Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp28 = _tmp26 - _tmp27;
  const Scalar _tmp29 = _tmp22 + _tmp28;
  const Scalar _tmp30 = _tmp29 + position_vector(0, 0);
  const Scalar _tmp31 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp32 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp33 = Scalar(1.0) * _tmp0 /
                        std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp34 = _tmp26 + _tmp27;
  const Scalar _tmp35 = _tmp22 + _tmp34;
  const Scalar _tmp36 = _tmp35 + position_vector(0, 0);
  const Scalar _tmp37 = _tmp18 + _tmp5;
  const Scalar _tmp38 = _tmp37 + position_vector(1, 0);
  const Scalar _tmp39 = -_tmp22;
  const Scalar _tmp40 = _tmp28 + _tmp39;
  const Scalar _tmp41 = -_tmp17;
  const Scalar _tmp42 = _tmp14 + _tmp41;
  const Scalar _tmp43 = _tmp42 + _tmp6;
  const Scalar _tmp44 = _tmp43 + position_vector(1, 0);
  const Scalar _tmp45 = _tmp44 + Scalar(8.3196563720703107);
  const Scalar _tmp46 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp47 = _tmp46 + Scalar(1.9874742031097401);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp45, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp45 * _tmp48;
  const Scalar _tmp50 = _tmp47 * _tmp48;
  const Scalar _tmp51 = _tmp38 + Scalar(-4.7744369927459998);
  const Scalar _tmp52 = _tmp36 + Scalar(-2.7171519410699099);
  const Scalar _tmp53 =
      std::sqrt(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp52, Scalar(2))));
  const Scalar _tmp54 = Scalar(1.0) / (_tmp53);
  const Scalar _tmp55 = Scalar(1.0) / (_tmp52);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = _tmp56 * (_tmp35 * _tmp51 * _tmp54 - _tmp37 * _tmp52 * _tmp54);
  const Scalar _tmp58 = -_tmp40 * _tmp49 + _tmp43 * _tmp50 + _tmp50 * _tmp57;
  const Scalar _tmp59 = _tmp42 + _tmp5;
  const Scalar _tmp60 = _tmp59 + position_vector(1, 0);
  const Scalar _tmp61 = _tmp60 + Scalar(-4.83288938413423);
  const Scalar _tmp62 = _tmp34 + _tmp39;
  const Scalar _tmp63 = _tmp62 + position_vector(0, 0);
  const Scalar _tmp64 = _tmp63 + Scalar(1.7965602546229);
  const Scalar _tmp65 = std::pow(Scalar(std::pow(_tmp61, Scalar(2)) + std::pow(_tmp64, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp66 = _tmp61 * _tmp65;
  const Scalar _tmp67 = _tmp51 * _tmp55;
  const Scalar _tmp68 = _tmp64 * _tmp65;
  const Scalar _tmp69 = -_tmp66 + _tmp67 * _tmp68;
  const Scalar _tmp70 = Scalar(1.0) / (-_tmp49 + _tmp50 * _tmp67);
  const Scalar _tmp71 = _tmp69 * _tmp70;
  const Scalar _tmp72 = _tmp57 * _tmp68 - _tmp58 * _tmp71 + _tmp59 * _tmp68 - _tmp62 * _tmp66;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp73;
  const Scalar _tmp75 = _tmp20 + Scalar(8.3885017487099702);
  const Scalar _tmp76 = _tmp30 + Scalar(-2.5193355532036801);
  const Scalar _tmp77 = std::pow(Scalar(std::pow(_tmp75, Scalar(2)) + std::pow(_tmp76, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp78 = _tmp76 * _tmp77;
  const Scalar _tmp79 = _tmp75 * _tmp77;
  const Scalar _tmp80 = fh1 * (-_tmp19 * _tmp78 + _tmp29 * _tmp79);
  const Scalar _tmp81 = _tmp67 * _tmp70;
  const Scalar _tmp82 = Scalar(0.20999999999999999) * _tmp23 - Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp83 = -_tmp82;
  const Scalar _tmp84 =
      -Scalar(0.010999999999999999) * _tmp2 - Scalar(0.010999999999999999) * _tmp21;
  const Scalar _tmp85 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp86 = _tmp84 - _tmp85;
  const Scalar _tmp87 = _tmp83 + _tmp86;
  const Scalar _tmp88 = _tmp84 + _tmp85;
  const Scalar _tmp89 = _tmp82 + _tmp88;
  const Scalar _tmp90 = _tmp70 * (-_tmp50 * _tmp87 + _tmp50 * _tmp89);
  const Scalar _tmp91 = _tmp67 * _tmp89;
  const Scalar _tmp92 = _tmp49 * _tmp87 - _tmp50 * _tmp91;
  const Scalar _tmp93 = _tmp81 * _tmp92 + _tmp91;
  const Scalar _tmp94 = Scalar(1.0) * _tmp37;
  const Scalar _tmp95 = -_tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp43 + _tmp95);
  const Scalar _tmp97 = Scalar(1.0) * _tmp35;
  const Scalar _tmp98 = _tmp96 * (-_tmp40 + _tmp97);
  const Scalar _tmp99 = _tmp67 * _tmp90 - _tmp89 - _tmp93 * _tmp98;
  const Scalar _tmp100 = _tmp83 + _tmp88;
  const Scalar _tmp101 = _tmp100 * _tmp66 - _tmp68 * _tmp91 - _tmp71 * _tmp92;
  const Scalar _tmp102 = -_tmp100 * _tmp68 - _tmp101 * _tmp98 + _tmp68 * _tmp89 - _tmp69 * _tmp90;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp102);
  const Scalar _tmp104 = _tmp103 * _tmp72;
  const Scalar _tmp105 = _tmp102 * _tmp73;
  const Scalar _tmp106 = _tmp105 * (-_tmp104 * _tmp99 - _tmp57 + _tmp58 * _tmp81);
  const Scalar _tmp107 = _tmp106 + _tmp99;
  const Scalar _tmp108 = _tmp103 * _tmp68;
  const Scalar _tmp109 = _tmp103 * _tmp69;
  const Scalar _tmp110 = -_tmp107 * _tmp109 - _tmp67;
  const Scalar _tmp111 = _tmp50 * _tmp70;
  const Scalar _tmp112 = _tmp78 * fh1;
  const Scalar _tmp113 = Scalar(1.0) * _tmp70;
  const Scalar _tmp114 = _tmp113 * _tmp92;
  const Scalar _tmp115 = _tmp114 * _tmp98 - Scalar(1.0) * _tmp90;
  const Scalar _tmp116 = _tmp105 * (-_tmp104 * _tmp115 - _tmp113 * _tmp58);
  const Scalar _tmp117 = _tmp115 + _tmp116;
  const Scalar _tmp118 = -_tmp109 * _tmp117 + Scalar(1.0);
  const Scalar _tmp119 = _tmp79 * fh1;
  const Scalar _tmp120 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp121 = _tmp94 * _tmp98 + _tmp97;
  const Scalar _tmp122 = 0;
  const Scalar _tmp123 = _tmp109 * _tmp122;
  const Scalar _tmp124 = _tmp103 * _tmp122;
  const Scalar _tmp125 = _tmp56 * (-_tmp111 * _tmp123 + _tmp124 * _tmp68);
  const Scalar _tmp126 = -_tmp112 * _tmp56 * (_tmp107 * _tmp108 + _tmp110 * _tmp111 + Scalar(1.0)) -
                         _tmp119 * _tmp56 * (_tmp108 * _tmp117 + _tmp111 * _tmp118) -
                         _tmp120 * _tmp125 -
                         _tmp56 * _tmp80 * (-_tmp50 * _tmp71 * _tmp74 + _tmp68 * _tmp74);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = _tmp59 + _tmp95;
  const Scalar _tmp129 = _tmp128 * _tmp98;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp62 + _tmp97);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp105 * _tmp131;
  const Scalar _tmp133 = -_tmp101 * _tmp74 + _tmp128 * _tmp132;
  const Scalar _tmp134 = Scalar(1.0) * _tmp96;
  const Scalar _tmp135 = _tmp101 * _tmp103;
  const Scalar _tmp136 = _tmp121 * _tmp130;
  const Scalar _tmp137 = _tmp96 * (-_tmp122 * _tmp135 - _tmp128 * _tmp136 + _tmp95);
  const Scalar _tmp138 = -Scalar(1.0) * _tmp121 * _tmp131 - Scalar(1.0) * _tmp137 + Scalar(1.0);
  const Scalar _tmp139 = _tmp128 * _tmp130;
  const Scalar _tmp140 = _tmp106 * _tmp139 - _tmp107 * _tmp135 + _tmp93;
  const Scalar _tmp141 = -_tmp114 + _tmp116 * _tmp139 - _tmp117 * _tmp135;
  const Scalar _tmp142 = fh1 * (_tmp82 + _tmp86);
  const Scalar _tmp143 = _tmp142 * _tmp78 + Scalar(3.29616) * _tmp25 + _tmp29 * fv1;
  const Scalar _tmp144 = _tmp128 * _tmp96;
  const Scalar _tmp145 = Scalar(1.0) * _tmp131 * _tmp144 - Scalar(1.0) * _tmp131;
  const Scalar _tmp146 = -Scalar(3.29616) * _tmp12 - _tmp142 * _tmp79 - _tmp19 * fv1;
  const Scalar _tmp147 = _tmp96 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp148 = _tmp131 * _tmp98;
  const Scalar _tmp149 = -Scalar(1.0) * _tmp147 + Scalar(1.0) * _tmp148;
  const Scalar _tmp150 = Scalar(1.0) * _tmp112 * (_tmp106 * _tmp131 - _tmp134 * _tmp140) +
                         Scalar(1.0) * _tmp119 * (_tmp116 * _tmp131 - _tmp134 * _tmp141) +
                         _tmp120 * _tmp138 + _tmp143 * _tmp145 + _tmp146 * _tmp149 +
                         Scalar(1.0) * _tmp80 * (_tmp132 - _tmp133 * _tmp134);
  const Scalar _tmp151 = std::asinh(_tmp127 * _tmp150);
  const Scalar _tmp152 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp153 =
      -_tmp151 * _tmp152 -
      Scalar(4.7744369927459998) *
          std::sqrt(
              Scalar(Scalar(0.32387954179207445) *
                         std::pow(Scalar(1 - Scalar(0.36803241838814449) * _tmp36), Scalar(2)) +
                     std::pow(Scalar(1 - Scalar(0.2094487793051498) * _tmp38), Scalar(2))));
  const Scalar _tmp154 = Scalar(0.1034955) * _tmp127;
  const Scalar _tmp155 = _tmp153 * _tmp154;
  const Scalar _tmp156 = Scalar(1.0) * _tmp151;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp125;
  const Scalar _tmp158 = std::pow(_tmp126, Scalar(-2));
  const Scalar _tmp159 = _tmp13 + _tmp41 + _tmp5;
  const Scalar _tmp160 = _tmp125 * _tmp158;
  const Scalar _tmp161 =
      (_tmp127 * (-_tmp138 + _tmp145 * _tmp29 + _tmp149 * _tmp159) - _tmp150 * _tmp160) /
      std::sqrt(Scalar(std::pow(_tmp150, Scalar(2)) * _tmp158 + 1));
  const Scalar _tmp162 = _tmp74 * _tmp80;
  const Scalar _tmp163 = _tmp70 * fh1;
  const Scalar _tmp164 = _tmp123 * _tmp70;
  const Scalar _tmp165 = _tmp110 * _tmp163 * _tmp78 + _tmp118 * _tmp163 * _tmp79 -
                         _tmp120 * _tmp164 - _tmp162 * _tmp71;
  const Scalar _tmp166 = Scalar(1.0) / (_tmp165);
  const Scalar _tmp167 = _tmp131 * _tmp143;
  const Scalar _tmp168 = _tmp112 * _tmp140 * _tmp96 + _tmp119 * _tmp141 * _tmp96 +
                         _tmp120 * _tmp137 + _tmp133 * _tmp80 * _tmp96 - _tmp144 * _tmp167 +
                         _tmp146 * _tmp147;
  const Scalar _tmp169 = std::asinh(_tmp166 * _tmp168);
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 -
      Scalar(8.3196563720703107) *
          std::sqrt(
              Scalar(std::pow(Scalar(-Scalar(0.12019727201198747) * _tmp44 - 1), Scalar(2)) +
                     Scalar(0.057067943527034905) *
                         std::pow(Scalar(-Scalar(0.50315118477277876) * _tmp46 - 1), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp166;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = _tmp131 * _tmp29;
  const Scalar _tmp175 = std::pow(_tmp165, Scalar(-2));
  const Scalar _tmp176 = _tmp164 * _tmp175;
  const Scalar _tmp177 =
      (_tmp166 * (-_tmp137 - _tmp144 * _tmp174 + _tmp147 * _tmp159) - _tmp168 * _tmp176) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp175 + 1));
  const Scalar _tmp178 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp179 = Scalar(1.0) * _tmp169;
  const Scalar _tmp180 = -_tmp106 * _tmp112 * _tmp130 - _tmp116 * _tmp119 * _tmp130 +
                         _tmp120 * _tmp136 - _tmp132 * _tmp80 - _tmp146 * _tmp148 + _tmp167;
  const Scalar _tmp181 =
      _tmp103 * _tmp107 * _tmp112 + _tmp103 * _tmp117 * _tmp119 + _tmp120 * _tmp124 + _tmp162;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = std::asinh(_tmp180 * _tmp182);
  const Scalar _tmp184 = Scalar(1.0) * _tmp183;
  const Scalar _tmp185 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp186 = _tmp124 * _tmp185;
  const Scalar _tmp187 = (_tmp180 * _tmp186 + _tmp182 * (-_tmp136 - _tmp148 * _tmp159 + _tmp174)) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp185 + 1));
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp189 =
      -_tmp183 * _tmp188 -
      Scalar(4.83288938413423) *
          std::sqrt(
              Scalar(std::pow(Scalar(1 - Scalar(0.20691555724053495) * _tmp60), Scalar(2)) +
                     Scalar(0.13818785160942856) *
                         std::pow(Scalar(-Scalar(0.55661923802822921) * _tmp63 - 1), Scalar(2))));
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp191 = _tmp189 * _tmp190;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp124;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp32 *
      (-_tmp33 * std::cosh(Scalar(1.0) * _tmp31) +
       _tmp33 *
           std::cosh(
               Scalar(0.1034955) * _tmp0 *
               (-_tmp31 * _tmp32 -
                Scalar(8.3885017487099702) *
                    std::sqrt(Scalar(
                        Scalar(0.090199313518583735) *
                            std::pow(Scalar(1 - Scalar(0.39693005512043167) * _tmp30), Scalar(2)) +
                        std::pow(Scalar(-Scalar(0.11921079949155229) * _tmp20 - 1), Scalar(2)))))));
  _res(1, 0) = _tmp152 * (-Scalar(1.0) * _tmp161 * std::cosh(_tmp156) -
                          (-Scalar(0.1034955) * _tmp153 * _tmp160 +
                           _tmp154 * (-_tmp151 * _tmp157 - _tmp152 * _tmp161)) *
                              std::cosh(_tmp155)) +
               _tmp157 * (-std::sinh(_tmp155) - std::sinh(_tmp156));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp177 * std::cosh(_tmp179) -
                          (-Scalar(0.1034955) * _tmp171 * _tmp176 +
                           _tmp172 * (-_tmp169 * _tmp178 - _tmp170 * _tmp177)) *
                              std::cosh(_tmp173)) +
               _tmp178 * (-std::sinh(_tmp173) - std::sinh(_tmp179));
  _res(3, 0) = _tmp188 * (-Scalar(1.0) * _tmp187 * std::cosh(_tmp184) -
                          (Scalar(0.1034955) * _tmp186 * _tmp189 +
                           _tmp190 * (_tmp183 * _tmp192 - _tmp187 * _tmp188)) *
                              std::cosh(_tmp191)) -
               _tmp192 * (-std::sinh(_tmp184) - std::sinh(_tmp191));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
