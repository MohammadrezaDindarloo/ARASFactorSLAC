// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl21
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl21(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 622

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (214)
  const Scalar _tmp0 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp1 = -2 * std::pow(_tmp0, Scalar(2));
  const Scalar _tmp2 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp3 = 1 - 2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp4 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp3;
  const Scalar _tmp5 = -_tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp7 = 2 * _tmp0 * _tmp6;
  const Scalar _tmp8 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp9 = _tmp2 * _tmp8;
  const Scalar _tmp10 = _tmp7 + _tmp9;
  const Scalar _tmp11 = -Scalar(0.010999999999999999) * _tmp10;
  const Scalar _tmp12 = 2 * _tmp2;
  const Scalar _tmp13 = _tmp12 * _tmp6;
  const Scalar _tmp14 = _tmp0 * _tmp8;
  const Scalar _tmp15 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp16 = _tmp11 + _tmp15;
  const Scalar _tmp17 = _tmp16 + _tmp5;
  const Scalar _tmp18 = _tmp17 + p_init0;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp13 + Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp20 = -_tmp19;
  const Scalar _tmp21 = _tmp0 * _tmp12;
  const Scalar _tmp22 = _tmp6 * _tmp8;
  const Scalar _tmp23 = _tmp21 - _tmp22;
  const Scalar _tmp24 = -Scalar(0.010999999999999999) * _tmp23;
  const Scalar _tmp25 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp1 + Scalar(0.20999999999999999) * _tmp25 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp27 = _tmp24 + _tmp26;
  const Scalar _tmp28 = _tmp20 + _tmp27;
  const Scalar _tmp29 = _tmp28 + p_init1;
  const Scalar _tmp30 = Scalar(1.0) / (fh1);
  const Scalar _tmp31 = _tmp30 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -_tmp33 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp18 + p_d(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp29 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp35 = Scalar(0.1034955) * _tmp30;
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp32;
  const Scalar _tmp40 = _tmp29 - p_d(1, 0);
  const Scalar _tmp41 = _tmp18 - p_d(0, 0);
  const Scalar _tmp42 = std::pow(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp41, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp43 = _tmp41 * _tmp42;
  const Scalar _tmp44 = _tmp40 * _tmp42;
  const Scalar _tmp45 = _tmp17 * _tmp44 - _tmp28 * _tmp43;
  const Scalar _tmp46 = _tmp11 - _tmp15;
  const Scalar _tmp47 = _tmp46 + _tmp5;
  const Scalar _tmp48 = _tmp47 + p_init0;
  const Scalar _tmp49 = _tmp48 - p_a(0, 0);
  const Scalar _tmp50 = _tmp24 - _tmp26;
  const Scalar _tmp51 = _tmp20 + _tmp50;
  const Scalar _tmp52 = _tmp51 + p_init1;
  const Scalar _tmp53 = _tmp52 - p_a(1, 0);
  const Scalar _tmp54 = std::pow(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp53, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp55 = _tmp49 * _tmp54;
  const Scalar _tmp56 = _tmp16 + _tmp4;
  const Scalar _tmp57 = _tmp56 + p_init0;
  const Scalar _tmp58 = _tmp57 - p_c(0, 0);
  const Scalar _tmp59 = _tmp19 + _tmp27;
  const Scalar _tmp60 = _tmp59 + p_init1;
  const Scalar _tmp61 = _tmp60 - p_c(1, 0);
  const Scalar _tmp62 = std::pow(Scalar(std::pow(_tmp58, Scalar(2)) + std::pow(_tmp61, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp63 = _tmp61 * _tmp62;
  const Scalar _tmp64 = _tmp58 * _tmp62;
  const Scalar _tmp65 = _tmp19 + _tmp50;
  const Scalar _tmp66 = _tmp65 + p_init1;
  const Scalar _tmp67 = _tmp66 - p_b(1, 0);
  const Scalar _tmp68 = _tmp4 + _tmp46;
  const Scalar _tmp69 = _tmp68 + p_init0;
  const Scalar _tmp70 = _tmp69 - p_b(0, 0);
  const Scalar _tmp71 =
      std::sqrt(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp70, Scalar(2))));
  const Scalar _tmp72 = Scalar(1.0) / (_tmp71);
  const Scalar _tmp73 = Scalar(1.0) / (_tmp70);
  const Scalar _tmp74 = _tmp71 * _tmp73;
  const Scalar _tmp75 = _tmp74 * (-_tmp65 * _tmp70 * _tmp72 + _tmp67 * _tmp68 * _tmp72);
  const Scalar _tmp76 = -_tmp56 * _tmp63 + _tmp59 * _tmp64 + _tmp64 * _tmp75;
  const Scalar _tmp77 = _tmp67 * _tmp73;
  const Scalar _tmp78 = Scalar(1.0) / (-_tmp63 + _tmp64 * _tmp77);
  const Scalar _tmp79 = _tmp53 * _tmp54;
  const Scalar _tmp80 = _tmp55 * _tmp77 - _tmp79;
  const Scalar _tmp81 = _tmp78 * _tmp80;
  const Scalar _tmp82 = -_tmp47 * _tmp79 + _tmp51 * _tmp55 + _tmp55 * _tmp75 - _tmp76 * _tmp81;
  const Scalar _tmp83 = Scalar(1.0) / (_tmp82);
  const Scalar _tmp84 = Scalar(1.0) * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) * _tmp78;
  const Scalar _tmp86 = _tmp80 * _tmp83 * _tmp85;
  const Scalar _tmp87 = _tmp45 * _tmp74 * (_tmp55 * _tmp84 - _tmp64 * _tmp86);
  const Scalar _tmp88 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp22;
  const Scalar _tmp89 = -_tmp88;
  const Scalar _tmp90 =
      -Scalar(0.010999999999999999) * _tmp25 - Scalar(0.010999999999999999) * _tmp3;
  const Scalar _tmp91 = Scalar(0.20999999999999999) * _tmp7 - Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp92 = _tmp90 + _tmp91;
  const Scalar _tmp93 = _tmp89 + _tmp92;
  const Scalar _tmp94 = _tmp88 + _tmp92;
  const Scalar _tmp95 = _tmp64 * _tmp93 - _tmp64 * _tmp94;
  const Scalar _tmp96 = _tmp90 - _tmp91;
  const Scalar _tmp97 = _tmp89 + _tmp96;
  const Scalar _tmp98 = _tmp77 * _tmp93;
  const Scalar _tmp99 = _tmp63 * _tmp94 - _tmp64 * _tmp98;
  const Scalar _tmp100 = -_tmp55 * _tmp98 + _tmp79 * _tmp97 - _tmp81 * _tmp99;
  const Scalar _tmp101 = Scalar(1.0) * _tmp65;
  const Scalar _tmp102 = -_tmp101;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp102 + _tmp59);
  const Scalar _tmp104 = Scalar(1.0) * _tmp68;
  const Scalar _tmp105 = _tmp103 * (_tmp104 - _tmp56);
  const Scalar _tmp106 = -_tmp100 * _tmp105 + _tmp55 * _tmp93 - _tmp55 * _tmp97 - _tmp81 * _tmp95;
  const Scalar _tmp107 = Scalar(1.0) / (_tmp106);
  const Scalar _tmp108 = _tmp85 * _tmp99;
  const Scalar _tmp109 = _tmp105 * _tmp108 - _tmp85 * _tmp95;
  const Scalar _tmp110 = _tmp107 * _tmp82;
  const Scalar _tmp111 = _tmp106 * _tmp83;
  const Scalar _tmp112 = _tmp111 * (-_tmp109 * _tmp110 - _tmp76 * _tmp85);
  const Scalar _tmp113 = _tmp109 + _tmp112;
  const Scalar _tmp114 = _tmp107 * _tmp113;
  const Scalar _tmp115 = _tmp107 * _tmp80;
  const Scalar _tmp116 = _tmp78 * (-_tmp113 * _tmp115 + Scalar(1.0));
  const Scalar _tmp117 = _tmp44 * _tmp74 * (_tmp114 * _tmp55 + _tmp116 * _tmp64);
  const Scalar _tmp118 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp119 = _tmp101 * _tmp105 + _tmp104;
  const Scalar _tmp120 = 0;
  const Scalar _tmp121 = _tmp77 * _tmp78;
  const Scalar _tmp122 = _tmp121 * _tmp99 + _tmp98;
  const Scalar _tmp123 = -_tmp105 * _tmp122 + _tmp121 * _tmp95 - _tmp93;
  const Scalar _tmp124 = _tmp111 * (-_tmp110 * _tmp123 + _tmp121 * _tmp76 - _tmp75);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = _tmp107 * _tmp125;
  const Scalar _tmp127 = _tmp78 * (-_tmp115 * _tmp125 - _tmp77);
  const Scalar _tmp128 = _tmp43 * _tmp74 * (_tmp126 * _tmp55 + _tmp127 * _tmp64 + Scalar(1.0));
  const Scalar _tmp129 = -_tmp117 * fh1 -
                         _tmp118 * _tmp74 * (_tmp120 * _tmp55 - _tmp120 * _tmp64 * _tmp81) -
                         _tmp128 * fh1 - _tmp87 * fh1;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp102 + _tmp51;
  const Scalar _tmp132 = _tmp105 * _tmp131;
  const Scalar _tmp133 = Scalar(1.0) / (_tmp104 - _tmp132 - _tmp47);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp100 * _tmp107;
  const Scalar _tmp136 = _tmp131 * _tmp133;
  const Scalar _tmp137 = _tmp122 + _tmp124 * _tmp136 - _tmp125 * _tmp135;
  const Scalar _tmp138 = Scalar(1.0) * _tmp103;
  const Scalar _tmp139 = Scalar(1.0) * _tmp43 * (_tmp124 * _tmp134 - _tmp137 * _tmp138);
  const Scalar _tmp140 = _tmp119 * _tmp133;
  const Scalar _tmp141 = -_tmp100 * _tmp120 + _tmp102 - _tmp131 * _tmp140;
  const Scalar _tmp142 = -_tmp108 + _tmp112 * _tmp136 - _tmp113 * _tmp135;
  const Scalar _tmp143 = Scalar(1.0) * _tmp44 * (_tmp112 * _tmp134 - _tmp138 * _tmp142);
  const Scalar _tmp144 = _tmp88 + _tmp96;
  const Scalar _tmp145 = _tmp144 * fh1;
  const Scalar _tmp146 = -_tmp145 * _tmp44 - Scalar(3.29616) * _tmp23 - _tmp28 * fv1;
  const Scalar _tmp147 = _tmp105 * _tmp134;
  const Scalar _tmp148 = _tmp103 * (_tmp132 * _tmp134 + Scalar(1.0));
  const Scalar _tmp149 = Scalar(1.0) * _tmp147 - Scalar(1.0) * _tmp148;
  const Scalar _tmp150 = _tmp111 * _tmp134;
  const Scalar _tmp151 = _tmp103 * (-_tmp100 * _tmp84 + _tmp131 * _tmp150);
  const Scalar _tmp152 = Scalar(1.0) * _tmp45;
  const Scalar _tmp153 = _tmp152 * (_tmp150 - Scalar(1.0) * _tmp151);
  const Scalar _tmp154 = Scalar(3.29616) * _tmp10 + _tmp145 * _tmp43 + _tmp17 * fv1;
  const Scalar _tmp155 = _tmp103 * _tmp131;
  const Scalar _tmp156 = Scalar(1.0) * _tmp134 * _tmp155 - Scalar(1.0) * _tmp134;
  const Scalar _tmp157 =
      Scalar(1.0) * _tmp118 * (-_tmp119 * _tmp134 - _tmp138 * _tmp141 + Scalar(1.0)) +
      _tmp139 * fh1 + _tmp143 * fh1 + _tmp146 * _tmp149 + _tmp153 * fh1 + _tmp154 * _tmp156;
  const Scalar _tmp158 = std::asinh(_tmp130 * _tmp157);
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp160 =
      -_tmp129 * _tmp159 - std::sqrt(Scalar(std::pow(Scalar(-_tmp66 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp69 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp161 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp162 = -_tmp117 - _tmp128 - _tmp87;
  const Scalar _tmp163 = _tmp161 * _tmp162;
  const Scalar _tmp164 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp165 = _tmp144 * _tmp44;
  const Scalar _tmp166 = _tmp144 * _tmp43;
  const Scalar _tmp167 =
      (_tmp130 * (_tmp139 + _tmp143 - _tmp149 * _tmp165 + _tmp153 + _tmp156 * _tmp166) -
       _tmp157 * _tmp163) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp161 + 1));
  const Scalar _tmp168 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp169 = _tmp160 * _tmp168;
  const Scalar _tmp170 = Scalar(1.0) * _tmp158;
  const Scalar _tmp171 = _tmp45 * _tmp86;
  const Scalar _tmp172 = _tmp127 * _tmp43;
  const Scalar _tmp173 = _tmp116 * _tmp44;
  const Scalar _tmp174 = _tmp118 * _tmp120;
  const Scalar _tmp175 = -_tmp171 * fh1 + _tmp172 * fh1 + _tmp173 * fh1 - _tmp174 * _tmp81;
  const Scalar _tmp176 = Scalar(1.0) / (_tmp175);
  const Scalar _tmp177 = _tmp103 * _tmp142 * _tmp44;
  const Scalar _tmp178 = _tmp103 * _tmp43;
  const Scalar _tmp179 = _tmp137 * _tmp178;
  const Scalar _tmp180 = _tmp134 * _tmp154;
  const Scalar _tmp181 = _tmp151 * _tmp45;
  const Scalar _tmp182 = _tmp103 * _tmp118 * _tmp141 + _tmp146 * _tmp148 - _tmp155 * _tmp180 +
                         _tmp177 * fh1 + _tmp179 * fh1 + _tmp181 * fh1;
  const Scalar _tmp183 = std::asinh(_tmp176 * _tmp182);
  const Scalar _tmp184 = Scalar(1.0) * _tmp183;
  const Scalar _tmp185 = std::pow(_tmp175, Scalar(-2));
  const Scalar _tmp186 = -_tmp171 + _tmp172 + _tmp173;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = (_tmp176 * (-_tmp131 * _tmp134 * _tmp144 * _tmp178 - _tmp148 * _tmp165 +
                                     _tmp177 + _tmp179 + _tmp181) -
                          _tmp182 * _tmp187) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp185 + 1));
  const Scalar _tmp189 = Scalar(9.6622558468725703) * _tmp175;
  const Scalar _tmp190 =
      -_tmp183 * _tmp189 - std::sqrt(Scalar(std::pow(Scalar(-_tmp57 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp60 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp186;
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp176;
  const Scalar _tmp193 = _tmp190 * _tmp192;
  const Scalar _tmp194 = _tmp112 * _tmp133 * _tmp44;
  const Scalar _tmp195 = _tmp150 * _tmp45;
  const Scalar _tmp196 = _tmp124 * _tmp133 * _tmp43;
  const Scalar _tmp197 = _tmp118 * _tmp140 - _tmp146 * _tmp147 + _tmp180 - _tmp194 * fh1 -
                         _tmp195 * fh1 - _tmp196 * fh1;
  const Scalar _tmp198 = _tmp152 * _tmp83;
  const Scalar _tmp199 = _tmp114 * _tmp44;
  const Scalar _tmp200 = _tmp126 * _tmp43;
  const Scalar _tmp201 = _tmp174 + _tmp198 * fh1 + _tmp199 * fh1 + _tmp200 * fh1;
  const Scalar _tmp202 = Scalar(1.0) / (_tmp201);
  const Scalar _tmp203 = std::asinh(_tmp197 * _tmp202);
  const Scalar _tmp204 = Scalar(9.6622558468725703) * _tmp201;
  const Scalar _tmp205 =
      -_tmp203 * _tmp204 - std::sqrt(Scalar(std::pow(Scalar(-_tmp48 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp52 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp206 = Scalar(0.1034955) * _tmp202;
  const Scalar _tmp207 = _tmp205 * _tmp206;
  const Scalar _tmp208 = Scalar(1.0) * _tmp203;
  const Scalar _tmp209 = _tmp198 + _tmp199 + _tmp200;
  const Scalar _tmp210 = Scalar(9.6622558468725703) * _tmp209;
  const Scalar _tmp211 = std::pow(_tmp201, Scalar(-2));
  const Scalar _tmp212 = _tmp209 * _tmp211;
  const Scalar _tmp213 = (-_tmp197 * _tmp212 + _tmp202 * (_tmp134 * _tmp166 + _tmp147 * _tmp165 -
                                                          _tmp194 - _tmp195 - _tmp196)) /
                         std::sqrt(Scalar(std::pow(_tmp197, Scalar(2)) * _tmp211 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp39) -
                    (-Scalar(0.1034955) * _tmp34 * _tmp37 +
                     _tmp35 * (Scalar(9.6622558468725703) * _tmp31 * _tmp38 - _tmp33)) *
                        std::cosh(_tmp36)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp36) -
               Scalar(9.6622558468725703) * std::sinh(_tmp39);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp162 * (-std::sinh(_tmp169) - std::sinh(_tmp170)) +
               _tmp164 * (-Scalar(1.0) * _tmp167 * std::cosh(_tmp170) -
                          (-Scalar(0.1034955) * _tmp160 * _tmp163 +
                           _tmp168 * (-_tmp159 * _tmp162 - _tmp164 * _tmp167)) *
                              std::cosh(_tmp169));
  _res(2, 0) = _tmp189 * (-Scalar(1.0) * _tmp188 * std::cosh(_tmp184) -
                          (-Scalar(0.1034955) * _tmp187 * _tmp190 +
                           _tmp192 * (-_tmp183 * _tmp191 - _tmp188 * _tmp189)) *
                              std::cosh(_tmp193)) +
               _tmp191 * (-std::sinh(_tmp184) - std::sinh(_tmp193));
  _res(3, 0) = _tmp204 * (-Scalar(1.0) * _tmp213 * std::cosh(_tmp208) -
                          (-Scalar(0.1034955) * _tmp205 * _tmp212 +
                           _tmp206 * (-_tmp203 * _tmp210 - _tmp204 * _tmp213)) *
                              std::cosh(_tmp207)) +
               _tmp210 * (-std::sinh(_tmp207) - std::sinh(_tmp208));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
