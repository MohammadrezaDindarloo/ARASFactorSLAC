...
void forward_kinematic_factor_graph_optimizer(std::vector<Eigen::Matrix<double, 4, 1>> cable_forces_collection , RobotParameters<double> params, 
                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    std::uniform_real_distribution<double> cable_1(-0.05, 0.05);
    std::uniform_real_distribution<double> cable_2(-0.05, 0.05);
    std::uniform_real_distribution<double> cable_3(-0.05, 0.05);
    std::uniform_real_distribution<double> cable_4(-0.05, 0.05);

    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));
    auto Sensor_noiseModel_cost3 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));
    auto Sensor_noiseModel_cost4 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(1));
    ...
    for (size_t i = 0; i < p_platform_collection.size(); i++)
    {
        ...
        graph.add(std::make_shared<FK_factor_graph_cost2>(Symbol('h', i), Symbol('v', i), Symbol('a', i), Symbol('b', i), Symbol('c', i), Symbol('d', i), Symbol('e', i), Symbol('f', i), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), p_platform_collection[i], DeltaRot, quaternion_rot_init_collection[i][0], quaternion_rot_init_collection[i][1], quaternion_rot_init_collection[i][2], quaternion_rot_init_collection[i][3], Sensor_noiseModel_cost2));
        ...
        // graph.add(std::make_shared<FK_factor_graph_cost4>(Symbol('h', i), Symbol('v', i), Symbol('a', i), Symbol('b', i), Symbol('c', i), Symbol('d', i), Symbol('e', i), Symbol('f', i), cable_forces_collection[i], Sensor_noiseModel_cost4));

        double random_noise_cable_1 = cable_1(generator);
        double random_noise_cable_2 = cable_2(generator);
        double random_noise_cable_3 = cable_3(generator);
        double random_noise_cable_4 = cable_4(generator);
        
        double b1_p1_distance = (b_in_w_collection[i][0] - p_in_w_collection[i][0]).norm() + random_noise_cable_1;
        double b2_p2_distance = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + random_noise_cable_2;
        double b3_p3_distance = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + random_noise_cable_3;
        double b4_p4_distance = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + random_noise_cable_4;

        graph.add(std::make_shared<BetweenTwoPoint3FactorOneVariable>(Symbol('p', 1), b_in_w_collection[i][0], b1_p1_distance, Sensor_noiseModel_betweenfactor_one_variable));
        graph.add(std::make_shared<BetweenTwoPoint3FactorOneVariable>(Symbol('p', 2), b_in_w_collection[i][1], b2_p2_distance, Sensor_noiseModel_betweenfactor_one_variable));
        graph.add(std::make_shared<BetweenTwoPoint3FactorOneVariable>(Symbol('p', 3), b_in_w_collection[i][2], b3_p3_distance, Sensor_noiseModel_betweenfactor_one_variable));
        graph.add(std::make_shared<BetweenTwoPoint3FactorOneVariable>(Symbol('p', 4), b_in_w_collection[i][3], b4_p4_distance, Sensor_noiseModel_betweenfactor_one_variable));
    }
    ...
    for (size_t i = 0; i < p_platform_collection.size(); i++)
    {
        initial_estimate.insert(Symbol('h', i), cable_forces_collection[i][0]/sqrt(2));
        initial_estimate.insert(Symbol('v', i), cable_forces_collection[i][0]/sqrt(2));
        initial_estimate.insert(Symbol('a', i), cable_forces_collection[i][1]/sqrt(2));
        initial_estimate.insert(Symbol('b', i), cable_forces_collection[i][1]/sqrt(2));
        initial_estimate.insert(Symbol('c', i), cable_forces_collection[i][2]/sqrt(2));
        initial_estimate.insert(Symbol('d', i), cable_forces_collection[i][2]/sqrt(2));
        initial_estimate.insert(Symbol('e', i), cable_forces_collection[i][3]/sqrt(2));
        initial_estimate.insert(Symbol('f', i), cable_forces_collection[i][3]/sqrt(2)); 
    }
