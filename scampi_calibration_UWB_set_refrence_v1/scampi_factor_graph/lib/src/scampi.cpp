                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    auto Sensor_noiseModel_cost4 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(500)); 
    ...
    for (size_t i = 0; i < p_platform_collection.size()-1; i++)
    {
        ...
        double encoder_data_1 = (b_in_w_collection[i][0] - p_in_w_collection[i][0]).norm() + random_noise_encoder_1 + 10.65;
        double encoder_data_2 = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + random_noise_encoder_2 + 5.52;
        double encoder_data_3 = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + random_noise_encoder_3 + 9.32;
        double encoder_data_4 = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + random_noise_encoder_4 + -8.65;
