// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost1_wrt_fv1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost1WrtFv1(
    const Scalar fh1, const Scalar fv1, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& position_vector, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 589

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (196)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp3 = 2 * _tmp2;
  const Scalar _tmp4 = _tmp1 * _tmp3;
  const Scalar _tmp5 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp6 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp7 = _tmp5 * _tmp6;
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp4 - Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -_tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp13 = _tmp3 * _tmp5;
  const Scalar _tmp14 = _tmp1 * _tmp6;
  const Scalar _tmp15 = _tmp13 + _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = -_tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp9;
  const Scalar _tmp19 = _tmp18 + position_vector(0, 0);
  const Scalar _tmp20 = -2 * std::pow(_tmp2, Scalar(2));
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp20;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp24 = 2 * _tmp1 * _tmp5;
  const Scalar _tmp25 = _tmp2 * _tmp6;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = -_tmp23 + _tmp28;
  const Scalar _tmp30 = _tmp22 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = Scalar(1.0) * _tmp0 /
                        std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp35 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp36 = _tmp22 + _tmp23 + _tmp28;
  const Scalar _tmp37 = _tmp36 + position_vector(1, 0);
  const Scalar _tmp38 = _tmp37 - p_b(1, 0);
  const Scalar _tmp39 = _tmp12 + _tmp16;
  const Scalar _tmp40 = _tmp39 + _tmp9;
  const Scalar _tmp41 = _tmp40 + position_vector(0, 0);
  const Scalar _tmp42 = _tmp41 - p_b(0, 0);
  const Scalar _tmp43 = Scalar(1.0) / (_tmp42);
  const Scalar _tmp44 = _tmp38 * _tmp43;
  const Scalar _tmp45 = _tmp21 + _tmp23;
  const Scalar _tmp46 = _tmp28 + _tmp45;
  const Scalar _tmp47 = _tmp46 + position_vector(1, 0);
  const Scalar _tmp48 = _tmp47 - p_c(1, 0);
  const Scalar _tmp49 = _tmp39 + _tmp8;
  const Scalar _tmp50 = _tmp49 + position_vector(0, 0);
  const Scalar _tmp51 = _tmp50 - p_c(0, 0);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp48, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp51 * _tmp52;
  const Scalar _tmp54 = _tmp48 * _tmp52;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp44 * _tmp53 - _tmp54);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = Scalar(1.0) * _tmp40;
  const Scalar _tmp58 = Scalar(1.0) * _tmp36;
  const Scalar _tmp59 = -_tmp58;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp46 + _tmp59);
  const Scalar _tmp61 = -_tmp49 + _tmp57;
  const Scalar _tmp62 = _tmp60 * _tmp61;
  const Scalar _tmp63 = _tmp57 + _tmp58 * _tmp62;
  const Scalar _tmp64 = 0;
  const Scalar _tmp65 = _tmp21 + _tmp29;
  const Scalar _tmp66 = _tmp65 + position_vector(1, 0);
  const Scalar _tmp67 = _tmp66 - p_d(1, 0);
  const Scalar _tmp68 = _tmp17 + _tmp8;
  const Scalar _tmp69 = _tmp68 + position_vector(0, 0);
  const Scalar _tmp70 = _tmp69 - p_d(0, 0);
  const Scalar _tmp71 = std::pow(Scalar(std::pow(_tmp67, Scalar(2)) + std::pow(_tmp70, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp72 = _tmp70 * _tmp71;
  const Scalar _tmp73 = _tmp67 * _tmp71;
  const Scalar _tmp74 = _tmp44 * _tmp72 - _tmp73;
  const Scalar _tmp75 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp76 = -_tmp75;
  const Scalar _tmp77 = -Scalar(0.010999999999999999) * _tmp10 -
                        Scalar(0.010999999999999999) * _tmp20 + Scalar(-0.010999999999999999);
  const Scalar _tmp78 = Scalar(0.20999999999999999) * _tmp13 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp79 = _tmp77 + _tmp78;
  const Scalar _tmp80 = _tmp76 + _tmp79;
  const Scalar _tmp81 = _tmp72 * _tmp80;
  const Scalar _tmp82 = _tmp77 - _tmp78;
  const Scalar _tmp83 = _tmp75 + _tmp82;
  const Scalar _tmp84 = _tmp53 * _tmp80;
  const Scalar _tmp85 = _tmp75 + _tmp79;
  const Scalar _tmp86 = -_tmp44 * _tmp84 + _tmp54 * _tmp85;
  const Scalar _tmp87 = _tmp55 * _tmp74;
  const Scalar _tmp88 = -_tmp44 * _tmp81 + _tmp73 * _tmp83 - _tmp86 * _tmp87;
  const Scalar _tmp89 = -_tmp53 * _tmp85 + _tmp84;
  const Scalar _tmp90 = -_tmp62 * _tmp88 - _tmp72 * _tmp83 + _tmp81 - _tmp87 * _tmp89;
  const Scalar _tmp91 = Scalar(1.0) / (_tmp90);
  const Scalar _tmp92 = _tmp74 * _tmp91;
  const Scalar _tmp93 = _tmp64 * _tmp92;
  const Scalar _tmp94 = _tmp64 * _tmp91;
  const Scalar _tmp95 =
      std::sqrt(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp42, Scalar(2))));
  const Scalar _tmp96 = _tmp43 * _tmp95;
  const Scalar _tmp97 = _tmp96 * (-_tmp56 * _tmp93 + _tmp72 * _tmp94);
  const Scalar _tmp98 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp99 = _tmp96 * (-_tmp36 * _tmp42 * _tmp98 + _tmp38 * _tmp40 * _tmp98);
  const Scalar _tmp100 = _tmp46 * _tmp53 - _tmp49 * _tmp54 + _tmp53 * _tmp99;
  const Scalar _tmp101 = _tmp44 * _tmp55;
  const Scalar _tmp102 = _tmp101 * _tmp86 + _tmp44 * _tmp80;
  const Scalar _tmp103 = _tmp101 * _tmp89 - _tmp102 * _tmp62 - _tmp80;
  const Scalar _tmp104 = -_tmp100 * _tmp87 + _tmp65 * _tmp72 - _tmp68 * _tmp73 + _tmp72 * _tmp99;
  const Scalar _tmp105 = _tmp104 * _tmp91;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp107 = _tmp106 * _tmp90;
  const Scalar _tmp108 = _tmp107 * (_tmp100 * _tmp101 - _tmp103 * _tmp105 - _tmp99);
  const Scalar _tmp109 = _tmp103 + _tmp108;
  const Scalar _tmp110 = _tmp72 * _tmp91;
  const Scalar _tmp111 = -_tmp109 * _tmp92 - _tmp44;
  const Scalar _tmp112 = _tmp31 - p_a(1, 0);
  const Scalar _tmp113 = _tmp19 - p_a(0, 0);
  const Scalar _tmp114 =
      std::pow(Scalar(std::pow(_tmp112, Scalar(2)) + std::pow(_tmp113, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp115 = _tmp113 * _tmp114;
  const Scalar _tmp116 = _tmp115 * fh1;
  const Scalar _tmp117 = Scalar(1.0) * _tmp60;
  const Scalar _tmp118 = Scalar(1.0) * _tmp55;
  const Scalar _tmp119 = _tmp117 * _tmp55 * _tmp61 * _tmp86 - _tmp118 * _tmp89;
  const Scalar _tmp120 = _tmp107 * (-_tmp100 * _tmp118 - _tmp105 * _tmp119);
  const Scalar _tmp121 = _tmp119 + _tmp120;
  const Scalar _tmp122 = -_tmp121 * _tmp92 + Scalar(1.0);
  const Scalar _tmp123 = _tmp112 * _tmp114;
  const Scalar _tmp124 = _tmp123 * fh1;
  const Scalar _tmp125 = Scalar(1.0) * _tmp106;
  const Scalar _tmp126 = fh1 * (-_tmp115 * _tmp30 + _tmp123 * _tmp18);
  const Scalar _tmp127 = -_tmp116 * _tmp96 * (_tmp109 * _tmp110 + _tmp111 * _tmp56 + Scalar(1.0)) -
                         _tmp124 * _tmp96 * (_tmp110 * _tmp121 + _tmp122 * _tmp56) -
                         _tmp126 * _tmp96 * (-_tmp125 * _tmp53 * _tmp87 + _tmp125 * _tmp72) -
                         _tmp35 * _tmp97;
  const Scalar _tmp128 = Scalar(1.0) / (_tmp127);
  const Scalar _tmp129 = Scalar(0.1034955) * _tmp128;
  const Scalar _tmp130 = fh1 * (_tmp76 + _tmp82);
  const Scalar _tmp131 = -_tmp123 * _tmp130 - Scalar(3.29616) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp132 = _tmp59 + _tmp65;
  const Scalar _tmp133 = _tmp132 * _tmp62;
  const Scalar _tmp134 = Scalar(1.0) / (-_tmp133 + _tmp57 - _tmp68);
  const Scalar _tmp135 = Scalar(1.0) * _tmp134;
  const Scalar _tmp136 = _tmp133 * _tmp135 + Scalar(1.0);
  const Scalar _tmp137 = _tmp135 * _tmp62;
  const Scalar _tmp138 = -Scalar(1.0) * _tmp117 * _tmp136 + Scalar(1.0) * _tmp137;
  const Scalar _tmp139 = _tmp115 * _tmp130 + Scalar(3.29616) * _tmp15 + _tmp18 * fv1;
  const Scalar _tmp140 = _tmp132 * _tmp60;
  const Scalar _tmp141 = Scalar(1.0) * _tmp135 * _tmp140 - Scalar(1.0) * _tmp135;
  const Scalar _tmp142 = _tmp88 * _tmp91;
  const Scalar _tmp143 = _tmp132 * _tmp134;
  const Scalar _tmp144 = -_tmp118 * _tmp86 + _tmp120 * _tmp143 - _tmp121 * _tmp142;
  const Scalar _tmp145 = _tmp107 * _tmp135;
  const Scalar _tmp146 = -_tmp125 * _tmp88 + _tmp132 * _tmp145;
  const Scalar _tmp147 = _tmp134 * _tmp63;
  const Scalar _tmp148 = _tmp60 * (-_tmp132 * _tmp147 - _tmp142 * _tmp64 + _tmp59);
  const Scalar _tmp149 = -Scalar(1.0) * _tmp147 - Scalar(1.0) * _tmp148 + Scalar(1.0);
  const Scalar _tmp150 = _tmp102 + _tmp108 * _tmp143 - _tmp109 * _tmp142;
  const Scalar _tmp151 = Scalar(1.0) * _tmp116 * (_tmp108 * _tmp135 - _tmp117 * _tmp150) +
                         Scalar(1.0) * _tmp124 * (-_tmp117 * _tmp144 + _tmp120 * _tmp135) +
                         Scalar(1.0) * _tmp126 * (-_tmp117 * _tmp146 + _tmp145) +
                         _tmp131 * _tmp138 + _tmp139 * _tmp141 + _tmp149 * _tmp35;
  const Scalar _tmp152 = std::asinh(_tmp128 * _tmp151);
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp127;
  const Scalar _tmp154 =
      -_tmp152 * _tmp153 - std::sqrt(Scalar(std::pow(Scalar(-_tmp37 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp41 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp155 = _tmp129 * _tmp154;
  const Scalar _tmp156 = Scalar(1.0) * _tmp152;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp97;
  const Scalar _tmp158 = std::pow(_tmp127, Scalar(-2));
  const Scalar _tmp159 = _tmp158 * _tmp97;
  const Scalar _tmp160 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp161 = _tmp27 + _tmp45;
  const Scalar _tmp162 =
      (_tmp128 * (_tmp138 * _tmp161 + _tmp141 * _tmp18 - _tmp149) - _tmp151 * _tmp159) /
      std::sqrt(Scalar(std::pow(_tmp151, Scalar(2)) * _tmp158 + 1));
  const Scalar _tmp163 = _tmp136 * _tmp60;
  const Scalar _tmp164 = _tmp135 * _tmp139;
  const Scalar _tmp165 = _tmp116 * _tmp150 * _tmp60 + _tmp124 * _tmp144 * _tmp60 +
                         _tmp126 * _tmp146 * _tmp60 + _tmp131 * _tmp163 - _tmp140 * _tmp164 +
                         _tmp148 * _tmp35;
  const Scalar _tmp166 = _tmp55 * _tmp93;
  const Scalar _tmp167 = _tmp125 * _tmp126;
  const Scalar _tmp168 =
      _tmp111 * _tmp116 * _tmp55 + _tmp122 * _tmp124 * _tmp55 - _tmp166 * _tmp35 - _tmp167 * _tmp87;
  const Scalar _tmp169 = Scalar(1.0) / (_tmp168);
  const Scalar _tmp170 = std::asinh(_tmp165 * _tmp169);
  const Scalar _tmp171 = Scalar(1.0) * _tmp170;
  const Scalar _tmp172 = std::pow(_tmp168, Scalar(-2));
  const Scalar _tmp173 = _tmp166 * _tmp172;
  const Scalar _tmp174 = _tmp135 * _tmp18;
  const Scalar _tmp175 =
      (-_tmp165 * _tmp173 + _tmp169 * (-_tmp140 * _tmp174 - _tmp148 + _tmp161 * _tmp163)) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp172 + 1));
  const Scalar _tmp176 = Scalar(9.6622558468725703) * _tmp168;
  const Scalar _tmp177 =
      -_tmp170 * _tmp176 - std::sqrt(Scalar(std::pow(Scalar(-_tmp47 + p_c(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp50 + p_c(0, 0)), Scalar(2))));
  const Scalar _tmp178 = Scalar(0.1034955) * _tmp169;
  const Scalar _tmp179 = _tmp177 * _tmp178;
  const Scalar _tmp180 = Scalar(0.1034955) * _tmp173;
  const Scalar _tmp181 = Scalar(9.6622558468725703) * _tmp166;
  const Scalar _tmp182 = -_tmp108 * _tmp116 * _tmp134 - _tmp120 * _tmp124 * _tmp134 -
                         _tmp126 * _tmp145 - _tmp131 * _tmp137 + _tmp147 * _tmp35 + _tmp164;
  const Scalar _tmp183 =
      _tmp109 * _tmp116 * _tmp91 + _tmp121 * _tmp124 * _tmp91 + _tmp167 + _tmp35 * _tmp94;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp182 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp189 =
      -_tmp185 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp66 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp69 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp190 = _tmp187 * _tmp189;
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp94;
  const Scalar _tmp192 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp193 = _tmp192 * _tmp94;
  const Scalar _tmp194 = (_tmp182 * _tmp193 + _tmp184 * (-_tmp137 * _tmp161 - _tmp147 + _tmp174)) /
                         std::sqrt(Scalar(std::pow(_tmp182, Scalar(2)) * _tmp192 + 1));
  const Scalar _tmp195 = Scalar(0.1034955) * _tmp193;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp34 * std::sinh(Scalar(1.0) * _tmp32) +
       _tmp34 * std::sinh(Scalar(0.1034955) * _tmp0 *
                          (-_tmp32 * _tmp33 -
                           std::sqrt(Scalar(std::pow(Scalar(-_tmp19 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp31 + p_a(1, 0)), Scalar(2)))))));
  _res(1, 0) =
      -_tmp153 * (-_tmp160 * p_b(2, 0) + Scalar(1.0) * _tmp162 * std::sinh(_tmp156) -
                  (_tmp129 * (-_tmp152 * _tmp157 - _tmp153 * _tmp162) - _tmp154 * _tmp160) *
                      std::sinh(_tmp155)) -
      _tmp157 * (_tmp129 * p_b(2, 0) - std::cosh(_tmp155) + std::cosh(_tmp156));
  _res(2, 0) =
      -_tmp176 * (Scalar(1.0) * _tmp175 * std::sinh(_tmp171) - _tmp180 * p_c(2, 0) -
                  (-_tmp177 * _tmp180 + _tmp178 * (-_tmp170 * _tmp181 - _tmp175 * _tmp176)) *
                      std::sinh(_tmp179)) -
      _tmp181 * (_tmp178 * p_c(2, 0) + std::cosh(_tmp171) - std::cosh(_tmp179));
  _res(3, 0) = -_tmp188 * (Scalar(1.0) * _tmp194 * std::sinh(_tmp186) + _tmp195 * p_d(2, 0) -
                           (_tmp187 * (_tmp185 * _tmp191 - _tmp188 * _tmp194) + _tmp189 * _tmp195) *
                               std::sinh(_tmp190)) +
               _tmp191 * (_tmp187 * p_d(2, 0) + std::cosh(_tmp186) - std::cosh(_tmp190));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
