// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost2_wrt_fh1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     ofset0: Scalar
 *     ofset1: Scalar
 *     ofset2: Scalar
 *     ofset3: Scalar
 *     distance_measure: Matrix41
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     position_vector: Matrix31
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost2WrtFh1(
    const Scalar fh1, const Scalar fv1, const Scalar ofset0, const Scalar ofset1,
    const Scalar ofset2, const Scalar ofset3, const Eigen::Matrix<Scalar, 4, 1>& distance_measure,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const sym::Rot3<Scalar>& DeltaRot, const Eigen::Matrix<Scalar, 3, 1>& position_vector,
    const Scalar rot_init_x, const Scalar rot_init_y, const Scalar rot_init_z,
    const Scalar rot_init_w, const Scalar epsilon) {
  // Total ops: 616

  // Unused inputs
  (void)ofset0;
  (void)ofset1;
  (void)ofset2;
  (void)ofset3;
  (void)distance_measure;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (211)
  const Scalar _tmp0 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp3 * _tmp8;
  const Scalar _tmp10 = -_DeltaRot[0] * rot_init_x - _DeltaRot[1] * rot_init_y -
                        _DeltaRot[2] * rot_init_z + _DeltaRot[3] * rot_init_w;
  const Scalar _tmp11 = 2 * _tmp1;
  const Scalar _tmp12 = _tmp10 * _tmp11;
  const Scalar _tmp13 = _tmp12 + _tmp9;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = _tmp11 * _tmp7;
  const Scalar _tmp16 = 2 * _tmp10 * _tmp3;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp14 - _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp6;
  const Scalar _tmp20 = _tmp19 + position_vector(0, 0);
  const Scalar _tmp21 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = _tmp11 * _tmp3;
  const Scalar _tmp25 = _tmp10 * _tmp8;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = -Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = _tmp23 + _tmp29;
  const Scalar _tmp31 = _tmp30 + position_vector(1, 0);
  const Scalar _tmp32 = Scalar(1.0) / (fh1);
  const Scalar _tmp33 = _tmp32 * fv1;
  const Scalar _tmp34 = std::asinh(_tmp33);
  const Scalar _tmp35 = Scalar(9.6622558468725703) * _tmp34;
  const Scalar _tmp36 =
      -_tmp35 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_a(0, 0)), Scalar(2)) +
                                       std::pow(Scalar(-_tmp31 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp0 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(0.1034955) * _tmp32;
  const Scalar _tmp39 = _tmp36 * _tmp38;
  const Scalar _tmp40 = Scalar(1.0) * _tmp34;
  const Scalar _tmp41 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp42 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp43 = -_tmp42;
  const Scalar _tmp44 = -Scalar(0.010999999999999999) * _tmp2 -
                        Scalar(0.010999999999999999) * _tmp21 + Scalar(-0.010999999999999999);
  const Scalar _tmp45 = -Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp46 = _tmp44 + _tmp45;
  const Scalar _tmp47 = _tmp43 + _tmp46;
  const Scalar _tmp48 = _tmp27 + _tmp28;
  const Scalar _tmp49 = _tmp23 + _tmp48;
  const Scalar _tmp50 = _tmp49 + position_vector(1, 0);
  const Scalar _tmp51 = _tmp50 - p_b(1, 0);
  const Scalar _tmp52 = _tmp18 + _tmp5;
  const Scalar _tmp53 = _tmp52 + position_vector(0, 0);
  const Scalar _tmp54 = _tmp53 - p_b(0, 0);
  const Scalar _tmp55 = Scalar(1.0) / (_tmp54);
  const Scalar _tmp56 = _tmp51 * _tmp55;
  const Scalar _tmp57 = _tmp47 * _tmp56;
  const Scalar _tmp58 = _tmp22 + _tmp29;
  const Scalar _tmp59 = _tmp58 + position_vector(1, 0);
  const Scalar _tmp60 = _tmp59 - p_d(1, 0);
  const Scalar _tmp61 = _tmp14 + _tmp17;
  const Scalar _tmp62 = _tmp6 + _tmp61;
  const Scalar _tmp63 = _tmp62 + position_vector(0, 0);
  const Scalar _tmp64 = _tmp63 - p_d(0, 0);
  const Scalar _tmp65 = std::pow(Scalar(std::pow(_tmp60, Scalar(2)) + std::pow(_tmp64, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp66 = _tmp64 * _tmp65;
  const Scalar _tmp67 = _tmp44 - _tmp45;
  const Scalar _tmp68 = _tmp42 + _tmp67;
  const Scalar _tmp69 = _tmp60 * _tmp65;
  const Scalar _tmp70 = _tmp22 + _tmp48;
  const Scalar _tmp71 = _tmp70 + position_vector(1, 0);
  const Scalar _tmp72 = _tmp71 - p_c(1, 0);
  const Scalar _tmp73 = _tmp5 + _tmp61;
  const Scalar _tmp74 = _tmp73 + position_vector(0, 0);
  const Scalar _tmp75 = _tmp74 - p_c(0, 0);
  const Scalar _tmp76 = std::pow(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp75, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = _tmp42 + _tmp46;
  const Scalar _tmp79 = _tmp72 * _tmp76;
  const Scalar _tmp80 = -_tmp57 * _tmp77 + _tmp78 * _tmp79;
  const Scalar _tmp81 = _tmp56 * _tmp66 - _tmp69;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp56 * _tmp77 - _tmp79);
  const Scalar _tmp83 = _tmp81 * _tmp82;
  const Scalar _tmp84 = -_tmp57 * _tmp66 + _tmp68 * _tmp69 - _tmp80 * _tmp83;
  const Scalar _tmp85 = Scalar(1.0) * _tmp49;
  const Scalar _tmp86 = -_tmp85;
  const Scalar _tmp87 = Scalar(1.0) / (_tmp70 + _tmp86);
  const Scalar _tmp88 = Scalar(1.0) * _tmp52;
  const Scalar _tmp89 = _tmp87 * (-_tmp73 + _tmp88);
  const Scalar _tmp90 = _tmp47 * _tmp77 - _tmp77 * _tmp78;
  const Scalar _tmp91 = _tmp47 * _tmp66 - _tmp66 * _tmp68 - _tmp83 * _tmp90 - _tmp84 * _tmp89;
  const Scalar _tmp92 = Scalar(1.0) / (_tmp91);
  const Scalar _tmp93 = _tmp85 * _tmp89 + _tmp88;
  const Scalar _tmp94 = 0;
  const Scalar _tmp95 = _tmp77 * _tmp83;
  const Scalar _tmp96 =
      std::sqrt(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp54, Scalar(2))));
  const Scalar _tmp97 = _tmp55 * _tmp96;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp96);
  const Scalar _tmp99 = _tmp97 * (-_tmp49 * _tmp54 * _tmp98 + _tmp51 * _tmp52 * _tmp98);
  const Scalar _tmp100 = _tmp70 * _tmp77 - _tmp73 * _tmp79 + _tmp77 * _tmp99;
  const Scalar _tmp101 = _tmp56 * _tmp82;
  const Scalar _tmp102 = _tmp101 * _tmp80 + _tmp57;
  const Scalar _tmp103 = _tmp101 * _tmp90 - _tmp102 * _tmp89 - _tmp47;
  const Scalar _tmp104 = -_tmp100 * _tmp83 + _tmp58 * _tmp66 - _tmp62 * _tmp69 + _tmp66 * _tmp99;
  const Scalar _tmp105 = _tmp104 * _tmp92;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp104);
  const Scalar _tmp107 = _tmp106 * _tmp91;
  const Scalar _tmp108 = _tmp107 * (_tmp100 * _tmp101 - _tmp103 * _tmp105 - _tmp99);
  const Scalar _tmp109 = _tmp92 * (_tmp103 + _tmp108);
  const Scalar _tmp110 = -_tmp109 * _tmp81 - _tmp56;
  const Scalar _tmp111 = _tmp77 * _tmp82;
  const Scalar _tmp112 = _tmp20 - p_a(0, 0);
  const Scalar _tmp113 = _tmp31 - p_a(1, 0);
  const Scalar _tmp114 =
      std::pow(Scalar(std::pow(_tmp112, Scalar(2)) + std::pow(_tmp113, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp115 = _tmp112 * _tmp114;
  const Scalar _tmp116 = _tmp115 * _tmp97 * (_tmp109 * _tmp66 + _tmp110 * _tmp111 + Scalar(1.0));
  const Scalar _tmp117 = Scalar(1.0) * _tmp82;
  const Scalar _tmp118 = _tmp117 * _tmp80;
  const Scalar _tmp119 = -_tmp117 * _tmp90 + _tmp118 * _tmp89;
  const Scalar _tmp120 = _tmp107 * (-_tmp100 * _tmp117 - _tmp105 * _tmp119);
  const Scalar _tmp121 = _tmp92 * (_tmp119 + _tmp120);
  const Scalar _tmp122 = -_tmp121 * _tmp81 + Scalar(1.0);
  const Scalar _tmp123 = _tmp113 * _tmp114;
  const Scalar _tmp124 = _tmp123 * _tmp97 * (_tmp111 * _tmp122 + _tmp121 * _tmp66);
  const Scalar _tmp125 = -_tmp115 * _tmp30 + _tmp123 * _tmp19;
  const Scalar _tmp126 = Scalar(1.0) * _tmp106;
  const Scalar _tmp127 = _tmp125 * _tmp97 * (_tmp126 * _tmp66 - _tmp126 * _tmp95);
  const Scalar _tmp128 = -_tmp116 * fh1 - _tmp124 * fh1 - _tmp127 * fh1 -
                         _tmp41 * _tmp97 * (_tmp66 * _tmp94 - _tmp94 * _tmp95);
  const Scalar _tmp129 = Scalar(1.0) / (_tmp128);
  const Scalar _tmp130 = _tmp43 + _tmp67;
  const Scalar _tmp131 = _tmp130 * fh1;
  const Scalar _tmp132 = -_tmp123 * _tmp131 - Scalar(3.29616) * _tmp26 - _tmp30 * fv1;
  const Scalar _tmp133 = _tmp58 + _tmp86;
  const Scalar _tmp134 = _tmp133 * _tmp89;
  const Scalar _tmp135 = Scalar(1.0) / (-_tmp134 - _tmp62 + _tmp88);
  const Scalar _tmp136 = Scalar(1.0) * _tmp135;
  const Scalar _tmp137 = _tmp87 * (_tmp134 * _tmp136 + Scalar(1.0));
  const Scalar _tmp138 = _tmp136 * _tmp89;
  const Scalar _tmp139 = -Scalar(1.0) * _tmp137 + Scalar(1.0) * _tmp138;
  const Scalar _tmp140 = _tmp115 * _tmp131 + Scalar(3.29616) * _tmp13 + _tmp19 * fv1;
  const Scalar _tmp141 = _tmp133 * _tmp87;
  const Scalar _tmp142 = Scalar(1.0) * _tmp136 * _tmp141 - Scalar(1.0) * _tmp136;
  const Scalar _tmp143 = _tmp133 * _tmp135;
  const Scalar _tmp144 = -_tmp118 + _tmp120 * _tmp143 - _tmp121 * _tmp84;
  const Scalar _tmp145 = Scalar(1.0) * _tmp87;
  const Scalar _tmp146 = Scalar(1.0) * _tmp123 * (_tmp120 * _tmp136 - _tmp144 * _tmp145);
  const Scalar _tmp147 = _tmp107 * _tmp136;
  const Scalar _tmp148 = _tmp87 * (-_tmp126 * _tmp84 + _tmp133 * _tmp147);
  const Scalar _tmp149 = Scalar(1.0) * _tmp125 * (_tmp147 - Scalar(1.0) * _tmp148);
  const Scalar _tmp150 = _tmp135 * _tmp93;
  const Scalar _tmp151 = _tmp87 * (-_tmp133 * _tmp150 - _tmp84 * _tmp94 + _tmp86);
  const Scalar _tmp152 = _tmp102 + _tmp108 * _tmp143 - _tmp109 * _tmp84;
  const Scalar _tmp153 = Scalar(1.0) * _tmp115 * (_tmp108 * _tmp136 - _tmp145 * _tmp152);
  const Scalar _tmp154 =
      _tmp132 * _tmp139 + _tmp140 * _tmp142 + _tmp146 * fh1 + _tmp149 * fh1 + _tmp153 * fh1 +
      Scalar(1.0) * _tmp41 * (-_tmp136 * _tmp93 - Scalar(1.0) * _tmp151 + Scalar(1.0));
  const Scalar _tmp155 = std::asinh(_tmp129 * _tmp154);
  const Scalar _tmp156 = Scalar(1.0) * _tmp155;
  const Scalar _tmp157 = std::pow(_tmp128, Scalar(-2));
  const Scalar _tmp158 = _tmp123 * _tmp130;
  const Scalar _tmp159 = _tmp115 * _tmp130;
  const Scalar _tmp160 = -_tmp116 - _tmp124 - _tmp127;
  const Scalar _tmp161 = _tmp157 * _tmp160;
  const Scalar _tmp162 =
      (_tmp129 * (-_tmp139 * _tmp158 + _tmp142 * _tmp159 + _tmp146 + _tmp149 + _tmp153) -
       _tmp154 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp154, Scalar(2)) * _tmp157 + 1));
  const Scalar _tmp163 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp164 =
      -_tmp155 * _tmp163 - std::sqrt(Scalar(std::pow(Scalar(-_tmp50 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp53 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp165 = Scalar(0.1034955) * _tmp129;
  const Scalar _tmp166 = _tmp164 * _tmp165;
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp160;
  const Scalar _tmp168 = _tmp123 * _tmp144 * _tmp87;
  const Scalar _tmp169 = _tmp115 * _tmp87;
  const Scalar _tmp170 = _tmp152 * _tmp169;
  const Scalar _tmp171 = _tmp125 * _tmp148;
  const Scalar _tmp172 = _tmp136 * _tmp140;
  const Scalar _tmp173 = _tmp132 * _tmp137 - _tmp141 * _tmp172 + _tmp151 * _tmp41 + _tmp168 * fh1 +
                         _tmp170 * fh1 + _tmp171 * fh1;
  const Scalar _tmp174 = _tmp41 * _tmp94;
  const Scalar _tmp175 = _tmp110 * _tmp115 * _tmp82;
  const Scalar _tmp176 = _tmp125 * _tmp126;
  const Scalar _tmp177 = _tmp176 * fh1;
  const Scalar _tmp178 = _tmp122 * _tmp123 * _tmp82;
  const Scalar _tmp179 = -_tmp174 * _tmp83 + _tmp175 * fh1 - _tmp177 * _tmp83 + _tmp178 * fh1;
  const Scalar _tmp180 = Scalar(1.0) / (_tmp179);
  const Scalar _tmp181 = std::asinh(_tmp173 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = std::pow(_tmp179, Scalar(-2));
  const Scalar _tmp184 = _tmp175 - _tmp176 * _tmp83 + _tmp178;
  const Scalar _tmp185 = _tmp183 * _tmp184;
  const Scalar _tmp186 =
      (-_tmp173 * _tmp185 + _tmp180 * (-_tmp130 * _tmp133 * _tmp136 * _tmp169 - _tmp137 * _tmp158 +
                                       _tmp168 + _tmp170 + _tmp171)) /
      std::sqrt(Scalar(std::pow(_tmp173, Scalar(2)) * _tmp183 + 1));
  const Scalar _tmp187 = Scalar(9.6622558468725703) * _tmp179;
  const Scalar _tmp188 =
      -_tmp181 * _tmp187 - std::sqrt(Scalar(std::pow(Scalar(-_tmp71 + p_c(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp74 + p_c(0, 0)), Scalar(2))));
  const Scalar _tmp189 = Scalar(0.1034955) * _tmp180;
  const Scalar _tmp190 = _tmp188 * _tmp189;
  const Scalar _tmp191 = Scalar(9.6622558468725703) * _tmp184;
  const Scalar _tmp192 = _tmp120 * _tmp123 * _tmp135;
  const Scalar _tmp193 = _tmp108 * _tmp115 * _tmp135;
  const Scalar _tmp194 = _tmp125 * _tmp147;
  const Scalar _tmp195 = -_tmp132 * _tmp138 + _tmp150 * _tmp41 + _tmp172 - _tmp192 * fh1 -
                         _tmp193 * fh1 - _tmp194 * fh1;
  const Scalar _tmp196 = _tmp109 * _tmp115;
  const Scalar _tmp197 = _tmp121 * _tmp123;
  const Scalar _tmp198 = _tmp174 + _tmp177 + _tmp196 * fh1 + _tmp197 * fh1;
  const Scalar _tmp199 = Scalar(1.0) / (_tmp198);
  const Scalar _tmp200 = std::asinh(_tmp195 * _tmp199);
  const Scalar _tmp201 = Scalar(1.0) * _tmp200;
  const Scalar _tmp202 = Scalar(9.6622558468725703) * _tmp200;
  const Scalar _tmp203 =
      -_tmp198 * _tmp202 - std::sqrt(Scalar(std::pow(Scalar(-_tmp59 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp63 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp204 = Scalar(0.1034955) * _tmp199;
  const Scalar _tmp205 = _tmp203 * _tmp204;
  const Scalar _tmp206 = _tmp176 + _tmp196 + _tmp197;
  const Scalar _tmp207 = Scalar(9.6622558468725703) * _tmp198;
  const Scalar _tmp208 = std::pow(_tmp198, Scalar(-2));
  const Scalar _tmp209 = _tmp206 * _tmp208;
  const Scalar _tmp210 = (-_tmp195 * _tmp209 + _tmp199 * (_tmp136 * _tmp159 + _tmp138 * _tmp158 -
                                                          _tmp192 - _tmp193 - _tmp194)) /
                         std::sqrt(Scalar(std::pow(_tmp195, Scalar(2)) * _tmp208 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp0 * _tmp37 * fv1 * std::cosh(_tmp40) -
                    (-Scalar(0.1034955) * _tmp0 * _tmp36 +
                     _tmp38 * (Scalar(9.6622558468725703) * _tmp33 * _tmp37 - _tmp35)) *
                        std::cosh(_tmp39)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp39) -
               Scalar(9.6622558468725703) * std::sinh(_tmp40);
  _res(1, 0) = _tmp163 * (-Scalar(1.0) * _tmp162 * std::cosh(_tmp156) -
                          (-Scalar(0.1034955) * _tmp161 * _tmp164 +
                           _tmp165 * (-_tmp155 * _tmp167 - _tmp162 * _tmp163)) *
                              std::cosh(_tmp166)) +
               _tmp167 * (-std::sinh(_tmp156) - std::sinh(_tmp166));
  _res(2, 0) = _tmp187 * (-Scalar(1.0) * _tmp186 * std::cosh(_tmp182) -
                          (-Scalar(0.1034955) * _tmp185 * _tmp188 +
                           _tmp189 * (-_tmp181 * _tmp191 - _tmp186 * _tmp187)) *
                              std::cosh(_tmp190)) +
               _tmp191 * (-std::sinh(_tmp182) - std::sinh(_tmp190));
  _res(3, 0) = Scalar(9.6622558468725703) * _tmp206 * (-std::sinh(_tmp201) - std::sinh(_tmp205)) +
               _tmp207 * (-Scalar(1.0) * _tmp210 * std::cosh(_tmp201) -
                          (-Scalar(0.1034955) * _tmp203 * _tmp209 +
                           _tmp204 * (-_tmp202 * _tmp206 - _tmp207 * _tmp210)) *
                              std::cosh(_tmp205));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
