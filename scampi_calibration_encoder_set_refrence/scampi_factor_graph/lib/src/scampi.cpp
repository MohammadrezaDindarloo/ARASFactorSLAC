                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    matrix.diagonal() = 0.001 * Eigen::VectorXd::Ones(rows);

    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(500));
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(1000));
    ...
    for (size_t i = 0; i < p_platform_collection.size()-1; i++)
    {
        ...
        // double b1_p1_distance = (b_in_w_collection[i][0] - p_in_w_collection[i][0]).norm() + random_noise_encoder_1 + 10.65;
        // double b2_p2_distance = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + random_noise_encoder_2 + 5.52;
        // double b3_p3_distance = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + random_noise_encoder_3 + 9.32;
        // double b4_p4_distance = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + random_noise_encoder_4 + -8.65;

        double b1_p1_distance = (b_in_w_collection[i][0] - p_in_w_collection[i][0]).norm() + 10.65;
        double b2_p2_distance = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + 5.55;
        double b3_p3_distance = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + 9.35;
        double b4_p4_distance = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + -8.65;
        ...
        graph.add(std::make_shared<FK_factor_graph_cost2>(Symbol('h', i), Symbol('v', i), Symbol('o', 0), Symbol('o', 1), Symbol('o', 2), Symbol('o', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('X', i), distance_measure, DeltaRot, Sensor_noiseModel_cost2));
        ...
        graph.add(std::make_shared<FK_factor_graph_cost4>(Symbol('o', 0), Symbol('o', 1), Symbol('o', 2), Symbol('o', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('h', i), Symbol('v', i), Symbol('X', i), distance_measure, DeltaRot, Sensor_noiseModel_cost4));
