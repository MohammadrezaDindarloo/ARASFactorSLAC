// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl0
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl0(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 577

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (192)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp4 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp7 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -_tmp8;
  const Scalar _tmp10 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                        _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp11 = 2 * _tmp10 * _tmp4;
  const Scalar _tmp12 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp13 = _tmp12 * _tmp6;
  const Scalar _tmp14 = _tmp11 + _tmp13;
  const Scalar _tmp15 = -Scalar(0.010999999999999999) * _tmp14;
  const Scalar _tmp16 = 2 * _tmp6;
  const Scalar _tmp17 = _tmp10 * _tmp16;
  const Scalar _tmp18 = _tmp12 * _tmp4;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp15 - _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp9;
  const Scalar _tmp22 = _tmp21 + p_init0;
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp24 = -_tmp23;
  const Scalar _tmp25 = -2 * std::pow(_tmp10, Scalar(2));
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp25 + Scalar(0.20999999999999999) * _tmp5 +
                        Scalar(0.20999999999999999);
  const Scalar _tmp27 = _tmp16 * _tmp4;
  const Scalar _tmp28 = _tmp10 * _tmp12;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = -_tmp30;
  const Scalar _tmp32 = -_tmp26 + _tmp31;
  const Scalar _tmp33 = _tmp24 + _tmp32;
  const Scalar _tmp34 = _tmp33 + p_init1;
  const Scalar _tmp35 = Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp36 =
      -Scalar(0.010999999999999999) * _tmp25 - Scalar(0.010999999999999999) * _tmp7;
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp11 - Scalar(0.20999999999999999) * _tmp13;
  const Scalar _tmp38 = _tmp36 + _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = _tmp15 + _tmp19;
  const Scalar _tmp41 = _tmp40 + _tmp8;
  const Scalar _tmp42 = _tmp41 + p_init0;
  const Scalar _tmp43 = _tmp42 - p_c(0, 0);
  const Scalar _tmp44 = _tmp26 + _tmp31;
  const Scalar _tmp45 = _tmp23 + _tmp44;
  const Scalar _tmp46 = _tmp45 + p_init1;
  const Scalar _tmp47 = _tmp46 - p_c(1, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = _tmp43 * _tmp48;
  const Scalar _tmp51 = -_tmp35;
  const Scalar _tmp52 = _tmp38 + _tmp51;
  const Scalar _tmp53 = _tmp23 + _tmp32;
  const Scalar _tmp54 = _tmp53 + p_init1;
  const Scalar _tmp55 = _tmp54 - p_b(1, 0);
  const Scalar _tmp56 = _tmp20 + _tmp8;
  const Scalar _tmp57 = _tmp56 + p_init0;
  const Scalar _tmp58 = _tmp57 - p_b(0, 0);
  const Scalar _tmp59 = Scalar(1.0) / (_tmp58);
  const Scalar _tmp60 = _tmp55 * _tmp59;
  const Scalar _tmp61 = _tmp52 * _tmp60;
  const Scalar _tmp62 = _tmp39 * _tmp49 - _tmp50 * _tmp61;
  const Scalar _tmp63 = Scalar(1.0) / (-_tmp49 + _tmp50 * _tmp60);
  const Scalar _tmp64 = Scalar(1.0) * _tmp63;
  const Scalar _tmp65 = _tmp62 * _tmp64;
  const Scalar _tmp66 =
      std::sqrt(Scalar(std::pow(_tmp55, Scalar(2)) + std::pow(_tmp58, Scalar(2))));
  const Scalar _tmp67 = Scalar(1.0) / (_tmp66);
  const Scalar _tmp68 = _tmp59 * _tmp66;
  const Scalar _tmp69 = _tmp68 * (-_tmp53 * _tmp58 * _tmp67 + _tmp55 * _tmp56 * _tmp67);
  const Scalar _tmp70 = -_tmp41 * _tmp49 + _tmp45 * _tmp50 + _tmp50 * _tmp69;
  const Scalar _tmp71 = -_tmp39 * _tmp50 + _tmp50 * _tmp52;
  const Scalar _tmp72 = Scalar(1.0) * _tmp53;
  const Scalar _tmp73 = -_tmp72;
  const Scalar _tmp74 = Scalar(1.0) / (_tmp45 + _tmp73);
  const Scalar _tmp75 = Scalar(1.0) * _tmp56;
  const Scalar _tmp76 = _tmp74 * (-_tmp41 + _tmp75);
  const Scalar _tmp77 = -_tmp64 * _tmp71 + _tmp65 * _tmp76;
  const Scalar _tmp78 = _tmp36 - _tmp37;
  const Scalar _tmp79 = _tmp35 + _tmp78;
  const Scalar _tmp80 = _tmp24 + _tmp44;
  const Scalar _tmp81 = _tmp80 + p_init1;
  const Scalar _tmp82 = _tmp81 - p_d(1, 0);
  const Scalar _tmp83 = _tmp40 + _tmp9;
  const Scalar _tmp84 = _tmp83 + p_init0;
  const Scalar _tmp85 = _tmp84 - p_d(0, 0);
  const Scalar _tmp86 = std::pow(Scalar(std::pow(_tmp82, Scalar(2)) + std::pow(_tmp85, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp87 = _tmp85 * _tmp86;
  const Scalar _tmp88 = _tmp52 * _tmp87;
  const Scalar _tmp89 = _tmp82 * _tmp86;
  const Scalar _tmp90 = _tmp60 * _tmp87 - _tmp89;
  const Scalar _tmp91 = _tmp63 * _tmp90;
  const Scalar _tmp92 = -_tmp60 * _tmp88 - _tmp62 * _tmp91 + _tmp79 * _tmp89;
  const Scalar _tmp93 = -_tmp71 * _tmp91 - _tmp76 * _tmp92 - _tmp79 * _tmp87 + _tmp88;
  const Scalar _tmp94 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp95 = _tmp69 * _tmp87 - _tmp70 * _tmp91 + _tmp80 * _tmp87 - _tmp83 * _tmp89;
  const Scalar _tmp96 = _tmp94 * _tmp95;
  const Scalar _tmp97 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp98 = _tmp93 * _tmp97;
  const Scalar _tmp99 = _tmp98 * (-_tmp64 * _tmp70 - _tmp77 * _tmp96);
  const Scalar _tmp100 = _tmp77 + _tmp99;
  const Scalar _tmp101 = _tmp92 * _tmp94;
  const Scalar _tmp102 = _tmp73 + _tmp80;
  const Scalar _tmp103 = _tmp102 * _tmp76;
  const Scalar _tmp104 = Scalar(1.0) / (-_tmp103 + _tmp75 - _tmp83);
  const Scalar _tmp105 = _tmp102 * _tmp104;
  const Scalar _tmp106 = -_tmp100 * _tmp101 + _tmp105 * _tmp99 - _tmp65;
  const Scalar _tmp107 = Scalar(1.0) * _tmp74;
  const Scalar _tmp108 = Scalar(1.0) * _tmp104;
  const Scalar _tmp109 = _tmp22 - p_a(0, 0);
  const Scalar _tmp110 = _tmp34 - p_a(1, 0);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp110 * _tmp111;
  const Scalar _tmp113 = _tmp112 * fh1;
  const Scalar _tmp114 = _tmp108 * _tmp98;
  const Scalar _tmp115 = Scalar(1.0) * _tmp97;
  const Scalar _tmp116 = _tmp102 * _tmp114 - _tmp115 * _tmp92;
  const Scalar _tmp117 = _tmp109 * _tmp111;
  const Scalar _tmp118 = fh1 * (_tmp112 * _tmp21 - _tmp117 * _tmp33);
  const Scalar _tmp119 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp120 = _tmp72 * _tmp76 + _tmp75;
  const Scalar _tmp121 = _tmp104 * _tmp120;
  const Scalar _tmp122 = 0;
  const Scalar _tmp123 = _tmp74 * (-_tmp101 * _tmp122 - _tmp102 * _tmp121 + _tmp73);
  const Scalar _tmp124 = -Scalar(1.0) * _tmp121 - Scalar(1.0) * _tmp123 + Scalar(1.0);
  const Scalar _tmp125 = _tmp60 * _tmp63;
  const Scalar _tmp126 = _tmp125 * _tmp62 + _tmp61;
  const Scalar _tmp127 = _tmp125 * _tmp71 - _tmp126 * _tmp76 - _tmp52;
  const Scalar _tmp128 = _tmp98 * (_tmp125 * _tmp70 - _tmp127 * _tmp96 - _tmp69);
  const Scalar _tmp129 = _tmp127 + _tmp128;
  const Scalar _tmp130 = -_tmp101 * _tmp129 + _tmp105 * _tmp128 + _tmp126;
  const Scalar _tmp131 = _tmp117 * fh1;
  const Scalar _tmp132 = fh1 * (_tmp51 + _tmp78);
  const Scalar _tmp133 = _tmp117 * _tmp132 + Scalar(3.29616) * _tmp14 + _tmp21 * fv1;
  const Scalar _tmp134 = _tmp102 * _tmp74;
  const Scalar _tmp135 = Scalar(1.0) * _tmp108 * _tmp134 - Scalar(1.0) * _tmp108;
  const Scalar _tmp136 = -_tmp112 * _tmp132 - Scalar(3.29616) * _tmp29 - _tmp33 * fv1;
  const Scalar _tmp137 = _tmp74 * (_tmp103 * _tmp108 + Scalar(1.0));
  const Scalar _tmp138 = _tmp108 * _tmp76;
  const Scalar _tmp139 = -Scalar(1.0) * _tmp137 + Scalar(1.0) * _tmp138;
  const Scalar _tmp140 = Scalar(1.0) * _tmp113 * (-_tmp106 * _tmp107 + _tmp108 * _tmp99) +
                         Scalar(1.0) * _tmp118 * (-_tmp107 * _tmp116 + _tmp114) +
                         _tmp119 * _tmp124 +
                         Scalar(1.0) * _tmp131 * (-_tmp107 * _tmp130 + _tmp108 * _tmp128) +
                         _tmp133 * _tmp135 + _tmp136 * _tmp139;
  const Scalar _tmp141 = _tmp90 * _tmp94;
  const Scalar _tmp142 = -_tmp100 * _tmp141 + Scalar(1.0);
  const Scalar _tmp143 = _tmp50 * _tmp63;
  const Scalar _tmp144 = _tmp87 * _tmp94;
  const Scalar _tmp145 = _tmp122 * _tmp94;
  const Scalar _tmp146 = _tmp145 * _tmp91;
  const Scalar _tmp147 = _tmp68 * (_tmp145 * _tmp87 - _tmp146 * _tmp50);
  const Scalar _tmp148 = -_tmp129 * _tmp141 - _tmp60;
  const Scalar _tmp149 = _tmp64 * _tmp90 * _tmp97;
  const Scalar _tmp150 = -_tmp113 * _tmp68 * (_tmp100 * _tmp144 + _tmp142 * _tmp143) -
                         _tmp118 * _tmp68 * (_tmp115 * _tmp87 - _tmp149 * _tmp50) -
                         _tmp119 * _tmp147 -
                         _tmp131 * _tmp68 * (_tmp129 * _tmp144 + _tmp143 * _tmp148 + Scalar(1.0));
  const Scalar _tmp151 = Scalar(1.0) / (_tmp150);
  const Scalar _tmp152 = std::asinh(_tmp140 * _tmp151);
  const Scalar _tmp153 = Scalar(9.6622558468725703) * _tmp150;
  const Scalar _tmp154 =
      -_tmp152 * _tmp153 - std::sqrt(Scalar(std::pow(Scalar(-_tmp54 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp57 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp155 = Scalar(0.1034955) * _tmp151;
  const Scalar _tmp156 = _tmp154 * _tmp155;
  const Scalar _tmp157 = std::pow(_tmp150, Scalar(-2));
  const Scalar _tmp158 = _tmp147 * _tmp157;
  const Scalar _tmp159 = _tmp23 + _tmp26 + _tmp30;
  const Scalar _tmp160 =
      (-_tmp140 * _tmp158 + _tmp151 * (-_tmp124 + _tmp135 * _tmp21 + _tmp139 * _tmp159)) /
      std::sqrt(Scalar(std::pow(_tmp140, Scalar(2)) * _tmp157 + 1));
  const Scalar _tmp161 = Scalar(9.6622558468725703) * _tmp147;
  const Scalar _tmp162 = Scalar(1.0) * _tmp152;
  const Scalar _tmp163 = _tmp119 * _tmp145;
  const Scalar _tmp164 = _tmp113 * _tmp142 * _tmp63 - _tmp118 * _tmp149 +
                         _tmp131 * _tmp148 * _tmp63 - _tmp163 * _tmp91;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp108 * _tmp133;
  const Scalar _tmp167 = _tmp106 * _tmp113 * _tmp74 + _tmp116 * _tmp118 * _tmp74 +
                         _tmp119 * _tmp123 + _tmp130 * _tmp131 * _tmp74 - _tmp134 * _tmp166 +
                         _tmp136 * _tmp137;
  const Scalar _tmp168 = std::asinh(_tmp165 * _tmp167);
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp170 =
      -_tmp168 * _tmp169 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp172 = _tmp170 * _tmp171;
  const Scalar _tmp173 = Scalar(1.0) * _tmp168;
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp145;
  const Scalar _tmp175 = _tmp174 * _tmp91;
  const Scalar _tmp176 = _tmp108 * _tmp21;
  const Scalar _tmp177 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp178 = _tmp146 * _tmp177;
  const Scalar _tmp179 =
      (_tmp165 * (-_tmp123 - _tmp134 * _tmp176 + _tmp137 * _tmp159) - _tmp167 * _tmp178) /
      std::sqrt(Scalar(std::pow(_tmp167, Scalar(2)) * _tmp177 + 1));
  const Scalar _tmp180 = -_tmp104 * _tmp113 * _tmp99 - _tmp104 * _tmp128 * _tmp131 -
                         _tmp114 * _tmp118 + _tmp119 * _tmp121 - _tmp136 * _tmp138 + _tmp166;
  const Scalar _tmp181 =
      _tmp100 * _tmp113 * _tmp94 + _tmp115 * _tmp118 + _tmp129 * _tmp131 * _tmp94 + _tmp163;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = std::asinh(_tmp180 * _tmp182);
  const Scalar _tmp184 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp185 =
      -_tmp183 * _tmp184 - std::sqrt(Scalar(std::pow(Scalar(-_tmp81 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp84 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp186 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp187 = _tmp185 * _tmp186;
  const Scalar _tmp188 = Scalar(1.0) * _tmp183;
  const Scalar _tmp189 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp190 = _tmp145 * _tmp189;
  const Scalar _tmp191 = (_tmp180 * _tmp190 + _tmp182 * (-_tmp121 - _tmp138 * _tmp159 + _tmp176)) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp189 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp3 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp3 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_a(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp34 + p_a(1, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp153 * (-Scalar(1.0) * _tmp160 * std::cosh(_tmp162) -
                          (-Scalar(0.1034955) * _tmp154 * _tmp158 +
                           _tmp155 * (-_tmp152 * _tmp161 - _tmp153 * _tmp160)) *
                              std::cosh(_tmp156)) +
               _tmp161 * (-std::sinh(_tmp156) - std::sinh(_tmp162));
  _res(2, 0) = _tmp169 * (-Scalar(1.0) * _tmp179 * std::cosh(_tmp173) -
                          (-Scalar(0.1034955) * _tmp170 * _tmp178 +
                           _tmp171 * (-_tmp168 * _tmp175 - _tmp169 * _tmp179)) *
                              std::cosh(_tmp172)) +
               _tmp175 * (-std::sinh(_tmp172) - std::sinh(_tmp173));
  _res(3, 0) = -_tmp174 * (-std::sinh(_tmp187) - std::sinh(_tmp188)) +
               _tmp184 * (-Scalar(1.0) * _tmp191 * std::cosh(_tmp188) -
                          (Scalar(0.1034955) * _tmp185 * _tmp190 +
                           _tmp186 * (_tmp174 * _tmp183 - _tmp184 * _tmp191)) *
                              std::cosh(_tmp187));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
