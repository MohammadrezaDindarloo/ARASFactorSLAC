// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fh1_Nl0
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFh1Nl0(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 641

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (218)
  const Scalar _tmp0 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp1 = Scalar(0.1034955) * p_a(2, 0);
  const Scalar _tmp2 = Scalar(1.0) / (fh1);
  const Scalar _tmp3 = _tmp2 * fv1;
  const Scalar _tmp4 = std::asinh(_tmp3);
  const Scalar _tmp5 = Scalar(9.6622558468725703) * _tmp4;
  const Scalar _tmp6 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp7 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp8 = 2 * _tmp7;
  const Scalar _tmp9 = _tmp6 * _tmp8;
  const Scalar _tmp10 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                        _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp12 = _tmp10 * _tmp11;
  const Scalar _tmp13 = -Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp14 = -_tmp13;
  const Scalar _tmp15 = -2 * std::pow(_tmp10, Scalar(2));
  const Scalar _tmp16 = 1 - 2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = 2 * _tmp10 * _tmp6;
  const Scalar _tmp19 = _tmp11 * _tmp7;
  const Scalar _tmp20 = _tmp18 + _tmp19;
  const Scalar _tmp21 = -Scalar(0.010999999999999999) * _tmp20;
  const Scalar _tmp22 = -_tmp17 + _tmp21;
  const Scalar _tmp23 = _tmp14 + _tmp22;
  const Scalar _tmp24 = _tmp23 + p_init0;
  const Scalar _tmp25 = Scalar(0.20999999999999999) * _tmp12 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp26 = -_tmp25;
  const Scalar _tmp27 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp28 = Scalar(0.20999999999999999) * _tmp15 +
                        Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999);
  const Scalar _tmp29 = _tmp10 * _tmp8;
  const Scalar _tmp30 = _tmp11 * _tmp6;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = -Scalar(0.010999999999999999) * _tmp31;
  const Scalar _tmp33 = -_tmp28 + _tmp32;
  const Scalar _tmp34 = _tmp26 + _tmp33;
  const Scalar _tmp35 = _tmp34 + p_init1;
  const Scalar _tmp36 =
      -Scalar(0.1034955) * _tmp5 * fh1 -
      Scalar(0.1034955) * std::sqrt(Scalar(std::pow(Scalar(-_tmp24 + p_a(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp35 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp37 = _tmp2 * _tmp36;
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp0 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp4;
  const Scalar _tmp40 = _tmp17 + _tmp21;
  const Scalar _tmp41 = _tmp13 + _tmp40;
  const Scalar _tmp42 = _tmp41 + p_init0;
  const Scalar _tmp43 = _tmp42 - p_c(0, 0);
  const Scalar _tmp44 = _tmp28 + _tmp32;
  const Scalar _tmp45 = _tmp25 + _tmp44;
  const Scalar _tmp46 = _tmp45 + p_init1;
  const Scalar _tmp47 = _tmp46 - p_c(1, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp43 * _tmp48;
  const Scalar _tmp50 = _tmp47 * _tmp48;
  const Scalar _tmp51 = _tmp25 + _tmp33;
  const Scalar _tmp52 = _tmp51 + p_init1;
  const Scalar _tmp53 = _tmp52 - p_b(1, 0);
  const Scalar _tmp54 = _tmp14 + _tmp40;
  const Scalar _tmp55 = _tmp54 + p_init0;
  const Scalar _tmp56 = _tmp55 - p_b(0, 0);
  const Scalar _tmp57 = Scalar(1.0) / (_tmp56);
  const Scalar _tmp58 = _tmp53 * _tmp57;
  const Scalar _tmp59 = Scalar(1.0) / (_tmp49 * _tmp58 - _tmp50);
  const Scalar _tmp60 =
      std::sqrt(Scalar(std::pow(_tmp53, Scalar(2)) + std::pow(_tmp56, Scalar(2))));
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = _tmp57 * _tmp60;
  const Scalar _tmp63 = _tmp62 * (-_tmp51 * _tmp56 * _tmp61 + _tmp53 * _tmp54 * _tmp61);
  const Scalar _tmp64 = -_tmp41 * _tmp50 + _tmp45 * _tmp49 + _tmp49 * _tmp63;
  const Scalar _tmp65 = Scalar(1.0) * _tmp59;
  const Scalar _tmp66 = Scalar(0.20999999999999999) * _tmp29 + Scalar(0.20999999999999999) * _tmp30;
  const Scalar _tmp67 = -_tmp66;
  const Scalar _tmp68 =
      -Scalar(0.010999999999999999) * _tmp16 - Scalar(0.010999999999999999) * _tmp27;
  const Scalar _tmp69 = Scalar(0.20999999999999999) * _tmp18 - Scalar(0.20999999999999999) * _tmp19;
  const Scalar _tmp70 = _tmp68 + _tmp69;
  const Scalar _tmp71 = _tmp67 + _tmp70;
  const Scalar _tmp72 = _tmp66 + _tmp70;
  const Scalar _tmp73 = _tmp49 * _tmp71 - _tmp49 * _tmp72;
  const Scalar _tmp74 = _tmp58 * _tmp71;
  const Scalar _tmp75 = -_tmp49 * _tmp74 + _tmp50 * _tmp72;
  const Scalar _tmp76 = _tmp65 * _tmp75;
  const Scalar _tmp77 = Scalar(1.0) * _tmp51;
  const Scalar _tmp78 = -_tmp77;
  const Scalar _tmp79 = Scalar(1.0) / (_tmp45 + _tmp78);
  const Scalar _tmp80 = Scalar(1.0) * _tmp54;
  const Scalar _tmp81 = _tmp79 * (-_tmp41 + _tmp80);
  const Scalar _tmp82 = -_tmp65 * _tmp73 + _tmp76 * _tmp81;
  const Scalar _tmp83 = _tmp68 - _tmp69;
  const Scalar _tmp84 = _tmp66 + _tmp83;
  const Scalar _tmp85 = _tmp26 + _tmp44;
  const Scalar _tmp86 = _tmp85 + p_init1;
  const Scalar _tmp87 = _tmp86 - p_d(1, 0);
  const Scalar _tmp88 = _tmp13 + _tmp22;
  const Scalar _tmp89 = _tmp88 + p_init0;
  const Scalar _tmp90 = _tmp89 - p_d(0, 0);
  const Scalar _tmp91 = std::pow(Scalar(std::pow(_tmp87, Scalar(2)) + std::pow(_tmp90, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp92 = _tmp90 * _tmp91;
  const Scalar _tmp93 = _tmp71 * _tmp92;
  const Scalar _tmp94 = _tmp87 * _tmp91;
  const Scalar _tmp95 = _tmp58 * _tmp92 - _tmp94;
  const Scalar _tmp96 = _tmp59 * _tmp95;
  const Scalar _tmp97 = -_tmp58 * _tmp93 - _tmp75 * _tmp96 + _tmp84 * _tmp94;
  const Scalar _tmp98 = -_tmp73 * _tmp96 - _tmp81 * _tmp97 - _tmp84 * _tmp92 + _tmp93;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp100 = _tmp63 * _tmp92 - _tmp64 * _tmp96 + _tmp85 * _tmp92 - _tmp88 * _tmp94;
  const Scalar _tmp101 = _tmp100 * _tmp99;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp100);
  const Scalar _tmp103 = _tmp102 * _tmp98;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp82 - _tmp64 * _tmp65);
  const Scalar _tmp105 = _tmp104 + _tmp82;
  const Scalar _tmp106 = _tmp95 * _tmp99;
  const Scalar _tmp107 = _tmp59 * (-_tmp105 * _tmp106 + Scalar(1.0));
  const Scalar _tmp108 = _tmp105 * _tmp99;
  const Scalar _tmp109 = _tmp24 - p_a(0, 0);
  const Scalar _tmp110 = _tmp35 - p_a(1, 0);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp110 * _tmp111;
  const Scalar _tmp113 = _tmp112 * _tmp62 * (_tmp107 * _tmp49 + _tmp108 * _tmp92);
  const Scalar _tmp114 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp115 = _tmp77 * _tmp81 + _tmp80;
  const Scalar _tmp116 = 0;
  const Scalar _tmp117 = _tmp116 * _tmp99;
  const Scalar _tmp118 = _tmp58 * _tmp59;
  const Scalar _tmp119 = _tmp118 * _tmp75 + _tmp74;
  const Scalar _tmp120 = _tmp118 * _tmp73 - _tmp119 * _tmp81 - _tmp71;
  const Scalar _tmp121 = _tmp103 * (-_tmp101 * _tmp120 + _tmp118 * _tmp64 - _tmp63);
  const Scalar _tmp122 = _tmp120 + _tmp121;
  const Scalar _tmp123 = _tmp122 * _tmp99;
  const Scalar _tmp124 = _tmp59 * (-_tmp106 * _tmp122 - _tmp58);
  const Scalar _tmp125 = _tmp109 * _tmp111;
  const Scalar _tmp126 = _tmp125 * _tmp62 * (_tmp123 * _tmp92 + _tmp124 * _tmp49 + Scalar(1.0));
  const Scalar _tmp127 = _tmp112 * _tmp23 - _tmp125 * _tmp34;
  const Scalar _tmp128 = _tmp102 * _tmp65 * _tmp95;
  const Scalar _tmp129 = Scalar(1.0) * _tmp102;
  const Scalar _tmp130 = _tmp127 * _tmp62 * (-_tmp128 * _tmp49 + _tmp129 * _tmp92);
  const Scalar _tmp131 = -_tmp113 * fh1 -
                         _tmp114 * _tmp62 * (-_tmp117 * _tmp49 * _tmp96 + _tmp117 * _tmp92) -
                         _tmp126 * fh1 - _tmp130 * fh1;
  const Scalar _tmp132 = Scalar(1.0) / (_tmp131);
  const Scalar _tmp133 = _tmp97 * _tmp99;
  const Scalar _tmp134 = _tmp78 + _tmp85;
  const Scalar _tmp135 = _tmp134 * _tmp81;
  const Scalar _tmp136 = Scalar(1.0) / (-_tmp135 + _tmp80 - _tmp88);
  const Scalar _tmp137 = _tmp134 * _tmp136;
  const Scalar _tmp138 = _tmp104 * _tmp137 - _tmp105 * _tmp133 - _tmp76;
  const Scalar _tmp139 = Scalar(1.0) * _tmp79;
  const Scalar _tmp140 = Scalar(1.0) * _tmp136;
  const Scalar _tmp141 = Scalar(1.0) * _tmp112 * (_tmp104 * _tmp140 - _tmp138 * _tmp139);
  const Scalar _tmp142 = _tmp103 * _tmp140;
  const Scalar _tmp143 = -_tmp129 * _tmp97 + _tmp134 * _tmp142;
  const Scalar _tmp144 = Scalar(1.0) * _tmp127 * (-_tmp139 * _tmp143 + _tmp142);
  const Scalar _tmp145 = _tmp115 * _tmp136;
  const Scalar _tmp146 = -_tmp116 * _tmp133 - _tmp134 * _tmp145 + _tmp78;
  const Scalar _tmp147 = _tmp67 + _tmp83;
  const Scalar _tmp148 = _tmp147 * fh1;
  const Scalar _tmp149 = -_tmp112 * _tmp148 - Scalar(5.1796800000000003) * _tmp31 - _tmp34 * fv1;
  const Scalar _tmp150 = _tmp79 * (_tmp135 * _tmp140 + Scalar(1.0));
  const Scalar _tmp151 = _tmp140 * _tmp81;
  const Scalar _tmp152 = -Scalar(1.0) * _tmp150 + Scalar(1.0) * _tmp151;
  const Scalar _tmp153 = _tmp119 + _tmp121 * _tmp137 - _tmp122 * _tmp133;
  const Scalar _tmp154 = Scalar(1.0) * _tmp125 * (_tmp121 * _tmp140 - _tmp139 * _tmp153);
  const Scalar _tmp155 = _tmp125 * _tmp148 + Scalar(5.1796800000000003) * _tmp20 + _tmp23 * fv1;
  const Scalar _tmp156 = _tmp134 * _tmp79;
  const Scalar _tmp157 = Scalar(1.0) * _tmp140 * _tmp156 - Scalar(1.0) * _tmp140;
  const Scalar _tmp158 =
      Scalar(1.0) * _tmp114 * (-_tmp115 * _tmp140 - _tmp139 * _tmp146 + Scalar(1.0)) +
      _tmp141 * fh1 + _tmp144 * fh1 + _tmp149 * _tmp152 + _tmp154 * fh1 + _tmp155 * _tmp157;
  const Scalar _tmp159 = std::asinh(_tmp132 * _tmp158);
  const Scalar _tmp160 = Scalar(1.0) * _tmp159;
  const Scalar _tmp161 = std::pow(_tmp131, Scalar(-2));
  const Scalar _tmp162 = -_tmp113 - _tmp126 - _tmp130;
  const Scalar _tmp163 = _tmp161 * _tmp162;
  const Scalar _tmp164 = _tmp125 * _tmp147;
  const Scalar _tmp165 = _tmp112 * _tmp147;
  const Scalar _tmp166 =
      (_tmp132 * (_tmp141 + _tmp144 - _tmp152 * _tmp165 + _tmp154 + _tmp157 * _tmp164) -
       _tmp158 * _tmp163) /
      std::sqrt(Scalar(std::pow(_tmp158, Scalar(2)) * _tmp161 + 1));
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp168 =
      -_tmp131 * _tmp167 - std::sqrt(Scalar(std::pow(Scalar(-_tmp52 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp55 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp169 = Scalar(0.1034955) * _tmp132;
  const Scalar _tmp170 = _tmp168 * _tmp169;
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp163;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp131;
  const Scalar _tmp173 = _tmp107 * _tmp112;
  const Scalar _tmp174 = _tmp124 * _tmp125;
  const Scalar _tmp175 = _tmp114 * _tmp117;
  const Scalar _tmp176 = _tmp127 * _tmp128;
  const Scalar _tmp177 = _tmp173 * fh1 + _tmp174 * fh1 - _tmp175 * _tmp96 - _tmp176 * fh1;
  const Scalar _tmp178 = Scalar(1.0) / (_tmp177);
  const Scalar _tmp179 = _tmp125 * _tmp79;
  const Scalar _tmp180 = _tmp153 * _tmp179;
  const Scalar _tmp181 = _tmp140 * _tmp155;
  const Scalar _tmp182 = _tmp127 * _tmp143 * _tmp79;
  const Scalar _tmp183 = _tmp112 * _tmp138 * _tmp79;
  const Scalar _tmp184 = _tmp114 * _tmp146 * _tmp79 + _tmp149 * _tmp150 - _tmp156 * _tmp181 +
                         _tmp180 * fh1 + _tmp182 * fh1 + _tmp183 * fh1;
  const Scalar _tmp185 = std::asinh(_tmp178 * _tmp184);
  const Scalar _tmp186 = Scalar(1.0) * _tmp185;
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp178;
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp177;
  const Scalar _tmp189 =
      -_tmp185 * _tmp188 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp190 = _tmp187 * _tmp189;
  const Scalar _tmp191 = _tmp173 + _tmp174 - _tmp176;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp191;
  const Scalar _tmp193 = std::pow(_tmp177, Scalar(-2));
  const Scalar _tmp194 = _tmp191 * _tmp193;
  const Scalar _tmp195 = Scalar(0.1034955) * _tmp194;
  const Scalar _tmp196 = (_tmp178 * (-_tmp134 * _tmp140 * _tmp147 * _tmp179 - _tmp150 * _tmp165 +
                                     _tmp180 + _tmp182 + _tmp183) -
                          _tmp184 * _tmp194) /
                         std::sqrt(Scalar(std::pow(_tmp184, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp197 = _tmp123 * _tmp125;
  const Scalar _tmp198 = _tmp108 * _tmp112;
  const Scalar _tmp199 = _tmp127 * _tmp129;
  const Scalar _tmp200 = _tmp197 + _tmp198 + _tmp199;
  const Scalar _tmp201 = _tmp175 + _tmp197 * fh1 + _tmp198 * fh1 + _tmp199 * fh1;
  const Scalar _tmp202 = _tmp121 * _tmp125 * _tmp136;
  const Scalar _tmp203 = _tmp127 * _tmp142;
  const Scalar _tmp204 = _tmp104 * _tmp112 * _tmp136;
  const Scalar _tmp205 = _tmp114 * _tmp145 - _tmp149 * _tmp151 + _tmp181 - _tmp202 * fh1 -
                         _tmp203 * fh1 - _tmp204 * fh1;
  const Scalar _tmp206 = Scalar(1.0) / (_tmp201);
  const Scalar _tmp207 = std::asinh(_tmp205 * _tmp206);
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp209 =
      -_tmp201 * _tmp208 - std::sqrt(Scalar(std::pow(Scalar(-_tmp86 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp89 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp210 = Scalar(0.1034955) * _tmp206;
  const Scalar _tmp211 = _tmp209 * _tmp210;
  const Scalar _tmp212 = Scalar(1.0) * _tmp207;
  const Scalar _tmp213 = std::pow(_tmp201, Scalar(-2));
  const Scalar _tmp214 = _tmp200 * _tmp213;
  const Scalar _tmp215 = Scalar(0.1034955) * _tmp214;
  const Scalar _tmp216 = Scalar(9.6622558468725703) * _tmp201;
  const Scalar _tmp217 = (-_tmp205 * _tmp214 + _tmp206 * (_tmp140 * _tmp164 + _tmp151 * _tmp165 -
                                                          _tmp202 - _tmp203 - _tmp204)) /
                         std::sqrt(Scalar(std::pow(_tmp205, Scalar(2)) * _tmp213 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = -Scalar(9.6622558468725703) * _tmp1 * _tmp2 -
               Scalar(9.6622558468725703) * fh1 *
                   (-_tmp0 * _tmp1 - Scalar(1.0) * _tmp0 * _tmp38 * fv1 * std::sinh(_tmp39) -
                    (-_tmp0 * _tmp36 + Scalar(0.1034955) * _tmp2 *
                                           (Scalar(9.6622558468725703) * _tmp3 * _tmp38 - _tmp5)) *
                        std::sinh(_tmp37)) +
               Scalar(9.6622558468725703) * std::cosh(_tmp37) -
               Scalar(9.6622558468725703) * std::cosh(_tmp39);
  _res(1, 0) =
      -Scalar(9.6622558468725703) * _tmp162 *
          (_tmp169 * p_b(2, 0) + std::cosh(_tmp160) - std::cosh(_tmp170)) -
      _tmp172 * (Scalar(1.0) * _tmp166 * std::sinh(_tmp160) - _tmp171 * p_b(2, 0) -
                 (-_tmp168 * _tmp171 + _tmp169 * (-_tmp162 * _tmp167 - _tmp166 * _tmp172)) *
                     std::sinh(_tmp170));
  _res(2, 0) =
      -_tmp188 * (-_tmp195 * p_c(2, 0) + Scalar(1.0) * _tmp196 * std::sinh(_tmp186) -
                  (_tmp187 * (-_tmp185 * _tmp192 - _tmp188 * _tmp196) - _tmp189 * _tmp195) *
                      std::sinh(_tmp190)) -
      _tmp192 * (_tmp187 * p_c(2, 0) + std::cosh(_tmp186) - std::cosh(_tmp190));
  _res(3, 0) =
      -Scalar(9.6622558468725703) * _tmp200 *
          (_tmp210 * p_d(2, 0) - std::cosh(_tmp211) + std::cosh(_tmp212)) -
      _tmp216 * (-_tmp215 * p_d(2, 0) + Scalar(1.0) * _tmp217 * std::sinh(_tmp212) -
                 (-_tmp209 * _tmp215 + _tmp210 * (-_tmp200 * _tmp208 - _tmp216 * _tmp217)) *
                     std::sinh(_tmp211));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
