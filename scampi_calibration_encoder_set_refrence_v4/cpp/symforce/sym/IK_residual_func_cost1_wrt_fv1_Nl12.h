// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl12
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl12(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 587

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp10 = 2 * _tmp6 * _tmp9;
  const Scalar _tmp11 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp12 = _tmp11 * _tmp4;
  const Scalar _tmp13 = _tmp10 + _tmp12;
  const Scalar _tmp14 = -Scalar(0.010999999999999999) * _tmp13;
  const Scalar _tmp15 = 2 * _tmp4;
  const Scalar _tmp16 = _tmp15 * _tmp9;
  const Scalar _tmp17 = _tmp11 * _tmp6;
  const Scalar _tmp18 = Scalar(0.20999999999999999) * _tmp16 - Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp19 = _tmp14 + _tmp18;
  const Scalar _tmp20 = _tmp19 + _tmp8;
  const Scalar _tmp21 = _tmp20 + p_init0;
  const Scalar _tmp22 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp23 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp24 = _tmp15 * _tmp6;
  const Scalar _tmp25 = _tmp11 * _tmp9;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp16 + Scalar(0.20999999999999999) * _tmp17;
  const Scalar _tmp30 = _tmp28 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + p_init1;
  const Scalar _tmp33 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp34 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp35 = -_tmp34;
  const Scalar _tmp36 = -Scalar(0.010999999999999999) * _tmp22 -
                        Scalar(0.010999999999999999) * _tmp5 + Scalar(-0.010999999999999999);
  const Scalar _tmp37 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp38 = _tmp36 - _tmp37;
  const Scalar _tmp39 = _tmp35 + _tmp38;
  const Scalar _tmp40 = -_tmp29;
  const Scalar _tmp41 = _tmp23 + _tmp28 + _tmp40;
  const Scalar _tmp42 = _tmp41 + p_init1;
  const Scalar _tmp43 = _tmp42 - p_d(1, 0);
  const Scalar _tmp44 = -_tmp8;
  const Scalar _tmp45 = _tmp19 + _tmp44;
  const Scalar _tmp46 = _tmp45 + p_init0;
  const Scalar _tmp47 = _tmp46 - p_d(0, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp43, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp47 * _tmp48;
  const Scalar _tmp50 = _tmp34 + _tmp38;
  const Scalar _tmp51 = _tmp43 * _tmp48;
  const Scalar _tmp52 = -_tmp23;
  const Scalar _tmp53 = _tmp40 + _tmp52;
  const Scalar _tmp54 = _tmp28 + _tmp53;
  const Scalar _tmp55 = _tmp54 + p_init1;
  const Scalar _tmp56 = _tmp55 - p_a(1, 0);
  const Scalar _tmp57 = _tmp14 - _tmp18;
  const Scalar _tmp58 = _tmp44 + _tmp57;
  const Scalar _tmp59 = _tmp58 + p_init0;
  const Scalar _tmp60 = _tmp59 - p_a(0, 0);
  const Scalar _tmp61 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp62 = _tmp56 * _tmp61;
  const Scalar _tmp63 = _tmp39 * _tmp62;
  const Scalar _tmp64 = _tmp30 + _tmp52;
  const Scalar _tmp65 = _tmp64 + p_init1;
  const Scalar _tmp66 = _tmp65 - p_b(1, 0);
  const Scalar _tmp67 = _tmp57 + _tmp8;
  const Scalar _tmp68 = _tmp67 + p_init0;
  const Scalar _tmp69 = _tmp68 - p_b(0, 0);
  const Scalar _tmp70 = std::pow(Scalar(std::pow(_tmp66, Scalar(2)) + std::pow(_tmp69, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp71 = _tmp69 * _tmp70;
  const Scalar _tmp72 = _tmp36 + _tmp37;
  const Scalar _tmp73 = _tmp35 + _tmp72;
  const Scalar _tmp74 = _tmp66 * _tmp70;
  const Scalar _tmp75 = -_tmp63 * _tmp71 + _tmp73 * _tmp74;
  const Scalar _tmp76 = _tmp49 * _tmp62 - _tmp51;
  const Scalar _tmp77 = Scalar(1.0) / (_tmp62 * _tmp71 - _tmp74);
  const Scalar _tmp78 = _tmp76 * _tmp77;
  const Scalar _tmp79 = -_tmp49 * _tmp63 + _tmp50 * _tmp51 - _tmp75 * _tmp78;
  const Scalar _tmp80 = Scalar(1.0) * _tmp54;
  const Scalar _tmp81 = -_tmp80;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp64 + _tmp81);
  const Scalar _tmp83 = Scalar(1.0) * _tmp58;
  const Scalar _tmp84 = -_tmp67 + _tmp83;
  const Scalar _tmp85 = _tmp82 * _tmp84;
  const Scalar _tmp86 = _tmp39 * _tmp71 - _tmp71 * _tmp73;
  const Scalar _tmp87 = _tmp39 * _tmp49 - _tmp49 * _tmp50 - _tmp78 * _tmp86 - _tmp79 * _tmp85;
  const Scalar _tmp88 = Scalar(1.0) / (_tmp87);
  const Scalar _tmp89 = _tmp80 * _tmp85 + _tmp83;
  const Scalar _tmp90 = 0;
  const Scalar _tmp91 = _tmp78 * _tmp90;
  const Scalar _tmp92 =
      std::sqrt(Scalar(std::pow(_tmp56, Scalar(2)) + std::pow(_tmp60, Scalar(2))));
  const Scalar _tmp93 = _tmp61 * _tmp92;
  const Scalar _tmp94 = _tmp93 * (_tmp49 * _tmp90 - _tmp71 * _tmp91);
  const Scalar _tmp95 = Scalar(1.0) / (_tmp92);
  const Scalar _tmp96 = _tmp93 * (-_tmp54 * _tmp60 * _tmp95 + _tmp56 * _tmp58 * _tmp95);
  const Scalar _tmp97 = _tmp64 * _tmp71 - _tmp67 * _tmp74 + _tmp71 * _tmp96;
  const Scalar _tmp98 = _tmp41 * _tmp49 - _tmp45 * _tmp51 + _tmp49 * _tmp96 - _tmp78 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp98);
  const Scalar _tmp100 = Scalar(1.0) * _tmp99;
  const Scalar _tmp101 = _tmp21 - p_c(0, 0);
  const Scalar _tmp102 = _tmp32 - p_c(1, 0);
  const Scalar _tmp103 =
      std::pow(Scalar(std::pow(_tmp101, Scalar(2)) + std::pow(_tmp102, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp104 = _tmp102 * _tmp103;
  const Scalar _tmp105 = _tmp101 * _tmp103;
  const Scalar _tmp106 = fh1 * (_tmp104 * _tmp20 - _tmp105 * _tmp31);
  const Scalar _tmp107 = Scalar(1.0) * _tmp77;
  const Scalar _tmp108 = Scalar(1.0) * _tmp82;
  const Scalar _tmp109 = -_tmp107 * _tmp86 + _tmp108 * _tmp75 * _tmp77 * _tmp84;
  const Scalar _tmp110 = _tmp88 * _tmp98;
  const Scalar _tmp111 = _tmp87 * _tmp99;
  const Scalar _tmp112 = _tmp111 * (-_tmp107 * _tmp97 - _tmp109 * _tmp110);
  const Scalar _tmp113 = _tmp88 * (_tmp109 + _tmp112);
  const Scalar _tmp114 = -_tmp113 * _tmp76 + Scalar(1.0);
  const Scalar _tmp115 = _tmp71 * _tmp77;
  const Scalar _tmp116 = _tmp104 * fh1;
  const Scalar _tmp117 = _tmp62 * _tmp77;
  const Scalar _tmp118 = _tmp117 * _tmp75 + _tmp63;
  const Scalar _tmp119 = _tmp117 * _tmp86 - _tmp118 * _tmp85 - _tmp39;
  const Scalar _tmp120 = _tmp111 * (-_tmp110 * _tmp119 + _tmp117 * _tmp97 - _tmp96);
  const Scalar _tmp121 = _tmp88 * (_tmp119 + _tmp120);
  const Scalar _tmp122 = -_tmp121 * _tmp76 - _tmp62;
  const Scalar _tmp123 = _tmp105 * fh1;
  const Scalar _tmp124 = -_tmp106 * _tmp93 * (_tmp100 * _tmp49 - _tmp100 * _tmp71 * _tmp78) -
                         _tmp116 * _tmp93 * (_tmp113 * _tmp49 + _tmp114 * _tmp115) -
                         _tmp123 * _tmp93 * (_tmp115 * _tmp122 + _tmp121 * _tmp49 + Scalar(1.0)) -
                         _tmp33 * _tmp94;
  const Scalar _tmp125 = Scalar(1.0) / (_tmp124);
  const Scalar _tmp126 = Scalar(0.1034955) * _tmp125;
  const Scalar _tmp127 = fh1 * (_tmp34 + _tmp72);
  const Scalar _tmp128 = _tmp105 * _tmp127 + Scalar(5.1796800000000003) * _tmp13 + _tmp20 * fv1;
  const Scalar _tmp129 = _tmp41 + _tmp81;
  const Scalar _tmp130 = _tmp129 * _tmp85;
  const Scalar _tmp131 = Scalar(1.0) / (-_tmp130 - _tmp45 + _tmp83);
  const Scalar _tmp132 = Scalar(1.0) * _tmp131;
  const Scalar _tmp133 = _tmp129 * _tmp82;
  const Scalar _tmp134 = Scalar(1.0) * _tmp132 * _tmp133 - Scalar(1.0) * _tmp132;
  const Scalar _tmp135 = _tmp131 * _tmp89;
  const Scalar _tmp136 = _tmp82 * (-_tmp129 * _tmp135 - _tmp79 * _tmp90 + _tmp81);
  const Scalar _tmp137 = -Scalar(1.0) * _tmp135 - Scalar(1.0) * _tmp136 + Scalar(1.0);
  const Scalar _tmp138 = -_tmp104 * _tmp127 - Scalar(5.1796800000000003) * _tmp26 - _tmp31 * fv1;
  const Scalar _tmp139 = _tmp130 * _tmp132 + Scalar(1.0);
  const Scalar _tmp140 = _tmp132 * _tmp85;
  const Scalar _tmp141 = -Scalar(1.0) * _tmp108 * _tmp139 + Scalar(1.0) * _tmp140;
  const Scalar _tmp142 = _tmp111 * _tmp132;
  const Scalar _tmp143 = -_tmp100 * _tmp79 + _tmp129 * _tmp142;
  const Scalar _tmp144 = _tmp129 * _tmp131;
  const Scalar _tmp145 = _tmp118 + _tmp120 * _tmp144 - _tmp121 * _tmp79;
  const Scalar _tmp146 = Scalar(1.0) * fh1;
  const Scalar _tmp147 = -_tmp107 * _tmp75 + _tmp112 * _tmp144 - _tmp113 * _tmp79;
  const Scalar _tmp148 = _tmp104 * _tmp146 * (-_tmp108 * _tmp147 + _tmp112 * _tmp132) +
                         _tmp105 * _tmp146 * (-_tmp108 * _tmp145 + _tmp120 * _tmp132) +
                         Scalar(1.0) * _tmp106 * (-_tmp108 * _tmp143 + _tmp142) +
                         _tmp128 * _tmp134 + _tmp137 * _tmp33 + _tmp138 * _tmp141;
  const Scalar _tmp149 = std::asinh(_tmp125 * _tmp148);
  const Scalar _tmp150 = Scalar(9.6622558468725703) * _tmp124;
  const Scalar _tmp151 =
      -_tmp149 * _tmp150 - std::sqrt(Scalar(std::pow(Scalar(-_tmp55 + p_a(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp59 + p_a(0, 0)), Scalar(2))));
  const Scalar _tmp152 = _tmp126 * _tmp151;
  const Scalar _tmp153 = Scalar(1.0) * _tmp149;
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp94;
  const Scalar _tmp155 = std::pow(_tmp124, Scalar(-2));
  const Scalar _tmp156 = _tmp155 * _tmp94;
  const Scalar _tmp157 = Scalar(0.1034955) * _tmp156;
  const Scalar _tmp158 = _tmp27 + _tmp53;
  const Scalar _tmp159 =
      (_tmp125 * (_tmp134 * _tmp20 - _tmp137 + _tmp141 * _tmp158) - _tmp148 * _tmp156) /
      std::sqrt(Scalar(std::pow(_tmp148, Scalar(2)) * _tmp155 + 1));
  const Scalar _tmp160 = _tmp33 * _tmp90;
  const Scalar _tmp161 = _tmp100 * _tmp106;
  const Scalar _tmp162 =
      _tmp114 * _tmp116 * _tmp77 + _tmp122 * _tmp123 * _tmp77 - _tmp160 * _tmp78 - _tmp161 * _tmp78;
  const Scalar _tmp163 = Scalar(1.0) / (_tmp162);
  const Scalar _tmp164 = _tmp128 * _tmp132;
  const Scalar _tmp165 = _tmp139 * _tmp82;
  const Scalar _tmp166 = _tmp82 * fh1;
  const Scalar _tmp167 = _tmp104 * _tmp147 * _tmp166 + _tmp105 * _tmp145 * _tmp166 +
                         _tmp106 * _tmp143 * _tmp82 - _tmp133 * _tmp164 + _tmp136 * _tmp33 +
                         _tmp138 * _tmp165;
  const Scalar _tmp168 = std::asinh(_tmp163 * _tmp167);
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp162;
  const Scalar _tmp170 =
      -_tmp168 * _tmp169 - std::sqrt(Scalar(std::pow(Scalar(-_tmp65 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp68 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp171 = Scalar(0.1034955) * _tmp163;
  const Scalar _tmp172 = _tmp170 * _tmp171;
  const Scalar _tmp173 = std::pow(_tmp162, Scalar(-2));
  const Scalar _tmp174 = _tmp173 * _tmp91;
  const Scalar _tmp175 = Scalar(0.1034955) * _tmp174;
  const Scalar _tmp176 = _tmp132 * _tmp20;
  const Scalar _tmp177 =
      (_tmp163 * (-_tmp133 * _tmp176 - _tmp136 + _tmp158 * _tmp165) - _tmp167 * _tmp174) /
      std::sqrt(Scalar(std::pow(_tmp167, Scalar(2)) * _tmp173 + 1));
  const Scalar _tmp178 = Scalar(9.6622558468725703) * _tmp90;
  const Scalar _tmp179 = _tmp178 * _tmp78;
  const Scalar _tmp180 = Scalar(1.0) * _tmp168;
  const Scalar _tmp181 = -_tmp106 * _tmp142 - _tmp112 * _tmp116 * _tmp131 -
                         _tmp120 * _tmp123 * _tmp131 + _tmp135 * _tmp33 - _tmp138 * _tmp140 +
                         _tmp164;
  const Scalar _tmp182 = _tmp113 * _tmp116 + _tmp121 * _tmp123 + _tmp160 + _tmp161;
  const Scalar _tmp183 = Scalar(1.0) / (_tmp182);
  const Scalar _tmp184 = std::asinh(_tmp181 * _tmp183);
  const Scalar _tmp185 = Scalar(9.6622558468725703) * _tmp182;
  const Scalar _tmp186 =
      -_tmp184 * _tmp185 - std::sqrt(Scalar(std::pow(Scalar(-_tmp42 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp187 = Scalar(0.1034955) * _tmp183;
  const Scalar _tmp188 = _tmp186 * _tmp187;
  const Scalar _tmp189 = std::pow(_tmp182, Scalar(-2));
  const Scalar _tmp190 = _tmp189 * _tmp90;
  const Scalar _tmp191 = (_tmp181 * _tmp190 + _tmp183 * (-_tmp135 - _tmp140 * _tmp158 + _tmp176)) /
                         std::sqrt(Scalar(std::pow(_tmp181, Scalar(2)) * _tmp189 + 1));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp190;
  const Scalar _tmp193 = Scalar(1.0) * _tmp184;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp3 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp3 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp21 + p_c(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp32 + p_c(1, 0)), Scalar(2)))))));
  _res(1, 0) =
      -_tmp150 * (-_tmp157 * p_a(2, 0) + Scalar(1.0) * _tmp159 * std::sinh(_tmp153) -
                  (_tmp126 * (-_tmp149 * _tmp154 - _tmp150 * _tmp159) - _tmp151 * _tmp157) *
                      std::sinh(_tmp152)) -
      _tmp154 * (_tmp126 * p_a(2, 0) - std::cosh(_tmp152) + std::cosh(_tmp153));
  _res(2, 0) =
      -_tmp169 * (-_tmp175 * p_b(2, 0) + Scalar(1.0) * _tmp177 * std::sinh(_tmp180) -
                  (-_tmp170 * _tmp175 + _tmp171 * (-_tmp168 * _tmp179 - _tmp169 * _tmp177)) *
                      std::sinh(_tmp172)) -
      _tmp179 * (_tmp171 * p_b(2, 0) - std::cosh(_tmp172) + std::cosh(_tmp180));
  _res(3, 0) = _tmp178 * (_tmp187 * p_d(2, 0) - std::cosh(_tmp188) + std::cosh(_tmp193)) -
               _tmp185 * (Scalar(1.0) * _tmp191 * std::sinh(_tmp193) + _tmp192 * p_d(2, 0) -
                          (_tmp186 * _tmp192 + _tmp187 * (_tmp178 * _tmp184 - _tmp185 * _tmp191)) *
                              std::sinh(_tmp188));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
