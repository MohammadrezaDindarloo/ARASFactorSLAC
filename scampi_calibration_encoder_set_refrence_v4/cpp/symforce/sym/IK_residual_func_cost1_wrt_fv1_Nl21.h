// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost1_wrt_fv1_Nl21
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost1WrtFv1Nl21(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 587

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (194)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp5 = 2 * _tmp3 * _tmp4;
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp8 = _tmp6 * _tmp7;
  const Scalar _tmp9 = Scalar(0.20999999999999999) * _tmp5 - Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp10 = -2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp11 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp12 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp6;
  const Scalar _tmp14 = _tmp13 * _tmp4;
  const Scalar _tmp15 = _tmp3 * _tmp7;
  const Scalar _tmp16 = _tmp14 + _tmp15;
  const Scalar _tmp17 = -Scalar(0.010999999999999999) * _tmp16;
  const Scalar _tmp18 = -_tmp12 + _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp9;
  const Scalar _tmp20 = _tmp19 + p_init0;
  const Scalar _tmp21 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp22 = -_tmp21;
  const Scalar _tmp23 = _tmp13 * _tmp3;
  const Scalar _tmp24 = _tmp4 * _tmp7;
  const Scalar _tmp25 = _tmp23 - _tmp24;
  const Scalar _tmp26 = Scalar(0.010999999999999999) * _tmp25;
  const Scalar _tmp27 = -_tmp26;
  const Scalar _tmp28 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp10 +
                        Scalar(0.20999999999999999) * _tmp28 + Scalar(0.20999999999999999);
  const Scalar _tmp30 = _tmp27 + _tmp29;
  const Scalar _tmp31 = _tmp22 + _tmp30;
  const Scalar _tmp32 = _tmp31 + p_init1;
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = -_tmp9;
  const Scalar _tmp35 = _tmp12 + _tmp17;
  const Scalar _tmp36 = _tmp34 + _tmp35;
  const Scalar _tmp37 = _tmp36 + p_init0;
  const Scalar _tmp38 = -_tmp29;
  const Scalar _tmp39 = _tmp27 + _tmp38;
  const Scalar _tmp40 = _tmp21 + _tmp39;
  const Scalar _tmp41 = _tmp40 + p_init1;
  const Scalar _tmp42 = _tmp18 + _tmp34;
  const Scalar _tmp43 = _tmp42 + p_init0;
  const Scalar _tmp44 = _tmp43 - p_a(0, 0);
  const Scalar _tmp45 = _tmp22 + _tmp39;
  const Scalar _tmp46 = _tmp45 + p_init1;
  const Scalar _tmp47 = _tmp46 - p_a(1, 0);
  const Scalar _tmp48 = std::pow(Scalar(std::pow(_tmp44, Scalar(2)) + std::pow(_tmp47, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp49 = _tmp44 * _tmp48;
  const Scalar _tmp50 = _tmp35 + _tmp9;
  const Scalar _tmp51 = _tmp50 + p_init0;
  const Scalar _tmp52 = _tmp51 - p_c(0, 0);
  const Scalar _tmp53 = _tmp21 + _tmp30;
  const Scalar _tmp54 = _tmp53 + p_init1;
  const Scalar _tmp55 = _tmp54 - p_c(1, 0);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp55 * _tmp56;
  const Scalar _tmp58 = _tmp52 * _tmp56;
  const Scalar _tmp59 = _tmp41 - p_b(1, 0);
  const Scalar _tmp60 = _tmp37 - p_b(0, 0);
  const Scalar _tmp61 =
      std::sqrt(Scalar(std::pow(_tmp59, Scalar(2)) + std::pow(_tmp60, Scalar(2))));
  const Scalar _tmp62 = Scalar(1.0) / (_tmp61);
  const Scalar _tmp63 = Scalar(1.0) / (_tmp60);
  const Scalar _tmp64 = _tmp61 * _tmp63;
  const Scalar _tmp65 = _tmp64 * (_tmp36 * _tmp59 * _tmp62 - _tmp40 * _tmp60 * _tmp62);
  const Scalar _tmp66 = -_tmp50 * _tmp57 + _tmp53 * _tmp58 + _tmp58 * _tmp65;
  const Scalar _tmp67 = _tmp59 * _tmp63;
  const Scalar _tmp68 = Scalar(1.0) / (-_tmp57 + _tmp58 * _tmp67);
  const Scalar _tmp69 = _tmp47 * _tmp48;
  const Scalar _tmp70 = _tmp49 * _tmp67 - _tmp69;
  const Scalar _tmp71 = _tmp68 * _tmp70;
  const Scalar _tmp72 = -_tmp42 * _tmp69 + _tmp45 * _tmp49 + _tmp49 * _tmp65 - _tmp66 * _tmp71;
  const Scalar _tmp73 = Scalar(1.0) / (_tmp72);
  const Scalar _tmp74 = Scalar(1.0) * _tmp73;
  const Scalar _tmp75 = Scalar(1.0) * _tmp68;
  const Scalar _tmp76 = _tmp70 * _tmp73 * _tmp75;
  const Scalar _tmp77 = _tmp32 - p_d(1, 0);
  const Scalar _tmp78 = _tmp20 - p_d(0, 0);
  const Scalar _tmp79 = std::pow(Scalar(std::pow(_tmp77, Scalar(2)) + std::pow(_tmp78, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp80 = _tmp78 * _tmp79;
  const Scalar _tmp81 = _tmp77 * _tmp79;
  const Scalar _tmp82 = fh1 * (_tmp19 * _tmp81 - _tmp31 * _tmp80);
  const Scalar _tmp83 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp24;
  const Scalar _tmp84 = -_tmp83;
  const Scalar _tmp85 =
      -Scalar(0.010999999999999999) * _tmp11 - Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp86 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp87 = _tmp85 + _tmp86;
  const Scalar _tmp88 = _tmp84 + _tmp87;
  const Scalar _tmp89 = _tmp83 + _tmp87;
  const Scalar _tmp90 = _tmp58 * _tmp88 - _tmp58 * _tmp89;
  const Scalar _tmp91 = _tmp67 * _tmp88;
  const Scalar _tmp92 = _tmp57 * _tmp89 - _tmp58 * _tmp91;
  const Scalar _tmp93 = _tmp75 * _tmp92;
  const Scalar _tmp94 = Scalar(1.0) * _tmp40;
  const Scalar _tmp95 = -_tmp94;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp53 + _tmp95);
  const Scalar _tmp97 = Scalar(1.0) * _tmp36;
  const Scalar _tmp98 = _tmp96 * (-_tmp50 + _tmp97);
  const Scalar _tmp99 = -_tmp75 * _tmp90 + _tmp93 * _tmp98;
  const Scalar _tmp100 = _tmp85 - _tmp86;
  const Scalar _tmp101 = _tmp100 + _tmp84;
  const Scalar _tmp102 = _tmp101 * _tmp69 - _tmp49 * _tmp91 - _tmp71 * _tmp92;
  const Scalar _tmp103 = -_tmp101 * _tmp49 - _tmp102 * _tmp98 + _tmp49 * _tmp88 - _tmp71 * _tmp90;
  const Scalar _tmp104 = Scalar(1.0) / (_tmp103);
  const Scalar _tmp105 = _tmp104 * _tmp72;
  const Scalar _tmp106 = _tmp103 * _tmp73;
  const Scalar _tmp107 = _tmp106 * (-_tmp105 * _tmp99 - _tmp66 * _tmp75);
  const Scalar _tmp108 = _tmp107 + _tmp99;
  const Scalar _tmp109 = _tmp104 * _tmp49;
  const Scalar _tmp110 = _tmp104 * _tmp70;
  const Scalar _tmp111 = -_tmp108 * _tmp110 + Scalar(1.0);
  const Scalar _tmp112 = _tmp58 * _tmp68;
  const Scalar _tmp113 = _tmp81 * fh1;
  const Scalar _tmp114 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp115 = _tmp94 * _tmp98 + _tmp97;
  const Scalar _tmp116 = 0;
  const Scalar _tmp117 = _tmp116 * _tmp71;
  const Scalar _tmp118 = _tmp64 * (_tmp116 * _tmp49 - _tmp117 * _tmp58);
  const Scalar _tmp119 = _tmp67 * _tmp68;
  const Scalar _tmp120 = _tmp119 * _tmp92 + _tmp91;
  const Scalar _tmp121 = _tmp119 * _tmp90 - _tmp120 * _tmp98 - _tmp88;
  const Scalar _tmp122 = _tmp106 * (-_tmp105 * _tmp121 + _tmp119 * _tmp66 - _tmp65);
  const Scalar _tmp123 = _tmp121 + _tmp122;
  const Scalar _tmp124 = -_tmp110 * _tmp123 - _tmp67;
  const Scalar _tmp125 = _tmp80 * fh1;
  const Scalar _tmp126 = -_tmp113 * _tmp64 * (_tmp108 * _tmp109 + _tmp111 * _tmp112) -
                         _tmp114 * _tmp118 -
                         _tmp125 * _tmp64 * (_tmp109 * _tmp123 + _tmp112 * _tmp124 + Scalar(1.0)) -
                         _tmp64 * _tmp82 * (_tmp49 * _tmp74 - _tmp58 * _tmp76);
  const Scalar _tmp127 = Scalar(1.0) / (_tmp126);
  const Scalar _tmp128 = _tmp45 + _tmp95;
  const Scalar _tmp129 = _tmp128 * _tmp98;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 - _tmp42 + _tmp97);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp102 * _tmp104;
  const Scalar _tmp133 = _tmp128 * _tmp130;
  const Scalar _tmp134 = _tmp120 + _tmp122 * _tmp133 - _tmp123 * _tmp132;
  const Scalar _tmp135 = Scalar(1.0) * _tmp96;
  const Scalar _tmp136 = _tmp115 * _tmp130;
  const Scalar _tmp137 = _tmp96 * (-_tmp102 * _tmp116 - _tmp128 * _tmp136 + _tmp95);
  const Scalar _tmp138 = -Scalar(1.0) * _tmp115 * _tmp131 - Scalar(1.0) * _tmp137 + Scalar(1.0);
  const Scalar _tmp139 = _tmp107 * _tmp133 - _tmp108 * _tmp132 - _tmp93;
  const Scalar _tmp140 = fh1 * (_tmp100 + _tmp83);
  const Scalar _tmp141 = _tmp140 * _tmp80 + Scalar(5.1796800000000003) * _tmp16 + _tmp19 * fv1;
  const Scalar _tmp142 = _tmp128 * _tmp96;
  const Scalar _tmp143 = Scalar(1.0) * _tmp131 * _tmp142 - Scalar(1.0) * _tmp131;
  const Scalar _tmp144 = -_tmp140 * _tmp81 - Scalar(5.1796800000000003) * _tmp25 - _tmp31 * fv1;
  const Scalar _tmp145 = _tmp131 * _tmp98;
  const Scalar _tmp146 = _tmp96 * (_tmp129 * _tmp131 + Scalar(1.0));
  const Scalar _tmp147 = Scalar(1.0) * _tmp145 - Scalar(1.0) * _tmp146;
  const Scalar _tmp148 = _tmp106 * _tmp131;
  const Scalar _tmp149 = -_tmp102 * _tmp74 + _tmp128 * _tmp148;
  const Scalar _tmp150 =
      Scalar(1.0) * _tmp113 * (_tmp107 * _tmp131 - _tmp135 * _tmp139) + _tmp114 * _tmp138 +
      Scalar(1.0) * _tmp125 * (_tmp122 * _tmp131 - _tmp134 * _tmp135) + _tmp141 * _tmp143 +
      _tmp144 * _tmp147 + Scalar(1.0) * _tmp82 * (-_tmp135 * _tmp149 + _tmp148);
  const Scalar _tmp151 = std::asinh(_tmp127 * _tmp150);
  const Scalar _tmp152 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp153 =
      -_tmp151 * _tmp152 - std::sqrt(Scalar(std::pow(Scalar(-_tmp37 + p_b(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp41 + p_b(1, 0)), Scalar(2))));
  const Scalar _tmp154 = Scalar(0.1034955) * _tmp127;
  const Scalar _tmp155 = _tmp153 * _tmp154;
  const Scalar _tmp156 = Scalar(1.0) * _tmp151;
  const Scalar _tmp157 = Scalar(9.6622558468725703) * _tmp118;
  const Scalar _tmp158 = std::pow(_tmp126, Scalar(-2));
  const Scalar _tmp159 = _tmp118 * _tmp158;
  const Scalar _tmp160 = _tmp21 + _tmp26 + _tmp38;
  const Scalar _tmp161 =
      (_tmp127 * (-_tmp138 + _tmp143 * _tmp19 + _tmp147 * _tmp160) - _tmp150 * _tmp159) /
      std::sqrt(Scalar(std::pow(_tmp150, Scalar(2)) * _tmp158 + 1));
  const Scalar _tmp162 = Scalar(0.1034955) * _tmp159;
  const Scalar _tmp163 = _tmp114 * _tmp116;
  const Scalar _tmp164 =
      _tmp111 * _tmp113 * _tmp68 + _tmp124 * _tmp125 * _tmp68 - _tmp163 * _tmp71 - _tmp76 * _tmp82;
  const Scalar _tmp165 = Scalar(1.0) / (_tmp164);
  const Scalar _tmp166 = _tmp131 * _tmp141;
  const Scalar _tmp167 = _tmp113 * _tmp139 * _tmp96 + _tmp114 * _tmp137 +
                         _tmp125 * _tmp134 * _tmp96 - _tmp142 * _tmp166 + _tmp144 * _tmp146 +
                         _tmp149 * _tmp82 * _tmp96;
  const Scalar _tmp168 = std::asinh(_tmp165 * _tmp167);
  const Scalar _tmp169 = Scalar(1.0) * _tmp168;
  const Scalar _tmp170 = std::pow(_tmp164, Scalar(-2));
  const Scalar _tmp171 = _tmp131 * _tmp19;
  const Scalar _tmp172 = _tmp117 * _tmp170;
  const Scalar _tmp173 =
      (_tmp165 * (-_tmp137 - _tmp142 * _tmp171 + _tmp146 * _tmp160) - _tmp167 * _tmp172) /
      std::sqrt(Scalar(std::pow(_tmp167, Scalar(2)) * _tmp170 + 1));
  const Scalar _tmp174 = Scalar(9.6622558468725703) * _tmp116;
  const Scalar _tmp175 = _tmp174 * _tmp71;
  const Scalar _tmp176 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp177 = Scalar(0.1034955) * _tmp165;
  const Scalar _tmp178 =
      -_tmp168 * _tmp176 - std::sqrt(Scalar(std::pow(Scalar(-_tmp51 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp54 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp179 = Scalar(0.1034955) * _tmp172;
  const Scalar _tmp180 = _tmp177 * _tmp178;
  const Scalar _tmp181 = -_tmp107 * _tmp113 * _tmp130 + _tmp114 * _tmp136 -
                         _tmp122 * _tmp125 * _tmp130 - _tmp144 * _tmp145 - _tmp148 * _tmp82 +
                         _tmp166;
  const Scalar _tmp182 =
      _tmp104 * _tmp108 * _tmp113 + _tmp104 * _tmp123 * _tmp125 + _tmp163 + _tmp74 * _tmp82;
  const Scalar _tmp183 = Scalar(1.0) / (_tmp182);
  const Scalar _tmp184 = std::asinh(_tmp181 * _tmp183);
  const Scalar _tmp185 = Scalar(1.0) * _tmp184;
  const Scalar _tmp186 = std::pow(_tmp182, Scalar(-2));
  const Scalar _tmp187 = _tmp116 * _tmp186;
  const Scalar _tmp188 = (_tmp181 * _tmp187 + _tmp183 * (-_tmp136 - _tmp145 * _tmp160 + _tmp171)) /
                         std::sqrt(Scalar(std::pow(_tmp181, Scalar(2)) * _tmp186 + 1));
  const Scalar _tmp189 = Scalar(0.1034955) * _tmp187;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp182;
  const Scalar _tmp191 =
      -_tmp184 * _tmp190 - std::sqrt(Scalar(std::pow(Scalar(-_tmp43 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp46 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp192 = Scalar(0.1034955) * _tmp183;
  const Scalar _tmp193 = _tmp191 * _tmp192;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp33 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_d(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp32 + p_d(1, 0)), Scalar(2)))))));
  _res(1, 0) =
      -_tmp152 * (Scalar(1.0) * _tmp161 * std::sinh(_tmp156) - _tmp162 * p_b(2, 0) -
                  (-_tmp153 * _tmp162 + _tmp154 * (-_tmp151 * _tmp157 - _tmp152 * _tmp161)) *
                      std::sinh(_tmp155)) -
      _tmp157 * (_tmp154 * p_b(2, 0) - std::cosh(_tmp155) + std::cosh(_tmp156));
  _res(2, 0) = -_tmp175 * (_tmp177 * p_c(2, 0) + std::cosh(_tmp169) - std::cosh(_tmp180)) -
               _tmp176 * (Scalar(1.0) * _tmp173 * std::sinh(_tmp169) - _tmp179 * p_c(2, 0) -
                          (_tmp177 * (-_tmp168 * _tmp175 - _tmp173 * _tmp176) - _tmp178 * _tmp179) *
                              std::sinh(_tmp180));
  _res(3, 0) = _tmp174 * (_tmp192 * p_a(2, 0) + std::cosh(_tmp185) - std::cosh(_tmp193)) -
               _tmp190 * (Scalar(1.0) * _tmp188 * std::sinh(_tmp185) + _tmp189 * p_a(2, 0) -
                          (_tmp189 * _tmp191 + _tmp192 * (_tmp174 * _tmp184 - _tmp188 * _tmp190)) *
                              std::sinh(_tmp193));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
