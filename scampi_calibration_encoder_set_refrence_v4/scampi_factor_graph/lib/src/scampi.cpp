                              ...
                              std::vector<Eigen::Matrix<double, 2, 1>> *static_force_collection_for_approving, gtsam::Values *oprimization_result_LM)
{
    ...
    double positionStdDev = 0.001; // in meters
    double orientationStdDev = 0.005* M_PI / 180.0; // convert degrees to radians
    ...
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(20));
    ...
    auto Sensor_noiseModel_prior = gtsam::noiseModel::Isotropic::Sigma(1, 0.001); 
    ...
    {
        if (i>3 && cable_length_collection[i][0]-cable_length_collection[i-1][0]<0.00001 
                && cable_length_collection[i][1]-cable_length_collection[i-1][1]<0.00001
                && cable_length_collection[i][2]-cable_length_collection[i-1][2]<0.00001
                && cable_length_collection[i][3]-cable_length_collection[i-1][3]<0.00001)
        {
            j +=1;
