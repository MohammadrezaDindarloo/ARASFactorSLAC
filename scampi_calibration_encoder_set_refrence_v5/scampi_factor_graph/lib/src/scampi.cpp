                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    std::uniform_real_distribution<double> cable_1(-0.02, 0.02);
    std::uniform_real_distribution<double> cable_2(-0.02, 0.02);
    std::uniform_real_distribution<double> cable_3(-0.02, 0.02);
    std::uniform_real_distribution<double> cable_4(-0.02, 0.02);
    
    double positionStdDev = 0.02; // in meters
    double orientationStdDev = 5.0 * M_PI / 180.0; // convert degrees to radians
    // Create a 6x6 matrix with 0.05 on the main diagonal and zeros elsewhere
    int rows = 6;
    int cols = 6;
    Eigen::MatrixXd covariance = Eigen::MatrixXd::Zero(rows, cols);
    // Set the standard deviations on the diagonal
    covariance.block<3, 3>(0, 0).diagonal().array() = positionStdDev;
    covariance.block<3, 3>(3, 3).diagonal().array() = orientationStdDev;

    Eigen::MatrixXd covariance_rot3 = Eigen::MatrixXd::Zero(3, 3);
    double DeltaorientationStdDev = 0.01 * M_PI / 180.0; // convert degrees to radians
    ...
    gtsam::noiseModel::Gaussian::shared_ptr noiseModel_pose3 = gtsam::noiseModel::Gaussian::Covariance(covariance);
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(300));
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));
    ...
    auto Sensor_noiseModel_cost5 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(50)); 
    auto Sensor_noiseModel_prior_enc = gtsam::noiseModel::Isotropic::Sigma(1, 0.01); 
    auto Sensor_noiseModel_prior_uwb = gtsam::noiseModel::Isotropic::Sigma(1, 0.01); 
    ...
    for (size_t i = 0; i < p_platform_collection.size()-1; i++)
    {
        ...
        double uwb_data_1 = (b_in_w_collection[i][0] - p_in_w_collection[i][0]).norm() + random_noise_encoder_1;
        double uwb_data_2 = (b_in_w_collection[i][1] - p_in_w_collection[i][1]).norm() + random_noise_encoder_2;
        double uwb_data_3 = (b_in_w_collection[i][2] - p_in_w_collection[i][2]).norm() + random_noise_encoder_3;
        double uwb_data_4 = (b_in_w_collection[i][3] - p_in_w_collection[i][3]).norm() + random_noise_encoder_4;
        ...
        graph.add(std::make_shared<FK_factor_graph_cost1>(Symbol('h', i), Symbol('v', i), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('X', i), Symbol('D', i), Sensor_noiseModel_cost1));
        graph.add(std::make_shared<FK_factor_graph_cost2>(Symbol('h', i), Symbol('v', i), Symbol('o', 0), Symbol('o', 1), Symbol('o', 2), Symbol('o', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('X', i), Symbol('D', i), encodr_data, Sensor_noiseModel_cost2));
        graph.add(std::make_shared<FK_factor_graph_cost3>(Symbol('h', i), Symbol('v', i), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('X', i), Symbol('D', i), Sensor_noiseModel_cost3));
        graph.add(std::make_shared<FK_factor_graph_cost4>(Symbol('o', 0), Symbol('o', 1), Symbol('o', 2), Symbol('o', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('h', i), Symbol('v', i), Symbol('X', i), Symbol('D', i), encodr_data, Sensor_noiseModel_cost4));
        graph.add(std::make_shared<FK_factor_graph_cost5>(Symbol('w', 0), Symbol('w', 1), Symbol('w', 2), Symbol('w', 3), Symbol('p', 1), Symbol('p', 2), Symbol('p', 3), Symbol('p', 4), Symbol('h', i), Symbol('v', i), Symbol('X', i), Symbol('D', i), uwb_data, Sensor_noiseModel_cost5));
        ...
        // graph.emplace_shared<gtsam::PriorFactor<gtsam::Rot3>>(Symbol('D', i), DeltaRot, noiseModel_rot3);
    }
    // graph.add(gtsam::PriorFactor<double>(Symbol('o', 0), 0.0, Sensor_noiseModel_prior_enc));
    // graph.add(gtsam::PriorFactor<double>(Symbol('o', 1), 0.0, Sensor_noiseModel_prior_enc));
    // graph.add(gtsam::PriorFactor<double>(Symbol('o', 2), 0.0, Sensor_noiseModel_prior_enc));
    // graph.add(gtsam::PriorFactor<double>(Symbol('o', 3), 0.0, Sensor_noiseModel_prior_enc));

    graph.add(gtsam::PriorFactor<double>(Symbol('w', 0), 0.0, Sensor_noiseModel_prior_uwb));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 1), 0.0, Sensor_noiseModel_prior_uwb));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 2), 0.0, Sensor_noiseModel_prior_uwb));
    graph.add(gtsam::PriorFactor<double>(Symbol('w', 3), 0.0, Sensor_noiseModel_prior_uwb));
    ...
    initial_estimate.insert(Symbol('w', 0), 0.0);
    initial_estimate.insert(Symbol('w', 1), 0.0);
    initial_estimate.insert(Symbol('w', 2), 0.0);
    initial_estimate.insert(Symbol('w', 3), 0.0);
    for (size_t i = 0; i < p_platform_collection.size()-1; i++)
    {
        ...
        initial_estimate.insert(Symbol('D', i), DeltaRot);  
    }
    ...
    std::cout << std::endl << "ofset ENC 0: " << result_LM.at<double>(Symbol('o', 0)) << std::endl;
    std::cout << std::endl << "ofset ENC 1: " << result_LM.at<double>(Symbol('o', 1)) << std::endl;
    std::cout << std::endl << "ofset ENC 2: " << result_LM.at<double>(Symbol('o', 2)) << std::endl;
    std::cout << std::endl << "ofset ENC 3: " << result_LM.at<double>(Symbol('o', 3)) << std::endl;
    std::cout << std::endl << "ofset UWB 0: " << result_LM.at<double>(Symbol('w', 0)) << std::endl;
    std::cout << std::endl << "ofset UWB 1: " << result_LM.at<double>(Symbol('w', 1)) << std::endl;
    std::cout << std::endl << "ofset UWB 2: " << result_LM.at<double>(Symbol('w', 2)) << std::endl;
    std::cout << std::endl << "ofset UWB 3: " << result_LM.at<double>(Symbol('w', 3)) << std::endl;
