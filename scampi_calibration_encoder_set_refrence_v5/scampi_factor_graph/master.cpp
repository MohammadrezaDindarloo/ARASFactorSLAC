...
{   
    ...
    std::ifstream file_position_for_size_of_data("./dataset/pos_i_cpp_test.csv");
    std::vector<std::vector<double>> real_data_position;
    if (file_position_for_size_of_data) {
        std::string line;
        while (getline(file_position_for_size_of_data, line)) {
    ...
    std::cout << "Number of ClibrationDataSet: " << data.size() << std::endl;
    } else {
        std::cout << "Unable to open file." << std::endl;
    }
