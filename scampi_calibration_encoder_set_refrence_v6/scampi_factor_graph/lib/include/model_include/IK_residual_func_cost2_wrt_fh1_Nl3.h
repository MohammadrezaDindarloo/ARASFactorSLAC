// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl3
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl3(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 622

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (212)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _tmp0 * fv1;
  const Scalar _tmp2 = std::asinh(_tmp1);
  const Scalar _tmp3 = Scalar(9.6622558468725703) * _tmp2;
  const Scalar _tmp4 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp5 = -2 * std::pow(_tmp4, Scalar(2));
  const Scalar _tmp6 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp7 = 1 - 2 * std::pow(_tmp6, Scalar(2));
  const Scalar _tmp8 = Scalar(0.20999999999999999) * _tmp5 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp9 = -_tmp8;
  const Scalar _tmp10 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                        _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp11 = 2 * _tmp6;
  const Scalar _tmp12 = _tmp10 * _tmp11;
  const Scalar _tmp13 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp14 = _tmp13 * _tmp4;
  const Scalar _tmp15 = _tmp12 + _tmp14;
  const Scalar _tmp16 = -Scalar(0.010999999999999999) * _tmp15;
  const Scalar _tmp17 = 2 * _tmp10 * _tmp4;
  const Scalar _tmp18 = _tmp13 * _tmp6;
  const Scalar _tmp19 = Scalar(0.20999999999999999) * _tmp17 - Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp20 = _tmp16 - _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp9;
  const Scalar _tmp22 = _tmp21 + p_init0;
  const Scalar _tmp23 = -2 * std::pow(_tmp10, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp7;
  const Scalar _tmp25 = -_tmp24;
  const Scalar _tmp26 = Scalar(0.20999999999999999) * _tmp17 + Scalar(0.20999999999999999) * _tmp18;
  const Scalar _tmp27 = _tmp11 * _tmp4;
  const Scalar _tmp28 = _tmp10 * _tmp13;
  const Scalar _tmp29 = _tmp27 - _tmp28;
  const Scalar _tmp30 = -Scalar(0.010999999999999999) * _tmp29;
  const Scalar _tmp31 = -_tmp26 + _tmp30;
  const Scalar _tmp32 = _tmp25 + _tmp31;
  const Scalar _tmp33 = _tmp32 + p_init1;
  const Scalar _tmp34 =
      -_tmp3 * fh1 - std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_a(0, 0)), Scalar(2)) +
                                      std::pow(Scalar(-_tmp33 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp35 = Scalar(0.1034955) * _tmp0;
  const Scalar _tmp36 = _tmp34 * _tmp35;
  const Scalar _tmp37 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp38 =
      std::pow(Scalar(_tmp37 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp39 = Scalar(1.0) * _tmp2;
  const Scalar _tmp40 = _tmp22 - p_a(0, 0);
  const Scalar _tmp41 = _tmp33 - p_a(1, 0);
  const Scalar _tmp42 = std::pow(Scalar(std::pow(_tmp40, Scalar(2)) + std::pow(_tmp41, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp43 = _tmp41 * _tmp42;
  const Scalar _tmp44 = _tmp40 * _tmp42;
  const Scalar _tmp45 = _tmp21 * _tmp43 - _tmp32 * _tmp44;
  const Scalar _tmp46 = _tmp26 + _tmp30;
  const Scalar _tmp47 = _tmp25 + _tmp46;
  const Scalar _tmp48 = _tmp47 + p_init1;
  const Scalar _tmp49 = _tmp48 - p_b(1, 0);
  const Scalar _tmp50 = _tmp20 + _tmp8;
  const Scalar _tmp51 = _tmp50 + p_init0;
  const Scalar _tmp52 = _tmp51 - p_b(0, 0);
  const Scalar _tmp53 = std::pow(Scalar(std::pow(_tmp49, Scalar(2)) + std::pow(_tmp52, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp54 = _tmp52 * _tmp53;
  const Scalar _tmp55 = _tmp49 * _tmp53;
  const Scalar _tmp56 = _tmp16 + _tmp19;
  const Scalar _tmp57 = _tmp56 + _tmp8;
  const Scalar _tmp58 = _tmp57 + p_init0;
  const Scalar _tmp59 = _tmp58 - p_c(0, 0);
  const Scalar _tmp60 = _tmp24 + _tmp46;
  const Scalar _tmp61 = _tmp60 + p_init1;
  const Scalar _tmp62 = _tmp61 - p_c(1, 0);
  const Scalar _tmp63 =
      std::sqrt(Scalar(std::pow(_tmp59, Scalar(2)) + std::pow(_tmp62, Scalar(2))));
  const Scalar _tmp64 = Scalar(1.0) / (_tmp63);
  const Scalar _tmp65 = Scalar(1.0) / (_tmp59);
  const Scalar _tmp66 = _tmp63 * _tmp65;
  const Scalar _tmp67 = _tmp66 * (_tmp57 * _tmp62 * _tmp64 - _tmp59 * _tmp60 * _tmp64);
  const Scalar _tmp68 = _tmp62 * _tmp65;
  const Scalar _tmp69 = _tmp54 * _tmp68 - _tmp55;
  const Scalar _tmp70 = _tmp24 + _tmp31;
  const Scalar _tmp71 = _tmp70 + p_init1;
  const Scalar _tmp72 = _tmp71 - p_d(1, 0);
  const Scalar _tmp73 = _tmp56 + _tmp9;
  const Scalar _tmp74 = _tmp73 + p_init0;
  const Scalar _tmp75 = _tmp74 - p_d(0, 0);
  const Scalar _tmp76 = std::pow(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp75, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = _tmp72 * _tmp76;
  const Scalar _tmp79 = Scalar(1.0) / (_tmp68 * _tmp77 - _tmp78);
  const Scalar _tmp80 = _tmp79 * (_tmp67 * _tmp77 + _tmp70 * _tmp77 - _tmp73 * _tmp78);
  const Scalar _tmp81 = _tmp47 * _tmp54 - _tmp50 * _tmp55 + _tmp54 * _tmp67 - _tmp69 * _tmp80;
  const Scalar _tmp82 = Scalar(1.0) / (_tmp81);
  const Scalar _tmp83 = Scalar(1.0) * _tmp82;
  const Scalar _tmp84 = _tmp69 * _tmp79;
  const Scalar _tmp85 = _tmp45 * _tmp66 * (_tmp54 * _tmp83 - _tmp77 * _tmp83 * _tmp84);
  const Scalar _tmp86 = Scalar(0.20999999999999999) * _tmp27 + Scalar(0.20999999999999999) * _tmp28;
  const Scalar _tmp87 = -Scalar(0.010999999999999999) * _tmp23 -
                        Scalar(0.010999999999999999) * _tmp5 + Scalar(-0.010999999999999999);
  const Scalar _tmp88 = Scalar(0.20999999999999999) * _tmp12 - Scalar(0.20999999999999999) * _tmp14;
  const Scalar _tmp89 = _tmp87 + _tmp88;
  const Scalar _tmp90 = _tmp86 + _tmp89;
  const Scalar _tmp91 = _tmp54 * _tmp90;
  const Scalar _tmp92 = -_tmp86;
  const Scalar _tmp93 = _tmp89 + _tmp92;
  const Scalar _tmp94 = _tmp77 * _tmp90;
  const Scalar _tmp95 = _tmp87 - _tmp88;
  const Scalar _tmp96 = _tmp86 + _tmp95;
  const Scalar _tmp97 = _tmp79 * (-_tmp77 * _tmp96 + _tmp94);
  const Scalar _tmp98 = _tmp79 * (-_tmp68 * _tmp94 + _tmp78 * _tmp96);
  const Scalar _tmp99 = _tmp55 * _tmp93 - _tmp68 * _tmp91 - _tmp69 * _tmp98;
  const Scalar _tmp100 = Scalar(1.0) * _tmp60;
  const Scalar _tmp101 = -_tmp100;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp101 + _tmp70);
  const Scalar _tmp103 = Scalar(1.0) * _tmp57;
  const Scalar _tmp104 = _tmp102 * (_tmp103 - _tmp73);
  const Scalar _tmp105 = -_tmp104 * _tmp99 - _tmp54 * _tmp93 - _tmp69 * _tmp97 + _tmp91;
  const Scalar _tmp106 = Scalar(1.0) / (_tmp105);
  const Scalar _tmp107 = _tmp68 * _tmp90 + _tmp68 * _tmp98;
  const Scalar _tmp108 = -_tmp104 * _tmp107 + _tmp68 * _tmp97 - _tmp90;
  const Scalar _tmp109 = _tmp106 * _tmp81;
  const Scalar _tmp110 = _tmp105 * _tmp82;
  const Scalar _tmp111 = _tmp110 * (-_tmp108 * _tmp109 - _tmp67 + _tmp68 * _tmp80);
  const Scalar _tmp112 = _tmp108 + _tmp111;
  const Scalar _tmp113 = _tmp106 * _tmp112;
  const Scalar _tmp114 = _tmp106 * _tmp69;
  const Scalar _tmp115 = _tmp79 * (-_tmp112 * _tmp114 - _tmp68);
  const Scalar _tmp116 = _tmp44 * _tmp66 * (_tmp113 * _tmp54 + _tmp115 * _tmp77 + Scalar(1.0));
  const Scalar _tmp117 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp118 = _tmp100 * _tmp104 + _tmp103;
  const Scalar _tmp119 = 0;
  const Scalar _tmp120 = _tmp114 * _tmp119 * _tmp79;
  const Scalar _tmp121 = _tmp106 * _tmp119;
  const Scalar _tmp122 = Scalar(1.0) * _tmp98;
  const Scalar _tmp123 = _tmp104 * _tmp122 - Scalar(1.0) * _tmp97;
  const Scalar _tmp124 = _tmp110 * (-_tmp109 * _tmp123 - Scalar(1.0) * _tmp80);
  const Scalar _tmp125 = _tmp123 + _tmp124;
  const Scalar _tmp126 = _tmp106 * _tmp125;
  const Scalar _tmp127 = _tmp79 * (-_tmp114 * _tmp125 + Scalar(1.0));
  const Scalar _tmp128 = _tmp43 * _tmp66 * (_tmp126 * _tmp54 + _tmp127 * _tmp77);
  const Scalar _tmp129 = -_tmp116 * fh1 -
                         _tmp117 * _tmp66 * (-_tmp120 * _tmp77 + _tmp121 * _tmp54) - _tmp128 * fh1 -
                         _tmp85 * fh1;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp101 + _tmp47;
  const Scalar _tmp132 = _tmp104 * _tmp131;
  const Scalar _tmp133 = Scalar(1.0) / (_tmp103 - _tmp132 - _tmp50);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp106 * _tmp99;
  const Scalar _tmp136 = _tmp118 * _tmp133;
  const Scalar _tmp137 = _tmp101 - _tmp119 * _tmp135 - _tmp131 * _tmp136;
  const Scalar _tmp138 = Scalar(1.0) * _tmp102;
  const Scalar _tmp139 = _tmp131 * _tmp133;
  const Scalar _tmp140 = -_tmp122 + _tmp124 * _tmp139 - _tmp125 * _tmp135;
  const Scalar _tmp141 = Scalar(1.0) * _tmp43 * (_tmp124 * _tmp134 - _tmp138 * _tmp140);
  const Scalar _tmp142 = _tmp92 + _tmp95;
  const Scalar _tmp143 = _tmp142 * fh1;
  const Scalar _tmp144 = -_tmp143 * _tmp43 - Scalar(3.29616) * _tmp29 - _tmp32 * fv1;
  const Scalar _tmp145 = _tmp102 * (_tmp132 * _tmp134 + Scalar(1.0));
  const Scalar _tmp146 = _tmp104 * _tmp134;
  const Scalar _tmp147 = -Scalar(1.0) * _tmp145 + Scalar(1.0) * _tmp146;
  const Scalar _tmp148 = _tmp107 + _tmp111 * _tmp139 - _tmp112 * _tmp135;
  const Scalar _tmp149 = Scalar(1.0) * _tmp44 * (_tmp111 * _tmp134 - _tmp138 * _tmp148);
  const Scalar _tmp150 = _tmp143 * _tmp44 + Scalar(3.29616) * _tmp15 + _tmp21 * fv1;
  const Scalar _tmp151 = _tmp131 * _tmp134;
  const Scalar _tmp152 = _tmp102 * _tmp151;
  const Scalar _tmp153 = -Scalar(1.0) * _tmp134 + Scalar(1.0) * _tmp152;
  const Scalar _tmp154 = _tmp102 * (_tmp110 * _tmp151 - _tmp83 * _tmp99);
  const Scalar _tmp155 = _tmp110 * _tmp134;
  const Scalar _tmp156 = Scalar(1.0) * _tmp45 * (-Scalar(1.0) * _tmp154 + _tmp155);
  const Scalar _tmp157 =
      Scalar(1.0) * _tmp117 * (-_tmp118 * _tmp134 - _tmp137 * _tmp138 + Scalar(1.0)) +
      _tmp141 * fh1 + _tmp144 * _tmp147 + _tmp149 * fh1 + _tmp150 * _tmp153 + _tmp156 * fh1;
  const Scalar _tmp158 = std::asinh(_tmp130 * _tmp157);
  const Scalar _tmp159 = Scalar(1.0) * _tmp158;
  const Scalar _tmp160 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp161 =
      -_tmp158 * _tmp160 - std::sqrt(Scalar(std::pow(Scalar(-_tmp58 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp61 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp162 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp163 = _tmp161 * _tmp162;
  const Scalar _tmp164 = -_tmp116 - _tmp128 - _tmp85;
  const Scalar _tmp165 = Scalar(9.6622558468725703) * _tmp164;
  const Scalar _tmp166 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp167 = _tmp164 * _tmp166;
  const Scalar _tmp168 = _tmp142 * _tmp44;
  const Scalar _tmp169 = _tmp142 * _tmp43;
  const Scalar _tmp170 =
      (_tmp130 * (_tmp141 - _tmp147 * _tmp169 + _tmp149 + _tmp153 * _tmp168 + _tmp156) -
       _tmp157 * _tmp167) /
      std::sqrt(Scalar(std::pow(_tmp157, Scalar(2)) * _tmp166 + 1));
  const Scalar _tmp171 = _tmp102 * _tmp140 * _tmp43;
  const Scalar _tmp172 = _tmp154 * _tmp45;
  const Scalar _tmp173 = _tmp134 * _tmp150;
  const Scalar _tmp174 = _tmp102 * _tmp148 * _tmp44;
  const Scalar _tmp175 = _tmp102 * _tmp117 * _tmp137 - _tmp102 * _tmp131 * _tmp173 +
                         _tmp144 * _tmp145 + _tmp171 * fh1 + _tmp172 * fh1 + _tmp174 * fh1;
  const Scalar _tmp176 = _tmp45 * _tmp83;
  const Scalar _tmp177 = _tmp176 * fh1;
  const Scalar _tmp178 = _tmp127 * _tmp43;
  const Scalar _tmp179 = _tmp115 * _tmp44;
  const Scalar _tmp180 = -_tmp117 * _tmp120 - _tmp177 * _tmp84 + _tmp178 * fh1 + _tmp179 * fh1;
  const Scalar _tmp181 = Scalar(1.0) / (_tmp180);
  const Scalar _tmp182 = std::asinh(_tmp175 * _tmp181);
  const Scalar _tmp183 = Scalar(1.0) * _tmp182;
  const Scalar _tmp184 = std::pow(_tmp180, Scalar(-2));
  const Scalar _tmp185 = -_tmp176 * _tmp84 + _tmp178 + _tmp179;
  const Scalar _tmp186 = _tmp184 * _tmp185;
  const Scalar _tmp187 = (-_tmp175 * _tmp186 + _tmp181 * (-_tmp145 * _tmp169 - _tmp152 * _tmp168 +
                                                          _tmp171 + _tmp172 + _tmp174)) /
                         std::sqrt(Scalar(std::pow(_tmp175, Scalar(2)) * _tmp184 + 1));
  const Scalar _tmp188 = Scalar(9.6622558468725703) * _tmp185;
  const Scalar _tmp189 = Scalar(9.6622558468725703) * _tmp180;
  const Scalar _tmp190 = Scalar(0.1034955) * _tmp181;
  const Scalar _tmp191 =
      -_tmp182 * _tmp189 - std::sqrt(Scalar(std::pow(Scalar(-_tmp71 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp74 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp192 = _tmp190 * _tmp191;
  const Scalar _tmp193 = _tmp113 * _tmp44;
  const Scalar _tmp194 = _tmp126 * _tmp43;
  const Scalar _tmp195 = _tmp117 * _tmp121 + _tmp177 + _tmp193 * fh1 + _tmp194 * fh1;
  const Scalar _tmp196 = Scalar(1.0) / (_tmp195);
  const Scalar _tmp197 = _tmp124 * _tmp133 * _tmp43;
  const Scalar _tmp198 = _tmp155 * _tmp45;
  const Scalar _tmp199 = _tmp111 * _tmp133 * _tmp44;
  const Scalar _tmp200 = _tmp117 * _tmp136 - _tmp144 * _tmp146 + _tmp173 - _tmp197 * fh1 -
                         _tmp198 * fh1 - _tmp199 * fh1;
  const Scalar _tmp201 = std::asinh(_tmp196 * _tmp200);
  const Scalar _tmp202 = Scalar(1.0) * _tmp201;
  const Scalar _tmp203 = Scalar(9.6622558468725703) * _tmp195;
  const Scalar _tmp204 =
      -_tmp201 * _tmp203 - std::sqrt(Scalar(std::pow(Scalar(-_tmp48 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp51 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp205 = Scalar(0.1034955) * _tmp196;
  const Scalar _tmp206 = _tmp204 * _tmp205;
  const Scalar _tmp207 = _tmp176 + _tmp193 + _tmp194;
  const Scalar _tmp208 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp209 = std::pow(_tmp195, Scalar(-2));
  const Scalar _tmp210 = _tmp207 * _tmp209;
  const Scalar _tmp211 =
      (_tmp196 * (_tmp134 * _tmp168 + _tmp146 * _tmp169 - _tmp197 - _tmp198 - _tmp199) -
       _tmp200 * _tmp210) /
      std::sqrt(Scalar(std::pow(_tmp200, Scalar(2)) * _tmp209 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) = Scalar(9.6622558468725703) * fh1 *
                   (Scalar(1.0) * _tmp37 * _tmp38 * fv1 * std::cosh(_tmp39) -
                    (-Scalar(0.1034955) * _tmp34 * _tmp37 +
                     _tmp35 * (Scalar(9.6622558468725703) * _tmp1 * _tmp38 - _tmp3)) *
                        std::cosh(_tmp36)) -
               Scalar(9.6622558468725703) * std::sinh(_tmp36) -
               Scalar(9.6622558468725703) * std::sinh(_tmp39);
  _res(1, 0) = _tmp160 * (-Scalar(1.0) * _tmp170 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp161 * _tmp167 +
                           _tmp162 * (-_tmp158 * _tmp165 - _tmp160 * _tmp170)) *
                              std::cosh(_tmp163)) +
               _tmp165 * (-std::sinh(_tmp159) - std::sinh(_tmp163));
  _res(2, 0) = _tmp188 * (-std::sinh(_tmp183) - std::sinh(_tmp192)) +
               _tmp189 * (-Scalar(1.0) * _tmp187 * std::cosh(_tmp183) -
                          (-Scalar(0.1034955) * _tmp186 * _tmp191 +
                           _tmp190 * (-_tmp182 * _tmp188 - _tmp187 * _tmp189)) *
                              std::cosh(_tmp192));
  _res(3, 0) = _tmp203 * (-Scalar(1.0) * _tmp211 * std::cosh(_tmp202) -
                          (-Scalar(0.1034955) * _tmp204 * _tmp210 +
                           _tmp205 * (-_tmp201 * _tmp208 - _tmp203 * _tmp211)) *
                              std::cosh(_tmp206)) +
               _tmp208 * (-std::sinh(_tmp202) - std::sinh(_tmp206));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
