                              ...
                              gtsam::Rot3 DeltaRot, std::vector<Eigen::Matrix<double, 3, 1>> p_platform_collection, gtsam::Values *oprimization_result_LM)
{
    ...
    std::uniform_real_distribution<double> cable_1(-0.01, 0.01);
    std::uniform_real_distribution<double> cable_2(-0.01, 0.01);
    std::uniform_real_distribution<double> cable_3(-0.01, 0.01);
    std::uniform_real_distribution<double> cable_4(-0.01, 0.01);
 
    double positionStdDev = 0.02; // in meters
    double orientationStdDev = 5.0 * M_PI / 180.0; // convert degrees to radians
    // Create a 6x6 matrix with ... on the main diagonal and zeros elsewhere
    int rows = 6;
    int cols = 6;
    ...
    double prior_positionStdDev = 0.00001; // in meters
    double prior_orientationStdDev = 0.00001* M_PI / 180.0; // convert degrees to radians
    Eigen::MatrixXd covariance_prior = Eigen::MatrixXd::Zero(rows, cols);
    // Set the standard deviations on the diagonal
    covariance_prior.block<3, 3>(0, 0).diagonal().array() = positionStdDev;
    covariance_prior.block<3, 3>(3, 3).diagonal().array() = orientationStdDev;

    Eigen::MatrixXd covariance_rot3 = Eigen::MatrixXd::Zero(3, 3);
    double DeltaorientationStdDev = 8.0 * M_PI / 180.0; // convert degrees to radians
    ...
    gtsam::noiseModel::Gaussian::shared_ptr prior_noiseModel_pose3 = gtsam::noiseModel::Gaussian::Covariance(covariance_prior);
    auto Sensor_noiseModel_cost1 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(300)); //300
    auto Sensor_noiseModel_cost2 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));  //10
    auto Sensor_noiseModel_cost3 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(1));
    auto Sensor_noiseModel_cost4 = gtsam::noiseModel::Isotropic::Sigma(4, sqrt(10));  //10
    auto Sensor_noiseModel_prior = gtsam::noiseModel::Isotropic::Sigma(1, 0.01); 
