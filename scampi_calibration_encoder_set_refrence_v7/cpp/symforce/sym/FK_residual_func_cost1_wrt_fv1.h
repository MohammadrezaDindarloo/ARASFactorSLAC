// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/pose3.h>
#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: FK_residual_func_cost1_wrt_fv1
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     DeltaRot: Rot3
 *     TransformationMatrix: Pose3
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> FkResidualFuncCost1WrtFv1(
    const Scalar fh1, const Scalar fv1, const Eigen::Matrix<Scalar, 3, 1>& p_a,
    const Eigen::Matrix<Scalar, 3, 1>& p_b, const Eigen::Matrix<Scalar, 3, 1>& p_c,
    const Eigen::Matrix<Scalar, 3, 1>& p_d, const sym::Rot3<Scalar>& DeltaRot,
    const sym::Pose3<Scalar>& TransformationMatrix, const Scalar epsilon) {
  // Total ops: 589

  // Unused inputs
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();
  const Eigen::Matrix<Scalar, 7, 1>& _TransformationMatrix = TransformationMatrix.Data();

  // Intermediate terms (192)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 =
      _DeltaRot[0] * _TransformationMatrix[3] - _DeltaRot[1] * _TransformationMatrix[2] +
      _DeltaRot[2] * _TransformationMatrix[1] + _DeltaRot[3] * _TransformationMatrix[0];
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 =
      -_DeltaRot[0] * _TransformationMatrix[1] + _DeltaRot[1] * _TransformationMatrix[0] +
      _DeltaRot[2] * _TransformationMatrix[3] + _DeltaRot[3] * _TransformationMatrix[2];
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = -_tmp7;
  const Scalar _tmp9 =
      _DeltaRot[0] * _TransformationMatrix[2] + _DeltaRot[1] * _TransformationMatrix[3] -
      _DeltaRot[2] * _TransformationMatrix[0] + _DeltaRot[3] * _TransformationMatrix[1];
  const Scalar _tmp10 = 2 * _tmp3 * _tmp9;
  const Scalar _tmp11 =
      -2 * _DeltaRot[0] * _TransformationMatrix[0] - 2 * _DeltaRot[1] * _TransformationMatrix[1] -
      2 * _DeltaRot[2] * _TransformationMatrix[2] + 2 * _DeltaRot[3] * _TransformationMatrix[3];
  const Scalar _tmp12 = _tmp11 * _tmp5;
  const Scalar _tmp13 = Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp5;
  const Scalar _tmp15 = _tmp14 * _tmp9;
  const Scalar _tmp16 = _tmp11 * _tmp3;
  const Scalar _tmp17 = _tmp15 - _tmp16;
  const Scalar _tmp18 = Scalar(0.010999999999999999) * _tmp17;
  const Scalar _tmp19 = -_tmp18;
  const Scalar _tmp20 = -_tmp13 + _tmp19;
  const Scalar _tmp21 = _tmp20 + _tmp8;
  const Scalar _tmp22 = _TransformationMatrix[5] + _tmp21;
  const Scalar _tmp23 = -2 * std::pow(_tmp9, Scalar(2));
  const Scalar _tmp24 = Scalar(0.20999999999999999) * _tmp23 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp25 = -_tmp24;
  const Scalar _tmp26 = _tmp14 * _tmp3;
  const Scalar _tmp27 = _tmp11 * _tmp9;
  const Scalar _tmp28 = _tmp26 + _tmp27;
  const Scalar _tmp29 = -Scalar(0.010999999999999999) * _tmp28;
  const Scalar _tmp30 = Scalar(0.20999999999999999) * _tmp10 - Scalar(0.20999999999999999) * _tmp12;
  const Scalar _tmp31 = _tmp29 - _tmp30;
  const Scalar _tmp32 = _tmp25 + _tmp31;
  const Scalar _tmp33 = _TransformationMatrix[4] + _tmp32;
  const Scalar _tmp34 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp35 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp36 = _tmp13 + _tmp19 + _tmp8;
  const Scalar _tmp37 = _TransformationMatrix[5] + _tmp36;
  const Scalar _tmp38 = _tmp37 - p_b(1, 0);
  const Scalar _tmp39 = _tmp24 + _tmp31;
  const Scalar _tmp40 = _TransformationMatrix[4] + _tmp39;
  const Scalar _tmp41 = _tmp40 - p_b(0, 0);
  const Scalar _tmp42 = Scalar(1.0) / (_tmp41);
  const Scalar _tmp43 = _tmp38 * _tmp42;
  const Scalar _tmp44 = _tmp29 + _tmp30;
  const Scalar _tmp45 = _tmp24 + _tmp44;
  const Scalar _tmp46 = _TransformationMatrix[4] + _tmp45;
  const Scalar _tmp47 = _tmp46 - p_c(0, 0);
  const Scalar _tmp48 = _tmp13 + _tmp7;
  const Scalar _tmp49 = _tmp19 + _tmp48;
  const Scalar _tmp50 = _TransformationMatrix[5] + _tmp49;
  const Scalar _tmp51 = _tmp50 - p_c(1, 0);
  const Scalar _tmp52 = std::pow(Scalar(std::pow(_tmp47, Scalar(2)) + std::pow(_tmp51, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp53 = _tmp47 * _tmp52;
  const Scalar _tmp54 = _tmp51 * _tmp52;
  const Scalar _tmp55 = Scalar(1.0) / (_tmp43 * _tmp53 - _tmp54);
  const Scalar _tmp56 = _tmp53 * _tmp55;
  const Scalar _tmp57 = Scalar(1.0) * _tmp39;
  const Scalar _tmp58 = Scalar(1.0) * _tmp36;
  const Scalar _tmp59 = -_tmp58;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp49 + _tmp59);
  const Scalar _tmp61 = -_tmp45 + _tmp57;
  const Scalar _tmp62 = _tmp60 * _tmp61;
  const Scalar _tmp63 = _tmp57 + _tmp58 * _tmp62;
  const Scalar _tmp64 = 0;
  const Scalar _tmp65 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp66 = -Scalar(0.010999999999999999) * _tmp23 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp67 = Scalar(0.20999999999999999) * _tmp26 - Scalar(0.20999999999999999) * _tmp27;
  const Scalar _tmp68 = _tmp66 - _tmp67;
  const Scalar _tmp69 = _tmp65 + _tmp68;
  const Scalar _tmp70 = _tmp20 + _tmp7;
  const Scalar _tmp71 = _TransformationMatrix[5] + _tmp70;
  const Scalar _tmp72 = _tmp71 - p_d(1, 0);
  const Scalar _tmp73 = _tmp25 + _tmp44;
  const Scalar _tmp74 = _TransformationMatrix[4] + _tmp73;
  const Scalar _tmp75 = _tmp74 - p_d(0, 0);
  const Scalar _tmp76 = std::pow(Scalar(std::pow(_tmp72, Scalar(2)) + std::pow(_tmp75, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp77 = _tmp75 * _tmp76;
  const Scalar _tmp78 = -_tmp65;
  const Scalar _tmp79 = _tmp66 + _tmp67;
  const Scalar _tmp80 = _tmp78 + _tmp79;
  const Scalar _tmp81 = _tmp72 * _tmp76;
  const Scalar _tmp82 = _tmp43 * _tmp77 - _tmp81;
  const Scalar _tmp83 = _tmp65 + _tmp79;
  const Scalar _tmp84 = _tmp55 * (_tmp53 * _tmp80 - _tmp53 * _tmp83);
  const Scalar _tmp85 = _tmp43 * _tmp80;
  const Scalar _tmp86 = _tmp55 * (-_tmp53 * _tmp85 + _tmp54 * _tmp83);
  const Scalar _tmp87 = _tmp69 * _tmp81 - _tmp77 * _tmp85 - _tmp82 * _tmp86;
  const Scalar _tmp88 = -_tmp62 * _tmp87 - _tmp69 * _tmp77 + _tmp77 * _tmp80 - _tmp82 * _tmp84;
  const Scalar _tmp89 = Scalar(1.0) / (_tmp88);
  const Scalar _tmp90 = _tmp82 * _tmp89;
  const Scalar _tmp91 = _tmp64 * _tmp90;
  const Scalar _tmp92 = _tmp64 * _tmp89;
  const Scalar _tmp93 =
      std::sqrt(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp41, Scalar(2))));
  const Scalar _tmp94 = _tmp42 * _tmp93;
  const Scalar _tmp95 = _tmp94 * (-_tmp56 * _tmp91 + _tmp77 * _tmp92);
  const Scalar _tmp96 = Scalar(1.0) * _tmp60;
  const Scalar _tmp97 = _tmp61 * _tmp86 * _tmp96 - Scalar(1.0) * _tmp84;
  const Scalar _tmp98 = Scalar(1.0) / (_tmp93);
  const Scalar _tmp99 = _tmp94 * (-_tmp36 * _tmp41 * _tmp98 + _tmp38 * _tmp39 * _tmp98);
  const Scalar _tmp100 = _tmp55 * (-_tmp45 * _tmp54 + _tmp49 * _tmp53 + _tmp53 * _tmp99);
  const Scalar _tmp101 = -_tmp100 * _tmp82 + _tmp70 * _tmp77 - _tmp73 * _tmp81 + _tmp77 * _tmp99;
  const Scalar _tmp102 = _tmp101 * _tmp89;
  const Scalar _tmp103 = Scalar(1.0) / (_tmp101);
  const Scalar _tmp104 = _tmp103 * _tmp88;
  const Scalar _tmp105 = _tmp104 * (-Scalar(1.0) * _tmp100 - _tmp102 * _tmp97);
  const Scalar _tmp106 = _tmp105 + _tmp97;
  const Scalar _tmp107 = -_tmp106 * _tmp90 + Scalar(1.0);
  const Scalar _tmp108 = _tmp77 * _tmp89;
  const Scalar _tmp109 = _tmp33 - p_a(0, 0);
  const Scalar _tmp110 = _tmp22 - p_a(1, 0);
  const Scalar _tmp111 =
      std::pow(Scalar(std::pow(_tmp109, Scalar(2)) + std::pow(_tmp110, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp112 = _tmp110 * _tmp111;
  const Scalar _tmp113 = _tmp112 * fh1;
  const Scalar _tmp114 = _tmp43 * _tmp86 + _tmp85;
  const Scalar _tmp115 = -_tmp114 * _tmp62 + _tmp43 * _tmp84 - _tmp80;
  const Scalar _tmp116 = _tmp104 * (_tmp100 * _tmp43 - _tmp102 * _tmp115 - _tmp99);
  const Scalar _tmp117 = _tmp115 + _tmp116;
  const Scalar _tmp118 = -_tmp117 * _tmp90 - _tmp43;
  const Scalar _tmp119 = _tmp109 * _tmp111;
  const Scalar _tmp120 = _tmp119 * fh1;
  const Scalar _tmp121 = Scalar(1.0) * _tmp103;
  const Scalar _tmp122 = fh1 * (_tmp112 * _tmp32 - _tmp119 * _tmp21);
  const Scalar _tmp123 = -_tmp113 * _tmp94 * (_tmp106 * _tmp108 + _tmp107 * _tmp56) -
                         _tmp120 * _tmp94 * (_tmp108 * _tmp117 + _tmp118 * _tmp56 + Scalar(1.0)) -
                         _tmp122 * _tmp94 * (-_tmp121 * _tmp56 * _tmp82 + _tmp121 * _tmp77) -
                         _tmp35 * _tmp95;
  const Scalar _tmp124 = std::pow(_tmp123, Scalar(-2));
  const Scalar _tmp125 = _tmp124 * _tmp95;
  const Scalar _tmp126 = Scalar(0.1034955) * _tmp125;
  const Scalar _tmp127 = Scalar(1.0) / (_tmp123);
  const Scalar _tmp128 = _tmp59 + _tmp70;
  const Scalar _tmp129 = _tmp128 * _tmp62;
  const Scalar _tmp130 = Scalar(1.0) / (-_tmp129 + _tmp57 - _tmp73);
  const Scalar _tmp131 = Scalar(1.0) * _tmp130;
  const Scalar _tmp132 = _tmp104 * _tmp131;
  const Scalar _tmp133 = -_tmp121 * _tmp87 + _tmp128 * _tmp132;
  const Scalar _tmp134 = _tmp87 * _tmp89;
  const Scalar _tmp135 = _tmp128 * _tmp130;
  const Scalar _tmp136 = _tmp114 + _tmp116 * _tmp135 - _tmp117 * _tmp134;
  const Scalar _tmp137 = fh1 * (_tmp68 + _tmp78);
  const Scalar _tmp138 = -_tmp112 * _tmp137 - Scalar(5.1796800000000003) * _tmp17 - _tmp21 * fv1;
  const Scalar _tmp139 = _tmp131 * _tmp62;
  const Scalar _tmp140 = _tmp129 * _tmp131 + Scalar(1.0);
  const Scalar _tmp141 = Scalar(1.0) * _tmp139 - Scalar(1.0) * _tmp140 * _tmp96;
  const Scalar _tmp142 = _tmp105 * _tmp135 - _tmp106 * _tmp134 - Scalar(1.0) * _tmp86;
  const Scalar _tmp143 = _tmp130 * _tmp63;
  const Scalar _tmp144 = _tmp60 * (-_tmp128 * _tmp143 - _tmp134 * _tmp64 + _tmp59);
  const Scalar _tmp145 = -Scalar(1.0) * _tmp143 - Scalar(1.0) * _tmp144 + Scalar(1.0);
  const Scalar _tmp146 = _tmp119 * _tmp137 + Scalar(5.1796800000000003) * _tmp28 + _tmp32 * fv1;
  const Scalar _tmp147 = _tmp128 * _tmp60;
  const Scalar _tmp148 = Scalar(1.0) * _tmp131 * _tmp147 - Scalar(1.0) * _tmp131;
  const Scalar _tmp149 = Scalar(1.0) * _tmp113 * (_tmp105 * _tmp131 - _tmp142 * _tmp96) +
                         Scalar(1.0) * _tmp120 * (_tmp116 * _tmp131 - _tmp136 * _tmp96) +
                         Scalar(1.0) * _tmp122 * (_tmp132 - _tmp133 * _tmp96) + _tmp138 * _tmp141 +
                         _tmp145 * _tmp35 + _tmp146 * _tmp148;
  const Scalar _tmp150 = std::asinh(_tmp127 * _tmp149);
  const Scalar _tmp151 = Scalar(9.6622558468725703) * _tmp123;
  const Scalar _tmp152 =
      -_tmp150 * _tmp151 - std::sqrt(Scalar(std::pow(Scalar(-_tmp37 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp40 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp153 = Scalar(0.1034955) * _tmp127;
  const Scalar _tmp154 = _tmp152 * _tmp153;
  const Scalar _tmp155 = Scalar(9.6622558468725703) * _tmp95;
  const Scalar _tmp156 = _tmp18 + _tmp48;
  const Scalar _tmp157 =
      (-_tmp125 * _tmp149 + _tmp127 * (_tmp141 * _tmp156 - _tmp145 + _tmp148 * _tmp32)) /
      std::sqrt(Scalar(_tmp124 * std::pow(_tmp149, Scalar(2)) + 1));
  const Scalar _tmp158 = Scalar(1.0) * _tmp150;
  const Scalar _tmp159 = _tmp55 * _tmp91;
  const Scalar _tmp160 = _tmp121 * _tmp122;
  const Scalar _tmp161 = _tmp107 * _tmp113 * _tmp55 + _tmp118 * _tmp120 * _tmp55 -
                         _tmp159 * _tmp35 - _tmp160 * _tmp55 * _tmp82;
  const Scalar _tmp162 = Scalar(1.0) / (_tmp161);
  const Scalar _tmp163 = _tmp140 * _tmp60;
  const Scalar _tmp164 = _tmp131 * _tmp146;
  const Scalar _tmp165 = _tmp113 * _tmp142 * _tmp60 + _tmp120 * _tmp136 * _tmp60 +
                         _tmp122 * _tmp133 * _tmp60 + _tmp138 * _tmp163 + _tmp144 * _tmp35 -
                         _tmp147 * _tmp164;
  const Scalar _tmp166 = std::asinh(_tmp162 * _tmp165);
  const Scalar _tmp167 = Scalar(1.0) * _tmp166;
  const Scalar _tmp168 = Scalar(0.1034955) * _tmp162;
  const Scalar _tmp169 = Scalar(9.6622558468725703) * _tmp161;
  const Scalar _tmp170 =
      -_tmp166 * _tmp169 - std::sqrt(Scalar(std::pow(Scalar(-_tmp46 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp50 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp171 = _tmp168 * _tmp170;
  const Scalar _tmp172 = Scalar(9.6622558468725703) * _tmp159;
  const Scalar _tmp173 = _tmp131 * _tmp32;
  const Scalar _tmp174 = std::pow(_tmp161, Scalar(-2));
  const Scalar _tmp175 = _tmp159 * _tmp174;
  const Scalar _tmp176 =
      (_tmp162 * (-_tmp144 - _tmp147 * _tmp173 + _tmp156 * _tmp163) - _tmp165 * _tmp175) /
      std::sqrt(Scalar(std::pow(_tmp165, Scalar(2)) * _tmp174 + 1));
  const Scalar _tmp177 = Scalar(0.1034955) * _tmp175;
  const Scalar _tmp178 =
      _tmp106 * _tmp113 * _tmp89 + _tmp117 * _tmp120 * _tmp89 + _tmp160 + _tmp35 * _tmp92;
  const Scalar _tmp179 = Scalar(1.0) / (_tmp178);
  const Scalar _tmp180 = -_tmp105 * _tmp113 * _tmp130 - _tmp116 * _tmp120 * _tmp130 -
                         _tmp122 * _tmp132 - _tmp138 * _tmp139 + _tmp143 * _tmp35 + _tmp164;
  const Scalar _tmp181 = std::asinh(_tmp179 * _tmp180);
  const Scalar _tmp182 = Scalar(1.0) * _tmp181;
  const Scalar _tmp183 = std::pow(_tmp178, Scalar(-2));
  const Scalar _tmp184 = _tmp183 * _tmp92;
  const Scalar _tmp185 = (_tmp179 * (-_tmp139 * _tmp156 - _tmp143 + _tmp173) + _tmp180 * _tmp184) /
                         std::sqrt(Scalar(std::pow(_tmp180, Scalar(2)) * _tmp183 + 1));
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp178;
  const Scalar _tmp187 =
      -_tmp181 * _tmp186 - std::sqrt(Scalar(std::pow(Scalar(-_tmp71 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp74 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp179;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(9.6622558468725703) * _tmp92;
  const Scalar _tmp191 = Scalar(0.1034955) * _tmp184;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      -_tmp34 *
      (_tmp2 * std::sinh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::sinh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp34 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp22 + p_a(1, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp33 + p_a(0, 0)), Scalar(2)))))));
  _res(1, 0) =
      -_tmp151 * (-_tmp126 * p_b(2, 0) + Scalar(1.0) * _tmp157 * std::sinh(_tmp158) -
                  (-_tmp126 * _tmp152 + _tmp153 * (-_tmp150 * _tmp155 - _tmp151 * _tmp157)) *
                      std::sinh(_tmp154)) -
      _tmp155 * (_tmp153 * p_b(2, 0) - std::cosh(_tmp154) + std::cosh(_tmp158));
  _res(2, 0) =
      -_tmp169 * (Scalar(1.0) * _tmp176 * std::sinh(_tmp167) - _tmp177 * p_c(2, 0) -
                  (_tmp168 * (-_tmp166 * _tmp172 - _tmp169 * _tmp176) - _tmp170 * _tmp177) *
                      std::sinh(_tmp171)) -
      _tmp172 * (_tmp168 * p_c(2, 0) + std::cosh(_tmp167) - std::cosh(_tmp171));
  _res(3, 0) = -_tmp186 * (Scalar(1.0) * _tmp185 * std::sinh(_tmp182) + _tmp191 * p_d(2, 0) -
                           (_tmp187 * _tmp191 + _tmp188 * (_tmp181 * _tmp190 - _tmp185 * _tmp186)) *
                               std::sinh(_tmp189)) +
               _tmp190 * (_tmp188 * p_d(2, 0) + std::cosh(_tmp182) - std::cosh(_tmp189));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
