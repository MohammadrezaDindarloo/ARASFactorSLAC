// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fh1_Nl20
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFh1Nl20(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 629

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (215)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp2 = -2 * std::pow(_tmp1, Scalar(2));
  const Scalar _tmp3 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp4 = 1 - 2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = Scalar(0.20999999999999999) * _tmp2 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp6 = -_tmp5;
  const Scalar _tmp7 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp8 = 2 * _tmp3 * _tmp7;
  const Scalar _tmp9 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                       2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp10 = _tmp1 * _tmp9;
  const Scalar _tmp11 = _tmp10 + _tmp8;
  const Scalar _tmp12 = -Scalar(0.010999999999999999) * _tmp11;
  const Scalar _tmp13 = 2 * _tmp1;
  const Scalar _tmp14 = _tmp13 * _tmp7;
  const Scalar _tmp15 = _tmp3 * _tmp9;
  const Scalar _tmp16 = Scalar(0.20999999999999999) * _tmp14 - Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp17 = _tmp12 + _tmp16;
  const Scalar _tmp18 = _tmp17 + _tmp6;
  const Scalar _tmp19 = _tmp18 + p_init0;
  const Scalar _tmp20 = Scalar(0.20999999999999999) * _tmp14 + Scalar(0.20999999999999999) * _tmp15;
  const Scalar _tmp21 = -_tmp20;
  const Scalar _tmp22 = _tmp13 * _tmp3;
  const Scalar _tmp23 = _tmp7 * _tmp9;
  const Scalar _tmp24 = _tmp22 - _tmp23;
  const Scalar _tmp25 = -Scalar(0.010999999999999999) * _tmp24;
  const Scalar _tmp26 = -2 * std::pow(_tmp7, Scalar(2));
  const Scalar _tmp27 = Scalar(0.20999999999999999) * _tmp26 + Scalar(0.20999999999999999) * _tmp4;
  const Scalar _tmp28 = _tmp25 + _tmp27;
  const Scalar _tmp29 = _tmp21 + _tmp28;
  const Scalar _tmp30 = _tmp29 + p_init1;
  const Scalar _tmp31 = _tmp0 * fv1;
  const Scalar _tmp32 = std::asinh(_tmp31);
  const Scalar _tmp33 = Scalar(9.6622558468725703) * _tmp32;
  const Scalar _tmp34 =
      -Scalar(0.1034955) * _tmp33 * fh1 -
      Scalar(0.1034955) * std::sqrt(Scalar(std::pow(Scalar(-_tmp19 + p_d(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp30 + p_d(1, 0)), Scalar(2))));
  const Scalar _tmp35 = _tmp0 * _tmp34;
  const Scalar _tmp36 = std::pow(fh1, Scalar(-2));
  const Scalar _tmp37 =
      std::pow(Scalar(_tmp36 * std::pow(fv1, Scalar(2)) + 1), Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp38 = Scalar(1.0) * _tmp32;
  const Scalar _tmp39 = _tmp12 - _tmp16;
  const Scalar _tmp40 = _tmp39 + _tmp6;
  const Scalar _tmp41 = _tmp40 + p_init0;
  const Scalar _tmp42 = _tmp41 - p_a(0, 0);
  const Scalar _tmp43 = _tmp25 - _tmp27;
  const Scalar _tmp44 = _tmp21 + _tmp43;
  const Scalar _tmp45 = _tmp44 + p_init1;
  const Scalar _tmp46 = _tmp45 - p_a(1, 0);
  const Scalar _tmp47 = std::pow(Scalar(std::pow(_tmp42, Scalar(2)) + std::pow(_tmp46, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp48 = _tmp42 * _tmp47;
  const Scalar _tmp49 = _tmp20 + _tmp43;
  const Scalar _tmp50 = _tmp49 + p_init1;
  const Scalar _tmp51 = _tmp50 - p_b(1, 0);
  const Scalar _tmp52 = _tmp39 + _tmp5;
  const Scalar _tmp53 = _tmp52 + p_init0;
  const Scalar _tmp54 = _tmp53 - p_b(0, 0);
  const Scalar _tmp55 =
      std::sqrt(Scalar(std::pow(_tmp51, Scalar(2)) + std::pow(_tmp54, Scalar(2))));
  const Scalar _tmp56 = Scalar(1.0) / (_tmp55);
  const Scalar _tmp57 = Scalar(1.0) / (_tmp54);
  const Scalar _tmp58 = _tmp55 * _tmp57;
  const Scalar _tmp59 = _tmp58 * (-_tmp49 * _tmp54 * _tmp56 + _tmp51 * _tmp52 * _tmp56);
  const Scalar _tmp60 = _tmp46 * _tmp47;
  const Scalar _tmp61 = -_tmp40 * _tmp60 + _tmp44 * _tmp48 + _tmp48 * _tmp59;
  const Scalar _tmp62 = _tmp51 * _tmp57;
  const Scalar _tmp63 = Scalar(1.0) / (_tmp48 * _tmp62 - _tmp60);
  const Scalar _tmp64 = _tmp62 * _tmp63;
  const Scalar _tmp65 = -Scalar(0.20999999999999999) * _tmp10 + Scalar(0.20999999999999999) * _tmp8;
  const Scalar _tmp66 = -Scalar(0.010999999999999999) * _tmp2 -
                        Scalar(0.010999999999999999) * _tmp26 + Scalar(-0.010999999999999999);
  const Scalar _tmp67 = Scalar(0.20999999999999999) * _tmp22 + Scalar(0.20999999999999999) * _tmp23;
  const Scalar _tmp68 = _tmp66 - _tmp67;
  const Scalar _tmp69 = _tmp65 + _tmp68;
  const Scalar _tmp70 = -_tmp65;
  const Scalar _tmp71 = _tmp68 + _tmp70;
  const Scalar _tmp72 = _tmp48 * _tmp69;
  const Scalar _tmp73 = _tmp60 * _tmp71 - _tmp62 * _tmp72;
  const Scalar _tmp74 = _tmp62 * _tmp69 + _tmp64 * _tmp73;
  const Scalar _tmp75 = Scalar(1.0) * _tmp49;
  const Scalar _tmp76 = -_tmp75;
  const Scalar _tmp77 = Scalar(1.0) / (_tmp44 + _tmp76);
  const Scalar _tmp78 = Scalar(1.0) * _tmp52;
  const Scalar _tmp79 = -_tmp40 + _tmp78;
  const Scalar _tmp80 = _tmp77 * _tmp79;
  const Scalar _tmp81 = -_tmp48 * _tmp71 + _tmp72;
  const Scalar _tmp82 = _tmp64 * _tmp81 - _tmp69 - _tmp74 * _tmp80;
  const Scalar _tmp83 = _tmp17 + _tmp5;
  const Scalar _tmp84 = _tmp83 + p_init0;
  const Scalar _tmp85 = _tmp84 - p_c(0, 0);
  const Scalar _tmp86 = _tmp20 + _tmp28;
  const Scalar _tmp87 = _tmp86 + p_init1;
  const Scalar _tmp88 = _tmp87 - p_c(1, 0);
  const Scalar _tmp89 = std::pow(Scalar(std::pow(_tmp85, Scalar(2)) + std::pow(_tmp88, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp90 = _tmp88 * _tmp89;
  const Scalar _tmp91 = _tmp85 * _tmp89;
  const Scalar _tmp92 = _tmp62 * _tmp91 - _tmp90;
  const Scalar _tmp93 = _tmp63 * _tmp92;
  const Scalar _tmp94 = _tmp59 * _tmp91 - _tmp61 * _tmp93 - _tmp83 * _tmp90 + _tmp86 * _tmp91;
  const Scalar _tmp95 = _tmp69 * _tmp91;
  const Scalar _tmp96 = _tmp66 + _tmp67;
  const Scalar _tmp97 = _tmp65 + _tmp96;
  const Scalar _tmp98 = -_tmp62 * _tmp95 - _tmp73 * _tmp93 + _tmp90 * _tmp97;
  const Scalar _tmp99 = -_tmp80 * _tmp98 - _tmp81 * _tmp93 - _tmp91 * _tmp97 + _tmp95;
  const Scalar _tmp100 = Scalar(1.0) / (_tmp99);
  const Scalar _tmp101 = _tmp100 * _tmp94;
  const Scalar _tmp102 = Scalar(1.0) / (_tmp94);
  const Scalar _tmp103 = _tmp102 * _tmp99;
  const Scalar _tmp104 = _tmp103 * (-_tmp101 * _tmp82 - _tmp59 + _tmp61 * _tmp64);
  const Scalar _tmp105 = _tmp76 + _tmp86;
  const Scalar _tmp106 = _tmp105 * _tmp80;
  const Scalar _tmp107 = Scalar(1.0) / (-_tmp106 + _tmp78 - _tmp83);
  const Scalar _tmp108 = _tmp105 * _tmp107;
  const Scalar _tmp109 = _tmp104 + _tmp82;
  const Scalar _tmp110 = _tmp100 * _tmp98;
  const Scalar _tmp111 = _tmp104 * _tmp108 - _tmp109 * _tmp110 + _tmp74;
  const Scalar _tmp112 = Scalar(1.0) * _tmp77;
  const Scalar _tmp113 = Scalar(1.0) * _tmp107;
  const Scalar _tmp114 = _tmp30 - p_d(1, 0);
  const Scalar _tmp115 = _tmp19 - p_d(0, 0);
  const Scalar _tmp116 =
      std::pow(Scalar(std::pow(_tmp114, Scalar(2)) + std::pow(_tmp115, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp117 = _tmp115 * _tmp116;
  const Scalar _tmp118 = Scalar(1.0) * _tmp117 * (_tmp104 * _tmp113 - _tmp111 * _tmp112);
  const Scalar _tmp119 = _tmp75 * _tmp80 + _tmp78;
  const Scalar _tmp120 = 0;
  const Scalar _tmp121 = _tmp107 * _tmp119;
  const Scalar _tmp122 = -_tmp105 * _tmp121 - _tmp110 * _tmp120 + _tmp76;
  const Scalar _tmp123 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp124 = _tmp114 * _tmp116;
  const Scalar _tmp125 = -_tmp117 * _tmp29 + _tmp124 * _tmp18;
  const Scalar _tmp126 = _tmp103 * _tmp113;
  const Scalar _tmp127 = Scalar(1.0) * _tmp102;
  const Scalar _tmp128 = _tmp105 * _tmp126 - _tmp127 * _tmp98;
  const Scalar _tmp129 = Scalar(1.0) * _tmp125 * (-_tmp112 * _tmp128 + _tmp126);
  const Scalar _tmp130 = _tmp70 + _tmp96;
  const Scalar _tmp131 = _tmp130 * fh1;
  const Scalar _tmp132 = -_tmp124 * _tmp131 - Scalar(5.1796800000000003) * _tmp24 - _tmp29 * fv1;
  const Scalar _tmp133 = _tmp113 * _tmp80;
  const Scalar _tmp134 = _tmp106 * _tmp113 + Scalar(1.0);
  const Scalar _tmp135 = -Scalar(1.0) * _tmp112 * _tmp134 + Scalar(1.0) * _tmp133;
  const Scalar _tmp136 = Scalar(1.0) * _tmp63;
  const Scalar _tmp137 = _tmp112 * _tmp63 * _tmp73 * _tmp79 - _tmp136 * _tmp81;
  const Scalar _tmp138 = _tmp103 * (-_tmp101 * _tmp137 - _tmp136 * _tmp61);
  const Scalar _tmp139 = _tmp137 + _tmp138;
  const Scalar _tmp140 = _tmp108 * _tmp138 - _tmp110 * _tmp139 - _tmp136 * _tmp73;
  const Scalar _tmp141 = Scalar(1.0) * _tmp124 * (-_tmp112 * _tmp140 + _tmp113 * _tmp138);
  const Scalar _tmp142 = Scalar(5.1796800000000003) * _tmp11 + _tmp117 * _tmp131 + _tmp18 * fv1;
  const Scalar _tmp143 = _tmp105 * _tmp77;
  const Scalar _tmp144 = Scalar(1.0) * _tmp113 * _tmp143 - Scalar(1.0) * _tmp113;
  const Scalar _tmp145 =
      _tmp118 * fh1 +
      Scalar(1.0) * _tmp123 * (-_tmp112 * _tmp122 - _tmp113 * _tmp119 + Scalar(1.0)) +
      _tmp129 * fh1 + _tmp132 * _tmp135 + _tmp141 * fh1 + _tmp142 * _tmp144;
  const Scalar _tmp146 = _tmp100 * _tmp92;
  const Scalar _tmp147 = _tmp120 * _tmp146 * _tmp63;
  const Scalar _tmp148 = _tmp100 * _tmp120;
  const Scalar _tmp149 = _tmp63 * (-_tmp139 * _tmp146 + Scalar(1.0));
  const Scalar _tmp150 = _tmp100 * _tmp139;
  const Scalar _tmp151 = _tmp124 * _tmp58 * (_tmp149 * _tmp48 + _tmp150 * _tmp91);
  const Scalar _tmp152 = _tmp125 * _tmp58 * (-_tmp127 * _tmp48 * _tmp93 + _tmp127 * _tmp91);
  const Scalar _tmp153 = _tmp63 * (-_tmp109 * _tmp146 - _tmp62);
  const Scalar _tmp154 = _tmp100 * _tmp109;
  const Scalar _tmp155 = _tmp117 * _tmp58 * (_tmp153 * _tmp48 + _tmp154 * _tmp91 + Scalar(1.0));
  const Scalar _tmp156 = -_tmp123 * _tmp58 * (-_tmp147 * _tmp48 + _tmp148 * _tmp91) -
                         _tmp151 * fh1 - _tmp152 * fh1 - _tmp155 * fh1;
  const Scalar _tmp157 = Scalar(1.0) / (_tmp156);
  const Scalar _tmp158 = std::asinh(_tmp145 * _tmp157);
  const Scalar _tmp159 = Scalar(1.0) * _tmp158;
  const Scalar _tmp160 = std::pow(_tmp156, Scalar(-2));
  const Scalar _tmp161 = -_tmp151 - _tmp152 - _tmp155;
  const Scalar _tmp162 = _tmp160 * _tmp161;
  const Scalar _tmp163 = _tmp117 * _tmp130;
  const Scalar _tmp164 = _tmp124 * _tmp130;
  const Scalar _tmp165 = (-_tmp145 * _tmp162 + _tmp157 * (_tmp118 + _tmp129 - _tmp135 * _tmp164 +
                                                          _tmp141 + _tmp144 * _tmp163)) /
                         std::sqrt(Scalar(std::pow(_tmp145, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp166 = Scalar(9.6622558468725703) * _tmp158;
  const Scalar _tmp167 = Scalar(9.6622558468725703) * _tmp156;
  const Scalar _tmp168 = Scalar(0.1034955) * _tmp157;
  const Scalar _tmp169 =
      -_tmp156 * _tmp166 - std::sqrt(Scalar(std::pow(Scalar(-_tmp50 + p_b(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp53 + p_b(0, 0)), Scalar(2))));
  const Scalar _tmp170 = _tmp168 * _tmp169;
  const Scalar _tmp171 = _tmp124 * _tmp77;
  const Scalar _tmp172 = _tmp140 * _tmp171;
  const Scalar _tmp173 = _tmp113 * _tmp142;
  const Scalar _tmp174 = _tmp125 * _tmp128 * _tmp77;
  const Scalar _tmp175 = _tmp117 * _tmp77;
  const Scalar _tmp176 = _tmp111 * _tmp175;
  const Scalar _tmp177 = _tmp132 * _tmp77;
  const Scalar _tmp178 = _tmp122 * _tmp123 * _tmp77 + _tmp134 * _tmp177 - _tmp143 * _tmp173 +
                         _tmp172 * fh1 + _tmp174 * fh1 + _tmp176 * fh1;
  const Scalar _tmp179 = _tmp125 * _tmp127;
  const Scalar _tmp180 = _tmp179 * fh1;
  const Scalar _tmp181 = _tmp124 * _tmp149;
  const Scalar _tmp182 = _tmp117 * _tmp153;
  const Scalar _tmp183 = -_tmp123 * _tmp147 - _tmp180 * _tmp93 + _tmp181 * fh1 + _tmp182 * fh1;
  const Scalar _tmp184 = Scalar(1.0) / (_tmp183);
  const Scalar _tmp185 = std::asinh(_tmp178 * _tmp184);
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp183;
  const Scalar _tmp187 =
      -_tmp185 * _tmp186 - std::sqrt(Scalar(std::pow(Scalar(-_tmp41 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp45 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp184;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = Scalar(1.0) * _tmp185;
  const Scalar _tmp191 = -_tmp179 * _tmp93 + _tmp181 + _tmp182;
  const Scalar _tmp192 = Scalar(9.6622558468725703) * _tmp191;
  const Scalar _tmp193 = std::pow(_tmp183, Scalar(-2));
  const Scalar _tmp194 = _tmp191 * _tmp193;
  const Scalar _tmp195 =
      (-_tmp178 * _tmp194 + _tmp184 * (-_tmp105 * _tmp113 * _tmp130 * _tmp175 -
                                       _tmp130 * _tmp134 * _tmp171 + _tmp172 + _tmp174 + _tmp176)) /
      std::sqrt(Scalar(std::pow(_tmp178, Scalar(2)) * _tmp193 + 1));
  const Scalar _tmp196 = _tmp107 * _tmp124 * _tmp138;
  const Scalar _tmp197 = _tmp104 * _tmp107 * _tmp117;
  const Scalar _tmp198 = _tmp125 * _tmp126;
  const Scalar _tmp199 = -_tmp113 * _tmp177 * _tmp79 + _tmp121 * _tmp123 + _tmp173 - _tmp196 * fh1 -
                         _tmp197 * fh1 - _tmp198 * fh1;
  const Scalar _tmp200 = _tmp124 * _tmp150;
  const Scalar _tmp201 = _tmp117 * _tmp154;
  const Scalar _tmp202 = _tmp123 * _tmp148 + _tmp180 + _tmp200 * fh1 + _tmp201 * fh1;
  const Scalar _tmp203 = Scalar(1.0) / (_tmp202);
  const Scalar _tmp204 = std::asinh(_tmp199 * _tmp203);
  const Scalar _tmp205 = Scalar(1.0) * _tmp204;
  const Scalar _tmp206 = std::pow(_tmp202, Scalar(-2));
  const Scalar _tmp207 = _tmp179 + _tmp200 + _tmp201;
  const Scalar _tmp208 = _tmp206 * _tmp207;
  const Scalar _tmp209 = (-_tmp199 * _tmp208 + _tmp203 * (_tmp113 * _tmp163 + _tmp133 * _tmp164 -
                                                          _tmp196 - _tmp197 - _tmp198)) /
                         std::sqrt(Scalar(std::pow(_tmp199, Scalar(2)) * _tmp206 + 1));
  const Scalar _tmp210 = Scalar(9.6622558468725703) * _tmp207;
  const Scalar _tmp211 = Scalar(9.6622558468725703) * _tmp202;
  const Scalar _tmp212 = Scalar(0.1034955) * _tmp203;
  const Scalar _tmp213 =
      -_tmp204 * _tmp211 - std::sqrt(Scalar(std::pow(Scalar(-_tmp84 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp87 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp214 = _tmp212 * _tmp213;

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      Scalar(9.6622558468725703) * fh1 *
          (Scalar(1.0) * _tmp36 * _tmp37 * fv1 * std::cosh(_tmp38) -
           (Scalar(0.1034955) * _tmp0 * (Scalar(9.6622558468725703) * _tmp31 * _tmp37 - _tmp33) -
            _tmp34 * _tmp36) *
               std::cosh(_tmp35)) -
      Scalar(9.6622558468725703) * std::sinh(_tmp35) -
      Scalar(9.6622558468725703) * std::sinh(_tmp38);
  _res(1, 0) = Scalar(9.6622558468725703) * _tmp161 * (-std::sinh(_tmp159) - std::sinh(_tmp170)) +
               _tmp167 * (-Scalar(1.0) * _tmp165 * std::cosh(_tmp159) -
                          (-Scalar(0.1034955) * _tmp162 * _tmp169 +
                           _tmp168 * (-_tmp161 * _tmp166 - _tmp165 * _tmp167)) *
                              std::cosh(_tmp170));
  _res(2, 0) = _tmp186 * (-Scalar(1.0) * _tmp195 * std::cosh(_tmp190) -
                          (-Scalar(0.1034955) * _tmp187 * _tmp194 +
                           _tmp188 * (-_tmp185 * _tmp192 - _tmp186 * _tmp195)) *
                              std::cosh(_tmp189)) +
               _tmp192 * (-std::sinh(_tmp189) - std::sinh(_tmp190));
  _res(3, 0) = _tmp210 * (-std::sinh(_tmp205) - std::sinh(_tmp214)) +
               _tmp211 * (-Scalar(1.0) * _tmp209 * std::cosh(_tmp205) -
                          (-Scalar(0.1034955) * _tmp208 * _tmp213 +
                           _tmp212 * (-_tmp204 * _tmp210 - _tmp209 * _tmp211)) *
                              std::cosh(_tmp214));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
