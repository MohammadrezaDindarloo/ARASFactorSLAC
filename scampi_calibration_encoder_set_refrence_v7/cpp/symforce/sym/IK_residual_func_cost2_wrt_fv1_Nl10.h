// -----------------------------------------------------------------------------
// This file was autogenerated by symforce from template:
//     function/FUNCTION.h.jinja
// Do NOT modify by hand.
// -----------------------------------------------------------------------------

#pragma once

#include <Eigen/Dense>

#include <sym/rot3.h>

namespace sym {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: IK_residual_func_cost2_wrt_fv1_Nl10
 *
 * Args:
 *     fh1: Scalar
 *     fv1: Scalar
 *     DeltaRot: Rot3
 *     p_a: Matrix31
 *     p_b: Matrix31
 *     p_c: Matrix31
 *     p_d: Matrix31
 *     p_init0: Scalar
 *     p_init1: Scalar
 *     p_init2: Scalar
 *     rot_init_x: Scalar
 *     rot_init_y: Scalar
 *     rot_init_z: Scalar
 *     rot_init_w: Scalar
 *     epsilon: Scalar
 *
 * Outputs:
 *     res: Matrix41
 */
template <typename Scalar>
Eigen::Matrix<Scalar, 4, 1> IkResidualFuncCost2WrtFv1Nl10(
    const Scalar fh1, const Scalar fv1, const sym::Rot3<Scalar>& DeltaRot,
    const Eigen::Matrix<Scalar, 3, 1>& p_a, const Eigen::Matrix<Scalar, 3, 1>& p_b,
    const Eigen::Matrix<Scalar, 3, 1>& p_c, const Eigen::Matrix<Scalar, 3, 1>& p_d,
    const Scalar p_init0, const Scalar p_init1, const Scalar p_init2, const Scalar rot_init_x,
    const Scalar rot_init_y, const Scalar rot_init_z, const Scalar rot_init_w,
    const Scalar epsilon) {
  // Total ops: 581

  // Unused inputs
  (void)p_init2;
  (void)epsilon;

  // Input arrays
  const Eigen::Matrix<Scalar, 4, 1>& _DeltaRot = DeltaRot.Data();

  // Intermediate terms (193)
  const Scalar _tmp0 = Scalar(1.0) / (fh1);
  const Scalar _tmp1 = std::asinh(_tmp0 * fv1);
  const Scalar _tmp2 = Scalar(1.0) * _tmp0 /
                       std::sqrt(Scalar(1 + std::pow(fv1, Scalar(2)) / std::pow(fh1, Scalar(2))));
  const Scalar _tmp3 = _DeltaRot[0] * rot_init_z + _DeltaRot[1] * rot_init_w -
                       _DeltaRot[2] * rot_init_x + _DeltaRot[3] * rot_init_y;
  const Scalar _tmp4 = -2 * std::pow(_tmp3, Scalar(2));
  const Scalar _tmp5 = -_DeltaRot[0] * rot_init_y + _DeltaRot[1] * rot_init_x +
                       _DeltaRot[2] * rot_init_w + _DeltaRot[3] * rot_init_z;
  const Scalar _tmp6 = 1 - 2 * std::pow(_tmp5, Scalar(2));
  const Scalar _tmp7 = Scalar(0.20999999999999999) * _tmp4 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp8 = _DeltaRot[0] * rot_init_w - _DeltaRot[1] * rot_init_z +
                       _DeltaRot[2] * rot_init_y + _DeltaRot[3] * rot_init_x;
  const Scalar _tmp9 = 2 * _tmp5 * _tmp8;
  const Scalar _tmp10 = -2 * _DeltaRot[0] * rot_init_x - 2 * _DeltaRot[1] * rot_init_y -
                        2 * _DeltaRot[2] * rot_init_z + 2 * _DeltaRot[3] * rot_init_w;
  const Scalar _tmp11 = _tmp10 * _tmp3;
  const Scalar _tmp12 = _tmp11 + _tmp9;
  const Scalar _tmp13 = -Scalar(0.010999999999999999) * _tmp12;
  const Scalar _tmp14 = 2 * _tmp3;
  const Scalar _tmp15 = _tmp14 * _tmp8;
  const Scalar _tmp16 = _tmp10 * _tmp5;
  const Scalar _tmp17 = Scalar(0.20999999999999999) * _tmp15 - Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp18 = _tmp13 - _tmp17;
  const Scalar _tmp19 = _tmp18 + _tmp7;
  const Scalar _tmp20 = _tmp19 + p_init0;
  const Scalar _tmp21 = -2 * std::pow(_tmp8, Scalar(2));
  const Scalar _tmp22 = Scalar(0.20999999999999999) * _tmp21 + Scalar(0.20999999999999999) * _tmp6;
  const Scalar _tmp23 = -_tmp22;
  const Scalar _tmp24 = _tmp14 * _tmp5;
  const Scalar _tmp25 = _tmp10 * _tmp8;
  const Scalar _tmp26 = _tmp24 - _tmp25;
  const Scalar _tmp27 = Scalar(0.010999999999999999) * _tmp26;
  const Scalar _tmp28 = -_tmp27;
  const Scalar _tmp29 = Scalar(0.20999999999999999) * _tmp15 + Scalar(0.20999999999999999) * _tmp16;
  const Scalar _tmp30 = _tmp28 + _tmp29;
  const Scalar _tmp31 = _tmp23 + _tmp30;
  const Scalar _tmp32 = _tmp31 + p_init1;
  const Scalar _tmp33 = Scalar(9.6622558468725703) * fh1;
  const Scalar _tmp34 = -_tmp29;
  const Scalar _tmp35 = _tmp28 + _tmp34;
  const Scalar _tmp36 = _tmp22 + _tmp35;
  const Scalar _tmp37 = _tmp36 + p_init1;
  const Scalar _tmp38 = _tmp37 - p_d(1, 0);
  const Scalar _tmp39 = -_tmp7;
  const Scalar _tmp40 = _tmp13 + _tmp17;
  const Scalar _tmp41 = _tmp39 + _tmp40;
  const Scalar _tmp42 = _tmp41 + p_init0;
  const Scalar _tmp43 = _tmp42 - p_d(0, 0);
  const Scalar _tmp44 = Scalar(1.0) / (_tmp43);
  const Scalar _tmp45 = _tmp38 * _tmp44;
  const Scalar _tmp46 =
      std::sqrt(Scalar(std::pow(_tmp38, Scalar(2)) + std::pow(_tmp43, Scalar(2))));
  const Scalar _tmp47 = Scalar(1.0) / (_tmp46);
  const Scalar _tmp48 = _tmp44 * _tmp46;
  const Scalar _tmp49 = _tmp48 * (-_tmp36 * _tmp43 * _tmp47 + _tmp38 * _tmp41 * _tmp47);
  const Scalar _tmp50 = _tmp18 + _tmp39;
  const Scalar _tmp51 = _tmp50 + p_init0;
  const Scalar _tmp52 = _tmp51 - p_a(0, 0);
  const Scalar _tmp53 = _tmp23 + _tmp35;
  const Scalar _tmp54 = _tmp53 + p_init1;
  const Scalar _tmp55 = _tmp54 - p_a(1, 0);
  const Scalar _tmp56 = std::pow(Scalar(std::pow(_tmp52, Scalar(2)) + std::pow(_tmp55, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp57 = _tmp55 * _tmp56;
  const Scalar _tmp58 = _tmp52 * _tmp56;
  const Scalar _tmp59 = _tmp49 * _tmp58 - _tmp50 * _tmp57 + _tmp53 * _tmp58;
  const Scalar _tmp60 = Scalar(1.0) / (_tmp45 * _tmp58 - _tmp57);
  const Scalar _tmp61 = _tmp45 * _tmp60;
  const Scalar _tmp62 = -Scalar(0.20999999999999999) * _tmp11 + Scalar(0.20999999999999999) * _tmp9;
  const Scalar _tmp63 = -_tmp62;
  const Scalar _tmp64 = -Scalar(0.010999999999999999) * _tmp21 -
                        Scalar(0.010999999999999999) * _tmp4 + Scalar(-0.010999999999999999);
  const Scalar _tmp65 = Scalar(0.20999999999999999) * _tmp24 + Scalar(0.20999999999999999) * _tmp25;
  const Scalar _tmp66 = _tmp64 + _tmp65;
  const Scalar _tmp67 = _tmp63 + _tmp66;
  const Scalar _tmp68 = _tmp64 - _tmp65;
  const Scalar _tmp69 = _tmp63 + _tmp68;
  const Scalar _tmp70 = _tmp58 * _tmp67 - _tmp58 * _tmp69;
  const Scalar _tmp71 = _tmp45 * _tmp67;
  const Scalar _tmp72 = _tmp57 * _tmp69 - _tmp58 * _tmp71;
  const Scalar _tmp73 = _tmp61 * _tmp72 + _tmp71;
  const Scalar _tmp74 = Scalar(1.0) * _tmp36;
  const Scalar _tmp75 = -_tmp74;
  const Scalar _tmp76 = Scalar(1.0) / (_tmp53 + _tmp75);
  const Scalar _tmp77 = Scalar(1.0) * _tmp41;
  const Scalar _tmp78 = -_tmp50 + _tmp77;
  const Scalar _tmp79 = _tmp76 * _tmp78;
  const Scalar _tmp80 = _tmp61 * _tmp70 - _tmp67 - _tmp73 * _tmp79;
  const Scalar _tmp81 = _tmp62 + _tmp66;
  const Scalar _tmp82 = _tmp40 + _tmp7;
  const Scalar _tmp83 = _tmp82 + p_init0;
  const Scalar _tmp84 = _tmp83 - p_c(0, 0);
  const Scalar _tmp85 = _tmp22 + _tmp30;
  const Scalar _tmp86 = _tmp85 + p_init1;
  const Scalar _tmp87 = _tmp86 - p_c(1, 0);
  const Scalar _tmp88 = std::pow(Scalar(std::pow(_tmp84, Scalar(2)) + std::pow(_tmp87, Scalar(2))),
                                 Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp89 = _tmp87 * _tmp88;
  const Scalar _tmp90 = _tmp84 * _tmp88;
  const Scalar _tmp91 = _tmp67 * _tmp90;
  const Scalar _tmp92 = _tmp45 * _tmp90 - _tmp89;
  const Scalar _tmp93 = _tmp60 * _tmp92;
  const Scalar _tmp94 = -_tmp45 * _tmp91 - _tmp72 * _tmp93 + _tmp81 * _tmp89;
  const Scalar _tmp95 = -_tmp70 * _tmp93 - _tmp79 * _tmp94 - _tmp81 * _tmp90 + _tmp91;
  const Scalar _tmp96 = Scalar(1.0) / (_tmp95);
  const Scalar _tmp97 = _tmp49 * _tmp90 - _tmp59 * _tmp93 - _tmp82 * _tmp89 + _tmp85 * _tmp90;
  const Scalar _tmp98 = _tmp96 * _tmp97;
  const Scalar _tmp99 = Scalar(1.0) / (_tmp97);
  const Scalar _tmp100 = _tmp95 * _tmp99;
  const Scalar _tmp101 = _tmp100 * (-_tmp49 + _tmp59 * _tmp61 - _tmp80 * _tmp98);
  const Scalar _tmp102 = _tmp101 + _tmp80;
  const Scalar _tmp103 = _tmp92 * _tmp96;
  const Scalar _tmp104 = -_tmp102 * _tmp103 - _tmp45;
  const Scalar _tmp105 = _tmp58 * _tmp60;
  const Scalar _tmp106 = _tmp90 * _tmp96;
  const Scalar _tmp107 = _tmp32 - p_b(1, 0);
  const Scalar _tmp108 = _tmp20 - p_b(0, 0);
  const Scalar _tmp109 =
      std::pow(Scalar(std::pow(_tmp107, Scalar(2)) + std::pow(_tmp108, Scalar(2))),
               Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp110 = _tmp108 * _tmp109;
  const Scalar _tmp111 = _tmp110 * fh1;
  const Scalar _tmp112 = Scalar(1.0) * _tmp60;
  const Scalar _tmp113 = Scalar(1.0) * _tmp76;
  const Scalar _tmp114 = -_tmp112 * _tmp70 + _tmp113 * _tmp60 * _tmp72 * _tmp78;
  const Scalar _tmp115 = _tmp100 * (-_tmp112 * _tmp59 - _tmp114 * _tmp98);
  const Scalar _tmp116 = _tmp114 + _tmp115;
  const Scalar _tmp117 = -_tmp103 * _tmp116 + Scalar(1.0);
  const Scalar _tmp118 = _tmp107 * _tmp109;
  const Scalar _tmp119 = _tmp118 * fh1;
  const Scalar _tmp120 = Scalar(1.0) * _tmp99;
  const Scalar _tmp121 = _tmp112 * _tmp92 * _tmp99;
  const Scalar _tmp122 = fh1 * (-_tmp110 * _tmp31 + _tmp118 * _tmp19);
  const Scalar _tmp123 = Scalar(43.164000000000001) - fv1;
  const Scalar _tmp124 = _tmp74 * _tmp79 + _tmp77;
  const Scalar _tmp125 = 0;
  const Scalar _tmp126 = _tmp125 * _tmp96;
  const Scalar _tmp127 = _tmp126 * _tmp93;
  const Scalar _tmp128 = _tmp48 * (_tmp126 * _tmp90 - _tmp127 * _tmp58);
  const Scalar _tmp129 = -_tmp111 * _tmp48 * (_tmp102 * _tmp106 + _tmp104 * _tmp105 + Scalar(1.0)) -
                         _tmp119 * _tmp48 * (_tmp105 * _tmp117 + _tmp106 * _tmp116) -
                         _tmp122 * _tmp48 * (_tmp120 * _tmp90 - _tmp121 * _tmp58) -
                         _tmp123 * _tmp128;
  const Scalar _tmp130 = Scalar(1.0) / (_tmp129);
  const Scalar _tmp131 = _tmp75 + _tmp85;
  const Scalar _tmp132 = _tmp131 * _tmp79;
  const Scalar _tmp133 = Scalar(1.0) / (-_tmp132 + _tmp77 - _tmp82);
  const Scalar _tmp134 = Scalar(1.0) * _tmp133;
  const Scalar _tmp135 = _tmp94 * _tmp96;
  const Scalar _tmp136 = _tmp124 * _tmp133;
  const Scalar _tmp137 = _tmp76 * (-_tmp125 * _tmp135 - _tmp131 * _tmp136 + _tmp75);
  const Scalar _tmp138 = -Scalar(1.0) * _tmp124 * _tmp134 - Scalar(1.0) * _tmp137 + Scalar(1.0);
  const Scalar _tmp139 = _tmp100 * _tmp134;
  const Scalar _tmp140 = -_tmp120 * _tmp94 + _tmp131 * _tmp139;
  const Scalar _tmp141 = fh1 * (_tmp62 + _tmp68);
  const Scalar _tmp142 = _tmp110 * _tmp141 + Scalar(5.1796800000000003) * _tmp12 + _tmp19 * fv1;
  const Scalar _tmp143 = _tmp131 * _tmp76;
  const Scalar _tmp144 = Scalar(1.0) * _tmp134 * _tmp143 - Scalar(1.0) * _tmp134;
  const Scalar _tmp145 = _tmp131 * _tmp133;
  const Scalar _tmp146 = -_tmp112 * _tmp72 + _tmp115 * _tmp145 - _tmp116 * _tmp135;
  const Scalar _tmp147 = -_tmp118 * _tmp141 - Scalar(5.1796800000000003) * _tmp26 - _tmp31 * fv1;
  const Scalar _tmp148 = _tmp134 * _tmp79;
  const Scalar _tmp149 = _tmp76 * (_tmp132 * _tmp134 + Scalar(1.0));
  const Scalar _tmp150 = Scalar(1.0) * _tmp148 - Scalar(1.0) * _tmp149;
  const Scalar _tmp151 = _tmp101 * _tmp145 - _tmp102 * _tmp135 + _tmp73;
  const Scalar _tmp152 = Scalar(1.0) * _tmp111 * (_tmp101 * _tmp134 - _tmp113 * _tmp151) +
                         Scalar(1.0) * _tmp119 * (-_tmp113 * _tmp146 + _tmp115 * _tmp134) +
                         Scalar(1.0) * _tmp122 * (-_tmp113 * _tmp140 + _tmp139) +
                         _tmp123 * _tmp138 + _tmp142 * _tmp144 + _tmp147 * _tmp150;
  const Scalar _tmp153 = std::asinh(_tmp130 * _tmp152);
  const Scalar _tmp154 = Scalar(9.6622558468725703) * _tmp129;
  const Scalar _tmp155 =
      -_tmp153 * _tmp154 - std::sqrt(Scalar(std::pow(Scalar(-_tmp37 + p_d(1, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp42 + p_d(0, 0)), Scalar(2))));
  const Scalar _tmp156 = Scalar(0.1034955) * _tmp130;
  const Scalar _tmp157 = _tmp155 * _tmp156;
  const Scalar _tmp158 = Scalar(1.0) * _tmp153;
  const Scalar _tmp159 = Scalar(9.6622558468725703) * _tmp128;
  const Scalar _tmp160 = std::pow(_tmp129, Scalar(-2));
  const Scalar _tmp161 = _tmp128 * _tmp160;
  const Scalar _tmp162 = _tmp22 + _tmp27 + _tmp34;
  const Scalar _tmp163 =
      (_tmp130 * (-_tmp138 + _tmp144 * _tmp19 + _tmp150 * _tmp162) - _tmp152 * _tmp161) /
      std::sqrt(Scalar(std::pow(_tmp152, Scalar(2)) * _tmp160 + 1));
  const Scalar _tmp164 = _tmp123 * _tmp126;
  const Scalar _tmp165 = _tmp104 * _tmp111 * _tmp60 + _tmp117 * _tmp119 * _tmp60 -
                         _tmp121 * _tmp122 - _tmp164 * _tmp93;
  const Scalar _tmp166 = Scalar(1.0) / (_tmp165);
  const Scalar _tmp167 = _tmp134 * _tmp142;
  const Scalar _tmp168 = _tmp111 * _tmp151 * _tmp76 + _tmp119 * _tmp146 * _tmp76 +
                         _tmp122 * _tmp140 * _tmp76 + _tmp123 * _tmp137 - _tmp143 * _tmp167 +
                         _tmp147 * _tmp149;
  const Scalar _tmp169 = std::asinh(_tmp166 * _tmp168);
  const Scalar _tmp170 = Scalar(9.6622558468725703) * _tmp165;
  const Scalar _tmp171 =
      -_tmp169 * _tmp170 - std::sqrt(Scalar(std::pow(Scalar(-_tmp51 + p_a(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp54 + p_a(1, 0)), Scalar(2))));
  const Scalar _tmp172 = Scalar(0.1034955) * _tmp166;
  const Scalar _tmp173 = _tmp171 * _tmp172;
  const Scalar _tmp174 = Scalar(1.0) * _tmp169;
  const Scalar _tmp175 = Scalar(9.6622558468725703) * _tmp126;
  const Scalar _tmp176 = _tmp175 * _tmp93;
  const Scalar _tmp177 = std::pow(_tmp165, Scalar(-2));
  const Scalar _tmp178 = _tmp134 * _tmp19;
  const Scalar _tmp179 = _tmp127 * _tmp177;
  const Scalar _tmp180 =
      (_tmp166 * (-_tmp137 - _tmp143 * _tmp178 + _tmp149 * _tmp162) - _tmp168 * _tmp179) /
      std::sqrt(Scalar(std::pow(_tmp168, Scalar(2)) * _tmp177 + 1));
  const Scalar _tmp181 =
      _tmp102 * _tmp111 * _tmp96 + _tmp116 * _tmp119 * _tmp96 + _tmp120 * _tmp122 + _tmp164;
  const Scalar _tmp182 = Scalar(1.0) / (_tmp181);
  const Scalar _tmp183 = -_tmp101 * _tmp111 * _tmp133 - _tmp115 * _tmp119 * _tmp133 -
                         _tmp122 * _tmp139 + _tmp123 * _tmp136 - _tmp147 * _tmp148 + _tmp167;
  const Scalar _tmp184 = std::asinh(_tmp182 * _tmp183);
  const Scalar _tmp185 = Scalar(1.0) * _tmp184;
  const Scalar _tmp186 = Scalar(9.6622558468725703) * _tmp181;
  const Scalar _tmp187 =
      -_tmp184 * _tmp186 - std::sqrt(Scalar(std::pow(Scalar(-_tmp83 + p_c(0, 0)), Scalar(2)) +
                                            std::pow(Scalar(-_tmp86 + p_c(1, 0)), Scalar(2))));
  const Scalar _tmp188 = Scalar(0.1034955) * _tmp182;
  const Scalar _tmp189 = _tmp187 * _tmp188;
  const Scalar _tmp190 = std::pow(_tmp181, Scalar(-2));
  const Scalar _tmp191 = _tmp126 * _tmp190;
  const Scalar _tmp192 = (_tmp182 * (-_tmp136 - _tmp148 * _tmp162 + _tmp178) + _tmp183 * _tmp191) /
                         std::sqrt(Scalar(std::pow(_tmp183, Scalar(2)) * _tmp190 + 1));

  // Output terms (1)
  Eigen::Matrix<Scalar, 4, 1> _res;

  _res(0, 0) =
      _tmp33 *
      (-_tmp2 * std::cosh(Scalar(1.0) * _tmp1) +
       _tmp2 * std::cosh(Scalar(0.1034955) * _tmp0 *
                         (-_tmp1 * _tmp33 -
                          std::sqrt(Scalar(std::pow(Scalar(-_tmp20 + p_b(0, 0)), Scalar(2)) +
                                           std::pow(Scalar(-_tmp32 + p_b(1, 0)), Scalar(2)))))));
  _res(1, 0) = _tmp154 * (-Scalar(1.0) * _tmp163 * std::cosh(_tmp158) -
                          (-Scalar(0.1034955) * _tmp155 * _tmp161 +
                           _tmp156 * (-_tmp153 * _tmp159 - _tmp154 * _tmp163)) *
                              std::cosh(_tmp157)) +
               _tmp159 * (-std::sinh(_tmp157) - std::sinh(_tmp158));
  _res(2, 0) = _tmp170 * (-Scalar(1.0) * _tmp180 * std::cosh(_tmp174) -
                          (-Scalar(0.1034955) * _tmp171 * _tmp179 +
                           _tmp172 * (-_tmp169 * _tmp176 - _tmp170 * _tmp180)) *
                              std::cosh(_tmp173)) +
               _tmp176 * (-std::sinh(_tmp173) - std::sinh(_tmp174));
  _res(3, 0) = -_tmp175 * (-std::sinh(_tmp185) - std::sinh(_tmp189)) +
               _tmp186 * (-Scalar(1.0) * _tmp192 * std::cosh(_tmp185) -
                          (Scalar(0.1034955) * _tmp187 * _tmp191 +
                           _tmp188 * (_tmp175 * _tmp184 - _tmp186 * _tmp192)) *
                              std::cosh(_tmp189));

  return _res;
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace sym
