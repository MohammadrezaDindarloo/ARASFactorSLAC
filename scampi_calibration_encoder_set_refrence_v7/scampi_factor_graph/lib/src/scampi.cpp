                              ...
                              std::vector<Eigen::Matrix<double, 2, 1>> *static_force_collection_for_approving, gtsam::Values *oprimization_result_LM)
{
    ...
    // std::uniform_real_distribution<double> poistion_noise(-0.003, 0.003);
    // std::uniform_real_distribution<double> orientation_noise(-0.1*M_PI / 180.0, 0.1*M_PI / 180.0);
    std::normal_distribution<double> poistion_noise(0.0, 0.001/3);
    std::normal_distribution<double> orientation_noise(0.0, (0.04 * M_PI / 180.0)/3);

    double translationnoise = 0.012; //in meter 
    double orientationnoise = (1.5 * M_PI / 180.0); // in degree and convert to radian
...
{   
    ...
    for (size_t i = 0; i < static_factor_for_calibration; i++)
    {
        ...
        // std::cout << std::endl << "DR: " << optimization_result.at<gtsam::Rot3>(Symbol('D', i)) << std::endl;
